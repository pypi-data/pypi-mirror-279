    ...
    schema = Schema(
        ...
        ListField(
            ...
            EmbeddedDictField(
                ...
                DictField("logConfiguration", ScalarField("logDriver", optional=True)),
            ),
        ),
        ScalarField("family"),
        ResourceLinkField("taskRoleArn", IAMRoleResourceSpec, value_is_id=True),
        ResourceLinkField("executionRoleArn", IAMRoleResourceSpec, value_is_id=True),
        ...
        for arn in tasks_return.get("taskDefinitionArns", []):
            ...
            for container in task_details["containerDefinitions"]:
                for secret in container["secrets"]:
                    secret["valueFrom"] = ":".join(secret["valueFrom"].split(":")[:7])
            clusters[arn] = task_details
