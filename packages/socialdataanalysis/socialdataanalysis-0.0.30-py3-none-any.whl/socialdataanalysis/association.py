...
def analisar_independencia_variaveis_tabela_contingencia(tabela_contingencia, mostrar_pearson=True, mostrar_continuity=True, mostrar_likelihood=True, mostrar_fisher=True):
    ...
    resultados_df_formated = resultados_df_formated.applymap(lambda x: '{:.3f}'.format(x) if isinstance(x, (int, float)) else x)
...
def calcular_odds_ratio_razao_risco_discrepancia(tabela_contingencia, print_results=False):
    """
    ...
    # Formatação do DataFrame
    results_df_formated = results_df.applymap(lambda x: '{:.3f}'.format(x) if isinstance(x, (int, float)) else x)
...
def calcular_distribuicao_probabilidades_e_decisao_hipotese(tabela_contingencia, alpha):
    ...
    df_formated[['p_value', 'cum_sum']] = df_reset[['p_value', 'cum_sum']].applymap("{:.3f}".format)
...
def complementar_tabela_contingencia_com_analise_estatistica(tabela_contingencia, incluir_residuos=True):
    ...
    # Formatação
    combined_df = combined_df.applymap(lambda x: f"{x:.1f}" if pd.notna(x) else "")
...
def eliminacao_reversa_com_comparacao_llm(dataframe, display_models=True, compare_models=True):
    """
    ...
    while True:
        ...
        if display_models:
            print(f"Passo {step}: Removendo {worst_predictor} com p-valor {max_p_value:.3f}")
        step += 1
    ...
    if compare_models:
        ...
        print(f"AIC Modelo Saturado: {saturated_model.aic:.2f}")
        print(f"AIC Modelo Ajustado: {adjusted_model.aic:.2f}")
        ...
        print("\nTeste de Razão de Verossimilhança (LRT):\n")
        print(f"LR stat: {lr_stat:.3f}, Diferença no Número de Parâmetros: {diff_num_params}, p-value: {p_value:.3f}")
...
def criar_tabela_contingencia_expandida_llm(dados_df, grupos, categorias, model):
    """
...
def realizar_analise_correspondencia(df):
    ...
    results_formated = results.applymap(lambda x: '{:.3f}'.format(x) if isinstance(x, (float)) else x)   
...
def detalhar_resultados_analise_correspondencia(df, n_components=2, focus='row', df_supplement=None):
    ...
    # Formatação do DataFrame
    resultados_df_formated = final_df.applymap(lambda x: '{:.3f}'.format(x) if isinstance(x, (int, float)) else x)
