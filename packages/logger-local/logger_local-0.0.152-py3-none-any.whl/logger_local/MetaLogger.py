...
def log_function(func: callable, logger: Logger) -> callable:
    ...
    def wrapper(*args, **kwargs) -> any:
        ...
        try:
            ...
            if isinstance(result, tuple) and len(return_variables) > 1:
                if len(return_variables) != len(result):
                    MiniLogger.warning(
                        ...
                        f"returned values ({len(result)}) in function {full_function_name}")
            ...
            logger.end(full_function_name, object=result_dict)
        return result
...
def module_wrapper(module_name: str, logger: Logger) -> None:
    ...
    def bar(): return 1
    logger = ...
    module_wrapper(__name__, logger)
    """
    ...
    try:
        if isinstance(node, ast.Name):
            ...
            try:
                ...
                MiniLogger.warning(f"Invalid return statement: {vars[0]}")
            except Exception:
                MiniLogger.warning(f"Invalid return statement: {ast.dump(node)}")
                vars = ("result",)
    except Exception:
        MiniLogger.warning(f"Invalid return statement: {ast.dump(node)}")
        vars = ("result",)
