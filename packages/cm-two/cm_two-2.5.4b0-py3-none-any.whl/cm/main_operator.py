...
class Operator:
    ...
    def __init__(self, root, settings, scale, can, deffaultScreenSize,
        ...
        self.kpp_page = KPP(root, settings, self, can)
        self.kpp_lift_page = KPPLift(root, settings, self, can)
        #########################LAUNCHING##############################
        if gravity:
            self.first_page_after_auth = self.mainPage
        if kpp and not gravity:
    ...
    def get_video_stream_inst(self, cam):
        ...
        inst = start_video_stream(self.root, self.terminal.can,
                                  ...
                                  cam_pass=cam['pw'],
                                  cam_ip=cam_ip,
                                  zoom_callback_func=self.currentPage.cam_zoom_callback,
    ...
    def operate_kpp_status(self, info):
        ...
        if "ожидается проезд" in text.lower():
            ...
            self.kpp_page.arriving_in_progress = False
        self.kpp_page.set_arriving_status(text=text)

    def cad_work(self, info):
        self.cad_activate_time = datetime.datetime.now()
        if ((
                self.current not in ('MainPage', 'ManualGateControl', "KPP",
                                     "KPPLift") or (self.currentPage.blockImgDrawn and not self.currentPage.orupState) or self.terminal.cam_zoom)):
            return
        text = 'ДВИЖЕНИЕ'
    ...
    def operate_gate_changes_command(self, gate_name, status, *args, **kwargs):
        ...
        if gate_name.lower() in (
            ...
            self.terminal.close_exit_gate_operation_start()
                ...
                'exit', "внутренний шлагбаум") and status == 'close':
            self.terminal.close_exit_gate_operation_start()
        elif "кпп шлагбаум" in gate_name.lower() and status == 'open':
            self.kpp_page.open_barrier()
        elif "кпп шлагбаум" in gate_name.lower() and status == 'close':
