...
for data_file in all_data_files:
    ...
    for record in all_data_files[data_file]:
        ...
        if not record.get("slug"):
            print(f"Error: {data_file} {record} does not have a 'slug' key. This page will not be generated.")
            continue

        all_opened_pages[
            os.path.join(ROOT_DIR, data_dir, record.get("slug"), "index.html")
        ] = JINJA2_ENV.from_string(contents)
        all_page_contents[os.path.join(ROOT_DIR, data_dir, record.get("slug"), "index.html")] = loads(
            contents
        )
        collections_to_files[data_dir].append(os.path.join(ROOT_DIR, data_dir, record.get("slug"), "index.html"))
...
sorted_files = toposort_flatten(all_dependencies, sort=True)
...
def recursively_build_page_template_with_front_matter(
    ...
    if front_matter and "layout" in front_matter.metadata:
        ...
        # print(f"Rendering {layout_path} with state keys: {front_matter.metadata.get('page', {}).keys()}")
