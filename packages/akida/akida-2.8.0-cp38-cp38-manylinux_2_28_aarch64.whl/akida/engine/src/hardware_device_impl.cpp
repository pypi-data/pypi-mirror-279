...
void HardwareDeviceImpl::dma_config_read(dma::w32* buffer, uint32_t nb_words,
  ...
  auto input_addr = mem_mgr_.alloc(dma::kConfigNpHeaderByteSize);
  ...
  // write header in DDR
  driver_->write(input_addr, header, dma::kConfigNpHeaderByteSize);

  // tell DMA engine to process descriptor
  dma::process(driver_, dma_config_, descriptor);

  driver_->read(output_addr, buffer, nb_words * sizeof(dma::w32));
...
void HardwareDeviceImpl::read_np_registers(uint32_t* output,
  ...
  dma::format_config_header(header.data(), np, dma::Target::NpRegisters,
                            nb_registers, 0);
  dma_config_read(output, nb_registers, header.data());
  if (has_alloc) {
    free_allocated_buffer(&mem_mgr_, &dma_config_.engine.descriptor_base_addr);
  }
}
