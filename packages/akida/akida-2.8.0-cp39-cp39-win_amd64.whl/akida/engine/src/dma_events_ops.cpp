...
static TensorUniquePtr format_output_potentials(
  ...
  switch (output_format) {
    case OutputFormat::FullyPotentials: {
    ...
    case OutputFormat::ConvHighPotentials: {
    ...
    case OutputFormat::ConvPotentials: {
      read_data = [](int32_t* output, const std::vector<uint32_t>& strides,
                     dma::w32 word1, dma::w32 word2) {
        // get index from coordinate
        std::array<Index, 3> coords;
...
static OutputFormat get_output_format(const ProgramInfo& program_info) {
  switch (program_info.outputs_type()) {
    case fb::IoType_fnp_sparse: {
      return program_info.activation_enabled() ? OutputFormat::FullyActivations
                                               : OutputFormat::FullyPotentials;
    }
    case fb::IoType_cnp_sparse: {
      return program_info.activation_enabled() ? OutputFormat::ConvActivations
                                               : OutputFormat::ConvPotentials;
    }
    case fb::IoType_hrc_sparse: {
      return program_info.activation_enabled()
                 ? OutputFormat::HrcActivations
                 : OutputFormat::ConvHighPotentials;
    }
    case fb::IoType_dense: {
      return program_info.activation_enabled() ? OutputFormat::DenseActivations
                                               : OutputFormat::DensePotentials;
    }
    default:
...
TensorUniquePtr dma_events_read_outputs(HardwareDriver* driver,
  ...
  const auto output_format = get_output_format(program_info);
  ...
  auto format_type = static_cast<FormatType>(get_field(header, FORMAT_TYPE));
  ...
  switch (output_format) {
    case OutputFormat::ConvActivations: {
    ...
    case OutputFormat::DensePotentials: {
      // Create an empty tensor
      TensorUniquePtr dense;
      size_t out_size = 0;

      // Format type 0xe corresponds to FNP short potential dense output format.
      if (format_type == FormatType::dense_fnp_short_potential ||
          format_type == FormatType::dense_cnp_short_potential) {
      ...
      } else if (format_type == FormatType::dense_fnp_activation ||
                 format_type == FormatType::dense_cnp_activation) {
