...
from .common import get_layer, ordinal, get_fields_aux, set_display_field_aux
from .properties import get_layer_crs_aux, get_objectid_field_aux, get_symbology_aux
from .properties import get_pop_up, update_pop_up, get_layer_geom_type_aux
...
def clone_layer(og_gis, og_id, new_gis, new_name=None, copy_data=True):
    """
    ...
    if end > len(layer_content):
        ...
        #TODO: Rename Long Fields
        for i in range(end, len(layer_content), BATCH_AMOUNT):
            end = i + BATCH_AMOUNT
...
def add_to_layer_aux(layer, gdf):
    """
...
def add_to_layer(gdf, gis, layer_id):
    ...
    return add_to_layer_aux(layer, gdf)
