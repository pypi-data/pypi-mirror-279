...
class ImageAnalyzerEngine:
    ...
    def analyze(
        ...
        bboxes += self._retry_failed_bboxes(
            text, analyzer_result, bboxes, ocr_result, allow_list
        )

        return bboxes

    def _retry_failed_bboxes(
        self, text, analyzer_result, bboxes, ocr_result, allow_list
    ):
        analyzer_texts = [text[r.start : r.end] for r in analyzer_result]
        bboxes_texts = [text[r.start : r.end] for r in bboxes]
        if set(analyzer_texts) != set(bboxes_texts):
            failed_words = set(analyzer_texts) - set(bboxes_texts)
            print("Failed bbox words: ", failed_words)
            # Finding indices of white spaces and creating new RecognizerResult objects
            failed_words_as_recognizer_results = []
            for word in failed_words:
                original_result = next(
                    r for r in analyzer_result if text[r.start : r.end] == word
                )
                start_index = original_result.start
                end_index = original_result.end

                # Find all white spaces
                matches = [(m.start(), m.end()) for m in re.finditer(r"\s", word)]

                # Filter out matches where the characters between start and end are all digits
                filtered_matches = [
                    (start, end)
                    for start, end in matches
                    if not re.fullmatch(r"\d+", word[start:end])
                ]

                current_start = start_index
                for punct_start, punct_end in filtered_matches:
                    failed_words_as_recognizer_results.append(
                        RecognizerResult(
                            entity_type=original_result.entity_type,
                            start=current_start,
                            end=start_index + punct_start,
                            score=1,
                        )
                    )
                    current_start = start_index + punct_end

                # Add the remaining part after the last whitespace
                failed_words_as_recognizer_results.append(
                    RecognizerResult(
                        entity_type=original_result.entity_type,
                        start=current_start,
            ...
            return self.map_analyzer_results_to_bounding_boxes(
                failed_words_as_recognizer_results, ocr_result, text, allow_list
            )
        ...
        for index, word in iter_ocr:
            if not word:
                ...
                for element in text_analyzer_results:
                    ...
                    word_normalized = re.sub(r"[\s\W_]+", "", word.lower())
                    text_normalized = re.sub(r"[\s\W_]+", "", text_element.lower())
                    # check position and text of ocr word matches recognized entity
                    if (
                        max(pos, element.start) < min(element.end, pos + len(word))
                    ) and (
                        (text_normalized in word_normalized)
                        or (word_normalized in text_normalized)
                    ):
