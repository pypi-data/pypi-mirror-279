...
class _SyncProtocol(Protocol):
    def __call__(
        ...
        *args: Any,
        callback_timeout: Optional[numbers.Number],
        **kwargs: Any,
    ) -> _T: ...
...
class Cloud(Generic[IsAsynchronous]):
    ...
    def close(self: CloudSyncAsync) -> Optional[Awaitable[None]]:
    ...
    async def _fetch_software_build(self, senv_build_id: int, workspace: Optional[str] = None):
        workspace = workspace or self.default_workspace
    ...
    async def _create_software_environment_v2(
        self,
        senv: Optional[SoftwareEnvSpec] = None,
        container: Optional[str] = None,
        workspace: Optional[str] = None,
        name: Optional[str] = None,
    ...
    @track_context
    async def _list_api_tokens_page(self, page: int) -> Tuple[Dict[str, dict], Optional[str]]:
        response = await self._do_request(
            "GET",
            self.server + "/api/v1/api-tokens/",
            params={"page": page},
    ...
    @track_context
    async def _revoke_api_token(self, *, identifier: Optional[str] = None, label: Optional[str] = None) -> None:
    ...
    async def _depaginate(
        func: Callable[..., Awaitable[Tuple[dict, Optional[Union[bool, str]]]]],
        *args,
        **kwargs,
    ) -> dict:
        results_all = {}
    ...
    @track_context
    async def _list_software_environments(self, workspace: Optional[str] = None) -> dict:
        return await self._depaginate_v2(self._list_software_environments_page_v2, workspace=workspace)

    @track_context
    async def _list_software_environments_page_v2(
        self, offset: int, workspace: Optional[str] = None
    ) -> Tuple[dict, int]:
        workspace = workspace or self.default_workspace
        response = await self._do_request(
            "GET",
    ...
    async def _list_software_environments_page(
        self, page: int, workspace: Optional[str] = None
    ...
    async def _list_instance_types(
        ...
        if user_provider and user_provider not in ALLOWED_PROVIDERS:
            raise UnsupportedBackendError(
                (f"Unknown cloud provider provided - {user_provider} is not" f" one of {ALLOWED_PROVIDERS}")
            )
    ...
    async def _get_software_spec(self, pk: str, workspace: Optional[str] = None) -> dict:
        workspace = workspace or self.default_workspace
        response = await self._do_request(
            "GET",
    ...
    @overload
    def list_core_usage(self: Cloud[Async], account: Optional[str] = None) -> Awaitable[dict]: ...

    def list_core_usage(self: CloudSyncAsync, account: Optional[str] = None) -> Union[Awaitable[dict], dict]:
...
@list_docstring
def list_software_environments(account: Optional[str] = None, workspace: Optional[str] = None):
    with Cloud() as cloud:
...
@delete_docstring
def delete_software_environment(name, account: Optional[str] = None, workspace: Optional[str] = None):
    with Cloud() as cloud:
...
def get_software_info(name: str, account: Optional[str] = None, workspace: Optional[str] = None) -> dict:
    """Retrieve solved spec for a Coiled software environment
