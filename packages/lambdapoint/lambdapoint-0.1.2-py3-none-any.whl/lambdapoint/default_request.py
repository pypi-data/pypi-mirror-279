...
class DefaultRequest(ABC):
    ...
    def default_request(
        ...
        if self.execution_type == ExecutionType.API:
            ...
            if method == 'GET':
                response = requests.get(prepared_url.url,headers=headers,allow_redirects=allow_redirects)
            if method == 'DELETE':
                response = requests.delete(prepared_url.url,headers=headers,allow_redirects=allow_redirects)
            elif method == 'POST':
                response = requests.post(prepared_url.url,json=body,headers=headers,allow_redirects=allow_redirects)
            elif method == 'PUT':
                response = requests.put(prepared_url.url,json=body,headers=headers,allow_redirects=allow_redirects)
