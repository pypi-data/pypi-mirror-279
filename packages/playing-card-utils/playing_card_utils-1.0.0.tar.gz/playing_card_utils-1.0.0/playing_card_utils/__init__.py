...
def same_suite(cards: Iterable[Card]) -> bool:
    ...
    """
    return len(set(map(lambda card: card.suite_code, cards))) == 1
...
def is_in_sequence(cards: List[Card]) -> bool:
    ...
    if cards[0].card_type.code == '2' and cards[-1].card_type.code == 'A':
        for i in range(1, len(sorted_cards) - 1):
            if cards[i].card_type.weight - cards[i - 1].card_type.weight != 1:
                return False
        return True
