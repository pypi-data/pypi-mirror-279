...
def add_airflow_integration_key(
    ctx, name: Optional[str] = None, description: Optional[str] = None
):
    config = ctx["config"]
    resource_uuid = AirflowService(config=config).get_airflow_resource_uuid(name=name)
    IntegrationKeyService(config=config).create(
...
@keys.command(
    name="list-airflow", help="List existing Airflow Integration Keys in the account"
)
@click.pass_obj
@click.option(
    "--name",
...
def list_airflow_integration_keys(ctx, name: Optional[str] = None):
    resource_uuid = (
        AirflowService(config=ctx["config"]).get_airflow_resource_uuid(name=name)
        if name
        else None
    )
    IntegrationKeyService(config=ctx["config"]).get_all(
        scope=_AIRFLOW_CALLBACKS_SCOPE, resource_uuid=resource_uuid
    )
...
def delete_airflow_integration_key(ctx, key_id: str):
    IntegrationKeyService(config=ctx["config"]).delete(key_id=key_id)
