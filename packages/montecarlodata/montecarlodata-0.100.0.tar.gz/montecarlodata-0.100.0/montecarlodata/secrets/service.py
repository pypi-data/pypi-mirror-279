...
class AccountSecretsService:
    ...
    def create_secret(
        ...
        secret = response.create_account_secret.secret
    ...
    def get_secret(self, name: str, reveal: bool = False):
        ...
        """
        response = self._pycarlo_client(
            get_account_secret, {"name": name, "reveal": reveal}
        )

        secret = response.get_account_secret
        if reveal:
            ...
            self._print_func(
                f"Expires at: {format_date(secret.expires_at) if secret.expires_at else 'never'}"
            )
            self._print_func(f"Created by: {secret.created_by}")
            self._print_func(f"Created at: {format_datetime(secret.created_at)}")
            self._print_func(f"Last updated by: {secret.updated_by}")
            self._print_func(f"Last updated at: {format_datetime(secret.last_update)}")
    ...
    def delete_secret(self, name: str):
        ...
        deleted = response.delete_account_secret.deleted
        if deleted:
    ...
    def list_secrets(self):
        ...
        secrets = response.get_account_secrets
        table = [
            [
                "Name",
                "Scope",
        ...
        for secret in secrets:
            table.append(
                [
                    ...
                    format_date(secret.expires_at) if secret.expires_at else "never",
                    secret.created_by,
                    format_datetime(secret.created_at),
                    secret.updated_by,
                    format_datetime(secret.last_update),
                ]
            )
        click.echo(
            tabulate(table, headers="firstrow", tablefmt="fancy_grid", maxcolwidths=100)
        )
