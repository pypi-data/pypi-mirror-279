...
DATABRICKS_DATA_COLLECTOR_OPTIONS = [
    ...
    click.option(
        ...
        help="Databricks secret key. This is a secret used by Databricks to communicate back to Monte Carlo.",
        default=DEFAULT_SECRET_NAME,
        show_default=True,
    ),
    click.option(
        "--databricks-secret-scope",
        help="Databricks secret scope. This is a secret used by Databricks to communicate back to Monte Carlo.",
...
RESOURCE_PROFILE_OPTIONS = [
    click.option(
        ...
        "bucket_aws_profile",
        required=True,
        help=f"The AWS profile to use for operations on S3/SNS.",
    )
]
...
@integrations.command(
    help="Setup a Databricks metastore sql warehouse integration. For metadata."
)
@click.pass_obj
@add_common_options(warehouse_create_option())
@add_common_options(DATABRICKS_SQL_WAREHOUSE_OPTIONS)
...
def add_oracle(ctx, database, name, **kwargs):
    ...
    TransactionalOnboardingService(
        ...
        warehouseName=name, dbName=database, dbType=ORACLE_DB_TYPE, **kwargs
    )
...
@integrations.command(
    help=f"Setup an Azure SQL Database integration. {LIGHTWEIGHT_VERBIAGE}."
)
@click.pass_obj
@add_common_options(
    warehouse_create_option(required=True)
...
@click.option(
    "--dc-id",
    help="The data collector UUID that you'd like to run jobs of this system in. This is needed when you have more "
    "than one active data collector.",
    required=False,
    cls=AdvancedOptions,
)
...
@integrations.command(
    help="Test and generate credential key for Confluent Kafka Connection."
)
@click.pass_obj
@click.option(
    "--cluster",
...
@click.option(
    "--dc-id",
    help="Data Collector UUID, if we'd like to test the credentials against the specific dc, or you have multiple DC.",
    required=False,
)
...
@click.option(
    "--url",
    help="Special URL for accessing Kafka Connect API in Confluent Cloud. By default, we use the cloud API URL.",
    required=False,
)
@click.option(
    "--dc-id",
...
@integrations.command(
    help="Test and generate credential key for a MSK Kafka Connect Connection."
)
@click.pass_obj
@click.option(
    "--cluster-arn",
...
@integrations.command(
    help="Test and generate credential key for a Self Hosted Kafka Connection."
)
@click.pass_obj
@click.option(
    "--cluster",
...
@integrations.command(
    help="Test and generate credential key for a Self Hosted Kafka Connect."
)
@click.pass_obj
@click.option(
    "--cluster",
...
@click.option(
    "--connection-type",
    help="Streaming Connection Type to create. Right now only support ['confluent-kafka', 'confluent-kafka-connect', "
...
@click.option(
    ...
    "You can obtain it by calling test-confluent-kafka-credentials for connection type confluent-kafka;"
    "Or calling test-confluent-kafka-connect-credentials for connection type confluent-kafka-connect."
    "It's ok to leave it blank, but fill [url, api-key, secret] field for confluent-kafka connection in this all;"
    "Or fill [api-key, secret, confluent-env] field for confluent-kafka-connect connection.",
    required=False,
    cls=AdvancedOptions,
...
@click.option(
    "--url",
    help="This is required only when the key is not specified and the connection type is confluent-kafka.",
    required=False,
    cls=AdvancedOptions,
...
@click.option(
    "--confluent-env",
    help="This is required only when the key is not specified and the connection type is confluent-kafka-connect.",
    required=False,
    cls=AdvancedOptions,
    mutually_exclusive_options=["key"],
)
...
@click.option(
    "--streaming-system-id",
    help="Streaming system UUID. If we are adding a cluster to an existing streaming system, we should use the UUID here."
    "When this is given, please leave new_streaming_system_name, new_streaming_system_type, dc_id empty.",
    required=False,
    cls=AdvancedOptions,
    mutually_exclusive_options=[
...
@click.option(
    ...
    help="Streaming System Name, if we are creating a new streaming system along the new cluster connection.",
    required=False,
    cls=AdvancedOptions,
...
@click.option(
    ...
    help="Streaming System Type, if we are creating a new streaming system along the new cluster connection.",
    required=False,
    type=click.Choice(["confluent-cloud", "msk", "self-hosted"]),
    cls=AdvancedOptions,
    mutually_exclusive_options=["streaming_system_id"],
...
@click.option(
    "--dc-id",
    help="Data Collector UUID. Only specify when there are more than one data collector in the system, and you are "
...
@click.option(
    "--mc-cluster-id",
    help="Existing Streaming Cluster MC UUID. If we are only adding a connection to a specific cluster, we set this.",
    required=False,
    cls=AdvancedOptions,
    mutually_exclusive_options=[
        "new_cluster_id",
...
@click.option(
    "--new-cluster-name",
    help="New streaming cluster name at MC side. If not specified, will use cluster ID as the cluster name when creating a new cluster.",
    required=False,
    cls=AdvancedOptions,
    mutually_exclusive_options=["mc_cluster_id"],
)
...
@click.pass_obj
@click.option(
    "--environment", help="Pinecone environment (e.g. us-east-1-aws).", required=True
)
