...
class ConfigManager:
    ...
    def __init__(
        ...
        self._profile_config_file = os.path.join(
            self._base_path, settings.PROFILE_FILE_NAME
        )
    ...
    def write(self, **kwargs) -> None:
        ...
        for k, v in kwargs.items():
            if k == self.MCD_TOKEN and len(v) != self.TOKEN_LENGTH:
                raise ValueError(
                    f"{self.MCD_TOKEN} received should have {self.TOKEN_LENGTH} length but received {len(v)}"
                )
    ...
    def read(self) -> Optional[Config]:
        ...
        try:
            ...
            click.echo(
                f"Failed to find configuration for '{self._profile_name}'. Please setup using 'configure' first"
            )
