...
@agents.command(
    name="get-operation-logs", help="Returns the operation logs for a remote agent."
)
@click.pass_obj
@click.option("--agent-id", help="UUID of Agent.", required=True)
@click.option(
...
@agents.command(
    ...
    help="Displays the current CloudFormation template parameters in use by an AWS Agent. For Terraform it displays "
    "the current value for MemorySize and ConcurrentExecutions.",
)
@click.pass_obj
@click.option("--agent-id", help="UUID of Agent.", required=True)
