...
class PlatformService:
    ...
    def caas_migration_test(self, dc_id: Optional[UUID]):
        ...
        migration_uuid = operation.trigger_platform_migration_test.migration_uuid
        click.echo(
                ...
                [
                    ...
                    (
                        ...
                        if agent
                            ...
                            dc.stack_arn if dc.deployment_type == "REMOTE_V1" else "N/A"
                        )
                    ),
                ]
            )
    ...
    def _echo_caas_migration_test_status(cls, output: Dict):
        ...
        if tested_agent:
            ...
            if tested_agent.agent_type == "REMOTE_AGENT":
                return
        table: List[List[str]] = []
        for connection in result.tested_connections:
            table.append(
                [
                    connection.name,
                    connection.uuid,
                    ...
                    (
                        ...
                        else connection.suggested_action or ""
                    ),
        ...
        click.echo(
            tabulate(table, headers=headers, tablefmt="fancy_grid", maxcolwidths=widths)
        )
    ...
    def _echo_caas_migration_agent_result(tested_agent: MigrationTestedAgent):
        table = [
            [
                ...
                (
                    "None"
                    if tested_agent.success
                    else tested_agent.suggested_action or ""
                ),
    ...
    def _get_caas_migration_test_status(
        self, dc_id: Optional[UUID], migration_uuid: UUID
    ) -> Dict:
        operation = self._mc_client(
            QUERY_CAAS_MIGRATION_TEST_STATUS,
            variables={
        ...
        migration_output = operation.get_platform_migration_status.output
        if (
            not migration_output
            or migration_output.get("status") != "dry_run_completed"
        ):
    ...
    def _get_active_data_collectors(self) -> List[Box]:
        ...
        data_collectors = operation.get_user.account.data_collectors
        return list(filter(lambda dc: dc.active, data_collectors))
