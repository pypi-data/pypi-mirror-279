...
class SecretsTest(TestCase):
    ...
    def test_delete_secret(self):
        ...
        runner = CliRunner()
        result = runner.invoke(
            delete_secret, obj={"config": ctx}, args=["--name", "a-secret"]
        )

        self.assertEqual(result.output, f"Deleted secret: 'a-secret'.\n")
        self.assertEqual(
            json.loads(responses.calls[0].request.body.decode("utf8"))["variables"],
            {"name": "a-secret"},
        )
    ...
    def test_delete_secret_not_found(self):
        ...
        self.assertEqual(result.output, f"Secret 'a-secret' was not found.\n")
        self.assertEqual(
            json.loads(responses.calls[0].request.body.decode("utf8"))["variables"],
            {"name": "a-secret"},
        )
