...
class GqlUtilTest(TestCase):
    def setUp(self) -> None:
        self._service = GqlWrapper(_SAMPLE_CONFIG)
    ...
    def test_make_request(self, request_mock):
        self.assertEqual(
            self._service.make_request(_SAMPLE_QUERY, _SAMPLE_VARIABLES),
    ...
    def test_make_request_with_user_id(self, request_mock):
        ...
        request_mock.assert_called_once_with(
            ...
            payload={"query": _SAMPLE_QUERY, "variables": _SAMPLE_VARIABLES},
        )
    ...
    def test_make_request_v2(self, request_mock):
        self.assertEqual(
            self._service.make_request_v2(
                _SAMPLE_QUERY, _SAMPLE_GQL_OP_NAME, _SAMPLE_GQL_RAW_VARS
            ),
            MonolithResponse(data=_SAMPLE_GQL_DATA, errors=None),
        )
        request_mock.assert_called_once_with(
            query=_SAMPLE_QUERY, variables=_SAMPLE_GQL_TRANSFORMED_VARS
        )
    ...
    def test_convert_snakes_to_camels(self):
        ...
        self.assertDictEqual(
            self._service.convert_snakes_to_camels(snakes), expected_camel
        )

    @staticmethod
    def _build_request(query: Optional[str] = None, variables: Optional[Dict] = None):
        return {
            "headers": {
                "x-mcd-id": _SAMPLE_ID,
            ...
            "payload": {
                "query": query or _SAMPLE_QUERY,
                "variables": variables or _SAMPLE_VARIABLES,
            },
        }
