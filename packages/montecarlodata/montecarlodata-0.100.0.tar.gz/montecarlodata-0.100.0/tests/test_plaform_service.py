...
_DC_CLOUD_V2 = Box(
    uuid=str(uuid.uuid4()), deployment_type="CLOUD_V2", active=True, agents=[]
)
_DC_WITH_AGENT = Box(
    uuid=str(uuid.uuid4()),
...
class PlatformServiceTest(TestCase):
    def setUp(self) -> None:
        ...
        self._service = PlatformService(mc_client=self._mc_client_mock)
    ...
    def test_trigger_migration_test_no_dcs(self, mock_click_echo):
        ...
        with self.assertRaises(click.Abort):
            self._service.caas_migration_test(None)
        mock_click_echo.assert_called_once_with(
            "There are no active services in this account."
        )
