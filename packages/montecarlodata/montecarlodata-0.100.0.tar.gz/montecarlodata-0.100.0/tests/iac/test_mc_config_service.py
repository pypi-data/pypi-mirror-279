...
class MonteCarloConfigServiceTest(TestCase):
    ...
    def test_namespace_override_no_default_namesapce(
        self, project_dir_name, config_file_name
    ):
        ...
        self.assertTrue(
            f"tests/iac/test_resources/{project_dir_name}/monitors_other.yml"
            in file_name
        )
        self.assertTrue(
            ...
            json.loads(json.dumps(template)),
            {
                "test": {
                    "custom_sql": [
                        {
                            ...
                            "sampling_sql": "select * from "
                            "analytics.prod.customer_360",
                    ...
                    "field_health": [
                        ...
                        {
                            "fields": ["account_id"],
                            "name": "a analytics:prod.client_hub or "
                            "analytics:prod.def",
                            "table": "analytics:prod.client_hub",
                            "yaml_file_name": f"{project_dir}/dir1/dir2/monitors.yml",
                            "yaml_line": 3,
    ...
    def test_misconfigured_warnings(self):
        ...
        responses = self.service.apply(
            namespace, abort_on_error=False, skip_confirmation=True
        )
...
""",
    ...
    def test_convert_to_mac(self, getcwd):
        ...
        self.assertEqual(
            json.loads(json.dumps(template)), {"test-namespace": {"foo": "bar"}}
        )

    @patch("os.getcwd")
    @responses.activate
    def test_convert_to_mac_with_monitors_file(self, getcwd):
        convert_response = responses.post(
            _SAMPLE_CONFIG.mcd_api_endpoint,
            json={
                "data": {
                    "convertUiMonitorsToConfigTemplate": {
        ...
        dir = self._get_temp_dir("convert_to_mac")
        monitors_file = os.path.join(
            self._get_project_dir("convert_to_mac"), "monitors_uuids"
        )
        shutil.rmtree(dir, ignore_errors=True)
