...
_SAMPLE_TEMPLATE_URI = (
    "https://s3.amazonaws.com/bucket/public/customer_templates/id.json"
)
_SAMPLE_INVALID_LAUNCH_URL = (
    f"https://test.com/cf/home?"
    f"region=us-east-1#/stacks/create/review?stackName=monte-carlo&malformed"
)
...
_SAMPLE_EXTRACTED_COLLECTOR_RESPONSE = _SAMPLE_COLLECTOR_RESPONSE[
    "generateCollectorTemplate"
]
...
class CollectorManagementTest(TestCase):
    ...
    def test_upgrade(self, mock_uuid):
        ...
        self._request_wrapper_mock.make_request_v2.return_value = (
            _SAMPLE_MONOLITH_RESPONSE
        )
        ...
        self._aws_wrapper_mock.deploy_gateway.assert_called_once_with(
            gateway_id=_SAMPLE_GATEWAY_ID
        )
    ...
    def test_deploy(self):
        ...
        monolith_response = MonolithResponse(
            data=collector_response["generateCollectorTemplate"]
        )
    ...
    def test_only_existing_params(self):
        ...
        self.assertEqual(
            parameters, [{"ParameterKey": "foo", "UsePreviousValue": True}]
        )

    def test_completely_new_params(self):
        parameters = self._service._build_param_list(
            existing_params=[], new_params={"foo": "42"}
        )
    ...
    def test_add_collector(
        self, click_mock, prompt_connection_mock, launch_quick_create_link_mock
    ):
        ...
        self._request_wrapper_mock.make_request_v2.assert_called_once_with(
            query=ADD_COLLECTOR_RECORD, operation="createCollectorRecord"
        )
        click_mock.echo.assert_called_once_with(
            "Created collector record with ID '1234'"
        )
