...
SAMPLE_GET_TABLES = {
    "TableList": [
        {"StorageDescriptor": {"Location": f"s3://{SAMPLE_BUCKET}{SAMPLE_PATH}"}}
    ],
    "NextToken": None,
}
...
class PolicyDiscoveryTest(TestCase):
    ...
    def test_generate_cf_role(self, render_mock, read_mock, generate_mock, click_mock):
        ...
        files, collector_props = ["a", "b", "c"], Box(
            {"stackArn": f"arn:partition:service:region:{account}:resource"}
        )
    ...
    @patch("montecarlodata.discovery.policy_gen.click")
    @patch("montecarlodata.discovery.policy_gen.render_dumped_json")
    def test_generate_athena_policy(
        self, render_mock, click_mock, common_mock, workgroup_mock
    ):
    ...
    def test_get_common_glue_athena_props(self, bucket_mock):
        ...
        collector_props, dc_resource_props = Box({"foo": "bar"}), copy.deepcopy(
            SAMPLE_DC_PROPS
        )
    ...
    def test_get_common_glue_athena_props_with_no_buckets(self, bucket_mock):
        ...
        self.assertEqual(
            self._service._get_common_glue_athena_props(
                database_names=database_names, dc_id=dc_id
            ),
            AwsGlueAthenaResourceProperties(
                database_names={"*"},
                bucket_names=bucket_names,
    ...
    def test_handle_wildcards(self):
        ...
        self.assertEqual(
            self._service._handle_wildcards(["foo", "foo", "foo"]), {"foo"}
        )
    ...
    def test_get_workgroup_results(self, click_mock):
        ...
        dc_resource_props.resources_client.get_athena_workgroup.return_value = (
            workgroup_details
        )
        self.assertEqual(
