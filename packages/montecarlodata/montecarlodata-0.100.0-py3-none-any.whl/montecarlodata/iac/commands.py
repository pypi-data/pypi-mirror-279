...
@click.option(
    "--namespace", required=False, help="List only monitors in this namespace"
)
@click.pass_obj
def list_monitors(ctx, monitor_type, limit, namespace):
    ...
    MonitorService(client=create_mc_client(ctx)).list_monitors(
        limit, namespaces, monitor_types
    )
...
@click.option(
    ...
    show_default=True,
    is_flag=True,
    help="Dry run (only export the monitors as monitors as code, but do not convert existing ones).",
)
@click.pass_obj
...
@monitors.command(
    help="Convert monitors from a monitors as code namespace to UI monitors."
)
@click.option(
    "--namespace",
    required=True,
