...
class AirflowService:
    ...
    def get_airflow_resource_uuid(self, name: Optional[str] = None) -> Optional[str]:
        ...
        airflow_containers = [
            container
            for container in etl_containers
            if self._is_airflow_container(container, name)
        ]
        if not airflow_containers:
            ...
            if name:
                complain_and_abort(
                    f"Multiple Airflow connections found with name {name}"
                )
            else:
                complain_and_abort(
                    "Multiple Airflow connections found, use --name to disambiguate"
                )
            return None
    ...
    def _is_airflow_container(container: Dict, name: Optional[str]) -> bool:
        ...
        return any(
            connection for connection in connections if connection["type"] == "AIRFLOW"
        )
