...
class MonitorService:
    ...
    MORE_MONITOR_MESSAGE = (
        "There are more monitors available. Increase the limit to view them."
    )
    ...
    @manage_errors
    def list_monitors(
        self, limit=100, namespaces: List[str] = None, monitor_types: List[str] = None
    ):
        """
        ...
        monitors = [
            (
                ...
                item.monitor_type,
                item.namespace,
                textwrap.fill(item.description, width=70) if item.description else "",
        ...
        self._print_func(
            tabulate(monitors, headers=self.MONITORS_HEADERS, tablefmt="fancy_grid")
        )
