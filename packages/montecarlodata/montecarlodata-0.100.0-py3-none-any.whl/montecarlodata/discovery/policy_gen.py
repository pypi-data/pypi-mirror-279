...
class PolicyDiscoveryService:
    ...
    def generate_cf_role(
        ...
        """
        click.echo(
            "Generating CloudFormation template for creating a role resource.", err=True
        )
    ...
    def _get_common_glue_athena_props(
        ...
        **kwargs
        ...
        if not bucket_names:
            bucket_names = self._get_buckets(
    ...
    def _get_workgroup_results(
        ...
        with click.progressbar(
            ...
            url = urlparse(
                workgroup_details.configuration.result_configuration.output_location
            )
            progress.update(1)
    ...
    def _get_table_locations(
        tables: Box, valid_scheme_prefix: Optional[str] = "s3"
    ) -> Iterator[str]:
        """
