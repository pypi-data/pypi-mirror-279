...
@collectors.command(
    help=f"Add a collector record to the account. {ADD_DC_VERBIAGE}", name="add"
)
@click.pass_obj
@click.option(
    "--no-prompt",
...
def add_collector(ctx, no_prompt):
    ...
    ).add_collector(no_prompt=no_prompt)
...
@collectors.command(
    help="Runs all validations for all active integrations in a given collector."
)
@click.pass_obj
@click.option(
    ...
    type=click.BOOL,
    default=False,
    help="Whether all validations or only those marked as 'periodic' will be executed. Running only "
    "periodic validations will require less time and run the same validations that "
