...
class SubsidiaryTrialBalance(models.Model):
    _name = "subsidiary_trial_balance"
    _inherit = [
    ...
    _description = "Subsidiary Trial Balance"
    ...
    type_id = fields.Many2one(
        related="ws_id.type_id",
        store=True,
    )
    date_start = fields.Date(
        related="ws_id.date_start",
        store=True,
    )
    date_end = fields.Date(
        related="ws_id.date_end",
        store=True,
    )
    ws_id = fields.Many2one(
        string="# Consolidation Worksheet",
    ...
    def _get_policy_field(self):
        res = super(SubsidiaryTrialBalance, self)._get_policy_field()
        policy_field = [
            "confirm_ok",
            "open_ok",
            "approve_ok",
