...
class TimeseriesSearchBuilder:
    ...
    def execute(self):
        url = f'{client.url}/north/v80/timeseries/provision/organizations/{self.organization_name}/{self.identifier}/data?utc={self.utc}'
        response = requests.post(url, headers=client.headers, json=self.body_data, verify=False, timeout=3000)
