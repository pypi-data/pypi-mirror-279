...
from ts_sdk.task.__util_decorators import return_on_failure
...
@retry(
    ...
    retry=retry_if_not_result(lambda v: v is True),
...
def update_task_status(task, result):
    try:
        ...
        response = requests.post(update_url, json=result, verify=False)
        if response.status_code >= 400:
...
def extend_task_timeout(task):
    try:
        ...
        response = requests.post(extend_timeout_url, json={}, verify=False)
        if response.status_code >= 400:
