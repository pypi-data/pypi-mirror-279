...
class GRSBA:
    def __init__(self, max_depth=3, units=16, activation='relu', epochs=100, batch_size=32, patience=10, optimizer='adam', loss='mse', verbose=1, num_numbers=6, num_features=6, t=None):
    ...
    def custom_output_layer(self, inputs, depth=0):
        if depth >= self.max_depth:
            return K.zeros_like(inputs[:, :self.num_numbers])  # Change here to match the output size
        ...
        term_limit = K.pow(a + epsilon, -1)  # Add epsilon to prevent division by zero

        result = K.pow((term1 + term2) * term_limit, self.t)  # Use the generic t
    ...
    def predict(self, input_data):
        ...
        predicted_numbers = np.argsort(predictions, axis=1)[:, -self.num_numbers:]  # Seleciona os 'num_numbers' números com maior probabilidade
