...
def db_migrate_data(errors: list[str] | None,
    ...
    For Oracle dstabases, this only applies to *GENERATED ALWAYS* identity columns, not to
    the ones *GENERATED BY DEFAULT*. The latter should either be omitted from *source_columns*,
    or not flagged altogether.
    ...
    # remove identity column on Oracle
    if identity_column and target_engine == "oracle":
        source_columns.remove(identity_column)
    ...
    # enable indentity column insert on SQLServer
    if identity_column and target_engine == "sqlserver":
        from . import db_pomes
        set_stmt: str = f"SET IDENTITY_INSERT {target_table} ON"
        db_pomes.db_execute(errors=op_errors,
                            exc_stmt=set_stmt,
                            engine=target_engine,
    ...
    # disable indentity column insert on SQLServer
    if identity_column and target_engine == "sqlserver":
        from . import db_pomes
        set_stmt: str = f"SET IDENTITY_INSERT {target_table} OFF"
        db_pomes.db_execute(errors=op_errors,
                            exc_stmt=set_stmt,
                            engine=target_engine,
                            connection=curr_target_conn,
