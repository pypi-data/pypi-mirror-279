...
<body>
    <div class="container">
        ...
        <div class="choose-address">
            ...
            <div id="choose-address-id" class="hint">выбери / вставь адрес</div>
        </div>
        <div class="jetton">
            ...
            <input type="text" id="input-field-social" class="input-field" placeholder="https: инфо-ресурсы"
                maxlength="512">
            ...
            <div href="#" id="gen-rand" class="hint">сгенерировать креативные значения</div>
        </div>
        ...
        <div class="edit">
            ...
            <div class="both">
                <div id="owner-title" class="hint">Подключенный кошелек:</div>
            ...
            <div class="block-transfer">
                ...
                <input type="text" id="input-comment" class="input-field-second" placeholder="TXT: коммент"
                    maxlength="256">
                <div id="isCipher">
                    <input id="chkCipher" type="checkbox">
                    <label for="chkCipher" class="label">Зашифровать коммент</label>
                </div>
    ...
    <script>
        ...
        document.querySelectorAll('.input-field').forEach(inputField => inputField.style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-bg-color') : '');
        document.querySelectorAll('.input-field-second').forEach(inputField => inputField.style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-bg-color') : '');
        document.getElementById('symbol-dt-id').style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-bg-color') : '';
        ...
        tonConnectUI.onStatusChange(async () => {{
            if (tonConnectUI.connected) {{
                ...
                jettons = await sendRequest('POST', '/ft-download', {{ 'dlAddress': dlAddress, 'owner': tonConnectUI.account.address, 'initData': tg.initData }})
                console.log(jettons)
                ...
                if (dlAddress !== "" && jettons[activeMaster]['admin'] === jettons[activeMaster]['owner'] && jettons[activeMaster]['admin'] !== nullAddress) {{
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            document.getElementById("choose-address-id").textContent = 'выбери / вставь адрес'
            ...
            document.getElementById("input-field-social").placeholder = "https: инфо-ресурсы"
            document.getElementById("hint-field-social").textContent = "ссылки: вебсайт/социальные сети"
            document.getElementById("gen-rand").textContent = "сгенерировать креативные значения"
            document.getElementById("ft-edit").style.display = "none"
            ...
            document.getElementById("owner-title").textContent = 'Подключенный кошелек:'
            ...
            document.getElementById("instruction").textContent = 'инструкция применения жетона'
        ...
        document.getElementById('image-upload').addEventListener('change', async () => {{
            ...
            try {{
                const response = await fetch("/ft-image", {{ method: 'POST', body: fd }})
        ...
        document.getElementById('input-field-image').addEventListener('input', async () => {{
            ...
            if (isCorrect) {{
                ...
                if (inputValue !== "") tg.showAlert("Не корректная ссылка")
            }}
        }})
        ...
        document.getElementById('gen-rand').addEventListener('click', async () => {{
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let res = await sendRequest('POST', '/ft-random', {{ 'initData': tg.initData }})
            console.log(res)
            ...
            let params = await sendRequest('POST', '/ft-params', body)
        ...
        document.getElementById('ft-send').addEventListener('click', async (event) => {{
            ...
            symbol = document.getElementById("input-field-symbol").value.trim()
            decimals = document.getElementById("input-field-decimals").value.trim() ? document.getElementById("input-field-decimals").value.trim() : "9"
            ...
            let res = await sendRequest('POST', '/ft-params', body)
            console.log(res)
            ...
            await sendRequest('POST', '/ft-success', body)
        }})
        document.getElementById('ft-edit').addEventListener('click', async (event) => {{
            ...
            if (activeMaster === "") {{ tg.showAlert("Создай жетон или выбери адрес мастера для редактирования"); return }}
            if (image === "") {{ tg.showAlert("image"); return }}
            if (name === "") {{ tg.showAlert("name"); return }}
            if (symbol === "") {{ tg.showAlert("symbol"); return }}
            if (total === "") {{ tg.showAlert("total"); return }}
            ...
            let res = await sendRequest('POST', '/ft-edit', body)
            console.log(res)
        ...
        document.getElementById('ft-mint').addEventListener('click', async (event) => {{
            ...
            let mint = await sendRequest('POST', '/ft-mint', body)
            console.log(mint)
        ...
        document.getElementById('ft-admin').addEventListener('click', async (event) => {{
            ...
            let admin = await sendRequest('POST', '/ft-admin', body)
            console.log(admin)
        ...
        document.getElementById('ft-burn').addEventListener('click', async (event) => {{
            ...
            let burn = await sendRequest('POST', '/ft-burn', body)
            console.log(burn)
        ...
        document.getElementById('ft-transfer').addEventListener('click', async (event) => {{
            ...
            let transfer = await sendRequest('POST', '/ft-transfer', body)
    ...
    <style>
        ...
        *::after {{
        ...
        #media-number {{
            ...
            cursor: pointer;
            color: var(--tg-theme-text-color);
            -webkit-text-stroke: 0.5px rgba(50, 50, 50, 0.99);
        }}
        ...
        #attribute-add {{
            ...
            color: rgba(140, 150, 160, 0.99);
            color: var(--tg-theme-section-header-text-color);
            border: 1.5px dashed rgba(50, 50, 50, 0.99) !important;
            line-height: 20px;
        }}
...
<body>
    <div class="container">
        ...
        <div class="choose-address">
            <div class="subscribe-datalist">
                <input list="address-datalist-list" id="address-datalist-id" placeholder="UQ: адрес коллекции">
                <datalist id="address-datalist-list">
                    <option value="solo">создать единичный NFT</option>
                    <option value="album">создать памятный альбом</option>
                    <option value="content">сохранить авторский контент</option>
                </datalist>
            </div>
            <div id="choose-address-id" class="hint">выбери / вставь адрес</div>
        </div>
        <div class="collection">
            ...
            <input type="text" id="input-field-image" class="input-field" maxlength="65536"
                placeholder="https: загрузи лого или вставь ссылку/base64">
            <div id="hint-field-image" class="hint">Обложка: &lt; 5 Mb&nbsp;
                <div id="link-jpg" class="hint">.jpg,</div>&nbsp;
                <div id="link-png" class="hint">.png,</div>&nbsp; 
                <div id="link-webp" class="hint">.webp,</div>&nbsp;
                <div id="link-gif" class="hint">.gif,</div>&nbsp;
                <div id="link-svg" class="hint">.svg</div>
            </div>
            
            <input type="text" id="input-field-name" class="input-field" placeholder="Name: название коллекции"
                maxlength="128">
            <div id="hint-field-name" class="hint">1-3 слова с заглавной буквы</div>

            <input type="text" id="input-field-description" class="input-field" placeholder="Desc: описание коллекции"
                maxlength="1024">
            <div id="hint-field-description" class="hint">история в 1 предложении</div>
            ...
            <input type="text" id="input-field-percent" class="input-field" placeholder="5,5%: процент от перепродажи"
                maxlength="8">
            <div id="hint-field-percent" class="hint">роялти: % процент от перепродажи отдельного NFT</div>

            <input type="text" id="input-field-royalty" class="input-field" placeholder="UQ: роялти адрес"
                maxlength="128">
            <div id="hint-field-royalty" class="hint">адрес для отправки % от перепродажи NFT</div>

            <div id="nft-send">Создать коллекцию</div>
            <div id="nft-edit">Редактировать коллекцию</div>
            <div id="gen-rand" class="hint">сгенерировать креативные значения</div>
        </div>
        <div class="nft">
            <div class="upload-media-nft">
                <label id="media-number">1/1</label>
                <a id="media-prev" class="media-control">❮</a>
                <label for="image-upload-nft">
                    <img id="logo-nft" src="photo.svg" alt="logo">
                </label>
                <div id="media-close">×</div>
                <a id="media-next" class="media-control">❯</a>

                <input type="file" id="image-upload-nft" accept=".jpg, .jpeg, .webp, .png, .svg, .gif, .mp4">
            </div>

            <div class="both">
                <div id="nft-title" class="hint">Адрес NFT:</div>
                <div id="nft-address">EQFT....5gHF</div>
            </div>

            <input type="text" id="input-field-nft-image" class="input-field" maxlength="65536"
                placeholder="ссылка на nft или base64">
            <div id="hint-field-nft-image" class="hint">Контент: &lt; 5 Mb&nbsp;
                <div id="link-jpg-nft" class="hint">.jpg,</div>&nbsp;
                <div id="link-png-nft" class="hint">.png,</div>&nbsp; 
                <div id="link-webp-nft" class="hint">.webp,</div>&nbsp;
                <div id="link-gif-nft">.gif,</div>&nbsp;
                <div id="link-svg-nft" class="hint">.svg,</div>&nbsp;
                <div id="link-webm-nft">.webm,</div>&nbsp;
                <div id="link-mp4-nft" class="hint">.mp4</div>&nbsp;
            </div>

            <input type="text" id="input-field-nft-name" class="input-field" placeholder="Name: название NFT"
                maxlength="128">
            <div id="hint-field-nft-name" class="hint">1-3 слова с заглавной буквы</div>

            <input type="text" id="input-field-nft-description" class="input-field" placeholder="Desc: описание NFT"
                maxlength="512">
            <div id="hint-field-nft-description" class="hint">история в 1 предложении</div>

            <div id="attributes-section">
                <div class="attribute-row">
                    <input type="text" class="attribute-item" placeholder="атрибут" maxlength="128">
                    <div class="thing">:</div>
                    <input type="text" class="attribute-item" placeholder="значение" maxlength="128">
                    <div id="attribute-rem" class="attribute-item">Удалить</div>
                </div>
            </div>
            <div id="attribute-add" class="input-field">Добавить атрибут</div>
            <div id="hint-attribute" class="hint">добавь [атрибут : значение] для каждого NFT</div>

            <div id="nft-mint">Отчеканить NFT</div>
            <div id="gen-rand-nft" class="hint">сгенерировать креативные значения</div>
        </div>
        <div class="copy-media">
            <div class="copy-div">Copied</div>
        </div>
        <div class="sticker">
            <tgs-player id="lottie" autoplay loop mode="normal"
                src="https://raw.githubusercontent.com/fereysitnerya/ferey/main/AnimatedSticker.tgs">
        ...
        <div class="edit">
            <div class="both">
                <div id="total-title" class="hint">Количество NFT:</div>
                <div id="total-text"></div>
            </div>
            <div class="both">
                <div id="admin-title" class="hint">Адрес админа:</div>
            ...
            <div class="both">
                <div id="collection-title" class="hint">Адрес коллекции:</div>
                <div id="collection-text">....</div>
            </div>
            <div class="both">
                <div id="owner-title" class="hint">Подключенный кошелек:</div>
                <div id="owner-text">....</div>
            ...
            <div id="nft-revoke">Отозвать права</div>
            <div class="hint">рекомендуется отозвать прав админа на: отчеканивание новых NFT в коллекции и передачу прав
                владения, а также на редактирование коллекции: ссылка на лого, название, описание, роялти</div>

            <div class="block">
                <input type="text" id="input-admin" class="input-field" placeholder="UQ: адрес нового админа"
                    maxlength="256">
                <div id="nft-admin">Передать права</div>
            </div>
            <div id="hint-admin" class="hint">передать права на редактирование коллекции новому админу</div>

            <div class="block">
                <input type="text" id="input-burn" class="input-field" placeholder="UQ: адрес NFT"
                    maxlength="256">
                <div id="nft-burn">Сжечь</div>
            </div>
            <div id="hint-burn" class="hint">удалить NFT со своего кошелька</div>

            <div class="block-transfer">
                <div class="block-outer">
                    <div class="block-inner">
                        <input type="text" id="input-amount" class="input-field-second"
                            placeholder="UQ: адрес NFT" maxlength="256">
                    </div>
                    <input type="text" id="input-address" class="input-field-second" placeholder="UQ: адрес кошелька"
                        maxlength="256">
                </div>
                <input type="text" id="input-comment" class="input-field-second" placeholder="TXT: коммент"
                    maxlength="256">
                ...
                <div id="nft-transfer">Отправить</div>
                <div id="hint-transfer" class="hint">отправить NFT другому пользователю</div>
                <div id="instruction" class="hint"></div>
                <div id="footer" class="hint"></div>
            </div>
        </div>
    ...
    <script>
        ...
        document.querySelectorAll('.input-field').forEach(inputField => inputField.style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-bg-color') : '');
        document.querySelectorAll('.attribute-item').forEach(inputField => inputField.style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-bg-color') : '');
        document.querySelectorAll('.input-field-second').forEach(inputField => inputField.style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-bg-color') : '');
        ...
        tonConnectUI.onStatusChange(async () => {{
            if (tonConnectUI.connected) {{
                document.getElementById("hint-transfer").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{tonConnectUI.account.address}}`, {{ try_instant_view: true }}) }})
                collections = await sendRequest('POST', '/nft-download', {{ 'dlAddress': dlAddress, 'owner': tonConnectUI.account.address, 'initData': tg.initData }})
                console.log(collections)
                if (!Object.keys(collections).length) {{ return }}
                if (activeCollection === "") {{ activeCollection = Object.keys(collections)[0] }}
                console.log('activeCollection', activeCollection)

                document.getElementById('address-datalist-list').innerHTML = ''
                document.getElementById('address-datalist-list').appendChild(new Option("создать единичный NFT", 'solo'));
                document.getElementById('address-datalist-list').appendChild(new Option("создать памятный альбом", 'album'));
                document.getElementById('address-datalist-list').appendChild(new Option('сохранить авторский контент', 'content'));

                for (const key in collections) {{
                    let collect = collections[key]
                    const option = document.createElement('option')
                    option.value = collect['collection']
                    option.text = collect['name']
                    document.getElementById('address-datalist-list').appendChild(option)
                }}

                if (dlAddress !== "" && collections[activeCollection]['admin'] === collections[activeCollection]['owner'] && collections[activeCollection]['admin'] !== nullAddress) {{
                    document.getElementById('nft-send').style.display = 'none'
                    document.getElementById('nft-edit').style.display = 'flex'
                    document.getElementById('nft-edit').textContent = `Редактировать коллекцию`

                    document.getElementById('nft-mint').style.backgroundColor = 'var(--tg-theme-button-color)'
                    document.getElementById('nft-admin').style.backgroundColor = 'var(--tg-theme-button-color)'
                }}
                else {{
                    document.getElementById('nft-send').style.display = 'flex'
                    document.getElementById('nft-edit').style.display = 'none'
                    
                    if (!images.length) {{
                        console.log('images.length', images.length)
                        document.getElementById('nft-mint').style.backgroundColor = 'var(--tg-theme-subtitle-text-color)'
                        document.getElementById('nft-admin').style.backgroundColor = 'var(--tg-theme-subtitle-text-color)'
                    }}
                }}
            }}
        }})
        document.addEventListener("DOMContentLoaded", async () => {{
            document.getElementById("address-datalist-id").placeholder = "UQ: адрес коллекции"
            document.querySelector('option[value="solo"]').textContent = 'создать единичный NFT'
            document.querySelector('option[value="album"]').textContent = 'создать памятный альбом'
            document.querySelector('option[value="content"]').textContent = 'сохранить авторский контент'
            document.getElementById("choose-address-id").textContent = 'выбери / вставь адрес'
            document.getElementById("input-field-image").placeholder = "https: загрузи лого или вставь ссылку/base64"
            document.getElementById("input-field-name").placeholder = "Name: название коллекции"
            document.getElementById("hint-field-name").textContent = "1-3 слова с заглавной буквы"
            document.getElementById("input-field-description").placeholder = "Desc: описание коллекции"
            document.getElementById("hint-field-description").textContent = "история в 1 предложении"
            document.getElementById("input-field-social").placeholder = "https: инфо-ресурсы"
            document.getElementById("hint-field-social").textContent = "ссылки: вебсайт/социальные сети"
            document.getElementById("input-field-percent").placeholder = "5,5%: процент от перепродажи"
            document.getElementById("hint-field-percent").textContent = "роялти: % процент от перепродажи отдельного NFT"
            document.getElementById("input-field-royalty").placeholder = "UQ: роялти адрес"
            document.getElementById("hint-field-royalty").textContent = "адрес для отправки % от перепродажи NFT"
            document.getElementById("gen-rand").textContent = "сгенерировать креативные значения"
            document.getElementById("gen-rand-nft").textContent = "сгенерировать креативные значения"
            document.getElementById("nft-edit").style.display = "none"

            document.getElementById("link-jpg").addEventListener("click", async () => {{ tg.openLink('https://nft.ton.diamonds/annihilation/annihilation.jpg', {{ try_instant_view: true }}) }})
            document.getElementById("link-png").addEventListener("click", async () => {{ tg.openLink('https://stickerface.io/nfts/collection_1658935519_0/icon.png?ts=1660918851', {{ try_instant_view: true }}) }})
            document.getElementById("link-webp").addEventListener("click", async () => {{ tg.openLink('https://nft.ton.diamonds/octopus-boyz/octopusboyz-logo.webp', {{ try_instant_view: true }}) }})
            document.getElementById("link-svg").addEventListener("click", async () => {{ tg.openLink('https://nft.fragment.com/numbers.svg', {{ try_instant_view: true }}) }})
            document.getElementById("link-jpg-nft").addEventListener("click", async () => {{ tg.openLink('https://nft.ton.diamonds/memory-of-soul/memory-of-soul.jpg', {{ try_instant_view: true }}) }})
            document.getElementById("link-png-nft").addEventListener("click", async () => {{ tg.openLink('https://nft.ton.diamonds/metamorphoses/nft/assets/MSS1532.png', {{ try_instant_view: true }}) }})
            document.getElementById("link-svg-nft").addEventListener("click", async () => {{ tg.openLink('https://stickerface.io/api/nft/svg/5010391?ts=1668710128.svg', {{ try_instant_view: true }}) }})
            document.getElementById("link-webp-nft").addEventListener("click", async () => {{ tg.openLink('https://nft.ton.diamonds/calligrafuturism-24-units/calligrafuturism-24-units-logo.webp', {{ try_instant_view: true }}) }})
            document.getElementById("link-mp4-nft").addEventListener("click", async () => {{ tg.openLink('https://nft.ton.diamonds/memory-of-soul/memory-of-soul.mp4', {{ try_instant_view: true }}) }})

            document.getElementById("total-title").textContent = 'Количество NFT:'
            document.getElementById("admin-title").textContent = 'Адрес админа:'
            document.getElementById("owner-title").textContent = 'Подключенный кошелек:'
            document.getElementById("collection-title").textContent = 'Адрес коллекции:'

            document.getElementById("hint-admin").textContent = 'передать права на редактирование коллекции новому админу'
            document.getElementById("hint-burn").textContent = 'удалить NFT из коллекции со своего кошелька'
            document.getElementById("hint-transfer").textContent = 'отправить NFT другому пользователю'

            document.getElementById("instruction").textContent = 'инструкция по коллекции'
            document.getElementById("instruction").addEventListener("click", async () => {{ tg.openLink('https://telegra.ph/FereyTONestBot-05-20', {{ try_instant_view: true }}) }})
            document.getElementById("footer").textContent = '@FereyTONestBot'
            document.getElementById("footer").addEventListener("click", async () => {{ tg.openLink('https://t.me/FereyTONestBot', {{ try_instant_view: true }}) }})
            
            let tmp = await sendRequest('POST', '/nft-images', {{ 'dlAddress': dlAddress, 'initData': tg.initData }})
            images = Object.values(tmp)
            await fillImage()
        }})
        async function fillImage() {{
            ...
            let currentIndex = 0
            document.getElementById('logo').src = images[currentIndex]
            document.getElementById("input-field-image").value = images[currentIndex]
            document.getElementById('logo').style.position = 'absolute'
            document.querySelector('.upload-media').style.border = 'none'
            
            let imageElement = document.getElementById('logo-nft')
            document.getElementById('logo-nft').src = images[currentIndex]
            document.getElementById("input-field-nft-image").value = images[currentIndex]
            document.getElementById('logo-nft').style.position = 'absolute'
            document.querySelector('.upload-media-nft').style.border = 'none'
            ...
            document.getElementById('media-prev').addEventListener('click', () => {{
                ...
                document.getElementById("input-field-nft-image").value = images[currentIndex]
                document.getElementById('media-number').textContent = `${{currentIndex + 1}}/${{images.length}}`
            }})
            ...
            document.getElementById('media-close').addEventListener('click', async () => {{
                images.splice(currentIndex, 1)
                if (currentIndex >= images.length) currentIndex = images.length - 1

                if (images.length === 0) {{
                    imageElement.src = 'photo.svg';
                    imageElement.style.position = 'static';
                    document.querySelector('.upload-media-nft').style.border = '2px dashed rgba(85, 149, 242, 0.99)'
                    document.getElementById('media-number').textContent = `0`;
                    document.getElementById("input-field-nft-image").value = ''
                }} else {{
                    imageElement.src = images[currentIndex];
            ...
            document.getElementById('nft-address').addEventListener('click', async () => {{
                navigator.clipboard.writeText(currentNftAddress)
                document.querySelector('.copy-media').classList.add('window--open')
                await sleep(1000).then(() => {{ document.querySelector('.copy-media').classList.remove('window--open') }})
            }})
        }}
        ...
        document.getElementById('address-datalist-id').addEventListener('change', async () => {{
            ...
            console.log('Выбрано или введено: ' + dlAddress)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            console.log('next: ', dlAddress)
            
            if (dlAddress === "" || /\s/.test(dlAddress)) {{ 
                ...
                document.getElementById("input-field-description").value = ""
                document.getElementById("input-field-social").value = ""
                document.getElementById("input-field-percent").value = ""
                document.getElementById("input-field-royalty").value = tonConnectUI.account.address
                ...
                document.getElementById("total-text").textContent = "134"
                document.getElementById("owner-text").textContent = "мой адрес"
                document.getElementById("admin-text").textContent = "тоже мой"
                document.getElementById("collection-text").textContent = "адрес коллекции"
            ...
            console.log(collections)
            if (!Object.keys(collections).length) return;
            console.log(collections[dlAddress]['name'])
            activeCollection = dlAddress

            let newOption = document.createElement("option")
            newOption.value = dlAddress
            newOption.text = collections[dlAddress]['name']
            document.getElementById("address-datalist-list").appendChild(newOption)
            console.log('end')

            document.getElementById("input-field-image").value = collections[dlAddress]['image']
            document.getElementById("input-field-image").dispatchEvent(new Event('input', {{ bubbles: true }}))
            document.getElementById("input-field-name").value = collections[dlAddress]['name']
            document.getElementById("input-field-description").value = collections[dlAddress]['description']
            document.getElementById("input-field-social").value = collections[dlAddress]['social']
            document.getElementById("input-field-percent").value = collections[dlAddress]['percent']
            document.getElementById("input-field-royalty").value = collections[dlAddress]['royalty']

            document.getElementById("total-text").textContent = collections[activeCollection]['index']
            document.getElementById("owner-text").textContent = collections[activeCollection]['owner_str'].slice(0, 4) + "...." + collections[activeCollection]['owner_str'].slice(-4)
            document.getElementById("admin-text").textContent = collections[activeCollection]['admin_str'].slice(0, 4) + "...." + collections[activeCollection]['admin_str'].slice(-4)
            document.getElementById("collection-text").textContent = collections[activeCollection]['collection_str'].slice(0, 4) + "...." + collections[activeCollection]['collection_str'].slice(-4)

            document.getElementById("total-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{activeCollection}}`, {{ try_instant_view: true }}) }})
            document.getElementById("admin-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{collections[activeCollection]['admin']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("owner-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{collections[activeCollection]['owner']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("collection-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{activeCollection}}`, {{ try_instant_view: true }}) }})

            if (collections[dlAddress]['admin'] === collections[dlAddress]['owner'] && collections[dlAddress]['admin'] !== nullAddress) {{
                document.getElementById('nft-send').style.display = 'none'
                document.getElementById('nft-edit').style.display = 'flex'
                document.getElementById('nft-edit').textContent = `Редактировать коллекцию`
            ...
            document.getElementById('input-burn').placeholder = `UQ: адрес NFT`
            document.getElementById('address-datalist-list').innerHTML = ''
            document.getElementById('address-datalist-list').appendChild(new Option("создать единичный NFT", 'solo'));
            document.getElementById('address-datalist-list').appendChild(new Option("создать памятный альбом", 'album'));
            document.getElementById('address-datalist-list').appendChild(new Option('сохранить авторский контент', 'content'));
        ...
        document.getElementById('image-upload').addEventListener('change', async () => {{
            ...
            try {{
                const response = await fetch("/nft-image", {{ method: 'POST', body: fd }})
                const res = await response.json()
                console.log(res)
        ...
        document.getElementById('collection-text').addEventListener('click', async () => {{
            await navigatorClipboard('collection')
        }})
        document.getElementById('gen-rand').addEventListener('click', async () => {{
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let res = await sendRequest('POST', '/nft-random', {{ 'owner': tonConnectUI.account.address, 'type': 'collection', 'initData': tg.initData }})
            console.log(res)
            
            if (document.getElementById("input-field-image").value.trim() === '') {{
                document.getElementById("input-field-image").value = res['image']  
            }}
            document.getElementById("input-field-name").value = res['name']
            document.getElementById("input-field-description").value = res['description']
            document.getElementById("input-field-social").value = res['social']
            document.getElementById("input-field-percent").value = res['percent']
            document.getElementById("input-field-royalty").value = res['owner_str']
            document.getElementById("input-field-image").dispatchEvent(new Event('input', {{ bubbles: true }}))
            ...
            let params = await sendRequest('POST', '/collection-params', body)
            document.getElementById("address-datalist-id").value = params['collection']
            ...
            document.getElementById("owner-text").textContent = params['owner_str'].slice(0, 4) + "...." + params['owner_str'].slice(-4)
            document.getElementById("admin-text").textContent = params['owner_str'].slice(0, 4) + "...." + params['owner_str'].slice(-4)
            document.getElementById("collection-text").textContent = params['collection_str'].slice(0, 4) + "...." + params['collection_str'].slice(-4)

            document.getElementById('nft-send').style.display = 'flex'
            document.getElementById('nft-edit').style.display = 'none'
        }})
        // -----------------------nft-----------------------------
        document.getElementById('attribute-rem').addEventListener('click', async () => {{
            const attributesSection = document.getElementById('attributes-section')
            const attributeRows = attributesSection.getElementsByClassName('attribute-row')
            if (attributeRows.length !== 1) {{ return }}
            document.querySelectorAll('.attribute-item').forEach(async (input) => {{ input.value = '' }})
        }})
        document.getElementById('attribute-add').addEventListener('click', async () => {{
            const attributesSection = document.getElementById('attributes-section')
            const attributeRows = attributesSection.getElementsByClassName('attribute-row')
            if (attributeRows.length >= 10) {{ return }}
            const attributeRow = document.createElement('div');
            attributeRow.className = 'attribute-row';

            const traitInput = document.createElement('input');
            traitInput.type = 'text';
            traitInput.className = 'attribute-item';
            traitInput.placeholder = 'атрибут';
            traitInput.maxLength = 128

            const thingClass = document.createElement('div')
            thingClass.className = 'thing'
            thingClass.textContent = ':'

            const valueInput = document.createElement('input');
            valueInput.type = 'text';
            valueInput.className = 'attribute-item';
            valueInput.placeholder = 'значение';
            valueInput.maxLength = 128

            const removeButton = document.createElement('div')
            removeButton.className = 'attribute-item'
            removeButton.style.cursor = 'pointer';
            removeButton.textContent = 'Удалить'
            removeButton.addEventListener('click', async () => {{ attributesSection.removeChild(attributeRow) }})

            attributeRow.appendChild(traitInput)
            attributeRow.appendChild(thingClass)
            attributeRow.appendChild(valueInput)
            attributeRow.appendChild(removeButton)

            attributesSection.appendChild(attributeRow);
        }})
        document.getElementById('image-upload-nft').addEventListener('change', async () => {{
            console.log('sdfdfdsd')
            let arr = document.getElementById('image-upload-nft').files
            if (!arr.length) {{ return; }}
            let file = arr[0]
            const validExtensions = ['.jpg', '.jpeg', '.png', '.svg', '.webp']
            const fileName = file.name.toLowerCase();
        ...
        document.getElementById('gen-rand-nft').addEventListener('click', async () => {{
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let res = await sendRequest('POST', '/nft-random', {{ 'owner': tonConnectUI.account.address, 'type': 'item', 'initData': tg.initData }})
            console.log(res)
            
            if (document.getElementById("input-field-nft-image").value.trim() === '') {{
                document.getElementById("input-field-nft-image").value = res['image']  
            }}
            document.getElementById("input-field-nft-name").value = res['name']
            document.getElementById("input-field-nft-description").value = res['description']
            document.getElementById("input-field-nft-image").dispatchEvent(new Event('input', {{ bubbles: true }}))
            
            const attributesSection = document.getElementById('attributes-section')
            const existingRows = attributesSection.getElementsByClassName('attribute-row');
            while (existingRows.length > 1) attributesSection.removeChild(existingRows[1])
            
            if (existingRows.length > 0 && res['attributes'].length) {{
                const firstTraitInput = existingRows[0].querySelector('input[placeholder="атрибут"]')
                const firstValueInput = existingRows[0].querySelector('input[placeholder="значение"]')
                
                firstTraitInput.value = res['attributes'][0]['trait_type'];
                firstValueInput.value = res['attributes'][0]['value'];
            }}
    
            for (let i = 1; i < res['attributes'].length; i++) {{
                const attributeRow = document.createElement('div')
                attributeRow.className = 'attribute-row'
                ...
                traitInput.placeholder = 'атрибут';
                traitInput.maxLength = 128;
                traitInput.value = res['attributes'][i]['trait_type']
                ...
                valueInput.placeholder = 'значение';
                valueInput.maxLength = 128;
                valueInput.value = res['attributes'][i]['value']
            ...
            currentNftAddress = res['address']
            console.log('currentNftAddress', currentNftAddress)
            document.getElementById("nft-address").textContent = currentNftAddress.slice(0, 4) + "...." + currentNftAddress.slice(-4)
            ...
            let params = await sendRequest('POST', '/nft-params', body)
            return
            document.getElementById("address-datalist-id").value = params['collection']
        ...
        document.getElementById('input-field-nft-image').addEventListener('input', async () => {{
            let inputValue = document.getElementById('input-field-nft-image').value.trim()
            let isCorrect = (/^(http|https):\/\/[^ "]+$/.test(inputValue)) ? true : false

            if (isCorrect) {{
                document.getElementById("logo-nft").src = inputValue
                document.getElementById("logo-nft").style.position = 'absolute'
                document.getElementById("input-field-nft-image").value = document.getElementById("logo-nft").src
                document.getElementById('image-upload-nft').style.border = 'none'
            }}
            else {{
                document.getElementById("logo-nft").src = 'photo.svg'
                document.getElementById("logo-nft").style.position = 'static'
                document.getElementById("input-field-nft-image").value = ""
                document.getElementById('image-upload-nft').style.border = '2px dashed rgba(85, 149, 242, 0.99)'
                
                if (inputValue !== "") tg.showAlert("Не корректная ссылка")
            }}
        }})
        // -----------------------buttons-----------------------------
        document.getElementById('nft-send').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let image = document.getElementById("input-field-image").value.trim()
            let name = document.getElementById("input-field-name").value.trim()
            let description = document.getElementById("input-field-description").value.trim()
            let social = document.getElementById("input-field-social").value.trim() ? document.getElementById("input-field-social").value.trim() : ""
            let percent = document.getElementById("input-field-percent").value.trim() ? document.getElementById("input-field-percent").value.trim() : "0"
            let royalty = document.getElementById("input-field-royalty").value.trim() ? document.getElementById("input-field-royalty").value.trim() : ""
            ...
            let res = await sendRequest('POST', '/collection-params', body)
            console.log(res)

            let message = [{{
                address: res['collection'],
                amount: res['amount'],
                payload: res['payload'],
                stateInit: res['stateInit'],
            }}]
            ...
            let success = await sendRequest('POST', '/nft-success', body)
            console.log(success)
        }})
        document.getElementById('nft-edit').addEventListener('click', async (event) => {{
            console.log(event.target.id)
        }})
        document.getElementById('nft-revoke').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeCollection === "") {{ tg.showAlert("Выбери адрес коллекции, чтобы отозвать права у админа"); return }}
        }})
        document.getElementById('nft-mint').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let dtListAddress = document.getElementById('address-datalist-id').value
            let nftName = document.getElementById('input-field-nft-name').value.trim()
            let nftDescription = document.getElementById('input-field-nft-description').value.trim()
            let nftImage = document.getElementById('input-field-nft-image').value.trim()
            
            let attributes = []
            document.querySelectorAll('.attribute-row').forEach(row => {{
                const traitType = row.querySelector('input:nth-child(1)').value.trim()
                const value = row.querySelector('input:nth-child(3)').value.trim()
                if (!traitType || !value) return;
                attributes.push({{ "trait_type": traitType, "value": value }})
            }})
            console.log('attributes', attributes, dtListAddress)
            
            if (nftName === "") {{ tg.showAlert("nftName"); return }}
            if (nftDescription === "") {{ tg.showAlert("nftDescription"); return }}
            if (nftImage === "") {{ tg.showAlert("nftImage"); return }}
            if (!attributes.length) {{ tg.showAlert("attributes"); return }}
            
            if (dtListAddress === 'solo') {{
                body = {{ 
                    'type': dtListAddress,
                    'owner': tonConnectUI.account.address,
                    'name': nftName,
                ...
                if (activeCollection === "") {{ tg.showAlert("Создай коллекцию или выбери адрес коллекции для отчеканивания еще большего количества NFT"); return }}
                if (collections[activeCollection]['admin'] !== collections[activeCollection]['owner'] || collections[activeCollection]['admin'] === nullAddress) {{ tg.showAlert("👩🏽‍💻 Отсутствуют права на редактирование коллекции"); return }}
                
                body = {{ 
                    'type': dtListAddress,
                    'owner': tonConnectUI.account.address,
                    'collection': collections[activeCollection]['collection'],
                    'wallet': collections[activeCollection]['wallet'],
                    'coins': inputMint.toString(), 
            ...
            let mint = await sendRequest('POST', '/nft-mint', body)
            currentNftAddress = mint['address']
            document.getElementById("nft-address").textContent = currentNftAddress.slice(0, 4) + "...." + currentNftAddress.slice(-4)
            ...
            let message = [{{
                ...
                amount: mint['amount'],
                payload: mint['payload'],
                stateInit: mint['stateInit'],
            }}]

            const result = await tonConnectUI.sendTransaction({{ validUntil: Math.round(Date.now() / 1000) + 5 * 60, messages: message }})
        }})
        document.getElementById('nft-admin').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeCollection === "") {{ tg.showAlert("Создай коллекцию или выбери адрес коллекции для передачи прав другому админу"); return }}
            if (collections[activeCollection]['admin'] !== collections[activeCollection]['owner'] || collections[activeCollection]['admin'] === nullAddress) {{ tg.showAlert("👩🏽‍💻 Отсутствуют права на редактирование коллекции"); return }}
            ...
            let admin = await sendRequest('POST', '/nft-admin', body)
            console.log(admin)
            ...
            const result = await tonConnectUI.sendTransaction({{ validUntil: Math.round(Date.now() / 1000) + 5 * 60, messages: message }})
            console.log('result', result)
            collections[activeCollection]['admin'] = admin['admin']
            collections[activeCollection]['admin_str'] = admin['admin_str']
            document.getElementById("admin-text").textContent = collections[activeCollection]['admin_str'].slice(0, 4) + "...." + collections[activeCollection]['admin_str'].slice(-4)
        }})
        document.getElementById('nft-burn').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let tmp = document.getElementById('address-datalist-id').value.trim()
            if (activeCollection === "" && tmp === "") {{ tg.showAlert("Создай коллекцию или выбери адрес коллекции/кошелька для сжигания NFT"); return }}
            let inputBurn = document.getElementById('input-burn').value.trim()
            if (inputBurn === "") {{ tg.showAlert("Выбери NFT для сжигания"); return }}
            let isWallet = true

            if (Object.keys(collections).length && tmp in collections) {{
                console.log('ключ существует')
                wallet = collections[activeCollection]['wallet']
                isWallet = false
            }}
            ...
            let burn = await sendRequest('POST', '/nft-burn', body)
            console.log(burn)
        ...
        document.getElementById('nft-transfer').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            ...
            let transfer = await sendRequest('POST', '/nft-transfer', body)
            console.log(transfer)
        ...
        async function navigatorClipboard(field) {{
            ...
            if (Object.keys(collections).length) {{
                ...
                navigator.clipboard.writeText(collections[activeCollection][field])
                document.querySelector('.copy-media').classList.add('window--open')
                await sleep(1000).then(() => {{ document.querySelector('.copy-media').classList.remove('window--open') }})
            }}
        }}
...
async def db_select(sql, param=None, db=None):
    ...
    while retry > 0:
        try:
            ...
            if 'no such column' in str(e): return
...
async def db_change(sql, param=None, db=None):
    ...
    while retry > 0:
        try:
            ...
            logger.info(log_ % f"{os.path.basename(db)}: {str(e)}")
            if 'binding' in str(e): return
            await asyncio.sleep(round(random.uniform(1, 2), 2))
            await db_change("VACUUM", (), db)
            retry -= 1
    return -1
