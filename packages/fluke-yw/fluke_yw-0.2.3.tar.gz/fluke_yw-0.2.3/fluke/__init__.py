...
from fluke.sites import AdminSite, site
...
def autodiscover():
    ...
    from fluke.views import register_builtin_views
    register_builtin_views(site)

    # load fluke settings from FLUKE_CONF module
    try:
        fluke_conf = getattr(settings, 'FLUKE_CONF', 'fluke_conf.py')
        conf_mod = import_module(fluke_conf)
    except Exception:
        conf_mod = None
    ...
    from fluke.plugins import register_builtin_plugins
    ...
    for app_config in apps.get_app_configs():
        ...
        try:
            ...
            import_module('%s.fluke' % app_config.name)
        except:
            ...
            if module_has_submodule(mod, 'fluke'):
                raise

default_app_config = 'fluke.apps.FlukeConfig'
