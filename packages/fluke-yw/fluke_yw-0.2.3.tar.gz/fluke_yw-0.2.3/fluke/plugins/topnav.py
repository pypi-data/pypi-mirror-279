...
from fluke.filters import SEARCH_VAR
...
class TopNavPlugin(BaseAdminPlugin):
                ...
                if getattr(self.admin_site._registry[model], 'search_fields', None):
                    try:
                        search_models.append({
                            ...
                            'url': reverse('fluke:%s_%s_changelist' % info, current_app=site_name),
                            'model': model
                        })
                    except NoReverseMatch:
                        pass
        return nodes.append(loader.render_to_string('fluke/blocks/comm.top.topnav.html', {'search_models': search_models, 'search_name': SEARCH_VAR}))
    ...
    def block_top_navmenu(self, context, nodes):
            ...
            if self.has_model_perm(model, "add"):
                ...
                try:
                    add_models.append({
                        ...
                        'url': reverse('fluke:%s_%s_add' % info, current_app=site_name),
                        'model': model
                    })
                except NoReverseMatch:
                    pass
        ...
        nodes.append(
            loader.render_to_string('fluke/blocks/comm.top.topnav.html', {'add_models': add_models}))
