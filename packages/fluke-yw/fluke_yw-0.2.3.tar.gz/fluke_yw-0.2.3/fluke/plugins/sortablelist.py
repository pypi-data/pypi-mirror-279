...
class SortableListPlugin(BaseAdminPlugin):
    ...
    def block_top_toolbar(self, context, nodes):
        save_node = render_to_string(
            'fluke/blocks/model_list.top_toolbar.saveorder.html', context_instance=context
        )
        nodes.append(save_node)

    def get_media(self, media):
        if self.is_list_sortable:
            media = media + self.vendor('fluke.plugin.sortablelist.js')
        return media
