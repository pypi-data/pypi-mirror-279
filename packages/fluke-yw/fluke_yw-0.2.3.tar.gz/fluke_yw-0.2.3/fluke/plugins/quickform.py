...
from fluke.util import get_model_from_relation, vendor
from fluke.views import BaseAdminPlugin, ModelFormAdminView
from fluke.layout import Layout
...
class QuickFormPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        if self.request.method == 'GET' and self.request.is_ajax() or self.request.GET.get('_ajax'):
            self.admin_view.add_form_template = 'fluke/views/quick_form.html'
            self.admin_view.change_form_template = 'fluke/views/quick_form.html'
            return True
        return False
...
class RelatedFieldWidgetWrapper(forms.Widget):
    ...
    @property
    def media(self):
        media = self.widget.media + vendor('fluke.plugin.quick-form.js')
        return media
