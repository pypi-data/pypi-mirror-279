...
from fluke.filters import FILTER_PREFIX, SEARCH_VAR
from fluke.plugins.relate import RELATE_PREFIX
from fluke.plugins.utils import get_context_dict
from fluke.sites import site
from fluke.views import ModelAdminView, BaseAdminPlugin, ListAdminView
from fluke.views.list import COL_LIST_VAR, ORDER_VAR
from fluke.views.dashboard import widget_manager, BaseWidget, PartialBaseWidget

from fluke.models import Bookmark
...
class BookmarkPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        ...
        list_base_url = reverse('fluke:%s_%s_changelist' %
        ...
        bookmarks_queryset = Bookmark.objects.filter(
            ...
            url_name='fluke:%s_%s_changelist' % model_info
        ...
        for bk in bookmarks_queryset:
            ...
            bookmarks.append({'title': bk.title, 'selected': selected, 'url': bk.url, 'edit_url':
                              reverse('fluke:%s_%s_%s' % (bk_model_info[0], bk_model_info[1], change_or_detail),
                                      args=(bk.id,))})
            if selected:
                menu_title = bk.title
                has_selected = True

        post_url = reverse('fluke:%s_%s_bookmark' % model_info,
        ...
        new_context = {
            ...
            'has_add_permission_bookmark': self.admin_view.request.user.has_perm('fluke.add_bookmark'),
            'has_change_permission_bookmark': self.admin_view.request.user.has_perm('fluke.change_bookmark')
        }
    ...
    def get_media(self, media):
        return media + self.vendor('fluke.plugin.bookmark.js')

    # Block Views
    def block_nav_menu(self, context, nodes):
        if self.show_bookmarks:
            nodes.insert(0, loader.render_to_string('fluke/blocks/model_list.nav_menu.bookmarks.html',
...
class BookmarkWidget(PartialBaseWidget):
    ...
    template = "fluke/widgets/list.html"
