...
from fluke.views.base import BaseAdminPlugin, BaseAdminView, csrf_protect_m
from fluke.views.website import LoginView
...
class ResetPasswordSendView(BaseAdminView):
    ...
    password_reset_template = 'fluke/auth/password_reset/form.html'
    password_reset_done_template = 'fluke/auth/password_reset/done.html'
    password_reset_token_generator = default_token_generator

    password_reset_from_email = None
    password_reset_email_template = 'fluke/auth/password_reset/email.html'
...
site.register_view(r'^fluke/password_reset/$', ResetPasswordSendView, name='fluke_password_reset')

class ResetLinkPlugin(BaseAdminPlugin):

    def block_form_bottom(self, context, nodes):
        reset_link = self.get_admin_url('fluke_password_reset')
...
class ResetPasswordComfirmView(BaseAdminView):
    ...
    password_reset_confirm_template = 'fluke/auth/password_reset/confirm.html'
    password_reset_token_generator = default_token_generator
    ...
    def do_view(self, request, uidb36, token, *args, **kwargs):
        ...
        return password_reset_confirm(request, uidb36, token,
                   ...
                   post_reset_redirect=self.get_admin_url('fluke_password_reset_complete'),
    ...
    def get_media(self):
        return super(ResetPasswordComfirmView, self).get_media() + \
            self.vendor('fluke.page.form.js', 'fluke.form.css')

site.register_view(r'^fluke/password_reset/(?P<uidb36>[0-9A-Za-z]{1,13})-(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
    ResetPasswordComfirmView, name='fluke_password_reset_confirm')
...
class ResetPasswordCompleteView(BaseAdminView):
    ...
    password_reset_complete_template = 'fluke/auth/password_reset/complete.html'
...
site.register_view(r'^fluke/password_reset/complete/$', ResetPasswordCompleteView, name='fluke_password_reset_complete')
