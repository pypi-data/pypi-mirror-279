...
from fluke.plugins.ajax import JsonErrorDict
from fluke.sites import site
from fluke.util import lookup_field, display_for_field, label_for_field, unquote, boolean_icon
from fluke.views import BaseAdminPlugin, ModelFormAdminView, ListAdminView
from fluke.views.base import csrf_protect_m, filter_hook
from fluke.views.edit import ModelFormAdminUtil
from fluke.views.list import EMPTY_CHANGELIST_VALUE
    ...
    def get_media(self, media):
        if self.editable_need_fields:
            media = media + self.model_form.media + \
                self.vendor(
                    'fluke.plugin.editable.js', 'fluke.widget.editable.css')
        return media
