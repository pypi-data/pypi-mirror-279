...
from fluke.layout import FormHelper, Layout, flatatt, Container, Column, Field, Fieldset
from fluke.plugins.utils import get_context_dict
from fluke.sites import site
from fluke.views import BaseAdminPlugin, ModelFormAdminView, DetailAdminView, filter_hook


class ShowField(Field):
    template = "fluke/layout/field_value.html"

    def __init__(self, admin_view, *args, **kwargs):
        super(ShowField, self).__init__(*args, **kwargs)
        self.admin_view = admin_view
        if admin_view.style == 'table':
            self.template = "fluke/layout/field_value_td.html"
...
class TDField(Field):
    template = "fluke/layout/td-field.html"
...
class InlineStyle(object):
    template = 'fluke/edit_inline/stacked.html'
...
class InlineFormset(Fieldset):
    ...
    def __init__(self, formset, allow_blank=False, **kwargs):
        ...
        if allow_blank and len(formset) == 0:
            self.template = 'fluke/edit_inline/blank.html'
    ...
    def get_media(self, media):
        ...
        if self.formsets:
            media = media + self.vendor(
                'fluke.plugin.formset.js', 'fluke.plugin.formset.css')
        return media
