...
from fluke.filters import manager as filter_manager, FILTER_PREFIX, SEARCH_VAR, DateFieldListFilter, \
    RelatedFieldSearchFilter
from fluke.sites import site
from fluke.views import BaseAdminPlugin, ListAdminView
from fluke.util import is_related_field
    ...
    def get_media(self, media):
        ...
        if bool(arr):
            media = media + self.vendor('datepicker.css', 'datepicker.js',
                                        'fluke.widget.datetime.js')
        arr = filter(lambda s: isinstance(s, RelatedFieldSearchFilter), self.filter_specs)
        if six.PY3:
            arr = list(arr)
        if bool(arr):
            media = media + self.vendor(
                'select.js', 'select.css', 'fluke.widget.select.js')
        return media + self.vendor('fluke.plugin.filters.js')

    # Block Views
    def block_nav_menu(self, context, nodes):
        if self.has_filters:
            nodes.append(loader.render_to_string('fluke/blocks/model_list.nav_menu.filters.html',
        ...
        if self.search_fields:
            ...
            nodes.append(
                loader.render_to_string(
                    'fluke/blocks/model_list.nav_form.search_form.html',
                    context=context)
            )
