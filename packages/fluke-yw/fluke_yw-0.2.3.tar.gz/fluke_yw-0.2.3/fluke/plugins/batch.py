...
from fluke.layout import FormHelper, Layout, Fieldset, Container, Col
from fluke.plugins.actions import BaseActionView, ACTION_CHECKBOX_NAME
from fluke.util import model_ngettext, vendor
from fluke.views.base import filter_hook
from fluke.views.edit import ModelFormAdminView
...
class ChangeFieldWidgetWrapper(forms.Widget):
    ...
    @property
    def media(self):
        media = self.widget.media + vendor('fluke.plugin.batch.js')
        return media
...
class BatchChangeAction(BaseActionView):
    ...
    def get_media(self):
        ...
        media = media + self.form_obj.media + self.vendor(
            'fluke.page.form.js', 'fluke.form.css')
        return media
