...
from fluke.util import model_format_dict, model_ngettext
from fluke.views import BaseAdminPlugin, ListAdminView
from fluke.views.base import filter_hook, ModelAdminView
...
class ActionPlugin(BaseAdminPlugin):
    ...
    def get_media(self, media):
        if self.actions and self.admin_view.result_count:
            media = media + self.vendor('fluke.plugin.actions.js', 'fluke.plugins.css')
        return media

    # Block Views
    def block_results_bottom(self, context, nodes):
        if self.actions and self.admin_view.result_count:
            nodes.append(loader.render_to_string('fluke/blocks/model_list.results_bottom.actions.html',
