...
from fluke.util import lookup_field, display_for_field, label_for_field, boolean_icon
...
class ListAdminView(ModelAdminView):
        ...
        if (self.show_all and self.can_show_all) or not self.multi_page:
            ...
            try:
                ...
                if ERROR_FLAG in self.request.GET.keys():
                    return SimpleTemplateResponse('fluke/views/invalid_setup.html', {
    ...
    @filter_hook
    def get_media(self):
        media = super(ListAdminView, self).get_media() + self.vendor('fluke.page.list.js', 'fluke.page.form.js')
        if self.list_display_links_details:
            media += self.vendor('fluke.plugin.details.js', 'fluke.form.css')
        return media

    # Blocks
    @inclusion_tag('fluke/includes/pagination.html')
