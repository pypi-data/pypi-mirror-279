...
from fluke import widgets as exwidgets
from fluke.layout import FormHelper
from fluke.models import UserSettings, UserWidget
from fluke.plugins.utils import get_context_dict
from fluke.sites import site
from fluke.views.base import CommAdminView, ModelAdminView, filter_hook, csrf_protect_m
from fluke.views.edit import CreateAdminView
from fluke.views.list import ListAdminView
from fluke.util import unquote, DJANGO_11
import copy
...
class BaseWidget(forms.Form):
    ...
    template = 'fluke/widgets/base.html'
...
class ModelChoiceIterator(object):
    ...
    def __iter__(self):
        from fluke import site as g_admin_site
...
class QuickBtnWidget(BaseWidget):
    ...
    template = "fluke/widgets/qbutton.html"
...
class AddFormWidget(ModelBaseWidget, PartialBaseWidget):
    ...
    template = "fluke/widgets/addform.html"
    model_perm = 'add'
    ...
    def media(self):
        return self.add_view.media + self.add_view.form_obj.media + self.vendor('fluke.plugin.quick-form.js')
...
class Dashboard(CommAdminView):
        ...
        try:
            ...
            class widget_with_perm(wid):
                def context(self, context):
                    ...
                    context.update({'has_change_permission': self.request.user.has_perm('fluke.change_userwidget')})
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.template_response('fluke/views/dashboard.html', self.get_context())
    ...
    def get_media(self):
        media = super(Dashboard, self).get_media() + \
            self.vendor('fluke.page.dashboard.js', 'fluke.page.dashboard.css')
        if self.widget_customiz:
            media = media + self.vendor('fluke.plugin.portal.js')
