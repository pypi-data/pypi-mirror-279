...
from tradex.constants.direction import Direction
...
class Size:
    ...
    def __repr__(self):
        return (f"{self.__class__.__name__}(buy={self.buy!r}, defend={self.defend!r}, loss={self.loss!r}, "
    ...
    @property
    def size(self) -> float:
        diff = self.buy - self.defend if self._is_long else self.defend - self.buy
        return round(self.loss * (self.buy / diff), 2)