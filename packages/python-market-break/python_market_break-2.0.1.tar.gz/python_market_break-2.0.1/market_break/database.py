...
import numpy as np
from typing import (
    ClassVar, Generator, Mapping, Self, Awaitable, Callable, overload, Iterable
)
...
def create_record_database_table(name: str, metadata: db.MetaData = None) -> db.Table:
    """
    ...
    return db.Table(
        ...
        *Columns.generate(RecordRow.KEYS, non_nullables=[TIMESTAMP], limit=64)
    )
...
class Record:
    ...
    class Callbacks:
        """Contains the record callbacks."""

        store = record_store_callback
        database = record_database_callback
