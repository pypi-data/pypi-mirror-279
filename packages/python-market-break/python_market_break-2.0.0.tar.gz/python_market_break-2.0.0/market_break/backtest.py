...
from market_break.labels import (
    ENTRY, EXIT, BID, ASK, LONG, SHORT, TYPE, RETURNS
)
...
class Trades:
    ...
    def adjust(trades: pd.DataFrame) -> pd.DataFrame:
        ...
        trades = trades[trades[EXIT] != EMPTY]
        trades = trades[trades[ENTRY] != EMPTY]
    ...
    def generate(
        ...
        for i in range(len(up)):
            ...
            if len(long) == len(short) == 0:
                if long_signal:
                    long.append(i)
                    data.append((short[-1] if short else EMPTY, long[-1], SHORT))

                elif short_signal:
                    short.append(i)
                    data.append((long[-1] if long else EMPTY, short[-1], LONG))
        ...
        if EMPTY not in data[-1]:
            if data[-1][2] == SHORT:
                data.append((data[-1][1], EMPTY, LONG))

            elif data[-1][2] == LONG:
                data.append((data[-1][1], EMPTY, SHORT))
...
class Plot:
    ...
    def returns_histogram(returns: pd.Series, bins: int = 50) -> None:
        ...
        plt.xlabel('Returns (%)')
        plt.ylabel('Count')
        plt.axvline(0, color='blue', label='zero', lw=1.5)
        plt.axvline(
            returns_pct_average, color='orange',
            label=f'mean return {returns_pct_average:.5f}%', lw=1.5
        )
        plt.axvline(
            gains_pct_average, color='green',
            label=f'mean gain {gains_pct_average:.5f}%', lw=1.5
        )
        plt.axvline(
            losses_pct_average, color='red',
            label=f'mean loss {losses_pct_average:.5f}%', lw=1.5
        )
    ...
    def returns_signals(returns: pd.Series, index: np.ndarray | pd.Series = None) -> None:
        ...
        plt.axhline(
            returns_pct_average, color='orange',
            label=f'mean {returns_pct_average:.5f}%', lw=1.5
        )
        plt.axhline(
            gains_pct_average, color='green',
            label=f'mean gain {gains_pct_average:.5f}%', lw=1.5
        ...
        plt.axhline(0, color='blue', label=f'zero', lw=1.5)
        plt.legend()
        plt.show()
