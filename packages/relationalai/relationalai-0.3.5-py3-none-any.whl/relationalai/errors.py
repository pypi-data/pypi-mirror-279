...
def print_source_error(source, name:str, content:str):
    ...
    dashes = '-' * num_dashes
    console = Console(width=80, force_jupyter=False)
    console.print("\n")
...
class RAIException(Exception):
    ...
    def clone(self):
        return RAIException(message=self.message, name=self.name, content=self.content, source=self.source)
...
class RelQueryError(Exception):
    ...
    def pprint(self):
        body = "\n\n".join(
            f'{problem["error_code"]}\n{problem["report"]}\n{problem["message"]}'
            for problem in self.problems
        )
        return f"Rel query error\n{body}"

    def clone(self):
        return RelQueryError(self.problems)
...
class RAIWarning(Warning):
    ...
    def pprint(self):
        if self.source and self.content:
            print_source_error(self.source, self.name or self.message, self.content)
        elif self.content:
            print_error(self.name or self.message, self.content)
        else:
            print_error_name(self.name or self.message)
...
class InvalidBoolWarning(RAIWarning):
    def __init__(self, source):
        ...
        self.content = self.format_message()
        self.raw_content = self.strip_rich_tags()
        super().__init__(self.name, source=source, content=self.content)
...
class RAIInvalidVersionWarning(RAIWarning):
    def __init__(
        ...
        super().__init__(name=self.name, content=full_msg)
