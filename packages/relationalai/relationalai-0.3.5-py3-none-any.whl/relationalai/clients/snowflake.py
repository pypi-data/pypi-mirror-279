...
from ..clients.util import escape_for_f_string, escape_for_sproc, scrub_exception
from .. import dsl, rel, metamodel as m
from ..errors import EngineNotFoundException, EnginePending, Errors, RAIException, SnowflakeAppMissingException, SnowflakeChangeTrackingNotEnabledException, SnowflakeImportMissingException, SnowflakeInvalidSource
...
LIST_TXN_SQL_FIELDS = ["id", "database_name", "state", "abort_reason", "read_only", "created_by", "created_on", "finished_at", "duration"]
GET_TXN_SQL_FIELDS = ["id", "database_name", "engine_name", "state", "abort_reason", "read_only",
                      "created_by", "created_on", "finished_at", "timings"]
...
ENGINE_SIZES = ["HighMem|S", "HighMem|M"]
COMPUTE_POOL_MAP = {
    "HIGHMEM_X64_S": "HighMem|S",
    "HIGHMEM_X64_M": "HighMem|M",
    "HIGHMEM_S": "HighMem|S",
    "HIGHMEM_M": "HighMem|M",
    "HighMem|S": "HighMem|S",
    "HighMem|M": "HighMem|M",
...
class Resources(ResourceProvider):
            ...
            if re.search(f"database '{rai_app}' does not exist or not authorized.".lower(), orig_message):
                ...
                if match:
                    ...
                    if "engine was deleted" in message or "engine not found" in message:
                        ...
                        raise exception from None
                    if "engine is in pending" in message:
    ...
    def list_engines(self, state: str | None = None):
        ...
        statement = f"SELECT NAME, ID, SIZE, STATUS, CREATED_BY, CREATED_ON, UPDATED_ON FROM {APP_NAME}.api.engines {where_clause}"
    ...
    def get_export_code(self, params: ExportParams):
        ...
        sql_code = textwrap.dedent(f"""
            CREATE OR REPLACE PROCEDURE {params.func_name}({sql_inputs})
    ...
    def _check_exec_async_status(self, txn_id: str):
        ...
        if status == "ABORTED":
            # see https://github.com/RelationalAI/snowflake-native-app/blob/d2f81bffca474fcec665101b7e0bb47fce2789a5/lib/integration/rai.sql#L956
            abort_reason = response_row['ABORT_REASON']
            raise Exception(
                f"Transaction aborted while waiting for results '{txn_id}' with reason '{abort_reason}'"
            )
    ...
    def _download_results(self, artifact_urls, txn_id, state: str) -> TransactionAsyncResponse:
        with debugging.span("download_results"):
            ...
            rsp.transaction = {"id": txn_id, "state": state, "response_format_version": None}
            rsp.metadata = meta
    ...
    # Transactions
    #--------------------------------------------------
    def txn_list_response_to_dicts(self, transactions):
        field_mapping = {
        ...
        parsed_results = [
            {
                field_mapping.get(field, field): row[field.upper()]
                for field in LIST_TXN_SQL_FIELDS
            }
    ...
    def get_transaction(self, transaction_id):
        ...
        txn = {field: results[0][field.upper()] for field in GET_TXN_SQL_FIELDS}
    ...
    def list_transactions(self, **kwargs):
        ...
        sql_fields = ", ".join([f.upper() for f in LIST_TXN_SQL_FIELDS])
        results = self._exec(f"select {sql_fields} from {APP_NAME}.api.transactions {where_clause} LIMIT ?", [limit])
        if not results:
            return []
        return self.txn_list_response_to_dicts(results)
