...
def coerce_group(x):
    ...
    This function is used by the wtforms SelectField to ensure that the
    selected value is properly converted into a GroupModel instance. It
    handles cases where the input is already a GroupModel, or a string
    representing the name of the group.

    Parameters:
        x (Union[str, GroupModel]): The input value to be coerced. This can be
                                    a string representing the group name or a
                                    GroupModel instance.

    Returns:
        GroupModel: The corresponding GroupModel instance if the input is valid,
                    or None if the input is neither a valid group name nor a
                    GroupModel instance.
    """
    group_model = flask.current_app.get_model(hawat.const.MODEL_GROUP)
    if isinstance(x, group_model):
        return x
    elif isinstance(x, str):
        return hawat.db.db_query(group_model).filter(group_model.name == x).first()
    else:
        return None
