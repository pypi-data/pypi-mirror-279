...
from mentat.stats.idea import KEY_UNKNOWN, ST_SKEY_REST
...
def _bq_get_unnested_set(parameters):
    if parameters['aggr_set'] in EVENTS_COLUMNS_ARRAY:
        return psycopg2.sql.SQL('unnest({})').format(
...
def _bq_timeline(parameters, qname, dbtoplist = None):
    ...
    is_toplist = dbtoplist and not is_count and 'limit' in parameters and parameters['aggr_set'] in EVENTS_COLUMNS_TOPLISTED

    params = []
    subqueries = [psycopg2.sql.SQL("timeline AS (SELECT * FROM (SELECT %s AS bucket UNION SELECT generate_series(%s, %s - INTERVAL '1 microsecond', %s) AS bucket) AS t ORDER BY bucket)")]
    ...
    if not is_count:
        if parameters['aggr_set'] in EVENTS_COLUMNS_ARRAY:
            subquery = psycopg2.sql.SQL('total AS (SELECT COALESCE(SUM(CARDINALITY({})), 0) AS total FROM events').format(
                psycopg2.sql.Identifier(parameters['aggr_set'])
            )
    ...
    if is_toplist:
        ...
        subquery = psycopg2.sql.SQL('toplist_with_rest AS (SELECT set::text, sum FROM toplist UNION (SELECT {} as set, total - SUM(sum)::bigint as sum FROM total, toplist GROUP BY total HAVING total - SUM(sum)::bigint > 0) ORDER BY sum DESC)').format(
            psycopg2.sql.Literal(ST_SKEY_REST)
        )
        subqueries.append(subquery)

    subquery = psycopg2.sql.SQL('raw AS (SELECT GREATEST(%s, %s + %s * (width_bucket(detecttime, (SELECT array_agg(bucket) FROM generate_series(%s, %s - INTERVAL \'1 microsecond\', %s) AS bucket)) - 1)) AS bucket{}, COUNT(*) AS count FROM {}').format(
        psycopg2.sql.SQL('') if is_count else psycopg2.sql.SQL(', {} AS set').format(_bq_get_unnested_set(parameters)) if not is_toplist else psycopg2.sql.SQL(', set'),
        psycopg2.sql.Identifier('events') if not is_toplist else psycopg2.sql.SQL('(SELECT detecttime, {} AS set FROM events').format(
            _bq_get_unnested_set(parameters)
        )
    )
    params.append(parameters['dt_from'])
    ...
    if is_toplist:
        subquery = psycopg2.sql.SQL('raw_with_rest AS (SELECT bucket, set::text, count FROM raw UNION ALL SELECT bucket, {} AS set, raw_totals.count - raw_sums.count AS count FROM (SELECT GREATEST(%s, %s + %s * (width_bucket(detecttime, (SELECT array_agg(bucket) FROM generate_series(%s, %s - INTERVAL \'1 microsecond\', %s) AS bucket)) - 1)) AS bucket, {} AS count FROM events').format(
            psycopg2.sql.Literal(ST_SKEY_REST),
            psycopg2.sql.SQL('SUM(CARDINALITY({}))').format(psycopg2.sql.Identifier(parameters['aggr_set'])) if parameters['aggr_set'] in EVENTS_COLUMNS_ARRAY else psycopg2.sql.SQL('COUNT(*)')
        )
        params.append(parameters['dt_from'])
        params.append(parameters['first_step'])
        params.append(parameters['step'])
        ...
        subquery, params = _bq_where(parameters, subquery, params, end_inclusive=False)
        subquery += psycopg2.sql.SQL(' GROUP BY bucket) raw_totals FULL JOIN (SELECT bucket, SUM(count)::bigint AS count FROM raw GROUP BY bucket) raw_sums USING (bucket))')
        subqueries.append(subquery)

    elif not is_count:
        subquery = psycopg2.sql.SQL('sums AS (SELECT raw.set::text AS set, SUM(raw.count)::bigint AS sum FROM raw GROUP BY raw.set ORDER BY sum DESC)')
        subqueries.append(subquery)

    main_query = psycopg2.sql.SQL(' (SELECT timeline.bucket{}, COALESCE(count, 0) AS count FROM {} LEFT JOIN {} ON {} ORDER BY bucket ASC{}) UNION ALL SELECT NULL, {} FROM {}')
    if is_count:
        ...
        main_query = main_query.format(
            ...
            psycopg2.sql.SQL('{} UNION ALL SELECT NULL, NULL, total FROM {} UNION ALL SELECT NULL, NULL, total FROM total_events').format(
                toplist_or_sums,
                psycopg2.sql.Identifier('total') if parameters['aggr_set'] in EVENTS_COLUMNS_ARRAY else psycopg2.sql.Identifier('total_events')  # in case the set is not array, just use total event count as total
            )
        )
        ...
        for item in (
            ...
            if parameters.get(item[0], None):
                item[2](
                    ...
                    parameters.get('not_{}'.format(item[0]), "False") in ["True", "true", "TRUE", "y", "Y", "yes", "YES", "1", True]
                )
...
def build_query(parameters = None, qtype = QTYPE_SELECT, qname = None, dbtoplist = False):
    """
    ...
    try:
        ...
        raise ValueError("Received invalid value '{}' for SQL query type.".format(qtype)) from error
...
class EventStorageCursor:
    ...
    def search_events_aggr(self, parameters = None, qtype = QTYPE_AGGREGATE, qname = None, dbtoplist = False):
        ...
        :rtype: tuple
        """
        query, params = build_query(parameters, qtype = qtype, qname = qname, dbtoplist = dbtoplist)
        self.lastquery = self.cursor.mogrify(query, params)
...
class EventStorageService:
    ...
    def handle_db_exceptions(func):  # pylint: disable=locally-disabled,no-self-argument
        """
    ...
    def query_status(self, qname):
        ...
        """
        qname = r'_mentatq\({}\)_'.format(qname)
    ...
    def query_cancel(self, qname):
        ...
        self.cursor.execute(
            "SELECT pg_cancel_backend(pid) AS opresult FROM pg_stat_activity WHERE datname = (SELECT current_database()) AND query ~ '{}' AND backend_type = 'client backend'".format(
                qname
            )
        )
        data_raw = self.cursor.fetchone()
