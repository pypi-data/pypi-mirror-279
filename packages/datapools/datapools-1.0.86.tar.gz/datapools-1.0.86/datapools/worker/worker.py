...
from typing import Optional, Set, List, Tuple, Any, Dict, NamedTuple, Union
...
class CrawlerWorker(Stoppable):
            ...
            if qm.type == QueueMessageType.Task:
                ...
                if message.redelivered is True:
                    ...
                    if processing_worker_id:
                        ...
                        if processing_worker_id == self.id:  # we are procesing this message already, ignoring
                            ...
                            return
                        if await self._is_worker_alive(processing_worker_id, session):
                            logger.info(f"worker still alive")
                            return
                        logger.info("it's dead, accepting task")
                ...
                while not await self.loop_done(session):
                    ...
                    if plugin == MAX_INSTANCES_ERROR:
                        ...
                        continue
                    break
                if plugin is None or plugin == MAX_INSTANCES_ERROR:
    ...
    async def _process_content_helper(self, cc: CrawlerContent, session: Session, url: AnyUrl) -> bool:
        res = False
