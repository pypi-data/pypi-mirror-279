...
class BackendServiceClient(BaseClient):
    ...
    def create_query(
        ...
        # sync the query table
        base_url = f"{self.BACKEND_URL}/api/database/tables/%s/syncdb/"
        task = self.refresh_resource(
            resource_type='table',
            refresh_baseurl=base_url,
            resource_id=response_dict['id'],
        )

        if run_on_peliqan_trino != empty or materialize != empty:
            # only a synced table can be materialized.
            if materialize is True:
                # wait for the refresh to complete
                _retry_get_resource_status(
                    lambda: self.get_refresh_resource_task_status(
                        'table',
                        refresh_baseurl=base_url + 'status',
                        resource_id=response_dict['id'],
