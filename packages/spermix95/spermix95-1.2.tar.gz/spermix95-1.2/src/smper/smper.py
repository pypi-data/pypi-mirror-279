...
class pict:
    def __init__(self):
        self.path = 'src/smper/files'
        self.sklad = []
        self.themes = []
        self.files = []

        k = 0
        for theme in [_ for _ in os.listdir(self.path) if '.' not in _]:
            if 'DS' not in theme:
                self.themes += [theme]
...
plt.tight_layout()
plt.show()''']],
4: ["""Построить  модель, используя панельные данные, для прогнозирования коэффициента рождаемости с учетом специфики регионов РФ.
