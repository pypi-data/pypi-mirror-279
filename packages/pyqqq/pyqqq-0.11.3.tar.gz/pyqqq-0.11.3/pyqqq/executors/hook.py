...
class HookExecutor:
    def __init__(
        ...
        self.market_close_time_margin = dtm.timedelta(
            minutes=5
        )  # 폐장 시간과 거래 시간 사이의 여유 시간
        self.before_market_open_schedule = dtm.timedelta(
            hours=1
        )  # 개장 전에 호출되는 hook이 호출되는 시간
        self.after_market_close_schedule = dtm.timedelta(
            hours=1
    ...
    async def initialize(self):
        ...
        if self._has_hook("on_initialize"):
            logger.info(f"[EXECUTOR] Call on_initialize hook")
        ...
        if self.stock_broker_factory is None:
            if self.brokerage == "kis":
                ...
                return KISSimpleDomesticStock(
                    auth, self.account_number, self.account_product_code, self.hts_id
                )
    ...
    def _load_env(self):
        ...
        if all(
            x is not None
            for x in [ebest_app_key, ebest_app_secret, kis_app_key, kis_app_secret]
        ):
            raise Exception(
                "Ambiguous brokerage setting. Please set only one of EBEST or KIS"
            )
        ...
        if all(x is not None for x in [ebest_app_key, ebest_app_secret]):
            ...
            raise Exception(
                "Brokerage setting is required. Please set either EBEST or KIS"
            )
    ...
    def _update_config(self, config: Dict) -> None:
        ...
        if "interval" in config:
            ...
            if not isinstance(interval, dtm.timedelta) or interval.total_seconds() < 10:
                raise Exception(
                    "interval must be dtm.timedelta type with at least 10 seconds"
                )
        ...
        if "take_profit_config" in config:
            ...
            for k, v in take_profit_config.items():
                if type(v) not in [int, float]:
                    raise Exception(
                        "take_profit_config must be dict type with int or float value"
                    )
                if v <= 0:
                    raise Exception(
                        "take_profit_config must be dict type with positive value"
                    )
                if type(k) != str:
        ...
        if "stop_loss_config" in config:
            ...
            for k, v in stop_loss_config.items():
                if type(v) not in [int, float]:
                    raise Exception(
                        "stop_loss_config must be dict type with int or float value"
                    )
                if v >= 0:
                    raise Exception(
                        "stop_loss_config must be dict type with negative value"
                    )
                if type(k) != str:
        ...
        if "open_market_time_margin" in config:
            ...
            if type(open_market_time_margin) == dtm.timedelta:
                self.market_open_time_margin = open_market_time_margin
            elif type(open_market_time_margin) == int:
                self.market_open_time_margin = dtm.timedelta(
                    seconds=open_market_time_margin
                )
            else:
                raise Exception(
                    "open_market_time_margin must be int or dtm.timedelta type"
                )
        ...
        if "close_market_time_margin" in config:
            ...
            elif type(close_market_time_margin) == int:
                self.market_close_time_margin = dtm.timedelta(
                    seconds=close_market_time_margin
                )
            else:
                raise Exception(
                    "close_market_time_margin must be int or dtm.timedelta type"
                )
        ...
        if "before_market_open_schedule" in config:
            ...
            if type(before_market_open_schedule) == int:
                self.before_market_open_schedule = dtm.timedelta(
                    seconds=before_market_open_schedule
                )
            elif type(before_market_open_schedule) == dtm.timedelta:
                self.before_market_open_schedule = before_market_open_schedule
            elif type(before_market_open_schedule) == dtm.time:
                self.before_market_open_schedule = before_market_open_schedule
            else:
                raise Exception(
                    "before_market_open_schedule must be int, dtm.timedelta or time type"
                )

        if "after_market_close_schedule" in config:
            after_market_close_schedule = config.get("after_market_close_schedule")
            if type(after_market_close_schedule) == int:
                self.after_market_close_schedule = dtm.timedelta(
                    seconds=after_market_close_schedule
                )
            elif type(after_market_close_schedule) == dtm.timedelta:
                self.after_market_close_schedule = after_market_close_schedule
            elif type(after_market_close_schedule) == dtm.time:
                self.after_market_close_schedule = after_market_close_schedule
            else:
                raise Exception(
                    "after_market_close_schedule must be int, dtm.timedelta or time type"
                )

    async def monitor_stop_loss(
        self, positions: List, stop_loss_config: Dict = {}
    ) -> None:
    ...
    async def _run_intraday_strategy(self):
        ...
        while True:
            ...
            if now.date() != today:
                ...
                logger.info(
                    f"-  before_open_call_time: {self.before_market_open_hook_call_time.strftime('%H:%M:%S')}"
                )
                logger.info(
                    f"-       market_open_time: {self.market_open_time.strftime('%H:%M:%S')}"
                )
                logger.info(
                    f"-  user_market_open_time: {self.user_defined_market_open_time.strftime('%H:%M:%S')}"
                )
                logger.info(
                    f"- user_market_close_time: {self.user_defined_market_close_time.strftime('%H:%M:%S')}"
                )
                logger.info(
                    f"-      market_close_time: {self.market_close_time.strftime('%H:%M:%S')}"
                )
                logger.info(
                    f"-  after_close_call_time: {self.after_market_close_hook_call_time.strftime('%H:%M:%S')}"
                )
                logger.info(
                    f"-               interval: {self.interval.seconds} seconds"
                )
                logger.info(
                    f"-           start_margin: {int(self.market_open_time_margin.total_seconds())} seconds"
                )
                logger.info(
                    f"-             end_margin: {int(self.market_close_time_margin.total_seconds())} seconds"
                )
            ...
            try:
                if self.is_trading_time():
                    ...
                    # 장 종료시 최종 1회 실행되는 함수 호출
                        ...
                        not market_closed and now >= self.user_defined_market_close_time
                    ):
                        market_closed = True
                    ...
                    # 장 시작 후 사용자 설정 주기마다 전략 실행
                        ...
                        and now >= (last_call_time + self.interval)
                    ):
                        last_call_time = now
                        await self.trade()
                else:
                    if now >= (
                        last_market_close_inform_time + market_close_inform_interval
                    ):
                        last_market_close_inform_time = now
                        logger.info(f"[EXECUTOR] Not trading time.")

                if self.is_trading_day():
                    if (
                        not before_market_open_hook_called
                        and now >= self.before_market_open_hook_call_time
                    ):
                    ...
                    if (
                        not after_market_close_hook_called
                        and now >= self.after_market_close_hook_call_time
                    ):
    ...
    async def trade(self) -> None:
        ...
        if len(positions) > 0 and (
            len(self.stop_loss_config) > 0 or len(self.take_profit_config) > 0
        ):
            # Stop loss 및 take profit 주문 집행
            await self.monitor_stop_loss(
                positions=positions, stop_loss_config=self.stop_loss_config
            )
            await self.monitor_take_profit(
                positions=positions, take_profit_config=self.take_profit_config
            )
    ...
    def validate_trade_signal(
        self, trade_signals: List[Tuple[str, int, int]], tag: str = "hook"
    ):
        if trade_signals is None:
            return []

        assert type(trade_signals) == list, f"{tag} must return list type"

        for r in trade_signals:
            assert type(r) == tuple, f"{tag} must return list of tuple"
            assert len(r) == 3, f"{tag} must return list of tuple with 3 elements"
            assert type(r[0]) == str, f"{tag} must return list of tuple with str type as first element(asset_code)"
            assert type(r[1]) == int, f"{tag} must return list of tuple with int type as second element(price)"
            assert type(r[2]) == int, f"{tag} must return list of tuple with int type as third element(quantity)"
        ...
        *예시*
        ...
        for instruction in trade_instructions:
            if len(instruction) != 4:
                raise Exception(
                    "trade_instructions must be list of tuple with 4 elements"
                )
            ...
            if quantity < 0:
                ...
                if self.brokerage == "kis":
                    self.stock_broker.update_order(
                        order_id, OrderType.LIMIT, price, quantity
                    )
                else:
                    self.stock_broker.update_order(
    ...
    async def trade_func(
        self, account_info: Dict, pending_orders: List, positions: List
    ) -> List[Tuple[str, int]]:
        """
        장 중에 연속적으로 실행되는 함수
        사용자가 설정하는 전략 함수
        ...
        if self._has_hook("trade_func"):
            logger.info(f"[EXECUTOR] Call trade_func hook")
            result = await maybe_awaitable(
                self.strategy_module.trade_func(
                    account_info, pending_orders, positions, self.stock_broker
                )
            )
            if result is None:
                return []
