...
class KISSimpleDomesticStock:
    ...
    def get_historical_daily_data(self,
        ...
        for i in range(0, total_days + 1, max_days_per_request):
    ...
    def map_order_event(self, data: dict) -> OrderEvent:
        ...
        if rejected:
            ...
            status = "partial" if filled_quantity < quantity else "completed"
        ...
        print(
            f"- rctf_cls={rctf_cls} acpt_yn={acpt_yn} cntg_yn={cntg_yn} accepted={accepted} executed={executed} rejected={rejected} status={status}"
        )
