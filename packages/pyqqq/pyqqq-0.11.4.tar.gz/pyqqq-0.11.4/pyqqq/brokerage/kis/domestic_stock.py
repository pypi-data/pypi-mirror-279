...
class KISDomesticStock:
    ...
    def get_price(self,
        ...
        Args:
            ...
            fid_cond_mrkt_div_code (str): FID 조건시장분류코드 - J:주식,ETF,ETN W:ELW
        ...
        assert fid_cond_mrkt_div_code in ['J', 'W']
        ...
        decimal_keys = [
            'marg_rate', 'prdy_ctrt', 'prdy_vrss_vol_rate',
            'wghn_avrg_stck_prc', 'hts_frgn_ehrt', 'per', 'pbr',
            'vol_tnrt', 'eps', 'bps', 'd250_hgpr_vrss_prpr_rate',
            'd250_lwpr_vrss_prpr_rate', 'dryy_hgpr_vrss_prpr_rate',
            'dryy_lwpr_vrss_prpr_rate', 'w52_hgpr_vrss_prpr_ctrt',
            'w52_lwpr_vrss_prpr_ctrt', 'whol_loan_rmnd_rate']
        ...
        int_keys = [
            'stck_prpr', 'prdy_vrss', 'prdy_vrss_sign', 'acml_tr_pbmn',
            'acml_vol', 'stck_oprc', 'stck_hgpr', 'stck_lwpr', 'stck_mxpr',
            'stck_llam', 'stck_sdpr', 'frgn_ntby_qty', 'pgtr_ntby_qty',
            'pvt_scnd_dmrs_prc', 'pvt_frst_dmrs_prc', 'pvt_pont_val',
            'pvt_frst_dmsp_prc', 'pvt_scnd_dmsp_prc', 'dmrs_val',
            'dmsp_val', 'cpfn', 'rstc_wdth_prc', 'stck_fcam', 'stck_sspr',
            'aspr_unit', 'hts_deal_qty_unit_val', 'lstn_stcn', 'hts_avls',
            'stac_month', 'd250_hgpr', 'd250_lwpr', 'stck_dryy_hgpr',
            'stck_dryy_lwpr', 'w52_hgpr', 'w52_lwpr', 'frgn_hldn_qty',
            'last_ssts_cntg_qty']
        ...
        date_keys = [
            'd250_hgpr_date', 'd250_lwpr_date', 'dryy_hgpr_date',
            'dryy_lwpr_date', 'w52_hgpr_date', 'w52_lwpr_date',
        ]
    ...
    def inquire_daily_price(self,
        ...
        Args:
            fid_input_iscd (str): FID 입력종목코드 - 종목번호(6자리) ETN의 경우 Q로 시작(EX. Q500001)
            fid_cond_mrkt_div_code (str): FID 조건시장분류코드 - J:주식,ETF,ETN
            fid_period_div_code (str): FID 기간구분코드 - D:일별 W:주별 M:월별
            fid_org_adj_prc (str): FID 조정가구분코드 - 0:무조정 1:조정
    ...
    def inquire_asking_price_exp_ccn(self,
                                     fid_input_iscd: str,
                                     fid_cond_mrkt_div_code: str = 'J'):
        '''
    ...
    def inquire_daily_itemchartprice(self,
                                     fid_input_iscd: str,
    ...
    def inquire_daily_indexchartprice(self,
                                      fid_input_iscd: str,
                                      fid_input_date_1: dtm.date,
                                      fid_input_date_2: dtm.date,
                                      fid_period_div_code: str):
        ...
        output1 = res_body['output1']
        decimal_keys = [
            'bstp_nmix_prdy_vrss', 'bstp_nmix_prdy_ctrt', 'prdy_nmix',
            'bstp_nmix_oprc', 'bstp_nmix_prpr', 'bstp_nmix_hgpr',
            'bstp_nmix_lwpr', 'futs_prdy_oprc', 'futs_prdy_hgpr',
            'futs_prdy_lwpr']
        for k in decimal_keys:
            if k in output1 and len(output1[k]) > 0:
                output1[k] = Decimal(output1[k])
    ...
    def inquire_time_itemconclusion(self,
                                    fid_input_iscd: str,
                                    fid_input_hour_1: dtm.time,
                                    fid_cond_mrkt_div_code: str = 'J',
                                    tr_cont: str = ''):
        '''
    ...
    def inquire_time_overtimeconclusion(self,
                                        fid_input_iscd: str,
                                        tr_cont: str = ''):
        '''
    ...
    def inquire_time_itemchartprice(self,
                                    fid_input_iscd: str,
        ...
        Args:
            ...
            fid_cond_mrkt_div_code (str): FID 조건시장분류코드 - J:주식,ETF,ETN U:업종
        ...
        assert fid_cond_mrkt_div_code in ['J', 'U'], 'fid_cond_mrkt_div_code must be "J" or "U"'
        assert fid_pw_data_incu_yn in ['Y', 'N'], 'fid_pw_data_incu_yn must be "Y" or "N"'
        assert fid_cond_mrkt_div_code == 'J' and isinstance(fid_input_hour_1, dtm.time) or \
               fid_cond_mrkt_div_code == 'U' and isinstance(fid_input_hour_1, int), 'fid_input_hour_1 type mismatch'
        if type(fid_input_hour_1) == int:
    ...
    def search_info(self,
        ...
        Raise:
            ValueError: API 에러 발생시
        '''
        assert self.auth.paper_trading == False, '실전계좌에서만 사용 가능한 API입니다.'
    ...
    def chk_holiday(self, base_dt: dtm.date, tr_cont: str = ''):
        '''
        ...
        tr_id = 'CTCA0903R'
        params = {
            'BASS_DT': base_dt.strftime('%Y%m%d'),
            'CTX_AREA_NK': '',
            'CTX_AREA_FK': '',
        }
        res_body, res_headers = self._tr_request(url_path, tr_id, tr_cont, params=params)
    ...
    def get_foreign_institution_total(self,
                                      fid_input_iscd: str,
    ...
    def get_stock_condition_search_list(self, user_id: str, tr_cont: str = ""):
        ...
        res_body, res_headers = self._tr_request(
            url_path, tr_id, tr_cont, params=params
        )
    ...
    def get_stock_condition_search_results(
        self, user_id: str, seq: str, tr_cont: str = ""
    ):
        """
    ...
    def inquire_time_indexchartprice(self,
        ...
        params = {
            ...
            'FID_ETC_CLS_CODE': fid_etc_cls_code,
            'FID_INPUT_ISCD': fid_input_iscd,
            'FID_INPUT_HOUR_1': str(fid_input_hour_1),
            'FID_PW_DATA_INCU_YN': fid_pw_data_incu_yn
        }
    ...
    def inquire_price_2(self, fid_input_iscd: str):
        '''
        ...
        for k in output.keys():
            if k in ['prdy_ctrt', 'prdy_vrss_vol_rate', 'prdy_clpr_vrss_oprc_rate', 'prdy_clpr_vrss_hgpr_rate', 'prdy_clpr_vrss_lwpr_rate', 'marg_rate', 'crdt_rate']:
                output[k] = Decimal(output[k])
            elif k in ['stck_prpr', 'prdy_vrss', 'prdy_vrss_sign', 'acml_tr_pbmn', 'acml_vol', 'prdy_vol', 'stck_prdy_clpr', 'stck_oprc', 'oprc_vrss_prpr_sign', 'oprc_vrss_prpr', 'stck_hgpr', 'hgpr_vrss_prpr_sign', 'hgpr_vrss_prpr', 'stck_lwpr', 'lwpr_vrss_prpr_sign', 'lwpr_vrss_prpr', 'stck_mxpr', 'stck_llam', 'bstp_cls_code', 'stck_sdpr']:
                output[k] = int(output[k])
            else:
    ...
    def inquire_daily_trade_volume(self,
                                   fid_input_iscd: str,
                                   fid_input_date_1: dtm.date,
                                   fid_input_date_2: dtm.date,
                                   tr_cont: str = ''):
    ...
    def inquire_index_price(self, fid_input_iscd: str):
        ...
        for k in output.keys():
            if k in ['dryy_bstp_nmix_hgpr_date', 'dryy_bstp_nmix_lwpr_date']:
                output[k] = dtm.datetime.strptime(output[k], '%Y%m%d').date()
            elif k in ['bstp_nmix_prdy_vrss', 'bstp_nmix_prdy_ctrt', 'bstp_nmix_prpr', 'bstp_nmix_oprc', 'prdy_nmix_vrss_nmix_oprc', 'bstp_nmix_oprc_prdy_ctrt', 'bstp_nmix_hgpr', 'prdy_nmix_vrss_nmix_hgpr', 'bstp_nmix_hgpr_prdy_ctrt', 'bstp_nmix_lwpr', 'prdy_clpr_vrss_lwpr', 'prdy_clpr_vrss_lwpr_rate', 'dryy_bstp_nmix_hgpr', 'dryy_hgpr_vrss_prpr_rate', 'dryy_bstp_nmix_lwpr', 'dryy_lwpr_vrss_prpr_rate', 'seln_rsqn_rate', 'shnu_rsqn_rate']:
                output[k] = Decimal(output[k])
            else:
                output[k] = int(output[k])
    ...
    def inquire_index_daily_price(self,
                                  fid_period_div_code: str,
                                  fid_input_iscd: str,
                                  fid_input_date_1: dtm.date,):
        '''
        ...
        Args:
            ...
            fid_input_iscd (str): 입력 종목코드 - 전체(0000) 코스피(0001), 코스닥(1001), 코스피200(2001)
        ...
        assert fid_input_iscd in ['0000', '0001', '1001', '2001'], 'fid_input_iscd must be "0000", "0001", "1001" or "2001"'
    ...
    def inquire_index_category_price(self,
                                     fid_blng_cls_code: str,
                                     fid_input_iscd: str,
    ...
    def order_cash(self,
        ...
        assert type(ord_dvsn) == str and len(ord_dvsn) == 2 and int(ord_dvsn) >= 0 and int(ord_dvsn) <= 16, 'ord_dvsn must be a string of integer between 00 and 16'
        assert type(ord_qty) == int and ord_qty > 0, 'ord_qty must be a positive integer'

        if ord_dvsn in ['00', '02', '11', '12']: # 지정가, 조건부지정가, IOC/FOK지정가
            assert type(ord_unpr) == int and ord_unpr > 0, 'ord_unpr must be a positive integer when limit order'

        url_path = '/uapi/domestic-stock/v1/trading/order-cash'

        if side == 'sell':
            tr_id = 'VTTC0801U' if self.auth.paper_trading else 'TTTC0801U'
        else:
            tr_id = 'VTTC0802U' if self.auth.paper_trading else 'TTTC0802U'
    ...
    def order_rvsecncl(self,
        ...
        tr_id = 'VTTC0803U' if self.auth.paper_trading else 'TTTC0803U'
        req_body = {
            'CANO': cano,
            'ACNT_PRDT_CD': acnt_prdt_cd,
            'KRX_FWDG_ORD_ORGNO': '',
    ...
    def inquire_daily_ccld(
        ...
        if self.auth.paper_trading:
            tr_id = 'VTTC8001R' if within_3month else 'VTSC9115R'
        else:
            tr_id = 'TTTC8001R' if within_3month else 'CTSC9115R'
    ...
    def inquire_balance_rlz_pl(self,
                               cano: str,
                               acnt_prdt_cd: str,
                               afhr_flpr_yn: str = 'N',
                               fund_sttl_icld_yn: str = 'N',
    ...
    def inquire_credit_psamount(self,
                                cano: str,
                                acnt_prdt_cd: str,
                                pdno: str,
                                ord_unpr: int,
    ...
    def inquire_period_trade_profit(self,
                                    cano: str,
                                    acnt_prdt_cd: str,
                                    inqr_strt_dt: dtm.date,
                                    inqr_end_dt: dtm.date,
    ...
    async def listen_order_event(
        self, hts_id: str, stop_event: asyncio.Event
    ) -> AsyncGenerator:
        """
            ...
            if tr_id in tr_ids:
                if iv and key:
                    ...
                    for i, k in enumerate(keys):
                        if (
                            k in ["cntg_unpr", "order_prc", "cntg_qty", "oder_qty"]
                            and len(values[i]) > 0
                        ):
