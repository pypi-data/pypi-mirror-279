...
class EBestTRWebsocketKeepConnectionStatus(Enum):
    """
    이베스트투자증권 TR 요청 웹소켓 클라이언트의 연결 유지 상태
    """
...
class EBestTRWebsocketClient:
    """
    이베스트투자증권 실시간 데이터 요청을 위한 웹소켓 클라이언트
    ...
    logger = get_logger("ebest_realtime")

    def __init__(
        self,
        auth: EBestAuth,
        on_connect: Callable[[websockets.WebSocketClientProtocol], asyncio.Task] = None,
        on_ask_keep_connection: Callable[
            [], EBestTRWebsocketKeepConnectionStatus
        ] = lambda: EBestTRWebsocketKeepConnectionStatus.KEEP,
                ...
                async with websockets.connect(websocket_url) as ws:
                    ...
                    while not self.stop_event.is_set():
                        ...
                        if data:
                            ...
                            self.on_ask_keep_connection()
                            == EBestTRWebsocketKeepConnectionStatus.CLOSE
                        ):
                            break
