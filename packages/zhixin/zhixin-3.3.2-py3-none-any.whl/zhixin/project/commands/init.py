...
def project_init_cmd(
    ...
    with fs.cd(project_dir):
        if environment:
            ...
            update_project_comm("name=%s" % os.path.basename(project_dir))
                                        
        generator = None
        config = ProjectConfig.get_instance(os.path.join(project_dir, "zhixin.ini"))
        if ide:
            config.validate()
...
def init_base_project(project_dir):
    with fs.cd(project_dir):
        ...
        dir_to_readme = [
            ...
            (config.get("zhixin", "include_dir"), init_include_readme),
...
ZhiXin will compile them to static libraries and link into executable file.
...
|  |     |- Bar.c
|  |     |- Bar.h
|  |  |- library.json (optional, custom build options, etc) https://docs.ZhiXin-Semi.com/page/librarymanager/config.html
|  |
|  |--Foo
|  |  |- Foo.c
|  |  |- Foo.h
...
ZhiXin Library Dependency Finder will find automatically dependent
libraries scanning project source files.

More information about ZhiXin Library Dependency Finder
- https://docs.ZhiXin-Semi.com/page/librarymanager/ldf.html
""",
        )
...
def init_test_readme(test_dir):
    with open(os.path.join(test_dir, "README"), mode="w", encoding="utf8") as fp:
        fp.write(
            """
This directory is intended for ZhiXin Test Runner and project tests.
...
More information about ZhiXin Unit Testing:
- https://docs.ZhiXin-Semi.com/en/latest/advanced/unit-testing/index.html
""",
        )
...
def update_board_envs(project_dir, boards, extra_project_options, env_prefix):
    config = ProjectConfig(
        os.path.join(project_dir, "zhixin.ini"), parse_extra=False
    )
    used_boards = []
...
def update_project_comm(extra_project_options=None):
    if not extra_project_options:
        return
