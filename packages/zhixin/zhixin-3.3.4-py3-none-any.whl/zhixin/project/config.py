...
from zhixin.compat import MISSING, hashlib_encode_data, string_types
from zhixin.project import exception
from zhixin.project.options import ProjectOptions

CONFIG_HEADER = """
; ZhiXin Project Configuration File
;
"""
...
class ProjectConfigBase:
    ...
    def get_default_path():
        from zhixin import app  # pylint: disable=import-outside-toplevel

        return app.get_session_var("custom_project_conf") or os.path.join(
            os.getcwd(), "zhixin.ini"
        )
    ...
    def read(self, path, parse_extra=True):
        ...
        # load extra configs
        for pattern in self.get("zhixin", "extra_configs", []):
        ...
        if option_meta.sysenvvar:
            ...
            if not envvar_value and option_meta.oldnames:
                for oldoption in option_meta.oldnames:
                    envvar_value = os.getenv("ZHIXIN_" + oldoption.upper())
                    if envvar_value:
                        break
            ...
            if option == "__env__":
                if not parent_section.startswith("env:"):
                    raise exception.ProjectOptionValueError(
                        ...
                        "section that is not valid ZhiXin environment. Please "
    ...
    def default_envs(self):
        return self.get("zhixin", "default_envs", [])
    ...
    def validate(self, envs=None, silent=False):
        if not os.path.isfile(self.path):
            raise exception.NotZhiXinProjectError(os.path.dirname(self.path))
        ...
        for env in known_envs:
            ...
            if self.get(f"env:{env}", "monitor_raw", False) and self.get(
                ...
                self.warnings.append(
                    ...
                    "`zhixin.ini` file. The `monitor_filters` option will "
...
class ProjectConfigDirsMixin:
    def get_optional_dir(self, name):
        """
        Deprecated, used by zhixin-node-helpers.project.observer.fetchLibDirs
        ZhiXin IDE for Atom depends on zhixin-node-helpers@~7.2.0
        ZX Home 3.0 Project Inspection depends on it
        """
        return self.get("zhixin", f"{name}_dir")
