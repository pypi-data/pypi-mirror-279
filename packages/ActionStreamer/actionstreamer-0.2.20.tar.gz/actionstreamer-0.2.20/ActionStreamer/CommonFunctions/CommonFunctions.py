...
def log_to_console(message, agent_name=''):
...
def send_signed_request(ws_config: WebServiceConfig, method, url, path, headers=None, parameters=None, body=None):
    ...
    try:
        ...
        # Adjust request method accordingly
...
def get_hmac_signature(secret_key, method, path, headers, parameters, body=None):
...
def get_sha256_hash_for_file(file_path):
...
def get_clock_cycles_per_millisecond():
