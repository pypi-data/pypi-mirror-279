...
class p:
    ...
    def g(self, groupby_cols, agg_func):
        ...
        if self.df is not None:
            ...
            # STEP 1: Duplicate each column that needs to be aggregated as {col_name}_{func}, and delete the original {col_name}
            new_columns = {}
            for col, funcs in agg_func.items():
                if isinstance(funcs, list):
                    for func in funcs:
                        ...
                        new_columns[new_col_name] = func
                else:
                    new_col_name = f"{col}_{funcs}"
                    df_copy[new_col_name] = df_copy[col]
                    new_columns[new_col_name] = funcs

            # Drop the original columns that have been duplicated
            df_copy.drop(columns=list(agg_func.keys()), inplace=True)
            print(df_copy)
            # STEP 2: Do a simple group by and aggregate using the new column names
            self.df = df_copy.groupby(groupby_cols).agg(new_columns).reset_index()
    ...
    def gt(self, target_cols, agg_funcs):
        """Transform columns according to the specified aggregation functions."""
