...
class p:
    ...
    def g(self, groupby_cols, agg_func):
        ...
        if self.df is not None:
            ...
            agg_func = {col: (func if func != 'css' else css) for col, func in agg_func.items()}

            # Using a list of tuples to handle multiple aggregation functions
            agg_funcs = {col: [func] if isinstance(func, str) else func for col, func in agg_func.items()}

            self.df = self.df.groupby(groupby_cols).agg(agg_funcs).reset_index()
            self.df.columns = ['_'.join(col).strip() for col in self.df.columns.values]  # Flatten MultiIndex columns
            self.pr()
        else:
