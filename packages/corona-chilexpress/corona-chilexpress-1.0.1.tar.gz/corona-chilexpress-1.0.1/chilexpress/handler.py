...
class ChilexpressHandler:
        ...
        try:
            payload = {
                'header': {
                    ...
                    'labelType': api_settings.CHILEXPRESS['LYBEL_TYPE'],
                    'marketplaceRut': self.marketplace_rut,
                    'sellerRut': self.marketplace_rut,
                },
                'details': [{
                    'addresses': [
                        {
    ...
    def create_shipping(self, data):
        ...
        try:
            ...
            if response.get('errors', None) or response.get('statusCode', None):
