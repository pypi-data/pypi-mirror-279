...
class ChilexpressHandler:
    ...
    def get_default_payload(self, instance):
        ...
        try:
            payload = {
                'header': {
                    'certificateNumber': 0,
                    'customerCardNumber': api_settings.CHILEXPRESS['CARD_NUMBER'],
                ...
                'details': [{
                    'addresses': [
                        ...
                        {
                            'addressId': 0,
                            'countyCoverageCode': instance.address.commune.code,
                    ...
                    'contacts': [
                        ...
                        {
                            'name': instance.address.customer.full_name,
                            'phoneNumber': instance.address.customer.phone,
                            'mail': instance.address.customer.email,
                            'contactType': 'D'
                        }
                    ],
                    'packages': [
                        {
                            'weight': str(item.base_weight),
                            'height': item.height or 1,
                            'width': item.width or 1,
                            'length': item.length or 1,
                            'serviceDeliveryCode': api_settings.CHILEXPRESS['SERVICE_DELIVERY_CODE'],
                            'productCode': api_settings.CHILEXPRESS['PRODUCT_CODE'],
                            'deliveryReference': instance.id,
                            'groupReference': f'GRUPO {instance.id}',
    ...
    def create_shipping(self, data):
        ...
        try:
            ...
            if response['errors'] or response['statusCode']:
