...
class BaseDialect:
    ...
    @staticmethod
    def create_insert_sql(table: str, cols: Collection[str], placeholder: str = '%s') -> str:
    ...
    def limit_offset_sql(sql: str):
        return '{} LIMIT ? OFFSET ?'.format(sql)
    ...
    def truncate_table(table_name: str):
        return execute('TRUNCATE TABLE %s' % Dialect.get_dialect_str(table_name))
...
class MySQLDialect(BaseDialect):
    ...
    @staticmethod
    def create_insert_sql(table: str, cols: Sequence[str], placeholder: str = '%s') -> str:
        columns, placeholders = zip(*[('`{}`'.format(col), placeholder) for col in cols])
        return 'INSERT INTO `{}`({}) VALUES({})'.format(table, ','.join(columns), ','.join(placeholders))
...
class SQLiteDialect(MySQLDialect):
    ...
    @staticmethod
    def create_insert_sql(table: str, cols: Sequence[str], placeholder: str = '?') -> str:
        return MySQLDialect.create_insert_sql(table, cols, placeholder)
    ...
    def truncate_table(table_name: str) -> int:
        return execute('DELETE FROM `%s`' % table_name)
...
class Dialect:
    ...
    @staticmethod
    def create_insert_sql(table: str, cols: Collection[str]) -> str:
