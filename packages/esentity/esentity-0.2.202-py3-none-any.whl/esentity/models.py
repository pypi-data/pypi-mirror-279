...
def p_func(paths):
    return paths[current_app.config['AVAILABLE_LOCALE'].index(g.language)]
...
class Page(ElasticEntity):
    ...
    @classmethod
    def provider_by_context(cls, country, _exclude=[], _exclude_params={}, _exclude_tags=['closed'], _count=12, _page=1, _offset=None, _source=["title", "alias", "logo"], _aggs={}, _sorting=None, _locale=None, **params):
        filter_list =  [ 
            {"term": {"is_active": True}},
            {"term": {"is_draft": False}},
            {"term": {"is_sandbox": False}},
