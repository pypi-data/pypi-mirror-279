...
def replace_text(line, language):
    # pattern = r"\[([^|]+)\/([^|]+)\]>"
    pattern = r"\[([^/]+)/([^]]+)\]"
    match = re.search(pattern, line)
    if match:
        cn_line, en_line = match.groups()
        if language == "cn":
            return re.sub(pattern, cn_line, line)
        elif language == "en":
            return re.sub(pattern, en_line, line)
    return line


def parse_file(filename, language=None, job_language=None):
    with open(filename, "rb") as f:
        # file encoding is assumed to be utf8. Oh, yes.
        data = f.read().decode("utf8")
    return parse_feature(data, language, filename, job_language=job_language)


def parse_feature(data, language=None, filename=None, job_language=None):
    # ALL data operated on by the parser MUST be unicode
    assert isinstance(data, six.text_type)

    try:
        result = Parser(language, job_language=job_language).parse(data, filename)
...
class Parser(object):
    ...
    def __init__(self, language=None, variant=None, job_language=None):
        if not variant:
            variant = "feature"
        self.language = language
        self.job_language = job_language
    ...
    def parse(self, data, filename=None):
        ...
        for line in data.split("\n"):
            self.line += 1
            line = replace_text(line, self.job_language)
