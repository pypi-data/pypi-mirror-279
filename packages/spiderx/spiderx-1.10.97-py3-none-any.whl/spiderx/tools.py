...
class DOWN_M3U8():
    def __init__(self, 文件路径='test.mp4', 网址='', m3u8字符串='', 线程数=20,show_text=False, istest=False,显示解码=True,二进制=False,分离器=False,显示错误=True,headers=None,cookies=None,proxies=None,curl=False,try_num=10,cmd_show=1,timeout=30, 类型=1):
        # 链接地址回调 可以调整ts链接
        if 类型==1:
            self.url = 网址
        elif 类型==2:
        ...
        try:
            if self.m3u8字符串:
                ...
                try:
                    ...
                    return 下载文件_进度条(文件路径=self.fpath, 网址=self.url, 线程数=self.线程数, 多线程=True, 分段长度=10 * 1024,headers=self.headers,try_num=self.try_num)
                    ...
                    if 返回:
                        ...
                        if self.分离器:
                            ...
                            if self.二进制:
                                ...
                                cmd=f'"{self.ff.ffmpeg_path}" -i "{temp_mp4}" -y -c copy -bsf:a aac_adtstoasc "{self.fpath}" -loglevel error'
