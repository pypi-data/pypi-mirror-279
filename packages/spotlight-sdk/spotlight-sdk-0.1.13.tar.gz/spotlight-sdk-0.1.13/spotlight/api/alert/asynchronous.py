...
from spotlight.api.alert.__util import (
    ...
    _get_alert_results_request_info,
...
@async_data_request()
async def async_get_alert_results(start: int, end: int) -> List[Dict[str, Any]]:
    """
    Asynchronously get all results for each alert in time window.
    ...
    Returns:
        Dict[str, Any]: AlertResult response
    """
    request_info = _get_alert_results_request_info(start, end)
    return await __async_get_request(**request_info)


@async_data_request()
def async_get_alert_signal(start: int, end: int) -> List[Dict[str, Any]]:
    """
