...
class DynamicIndustry:
    ...
    def preview(self, trade_dt, adjust=False):
        ...
        all_stk = get_industry(trade_dt=trade_dt,
                               level=1,
                               sid=None,
                               adjust=adjust)
...
def get_industry_all(level=1, adjust=False):
    ...
    try:
        df = df.filter(
            func.substring(clss.CITICS_IND_CODE, 1, 2 + 2 * level) ==
            func.substring(ind_code.INDUSTRIESCODE, 1, 2 + 2 * level)).to_df()
    except:
        df = df.filter(
            func.substr(clss.CITICS_IND_CODE, 1, 2 + 2 *
                        level) == func.substr(ind_code.INDUSTRIESCODE, 1, 2 +
                                              2 * level)).to_df()
...
def get_industry_all_hk():
    """[summary]
    """
    clss = WIND_DB.hksharesciticsindustriesclass
    df = WIND_DB.query(clss.S_INFO_WINDCODE.label('sid'),
                       clss.CHANGE_DT.label('change_dt'),
                       clss.citics_ind_name.label('ind')).filter(
                           clss.change_type == 1).to_df()
