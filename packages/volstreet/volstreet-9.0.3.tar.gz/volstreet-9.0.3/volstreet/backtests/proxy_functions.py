...
def sleep_until_next_action(
    ...
    # # Below will be added in the future
    # tasks = kwargs.get("tasks_to_perform", [])
    #
    # # Perform all tasks before advancing the time
    # for task in tasks:
    #     task()

    interruption_condition = kwargs.get("interruption_condition")
    if interruption_condition is None:
        # If no interruption condition is provided, we will simply advance the time
        config.backtest_state = target_time
        ProxyPriceFeed.update_prices()
    else:
        # If an interruption condition is provided, we will keep advancing the time
        # by 1 minute until the condition is met or the interval is reached
