...
class PositionManager:
    ...
    def generate_all_calls(
        self, options_with_cache: bool, available_strikes: dict[str, list[int]]
    ) -> list[ActiveOption]:
        return self._generate_single_side_options(
            OptionType.CALL, available_strikes["CE"], options_with_cache
        )
    ...
    def generate_all_puts(
        ...
        return self._generate_single_side_options(
            OptionType.PUT, available_strikes["PE"], options_with_cache
        )
    ...
    def set_options(
        ...
        *,
        ...
        if calls is None or puts is None:
            ...
            if calls is None:
                calls = self.generate_all_calls(options_with_cache, available_strikes)
            if puts is None:
                puts = self.generate_all_puts(options_with_cache, available_strikes)
    ...
    def _act_on_recommended_qty(self, at_market: bool) -> dict[ActiveOption, float]:
        instructions = {
            option: {
    ...
    def enter_positions(self, at_market: bool) -> dict[ActiveOption, float]:
        ...
        return self._act_on_recommended_qty(at_market)

    def modify_positions(self, at_market: bool) -> dict[ActiveOption, float]:
        """To be used once the new recommended qty is set and there are old active positions."""
    ...
    def exit_positions(self, at_market: bool) -> None:
        ...
        for option in self.all_options:
            option.recommended_qty = 0
        self.adjust_recommended_qty()
        self._act_on_recommended_qty(at_market)

    def append_position_status(self) -> None:
...
class TrendPosition(PositionManager):
    ...
    def set_recommended_qty_optimized(
        ...
        all_greeks = self.get_option_chain_with_greeks(0.75)
