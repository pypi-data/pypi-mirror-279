...
from volstreet.strategies.strategy_modifiers import exit_handler
...
def delta_hedged_strangle(
    ...
    def interruption_condition():
        ...
        condition_1 = (
            (abs(delta_position.aggregate_greeks().delta) >= (0.15 * base_qty_shares))
            if interrupt
            else False
        )
    ...
    file_name = (
        ...
        f"{underlying.name.lower()}_delta_data\\"
        f"{date}.json"
    )

    delta_position.set_options(options_with_cache=use_cache)
    while current_time().time() < time(*exit_time):
        ...
        while not any(delta_position.exit_triggers.values()):
            delta_position.append_position_status()
            sleep_until_next_action(
                delta_interval_minutes,
                exit_time,
                tasks_to_perform=[
                    lambda: record_position_status(delta_position, file_name)
                ],
...
def trend_v2(
    ...
    trend_position.set_options(options_with_cache=True)

    # The file name for recording the position status
    date = current_time().strftime("%d-%m-%Y")
    file_name = (
        f"{ActiveSession.obj.userId}\\"
        f"{underlying.name.lower()}_trend_data\\"
        f"{date}.json"
    )
    ...
    while entries < max_entries and current_time().time() < exit_time_object:
        ...
        while current_time().time() < exit_time_object:
            trend_position.append_position_status()
            sleep_until_next_action(
                1,
                exit_time,
                tasks_to_perform=[
                    lambda: record_position_status(trend_position, file_name)
                ],
            )
            ltp = underlying.fetch_ltp()
            movement = (ltp - open_price) / open_price * 100
...
def reentry_straddle_old(
    ...
    strangle = most_equal_strangle(
        underlying, strike_offset, -strike_offset, range_of_strikes=1
    )
    ...
    call_avg_price, put_avg_price = execute_order(
        instrument=strangle, action=Action.SELL
    )
    notifier(
        f"{underlying.name} {strategy_tag} starting with "
        f"straddle {strangle} and prices {(call_avg_price, put_avg_price)}",
        notification_url,
        "INFO",
    )
    call_sl_price = call_avg_price * (1 + call_stop_loss)
    put_sl_price = put_avg_price * (1 + put_stop_loss)

    # A tuple of (in_trade, reattempts)
    call_reentries = call_reentries or reentries
    put_reentries = put_reentries or reentries
    status_dict = {
        strangle.call_option: (True, call_reentries),
        strangle.put_option: (True, put_reentries),
    }
...
def reentry_straddle(
    ...
    # Setting up the stop-losses in pct
    call_stop_loss = call_stop_loss or stop_loss
    put_stop_loss = put_stop_loss or stop_loss
    ...
    reentry_position.set_options(calls=[call_active_option], puts=[put_active_option])

    # Just checking if the active option in this scope is the same as the one in the reentry_position
    logger.info(
        f"The ids of the active options in this scope is {id(call_active_option), id(put_active_option)} "
        f"and in the reentry_position is {[id(opt) for opt in reentry_position.all_options]}"
    )
    ...
    call_active_option.stop_loss = call_active_option.avg_price * (1 + call_stop_loss)
    call_active_option.reentries = call_reentries or reentries

    put_active_option.avg_price = execution_details[put_active_option]
    put_active_option.stop_loss = put_active_option.avg_price * (1 + put_stop_loss)
        ...
        for option in reentry_position.all_options:
            ...
            if option.active_qty != 0:
                if option.fetch_ltp() >= option.stop_loss:
                    reentry_position.retain_existing_quantities()
            ...
            # If its inactive but has reattempts left, check for reentry
                ...
                if option.fetch_ltp() <= option.avg_price:
                    reentry_position.retain_existing_quantities()
                    option.recommended_qty = -quantity_in_shares
                    execution_details = reentry_position.modify_positions(
                        at_market=at_market
                    )
