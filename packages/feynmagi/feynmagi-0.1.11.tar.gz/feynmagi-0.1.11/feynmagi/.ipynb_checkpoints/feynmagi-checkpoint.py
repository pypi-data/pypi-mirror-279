...
import autollm  # Importer votre module
...
def handle_message(data):
    ...
    if cfg.actual_engine == "llm":
        ret=""
        for response_text in llmsapis.llmchatgenerator(cfg.session_message_history, temperature=0,stream=True):
            ret+=response_text
        ...
        if base64_string == "":
            ...
            return 
        for response_text in llmsapis.llimgenerator(prompt, temperature=0,stream=False, raw=False, image=base64_string):
            socketio.emit('response_token', {'token': response_text})
            if cfg.stop_event.is_set():  # Vérifiez si l'événement a été déclenché
                break
    else:
...
if __name__ == '__main__':
    ...
    # socketio.run(app, ssl_context=('../certificate/lisdev.extra.cea.fr+4.pem','../certificate/lisdev.extra.cea.fr+4-key.pem'), host='0.0.0.0', debug=True)
