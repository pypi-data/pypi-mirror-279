...
def execute_command(command):
    ...
    def run_command(command, results):
        try:
            ...
            filtered_error = "\n".join(line for line in error_message.splitlines() if not any(s in line for s in ["Descripteur non valide", "failed to get console mode for stderr"]))
...
results, status = execute_command(command)
