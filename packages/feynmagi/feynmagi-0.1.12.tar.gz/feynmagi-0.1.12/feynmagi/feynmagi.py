...
def help():
    llama_msg=""
    retcode,version=check_ollama_installed()
    if  retcode:
    ...
    if not retcode:
        ...
        if "phi3:latest" not in  models:
            model_msg="For best local LLM experience please pull test model : ollama run phi3 <button id='installphi3' class='icon-btn' title='Install ollama'><span class='bi bi-cloud-arrow-down-fill'></span></button>"
...
def handle_connect():
    ...
    retcode,version=check_ollama_installed()
    if  retcode:
        socketio.emit('error', {'message': 'Ollama is not installed ! ! ! please install it first'})
...
def pull_model():
    ...
    try:
        print("trying")
        result = subprocess.run(["ollama", "pull phi3"], capture_output=True, text=True)
...
def open_browser():
    time.sleep(5)  # Attendre une seconde pour s'assurer que le serveur est démarré
...
def main():
    ...
    if os.path.exists(cert_file) and os.path.exists(key_file):
        threading.Thread(target=lambda: socketio.run(app,ssl_context=(cert_file, key_file),host='0.0.0.0', debug=False)).start()
    else:
        threading.Thread(target=lambda: socketio.run(app,host='0.0.0.0', debug=False)).start()
