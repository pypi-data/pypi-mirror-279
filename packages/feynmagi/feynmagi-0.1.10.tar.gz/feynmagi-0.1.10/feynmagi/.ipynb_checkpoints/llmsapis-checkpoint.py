...
def mllmgenerator(image, messages, temperature=0, max_tokens=None, stream=True):
    headers = {
        "Content-Type": "application/json",
    }
    ...
    response = requests.post(API_ENDPOINT, headers=headers, data=json.dumps(data), stream=stream)
    ...
    if response.status_code == 200:
        for line in response.iter_lines():
            ...
            if line:
                if not stream:
                    yield json.loads(line)[cfg.json_str]
                else:
                    line = line.decode('utf-8').strip()
                    if line.startswith('data:'):
                        line = line[5:]  # Supprimer 'data:'
...
def mllm(imagaprompt,temperature=0):
    ret=""
    for response_text in mllmgenerator(prompt, temperature, stream=False):
        ret+=response_text
    return ret

# code llm

def cllmgenerator(image, messages, temperature=0, max_tokens=None, stream=True):
    headers = {
        "Content-Type": "application/json",
    }
...
def cllm(imagaprompt,temperature=0):
    ret=""
    for response_text in mllmgenerator(prompt, temperature, stream=False):
        ret+=response_text
    return ret
