...
class CheckPoint(Callback):
    def __init__(self, path:Path, run_on:Literal["epoch", "step"]="epoch", freq=1):
        self.path = path
        self.run_on = run_on
        self.freq = freq
        super().__init__()
    ...
    def on_step_end(self, batch: int, logs: Logs):
        if self.run_on == "step" and batch % self.freq == 0:
            self.save(logs)
    ...
    def on_train_start(self, logs: Logs):
        ...
        logs.state = TrainState(step=step, apply_fn=logs.state.apply_fn, params=params, tx=logs.state.tx, opt_state=opt_state)