...
class ControllerApi:
    ...
    async def get_current_robot_controller_state(
        self,
        cell: Annotated[StrictStr, Field(description="Unique identifier addressing a cell in all API calls. Since the cell identifier is used as Kubernetes namespace, the following rules apply: * Contain only lowercase alphanumeric characters or '-'. * Start with an alphanumeric character. * End with an alphanumeric character. * A maximum length of 63 characters. * The identifier \"kube-system\", \"default\" and \"wandelbots\" are reserved and cannot be used. ")],
        controller: Annotated[StrictStr, Field(description="Unique identifier addressing a controller in the cell.")],
        ...
        :type cell: str
        :param controller: Unique identifier addressing a controller in the cell. (required)
        :type controller: str
    ...
    async def get_virtual_robot_configuration(
        ...
        Receive the configuration for a virtual robot controller. Used to configure a virtual robot controller via [addRobotController](addRobotController). 
    ...
    async def set_default_mode(
        ...
        Switch between monitor and control usage as default for a robot controller. Monitoring mode is used to read information from the robot controller and control mode is used to command the robot system. As long as the robot controller is connected via network monitoring mode is always possible. To switch to control mode the robot controller must be in `automatic` or `manual` operating mode. If the robot controller is in `manual` operating mode, you have manually confirm the control usage activation on the robot control panel. This manual confirmation can't be replaced with this API. Without manual confirmation the robot controller will stay in monitor mode.  The robot system will try to activate the required operation mode for the requested usage unless no active call requires a different mode. Note: Some robot controllers prevent the external activation of automatic operating mode. In this case, changing the operating mode manually at the robot controller is mandatory. If a mode change is not possible, the response lists reasons for the failed change. 
    ...
    @validate_call
    async def stream_free_drive(self, cell: Annotated[StrictStr, Field(description="Unique identifier addressing a cell in all API calls. Since the cell identifier is used as Kubernetes namespace, the following rules apply: * Contain only lowercase alphanumeric characters or '-'. * Start with an alphanumeric character. * End with an alphanumeric character. * A maximum length of 63 characters. * The identifier \"kube-system\", \"default\" and \"wandelbots\" are reserved and cannot be used. ")], controller: Annotated[StrictStr, Field(description="Unique identifier addressing a controller in the cell.")], request: Optional[StrictInt]) -> AsyncGenerator[RobotControllerState, None]:  # noqa: E501
        ...
        url_schema = "ws://{self.api_client.configuration.host}"+ path
        url_schema =  url_schema.format(self=self).replace("http://", "")

        attributes = vars(request)
        if len(attributes) > 0:
            url_schema = url_schema + "?"
            url_schema = url_schema + '&'.join([f'{k}={v}' for k, v in attributes.items() if v is not None])
        ...
        async with websockets.connect(full_url.url, open_timeout=10) as websocket:
            try:
                async for response in websocket:
                    ...
                    yield RobotControllerState.from_dict(data)
            except websockets.exceptions.ConnectionClosed:
                return


    @validate_call
    async def stream_mode_change(self, cell: Annotated[StrictStr, Field(description="Unique identifier addressing a cell in all API calls. Since the cell identifier is used as Kubernetes namespace, the following rules apply: * Contain only lowercase alphanumeric characters or '-'. * Start with an alphanumeric character. * End with an alphanumeric character. * A maximum length of 63 characters. * The identifier \"kube-system\", \"default\" and \"wandelbots\" are reserved and cannot be used. ")], controller: Annotated[StrictStr, Field(description="Unique identifier addressing a controller in the cell.")]) -> AsyncGenerator[ModeChangeResponse, None]:  # noqa: E501
        ...
        async with websockets.connect(full_url.url, open_timeout=10) as websocket:
            try:
                async for response in websocket:
                    data  = json.loads(response)["result"]
                    yield ModeChangeResponse.from_dict(data)
            except websockets.exceptions.ConnectionClosed:
                return


    @validate_call
    async def stream_robot_controller_state(self, cell: Annotated[StrictStr, Field(description="Unique identifier addressing a cell in all API calls. Since the cell identifier is used as Kubernetes namespace, the following rules apply: * Contain only lowercase alphanumeric characters or '-'. * Start with an alphanumeric character. * End with an alphanumeric character. * A maximum length of 63 characters. * The identifier \"kube-system\", \"default\" and \"wandelbots\" are reserved and cannot be used. ")], controller: Annotated[StrictStr, Field(description="Unique identifier addressing a controller in the cell.")], request: Optional[StrictInt]) -> AsyncGenerator[RobotControllerState, None]:  # noqa: E501
