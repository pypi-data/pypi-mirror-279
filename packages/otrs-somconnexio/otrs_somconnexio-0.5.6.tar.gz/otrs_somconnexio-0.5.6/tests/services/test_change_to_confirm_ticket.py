...
class TestChangeToConfirmTicket:
    ...
    def _execute_and_assert(self, mocker, queue_id):
        ...
        confirmation_dynamic_fields_fixture = {"enviatOdooOV": True}
        ...
        with mocker.patch.object(
            ChangeConfirmTicketInstance,
            "_confirmation_dynamic_fields",
            return_value=confirmation_dynamic_fields_fixture,
        ):
            ChangeConfirmTicketInstance.confirm(self.TOKEN)

        DynamicFieldMock.assert_called_once_with("IDOV", search_patterns=self.TOKEN)
        ...
        OTRSClientMock.return_value.client.ticket_update.assert_called_once_with(
            TICKET_ID,
            article=None,
            dynamic_fields=confirmation_dynamic_fields_fixture,
            State=None,
        )
    ...
    def test_confirmation_dynamic_fields(self):
        return_confirmation_dynamic_fields = (
            ChangeToConfirmTicket()._confirmation_dynamic_fields()
        )
        assert vars(return_confirmation_dynamic_fields[0])["name"] == "enviatOdooOV"
        assert vars(return_confirmation_dynamic_fields[0])["value"]
