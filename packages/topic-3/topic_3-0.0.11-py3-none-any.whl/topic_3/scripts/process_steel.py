...
def main(excel_filepath, excel_filename):
    raw_data = pd.read_excel(os.path.join(excel_filepath, excel_filename))
    ...
    csv_filename = "train.csv" if "TRain" in excel_filepath else "test.csv"
    train_df.to_csv(os.path.join(excel_filepath, csv_filename))
    ...
    f = open(os.path.join(excel_filepath, "list.txt"), 'w')
    for col in raw_data.columns:
        f.write(col + '\n')
    f.close()
...
if __name__ == '__main__':
    excel_filepath = '../../data/steel/TRain/Task1(拉速1.2)/板柸1_1数据'
    excel_filename = "板坯1_1.xlsx"
    main(excel_filepath, excel_filename)
