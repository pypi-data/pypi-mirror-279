...
from fvmd.utils.basic import print_
...
def get_box2d_from_mask(mask, normalize=False):
    ...
    B, C, H, W = mask.shape
    assert(C==1)
    xy = fvmd.utils.basic.gridcloud2d(B, H, W, norm=False, device=mask.device) # B, H*W, 2
...
def depth2pointcloud(z, pix_T_cam):
    ...
    device = z.device
    y, x = fvmd.utils.basic.meshgrid2d(B, H, W, device=device)
