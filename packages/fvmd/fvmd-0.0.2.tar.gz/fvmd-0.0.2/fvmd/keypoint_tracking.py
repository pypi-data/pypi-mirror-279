...
import numpy as np
import utils.saverloader as saverloader
...
def tracking_fullseq(model, rgbs, sw, N = 400, iters=8, S_max=16, name='sample'):
    ...
    while not done:
        ...
        if sw is not None and sw.save_this and cur_frame == 0:
            sw.summ_traj2ds_on_rgbs('outputs/'+name, trajs_e[0:1], utils.improc.preprocess_color(rgb_seq[0:1]),
...
if __name__ == '__main__':
    ...
    keypoint_tracking(log_dir=args.log_dir, dataset_gen=args.dataset_gen, dataset_gt=args.dataset_gt, v_stride=args.v_stride)
