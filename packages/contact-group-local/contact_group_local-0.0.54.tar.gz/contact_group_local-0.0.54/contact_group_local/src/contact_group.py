...
class ContactGroups(GenericMapping, metaclass=MetaLogger, object=CONTACT_GROUP_PYTHON_PACKAGE_CODE_LOGGER_OBJECT):
    def __init__(self, default_schema_name: str = DEFAULT_SCHEMA_NAME, default_entity_name1: str = DEFAULT_ENTITY_NAME1,
        ...
        # TODO self.groups_local = 
        self.group_local = GroupsLocal(is_test_data=is_test_data)
    ...
    def insert_link_contact_group_with_group_local(
            self, *, contact_id: int,
            groups_dicts: list[dict], lang_code: LangCode = None, parent_group_id: int = None,
            is_interest: bool = None, image: str = None,
            mapping_data_dict: dict = None) -> list[dict]:

        groups_array = []
        for group_num, group_dict in enumerate(groups_dicts):
            # check if group already exists
            group_id = self.__get_group_id_by_title(group_title=group_dict.get("name"))
            ...
            if not group_id:
                ...
                lang_code = LangCode.detect_lang_code_restricted(
                    text=groups_dicts[group_num].get("name"),
                ...
                upsert_information = self.group_local.upsert(
    ...
    def __get_group_id_by_title(self, group_title: str) -> int or None:
        group_dict = self.group_local.select_one_dict_by_column_and_value(
