...
class _IdeEditorHandler extends TerminalRunner {
  ...
  build(){
    ...
    if(this.isVert) this.resized = false
  ...
  resizeVerticalTerm(mark=true){
    if(!this.isVert || this.resized) return
  ...
  setupAceEditor() {
    ...
    const options = {
        ...
        minLines: this.isVert ? this.maxIdeLines : 6,
    ...
    return withPyodideAsyncLock('Validate', async(e)=>{
      ...
      try{
        ...
        if(!options.stdErr){
          ...
          options.withStdOut    = !this.deactivateStdoutForSecrets
          options.isPublic      = !this.showOnlyAssertionErrorsForSecrets
          options.autoLogAssert = this.autoLogAssert
          // before Meta:
        ...
        */
        ...
        options.stdErr = youAreInTroubles(e)
        this.terminal.error(options.stdErr)
        throw e
