...
class RecognitionModel(ABC):
    ...
    @property
    def speaker_labeling(self) -> bool:
        """
        Set speaker labeling execution (if available)
        """
        return self._get_config_parameter('speaker_labeling')

    @speaker_labeling.setter
    def speaker_labeling(self, value: bool):
        """
        Get speaker_labeling value (if available)
        """
        self._set_config_parameter('speaker_labeling', value)
