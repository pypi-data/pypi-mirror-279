...
class Actor(actions.Parametrizable, Permittable, metaclass=ActorMetaClass):
    ...
    def detail_link(cls, obj, ar):
        ...
        It should contain a clickable link that opens the :term:`detail window`.
    ...
    def get_table_summary(cls, obj, ar):
        ...
        detail form when :attr:`display_mode` is DISPLAY_MODE_SUMMARY or (2)
        in a grid.
        ...
        # assert not "&lt;" in p
        toolbar = sar.plain_toolbar_buttons()
        if len(toolbar):
            # p += "<br/>"
            # p += " | "
            if p:
