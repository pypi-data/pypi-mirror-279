...
class GridColumn(jsgen.Component):
    ...
    def __init__(self, layout_handle, index, editor, **kw):
        ...
        if settings.SITE.use_gridfilters and editor.gridfilters_settings:
            if isinstance(editor, FieldElement) and not isinstance(
                editor.field, fields.VirtualField
            ):
        ...
        if isinstance(editor, FieldElement):
            ...
            def fk_renderer(fld, name):
                ...
                if rpt is not None and rpt.detail_action is not None:
                    if rpt.detail_action.get_view_permission(get_user_profile()):
...
class ButtonElement(LayoutElement):
    ...
    def __init__(self, lh, name, a, **kwargs):
        ...
        kwargs.update(
            text=a.label, handler=js_code(a.js_handler), scope=js_code("this")
        )
...
class FieldElement(LayoutElement):
    ...
    def as_plain_html(self, ar, obj):
        ...
        if self.layout_handle.layout.label_align == layouts.LABEL_ALIGN_TOP:
            yield E.br()
        input_classes = (
            "form-control " + self.input_classes
            if self.input_classes is not None
            else "form-control"
        )
    ...
    def get_column_options(self, **kw):
        ...
        if self.field.help_text is not None:
            ...
            help_text = "(%s.%s) " % (
                self.layout_handle.layout._datasource,
                self.field.name,
            )
        else:
            help_text = ""
    ...
    def value2html(self, ar, v, **cellattrs):
        ...
        if self.field.primary_key:
            ...
            # elem = ar.obj2html(v, self.format_value(ar, v))
            #
            url = ar.get_detail_url(ar.actor, v)
            # 20171227 in extjs6 above line was:
            # url = ar.pk2url(v)
            if url is not None:
                elem = ar.renderer.clickable_link(
                    None, self.format_value(ar, v), href=url
                )
                return E.td(elem)
                # return E.td(E.a(self.format_value(ar, v), href=url),
                #             **cellattrs)
...
class TextFieldElement(FieldElement):
    ...
    def as_plain_html(self, ar, obj):
        ...
        yield E.div(
            ...
            E.textarea(
                text, rows=str(self.preferred_height), **{"class": "form-control"}
            ),
            **{"class": "form-group"},
        )
    ...
    def value2html(self, ar, v, **cellattrs):
        ...
        if isinstance(v, SafeString) and v:
            top = fromstring(v)
            # xv = html2xhtml(v)
            # # v = v.decode('utf-8')
            # # top = E.fromstring(v)
    ...
    def format_value(self, ar, v):  # new since 20161120
        if self.format == "html" and v:
            if etree.iselement(v):
                return html2rst(v)
            return html2text(v)
        return super().format_value(ar, v)
...
class DateTimeFieldElement(FieldElement):
    ...
    def value2html(self, ar, v, **cellattrs):
        if v is None:
            ...
            v = fds(v) + " " + v.strftime(settings.SITE.time_format_strftime)
...
class DisplayElement(FieldElement):
    ...
    def __init__(self, *args, **kw):
        ...
        if (
            hasattr(fld, "max_length") and fld.max_length is not None
        ):  # might be explicit 0
            # if fld.max_length == 0:
            ...
            if etree.iselement(v) and v.tag == "div":
                if etree.iselement(v):
                    ...
                    if v.tag == "td":
                        return v
                return E.td(v, **cellattrs)
                # return E.td(*[child for child in v.iter()], **cellattrs)
            return E.td(v, **cellattrs)
        except Exception as e:
            logger.error(e)
    ...
    def as_plain_html(self, ar, obj):
        ...
        if etree.iselement(value):
            ...
            for x in super().as_plain_html(ar, obj):
                yield x
...
class SingleRelatedObjectElement(DisplayElement):
    ...
    def get_label(self):
        return (
            self._label or self.relobj.model._meta.verbose_name or self.relobj.var_name
        )
...
class HtmlBoxElement(DisplayElement):
    ...
    def value2html(self, ar, v, **cellattrs):
        if isinstance(v, SafeString) and v:  # added 20240504
            v = fromstring(v)
        # elif isinstance(v, str) and v.startswith("<"):
        #     # added 20181102, expecting  side effects.
        #     v = fromstring(v)
        return super().value2html(ar, v, **cellattrs)
    ...
    def as_plain_html(self, ar, obj):
        ...
        elif isinstance(value, SafeString) and value:  # added 20240504
...
class StoryElement(LightWeightContainer):
    ...
    Used to display objects as plain html generated by
    :meth:`as_summary_row <lino.core.fields.TableRow.as_summary_row>` from
...
class ManyRelatedObjectElement(HtmlBoxElement):
    def __init__(self, lh, relobj, **kw):
        ...
        def f(obj, ar):
            # return "<p>{}</p>".format(qs2summary(ar, getattr(obj, name).all()))
...
class Container(LayoutElement):
    ...
    def as_plain_html(self, ar, obj):
        if self.vertical:
            ...
            for e in self.elements:
                cell = E.td(
                    *tuple(e.as_plain_html(ar, obj)), style="vertical-align: top;"
                )
                tr.append(cell)
    ...
    def get_view_permission(self, user_type):
        ...
        if not super(Container, self).get_view_permission(user_type):
            return False
        for e in self.elements:
            if (not isinstance(e, Permittable)) or e.get_view_permission(user_type):
...
class Panel(Container):
    ...
    *all* its children are vflex (if vflex and non-vflex elements are
    ...
    def __init__(self, layout_handle, name, vertical, *elements, **kw):
        ...
        if w == 0:
            raise Exception(
                "{} has preferred_width 0 ({} children: {})".format(
...
class GridElement(Container):
    ...
    def __init__(self, layout_handle, name, rpt, *columns, **kw):
        ...
        layout_handle.ui.renderer.add_help_text(
            kw, rpt.help_text, rpt.title or rpt.label, rpt.app_label, rpt.actor_id
        )
...
class TabPanel(Panel):
    ...
    def as_plain_html(self, ar, obj):
        ...
        for e in self.elements:
            tab = E.li()
            tab.append(
                E.a(str(e.get_label()), data_toggle="tab", href="#" + e.ext_name)
            )
            nav.append(tab)
...
def create_layout_panel(lh, name, vertical, elems, **kwargs):
    ...
    if name == "main":
        if isinstance(lh.layout, ColumnsLayout):
            e = GridElement(lh, name, lh.layout._datasource, *elems, **pkw)
            ...
            if len(elems) == 1 or vertical:
                e = DetailMainPanel(lh, name, vertical, *elems, **pkw)
            else:
...
def create_layout_element(lh, name, **kw):
    ...
    if isinstance(de, (ManyRelatedObjectsDescriptor, ForeignRelatedObjectsDescriptor)):
        ...
        if isinstance(lh.layout, FormLayout):
            ...
            if lh.ui.renderer.extjs_version is not None:
                if de.label is not None:
                    js = (
                        "Lino.show_in_own_window_button(Lino.%s)"
            ...
            #     return e
                ...
                raise Exception("Invalid display_mode %r" % dm)
