...
class Table(AbstractTable):
    ...
    """Set this to `False` if this actor should *not* become the
    model's default table.

    """

    live_panel_update = False
    """Update this table panel on row modification when visible in react UI"""
    ...
    def get_screenshot_requests(self, language):
        ...
        for u in settings.SITE.user_model.objects.filter(language=language):
            if (
                u.user_type
                and u.user_type.name in self.screenshot_profiles
                and not u.user_type in profiles2user
            ):
    ...
    #                 if ba.action.show_in_toolbar and not self.get_row_permission(obj, ar, state, ba):
    #                 # if ba.action.show_in_toolbar and not obj.get_row_permission(u,state,ba.action):
    #                 # if a.show_in_toolbar and not a.get_action_permission(ar.get_user(),obj,state):
    ...
    def get_actor_label(self):
        ...
        if isinstance(self.model, str):
            # print("20231104", settings.SITE.is_hidden_app(self.app_label))
            raise Exception("20231104 {}.model is {!r}".format(self, self.model))
    ...
    def class_init(self):
        if self.model is not None:
            ...
            if isinstance(model, UnresolvedModel):
                ...
                raise Exception("Invalid model {} on table {}".format(self.model, self))
            self.model = model
        ...
        if self.order_by is not None:
            if not isinstance(self.order_by, (list, tuple)):
                raise Exception(
                    "%s.order_by is %r (must be a list or tuple)"
                    % (self, self.order_by)
                )
            if False:
                ...
                for fieldname in self.order_by:
                    ...
                    try:
                        ...
                        raise Exception(
                            "Unknown fieldname %r in %s.order_by" % (fieldname, self)
                        )
    ...
    def make_disabled_fields(cls, obj, ar):
        ...
        if obj is not None and ar is not None:
            ...
            for ba in cls.get_button_actions(parent):
                a = ba.action
                if a.action_name and a.show_in_toolbar:
    ...
    def get_request_queryset(self, ar, **filter):
        # **filter is allowed only on abstract models.
        ...
        def apply(qs):
            ...
            if ar.known_values:
                ...
                d = {}
                for k, v in ar.known_values.items():
                    if v is None:
                        d[k + "__isnull"] = True
                    else:
                        # d[k+"__exact"] = v
        ...
        if self.model._meta.abstract:
            # Django's UNION doesn't support count()
            # (Django ticket `#28281 <https://code.djangoproject.com/ticket/28281>`__)
            # and doesn't work on mysql (is this still true?).
            # So here we emulate a union and return an iteration over the
            # different models that implement the abstract model. One limitation
            # is of course that they are ordered only within their model. Used
            # for vat.IntracomPurchases and vat.IntracomSales
            def func():
    ...
    #     # raise Exception("20240321 get_queryset", self)
