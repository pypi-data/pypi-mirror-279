...
def fof_routes(self: BasicServer):
    ...
    def fof_page(user_name, **kwarg):
        if flask.session.get('logged_in') and flask.session.get('username') == user_name:
            if not flask.session.get("fof_list"):
                ...
                ).drop_duplicates(keep="last").dropna().set_index("产品编号").sort_index()
                products = products.iloc[[x for x in range(len(products.index)) if not 65 <= ord(products.index[x][-1]) <= 122]]
                _products: list = [" ".join([x, y]) for x, y in zip(products.index, products["产品名称"])]
    ...
    def fof_analysis(user_name):
        if not flask.request.args.get("fof"):
            ...
            os.makedirs(f"outputs/FOF_Analysis/{fof_code}", exist_ok=True)
