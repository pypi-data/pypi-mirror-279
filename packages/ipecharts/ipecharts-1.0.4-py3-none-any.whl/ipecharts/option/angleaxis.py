...
The angle axis in Polar Coordinate.
    ...
    _model_name = Unicode("AngleAxisModel").tag(sync=True)
    ...
    polarIndex = Float(None, allow_none=True, help="""The index of angle axis in Polar Coordinate. The first axis is used by default.""").tag(sync=True)
    
    startAngle = Float(None, allow_none=True, help="""Starting angle of axis. 90 degrees by default, standing for top position of center. 0 degree stands for right position of center.


The following shows an example with `startAngle` of 45 deg.""").tag(sync=True)
    
    endAngle = Float(None, allow_none=True, help="""> Since `v5.5.0`


Ending angle of axis. `null` by default, standing for a whole circle.


The following shows an example with `endAngle` of -180 deg.""").tag(sync=True)
    
    clockwise = Bool(None, allow_none=True, help="""Whether the positive position of axis is clockwise. True for clockwise by default.


The following shows an example with clockwise as `false`.""").tag(sync=True)
...
* `'category'`
 Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](#series.data) or [dataset.source](#dataset.source), or can be specified via [angleAxis.data](#angleAxis.data).
* `'time'`
...
`value` is an object, containing the `min` value and `max` value of the data. This function should return the max value of axis, or return `null`/`undefined` to make echarts use the auto calculated max value (`null`/`undefined` return is only supported since `v4.8.0`).""").tag(sync=True)
    ...
    scale = Bool(None, allow_none=True, help="""It is available only in numerical axis, i.e., [type](#angleAxis.type): 'value'.
...
This configuration item is unavailable when the [min](#angleAxis.min) and [max](#angleAxis.max) are set.""").tag(sync=True)
...
It is available only for axis of [type](#angleAxis.type) 'value' or 'time'.""").tag(sync=True)
...
This is unavailable for category axis. Timestamp should be passed for [type](#angleAxis.type): 'time' axis. Logged value should be passed for [type](#angleAxis.type): 'log' axis.""").tag(sync=True)
    
    logBase = Float(None, allow_none=True, help="""Base of logarithm, which is valid only for numeric axes with [type](#angleAxis.type): 'log'.""").tag(sync=True)
...
Minor split lines of axis in the [grid](#grid) area。It will align to the [minorTick](#angleAxis.minorTick)""").tag(sync=True)
    ...
    data = Any(None, allow_none=True, help="""Category data, available in [type](#angleAxis.type): 'category' axis.
...
See [this example](https://echarts.apache.org/examples/en/editor.html?c=bar-animation-delay) for more information.""").tag(sync=True)
    ...
    zlevel = Float(None, allow_none=True, help="""`zlevel` value of all graphical elements in Angle Axis.
...
Canvases with bigger `zlevel` will be placed on Canvases with smaller `zlevel`.""").tag(sync=True)
    ...
    z = Float(None, allow_none=True, help="""`z` value of all graphical elements in Angle Axis, which controls order of drawing graphical components. Components with smaller `z` values may be overwritten by those with larger `z` values.
