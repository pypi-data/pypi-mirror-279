...
```
    ...
    type = Unicode("custom", allow_none=True, ).tag(sync=True)
...
* `'cartesian2d'`
 ...
 Use a two-dimensional rectangular coordinate (also known as Cartesian coordinate), with [xAxisIndex](#series-custom.xAxisIndex) and [yAxisIndex](#series-custom.yAxisIndex) to assign the corresponding axis component.


* `'polar'`


 Use polar coordinates, with [polarIndex](#series-custom.polarIndex) to assign the corresponding polar coordinate component.


* `'geo'`


 Use geographic coordinate, with [geoIndex](#series-custom.geoIndex) to assign the corresponding geographic coordinate components.


* `'calendar'`


 Use calendar coordinates, with [calendarIndex](#series-custom.calendarIndex) to assign the corresponding calendar coordinate components.
...
* `'none'`
 ...
 Do not use coordinate system.""").tag(sync=True)
    ...
    renderItem = Any(None, allow_none=True, help="""`custom series` requires developers to write a render logic by themselves. This render logic is called [renderItem](#series-custom.renderItem).
...
Moreover, [api.style(...)](#series-custom.renderItem.arguments.api.style) method can be used to set style. It provides not only the style settings specified in [series.itemStyle](#series-custom.itemStyle), but also the result of visual mapping. This method can also be called like `api.style({fill: 'green', stroke: 'yellow'})` to override those style settings.""").tag(sync=True)
...
All these series have default value `true` except pictorialBar and custom series. Set it to `false` if you don't want to clip.""").tag(sync=True)
    ...
    zlevel = Float(None, allow_none=True, help="""`zlevel` value of all graphical elements in custom series.
...
Canvases with bigger `zlevel` will be placed on Canvases with smaller `zlevel`.""").tag(sync=True)
    ...
    z = Float(None, allow_none=True, help="""`z` value of all graphical elements in custom series, which controls order of drawing graphical components. Components with smaller `z` values may be overwritten by those with larger `z` values.
