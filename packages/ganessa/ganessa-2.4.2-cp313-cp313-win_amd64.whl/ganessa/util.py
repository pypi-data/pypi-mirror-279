...
def hhmmss(seconds: float, rounded: bool = False, days: Union[bool, str] = False) -> str:
    ...
    if days and hours > 23:
        ...
        hours %= 24
    return f"{sign}{hours:02d}:{m%60:02d}:{int(seconds - 60*m):02d}"
...
def profilise_folder(folder: str, create: bool = False) -> str:
    ...
    drive, root, relative_folder = OP.normpath(folder).split("\\", maxsplit=2)
...
def user_writeable(folder: str, create: bool = True) -> str:
    ...
    filetemp = OP.join(folder, "tmp_checkwriteable.tmp")
    try:
        with open(filetemp, "w", encoding="utf-8") as ftest:
...
class FichInfo:
    ...
    def banner(self) -> str:
        ...
        content = [self.nom, self.vers, '- Python', self.pyvers, '-', bits, 'bits']
        try:
            sub_module = self.sub_module
            if sub_module is not None and sub_module.nom != self.nom:
                content[2:2] = ["-", sub_module.nom, sub_module.vers]
        except AttributeError:
            pass
        return ' '.join(content)
...
def tsadd(t1: VectF, v1: VectF, n1: int,
    ...
    while True:
        ...
        if k1 == n1 or k2 == n2:
            break
    return np.array(tr), np.array(vr), len(tr)
...
def update_package(package_name, minvers='', pypi_name=None, deps=False,
    ...
    try:
        ...
        with open(tmp, 'w', encoding="utf-8") as ftest:
            ftest.write('Folder available for writing!')
