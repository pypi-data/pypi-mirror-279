...
@app.command()
def add(pkg: typing_extensions.Annotated[str, typer.Argument()] = '', name=None, path='.', args=''):
    ...
    os.system(f'dektools cmd retry pdm lock -p {path_scripts_item}')
    command_record = 'dekvenv add '
    if pkg:
        cmd = f'dektools cmd retry pdm add "{pkg}" {args} -v -p {path_scripts_item}'
        ...
        cmd = f'dektools cmd retry pdm add file:///{file} {args} -v -p {path_scripts_item}'
...
def pop(name: str = '', pkg: str = ''):
    ...
    if os.path.exists(path_scripts_item):
        shell_wrapper(f'dektools cmd retry pdm remove "{pkg}" -v -p {path_scripts_item}')
    else:
        raise
