...
@click.option("--generate", 'only_generate', 
              is_flag=True, 
              default=False, 
...
def authenticate(only_generate: bool):
    ...
    if only_generate:
        click.secho("🚨 Mode is set to generate only. The key will not be saved on your computer. At the end of the authentification, the key will be shown.", fg="cyan")
    else:
        click.secho("👋 Welcome!")
    ...
    if not only_generate:
        ...
        click.secho("--------------------------------------------------------------------\n")
        click.secho("⚠️ The value above is your API key. It won't be show again and the key has not been saved on your computer.", bg="red", fg="white", bold=True)
        click.echo("Use the env `HECTIQLAB_API_KEY` to authenticate with this key or visit https://docs.hectiq.ai/authenticate.html to learn how to use the key.")
        return
