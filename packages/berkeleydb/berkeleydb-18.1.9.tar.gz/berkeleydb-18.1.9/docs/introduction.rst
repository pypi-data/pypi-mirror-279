...
.. function:: py_limited_api()

   Returns ``None`` or an integer describing the minimum supported
   Python Stable ABI. If ``None``, the module is not compiled with
   Stable ABI and can not be used with a different Python version.
   When not ``None``, the value of ``berkeleydb.py_limited_api()``
   can be easily interpreted using something like
