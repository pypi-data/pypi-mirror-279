...
from comtypes.client._events import CreateEventReceiver
from comtypes.client import GetModule,dynamic,CreateObject
...
class BaseEvents():
    ...
    def isduringdt(self,symbol,dt):
        if ("CFFEX.I" in symbol and dt<datetime.strptime("2016-1-2 01:00", '%Y-%m-%d %H:%M').replace(tzinfo=self.tz)+timedelta(hours=8)) \
            or ("CFFEX.T" in symbol and dt<datetime.strptime("2020-7-19 01:00", '%Y-%m-%d %H:%M').replace(tzinfo=self.tz)+timedelta(hours=8)):
                        ...
                        if basedata['DateTime']<=datatemp[-1]['DateTime']:
                            ...
                            if datatype==2:
                                ...
                                if datatype==4:
                                    ...
                                    #datetime.strptime(str(basedata["DateTime"].date())+str(basedata["DateTime"].hour)+":"+str(basedata["DateTime"].minute)+":00" ,'%Y-%m-%d%H:%M:%S').replace(tzinfo=self.tz)+timedelta(minutes=interval-1)
                                if datatype==5:
                                ...
                                if len(line)>0 and te["DateTime"]<=line[-1]['DateTime']:
                                    ...
                                    if "TC.O" in symbol:
                                        if linetype==800 or linetype==820:
                                            itemkeys=['DateTime','Symbol','AnnualTradeday','TheoVal','IntVal','ATV','ExtVal','TV','IV','MIV','CPIV','AIV','BIV','Delta','Gamma','Vega','Theta','Rho','LR','RealLR',
                                            'OPR','ROI','BER','Charm','Vanna','Vomma','Speed','Zomma','Last','UnderlyingPrice','Volume','OI','Bid','Bid1','Bid2','Bid3','Bid4','Bid5',
                                            'Bid6','Bid7','Bid8','Bid9','BidVolume','BidVolume1','BidVolume2','BidVolume3','BidVolume4','BidVolume5','BidVolume6','BidVolume7','BidVolume8',
                                            'BidVolume9','Bid_UpdateDatetime','Bid1_UpdateDatetime','Bid2_UpdateDatetime','Bid3_UpdateDatetime','Bid4_UpdateDatetime','Ask','Ask1','Ask2',
                                            'Ask3','Ask4','Ask5','Ask6','Ask7','Ask8','Ask9','AskVolume','AskVolume1','AskVolume2','AskVolume3','AskVolume4','AskVolume5','AskVolume6',
                                            'AskVolume7','AskVolume8','AskVolume9','Ask_UpdateDatetime','Ask1_UpdateDatetime','Ask2_UpdateDatetime','Ask3_UpdateDatetime','Ask4_UpdateDatetime',
                                            'us','us_datetime','us_p','us_bp1','us_sp1','uf','uf_datetime','uf_p','uf_bp1','uf_sp1','usf','usf_datetime','usf_p']
                                        if linetype==9 or linetype==10 or linetype==19:
                                            itemkeys=['DateTime','Symbol','Delta','Gamma', 'Rho', 'Theta',  'Vega','TV','ATV',"IV",'CPIV']
                                    else:
                                        if linetype==800 or linetype==820:
                                            if "TC.F" in symbol:
                                                itemkeys=["DateTime","Symbol","AnnualTradeday","ATV","ExtVal","TV","IV","Last","CSkew","PSkew","CIV25D","PIV25D","CIV10D","PIV10D","CallPutOIratio","CallPutVolratio","CallOI","PutOI","CallVol","PutVol","CKUpCnt","CKUpVol","CKDnCnt","CKDnVol","PKUpCnt","PKUpVol","PKDnCnt","PKDnVol"]
                                            else:
                                                itemkeys=["DateTime","Symbol","IV","Last","CallPutOIratio","CallPutVolratio","CallOI","PutOI","CallVol","PutVol","CKUpCnt","CKUpVol","CKDnCnt","CKDnVol","PKUpCnt","PKUpVol","PKDnCnt","PKDnVol"]
                                        if linetype==9 or linetype==10 or linetype==19:
                                            if "TC.F" in symbol:
                                                itemkeys=["DateTime","Symbol","CTR","PTR","RCTR","RPTR","FCIV25","FPIV25","TV","ATV","HV_W4","HV_W8","HV_W13","HV_W26","HV_W52","PutD","CallD","D25CStraddle","D25PStraddle","D25CTV","D25PTV","FIV","IV","Straddle","StraddleStrike","StraddleWeight","CIV25D","PIV25D","CIV10D","PIV10D","CallOI","PutOI","CallVol","PutVol","CKUpCnt","CKUpVol","CKDnCnt","CKDnVol","PKUpCnt","PKUpVol","PKDnCnt","PKDnVol","CallPutOIratio","CallPutVolratio","CSkew","PSkew","VIX"]
                                            else:
                                                itemkeys=["DateTime","Symbol","IV","VIX","CallPutOIratio","CallPutVolratio","CallOI","PutOI","CallVol","PutVol","CKUpCnt","CKUpVol","CKDnCnt","CKDnVol","PKUpCnt","PKUpVol","PKDnCnt","PKDnVol"]
    ...
    def ongreekshistory(self,datatype,symbol,starttime,endtime,data:list):
        for interval in self.lineinterval[symbol+"-"+str(datatype)]:
            ...
            if interval==1:
                ...
                if symbol+"-"+str(datatype)+"-"+str(interval) in self.linedata.keys() or symbol+"-"+str(datatype)+str(interval)+":"+str(starttime)+"~"+str(endtime) in self.greekshistory.keys():
                    for basedata in data:
                        basedata["DateTime"]=self.dtfilter(symbol,basedata["DateTime"])
                        if datatemp:
                            if basedata['DateTime']<=datatemp[-1]['DateTime']:
    ...
    def OnQuoteData(self, SymbolType, DataType, QuoteData):
        ...
        if DataType==820 or DataType==800:
            ...
            # print(quote.GetItemValue(-1,1),"  ",quote.GetStringData("Symbol")," ",quote.GetValueData("StartTime")," ",quote.GetValueData("EndTime")," ",quote.GetValueData("Type")," ",quote.GetValueData("Count"))
            ...
            for i in range(quote.GetValueData("Count")):  
                if "TC.O" in symbol:
                    ...
                    usdatetime=str(quote.GetItemValue(i,123))+" "+"{:0>6d}".format((quote.GetItemValue(i,124) if quote.GetItemValue(i,124)>0 and quote.GetItemValue(i,124)<240000000 else quote.GetItemValue(i,1))//1000)
                    ufdatetime=str(quote.GetItemValue(i,140)) +" "+"{:0>6d}".format((quote.GetItemValue(i,141) if quote.GetItemValue(i,141)>0 and quote.GetItemValue(i,141)<240000000 else quote.GetItemValue(i,1))//1000)
                    usfdatetime=str(quote.GetItemValue(i,157))+" "+"{:0>6d}".format((quote.GetItemValue(i,158) if quote.GetItemValue(i,158)>0 and quote.GetItemValue(i,158)<240000000 else quote.GetItemValue(i,1))//1000)

                    dogshis.append({"DateTime":datetime.strptime(str(date)+" "+"{:0>6d}".format(quote.GetItemValue(i,1)//1000),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                    "Symbol":symbol,
                    ...
                    if quote.GetItemValue(-1,1)==2:
                        self.greeksputcallvol[symbol+"-"+str(DataType)][datetime.strptime(str(quote.GetItemValue(i,0))+" "+"{:0>6d}".format(quote.GetItemValue(i,1)//1000),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]={'CallVol':iff(quote.GetItemValue(i,39)),
                        ...
                        if symbol+"-"+str(DataType) in self.greeksputcallvol:
                            if datetime.strptime(str(quote.GetItemValue(i,0))+" "+"{:0>6d}".format(quote.GetItemValue(i,1)//1000),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8) in self.greeksputcallvol[symbol+"-"+str(DataType)]:
                                oivol=self.greeksputcallvol[symbol+"-"+str(DataType)][datetime.strptime(str(quote.GetItemValue(i,0))+" "+"{:0>6d}".format(quote.GetItemValue(i,1)//1000),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]
                        if "TC.F" in symbol:
                            dogshis.append({"DateTime":datetime.strptime(str(quote.GetItemValue(i,0))+" "+"{:0>6d}".format(quote.GetItemValue(i,1)//1000),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                            "Symbol":symbol,
                            'AnnualTradeday':iff(quote.GetItemValue(i,2),10000000000),
                            # 'TheoVal':iff(quote.GetItemValue(i,3),10000000000),
            ...
            for i in range(datacout):
                if quote.GetItem(i,item):
                    his.append({"DateTime":datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.FilledTime),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                        "Symbol":quote.Symbol,
            ...
            for i in range(datacout):
                if quote.GetItem(i,item):
                    his.append({"DateTime":datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                        "Symbol":quote.Symbol,
            ...
            for i in range(datacout):
                if quote.GetItem(i,item):
                    if "TC.O" in quote.Symbol:
                        greekshis.append({"DateTime":datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                                        'Symbol':quote.Symbol,
                        ...
                        if quote.GetItemData(-1, 2)=="2":
                            self.greeksputcallvol[quote.Symbol+"-"+str(DataType)][datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]={"CallOI":iff(item.GetOptValue("CallOI")),
                            ...
                            if quote.Symbol+"-"+str(DataType) in self.greeksputcallvol:
                                if datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8) in self.greeksputcallvol[quote.Symbol+"-"+str(DataType)]:
                                    oivol=self.greeksputcallvol[quote.Symbol+"-"+str(DataType)][datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]
                            if "TC.F" in quote.Symbol:
                                greekshis.append({"DateTime":datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                                        ...
                                        "CallPutOIratio": oivol["CallOI"]/oivol["PutOI"] if oivol and oivol["CallOI"]!=0 and oivol["PutOI"]!=0 else None,
                                        "CallPutVolratio": oivol["CallVol"]/oivol["PutVol"] if oivol and oivol["CallVol"]!=0 and oivol["PutVol"]!=0 else None,
                                        "CSkew": item.GetOptValue("Calld")/item.GetOptValue("Volatility")*100 if item.GetOptValue("Calld")>-9999999999999999 and item.GetOptValue("Volatility")>-9999999999999999 and item.GetOptValue("Volatility")!=0 else None,
                                        "PSkew": item.GetOptValue("Putd")/item.GetOptValue("Volatility")*100 if item.GetOptValue("Putd")>-9999999999999999 and item.GetOptValue("Volatility")>-9999999999999999 and item.GetOptValue("Volatility")!=0 else None,
            ...
            for i in range(datacout):
                if quote.GetItem(i,item):
                    if "TC.O" in quote.Symbol:
                        greekshis.append({"DateTime":datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.GetItemValue("TradingHours")),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                                        'Symbol':quote.Symbol,
                        ...
                        if quote.GetItemData(-1, 2)=="2":
                            self.greeksputcallvol[quote.Symbol][datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.GetItemValue("TradingHours")),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]={"CallOI":iff(item.GetItemValue("CallOI")),
                            ...
                            if quote.Symbol in self.greeksputcallvol:
                                if datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.GetItemValue("TradingHours")),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8) in self.greeksputcallvol[quote.Symbol]:
                                    oivol=self.greeksputcallvol[quote.Symbol][datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.GetItemValue("TradingHours")),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]
                            if "TC.F" in quote.Symbol:
                                greekshis.append({"DateTime":datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.GetItemValue("TradingHours")),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                                        'Symbol':quote.Symbol,
                                        ...
                                        "CallPutOIratio": oivol["CallOI"]/oivol["PutOI"] if oivol and oivol["CallOI"]!=0 and oivol["PutOI"]!=0 else None,
                                        "CallPutVolratio": oivol["CallVol"]/oivol["PutVol"] if oivol and oivol["CallVol"]!=0 and oivol["PutVol"]!=0 else None,
                                        "CSkew": item.GetItemValue("Calld")/item.GetItemValue("Volatility")*100 if item.GetItemValue("Calld")>-9999999999999999 and item.GetItemValue("Volatility")>-9999999999999999 and item.GetItemValue("Volatility")!=0 else None,
                                        "PSkew": item.GetItemValue("Putd")/item.GetItemValue("Volatility")*100 if item.GetItemValue("Putd")>-9999999999999999 and item.GetItemValue("Volatility")>-9999999999999999 and item.GetItemValue("Volatility")!=0 else None,
    ...
    def OnQuoteDataSyn(self, SymbolType, DataType, QuoteData, bstrModifiedFields):
        ...
        if DataType==1 and SymbolType!=8: #实时
            temp={"DateTime":datetime.strptime(str(quote.TradeDate)+" "+"{:0>6d}".format(quote.FilledTime),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
                    ...
                    "adjust":quote.GetValueFromIndex(250)/10000000000 if quote.GetValueFromIndex(250)>0 else None,
            ...
            if "TC.F" in quote.Symbol:
                self.ongreeksreal(DataType,quote.Symbol,
                        ...
                        "CallPutOIratio": quote.GetGreeksValue("CallOI")/quote.GetGreeksValue("PutOI") if quote.GetGreeksValue("CallOI") and quote.GetGreeksValue("PutOI") and quote.GetGreeksValue("CallOI")!=0 and quote.GetGreeksValue("PutOI")!=0 else None,
                        "CallPutVolratio": quote.GetGreeksValue("CallVol")/quote.GetGreeksValue("PutVol") if quote.GetGreeksValue("CallVol") and quote.GetGreeksValue("PutVol") and quote.GetGreeksValue("CallVol")!=0 and quote.GetGreeksValue("PutVol")!=0 else None,
                        ...
                        "CSkew": quote.GetGreeksValue("Calld")/quote.GetGreeksValue("Volatility")*100 if quote.GetGreeksValue("Calld")>-9999999999999999 and quote.GetGreeksValue("Volatility")>-9999999999999999 and quote.GetGreeksValue("Volatility")!=0 else None,
                        "PSkew": quote.GetGreeksValue("Putd")/quote.GetGreeksValue("Volatility")*100 if quote.GetGreeksValue("Putd")>-9999999999999999 and quote.GetGreeksValue("Volatility")>-9999999999999999 and quote.GetGreeksValue("Volatility")!=0 else None})
            elif "TC.S" in quote.Symbol:
                self.ongreeksreal(DataType,quote.Symbol,{"DateTime":datetime.strptime(str(quote.TradingDay)+" "+"{:0>6d}".format(quote.TradingHours),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8),
    ...
    def OnSetTopic(self, strTopic, lParam, strParam, pvParam):
        ...
        if os.path.exists(v.value):
            ...
            icmpath=icmpathlist[0]+"/"+icmpathlist[1]+"/"+icmpathlist[2]+"/"+icmpathlist[3]+"/Configs/ContractList_"+strParam+".ini"
            print("无法获取到历史合约,当前返回今日合约数据,请确认账户是否有dogs权限")
...
class QuoteClass:
    def __init__(self,extendevent,dllpath):
        try:
    ...
    def connect(self,appid:str,servicekey:str):
        ...
        rcv = CreateEventReceiver(self.interface, self.eventobj)
        cpc = self.pquote.QueryInterface(connectionpoints.IConnectionPointContainer)
        cp = cpc.FindConnectionPoint(byref(self.interface._iid_))
        cp.Advise(rcv)
        
        # if int(self.get_fileversion(self.apppath+'/TCoreRelease/Components/TCQuoteAPI.dll').replace(".",""))>=10497:
        #     skey="API"
        # else:
        #     skey=servicekey
    ...
    def getexpirationdate(self, symbol:str):
        try:
            ...
            pass
        try:
            return self.pquote.GetExpirationDate(symbol)
        except COMError as e:
    ...
    def gettradeingdays(self, strSymbol):
        ...
        try:
            ...
            if lEndDate:
                return self.pquote.GetTradeingDays(strSymbol, int(lEndDate))
            else:
    ...
    #获取剩余交易日
    def gettradeingdate(self, strSymbol,startDate,endDate):
        try:
            CoInitializeEx(COINIT_MULTITHREADED)
        except:
            pass
        try:
            return self.pquote.GetTradeingDate(strSymbol, int(startDate),int(endDate))
        except COMError as e:
            print("gettradeingdays错误:",e)
    ...
    def __getsymbollist(self, Classify, Exchange, Symbol, Month, CallPut):
        try:
            ...
            pass
        try:
            return self.pquote.GetSymbolClassifications(Classify, Exchange, Symbol, Month, CallPut)
        except COMError as e:
            print("getsymbol错误:",e)
    ...
    def symbollookup(self, SymbolType, Keyword):
        try:
            ...
            pass
        try:
            return self.pquote.SymbolLookup(SymbolType, Keyword)
        except COMError as e:
