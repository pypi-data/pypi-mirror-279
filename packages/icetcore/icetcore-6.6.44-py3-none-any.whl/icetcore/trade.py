...
class TradeClass:
    def __init__(self,eventclass:type,dllpath):
        try:
    ...
    def connect(self,appid:str,servicekey:str):
        ...
        rcv = CreateEventReceiver(interface, self.eventobj)
        cpc = self.ptrade.QueryInterface(connectionpoints.IConnectionPointContainer)
        cp = cpc.FindConnectionPoint(byref(interface._iid_))
        cp.Advise(rcv)
        # if int(self.get_fileversion(self.apppath+'/TCoreRelease/Components/TCTradeAPI.dll').replace(".",""))>=10620 :
        #     skey="API"
        # else:
        #     skey=servicekey
    ...
    def getexpirationdate(self, symbol:str):
        try:
            ...
            pass
        try:
            return self.ptrade.GetExpirationDate(symbol)
        except COMError as e:
    ...
    def gettradeingdays(self, strSymbol):
        ...
        try:
            ...
            return self.ptrade.GetTradeingDays(strSymbol, lEndDate)
        except COMError as e:
    ...
    def gethotmonth(self, symbol:str, strdate="", strtime=""):
        ...
        try:
            ...
            if strdate and strtime:
                strhot=self.ptrade.GetHotMonthByDateTime(symbol, strdate, strtime)
                hot=strhot.split("~")[0].split(":")
                temp[datetime.strptime(str(hot[1]),'%Y%m%d%H%M%S').replace(tzinfo=self.tz)+timedelta(hours=8)]=hot[0]
                return temp
            else:
                self.ptrade.GetHotMonthByDateTime(symbol, strdate, strtime)
    ...
    def getsymbol_ticksize(self,symbol:str):
        try:
            ...
            pass
        return float(self.__getsymbolInfo("2",symbol))
    ...
    def getsymbolvolume_multiple(self,symbol:str):
        try:
            ...
            pass
        return float(self.__getsymbolInfo("6",symbol))
    ...
    def __getsymbollist(self, Classify, Exchange, Symbol, Month, CallPut):
        try:
            ...
            pass
        try:
            return self.ptrade.GetSymbolClassifications(Classify, Exchange, Symbol, Month, CallPut)
        except COMError as e:
            print("getsymbol错误:",e)
    ...
    def cancelorder(self, ReportID, Key):
        try:
            ...
            pass
        try:
            return self.ptrade.CancelOrder(ReportID, Key)
        except COMError as e:
    ...
    def cancelspreadorder(self, bstrReportID):
        try:
            ...
            pass
        try:
            return self.ptrade.CancelSpreadOrder(bstrReportID)
        except COMError as e:
            print("CancelOrder错误:",e) 
    ...
    def getcombposition(self,index,acc):
        ...
        try:
            ...
            if self._getaccountdata(8,index,acc,combpositem)==1:
    ...
    def _neworder(self, SecurityType, NewOrderParameters):
        try:
            ...
            pass
        try:
            return self.ptrade.NewOrder2(SecurityType, NewOrderParameters)
        except COMError as e:
            print("neworder错误:",e) 
    ...
    def replaceorder(self, reportid,orderqty,price,stopprice):
        ...
        try:
            ...
            return self.ptrade.ReplaceOrder(ReplaceOrderParameters)
        except COMError as e:
