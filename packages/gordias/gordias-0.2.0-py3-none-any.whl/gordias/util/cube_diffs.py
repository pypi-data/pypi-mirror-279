...
def find_cube_differences(
    ...
    Returns
    -------
    dict | pandas.DataFrame | None (depending on return_type)
    ...
    def _attr_to_dict(obj, attr_list):
        ...
        for attr in attr_list:
            if attr == "data":
                attr_dict["masked data"] = _compare_cube_data(obj)
            else:
    ...
    def _compare_cell_methods(cube_list):
        ...
        def _reformat_cell_method(cm):
            ...
            if len(comments) == 0:
                ...
                if len(intervals) > 0:
                    intervals = "".join("interval: {} ".format(t) for t in intervals)
    ...
    def _compare_time_start_end(coord_list):
        """
    ...
    def _compare_coordinate_values(array_dict, attr):
        ...
        if same:
            ...
            for array in array_list:
                val_dict = {}
                if (
                    array is not None and array.size > 0 and not np.all(array == None)
                ):  # noqa
    ...
    def _merge_dicts(dicts_dict, labels):
    ...
    def _print_dataframe(dataframe, var_name):
        with pd.option_context(
            ...
            for i in np.arange(0, len(cube_list), MAX_COL_NUMBER):
                msg += (
                    dataframe.iloc[
                        :, i : min(i + MAX_COL_NUMBER, len(cube_list) + 1)
                    ].to_string()
                    + "\n"
                )
            logger.error(msg)
    ...
    if return_type == "dict":
        ...
        if return_type == "DataFrame":
            ...
            _print_dataframe(dataframe, cube_list[0].var_name)
            return
