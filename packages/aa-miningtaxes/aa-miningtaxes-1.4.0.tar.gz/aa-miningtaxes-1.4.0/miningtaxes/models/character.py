...
class CharacterMiningLedgerEntry(models.Model):
    ...
    def calc_prices(self):
        ...
        if (
            ...
            or (MININGTAXES_TAX_HISEC is False and self.eve_solar_system.is_high_sec)
            or (MININGTAXES_TAX_LOSEC is False and self.eve_solar_system.is_low_sec)
            or (MININGTAXES_TAX_NULLSEC is False and self.eve_solar_system.is_null_sec)
            or (MININGTAXES_TAX_JSPACE is False and self.eve_solar_system.is_w_space)
            or (
                MININGTAXES_TAX_POCHVEN is False and self.eve_solar_system.is_trig_space
            )
        ):
            self.taxes_owed = 0.0
