...
class AttributesMod(Module):
    ...
    def proc_tree(self, tree):
        resolut = self.compiler_instance.get_action("RESOLUT")
        console.log(f"[PHP] {tree}")
        id = resolut(tree['ID'])
        line = tree['ID'][-1]

        # Capture the next call?
        captured_tree = self.compiler_instance.capture_next()
        console.log(f"[PHP] {captured_tree}")

        if captured_tree[0] == "CLASS_DECLARATION":
            mod: Module = self.compiler_instance.get_action('CLASS_DECLARATION')
            # Inject marker into instance
            instance = mod(captured_tree[1], add_markers=[id.value])

        else:
            raise TranspilerExceptions.Generic(f"Tried to add an attribute {id} to a non-class declaration. On line {line}")
...
class ClassDeclarationMod(Module):
    ...
    def __call__(self, tree, no_construct=False, add_markers=[]):
        self.markers = add_markers
        return super().__call__(tree, no_construct)
