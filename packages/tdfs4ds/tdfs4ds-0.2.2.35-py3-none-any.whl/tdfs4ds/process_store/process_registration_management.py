...
def register_process_view(view_name, entity_id, feature_names, metadata={}, **kwargs):
    ...
    if kwargs.get('with_process_id'):
        ...
        if tdfs4ds.REGISTER_PROCESS == 'UPDATE_INSERT':
            query_upsert, process_id, query_upsert_dist, query_upsert_filtermanager = _register_process_view_update_insert.__wrapped__(view_name, entity_id, feature_names, metadata=metadata, **kwargs)
        else:
            query_upsert, process_id, query_upsert_dist, query_upsert_filtermanager = _register_process_view_merge.__wrapped__(view_name, entity_id, feature_names, metadata=metadata, **kwargs)
        ...
        if tdfs4ds.REGISTER_PROCESS == 'UPDATE_INSERT':
            query_upsert, query_upsert_dist, query_upsert_filtermanager = _register_process_view_update_insert.__wrapped__(view_name, entity_id, feature_names, metadata=metadata, **kwargs)[:3]
        else:
            query_upsert, query_upsert_dist, query_upsert_filtermanager = _register_process_view_merge.__wrapped__(view_name, entity_id, feature_names, metadata=metadata, **kwargs)[:3]
        return query_upsert, query_upsert_dist, query_upsert_filtermanager
@execute_query_wrapper
def _register_process_view_merge(view_name, entity_id, feature_names, metadata={}, **kwargs):
    """
    ...
    query_process_id = f"{validtime_statement} SEL PROCESS_ID FROM {tdfs4ds.SCHEMA}.{tdfs4ds.PROCESS_CATALOG_NAME} WHERE view_name = '{view_name}'"
    process_id = tdml.execute_sql(query_process_id).fetchall()
    if len(process_id)>0:
        process_id            = process_id[0][0]
        query_feature_version = f"{validtime_statement} SEL FEATURE_VERSION FROM {tdfs4ds.SCHEMA}.{tdfs4ds.PROCESS_CATALOG_NAME} WHERE view_name = '{view_name}'"
        feature_version       = tdml.execute_sql(query_feature_version).fetchall()[0][0]
        query_primary_index   = f"{validtime_statement} SEL FOR_PRIMARY_INDEX, FOR_DATA_PARTITIONING FROM {tdfs4ds.SCHEMA}.{tdfs4ds.DATA_DISTRIBUTION_NAME} WHERE process_id = '{process_id}'"
        ...
        if len(query_primary_index_res)>0:
            ...
            raise ValueError(f"""
                ...
                {print(tdml.DataFrame.from_query(f"NONSEQUENCED VALIDTIME SEL * FROM {tdfs4ds.SCHEMA}.{tdfs4ds.DATA_DISTRIBUTION_NAME} WHERE process_id = '{process_id}'"))}
            """)
    else:
    ...
    if tdfs4ds.FEATURE_STORE_TIME == None:
        ...
        USING (
            ...
            ,   '{json.dumps(entity_id).replace("'", '"')}' AS ENTITY_ID
            ,   '{feature_names}' AS FEATURE_NAMES
            ,   '{int(feature_version)+1}' AS FEATURE_VERSION
    ...
    if kwargs.get('with_process_id'):
        ...
        return query_upsert, query_upsert_dist, query_upsert_filtermanager
@execute_query_wrapper
def _register_process_view_update_insert(view_name, entity_id, feature_names, metadata={}, **kwargs):
    """
