...
def train(datasetloader, metadata_file, root_dir, epochs, batch_size, network):
    ...
    if datasetloader.lower() == "local":
        datasetloader = DatasetLoader(metadata_file=metadata_file, root_dir=root_dir, transform=transform) 
        print(f"Dataset length: {len(dataset)}")
    elif datasetloader.lower() == "internet":
        datasetloader = DatasetLoaderfromInternet(metadata_url=metadata_file, root_url=root_dir, transform=transform)
        print(f"Dataset length: {len(dataset)}")
    else:
        SystemError
        SystemExit
