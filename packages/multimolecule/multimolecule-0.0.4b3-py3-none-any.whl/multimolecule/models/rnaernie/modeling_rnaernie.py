...
class RnaErnieForNucleotidePrediction(RnaErniePreTrainedModel):
    """
    Examples:
...
class RnaErnieLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = RnaErnieAttention(config, position_embedding_type="absolute")
...
class RnaErnieAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = RnaErnieSelfAttention(config, position_embedding_type=position_embedding_type)
...
class RnaErnieSelfAttention(nn.Module):
    def __init__(self, config: RnaErnieConfig, position_embedding_type: str | None = None):
        super().__init__()
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in RnaErnieModel forward() function)
