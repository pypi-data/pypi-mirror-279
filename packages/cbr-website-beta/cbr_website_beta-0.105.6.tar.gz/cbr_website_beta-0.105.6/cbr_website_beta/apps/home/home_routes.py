...
from cbr_website_beta.apps.llms.llms_routes import target_athena_url
...
COGNITO_SIGN_IN       = 'https://the-cbr-beta.auth.eu-west-2.amazoncognito.com/login?client_id=5ij6l5kdho4umoks5rjfh9cbid&response_type=code&scope=email+openid+phone&redirect_uri={cbr_domain}/sign-in'
COGNITO_SIGN_OUT      = 'https://the-cbr-beta.auth.eu-west-2.amazoncognito.com/logout?client_id=5ij6l5kdho4umoks5rjfh9cbid&response_type=code&scope=email+openid+phone&logout_uri={cbr_domain}/sign-out'
...
#     if running_in_aws():
#         folder_assets = '/tmp/static/assets'
#     else:
#         folder_assets = '../../flask_assets/static/assets'
#     return send_from_directory(folder_assets, path)

# path_osbot_llms_web_static = API_OSBot_LLMs().path_web_static()
...
# @blueprint.route('/llms-ui/<path:path>')
# @allow_anonymous
# def llms_web_static(path):
#     try:
#         return send_from_directory(path_osbot_llms_web_static, path)
...
def path_path(path):            # todo: remove the need for this route
    try:
        ...
        kwargs = dict(template_name_or_list = "home/" + path                                             ,
                      ...
                      url_athena            = target_athena_url() + '/open_ai/prompt_with_system__stream',  # todo: refactor direct render of the multiple athena UIs
                      page_title            = 'title'                                                    )
