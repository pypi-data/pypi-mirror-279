...
from cbr_website_beta.utils.remote_shell.CBR__Remote_Shell__Copy_Files import \
    CBR__REMOTE_SHELL__FILE_TRANSFER__S3_BUCKET, CBR__REMOTE_SHELL__FILE_TRANSFER__PARENT_FOLDER
...
from osbot_utils.utils.Files import file_move, file_move_to_folder, file_exists, file_not_exists, path_combine, \
    folder_copy, file_name, file_size, files_list, file_contents, file_write
from osbot_utils.utils.Zip import zip_folder, zip_file_list, zip_bytes_file_list, zip_bytes_get_file, \
    zip_bytes_unzip_to_folder
...
class Release_Package__via__S3(Type_Safe):
    zip_file   : str = '/tmp/cbr_website_beta.zip'
    s3         : S3
    ...
    def unzip_from_s3_to_folder(self, target_folder):
        if self.enabled():
            ...
            zip_bytes_unzip_to_folder(zip_bytes, target_folder)
            return target_folder

    def upload_zip_file_to_s3(self):
        if self.enabled():
            self.s3.file_upload_to_key(file=self.zip_file, bucket=self.s3_bucket(), key=self.s3_key())

    def create_zip_with__cbr_website_beta(self):
        if self.enabled():
            source_folder = cbr_website_beta.path
            ignore_pattern = ['*.pyc', '.DS_Store', '__pycache__', '*.env']
            ignore_pattern.append('__to_delete')
            ...
            with Temp_Folder(temp_prefix=self.package_name()) as _:
                ...
                with Temp_Zip(_.path()) as zip_file:
                    zip_file.move_to(self.zip_file)

    @trace_calls(contains=['cbr_website_beta'], show_duration=True, duration_padding=60)
