...
def make_configuration_file() -> dict:
    try:
        home_dir = environ['HOME']
    except Exception as e:
        erro = f'Variável de ambiente HOME não configurada.\n Comando retornou {e}'
        return {'Message': erro, 'Status': False}    
    try:
        path = Path(home_dir) / 'worker-automate-hub'
        path.mkdir()
        ...
        arq_conf.write('name = "WORKER"\n\n[id]\nrobot_uuid = "ALTERAR"\nrobot_name = "ALTERAR"\n[params]\napi_base_url = "ALTERAR"\napi_auth = "ALTERAR"\nnotify_alive_interval = 5')
        arq_conf.close()
        retorno = {'Message': f'Arquivo de configuração criado em {path}', 'Status': True}
    return retorno
