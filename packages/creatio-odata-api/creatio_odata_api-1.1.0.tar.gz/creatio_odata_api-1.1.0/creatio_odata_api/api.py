...
class CreatioODataAPI:
    ...
    def add_attachment(  # pylint: disable=line-too-long
        self, collection: str, record_id: str, field_name: str, file_path: str
    ) -> requests.models.Response:
        """
        Upload an attachment to a field of an object collection instance.

        Args:
            collection (str): The collection to which the attachment belongs.
            record_id (str): The object instance Id.
            field_name (str): The name of the field where the attachment will be uploaded.
            file_path (str): The path to the file to be uploaded as an attachment.
        ...
        Examples:
            Upload an attachment to a field of an object collection instance:
            >>> response = add_attachment("Collection1", record_id="IdValue", field_name="Field1", file_path="path/to/file")
        """
        try:
            with open(file_path, "rb") as file:
                data = file.read()
        except FileNotFoundError as e:
            print_exception(e, custom_msg=f"File not found: '{file_path}'")
            raise

        return self._make_request(
            "PUT", f"/0/odata/{collection}({record_id})/{field_name}"
        )
