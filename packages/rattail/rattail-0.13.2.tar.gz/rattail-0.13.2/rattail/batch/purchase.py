...
class PurchaseBatchHandler(BatchHandler):
    ...
    * ``rattail.enum.PURCHASE_BATCH_MODE_COSTING``
    """
    batch_model_class = PurchaseBatch
    penny = decimal.Decimal('0.01')

    # employ versioning workarounds
    ...
    def allow_versioning(self, action):
        if action == 'auto_receive':
            return False
        return super().allow_versioning(action)
    ...
    def auto_missing_credits(self, **kwargs):
        """
        Return boolean indicating whether missing/DNR credits should
    ...
    def set_input_file(self, batch, path, attr=None, **kwargs):
        ...
        super().set_input_file(batch, path, attr=attr, **kwargs)
    ...
    def assign_purchase_order(self, batch, purchase_key, session=None):
        ...
        The purchase order is located (in whatever system) and will be
        returned.  The batch will be updated to include a reference to
        the PO.  Note that which batch attribute is used to store the
        reference may vary depending on the system where the PO lives.
        ...
        if isinstance(purchase, model.Purchase):
            # TODO: is there a reason we previously did this?
            #batch.purchase_uuid = purchase.uuid
            batch.purchase = purchase
        else:
            field = self.get_purchase_order_fieldname()
            setattr(batch, field, purchase_key)
    ...
    def get_purchase_order_fieldname(self):
        return 'purchase'
    ...
    def get_eligible_purchase_key(self, purchase):
        return purchase.uuid
    ...
    def populate_from_invoice(self, batch, progress=None):
        ...
        try:
            ...
            log.warning("failed to parse invoice date from file: %s", path,
                        exc_info=True)

        try:
            batch.invoice_number = parser.parse_invoice_number(path)
        except:
            log.warning("failed to parse invoice number from file: %s", path,
                        exc_info=True)
    ...
    def populate_from_multiple_invoices(self, batch, progress=None):
        ...
        def append(invrow, i):
            ...
            thisrow.invoice_number = parser._invoice_number
            thisrow.invoice_date = parser._invoice_date
        ...
        for filename in batch.get_param('invoice_files', []):
            ...
            # nb. invoice number/date will be assigned to each row of the invoice
            parser._invoice_number = parser.parse_invoice_number(path)
            parser._invoice_date = parser.parse_invoice_date(path)
    ...
    def update_batch_row_from_invoice_row(self, row, invrow):
        ...
        if case_quantity:
            case_quantity = decimal.Decimal(str(case_quantity))
    ...
    def find_batch_row_for_invoice_row(self, batch, invrow):
        ...
        product = self.locate_product_for_entry(session, invrow.item_entry,
                                                ...
                                                vendor=batch.vendor)
        if product:
            prod_matches = [row for row in batch.data_rows
            ...
            if prod_matches:
                ...
                # but if more than one, try to match on quantity too
                invrow_units = self.get_units_ordered(invrow)
                qty_matches = [row for row in prod_matches
                               if self.get_units_ordered(row) == invrow_units]
                if qty_matches:
                    if len(qty_matches) > 1:
                        log.warning("%s rows of batch %s matched both product "
                                    "and quantity for invoice item: %s",
                                    len(qty_matches), batch.uuid, invrow.item_entry)
                    return qty_matches[0]

                # hm no quantity match..
                log.warning("%s rows of batch %s matched product but none "
                            "matched quantity for invoice item: %s",
                            len(prod_matches), batch.uuid, invrow.item_entry)
                return prod_matches[0]
    ...
    def populate_from_truck_dump_invoice(self, batch, progress=None):
        ...
        session = self.app.get_session(child_batch)
    ...
    def make_row_from_invoice(self, batch, invoice_row):
        ...
        """
        row = self.make_row()
        row.invoice_number = batch.invoice_number
        row.invoice_date = batch.invoice_date
        ...
        # TODO: not sure if/why this was needed?  it can cause issues
        # if invoice has incorrect case size
    ...
    def refresh(self, batch, progress=None):
        ...
        result = super().refresh(batch, progress=progress)
        if result:
    ...
    def locate_product(self, row, batch=None, session=None, vendor=None):
        """
        ...
        # nb. lookup should find products even if not for sale, at
        # least for receiving.  may need to expand this later..
        kw = {}
        if batch.mode == self.enum.PURCHASE_BATCH_MODE_RECEIVING:
            kw['include_not_for_sale'] = True

        if row.item_entry:
            product = self.locate_product_for_entry(session, row.item_entry, **kw)
            if product:
                return product
    ...
    def quick_entry(self, session, batch, entry):
        ...
        if key == 'upc':
            ...
            provided = self.app.make_gpc(entry, calc_check_digit=False)
            checked = self.app.make_gpc(entry, calc_check_digit='upc')
            ...
            if len(entry) > self.app.maxlen(model.PurchaseBatchRow.item_id):
                return
    ...
    def quick_locate_product(self, session, batch, entry):
        ...
        product = self.products_handler.locate_product_for_entry(session, entry)
        if product:
            return product

        # now we'll attempt lookup by vendor item code
        product = self.products_handler.locate_product_for_vendor_code(session, entry,
                                                                       vendor=batch.vendor)
        if product:
            return product

        # okay then, let's attempt lookup by "alternate" code
        product = self.products_handler.locate_product_for_alt_code(session, entry)
        if product:
            return product
    ...
    def update_row_cost(self, row, **kwargs):
        ...
        * ``invoice_unit_cost``
        ...
        # TODO: this used to *not* have a fallback of zero; i'm not
        # clear in which circumstances it might be None but apparently
        # that is possible..  so we must fallback to avoid error and
        # log a warning that we'll hopefully see (?) to learn more
        if old_invoice_total_calculated is None:
            log.warning("invoice_total_calculated is null for row %s of batch %s: row",
                        row.uuid, batch.id_str, row)
            old_invoice_total_calculated = 0
        ...
        * ``units_ordered``
        ...
        if batch.mode == self.enum.PURCHASE_BATCH_MODE_ORDERING:
            ...
            if cases_ordered is not None:
                if isinstance(cases_ordered, str):
            ...
            if units_ordered is not None:
                if isinstance(units_ordered, str):
    ...
    def populate_credit(self, credit, row):
        ...
        credit.invoice_number = row.invoice_number or batch.invoice_number
        credit.invoice_date = row.invoice_date or batch.invoice_date
    ...
    def is_row_deletable(self, row, **kwargs):
        """
        Check whether deleting the given row should be allowed.

        The logic for this depends on the receiving workflow.
        """
        batch = row.batch

        # logic below is only concerned with *receiving* - so just use
        # normal logic for other modes
        if batch.mode != self.enum.PURCHASE_BATCH_MODE_RECEIVING:
            return super().is_row_deletable(row, **kwargs)
        ...
        # okay, normal receiving batch..

        # when the order quantities are known, this implies the
        # item came from invoice or PO, so should not be deleted
        if batch.order_quantities_known:

            # but even for this kind of batch, some rows may have
            # been added manually, and those can be deleted.  we
            # just check for empty order quantities to determine.
            if not self.get_units_ordered(row):
                return True
            return False

        # allow delete if receiving from scratch
        return True
    ...
    def auto_receive_all_items(self, batch, progress=None, **kwargs):
        """
        Automatically "receive" all items for the given batch.

        This only makes sense in the context of a receiving batch.
        For each row, calculate the difference between the "shipped"
        and "received" quantities.  Then adjust "received" by that
        difference, so it matches "shipped" quantity.
        """
        def receive(row, i):
        ...
        # nb. this is needed for `rattail auto-receive` cmd
        return True

    def confirm_all_receiving_costs(self, batch, progress=None, **kwargs):
        """
        Automatically confirm all catalog and invoice costs for the
        given batch.

        This only makes sense in the context of a receiving batch.
        """
        def confirm(row, i):
            if row.catalog_unit_cost is not None and not row.catalog_cost_confirmed:
                row.catalog_cost_confirmed = True
            if row.invoice_unit_cost is not None and not row.invoice_cost_confirmed:
                row.invoice_cost_confirmed = True

        self.progress_loop(confirm, batch.active_rows(), progress,
                           message="Confirming all receiving costs")

        batch.set_param('confirmed_all_costs', True)
        self.refresh(batch, progress=progress)
    ...
    def update_order_counts(self, purchase, progress=None):
        ...
        vendor.
        """
        session = self.app.get_session(purchase)
        model = self.model
        ...
        def update(item, i):
            if item.product:
                ...
                if not inventory:
                    inventory = model.ProductInventory(product=item.product)
    ...
    def update_receiving_inventory(self, purchase, consume_on_order=True, progress=None):
        """
        Increase on-hand counts and (by default) decrease on-order
        counts, as part of final "receiving" for the given purchase.
        """
        session = self.app.get_session(purchase)
        model = self.model
    ...
    def make_credits(self, batch, progress=None):
        ...
        if not date_received:
            date_received = self.app.today()
        auto_missing_credits = self.auto_missing_credits()
        ...
        def add_credits(row, i):
            ...
            if auto_missing_credits and row.product_uuid and not row.out_of_stock:
    ...
    def make_purchase(self, batch, user,
        ...
        purchase.created = self.app.make_utc()
