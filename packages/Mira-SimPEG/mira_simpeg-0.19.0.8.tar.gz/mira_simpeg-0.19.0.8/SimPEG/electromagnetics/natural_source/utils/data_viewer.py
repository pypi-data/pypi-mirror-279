...
class NSEM_data_viewer:
    ...
    def __init__(self, data, data_dict=None, backend="qt"):
        ...
        self.location_fig, self._location_ax = self._data.map_data_locations(picker=5)
        ...
        self.text = self._location_ax.text(
            ...
            1.05,
            "Selected: none",
            transform=self._location_ax.transAxes,
            va="top",
        )
        ...
        self._selected_loc = self._location_ax.plot(
            [], [], "o", ms=12, alpha=0.4, color="yellow", visible=False, zorder=5
        )[0]
        ...
        unique_rx = np.unique(
            np.array(
                [
                    rx.__class__
                    for src in self._data.survey.source_list
