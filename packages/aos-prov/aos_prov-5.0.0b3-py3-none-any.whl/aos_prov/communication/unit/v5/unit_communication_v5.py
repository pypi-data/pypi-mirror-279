...
class UnitCommunicationV5:
    ...
    def start_provisioning(self, node_id: str, password: str) -> None:
        with self.unit_provisioning_stub(True) as stub:
            ...
            if int(response.aos_code) == 0:
                print_done()
            else:
                UnitError(f'FAILED! Received error from the unit: \n aos_code: {response.aos_code}\n'
                          f'exit_code: {response.exit_code}\n message: {response.message}')

    def finish_provisioning(self, node_id: str, password: str):
        with self.unit_provisioning_stub(True) as stub:
            print_left('Finishing provisioning...')
            response = stub.FinishProvisioning(iam_manager.FinishProvisioningRequest(node_id=node_id, password=password))
            if int(response.aos_code) == 0:
                print_done()
            else:
                UnitError(f'FAILED! Received error from the unit: \n aos_code: {response.aos_code}\n'
