...
import threading
from typing import Any, Callable, cast, Dict, Generator, Iterator, List, Literal, Mapping, Optional, \
...
class GrpcExtractionTraceBuilder(ExtractionTraceBuilder):
    ...
    def open(self, data_type: Optional[str] = None, offset: int = 0, size: Optional[int] = None,
             mode: Literal['rb', 'wb'] = 'rb') -> Union[BufferedReader, BufferedWriter]:
...
class GrpcExtractionTrace(ExtractionTrace):
    ...
    def open(self, data_type: Optional[str] = None, offset: int = 0, size: Optional[int] = None,
        ...
        if mode == 'rb':
            ...
            return self._get_reader(offset, size)
        elif mode == 'wb':
            if offset != 0:
                raise ValueError('Offsets other than 0 are not supported for mode "wb".')
            if size is not None:
                raise ValueError('Argument size is not supported for mode "wb".')
            return self._get_writer(str(data_type or 'raw'))
        else:
            raise ValueError('Invalid mode: Only rb or wb are allowed')

    def _get_reader(self, offset=0, size=None) -> BufferedReader:
    ...
    def _get_writer(self, data_type: str) -> BufferedWriter:
        return self._grpc_handler.writer(self._trace_id, data_type)
...
class GrpcTraceSearcher(TraceSearcher):
    ...
    def search(self, query: str, count: int, scope: Union[str, SearchScope] = SearchScope.image) -> SearchResult:
        ...
        traces = self._handler.search_traces(query, count, scope)
        return self._unpack_search_result(traces)
    ...
    def _unpack_search_result(self, search_result: GrpcAny) -> SearchResult:
        ...
        search_traces = [self._unpack_search_trace(rpc_trace) for rpc_trace in result.traces]
        return GrpcSearchResult(result.totalResults, search_traces)


class GrpcWriter(BufferedWriter):
    """
    ...
    def __enter__(self):
        ...
        start_message = pack.begin_writing(self._trace_id, self._data_type)
    ...
    def write(self, buffer: Union[bytes, bytearray, memoryview, array, mmap], /) -> int:  # type: ignore
        ...
        if isinstance(buffer, bytes):
            ...
            return self.write_from_buffer(buffer)
        elif isinstance(buffer, mmap):
            return self.write_from_mmap(buffer)
        ...
        size = len(data)
        goal = self._position + size
        start_pos = self._position
        while self._position < goal:
            to_write = min(goal - self._position, MAX_CHUNK_SIZE)
            write_start = self._position - start_pos
            data_message = pack.write_data(self._trace_id, self._data_type,
                                           data[write_start: write_start + to_write])
            self._handler.handle_message(data_message)
            self._position += to_write
        return size

    def write_from_buffer(self, reader: BufferedReader) -> int:
        """
        Write data to gRPC stream.
    ...
    def write_from_mmap(self, reader: mmap) -> int:
        """
        Write data to gRPC stream.
...
class ProcessHandler:
    ...
    def writer(self, trace_id: str, data_type: str) -> BufferedWriter:
        """
        Write a single data stream to the client for a given trace id using a GrpcWriter.
        ...
        :param data_type: type of the data streams
        """
        return GrpcWriter(trace_id, data_type, self)
    ...
    def enrich_trace(self, trace_id: str, types: List[str], properties: Dict[str, Any],
        ...
        """
        log.info('Process handler enriches trace {} with properties {}', trace_id, properties)
    ...
    def search_traces(self, query: str, count: int, scope: Union[str, SearchScope]) -> GrpcAny:
        """
        ...
        rpc_search_request = pack.rpc_search_request(query, count, scope)
...
class HealthServiceServicer(HealthServicer):
    ...
    def Watch(self, request, context):  # noqa: N802
        ...
        yield HealthCheckResponse(status=HealthCheckResponse.SERVING)
...
def _start_server(extraction_plugin_class: Type[BaseExtractionPlugin], address: str) -> grpc.Server:
    ...
    if plugin_info.resources and plugin_info.resources.maximum_workers:
        num_workers = plugin_info.resources.maximum_workers
    ...
    add_HealthServicer_to_server(HealthServiceServicer(), server)
