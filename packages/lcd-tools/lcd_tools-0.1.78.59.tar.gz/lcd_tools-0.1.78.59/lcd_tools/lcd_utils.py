...
def write_to_lcd(*args, **kwargs) -> None:
    ...
    # 3 Arg Override
        ...
        # Three argument override: lcd object, frame_buffer list, and optionally num_cols or row keyword argument
        lcd = args[0]
        frame_buffer = args[1]
        ...
        for row in frame_buffer:
            lcd.write_string(row.ljust(frame_buffer)[:frame_buffer])
            lcd.write_string("\r\n")
    else:
