...
def find_from_id(cookies):
    ...
    if choice == 1:
        ...
        if now.get_info(cookies=cookies) == -1:
            print(
                format_output(
                    "You don't have the currect cookies to get the info of this Playlist.",
                    type="Error",
                )
            )
        ...
        while now.done_sum() != now.track_count:
            ...
            print(
                format_output(
                    f"Done: {count}/{now.track_count}, still need: {least} seconds",
                    type="Info",
                )
            )
            time.sleep(1)
        for i in range(0, len(now.track)):
            j = now.track[i]
            print(
                format_output(f"Song #{i + 1}: {j.title} - {j.artist_str}", type="Info")
            )
    while True:
        FUNC_CHOICE = [
            "Save to a json file",
