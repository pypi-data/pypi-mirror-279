...
from kelvin.application.config import AppConfig
...
async def simulator(
    ...
    if asset_count is not None and app_config.app.type == "bridge":
...
@click.option(
    ...
    type=click.FLOAT,
    show_default=True,
    help="Publish period in seconds. Publish csv rows at this rate instead of the rate determined between timestamps",
)
@click.option(
    "--playback",
    is_flag=True,
...
async def csv(
    ...
    if (publish_rate is not None and playback) or (publish_rate is None and playback is False):
        print("Error: invalid csv publisher configuration, please set one of 'publish_rate' or 'playback'")
        return
    ...
    gen = CSVPublisher(csv, publish_rate, playback)
