...
class DuckDBCompute(SQLCompute):
    ...
    def register_temp_table_from_offline_store(
        ...
        join_keys_filter: Optional[Dict[str, JoinKeyBoundaries]] = None,
        ...
        """
        files = [os.path.join(dataset_root_path, fragment.path) for fragment in dataset.get_fragments()]
        if not files:
            self.session.from_arrow(dataset.schema.empty_table()).create_view(table_name)
            return

        if s3_options:
            self.run_sql(
                f"""CREATE OR REPLACE secret {table_name} (
                type s3,
                key_id '{s3_options.access_key_id}',
                secret '{s3_options.secret_access_key}',
                session_token '{s3_options.session_token}',
                scope '{dataset_root_path}',
                region '{s3_options.region or 'us-east-1'}')"""
            )

        relation = self.session.read_parquet(files, hive_partitioning=True, union_by_name=True)
        relation = relation.select(
            *[
                duckdb.ColumnExpression(field.name).cast(_arrow_type_to_duckdb_type(field.type)).alias(field.name)
                for field in dataset.schema
            ]
        )

        if join_keys_filter:
            for join_key, boundaries in join_keys_filter.items():
                relation = relation.filter(
                    (duckdb.ColumnExpression(join_key) >= _duckdb_value(boundaries.min))
                    & (duckdb.ColumnExpression(join_key) <= _duckdb_value(boundaries.max))
                )
