...
from tecton_materialization.ray.delta import maintenance_is_supported
...
def _run_maintenance(materialization_task_params: MaterializationTaskParams, fd: FeatureDefinitionWrapper) -> None:
    if not maintenance_is_supported():
    ...
    if maintenance_params.vacuum:
        retention_hours = (
            maintenance_params.vacuum_retention_hours if maintenance_params.HasField("vacuum_retention_hours") else None
        )
        delta_writer.run_vacuum(
            delta_log_table_name,
            delta_log_table_region,
            cross_account_role,
