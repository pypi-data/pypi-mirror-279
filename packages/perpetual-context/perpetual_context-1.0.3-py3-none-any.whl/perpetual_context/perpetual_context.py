...
# in Artificial Intelligence projects for language models. It manages context memory by saving and indexing the encrypted dialogs
...
class PerpetualContext:
    def __init__(self, display_error_point=True):
        try:
            ...
            try: from numpy import array
                ...
                installModule(module_name='numpy', version='1.26.4')
    ...
    def getSummaryImage(self, file_path='', max_tokens=1):
        try:
            ...
            try:
                ...
                use_gpu = self.__cuda.is_available()
                ...
                complete_text = ' '.join(textual_elements)
                complete_text = complete_text.strip()
            except: image_type, complete_text = self.getFileType(file_path=file_path).upper(), ''
            ...
            if sum((len_complete_text, len_object_labels, len_color_names)) > 0:
                ...
                if len(complete_text) > 0: list_of_descriptions.append('texts = '+complete_text)
                if len(object_labels) > 1: list_of_descriptions.append('objects = '+object_labels)
                if len(color_names) > 1: list_of_descriptions.append('colors = '+color_names)
    ...
    def getSummaryFile(self, file_path='', max_tokens=1, main_page=None):
        try:
            ...
            if file_type == pdf_types[0]: file_text = self.getSummaryPDF(file_path=file_path, max_tokens=max_tokens, main_page=main_page)
            elif file_type == word_types[0]: file_text = self.getSummaryWord(file_path=file_path, max_tokens=max_tokens, main_page=main_page)
            ...
            elif file_type == excel_types[0]: file_text = self.getSummaryExcel(file_path=file_path, max_tokens=max_tokens)
            elif file_type == csv_types[0]: file_text = self.getSummaryCSV(file_path=file_path, max_tokens=max_tokens)
                ...
                has_pdf_type = self.__search(r'\b(' + '|'.join(pdf_types) + r')\b', file_path_lower)
                has_word_type = self.__search(r'\b(' + '|'.join(word_types) + r')\b', file_path_lower)
                has_powerpoint_type = self.__search(r'\b(' + '|'.join(powerpoint_types) + r')\b', file_path_lower)
                has_excel_type = self.__search(r'\b(' + '|'.join(excel_types) + r')\b', file_path_lower)
                has_csv_type = self.__search(r'\b(' + '|'.join(csv_types) + r')\b', file_path_lower)
                ...
                has_text_type = self.__search(r'\b(' + '|'.join(text_types) + r')\b', file_path_lower)
    ...
    def getContext(self, user_id=0, dialog_id=0, config=None):
        try:
            ...
            if return_format in ('gemini_pattern', 'gemma_pattern'): max_tokens = max((1, max_tokens-(self.__countTokens(string=system)+self.__countTokens(string=prompt)+self.__countTokens(string=google_system))))
            else: max_tokens = max((1, max_tokens-(self.__countTokens(string=system)+self.__countTokens(string=prompt))))
            ...
            if self.__path.isdir(dialogue_context):
                ...
                if all_tokens <= max_tokens:
                    if len(dialogue_indexes) > 0: context_window = self.__getRelatedContext(user_id=user_id, dialog_id=dialog_id, key_words=self.__getKeyWords(string=prompt), max_tokens=max_tokens, indexes=dialogue_indexes)
                    else:
                        dialog_length = len(self.__listdir(dialogue_context))
                        for index in range(0, dialog_length): context_window.append(self.__getIndex(user_id=user_id, dialog_id=dialog_id, index=index))
                else: context_window = self.__getRelatedContext(user_id=user_id, dialog_id=dialog_id, key_words=self.__getKeyWords(string=prompt), max_tokens=max_tokens, indexes=dialogue_indexes)
                if return_format == 'dictionaries_list':
                    ...
                    return template
                elif return_format == 'mistral_pattern':
                    ...
                    return template
                elif return_format == 'gemma_pattern':
