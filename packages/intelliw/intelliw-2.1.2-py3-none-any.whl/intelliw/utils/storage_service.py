...
class StorageService:
    ...
    def __client_init(self, key, process_type):
        ...
        下载链接一般可以直接下载的
        """
        if not config.STORAGE_SERVICE_URL and process_type == "download" and key.startswith("http"):
            return self.key
        data = {
            'key': key,
        ...
        resp = iuap_request.get(url=config.STORAGE_SERVICE_URL, params=data)
    ...
    def __put_file__(self, file, filename) -> None:
        ...
        try:
            if config.is_server_mode():
                ...
                # file upload
                StorageService(self.curkey, "upload").upload(file)
