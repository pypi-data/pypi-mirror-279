...
class Logger:
    ...
    def __global_logger(cls):
        ...
        logging.root.setLevel(os.environ.get("intelliw.logger.level", default_level))

    def __init__(self):
        level = os.environ.get("intelliw.logger.level", default_level)
        self.framework_logger = self._get_logger(
            "Framework Log", level=level, filename="iw-algo-fx-framework.log")
