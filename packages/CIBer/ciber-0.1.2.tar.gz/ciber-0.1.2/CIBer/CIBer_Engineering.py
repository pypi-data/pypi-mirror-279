...
class Joint_Encoding(Encoder):
    def fit_transform(self, X_train, y_train=None):
        col_index = list(np.arange(len(self.cate_col)))
        df_train = pd.DataFrame(X_train[:,self.cate_col], columns=col_index)
        
        df_freq = self._contingency_table(df_train)
        encode_ref = {k: dict.fromkeys(df_freq[k].unique()) for k in col_index}    # encode number {feature: {value: encode_num}}
        self.col_order = np.argsort([len(encode_ref[k]) for k in col_index])[::-1]      # decreasing col order with number of categories
        
        df_sorted = df_train[self.col_order].copy()
        self.encode_ref = self._encoding(df_sorted, encode_ref, 0, df_freq)
        self.col_index = col_index
        
        X_train[:,self.cate_col] = df_train.replace(self.encode_ref).to_numpy()
        return X_train
    
    def _encoding(self, df, encode_ref, encode_num, frequency_df):
        columns = df.columns.values
        while len(columns) > 0:
            encoded = False
            for row in frequency_df.index:
                ...
                if np.all([encode_ref[col][frequency_df[col].iloc[row]] is None for col in columns]):
                    ...
                    if encode_num == len(encode_ref[columns[-1]])-1:
                        break                           # break one for-loop
            
            while len(columns) > 0 and encode_num == len(encode_ref[columns[-1]])-1:
                for index in encode_ref[columns[-1]]:
                    if encode_ref[columns[-1]][index] is None:
                        encode_ref[columns[-1]][index] = encode_num
                        break                           # break one for-loop
            ...
            if len(columns) > 0:
                frequency_df = self._contingency_table(df)
