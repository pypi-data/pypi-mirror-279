...
class PrepareSidecarTask(PrepareTask):
    ...
    def _run(self):
        ...
        all_containers = self.db.count_all_containers(M.Container.sidecar == True)
    ...
    def _create_container(self, container, parents):
        ...
        if container.level.name == "project":
            ...
            for permission in side_project.permissions:
                side_perms.setdefault(permission.id, []).append(permission.role_ids)
            for permission in orig_project.permissions:
                if (
                    permission.id not in side_perms
                    or permission.role_ids not in side_perms[permission.id]
                ):
                    try:
            ...
            # delete existing rule from new project(gear rule template) and copy the
            # project's rules
            ...
            for rule in rules:
                ...
                for fixed_input in rule.get("fixed_inputs", []):
                    resp = self.fw.api_client.call_api(
                        f"/containers/{fixed_input['id']}/files/{fixed_input['name']}",
                        "GET",
                        auth_settings=["ApiKey"],
                        _return_http_data_only=True,
                    ...
                    fixed_input["id"] = side_project_id
    ...
    def _create_sidecar_container_hierarchy(self, orig_project_id):
        ...
        # in most cases it will return the replacement container's id from the
        # container_replace_map
        ...
        self.insert_containers.push(new_container.model_dump())
