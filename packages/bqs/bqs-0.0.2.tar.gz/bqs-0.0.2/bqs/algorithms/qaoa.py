...
# Test
from ..utils import QAOACostFunction, QAOACirquit, VariationalCircuit, pQAOACircuit, pQAOASingleParametersCircuit, \
    SingleSliceQAOACircuit, ppQAOACircuit, ppQAOASingleParametersCircuit
...
class BaseQAOA(ABC):
    ...
    def optimize(self, num_samples_training: int, num_samples_reading: int, num_iterations: int, **kwargs):
        ...
        self.hyperparameters = results
        self.circuit.hyperparameters = results

        return self.circuit.sample(num_samples=num_samples_reading, hyperparameters=self.hyperparameters, original_basis=True)
...
class QAOA(BaseQAOA):
    ...
    def __init__(
        ...
        super().__init__(model, p=p, hyperparameters=hyperparameters)
    ...
    def _set_circuit(self):
        self.circuit = QAOACirquit(self.model, self.p, self.hyperparameters)
...
class pQAOA(BaseQAOA):
    """
    Parallel version of the Quantum Aproximate Optimization Algorithm
        ...
        args:
            ...
            hyperparameters: list containing the hyperparameters structured as a list containing the list of hyperparameters 
                of the slices
    """
    def __init__(self, model: dimod.BinaryQuadraticModel, models: list[dimod.BinaryQuadraticModel], p: int = 1, hyperparameters: np.ndarray = None):
    ...
    def get_circuit(self) -> list[cirq.Circuit]:
...
class SingleSliceQAOA(BaseQAOA):
    ...
    def __init__(
        ...
        super().__init__(model, p, hyperparameters)
...
class ppQAOA(pQAOA):
    """pQAOA version where the samples are glued elemen-twise, instead of using the vectorial product of the space"""
    def __init__(self, model: dimod.BinaryQuadraticModel, models: list[dimod.BinaryQuadraticModel], p: int = 1, hyperparameters: np.ndarray = None):
        super().__init__(model, models, p, hyperparameters)
