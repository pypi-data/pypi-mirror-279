...
class Adam:
    ...
    def run(self, objective_function, method, number_of_iterations=100, display=True):
        res_sample = minimize(objective_function, self.theta, method=method, options={'maxiter': number_of_iterations, 'disp': display})
...
class Optimizer(ABC):
    ...
    @abstractmethod
    def optimize(self, num_samples_training: int, num_iterations: int, display: bool, **kwargs):
        pass
...
class MonteCarlo(Optimizer):
    ...
    def optimize(self, num_samples_training: int, num_iterations: int, display: bool=False, **kwargs):
        """
        ...
        while iter < num_iterations:
            ...
            solutions = self.circuit.sample(
                    num_samples=num_samples_training, 
                    hyperparameters=hyperparameters,
                    **circuit_options
                )

            new_expectation_value = self.cost_function.evaluate_samples(
                solutions,
                **kwargs
            )
        ...
        return self.circuit.hyperparameters
