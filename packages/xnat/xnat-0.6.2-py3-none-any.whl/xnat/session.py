...
from .core import XNATBaseObject, XNATObject, XNATListing, caching
    ...
    * :py:meth:`XNATSession.users <xnat.session.XNATSession.users>`
    ...
    def download_stream(self,
        ...
        if expected_md5_digest:
            ...
            if data_digest != expected_md5_digest:
                raise exceptions.XNATValueError(f'The downloaded md5 digest ({data_digest}) does not match the '
    ...
    def download(self,
        ...
        with open(target, 'wb') as out_fh:
            self.download_stream(uri, out_fh,
                                 format=format,
                                 verbose=verbose,
                                 timeout=timeout,
                                 expected_md5_digest=expected_md5_digest)
    ...
    def create_object(self,
        ...
        if type_ not in self.XNAT_CLASS_LOOKUP:
            raise KeyError(f'Type {type_} unknow to this XNATSession REST client (see XNAT_CLASS_LOOKUP class variable)')
        ...
        if cache_id not in self._cache['__objects__']:
            ...
            obj: XNATBaseObject = cls(uri, self, datafields=datafields, fieldname=fieldname, overwrites=overwrites, **kwargs)
    ...
    def search(self,
        ...
        """
        filters = create_filter_funcs(project=project,
                                      subject=subject,
    ...
    def batch_download(self,
                       ...
                       resource: Optional[str] = None,
                       use_regex: bool = False,
                       level: MappingLevel = MappingLevel.SCAN) -> None:
        """
