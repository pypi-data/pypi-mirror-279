...
from .constants import TYPE_HINTS, DATA_FIELD_HINTS, REST_SHORTCUTS, ALL_REST_SHORTCUT_NAMES
from .type_hints import TimeoutType, JSONType
from .utils import mixedproperty, pythonize_attribute_name, parse_datetime, get_close_matches_icase
        ...
        if columns is None:
            ...
            for column in columns:
                if column not in ALL_REST_SHORTCUT_NAMES:
                    ...
                    self.logger.warning(f"Column {column} is not a valid REST shortcut, did you mean {', '.join(options)}")
