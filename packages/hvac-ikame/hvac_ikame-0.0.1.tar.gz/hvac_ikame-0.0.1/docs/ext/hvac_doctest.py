...
class TestDirective(SphinxDirective):
    ...
    def run(self):
        ...
        if "skipif" in self.options:
            ...
            if self.config.doctest_global_enabled:
...
class DocTestBuilder(Builder):
    ...
    def init(self):
        ...
        self.outfile = codecs.open(
            path.join(self.outdir, "output.txt"), "w", encoding="utf-8"  # type: ignore
        )
        self.outfile.write(
            (
...
"""
    ...
    def test_group(self, group):
        ...
        if not self.config.doctest_global_enabled:
            return
...
def setup(app):
    ...
    app.add_config_value("doctest_global_enabled", True, False)
