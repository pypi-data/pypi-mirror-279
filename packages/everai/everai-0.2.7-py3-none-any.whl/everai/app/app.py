...
import yaml
from everai_autoscaler.builtin import SimpleAutoScaler, BuiltinManager
...
class App(AppRunnerMixin, AppVersionedMixin, ShowMixin):
    ...
    @staticmethod
    def _get_image_info(image: typing.Optional[Image]) -> typing.Tuple[str, typing.Optional[V1ImagePullSecrets]]:
        if image is None:
            return "", None
    ...
    ) -> typing.List[V1AppVolume]:
        volumes_volumes = [
            V1AppVolume(
                name=regular_name(v.name),
                volume=VolumeVolumeItem(volume=v.name),
            )
    ...
    def _make_autoscaler(cls,
                         ...
                         volumes: typing.Optional[typing.List[V1AppVolume]]
                         ) -> typing.Optional[V1Autoscaler]:
    ...
    def __init__(self,
                 ...
                 autoscaler: typing.Optional[AutoScaler] = None,
                 secret_requests: typing.Optional[typing.List[str]] = None,
                 configmap_requests: typing.Optional[typing.List[str]] = None,
                 volume_requests: typing.Optional[typing.List[VolumeRequest]] = None,
                 *args, **kwargs):
    ...
    @staticmethod
    def from_dict(data: typing.Dict[str, typing.Any]) -> App:
    ...
    @staticmethod
    def from_yaml_stream(stream: typing.IO) -> App:
        data = yaml.safe_load(stream)
        return App.from_dict(data)

    @property
    def volume_requests(self) -> typing.List[VolumeRequest]:
        return [VolumeRequest(x.volume.volume, False) for x in self.spec.volumes if x.volume is not None]

    @property
    def secret_requests(self) -> typing.List[str]:
        return [x.secret.secret_name for x in self.spec.volumes if x.secret is not None]

    @property
    def configmap_requests(self) -> typing.List[str]:
        return [x.config_map.name for x in self.spec.volumes if x.config_map is not None]
    ...
    def autoscaler(self) -> AutoScaler:
        if self.spec.autoscaler is None:
            ...
            BuiltinManager().create_autoscaler(
