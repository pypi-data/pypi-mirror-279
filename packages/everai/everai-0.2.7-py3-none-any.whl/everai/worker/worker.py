...
class Worker(ShowMixin):
    ...
    table_fields: typing.List[TableField] = [
        ...
        TableField('status', formatter=lambda s: 'None' if s is None else s.lstrip("STATUS_")),
        TableField('detail_status', formatter=lambda s: 'None' if s is None else s.value.lstrip("DETAIL_STATUS_")),
    ...
    def to_dict(self):
        return {
            ...
            'status': self.status.value.lstrip("STATUS_"),
            "detail_status": self.detail_status.lstrip("DETAIL_STATUS_"),
            'created_at': self.created_at,
            'deleted_at': self.deleted_at,
            'last_serve_at': self.last_serve_at,
            'success_count': self.success_count,
