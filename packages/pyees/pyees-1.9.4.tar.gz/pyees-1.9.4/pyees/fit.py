    ...
    def scatter(self, ax, label=True, showUncert=True, **kwargs):
        ...
        if isinstance(ax, axes.Axes):
            ...
            if showUncert:
                ...
                ax.add_trace(go.Scatter(x = self.xVal, y = self.yVal, name = label, mode = 'markers', **kwargs))
        else:
...
if __name__ == "__main__":
    ...
    x = variable([1,2,3], '', [0.3, 1.2, 2.1])
    y = variable([5,8,11], '', [2.5, 5.3, 7.8])
    f = dummy_fit(x,y)
