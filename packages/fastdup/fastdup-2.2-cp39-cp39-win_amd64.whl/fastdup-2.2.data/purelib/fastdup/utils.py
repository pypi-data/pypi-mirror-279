    ...
    if (ret != 0 and 'JPY_PARENT_PID' in os.environ) or 'COLAB_JUPYTER_IP' in os.environ:
        if os.path.exists(local_error_file):
            ...
            try:
                ...
                with open(local_error_file, "r", encoding="rb") as f:
                    error = f.read()
                    data_type = "error" if ret != 0 else "info"
                    print(f"fastdup C++ {data_type} received: ", error[:5000], "\n")
                    if ret != 0:
...
def merge_stats_with_filenames(df, filenames):
    ...
    assert 'filename' in df.columns and df['filename'].values[0] is not None, f"Failed to find filename {df.head(3)}"
    return df
...
def find_model_path(model_path, d, high_accuracy=False):
    if high_accuracy:
        assert (not model_path.lower().startswith('dinov2')) and (model_path.lower() not in ['efficientnet', 'resnet50', 'clip', 'clip336', 'clip14']), \
            f"high_accuracy flag can not be combined with model_path {model_path}, please set high_accuracy=False or remove the model_path"
        model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'UndisclosedFastdupModel2.ort')
...
def create_tar(tar_filename, files, input_dir):
    ...
    assert os.path.isfile(tar_filename), f"Failed to create tar {tar_filename}"
