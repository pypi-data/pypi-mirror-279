...
from fastdup.vl.utils.common import parse_device_list
...
class Settings(metaclass=MyMeta):
    # Common
    MAX_NUM_OF_IMAGES_TO_EXPORT = ConfigValue(int, 1000)
    ...
    DATA_INGESTION_UX_ENABLED: ConfigValue = ConfigValue(str2bool, default=False)
    ...
    STEPS_TO_EXECUTE: ConfigValue = ConfigValue(str, '')
    ...
    SUPPORTED_IMG_FORMATS: ConfigValue = ConfigValue(
        list,
        [".png", ".jpg", ".jpeg", ".gif", ".giff", ".tif", ".tiff", ".heic", ".heif", ".bmp", ".webp", ".jfif"]
    )
    ...
    LIMIT_NUM_IMAGES: ConfigValue = ConfigValue(int) # if number of images > LIMIT_NUM_IMAGES then take random LIMIT_NUM_IMAGES
    LIMIT_NUM_OBJECTS: ConfigValue = ConfigValue(int) # if number of objects > LIMIT_NUM_OBJECTS then take random LIMIT_NUM_OBJECTS
    ...
    USE_RAM_EMBEDDINGS: ConfigValue = ConfigValue(str2bool, True)
    ...
    SEARCH_BASE_PATH: ConfigValue = ConfigValue(Path)
    SEARCH_API_PORT: ConfigValue = ConfigValue(int, 9998)
    SEARCH_API_URL: ConfigValue = ConfigValue(str, 'http://0.0.0.0')
    SEARCH_SERVICE_LOG_LEVEL: ConfigValue = ConfigValue(str2loglevel, logging.INFO)
