...
@timed
def dataframe_to_table(df: Union[pl.DataFrame, pd.DataFrame], table_name: str, columns=None):
    ...
    with get_session(autocommit=True) as session:
        ...
        with tempfile.NamedTemporaryFile(delete=True) as tmp:
            ...
            for col, handler in column_override_handlers.get(dialect, {}).get(table_name, {}).items():
                if isinstance(sorted_df, pl.DataFrame):  # polars
                    sorted_df = sorted_df.with_columns(pl.col(col).map_elements(handler, return_dtype=sorted_df.schema[col]).alias(col))
                else:
                    sorted_df[col] = sorted_df[col].apply(handler)

            if isinstance(sorted_df, pl.DataFrame):  # polars
                sorted_df.write_csv(tmp_path, include_header=False, )
            else:
                sorted_df.to_csv(tmp_path, index=False, header=False)

            if dialect == "postgresql":
                with (session.connection().connection.dbapi_connection.cursor() as cur,
                ...
                session.execute(sa.text(f"COPY {table_name}({fields}) FROM '{tmp_path}' "))
            else:
