...
class SimilarityClusterDB(BaseDB):
    ...
    async def get_cluster_labels(
        ...
        async with get_async_session() as session:
            ...
            offset: Optional[int] = Settings.CLUSTERS_PAGE_SIZE * context.page_number
            query = sql_template_utils.render(
                QueryModule.EXPLORATION,"cluster_page_labels.jinja2",
                # flags
                offset=offset,
                page_size=Settings.CLUSTERS_PAGE_SIZE,
                # context
            ...
            with logging_helpers.log_sql_query_time(
                    __name__, "get_cluster_labels", query, context.dict(), duration_seconds=1):
                rows: Sequence[RowMapping] = (await session.execute(sa.text(query), context.dict())).mappings().all()

        res: dict[UUID, dict[str, int]] = {}
        for row in rows:
            cluster_id = row['cluster_id']
            d: dict[str, int] = {}
            for tup in row['top_labels']:
                d[tup[0]] = int(tup[1])
            res[cluster_id] = d
    ...
    @staticmethod
    async def find_similar_objects_by_object(
            object_id: UUID
    ) -> Optional[list[tuple[UUID, float]]]:
        async with get_async_session() as session:
            query = """
                SELECT
                    similar_object_id, distance
                FROM
                    object_similarity_mapping
                WHERE
                    object_similarity_mapping.object_id = :object_id
                ORDER BY
                    distance DESC                   
                """
            query_params = {'object_id': object_id}

            rows: List[tuple[UUID, float]] = (await session.execute(sa.text(query), query_params)).all()
    ...
    async def get_similarity_cluster(context: ExplorationContext) -> Optional[SimilarityClusterView]:
        ...
        SimilarityClusterView.enrich_with_labels([cluster], cluster_labels_rows)
