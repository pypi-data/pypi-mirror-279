...
def read_process_csr_matrix(source_group: h5py.Group, row_indices: np.ndarray, col_indices: np.ndarray) -> csr_matrix:
    """
...
def read_process_csc_matrix(source_group: h5py.Group, row_indices: np.ndarray, col_indices: np.ndarray) -> csc_matrix:
    """
...
def read_process_matrix(
    ...
    """
    if is_csr:
        result = slicers_read.read_process_csr_matrix(source_group.file.filename, source_group.name, row_indices,
                                                      col_indices)
    ...
    result = slicers_read.read_process_csc_matrix(source_group.file.filename, source_group.name, row_indices,
                                                  col_indices)
...
def read_process_categorical_group(
    ...
    code_map = {old_code: new_index for new_index, old_code in enumerate(unique_codes[unique_indices])}

    # Map the old codes to the new codes, falling back to "NaN" for unknown codes
    new_codes = np.array([code_map.get(code, len(unique_new_categories) - 1) for code in codes], dtype=codes.dtype)
...
def read_process_dataframe_group(source_group: h5py.Group, indices: np.ndarray, is_obs: bool) -> pd.DataFrame:
    """
...
def read_process_group(source_group: h5py.Group, row_indices: np.ndarray, col_indices: np.ndarray) -> dict:
    """
...
def copy_attrs(source: h5py.AttributeManager, dest: h5py.AttributeManager, shape: tuple = None) -> None:
    """
...
def write_process_matrix(
    ...
    if is_csr:
        ...
        slicers_write.write_process_csr_matrix(source_group.file.filename, dest_file_path, source_group.name, row_indices, col_indices, batch_size)
    else:
        with write_lock:
            with h5py.File(dest_file_path, 'a') as f_dest:
                dest_group = f_dest.require_group(source_group.name)
        ...
        slicers_write.write_process_csc_matrix(source_group.file.filename, dest_file_path, source_group.name, row_indices, col_indices, batch_size)
    with write_lock:
        with h5py.File(dest_file_path, 'a') as f_dest:
            dest_group = f_dest.require_group(source_group.name)
            copy_attrs(source_group, dest_group, shape=(len(row_indices), len(col_indices)))
...
def write_process_dataset(
    ...
    Args:
        ...
        row_indices (np.ndarray): The indices of the rows to include in the slice.
        col_indices (np.ndarray): The indices of the columns to include in the slice.
        parent_encoding_type (str, optional): The encoding type of the parent group. Default is None.
        parent_group_name (str, optional): The name of the parent group. Default is None.
    """
...
def write_process_group(
    ...
    """
    Processes and writes a group to the destination HDF5 file, handling different encoding types.
    ...
    Args:
        ...
        row_indices (np.ndarray): The indices of the rows to include in the slice.
        col_indices (np.ndarray): The indices of the columns to include in the slice.
        batch_size (int, optional): The size of the batch for processing large datasets. Default is 1000.
    """
    parent_encoding_type = source_group.attrs.get('encoding-type', None)
