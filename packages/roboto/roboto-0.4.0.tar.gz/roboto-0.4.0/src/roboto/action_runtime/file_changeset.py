...
from roboto.env import RobotoEnvKey, default_env
...
class FilesChangesetFileManager:
    ...
    def __init__(self):
        path_str = default_env.file_metadata_changeset_file
        if path_str is None:
            raise ValueError(
    ...
    def apply_to_dataset(self, dataset: datasets.Dataset):
        if default_env.output_dir is None:
            raise ValueError(
        ...
        for relevant_file in relevant_files:
            relevant_file.update(updates_by_path[relevant_file.relative_path])
