...
def add_patient_to_database(database_name : str, patient_path : str):
    ...
    new_patient = load_patient(patient_path)
    database = load_database(database_name)
    for patient in database:
        if int(float(patient["patient id"])) == int(new_patient.patient_id):
            return
    # patient is not in the databse, so we can just add it
    parameter_list = load_parameter_list(database_name)
...
def remove_patient_from_database(database_name, patient_path):
...
def load_patient(patient_path : str) -> 'Patient':
    ...
    """    
    return Patient(patient_path, wanted_structures="default", ctvfallback="prostate")
...
def get_parameter_list_path(database_name : str) -> str:
...
def load_parameter_list(database_name : str) -> list[str]:
...
def write_database(database, database_name):
...
def update_database(database_name : str):
    ...
    parameter_set = load_parameter_list(database_name)

    database = load_database(database_name)
    parameter_set = ["path"] + parameter_set
    ...
    for patient in database:
        ...
        for param in parameter_set:
            if param in patient:
                new_pat[param] = patient[param]
                continue 
            # now we need to load the patient because the parameter was not found
            if loaded_patient is None:
                loaded_patient = load_patient(patient_path)
            new_pat[param] = parameter_parser(loaded_patient, param)
...
def edit_database(database: list[dict], new_param_file : str):
...
def create_new_database(new_database_path : str, parameter_file: str, only_name : bool = False):
...
def add_database_to_config(new_database_path : str, parameter_file: str):
