...
from threading import Barrier, Condition, RLock, Thread, get_ident
...
def syscall_impl(f: Callable[P, R]) -> Callable[P, R | int]:
    ...
    def inner(*args: P.args, **kwargs: P.kwargs) -> R | int:
        ...
        try:
            res = f(*args, **kwargs)
            span.set_attribute("result", str(res))
...
class RequestThread:
    ...
    def call_as_future(self, cb: Coroutine[Any, Any, T]) -> Future[T]:
        span = get_current_span()
        span.set_attributes({"callback": cb.__qualname__})
...
# AWS rate limits per presign
MAXIMUM_DOWNLOADS_PER_URL = 10
...
class LDataFile:
    ...
    download_part_size: ClassVar[int] = 4 * 2**20

    max_concurrent_downloads: ClassVar[int] = 100
    concurrent_downloads: ClassVar[Semaphore] = Semaphore(max_concurrent_downloads)

    max_concurrent_presigns: ClassVar[int] = 100
    concurrent_presigns: ClassVar[Semaphore] = Semaphore(max_concurrent_presigns)
    ...
    presigned_urls: dict[int, Future[str]] = {}
    ...
    def _download(self):
        ...
        async def get_signed_url() -> str:
            async with self.concurrent_presigns:
                span = get_current_span()
        ...
        async def schedule_downloads(content_size: int):
            ...
            nrof_parts = ceil(content_size / self.download_part_size)
            nrof_urls = ceil(nrof_parts / MAXIMUM_DOWNLOADS_PER_URL)

            self.presigned_urls = {i: self.fs.req_thread.call_as_future(get_signed_url()) for i in range(nrof_urls)}

            for i in range(nrof_parts):
                self.download_futures.append(
                    self.fs.req_thread.call_as_future(download_part(i))
                )

        @trace_app_function
        async def download_part(i: int):
            span = get_current_span()
            ...
            url = await asyncio.wrap_future(self.presigned_urls[int(i / MAXIMUM_DOWNLOADS_PER_URL)])

            async with self.concurrent_downloads:
                async with self.fs.generic_sess.get(url, headers=headers) as res:
                    ...
                    with self.part_finished_in_bin_cvs[i % self.cv_bins]:
                        self.bitmap |= 1 << i
                        self.part_finished_in_bin_cvs[i % self.cv_bins].notify_all()
        ...
        self.bitmap = 0
        self.part_finished_in_bin_cvs = [Condition() for i in range(self.cv_bins)]
    ...
    def read(self, size, offset):
        ...
        # Handle case when offset is greater than actual file size
        end = ceil(min(size + offset, self.content_size) / self.download_part_size)
        start = ceil(offset / self.download_part_size)

        mask = 1 << (end - start)
        mask -= 1
        mask = mask << start
        # The final bit mask has 1s in [end, start] and 0s elsewhere

        with self.part_finished_in_bin_cvs[(end - 1) % self.cv_bins]:
            while (self.bitmap & mask) != mask:
                self.part_finished_in_bin_cvs[(end - 1) % self.cv_bins].wait()
...
class LDataFS(Fuse):
    ...
    def readdir(self, path: str, offset: int):
        ...
        with app_tracer.start_as_current_span("readdir-generator") as span:
            span.set_attributes(
                dict_to_attrs(
    ...
    def invalidate_cache(self, path: str):
        self.ldata_path_cache.pop(path, None)
        parent_path = str(PurePosixPath(path).parent)
        self.ldata_node_cache.pop(parent_path, None)
        self.ldata_path_cache.pop(parent_path, None)
