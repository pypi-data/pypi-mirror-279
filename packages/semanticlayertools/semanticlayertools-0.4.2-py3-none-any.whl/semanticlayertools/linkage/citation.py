...
class Couplings:
    ...
    def _checkFileExists(self, basefilename:str) -> Path:
        ...
        if outfilepathGC.is_file() or outfilepathNCOL.is_file():
            text = f"Output file for GC or Net at {self.outfilepathGC} exists. Please move or delete."
            raise OSError(text)
    ...
    def _writeGraphMetadata(self, citetype:str, yearname: int, graph: ig.Graph, *, writeGC:bool=True) -> None:
    ...
    def _getCombinations(self, chunk: pd.DataFrame) -> list:
        ...
        if isinstance(self.limitRefLength, int):
