...
class BaseToken:
    ...
    def ensure_token(self) -> None:
        ...
        self.acquiring = (  # pylint: disable=possibly-used-before-assignment
...
class Token(BaseToken):
    ...
    def _refresh_authorized_user(self, timeout: int) -> TokenResponse:
        payload = urlencode({
    ...
    def _refresh_service_account(self, timeout: int) -> TokenResponse:
        now = int(time.time())
        assertion_payload = {
            'aud': self.token_uri,
    ...
    def _impersonate(self, token: TokenResponse,
    ...
    def refresh(self, *, timeout: int) -> TokenResponse:
        ...
        if self.target_principal:
            resp = self._impersonate(resp, timeout=timeout)
...
class IapToken(BaseToken):
    ...
    def _get_iap_client_id(self, *, timeout: int) -> str:
        """
        ...
        """
        resp = self.session.head(self.app_uri, timeout=timeout,
    ...
    def _refresh_authorized_user(
        ...
        refresh_content = refresh_resp.json()
    ...
    def refresh(self, *, timeout: int) -> TokenResponse:
        iap_client_id = self._get_iap_client_id(timeout=timeout)
        if self.token_type == Type.AUTHORIZED_USER:
            resp = self._refresh_authorized_user(
                iap_client_id, timeout)
        elif self.token_type == Type.GCE_METADATA:
            resp = self._refresh_gce_metadata(
                iap_client_id, timeout)
        elif self.token_type == Type.SERVICE_ACCOUNT:
            resp = self._refresh_service_account(
                iap_client_id, timeout)
        else:
            raise Exception(f'unsupported token type {self.token_type}')
