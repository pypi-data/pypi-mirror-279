...
from dvcx.node import DirType, DirTypeGroup, Entry, Node, NodeWithPath, get_path
from dvcx.sql.types import Int, SQLType
from dvcx.storage import StorageURI
from dvcx.utils import sql_escape_like
...
class AbstractWarehouse(ABC, Serializable):
    ...
    def dataset_select_paginated(
        ...
        try:
            while True:
                ...
                for row in results:
                    processed = True
                    yield DatasetRow(zip(cols_names, row))
                    num_yielded += 1
    ...
    def nodes_dataset_query(
        ...
        select_query = dr.dataset_query(*(column_names or []))
        if path is None:
            return select_query
        if recursive:
            root = False
    ...
    def prepare_entries(
        ...
        def _prepare_entry(entry: Entry):
            ...
            return {
                f: d.get(f) for f in [c for c in DATASET_CORE_COLUMN_NAMES if c != "id"]
            }
    ...
    def _get_nodes_by_glob_path_pattern(
        self, dataset_rows, path_list: list[str], glob_name: str
    ) -> Iterator[Node]:
    ...
    def _get_node_by_path_list(
        self, dataset_rows, path_list: list[str], name: str
    ) -> Node:
        """
    ...
    def _populate_nodes_by_path(self, dataset_rows, path_list: list[str]) -> list[Node]:
        """
    ...
    @staticmethod
    def expand_query(dir_expanded_query, dataset_rows):
        dr = dataset_rows
    ...
    def get_node_by_path(self, dataset_rows, path: str) -> Node:
    ...
    def expand_path(self, dataset_rows, path: str) -> list[Node]:
    ...
    def select_node_fields_by_parent_path_tar(
        self, dataset_rows, parent_path: str, fields: Iterable[str]
    ) -> Iterator[tuple[Any, ...]]:
        """
        Gets latest-version file nodes from the provided parent path
        """
    ...
    def size(
        self, dataset_rows, node: Union[Node, dict[str, Any]], count_files: bool = False
    ) -> tuple[int, int]:
        """
    ...
    def _find_query(
        ...
        if fields is None:
            fields = DATASET_CORE_COLUMN_NAMES
