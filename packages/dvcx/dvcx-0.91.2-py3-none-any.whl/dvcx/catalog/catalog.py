...
FEATURE_CLASSES = ["ShallowFeature", "Feature"]
...
class Catalog:
    ...
    def attach_query_wrapper(self, code_ast):
        if code_ast.body:
            ...
            if isinstance(last_expr, ast.Expr):
                new_expressions = [
                    ast.Import(
                        names=[ast.alias(name="dvcx.query.dataset", asname=None)]
                    ),
                    ast.Expr(
                        value=ast.Call(
                            func=ast.Attribute(
                                value=ast.Attribute(
                                    value=ast.Attribute(
                                        value=ast.Name(id="dvcx", ctx=ast.Load()),
                                        attr="query",
                                        ctx=ast.Load(),
                                    ),
                                    attr="dataset",
    ...
    def _get_row_uid(self, row: DatasetRow) -> UniqueId:
        return UniqueId(
            row["source"],
            row["parent"],
            row["name"],
    ...
    def run_query(
        ...
        if feature_code:
            envs["DVCX_FEATURE_CLASS_SOURCE"] = json.dumps(
