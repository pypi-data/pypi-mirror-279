...
def test_udf(cloud_test_catalog):
    ...
    default_schema = {
        ...
        for c in catalog.warehouse.dataset_row_cls.dataset_default_columns()
        if isinstance(c.type, SQLType)
    }
    assert dataset.schema == default_schema | {"name_len": Int}
...
def test_rrow_generator_with_new_columns(cloud_test_catalog, dogs_dataset):
    catalog = cloud_test_catalog.catalog
...
def test_subtract(cloud_test_catalog):
    ...
    result = q.results(row_factory=from_result_row)
...
def test_from_pandas(cloud_test_catalog):
    import pandas as pd
    ...
    DatasetQuery.from_pandas(df, name="df", catalog=catalog)
    dataset = catalog.get_dataset("df")
    assert dataset.schema == {
        "int": Int64,
        "int32": Int32,
        "datetime": DateTime,
    ...
    # `bytes` is not supported by the interchange protocol
    pa_tbl = pa.Table.from_pandas(df.drop("bytes", axis=1))
    DatasetQuery.from_dataframe(pa_tbl, name="pa", catalog=catalog)
    dataset = catalog.get_dataset("pa")
    assert dataset.schema == {
        "int": Int64,
        "int32": Int32,
        "datetime": DateTime,
...
def test_from_pandas_udf(cloud_test_catalog):
    import pandas as pd

    @udf(params=("a", "b"), output={"prod": Int})
    def prod(a, b) -> tuple[int]:
        return (a * b,)

    catalog = cloud_test_catalog.catalog
    df = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
    query = (
        DatasetQuery.from_pandas(df, catalog=catalog)
        .add_signals(prod)
        .save("df-signals")
    )
    dataset = catalog.get_dataset("df-signals")
    schema = dataset.schema
    del schema["id"]

    assert schema == {"a": Int64, "b": Int64, "prod": Int}
    records = query.to_records()
    for expected in [
        {"id": ANY, "a": 1, "b": 3, "prod": 3},
        {"id": ANY, "a": 2, "b": 4, "prod": 8},
    ]:
        assert any(row == expected for row in records)
...
def test_from_parquet(tmp_dir, cloud_test_catalog):
    import pandas as pd
    ...
    df["int32"] = df["int32"].astype("int32")
    uri = tmp_dir / "test.parquet"
    df.to_parquet(uri)

    DatasetQuery.from_parquet(uri, name="df", catalog=catalog)
    dataset = catalog.get_dataset("df")
    assert dataset.schema == {
        "int": Int64,
        "int32": Int32,
        ...
        "string": String,
        "category": String,
        "list": dataset.schema["list"],
    }
    # assert Array(Int64) == Array(Int64) fails since they aren't the same instance
    assert dataset.schema["list"].to_dict() == {
        "type": "Array",
