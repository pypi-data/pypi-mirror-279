...
class ThreadPoolMessage:
    ...
    timeout: int = 5
    content: Any = None
    hook: Callable[[Any, queue.Queue, queue.Queue, Any], None] = None
...
def create_threadpool_message(
    ...
    Args:
        ...
        hook(Callable): The hook function to call after the message was processed.
...
class ThreadPool:
    ...
    def __init__(
        self,
        default_thread_pool_user_object: ThreadPoolUserObject,
        ...
        self.thread_pool_user_objects["default"] = (
            ...
            else ThreadPoolUserObject()
        )
        # read num_of_threads from environment
        num_of_threads = decouple.config("THREADPOOL.NUMOFTHREADS", num_of_threads, cast=int)
        self.num_of_threads = num_of_threads if num_of_threads is not None else 5
    ...
    # -------------------------------------------------------------
    def register_message_handler(self, msg_type: str, msg_handler: ThreadPoolUserObject) -> None:
        """
        Registers a message handler for a specific message type.
        """
    ...
    # -------------------------------------------------------------
    def get_message_handler(self, msg_type: str) -> ThreadPoolUserObject:
        """
        Get message handler for a specific message type.
        """
    ...
    def callback(
        ...
        for message in iter(input_queue.get, "##STOP##"):
            ...
            for try_counter in range(message.retry):
                with TimerThread(message.timeout, threading.get_ident()):
                    ...
                    try:
                        ...
                        print(
                            f"##TIMEOUT## - try: ({try_counter+1} of {message.retry}) - "
                            f"{message.content}"
                        )
                    except Exception as e:
                        print(
                            f"##EXCEPTION## - try: ({try_counter+1} of {message.retry}) - "
                            f"{message.content} - {e}"
                        )
            ...
            if result not in (None, 0):
                print(f"##FAILURE## - command - {message.content}")
                # check message.abort_on_error
                if message.abort_on_error:
                    input_queue.task_done()
                    print(
                        f"##ABORT## - retried: ({try_counter+1} of {message.retry}) - "
                        f"{message.content}"
                    )
                    raise ValueError(
                        f"##ABORT## - retried: ({try_counter+1} of {message.retry}) - "
                ...
                output_queue.put(
                    {
                        f"##EXCEPTION## - retried: "
                        f"({try_counter+1} of {message.retry}) -": message.content
                    }
                )
            else:
                # check to call the hook function
                if message.hook is not None:
                    message.hook(thread_local_data, input_queue, output_queue, message.content)
