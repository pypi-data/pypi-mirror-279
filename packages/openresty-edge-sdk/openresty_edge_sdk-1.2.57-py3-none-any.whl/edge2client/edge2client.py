...
class Edge2Client(object):
    ...
    def prepare_conseq(self, conseq):
        ...
        basic_auth_groups = None
        for item in conseq:
            for action_name, action_spec in item.items():
                if action_name == "enable-basic-authentication":
                    if action_spec.get('group_name', None) and not action_spec.get('app_auth_id', None):
                        if basic_auth_groups is None:
                            basic_auth_groups = dict()
                            groups = self.get_all_app_basic_auth_user_groups()
                            for group in groups:
                                basic_auth_groups[group['name']] = group['id']

                        group_name = action_spec['group_name']
                        if not basic_auth_groups.get(group_name, None):
                            raise Exception('basic auth group not found: ' + group_name)

                        action_spec['app_auth_id'] = basic_auth_groups.get(group_name, None)
                        del(action_spec['group_name'])
    ...
    def upload_static_file(self, filename, content, label=None, gid=None):
        gids = ""
        if gid:
            gids = ",".join(str(id) for id in gid)

        url = "static/file?gid={}&name={}&label={}".format(gids, filename, label)
    ...
    def get_all_static_files(self):
        return self.get_all("static/list")
