...
# Comma-separated list of the cluster partitions where jobs can run.
# Existing partitions can be listed with the following SLURM command: scontrol show partitions.
# The default value of this parameter can be configured in the defaults.cfg file.
...
# In the CP3 cluster, SLURM has a backfill scheduling mechanism that will start lower priority jobs if doing so does not delay the expected 
...
# The default value of this parameter can be configured in the defaults.cfg file.
# At CP3 the default is "0-04:00" (4 hours).
...
# In the CP3 cluster, SLURM will kill the jobs that use more memory (RSS) than requested.
# Obviously, the more you request the longer your jobs will wait in the queue for the resources to become available.
# The default value of this parameter can be configured in the defaults.cfg file.
# At CP3 the default is "2048" (2 GB).
...
# If a relative path is given, it will be relative to the work directory 'sbatch_chdir'.
...
# Defaults to the directory defined in 'sbatch_chdir'.
#config.batchScriptsDir = config.sbatch_chdir
...
# The tarball will be copied to, and unpacked in, each job's local scratch directory on the worker nodes.
...
# then all txt files present in ./mydir will be included in the input sandbox and when unpacked on the worker nodes
...
# Defaults to the directory defined in 'sbatch_chdir'.
#config.inputSandboxDir = config.sbatch_chdir
...
# This parameter specifies a scratch directory where the job should run.
# It could well be defined via an environment variable: config.scratchDir = "${scratchDir}".
# In that case, the environment variable must exist during job execution.
# At CP3, each job has a scratch directory that is defined, created and cleaned-up by SLURM;
# the scratch directory name is defined after job submission and is exported to an environment
# variable named LOCALSCRATCH.
# If you don't want to use a scratch directory, set this parameter to an empty string or None.
# The default value of this parameter can be configured in the defaults.cfg file.
# At CP3 the default is "${LOCALSCRATCH}".
#config.scratchDir = "${LOCALSCRATCH}"

# Set this parameter to True to create/remove the scratch directory before/after
# the user payload starts/ends.
# Defaults to False.
...
# Only file basenames are allowed; i.e. no directory paths (not even relative paths).
# The file will be created in each job's local scratch directory on the worker nodes.
...
# To not setup any environment, set this parameter to an empty string or None.
# Defaults to an empty string.
...
# This parameter is only used when 'useJobArray' is True.
# It must be a non-negative integer (or None).
# It specifies the maximum number of jobs from the job array that can be simultaneously running.
...
# This configuration parameter is only relevant when input parameters are provided
# to the jobs (via the 'inputParamsNames' and 'inputParams' configuration parameters).
# Via this configuration parameter one can choose whether to write the input parameters
# into the batch script itself or into a separate file. The last option could be useful
# to avoid SLURM submission errors due to the batch script being too large in size.
# Defaults to True for backwards compatibility reasons.
#config.putInputParamsInBatchScript = True

# This configuration parameter is only relevant when 'putInputParamsInBatchScript' is False.
# This is the name of the file where the job input parameters will be written in that case.
# The file will be created in the current working directory, added to the input sandbox
# and removed (the file can be recovered from the input sandbox).
# Defaults to "input_params_<timestamp>_<4-random-characters>.sh".
#config.inputParamsFilename = ""

# This flag tells whether to run the payload with Apptainer (Singularity) in a container.
# The default value of this parameter can be configured in the defaults.cfg file.
# At CP3 the default is True.
#config.apptainer = True

# This parameter is only relevant when 'apptainer' is True.
# It specifies the image to use with Apptainer (Singularity).
# The default value of this parameter can be configured in the defaults.cfg file.
# At CP3 the default is /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/el7:x86_64.
#config.apptainerImage = "/cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/el7:x86_64"
...
# 4 Checkpointing
#
# So far only the signaling to the payload was implemented. The user should implement a chackpointing in the payload.
#--------------------------------------------------------------------------------

# This flag tells whether to enable checkpointing (only the signaling to the payload).
# To disable the checkpointing, you can set 'checkpointSignal' to an empty string
# and 'checkpointSecsBeforeTimeout' to 0.
# Defaults to False.
#config.doCheckpointing = False

# This parameter is only relevant when 'doCheckpointing' is True.
# For the moment only signals 'SIGUSR1' and 'SIGUSR2' are allowed.
# When receiving and trapping this signal, the job will kill the payload and exit.
# It is the job of the payload to do the checkpointing.
# In particular, if 'checkpointSecsBeforeTimeout' is greater than 0, this signal
# will be send to the batch shell 'checkpointSecsBeforeTimeout' seconds before the time allocation limit.
# Note that the scancel command also sends signals to jobs. So if you do scancel
# with the signal defined here, the job will be checkpointed and exit.
# A job that has trapped the signal and exited,
# is expected to have status FAILED with SLURM exit code 10 (for SIGUSR1) or 12 (for SIGUSR2).
# If set to an empty string, no system signal will be trapped for checkpoint-and-exit.
# Defaults to an empty string.
#config.checkpointSignal = ""

# This parameter is only relevant when 'doCheckpointing' is True.
# This parameter specifies how many seconds before the time allocation limit to send a system signal 'checkpointSignal' to the job.
# It must be an integer.
# If set to 0 (or less than 0), the signal will not be programmed.
# It uses the SLURM sbatch option --signal. So you can not define this parameter
# and at the same time use the SLURM sbatch option --signal for other purpose.
# Defaults to 0.
#config.checkpointSecsBeforeTimeout = 0

# This parameter is only relevant when 'doCheckpointing' is True and 'checkpointSignal' is defined.
# This parameter specifies the code that you want the job to run
# just after receiving the checkpoint signal.
#config.checkpointCode = ""
