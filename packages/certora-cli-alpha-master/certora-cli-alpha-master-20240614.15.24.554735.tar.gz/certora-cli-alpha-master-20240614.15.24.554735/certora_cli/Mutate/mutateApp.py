...
from EVMVerifier.certoraContextAttribute import ContextAttribute
    ...
    if metadata_file.exists():
        with metadata_file.open() as orig_run_conf:
            ...
            if Constants.CONF in metadata:
                return CertoraContext(**metadata[Constants.CONF])
            else:
...
class WebUtils:
    ...
    def config_server(args: SimpleNamespace) -> str:
        ...
        if prover_context:
            server_in_prover_conf = getattr(prover_context, Constants.SERVER, None)
...
class MutateApp:
    ...
    def fetch_and_extract_inputs_dir_from_orig_run(self) -> Path:
        ...
        zip_output_url = self.orig_run.replace(Constants.OUTPUT, Constants.ZIPOUTPUT)
    ...
    def read_conf_from_orig_run(self) -> None:
        ...
        if len(dict) != 1 and Constants.MUTATIONS not in dict:
        ...
        self.sources_dir = input_dir / Constants.CERTORA_SOURCES
        ...
        shutil.copy(Constants.ORIG_RUN_PROVER_CONF, self.sources_dir)
        ...
        if Constants.MUTATIONS in dict:
            setattr(self.prover_context, Constants.MUTATIONS, dict[Constants.MUTATIONS])
    ...
    def validate_mutants_in_source_tree(self) -> None:
        if self.manual_mutants:
            ...
            for mutant in self.manual_mutants:
                file_to_mutate = mutant.get('file_to_mutate', None)
    ...
    def submit(self) -> None:
        ...
        if not self.orig_run:
            ...
            success, certora_run_result = self.run_certora_prover(self.conf, mutation_test_id, msg=Constants.ORIGINAL, is_mutant=False)
    ...
    def check_prover_context(self) -> None:
        prover_attrs = [attr.get_conf_key() for attr in ContextAttribute]
    ...
    def get_common_solc_flags(self) -> List[str]:
        ...
        if hasattr(self.prover_context, Constants.SOLC_OPTIMIZE) or \
           hasattr(self.prover_context, Constants.SOLC_OPTIMIZE_MAP):
            common_flags.extend(['--optimize'])

        if hasattr(self.prover_context, Constants.SOLC_ALLOW_PATH):
            common_flags.extend([f'--allow-paths, {self.prover_context.solc_allow_path}'])

        if hasattr(self.prover_context, Constants.SOLC_EVM_VERSION):
            common_flags.extend(['--evm-version', self.prover_context.solc_evm_version])

        if hasattr(self.prover_context, Constants.PACKAGES):
    ...
    def compile_manual_mutants(self, mutant: Mutant, trg_dir: Path) -> None:
        ...
        via_ir_flag = []
        if getattr(self.prover_context, Constants.SOLC_VIA_IR, '') or \
                getattr(self.prover_context, Constants.SOLC_EXPERIMENTAL_VIA_IR, ''):
