...
from django_htmx_include.cache import ctx_cache


def get_include_html(fragment_id, primary_nodes=""):
    return (
        f"""<div hx-get="/__htmx-include__/?fragment_id={fragment_id}"""
...
class TemplateTagTests(SimpleTestCase):
    ...
    def setUp(self):
        super().setUp()
        node_klass = "django_htmx_include.templatetags.xinclude.HtmxIncludeNode"
        ...
        cache_patcher = mock.patch("django_htmx_include.cache.ContextCache.cache")
        self.cache = cache_patcher.start()
        self.addCleanup(cache_patcher.stop)

    def test_htmx_include(self):
        template = """
        {% load xinclude %}
    ...
    def test_skips_unpickable_objects(self):
        ...
        """
        t = engines["django"].from_string(template)
        with mock.patch("django_htmx_include.cache.logger") as logger:
    ...
    def test_sync_include(self):
        ...
        """
        t = engines["django"].from_string(template)
        rendered = t.render(request=mock.MagicMock(htmx_sync_include=True)).strip()
        self.assertEqual(rendered, "Hello")

    def test_htmx_include_template_variable(self):
        template = """
        {% load xinclude %}
    ...
    def test_htmx_include_template_variable_iterable(self):
        template = """
        {% load xinclude %}
        {% xinclude template_name %}{% endxinclude %}
        """
...
class ViewTests(SimpleTestCase):
    ...
    def test_view_renders_passed_template(self):
        ...
        url = "/__htmx-include__/?fragment_id=abc123"
    ...
    def test_missing_context(self):
        ...
        url = "/__htmx-include__/?fragment_id=missing"
        with mock.patch("django_htmx_include.views.logger") as logger:
            response = self.client.get(url)
    ...
    def test_only_1_fragment_id_allowed(self):
        self.cache.get.return_value = {"meta": {"user": self.anon}, "context": {}}
        url = "/__htmx-include__/?fragment_id=abc123&fragment_id=abc123"
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
...
class AuthTests(TestCase):
    ...
    @override_settings(
        INSTALLED_APPS=["django.contrib.auth", "django_htmx_include", "tests"],
    ...
    @override_settings(
        INSTALLED_APPS=["django_htmx_include", "tests"],
        TEMPLATES=get_templates_settings(
            ["django.contrib.auth.context_processors.auth"]
        ),
        MIDDLEWARE=[],
    ...
    @override_settings(INSTALLED_APPS=["django_htmx_include", "tests"], MIDDLEWARE=[])
