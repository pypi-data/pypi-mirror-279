...
from django_htmx_include import logger
from django_htmx_include.conf import conf
from django_htmx_include.exceptions import FragmentNotFoundError
...
class ContextCache:
    ...
    def cache(self) -> BaseCache:
        return caches[conf.HTMX_INCLUDE_CACHE_ALIAS]
    ...
    def set(self, key: str, val: dict[str, Any]) -> None:
        ...
        It uses the ``HTMX_INCLUDE_CACHE_TIMEOUT`` setting if specified,
        otherwise Django will use the timeout argument of the appropriate backend
        in the CACHES setting.
        """
        if conf.HTMX_INCLUDE_CACHE_TIMEOUT is not None:
            set_kwargs = {"timeout": conf.HTMX_INCLUDE_CACHE_TIMEOUT}
        else:
            set_kwargs = {}
