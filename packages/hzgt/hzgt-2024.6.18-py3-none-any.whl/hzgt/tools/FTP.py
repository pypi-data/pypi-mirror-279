...
from ..strop import restrop, reputstr
from ..fileop import get_file_size, bit_unit_conversion
...
try:
    ...
    print(err)
    os.system(INSTALLCMD("pyftpdlib==1.5.9"))
    from pyftpdlib.authorizers import DummyAuthorizer
    from pyftpdlib.handlers import FTPHandler, ThrottledDTPHandler
    from pyftpdlib.servers import FTPServer
    from pyftpdlib.log import LogFormatter
...
class Ftpserver:
    ...
    def add_user(self, homedir: str, username: str = "anonymous", password: str = "", perm: str = ""):
        ...
        :param password: 用户密码
        :param homedir: 目录
        :param perm: 权限组合体 ”elradfmw“ 表示拥有所有权限
        :return:
        """
...
class Ftpclient:
    def __init__(self, host: str, port: int = 2121, username: str = "anonymous", password: str = "", encoding: str = "utf-8"):
        """
        FTP客户端
        :param host: 目标主机IP
        :param port: 端口
        :param username: 用户昵称 为空则为 anonymous
        :param password: 密码
    ...
    def getfile(self, server_filename: str, savepath: str = "FTP_Files", savename: str = "", blocksize: int = 8 * 1024):
        ...
        sfsize = bit_unit_conversion(self.size(server_filename))  # 转换总大小
