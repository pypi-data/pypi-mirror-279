...
from etao_ds_langchain.retrievers import EnsembleRetriever
from etao_ds_langchain.vectorstores import ElasticKeywordsSearch, Milvus
from etao_ds_langchain.rag.init_retrievers import (
    BaselineVectorRetriever,
    KeywordRetriever,
    MixRetriever,
    SmallerChunksVectorRetriever,
)
from etao_ds_langchain.rag.scoring.ragas_score import RagScore
from etao_ds_langchain.rag.extract_info import extract_title
from etao_ds_langchain.rag.utils import import_by_type, import_class


class etao_dsRagPipeline:
    ...
    def __init__(self, yaml_path) -> None:
        ...
        if self.params['elasticsearch'].get('extract_key_by_llm', False):
            extract_key_prompt = import_class(f'etao_ds_langchain.rag.prompts.EXTRACT_KEY_PROMPT')
    ...
    def retrieval_and_rerank(self, question, collection_name, max_content=100000):
        ...
        if self.params['post_retrieval']['with_rank'] and len(docs):
            if not hasattr(self, 'ranker'):
                ...
                rerank_object = import_class(f'etao_ds_langchain.rag.rerank.{rerank_type}')
    ...
    def question_answering(self):
        ...
        if 'prompt_type' in self.params['generate']:
            ...
            prompt = import_class(f'etao_ds_langchain.rag.prompts.{prompt_type}')
        else:
            prompt = None
    ...
    def score(self):
        ...
        if metric_params['type'] == 'etao_ds-ragas':
            score_params = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params', type=str, default='config/test/baseline_s2b.yaml', help='etao_ds rag params')
    # 解析参数
    args = parser.parse_args()

    rag = etao_dsRagPipeline(args.params)
