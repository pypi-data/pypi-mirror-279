...
from ragas.metrics import AnswerCorrectness, AnswerCorrectnessetao_ds, AnswerRecalletao_ds
...
class RagScore:
    ...
    def ragas_answer_correctness_etao_ds(self, dataset: Dataset) -> pd.DataFrame:
        answer_correctness = AnswerCorrectnessetao_ds(batch_size=self.batch_size)
        result = evaluate(
            dataset=dataset,
    ...
    def ragas_answer_recall_etao_ds(self, dataset: Dataset) -> pd.DataFrame:
        answer_recall =AnswerRecalletao_ds(batch_size=self.batch_size, 
    ...
    def score(self) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
        ...
        for metric_name in self.metrics:
            ...
            if metric_name =='answer_recall_etao_ds':
                if self.gt_split_column:
...
if __name__ == '__main__':
    params = {
        'excel_path': '/home/gulixin/workspace/llm/etao_ds/src/etao_ds-langchain/experimental/rag/data/test.xlsx',
        'save_path': '/home/gulixin/workspace/llm/etao_ds/src/etao_ds-langchain/experimental/rag/data',
        'question_column': '问题',
        'gt_column': 'GT',
        'answer_column': 'rag_answer',
        'query_type_column': '问题类型',
        # 'metrics': ['answer_correctness_etao_ds'],
        'metrics': ['answer_recall_etao_ds'],
        'batch_size': 10,
