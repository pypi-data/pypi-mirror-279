...
def rag_evaluate(excel_file, metric='answer_recall_etao_ds', batch_size=5):
        ...
        with gr.Row():
            with gr.Column(scale=2):
                with gr.Row():
                    ...
                    with gr.Column():
                        metric_options = ["answer_recall_etao_ds", "answer_correctness_etao_ds"]
                        metric = gr.components.Dropdown(label="评估方法", choices=metric_options, default=metric_options[0], interactive=True)
                        # metric = gr.Textbox(label='评估方法', value='answer_recall_etao_ds', interactive=True, lines=2)
