...
import httpx
import yaml
from etao_ds_langchain.gpts.load_tools import get_all_tool_names, load_tools
from etao_ds_langchain.gpts.utils import import_by_type, import_class
...
class ConfigurableAssistant(RunnableBinding):
    ...
    def __init__(
        ...
        **others: Any,
        ...
        agent_executor_object = import_class(f'etao_ds_langchain.gpts.agent_types.{agent_executor_type}')
...
class etao_dsAssistant:
    ...
    def __init__(self, yaml_path) -> None:
        ...
        assistant_message = import_class(f'etao_ds_langchain.gpts.prompts.{prompt_type}')
...
if __name__ == "__main__":
    ...
    # query = '去年这个时候的股价是多少？'
    # etao_ds_assistant = etao_dsAssistant("config/base_scene.yaml")
    # etao_ds_assistant = etao_dsAssistant("config/knowledge_scene.yaml")
    etao_ds_assistant = etao_dsAssistant("config/rag_scene.yaml")
    result = etao_ds_assistant.run(query, chat_history=chat_history)
    for r in result:
        print(f'------------------')
        print(type(r), r)
