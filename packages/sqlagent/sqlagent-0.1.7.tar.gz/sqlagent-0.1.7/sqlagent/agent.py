...
class SQLAgent:
    def __init__(
        ...
        self.obj_index = ObjectIndex.from_persist_dir(
            persist_dir=self.object_index_dir, object_node_mapping=self.table_node_mapping
        )
    ...
    async def ingest_db(self) -> ObjectIndex:
        ...
        return await arun(db_url=self.db_url, api_key=self.api_key, object_index_dir=self.object_index_dir, model=self.model)
