...
def prompt_action(args):
    ...
    if (init == 'chat'):
        initialize_prompt_chat_mode(classifier, param)
    # TODO introduce update command that updates the config_prompt files in case new files have been generated
    # Proposed call
    # if (init == 'update'):
    #     update_prompt_config_files(classifier, param)
...
def prompt_parser(subparsers):
    ...
    prompt_parser.add_argument("steps", choices=['init', 'load', 'send', 'extract', 'chat', 'init-rag'],
                               help="steps to be performed: 'init', 'load', 'send', 'extract', 'chat', or 'init-rag' the prompt config files in the prompt directory") #TODO needs to be updated in case the update command is introduced
...
def chat_parser(subparsers):
    chat_parser = subparsers.add_parser("chat", help="Command line chatbot. Chat control with GO | CONTINUE | END")
