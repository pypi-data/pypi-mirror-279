...
class OpsRampEnv:
    ...
    def __init__(self, env, isSecure=True, debug=False):
        self.env = env
        self.debug = debug
        self.isSecure = True
    ...
    def post_object(self, path, obj):
        if self.debug:
            print(f'{datetime.now()} DEBUG OpsRampEnv.post_object called - path:{path} obj:{obj}')
        ...
        while True:
            try:
                ...
                if self.debug:
                    print(f'{datetime.now()} DEBUG OpsRampEnv.post_object POST response - response:{response} response.text:{response.text} response.headers:{response.headers}')
                response.raise_for_status()
                break
            except HTTPError as exc:
                code = exc.response.status_code
                print(f'{datetime.now()} WARN OpsRampEnv.post_object POST HTTPError - {code} - {exc.response.text}.  Retry in 30 sec...')              
                time.sleep(30)
                continue
            except TimeoutError as exc:
                print(f'{datetime.now()} WARN OpsRampEnv.post_object POST connection timed out.  Retry in 30 sec...')
                time.sleep(30)
                continue
        ...
        if 'x-ratelimit-remaining' in response.headers and int(response.headers['x-ratelimit-remaining']) < 2 :
            ...
            print(f'{datetime.now()} WARN Sleeping for {str(sleeptime)} sec to wait for API throttling limit to reset..')
    ...
    def put_object(self, path, obj):
        if self.debug:
            print(f'{datetime.now()} DEBUG OpsRampEnv.put_object called - path:{path} obj:{obj}')
        token = self.get_token()
        url = self.env['url'] + path
        headers = {
            'Content-Type': 'application/json',
        ...
        if self.debug:
            print(f'{datetime.now()} DEBUG OpsRampEnv.put_object POST response - response:{response} response.text:{response.text} response.headers:{response.headers}')
    ...
    def get_with_pagination(self, path:str, result_array_key:str='results', params:dict={}, page_no:int=1, page_size:int=500):
        if self.debug:
            print(f'{datetime.now()} DEBUG OpsRampEnv.get_with_pagination called - path:{path} result_array_key:{result_array_key} params:{params} page_no:{page_no} page_size:{page_size}')
        token = self.get_token()
        url = self.env['url'] + path
        headers = {
            'Content-Type': 'application/json',
        ...
        if self.debug:
            print(f'{datetime.now()} DEBUG OpsRampEnv.get_with_pagination GET response - response:{response} response.text:{response.text} response.headers:{response.headers}')
    ...
    def do_opsql_query(self, object_type:str, fields:list=None, filter_criteria:str=None, aggregate:str=None, group_by:list=None, sort_by:str=None, page_no:int=1, page_size:int=1000):
        url = f'/opsql/api/v7/tenants/{self.env["tenant"]}/queries'
        payload = {}
        ...
        if 'nextPage' in bodydict and bodydict['nextPage']:
            results + self.do_opsql_query(object_type, fields, filter_criteria, aggregate, group_by, sort_by, page_no+1, page_size)
        return results
    ...
    def get_objects(self, obtype, page=1, queryString=None, searchQuery=None,countonly=False, itemId=None, filter=None):
        if self.debug:
            print(f'{datetime.now()} DEBUG OpsRampEnv.get_objects called - obtype:{obtype} page:{page} queryString:{queryString} searchQuery:{searchQuery} countonly:{countonly} itemId:{itemId} filter:{filter}')
        ...
        if self.debug:
            print(f'{datetime.now()} DEBUG OpsRampEnv.get_objects GET response - response:{response} response.text:{response.text} response.headers:{response.headers}')
    ...
    def get_token(self):
        ...
        if (not self.token) or (now_plus_30_sec >= self.token_expire_time):
            if self.debug:
                print(f'{datetime.now()} DEBUG OpsRampEnv.get_token Token is expired, getting new one - old vals token:{self.token} expire_time:{self.token_expire_time} now_plus_30_sec:{now_plus_30_sec}')
            ...
            try:
                ...
                print(f'\nUnable to connect to {self.env["url"]}: {response}')
                ...
                if self.debug:
                    print(f'{datetime.now()} DEBUG OpsRampEnv.get_token Token is OK, re-using it - token:{self.token} expire_time:{self.token_expire_time} now_plus_30_sec:{now_plus_30_sec}')
    ...
    def unset_custom_attr_on_devices(self, attr_id, value_id, device_ids):
        path = f'/api/v3/tenants/{self.env["tenant"]}/tags/{attr_id}/untagged-entities'
        devices = []
