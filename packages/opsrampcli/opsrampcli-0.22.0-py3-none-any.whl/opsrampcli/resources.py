...
def import_resources_from_dataframe(ops: OpsRampEnv, args, df: pd.DataFrame):
    ...
    for column in vals_to_add.keys():
        ...
        newvals = ops.add_custom_attr_value(attrinfo[column]['id'], newvalsarray, is_metric_label=args.metricLabel)
    ...
    for idx,resource in df.iterrows():
        ...
        if 'Processing Action' in resource and resource['Processing Action'] == 'Delete':
            if is_new:
                print(f'WARNING: Delete action specified but there is no such resource name:{resource["resourceName"]} of specified type.')
            else:
        ...
        if is_new:
            if args.nocreate:
                print(f'Unmatched resource name:{resource_dict["resourceName"]} not created because --nocreate option is set.')
                continue
            else:
