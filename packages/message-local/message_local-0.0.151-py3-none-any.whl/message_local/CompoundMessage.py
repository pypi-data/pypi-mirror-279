...
from criteria_local.criteria import CriteriaLocal
...
class CompoundMessage(GenericCRUD, metaclass=MetaLogger, object=object_message):
    ...
    def get_recipients_by_criteria_set_ids(self, criteria_set_ids: list[int]) -> list[Recipient]:
        qeury = f"""
        ...
        if criteria_set_ids:  # otherwise get all profiles
            profile_ids_per_criteria_set_id = self.criteria_local.get_profile_ids_per_criteria_set_id(criteria_set_ids)
            all_profile_ids = list(set([profile_id for profile_ids in profile_ids_per_criteria_set_id.values()
                                        for profile_id in profile_ids]))
            if not all_profile_ids:
                return []
            qeury += f"WHERE profile_ml.profile_id IN ({', '.join(['%s'] * len(all_profile_ids))})"
        self.cursor.execute(qeury, all_profile_ids)
        result = self.convert_multi_to_dict(
            rows=self.cursor.fetchall(),
            select_clause_value="phone_number, email_address, title, lang_code_str, profile_id")
        ...
        for profile_id, profile_dicts in profile_dicts.items():
            ...
            title_per_lang_code_str = {row["lang_code_str"]: row["title"] for row in profile_dicts}
    ...
    def set_compound_message_after_text_template(
        ...
        recipients = recipients or self._recipients or []
        message_id = message_id or (self.message_ids[0] if self.message_ids else None)
        ...
        if not recipients and campaign_id:
            # Get campaign recipients:
            criteria_set_ids = self.criteria_local.get_criteria_set_ids_list_by_campaign_id(campaign_id=campaign_id)
            recipients = self.get_recipients_by_criteria_set_ids(criteria_set_ids)
        ...
        if body:
            ...
            if not recipients:
                ...
                criteria_set_ids = [row.get("criteria_set_id")
                                    for row in textblocks_and_attributes if row.get("criteria_set_id")]
                recipients = self.get_recipients_by_criteria_set_ids(criteria_set_ids)
        else:
        ...
        if not recipients:
            raise Exception(f"No recipients found for campaign_id={campaign_id} or form_id={form_id}")
        ...
        if compound_message["data"]:
            self.message_ids = self._split_and_push_compound_message(compound_message, is_require_moderator)
    ...
    def _split_and_push_compound_message(self, compound_message: dict, is_require_moderator: bool) -> list:
        ...
        for i in range(0, len(all_profile_ids), RECIPIENTS_LIMIT_PER_COMPOUND_MESSAGE):
            profile_ids = all_profile_ids[i:i + RECIPIENTS_LIMIT_PER_COMPOUND_MESSAGE]
            ...
            for channel_key, channel_value in compound_message['data'].items():
                mini_channel_value = {key: channel_value[key] for key in channel_value.keys() if key != 'Page'}
                mini_channel_value['Page'] = []
                for page in channel_value['Page']:
                    mini_page = {key: page[key] for key in page.keys() if key != 'MessageTemplates'}
                    ...
                    for message_template in page['MessageTemplates']:
                        ...
                        for text_block in message_template['MessageTemplateTextBlocks']:
                            ...
                            if profile_blocks:
                                ...
                                mini_message_template['MessageTemplateTextBlocks'].append(mini_text_block)

                        if mini_message_template['MessageTemplateTextBlocks']:
                            mini_page['MessageTemplates'].append(mini_message_template)
            ...
            message_id = self.get_queue().push(data_dict={
                "compound_message_json": json.dumps(mini_compound_message),
...
"""
        if text_block.get("criteria_set_id"):
            potentials_recipients = self.criteria_local.get_profile_ids_per_criteria_set_id(
                criteria_set_ids=[text_block["criteria_set_id"]]).get(text_block["criteria_set_id"], [])
        else:
            potentials_recipients = [recipient.get_profile_id() for recipient in recipients]
        ...
        for recipient in recipients:
            ...
            profile_block = {
                ...
                "subject_per_language": {
                    lang_code_str: self._process_text_block(content, recipient, lang_code_str)
...
Page[]
    ...
    MessageTemplates - Not need hierarchy in the JSON. Container of message_template_text_blocks
...
"""
                ...
                for question_id, _text_blocks in text_blocks_by_question_id.items():
                    for text_block in text_blocks:
                        for field, (lang_field, lang_code_field) in field_to_lang_field.items():
                            lang_code_str = text_block.get(lang_code_field) or LangCode.ENGLISH.value
    ...
    def _process_text_block(self, text_block_body: str, recipient: Recipient, lang_code_str: str) -> str:
        ...
        # TODO: what to do in case the message contains to.first_name but the recipient has no first name defined in that language? We should take the English otherwise the Heberew
