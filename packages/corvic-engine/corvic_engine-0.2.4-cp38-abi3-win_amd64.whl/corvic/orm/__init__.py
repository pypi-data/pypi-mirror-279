...
class FeatureView(SoftDeleteMixin, BelongsToOrgMixin, Base):
    ...
    feature_view_output: sa_orm.Mapped[space_pb2.SpaceOutput | None] = (
        sa_orm.mapped_column(default_factory=space_pb2.SpaceOutput)
    )
    ...
    space_parameters: sa_orm.Mapped[list[SpaceParameters]] = sa_orm.relationship(
        viewonly=True,
        collection_class=list,
        cascade="all",
        init=False,
        secondary="space_parameters",
        default_factory=list,
        secondaryjoin=lambda: SpaceParameters.feature_view_id == FeatureView.id,
    )
...
class Space(BelongsToOrgMixin, Base):
    ...
    name: sa_orm.Mapped[str] = sa_orm.mapped_column(sa.Text, default=None)
    description: sa_orm.Mapped[str] = sa_orm.mapped_column(sa.Text, default="")
    parameters: sa_orm.Mapped[list[SpaceParameters]] = sa_orm.relationship(
        viewonly=True,
        init=True,
        default_factory=list,
        secondary="space_parameters",
        secondaryjoin=lambda: (Space.id == SpaceParameters.space_id),
    )
...
class SpaceParameters(BelongsToOrgMixin, Base):
    """Parameters for a space."""
