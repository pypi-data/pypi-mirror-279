...
from dataclasses import dataclass
from typing import Any, Generic, Literal, NoReturn, ParamSpec, TypeVar, overload

from typing_extensions import Self

from corvic.well_known_types import JSONExpressable
...
.unwrap_or_else(lambda err: "error happened") == "error happened"
    ...
    @abstractmethod
    def map(self, fn: Callable[[T], R]) -> _Resultable[R, E]:
...
.map_or("error happened", lambda _: "success") == "success"
    ...
    @abstractmethod
    def map_or_else(self, default_fn: Callable[[E], R], fn: Callable[[T], R]) -> R:
...
.map_error(lambda _: Error("error happened")) == Ok("value")
    ...
    @abstractmethod
    def and_then(self, fn: Callable[[T], _Resultable[U, E]]) -> _Resultable[U, E]:
    ...
    @abstractmethod
    def or_else(self, fn: Callable[[E], _Resultable[T, R]]) -> _Resultable[T, R]:
...
@dataclass
class Ok(Generic[T], _Resultable[T, Any]):
    """A successful result."""
    ...
    def unwrap_or_else(self, default_fn: _IgnoredFn) -> T:
        return self.value

    def map(self, fn: Callable[[T], R]) -> Ok[R]:
        return Ok(fn(self.value))

    def map_or(self, default: R, fn: Callable[[T], R]) -> R:
        return fn(self.value)

    def map_or_else(self, default_fn: _IgnoredFn, fn: Callable[[T], R]) -> R:
        return fn(self.value)

    def map_error(self, fn: _IgnoredFn) -> Self:
        return self

    def and_then(self, fn: Callable[[T], _Resultable[U, Any]]) -> _Resultable[U, Any]:
        return fn(self.value)
...
class Error(Exception, _Resultable[Any, "Error"]):
    ...
    def __init__(self, message: str, **extra_info: JSONExpressable):
        self._extra_info = extra_info
        self._message = message

    @classmethod
    def from_(cls, other_error: Error):
    ...
    def _render_extra_args(self) -> Iterator[str]:
        return (f"{key}={value!r}" for key, value in self.extra_info.items())
    ...
    def unwrap_or_else(self, default_fn: Callable[[Error], T]) -> T:
    ...
    def or_else(self, fn: Callable[[Error], _Resultable[T, R]]) -> _Resultable[T, R]:
        return fn(self)
...
class ForeignError(Generic[ExceptionT], _Resultable[Any, "ForeignError[ExceptionT]"]):
    ...
    def unwrap_or_else(self, default_fn: Callable[[ForeignError[ExceptionT]], T]) -> T:
        return default_fn(self)
    ...
    def or_else(
        self, fn: Callable[[ForeignError[ExceptionT]], _Resultable[T, R]]
    ) -> _Resultable[T, R]:
        return fn(self)


# Spiritually, a Result is Ok[T] | Error[E0] | Error[E1] | ... but there is no
