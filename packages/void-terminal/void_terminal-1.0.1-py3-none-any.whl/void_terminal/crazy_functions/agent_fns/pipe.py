...
class PluginMultiprocessManager:
    ...
    def immediate_showoff_when_possible(self, fp):
        # ⭐ Main process
        # Get the extension name of fp
        file_type = fp.split('.')[-1]
        # If it is a text file, Then display the text content directly
    ...
    def overwatch_workdir_file_change(self):
        # ⭐ Main Process Docker External Folder Monitoring
        path_to_overwatch = self.autogen_work_dir
        change_list = []
        # Scan all files under the path, And compare with the files recorded in self.previous_work_dir_files，
        # If there is a new file，Or the modification time of the file has changed，Then update in self.previous_work_dir_files
        # Record the paths of new files and files that have changed in change_list
        ...
        if len(change_list) > 0:
            ...
            self.chatbot.append(['Detect new documents.', f'The document list is as follows: {file_links}'])
            yield from update_ui(chatbot=self.chatbot, history=self.history)
        return change_list
    ...
    def main_process_ui_control(self, txt, create_or_resume) -> str:
        ...
        if txt == 'exit':
            self.chatbot.append([f"End", "The end signal is clear，Terminate AutoGen program。"])
            yield from update_ui(chatbot=self.chatbot, history=self.history)
            self.terminate()
            return "terminate"
        ...
        while True:
            ...
            if self.parent_conn.poll():
                ...
                if "Waiting for your further instructions" in self.chatbot[-1][-1]:
                    self.chatbot.pop(-1)  # remove the last line
                if '[GPT-Academic] Waiting' in self.chatbot[-1][-1]:
                    self.chatbot.pop(-1)    # remove the last line
                msg = self.parent_conn.recv() # PipeCom
                if msg.cmd == "done":
                    self.chatbot.append([f"End", msg.content])
                    self.cnt += 1
                    yield from update_ui(chatbot=self.chatbot, history=self.history)
                    self.terminate()
                    break
                if msg.cmd == "show":
                    ...
                    notice = ""
                    if repeated: notice = "（Automatically ignore duplicate inputs）"
                    self.chatbot.append([f"Running phase-{self.cnt}（Last user feedback input is: 「{cmd_to_autogen}」{notice}", msg.content])
                    self.cnt += 1
                    yield from update_ui(chatbot=self.chatbot, history=self.history)
                if msg.cmd == "interact":
                    yield from self.overwatch_workdir_file_change()
                    self.chatbot.append([f"The program reaches the user feedback node.", msg.content +
                                         "\n\nWaiting for your further instructions." +
                                         "\n\n(1) In general, you don`t need to say anything, Clear input area, Then click `Submit` to continue. " +
                                         "\n\n(2) If you need to add something, Enter the content to be feedbacked, Click `Submit` to continue. " +
                                         "\n\n(3) If you want to terminate the program, Enter exit, Click `Submit` to terminate AutoGen and unlock. "
                    ])
                    yield from update_ui(chatbot=self.chatbot, history=self.history)
                ...
                if '[GPT-Academic] Waiting' not in self.chatbot[-1][-1]:
                    ...
                    self.chatbot.append(["[GPT-Academic] Waiting for AutoGen execution result ...", "[GPT-Academic] Waiting"])
                self.chatbot[-1] = [self.chatbot[-1][0], self.chatbot[-1][1].replace("[GPT-Academic] Waiting", "[GPT-Academic] Waiting.")]
                yield from update_ui(chatbot=self.chatbot, history=self.history)
                # if time.time() - begin_waiting_time > patience:
                #     self.chatbot.append([f"End", "Waiting超When, Terminate AutoGen program。"])
                #     yield from update_ui(chatbot=self.chatbot, history=self.history)
                #     self.terminate()
                #     return "terminate"
