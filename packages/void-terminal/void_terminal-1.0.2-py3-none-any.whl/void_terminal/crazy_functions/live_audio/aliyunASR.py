...
class AliyunASR():
    ...
    def audio_convertion_thread(self, uuid):
        # Collect audio in an asynchronous thread
        ...
        is_previous_frame_transmitted = False   # Was there anyone speaking in the previous frame
        previous_frame_data = None
        echo_cnt = 0        # After No Sound，Continue to send audio data to the server n times
        echo_cnt_max = 4   # After No Sound，Continue to send audio data to the server n times
        ...
        while not self.stop:
            ...
            if audio is not None:
                ...
                if is_speaking or echo_cnt > 0:
                    # If the microphone is active / If it is in the echo tail stage
                    echo_cnt -= 1
                    if not is_previous_frame_transmitted:   # No human voice in the previous frame，But we also add the previous frame
                        if previous_frame_data is not None: data = previous_frame_data + data
                    if is_speaking:
                        echo_cnt = echo_cnt_max
                    slices = zip(*(iter(data),) * 640)      # 640 bytes per group
                    ...
                    echo_cnt = 0
                    # Keep the link active，Even without sound，Also according to the time interval，Send some audio clips to the server
            ...
            if not self.aliyun_service_ok:
                self.stop = True
                self.stop_msg = 'Aliyun audio service exception，Please check if ALIYUN_TOKEN and ALIYUN_APPKEY have expired。'
        r = sr.stop()
