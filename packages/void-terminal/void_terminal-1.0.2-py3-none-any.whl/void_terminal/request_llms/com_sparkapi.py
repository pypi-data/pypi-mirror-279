from void_terminal.toolbox import get_conf, get_pictures_list, encode_image
import base64
import datetime
import hashlib
import hmac
...
class Ws_Param(object):
    ...
    def create_url(self):
        ...
        # Print the URL when establishing the connection here,You can remove the comments above when referring to this demo，Check if the generated URL matches the one generated by your code when comparing the same parameters
        return url
...
class SparkRequestInstance():
    def __init__(self):
        ...
        if XFYUN_APPID == '00000000' or XFYUN_APPID == '': raise RuntimeError('Please configure XFYUN_APPID for the Xunfei Starfire model, XFYUN_API_KEY, XFYUN_API_SECRET')
    ...
    def create_blocking_request(self, inputs, llm_kwargs, history, system_prompt, use_image_api):
        ...
        if use_image_api and llm_kwargs.get('most_recent_uploaded'):
            if llm_kwargs['most_recent_uploaded'].get('path'):
                ...
                if len(file_manifest) > 0:
                    print('Using the Xunfei Image Understanding API')
                    gpt_url = self.gpt_url_img
        wsParam = Ws_Param(self.appid, self.api_key, self.api_secret, gpt_url)
        websocket.enableTrace(False)
        wsUrl = wsParam.create_url()

        # Processing when receiving websocket connection establishment
        def on_open(ws):
            import _thread as thread
        ...
        # Processing when receiving websocket closure
...
def gen_params(appid, inputs, llm_kwargs, history, system_prompt, file_manifest):
    """
    Generate request parameters through appid and user`s question
    """
    domains = {
        "spark": "general",
        "sparkv2": "generalv2",
