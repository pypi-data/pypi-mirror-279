import time
import threading
from void_terminal.toolbox import update_ui, Singleton
from void_terminal.toolbox import ChatBotWithCookies
from multiprocessing import Process, Pipe
from contextlib import redirect_stdout
from void_terminal.request_llms.queued_pipe import create_queue_pipe
...
class LocalLLMHandle(Process):
    ...
    def check_dependency(self):
        ...
        try:
            ...
            self.set_state(f"Missing{self.model_name}Dependencies of，If you want to use{self.model_name}，In addition to the basic pip dependencies，You also need to run{self.cmd_to_install}Installation{self.model_name}Dependencies of。")
            self.running = False
    ...
    def run(self):
        ...
        reset_tqdm_output()
        self.set_state("`Try to load the model`")
        try:
            with redirect_stdout(self.child):
                self._model, self._tokenizer = self.load_model_and_tokenizer()
        except:
            self.set_state("`Failed to load the model`")
            self.running = False
            from void_terminal.toolbox import trimmed_format_exc
            self.child.send(
                f'[Local Message] Cannot load properly{self.model_name}Parameters of.' + '\n```\n' + trimmed_format_exc() + '\n```\n')
        ...
        while True:
            ...
            try:
                ...
                from void_terminal.toolbox import trimmed_format_exc
                self.child.send(
                    f'[Local Message] Call{self.model_name}Failure.' + '\n```\n' + trimmed_format_exc() + '\n```\n')
                self.child.send('[Finish]')
    ...
    def stream_chat(self, **kwargs):
        ...
        if self.get_state() == "`Ready`":
            yield "`Waiting for thread lock，Please wait in line ...`"

        with self.threadLock:
            if self.parent.poll():
                yield "`Please wait in line ...`"
...
def get_local_llm_predict_fns(LLMSingletonClass, model_name, history_format='classic'):
    ...
    def predict_no_ui_long_connection(inputs:str, llm_kwargs:dict, history:list=[], sys_prompt:str="", observe_window:list=[], console_slience:bool=False):
        ...
        if history_format == 'classic':
            ...
            # There is a sys_prompt interface
