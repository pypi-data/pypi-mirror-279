...
from void_terminal.toolbox import get_conf, update_ui, log_chat
...
class MoonShotInit:
    ...
    def generate_payload(self, inputs, llm_kwargs, history, system_prompt, stream):
        ...
        for i in history[0::2]:    # Continue uploading historical files
...
def msg_handle_error(llm_kwargs, chunk_decoded):
    ...
    openai_website = f' Please log in to OpenAI to view details at https://platform.openai.com/signup  api-key: `{api_key_encryption}`'
    error_msg = ''
    if "does not exist" in chunk_decoded:
        error_msg = f"[Local Message] Model {llm_kwargs['llm_model']} Model does not exist, Or you do not have the qualification for experience."
    elif "Incorrect API key" in chunk_decoded:
        error_msg = f"[Local Message] Incorrect API key. OpenAI claims that an incorrect API_KEY was provided, Service refused." + openai_website
    elif "exceeded your current quota" in chunk_decoded:
        error_msg = "[Local Message] You exceeded your current quota. OpenAI claims that the account balance is insufficient, Service refused." + openai_website
    elif "account is not active" in chunk_decoded:
        error_msg = "[Local Message] Your account is not active. OpenAI states that it is due to account expiration, Service refused." + openai_website
    elif "associated with a deactivated account" in chunk_decoded:
        error_msg = "[Local Message] You are associated with a deactivated account. OpenAI considers it as an account expiration, Service refused." + openai_website
    elif "API key has been deactivated" in chunk_decoded:
        error_msg = "[Local Message] API key has been deactivated. OpenAI considers it as an account failure, Service refused." + openai_website
    elif "bad forward key" in chunk_decoded:
        error_msg = "[Local Message] Bad forward key. API2D account balance is insufficient."
    elif "Not enough point" in chunk_decoded:
        error_msg = "[Local Message] Not enough point. API2D account points are insufficient."
...
def predict_no_ui_long_connection(inputs, llm_kwargs, history=[], sys_prompt="", observe_window=None,
    ...
    gpt_bro_init = MoonShotInit()
    watch_dog_patience = 60  # The patience of the watchdog, Set to 10 seconds
    ...
    for content, moonshot_bro_result, error_bro_meg in stream_response:
        ...
        if error_bro_meg:
            ...
            return f'{moonshot_bro_result} Dialogue Error'
            # Observation window
        if len(observe_window) >= 1:
            observe_window[0] = moonshot_bro_result
        if len(observe_window) >= 2:
            if (time.time() - observe_window[1]) > watch_dog_patience:
                observe_window[2] = "Request timed out，Program terminated。"
                raise RuntimeError(f"{moonshot_bro_result} Program terminated。")
