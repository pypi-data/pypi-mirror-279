...
class QwenRequestInstance():
    def __init__(self):
        ...
        if not validate_key():
            raise RuntimeError('Please Configure DASHSCOPE_API_KEY')
        ...
        for response in responses:
            if response.status_code == HTTPStatus.OK:
                if response.output.choices[0].finish_reason == 'stop':
                    ...
                    self.result_buf += "[Local Message] Excessive Length Generated，Subsequent output is truncated"
                    yield self.result_buf
                    break
                else:
                    self.result_buf += response.output.choices[0].message.content
                ...
                self.result_buf += f"[Local Message] Request error：Status code：{response.status_code}，Error code:{response.code}，Message：{response.message}"
                yield self.result_buf
                break
