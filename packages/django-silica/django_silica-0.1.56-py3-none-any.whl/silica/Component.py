    ...
    def set_property(self, attr, value, with_hooks=True):
        ...
        if '.' not in attr:
            if hasattr(self, attr):
                # If we're setting a query_param, and it's not a list, then we need to check if we need to convert it to a list
        ...
        # instance, we'll need to check if the segment is a key in the validation_rules
            ...
            for i, segment in enumerate(segments[:-1]):
                # Check for dict or attribute existence and navigate
                if isinstance(obj, dict) and segment in obj or hasattr(obj, segment):
                    obj = obj[segment] if isinstance(obj, dict) else getattr(obj, segment)
                else:
                    raise SetPropertyException(f'Dictionary key or attribute "{segment}" does not exist.')

                if not isinstance(obj, (dict, type(self))):
                    raise SetPropertyException('Cannot navigate through non-dict/non-component object.')

            # Set the value on the final target
            final_segment = segments[-1]
            if isinstance(obj, dict) and final_segment in obj or hasattr(obj, final_segment):
