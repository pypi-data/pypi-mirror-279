...
class nacos:
        ...
        while True:
            ...
            try:
                for item in self.__configDict:
                    ...
                    tenant = configMsg[2]
                    x = int(
                        time.time()) - self.__threadHealthyDict[dataId +
                                                                group + tenant]
                    if (x > 50):
                        md5Content = configMsg[3]
                        myConfig = self.__configDict[item]
                        configThread = threading.Thread(
                            target=self.__configListeningThreadRun,
                            args=(dataId, group, tenant, md5Content, myConfig))
                        self.__threadHealthyDict[dataId + group +
                                                 tenant] = int(time.time())
                        configThread.start()
                        logging.info("配置信息监听线程重启成功: dataId=" + dataId +
    ...
    def __configListeningThreadRun(self, dataId, group, tenant, md5Content,
                                   myConfig):
        getConfigUrl = "http://" + self.ip + ":" + str(
            self.port) + "/nacos/v1/cs/configs"
        params = {"dataId": dataId, "group": group, "tenant": tenant}

        licenseConfigUrl = "http://" + self.ip + ":" + str(
            self.port) + "/nacos/v1/cs/configs/listener"
        header = {"Long-Pulling-Timeout": "30000"}
        while True:
            self.__threadHealthyDict[dataId + group + tenant] = int(
                time.time())
            if (tenant == "public"):
                files = {
                    "Listening-Configs":
                        (None,
                         dataId + "\002" + group + "\002" + md5Content + "\001")
                }
            else:
                files = {
                    "Listening-Configs":
                        (None, dataId + "\002" + group + "\002" + md5Content +
                         "\002" + tenant + "\001")
                }
            ...
            if (re.text != ""):
                try:
                    ...
                    logging.info("配置信息更新成功: dataId=" + dataId + "; group=" +
                                 group + "; tenant=" + tenant)
                except:
                    logging.exception("配置信息更新失败：dataId=" + dataId +
                                      "; group=" + group + "; tenant=" +
                                      tenant,
                                      exc_info=True)
                    break
    ...
    def config(self, myConfig, dataId, group="DEFAULT_GROUP", tenant="public"):
        logging.info("正在获取配置: dataId=" + dataId + "; group=" + group +
                     "; tenant=" + tenant)
        getConfigUrl = "http://" + self.ip + ":" + str(
            self.port) + "/nacos/v1/cs/configs"
        params = {"dataId": dataId, "group": group, "tenant": tenant}
        try:
            ...
            self.__configDict[dataId + "\001" + group + "\001" + tenant +
                              "\001" + md5Content] = myConfig

            for item in nacosJson:
                myConfig[item] = nacosJson[item]
            logging.info("配置获取成功：dataId=" + dataId + "; group=" + group +
                         "; tenant=" + tenant)
            configThread = threading.Thread(
                target=self.__configListeningThreadRun,
                args=(dataId, group, tenant, md5Content, myConfig))
            ...
            logging.exception("配置获取失败：dataId=" + dataId + "; group=" + group +
                              "; tenant=" + tenant,
                              exc_info=True)
    ...
    def __registerBeatThreadRun(self, serviceIp, servicePort, serviceName,
        ...
        beatUrl = "http://" + self.ip + ":" + str(
            self.port) + "/nacos/v1/ns/instance/beat?"
        beatJson = {
            "ip": serviceIp,
            "port": servicePort,
    ...
    def registerService(self,
        ...
        registerUrl = "http://" + self.ip + ":" + str(
            self.port) + "/nacos/v1/ns/instance"
        params = {
            "ip": serviceIp,
            "port": servicePort,
            "serviceName": serviceName,
        ...
        try:
            ...
            if (re.text == "ok"):
                logging.info("服务注册成功。")
                beatThread = threading.Thread(
                    target=self.__registerBeatThreadRun,
                    args=(serviceIp, servicePort, serviceName, groupName,
                          namespaceId, metadata, weight))
                beatThread.start()
            else:
...
class nacosBalanceClient:
    ...
    def __getAddress(self, serviceName, group, namespaceId):
        ...
        md5Content = md5.hexdigest()
        try:
            oldMd5 = self.__LoadBalanceDict[serviceName + group + namespaceId +
                                            "md5"]
        except KeyError:
        ...
        if oldMd5 != md5Content:
            ...
            self.__LoadBalanceDict[serviceName + group +
                                   namespaceId] = healthyHosts
            self.__LoadBalanceDict[serviceName + group + namespaceId +
                                   "index"] = 0

    def __loadBalanceClient(self, serviceName, group, namespaceId):
        try:
            x = int(time.time()) - self.__LoadBalanceDict[serviceName + group +
                                                          namespaceId + "time"]
        except KeyError:
            x = 11
        if x > 10:
        ...
        index = self.__LoadBalanceDict[serviceName + group + namespaceId +
                                       "index"]
        l = len(self.__LoadBalanceDict[serviceName + group + namespaceId])
        if l == 0:
            logging.error("无可用服务 serviceName: " + serviceName + ";group: " +
                          group + ";namespaceId: " + namespaceId)
            return ""
        if index >= l:
            self.__LoadBalanceDict[serviceName + group + namespaceId +
                                   "index"] = 1
            return self.__LoadBalanceDict[
                serviceName + group + namespaceId][0]['ip'] + ":" + str(
                self.__LoadBalanceDict[serviceName + group +
                                       namespaceId][0]['port'])
        else:
            self.__LoadBalanceDict[serviceName + group + namespaceId +
                                   "index"] = index + 1
            return self.__LoadBalanceDict[
                serviceName + group + namespaceId][index]['ip'] + ":" + str(
                self.__LoadBalanceDict[serviceName + group +
                                       namespaceId][index]['port'])

    def customRequestClient(self,
                            method,
                            url,
                            requestParamJson=False,
                            https=False):
            ...
            def mainPro(*args, **kwargs):
                ...
                if address == "":
                    ...
                    try:
                        ...
                        return self.timeOutFun()
                    except Exception as ex:
                        logging.exception("链接失败   ", exc_info=True)
                        return self.fallbackFun()
