...
def _build_to_date(args: t.List) -> exp.TsOrDsToDate:
    this = seq_get(args, 0)
    format = str(seq_get(args, 1))
    if "yyyy-mm-dd" in format:
        time_format = format.replace("mm", "MM")
        return exp.TsOrDsToDate(this=this, format=time_format)
    return exp.TsOrDsToDate(this=this, format=format)


def build_url(type) -> t.Callable[[t.List], exp.ParseUrl]:
    def _builder(args: t.List) -> exp.ParseUrl:
        this = seq_get(args, 0)
        return exp.ParseUrl(this=this, type=exp.Literal.string(type))

    return _builder


def build_split_part(args: t.List) -> exp.SplitPart | exp.Coalesce:
    this = seq_get(args, 0)
    delimiter = seq_get(args, 1)
    field = seq_get(args, 2)
    return exp.SplitPart(this=this, delimiter=delimiter, field=field)


def build_bit_count(args: t.List) -> exp.BitCount:
    this = seq_get(args, 0)
    if isinstance(this, exp.Neg):
        raise ValueError("Negative number not allowed")
    return exp.BitCount(this=this)


def build_ascii(args: t.List) -> exp.Ascii:
    this = seq_get(args, 0)
    if this is not None and "\u4e00" <= this.args.get("this") <= "\u9fff":
        raise ValueError("Chinese characters not allowed")
    return exp.Ascii(this=this)


def build_dayofweek(args: t.List) -> exp.DayOfWeek:
    this = seq_get(args, 0)
    return exp.DayOfWeek(this=this, dialect="presto")
...
class Presto(Dialect):
    ...
    def to_json_path(self, path: t.Optional[exp.Expression]) -> t.Optional[exp.Expression]:
        if isinstance(path, exp.Literal):
            # Compatible with lax mode in trino's json path
            # https://trino.io/docs/current/functions/json.html#json-value
            path_text = path.name
            if path_text.startswith("lax") or "lax" in path_text:
                path_text = path_text.replace("lax ", "")
    ...
    class Parser(parser.Parser):
        ...
        FUNCTIONS = {
            **parser.Parser.FUNCTIONS,
            ...
            "BIT_COUNT": build_bit_count,
            "CARDINALITY": exp.ArraySize.from_arg_list,
            "CONTAINS": exp.ArrayContains.from_arg_list,
            "CODEPOINT": build_ascii,
            ...
            "FORMAT_DATETIME": _build_to_date,
            "INDEX": exp.StrPosition.from_arg_list,
            "JSON_ARRAY_LENGTH": exp.JsonArrayLength.from_arg_list,
            "JSON_ARRAY_CONTAINS": lambda args: exp.JSONArrayContains(
                this=seq_get(args, 0), expression=exp.Literal.string(seq_get(args, 1))
            ),
            "JSON_ARRAY_GET": build_extract_json_with_path(exp.JSONExtract),
            "JSON_VALUE": parser.build_extract_json_with_path(exp.JSONExtractScalar),
            "JSON_SIZE": exp.JsonArrayLength.from_arg_list,
            ...
            "REGEXP_EXTRACT_ALL": lambda args: exp.RegexpExtractAll(
            ...
            "REPEAT": exp.ArrayRepeat.from_arg_list,
            ...
            "URL_EXTRACT_HOST": build_url("HOST"),
            "URL_EXTRACT_PATH": build_url("PATH"),
            "URL_EXTRACT_PROTOCOL": build_url("PROTOCOL"),
            "URL_EXTRACT_QUERY": build_url("QUERY"),
            "URL_EXTRACT_PORT": build_url("PORT"),
            "URL_EXTRACT_PARAMETER": exp.ExtractUrlParameter.from_arg_list,
    ...
    class Generator(generator.Generator):
        ...
        TRANSFORMS = {
            **generator.Generator.TRANSFORMS,
            ...
            exp.DayOfWeek: lambda self, e: f"DAY_OF_WEEK({self.sql(e, 'this')})",
