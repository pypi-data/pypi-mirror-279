...
from sqlink import Entity, Define as d, Dao, Insert
...
class DaoStudent:
    ...
    def select_student_with_conditions(self, student_id: int, age: int):
        sql = "select * from student where student_id=? and age=? limit 1;"
        return self.execute(sql=sql, args=[student_id, age])
...
# ====================================================================================================================
@use_db(Database)
def test_execute_sql_without_dao_func(db: Database):
    """执行sql方法"""
...
@use_db(Database)
def test_execute_sql_with_substitute_without_dao_func(db: Database):
    """执行sql方法，使用替代符"""
    students = insert_data(db)
    for student in students:
        select_id = student.student_id
...
@use_db(Database)
def test_execute_sql_with_limit_without_dao_func(db: Database):
    """执行sql方法，使用limit"""
    students = insert_data(db)
    for student in students:
        select_id = student.student_id
...
@use_db(Database)
def test_execute_sql_with_substitute_and_limit_without_dao_func(db: Database):
    """执行sql方法，使用替代符和limit"""
    students = insert_data(db)
    for student in students:
        select_id = student.student_id
...
@use_db(Database)
def test_execute_sql_with_conditions_without_dao_func(db: Database):
    """执行sql方法，使用替代符和limit"""
    students = insert_data(db)
    for student in students:
        select_id = student.student_id
