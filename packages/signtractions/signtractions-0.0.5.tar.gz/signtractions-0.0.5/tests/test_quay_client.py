...
@mock.patch("signtractions.resources.quay_client.QuaySession")
def test_init(mock_session):
    client = quay_client.QuayClient(username="user", password="pass", host="stage.quay.io")
...
@mock.patch("signtractions.resources.quay_session.QuaySession")
...
@mock.patch("signtractions.resources.quay_client.QuaySession")
@mock.patch("signtractions.resources.quay_session.requests.Session")
...
@mock.patch("signtractions.resources.quay_client.QuayClient._authenticate_quay")
...
def test_get_manifest_list_raw_success_return_headers():
    ml = {
        "schemaVersion": 2,
        "mediaType": "application/vnd.docker.distribution.manifest.list.v2+json",
        "manifests": [
    ...
    with requests_mock.Mocker() as m:
        ...
        client = quay_client.QuayClient(username="user", password="pass", host="quay.io")
        ret_ml, ret_headers = client.get_manifest(
            "quay.io/namespace/image:1",
            raw=True,
            return_headers=True,
            media_type="application/vnd.docker.distribution.manifest.list.v2+json",
