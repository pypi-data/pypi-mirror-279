...
from dynatrace.environment_v2.metrics import MetricDescriptor, UnitEnum, AggregationType, Transformation, ValueType, MetricSeriesCollection
...
def test_list(dt: Dynatrace):
    ...
    for metric in metrics:
        ...
        assert metric.unit == UnitEnum.NOTAPPLICABLE
        break
...
def test_list_params(dt: Dynatrace):
    ...
    for metric in metrics:
        ...
        assert metric.description == ""
        assert metric.unit == UnitEnum.PERCENT
        assert not metric.ddu_billable
