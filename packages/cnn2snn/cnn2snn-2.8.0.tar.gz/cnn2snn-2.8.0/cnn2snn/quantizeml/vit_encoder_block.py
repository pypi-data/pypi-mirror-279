...
from akida import Model, LayerType, VitEncoderBlock
    ...
    if len(outbounds) == 1 and outbounds[0].parameters.layer_type == LayerType.BatchNormalization:
        ...
        if (len(outbounds) in [1, 2]
            ...
            if len(outbounds) == 1:
                ...
                if len(outbounds) == 1 and outbounds[0].parameters.layer_type == LayerType.Dense2D:
                    head = outbounds[0]
            else:
                ...
                if extract_head is not None:
                    ...
                    if (len(extract_head_outb) == 1
                            and extract_head_outb[0].parameters.layer_type == LayerType.Dense2D
...
def _validate_candidate(candidate):
    ...
    def _check_dense(layer, units=hidden_size, activation=False):
        return (layer.parameters.units == units and layer.parameters.activation == activation
                and _check_out_params(layer))
    ...
    # MLP
    ret &= _check_dense(candidate['mlp_1'], units=mlp_1_units, activation=True)
    ...
    if bn is not None:
        ret &= bn.parameters.activation == 0 and _check_out_params(bn)

    head = candidate['head']
    if head is not None:
        ret &= head.parameters.activation == 0 and head.parameters.collapse_dims == 1
        ...
        if dist_head is None:
            ...
            ret &= (dist_head.parameters.activation == 0
                    and dist_head.parameters.collapse_dims == 1
