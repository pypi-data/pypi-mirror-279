...
def func_sig(f):
    ...
    for p in s.parameters.values():
        if p.kind is inspect.Parameter.POSITIONAL_OR_KEYWORD:
            ...
            raise Exception("tdxlwings does not support UDFs with keyword arguments")
...
def get_category(**func_kwargs):
    ...
    return "tdxlwings"  # Default category
    ...
    def inner(f):
        if not hasattr(f, "__xlfunc__"):
            ...
            if sig["vararg"] and nDefaults > 0:
                raise Exception(
                    "tdxlwings does not support UDFs "
...
def import_udfs(module_names, xl_workbook):
    ...
    vba.writeln(
        "'Autogenerated code by tdxlwings - changes will be lost with next import!"
    )
    vba.writeln(
