...
tdxlwings PRO is dual-licensed under one of the following licenses:

* PolyForm Noncommercial License 1.0.0 (for noncommercial use):
  https://polyformproject.org/licenses/noncommercial/1.0.0
* tdxlwings PRO License (for commercial use):
...
class LicenseHandler:
    ...
    def get_cipher():
        try:
            ...
            raise tdxlwings.LicenseError(
                "Couldn't validate tdxlwings license key."
            ) from None
    ...
    def get_license():
        ...
        raise tdxlwings.LicenseError("Couldn't find an tdxlwings license key.")
    ...
    def validate_license(product, license_type=None):
        ...
        if key.startswith("gA") and not Fernet:
            ...
            raise ImportError(
                "You are using a legacy tdxlwings license key that requires the "
            ...
            try:
                ...
                raise tdxlwings.LicenseError("Invalid tdxlwings license key.") from None
        else:
            signature = hmac.new(
        ...
        try:
            if (
                ...
                raise tdxlwings.LicenseError(
                    "You need a paid tdxlwings license key for this action."
                )
        except KeyError:
            raise tdxlwings.LicenseError(
                "You need a paid tdxlwings license key for this action."
        ...
        if (
            ...
            raise tdxlwings.LicenseError(
                "Your tdxlwings deploy key is only valid for v{0}. To use a different "
                "version of tdxlwings, re-release your tool or generate a new deploy key "
                "via 'tdxlwings license deploy'.".format(license_info["version"])
            ) from None
        if (license_valid_until - dt.date.today()) < dt.timedelta(days=30):
            warnings.warn(
                f"Your tdxlwings license key expires in "
...
def get_embedded_code_temp_dir():
    tmp_base_path = os.path.join(tempfile.gettempdir(), "tdxlwings")
