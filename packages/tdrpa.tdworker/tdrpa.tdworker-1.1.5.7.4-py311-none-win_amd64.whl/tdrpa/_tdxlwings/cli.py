...
def _auth_aad(
    ...
    from _tdxlwings.utils import read_user_config
...
def addin_install(args):
    ...
    if args.dir:
        ...
        addin_source_path = os.path.join(this_dir, "addin", "_tdxlwings.xlam")
        _addin_install(addin_source_path, global_install)
            ...
            for app in xw.apps:
                if sys.platform.startswith("win"):
                    try:
                        ...
                        print("Successfully installed the _tdxlwings add-in!")
                    except:  # noqa: E722
                        print(
                            "Successfully installed the _tdxlwings add-in! "
...
def addin_remove(args):
    ...
    if args.dir:
        ...
        _addin_remove("_tdxlwings.xlam", global_install)
...
def addin_status(args):
    ...
    if args.file:
        ...
        addin_name = "_tdxlwings.xlam"
    addin_path = os.path.join(get_addin_dir(global_install), addin_name)
    if os.path.isfile(addin_path):
        print("The add-in is installed at {}".format(addin_path))
        print('Use "_tdxlwings addin remove" to uninstall it.')
    else:
        print("The add-in is not installed.")
        print('"_tdxlwings addin install" will install it at: {}'.format(addin_path))
...
def quickstart(args):
    ...
    if not args.fastapi:
        with open(os.path.join(project_path, project_name + ".py"), "w") as f:
            f.write("from tdrpa import _tdxlwings as xw\n\n\n")
            f.write("def main():\n")
            f.write("    wb = xw.Book.caller()\n")
            f.write("    sheet = wb.sheets[0]\n")
            f.write('    if sheet["A1"].value == "Hello _tdxlwings!":\n')
            f.write('        sheet["A1"].value = "Bye _tdxlwings!"\n')
            f.write("    else:\n")
            f.write('        sheet["A1"].value = "Hello _tdxlwings!"\n\n\n')
            if sys.platform.startswith("win"):
                f.write("@xw.func\n")
...
def runpython_install(args):
    ...
    shutil.copy(
        os.path.join(this_dir, f"_tdxlwings-{xw.__version__}.applescript"), destination_dir
    )
    if args:
        # Don't print when called as part of "_tdxlwings addin install"
...
def restapi_run(args):
    ...
    try:
        ...
        sys.exit("To use the _tdxlwings REST API server, you need Flask>=1.0.0 installed.")
    host = args.host
    port = args.port

    os.environ["FLASK_APP"] = "_tdxlwings.rest.api"
    subprocess.check_call(["flask", "run", "--host", host, "--port", port])


def license_update(args):
    """license handler for _tdxlwings PRO"""
    key = args.key
    if not key:
        sys.exit(
            "Please provide a license key via the -k/--key option. "
            "For example: _tdxlwings license update -k MY_KEY"
        )
...
def get_conda_settings():
    ...
    if conda_env and conda_exe:
        # _tdxlwings currently expects the path
...
def code_embed(args):
    ...
    with wb.app.properties(screen_updating=False):
        ...
        if "_tdxlwings.conf" in wb.sheet_names and not single_file:
            ...
            wb.sheets["_tdxlwings.conf"]["A1"].value = config
        elif not single_file:
            config_sheet = wb.sheets.add(
                "_tdxlwings.conf", after=wb.sheets[len(wb.sheets) - 1]
            )
...
def copy_os(args):
    copy_code(Path(this_dir) / "js" / "_tdxlwings.ts")


def copy_gs(args):
    copy_code(Path(this_dir) / "js" / "_tdxlwings.js")
...
def copy_vba(args):
    if args.addin:
        ...
        copy_code(Path(this_dir) / "_tdxlwings.bas")
...
def copy_code(fpath):
    ...
    with open(fpath, "r", encoding="utf-8") as f:
        if "bas" in str(fpath):
            text = (
                f.read()
                .replace('Attribute VB_Name = "_tdxlwings"\n', "")
                .replace('Attribute VB_Name = "_tdxlwings"\r\n', "")
            )
        else:
            text = f.read()
...
def release(args):
    from _tdxlwings.pro import LicenseHandler
    from _tdxlwings.utils import query_yes_no, read_user_config
    ...
    # Sheet Config
    if "_tdxlwings.conf" not in [i.name for i in book.sheets]:
        ...
        use_without_addin = query_yes_no(
            "Allow your tool to run without the _tdxlwings add-in?"
        )
        ...
        if not query_yes_no(f'This will release "{book.name}", proceed?'):
            ...
            print("* Add _tdxlwings.conf sheet")
            config_sheet = book.sheets.add(
                "_tdxlwings.conf", after=book.sheets[len(book.sheets) - 1]
            )
        ...
        if not query_yes_no(
            ...
            f'according to the "_tdxlwings.conf" sheet, proceed?'
        ):
            sys.exit()
        print()
        ...
        book.sheets["_tdxlwings.conf"]["A1"].value = config

    # Remove Reference
    if config["RELEASE_NO_ADDIN"]:
        if "_tdxlwings" in [i.Name for i in book.api.VBProject.References]:
            print("* Remove VBA Reference")
            ref = book.api.VBProject.References("_tdxlwings")
        ...
        book.api.VBProject.VBComponents.Import(this_dir / "_tdxlwings.bas")
    ...
    if config.get("RELEASE_HIDE_CONFIG_SHEET"):
        ...
        book.sheets["_tdxlwings.conf"].visible = False
    ...
    print()
    print(
        "Checking for _tdxlwings version compatibility "
    ...
    if interpreter_path and Path(interpreter_path).is_file():
        res = subprocess.run(
            [
                ...
                "from tdrpa import _tdxlwings;print(_tdxlwings.__version__)",
            ],
        ...
        if xlwings_version_installer == xw.__version__:
            ...
            print(
                f"ERROR: You are running this command with _tdxlwings {xw.__version__} "
                f"but your installer uses {xlwings_version_installer}!"
            )
    else:
        print(
            f"WARNING: Prepared '{book.name}' for release "
            "but couldn't verify the _tdxlwings version!"
        )
        ...
        if (
            ...
            if overwrite or not file_path.exists():
                ...
                if vb_component.Type == 100:
                    ...
                    # classes when running "_tdxlwings vba import"
...
def vba_get_book(args):
    ...
    from _tdxlwings.utils import query_yes_no
...
def main():
    print("_tdxlwings version: " + xw.__version__)
    ...
    addin_parser = subparsers.add_parser(
        "addin",
        help='Run "_tdxlwings addin install" to install the Excel add-in '
        ...
        'e.g. "_tdxlwings add-in install -f custom.xlam or copy all Excel '
    ...
    quickstart_parser = subparsers.add_parser(
        "quickstart",
        help='Run "_tdxlwings quickstart myproject" to create a '
    ...
    quickstart_parser.add_argument(
        "-s", "--standalone", action="store_true", help="Include _tdxlwings as VBA module."
    )
    quickstart_parser.add_argument(
        "-r",
        "--remote",
    ...
    restapi_parser = subparsers.add_parser(
        "restapi",
        help='Use "_tdxlwings restapi run" to run the _tdxlwings REST API via Flask '
    ...
    license_parser = subparsers.add_parser(
        "license",
        help='_tdxlwings PRO: Use "_tdxlwings license update -k KEY" where '
        '"KEY" is your personal (trial) license key. This will '
        "update ~/._tdxlwings/_tdxlwings.conf with the LICENSE_KEY entry. "
        'If you have a paid license, you can run "_tdxlwings license deploy" '
    ...
    license_update_parser.add_argument(
        "-k", "--key", help="Updates the LICENSE_KEY in ~/._tdxlwings/_tdxlwings.conf."
    )
    ...
    config_parser = subparsers.add_parser(
        "config",
        help='Run "_tdxlwings config create" to create the user config file '
        "(~/._tdxlwings/_tdxlwings.conf) which is where the settings from "
    ...
    code_parser = subparsers.add_parser(
        "code",
        help='Run "_tdxlwings code embed" to embed all Python modules of the '
    ...
    release_parser = subparsers.add_parser(
        "release",
        help='Run "_tdxlwings release" to configure your active workbook to work with a '
        "one-click installer for easy deployment. Requires _tdxlwings PRO.",
    )
    ...
    copy_parser = subparsers.add_parser(
        "copy",
        help='Run "_tdxlwings copy os" to copy the _tdxlwings Office Scripts module. '
        'Run "_tdxlwings copy gs" to copy the _tdxlwings Google Apps Script module. '
        'Run "_tdxlwings copy vba" to copy the standalone _tdxlwings VBA module. '
        'Run "_tdxlwings copy vba --addin" to copy the _tdxlwings VBA module for custom '
        "add-ins.",
    )
    ...
    auth_parser = subparsers.add_parser(
        "auth",
        help='Microsoft Azure AD: "_tdxlwings auth azuread", see '
    ...
    vba_parser = subparsers.add_parser(
        ...
        editor: run "_tdxlwings vba edit" to update the VBA modules of the active workbook
        ...
        run "_tdxlwings vba export". To overwrite the VBA modules in Excel with their local
        versions, run "_tdxlwings vba import".
        ...
        form or the properties of a module), you have to run "_tdxlwings vba export".
        """,
    )
    ...
    py_parser = subparsers.add_parser(
        ...
        Microsoft's Python in Excel cells (=PY) via a local editor: run "_tdxlwings py edit" to
