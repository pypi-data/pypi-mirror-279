...
def func_sig(f):
    ...
    for p in s.parameters.values():
        if p.kind is inspect.Parameter.POSITIONAL_OR_KEYWORD:
            ...
            raise Exception("_tdxlwings does not support UDFs with keyword arguments")
...
def get_category(**func_kwargs):
    ...
    return "_tdxlwings"  # Default category
    ...
    def inner(f):
        if not hasattr(f, "__xlfunc__"):
            ...
            if sig["vararg"] and nDefaults > 0:
                raise Exception(
                    "_tdxlwings does not support UDFs "
...
class ComRange(Range):
    ...
    def impl(self):
        ...
        self._deser = tdxlwings._xlwindows.Range(xl=dispatch)
...
def get_udf_module(module_name, xl_workbook):
    ...
    if module_info is not None:
        ...
        if xl_workbook:
            wb = Book(impl=tdxlwings._xlwindows.Book(Dispatch(xl_workbook)))
...
def get_cache_key(func, args, caller):
    ...
    xw_caller = Range(impl=tdxlwings._xlwindows.Range(xl=caller))
    return (
        func.__name__
        + str(args)
...
def call_udf(module_name, func_name, args, this_workbook=None, caller=None):
    ...
    for i, arg in enumerate(args):
        ...
        elif tdxlwings._xlwindows.is_range_instance(arg):
            args[i] = conversion.read(
                Range(impl=tdxlwings._xlwindows.Range(xl=arg)),
                None,
                arg_info["options"],
            )
        else:
    ...
    if this_workbook:
        tdxlwings._xlwindows.BOOK_CALLER = Dispatch(this_workbook)
...
def import_udfs(module_names, xl_workbook):
    ...
    vba.writeln(
        "'Autogenerated code by _tdxlwings - changes will be lost with next import!"
    )
    vba.writeln(
    ...
    if __pro__:
        for module_name in module_names:
            sheet_config = read_config_sheet(
                Book(impl=tdxlwings._xlwindows.Book(Dispatch(xl_workbook)))
            )
