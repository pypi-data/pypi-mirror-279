...
_tdxlwings PRO is dual-licensed under one of the following licenses:

* PolyForm Noncommercial License 1.0.0 (for noncommercial use):
  https://polyformproject.org/licenses/noncommercial/1.0.0
* _tdxlwings PRO License (for commercial use):
...
from tdrpa import tdxlwings  # To prevent circular imports
...
class LicenseHandler:
    ...
    def get_cipher():
        try:
            ...
            raise tdxlwings.LicenseError(
                "Couldn't validate _tdxlwings license key."
            ) from None
    ...
    def get_license():
        ...
        try:
            sheet_license_key = read_config_sheet(tdxlwings.Book.caller()).get(
                "LICENSE_KEY"
            )
            ...
            pass
        # User config file
        config_file = tdxlwings.USER_CONFIG_FILE
        ...
        raise tdxlwings.LicenseError("Couldn't find an _tdxlwings license key.")
    ...
    def validate_license(product, license_type=None):
        ...
        if key.startswith("gA") and not Fernet:
            ...
            raise ImportError(
                "You are using a legacy _tdxlwings license key that requires the "
            ...
            try:
                ...
                raise tdxlwings.LicenseError("Invalid _tdxlwings license key.") from None
        else:
            signature = hmac.new(
        ...
        try:
            if (
                ...
                raise tdxlwings.LicenseError(
                    "You need a paid _tdxlwings license key for this action."
                )
        except KeyError:
            raise tdxlwings.LicenseError(
                "You need a paid _tdxlwings license key for this action."
        ...
        if (
            ...
            raise tdxlwings.LicenseError("Invalid _tdxlwings license key format.") from None
        license_valid_until = dt.datetime.strptime(
            license_info["valid_until"], "%Y-%m-%d"
        ).date()
        if dt.date.today() > license_valid_until:
            raise tdxlwings.LicenseError(
                "Your _tdxlwings license expired on {}.".format(
                    license_valid_until.strftime("%Y-%m-%d")
                )
            ) from None
        if product not in license_info["products"]:
            raise tdxlwings.LicenseError(
                f"Your _tdxlwings license key isn't valid for the '{product}' "
                "functionality."
            ) from None
        if (
            "version" in license_info.keys()
            and license_info["version"] != tdxlwings.__version__
        ):
            raise tdxlwings.LicenseError(
                "Your _tdxlwings deploy key is only valid for v{0}. To use a different "
                "version of _tdxlwings, re-release your tool or generate a new deploy key "
                "via '_tdxlwings license deploy'.".format(license_info["version"])
            ) from None
        if (license_valid_until - dt.date.today()) < dt.timedelta(days=30):
            warnings.warn(
                f"Your _tdxlwings license key expires in "
...
def get_embedded_code_temp_dir():
    tmp_base_path = os.path.join(tempfile.gettempdir(), "_tdxlwings")
