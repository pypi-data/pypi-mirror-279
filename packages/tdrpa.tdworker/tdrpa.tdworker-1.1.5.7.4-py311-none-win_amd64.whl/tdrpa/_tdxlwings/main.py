    ...
    def __getitem__(self, name_or_index):
        if isinstance(name_or_index, numbers.Number):
            ...
            try:
                ...
                if not tdxlwings.__pro__ and name_or_index != "excel":
                    raise LicenseError(
...
class Book:
    ...
    def caller(cls):
        ...
        if hasattr(Book, "_mock_caller"):
            ...
            name = wb.lower()
            if sys.platform.startswith("win"):
                app = App(impl=tdxlwings._xlwindows.App(xl=int(hwnd)))
        ...
        elif tdxlwings._xlwindows.BOOK_CALLER:
            # Called via OPTIMIZED_CONNECTION = True
            return cls(impl=tdxlwings._xlwindows.Book(tdxlwings._xlwindows.BOOK_CALLER))
        else:
            raise Exception(
...
class Shape:
    ...
    @text.setter
    def text(self, value):
        if tdxlwings.__pro__:
            from _tdxlwings.pro import Markdown
            from _tdxlwings.pro.reports.markdown import format_text, render_text
