...
class Books(base_classes.Books):
    ...
    def open(
        ...
        if local is not None:
            # TODO: replace with specific error when Exceptions are refactored
            raise tdxlwings.XlwingsError("local is not supported on macOS")
        if corrupt_load is not None:
            # TODO: replace with specific error when Exceptions are refactored
            raise tdxlwings.XlwingsError("corrupt_load is not supported on macOS")
...
class Book(base_classes.Book):
    ...
    def fullname(self):
        ...
        if "://" in path:
            config = read_config_sheet(tdxlwings.Book(impl=self))
            ...
            if len(arg1) == 2:
                if 0 in arg1:
                    raise IndexError(
                        ...
                        "_tdxlwings/Excel Ranges are 1-based."
                    )
                row1 = arg1[0]
                col1 = arg1[1]
...
class Range(base_classes.Range):
    ...
    def to_pdf(self, path, quality=None):
        raise tdxlwings.XlwingsError("Range.to_pdf() isn't supported on macOS.")
...
class Chart(base_classes.Chart):
    ...
    def to_png(self, path):
        raise tdxlwings.XlwingsError("Chart.to_png() isn't supported on macOS.")
    ...
    def to_pdf(self, path, quality=None):
        raise tdxlwings.XlwingsError("Chart.to_pdf() isn't supported on macOS.")
    ...
    if is_excel_running():
        ...
        for app in Apps():
            try:
                ...
                # Excel files initiated from Python don't have the _tdxlwings VBA module
                pass
