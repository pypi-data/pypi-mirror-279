    ...
    def __setattr__(self, key, value):
        ...
        while True:
            try:
                ...
                # characters would be tried forever, causing tdxlwings to hang (they
...
def __isWPSHwnd(hwnd):
    ...
    if xldeskHwnd==0:
        handle = __isWPSHwnd(childWindow)
        if handle == 0:
            handlenext = windll.user32.GetWindow(childWindow,2)
            if handlenext != 0:
                while(1):
                    handletmp = __isWPSHwnd(handlenext)
                    if handletmp == 0:
...
def get_excel_hwnds():
    ...
    while hwnd:
        try:
            ...
            if child_hwnd == 0 and win32gui.GetClassName(hwnd) == 'XLMAIN':  # ID1016716
                child_hwnd = hwnd
            if child_hwnd:
                child_hwnd = __isWPSHwnd(hwnd)
                if child_hwnd:
...
class Apps(base_classes.Apps):
    ...
    def cleanup():
        ...
        active_pids = {app.pid for app in tdxlwings.apps}
