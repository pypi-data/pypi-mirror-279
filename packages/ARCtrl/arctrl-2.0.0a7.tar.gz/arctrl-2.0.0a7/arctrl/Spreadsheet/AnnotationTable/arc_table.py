...
def compose_columns(columns: IEnumerable_1[FsColumn]) -> Array[CompositeColumn]:
    def _arrow980(columns_2: FSharpList[FsColumn], columns: Any=columns) -> CompositeColumn:
        return from_fs_columns(columns_2)

    return to_array(map(_arrow980, group_columns_by_header(to_list(columns))))
...
def to_fs_worksheet(table: ArcTable) -> FsWorksheet:
    ...
    if len(table.Columns) == 0:
        ...
        def _arrow981(column_1: CompositeColumn, table: Any=table) -> FSharpList[FSharpList[FsCell]]:
            return to_fs_columns(column_1)

        def _arrow982(column: CompositeColumn, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
        ...
        columns: FSharpList[FSharpList[FsCell]] = collect(_arrow981, sort_by(_arrow982, of_array(table.Columns), ObjectExpr983()))
