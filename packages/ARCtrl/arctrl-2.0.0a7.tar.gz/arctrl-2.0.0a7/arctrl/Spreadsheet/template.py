...
from ..fable_modules.fable_library.seq import (map as map_1, exists, head, append, singleton, delay, to_list, iterate_indexed, try_pick)
...
from .Metadata.sparse_table import (SparseTable, SparseTable__TryGetValueDefault_5BAE6133, SparseTable_Create_Z2192E64B, SparseTable_FromRows_Z5579EC29, SparseTable_ToRows_759CAFC1, SparseRowModule_tryGetValueAt, SparseRowModule_fromValues, SparseRowModule_writeToSheet, SparseRowModule_fromFsRow)

def _expr1056() -> TypeInfo:
...
TemplateReadError_reflection = _expr1056
...
Metadata_Template_TemplateInfo_reflection = _expr1058
...
def Metadata_Template_TemplateInfo_ToSparseTable_48C39BE1(template: Template) -> SparseTable:
    ...
    def _arrow1059(__unit: None=None, template: Any=template) -> str:
        copy_of_struct: str = template.Id
        return str(copy_of_struct)

    def _arrow1060(__unit: None=None, template: Any=template) -> str:
        copy_of_struct_1: str = template.Id
        return str(copy_of_struct_1)

    add_to_dict(matrix.Matrix, ("Id", 1), "" if (_arrow1059().find("MISSING_IDENTIFIER_") == 0) else _arrow1060())
    ...
    class ObjectExpr1062:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1061(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr1062())), matrix.ColumnCount)
...
def Metadata_Template_toRows(template: Template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    def _arrow1070(__unit: None=None, template: Any=template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow1069(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow1068(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                def _arrow1067(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    def _arrow1066(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                        def _arrow1065(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                            def _arrow1064(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                def _arrow1063(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                    return to_rows_1("Author", of_seq(template.Authors))

                                return append(singleton(SparseRowModule_fromValues(to_enumerable(["AUTHORS"]))), delay(_arrow1063))

                            return append(Metadata_Tags_toRows(None, to_list(template.Tags)), delay(_arrow1064))

                        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TAGS"]))), delay(_arrow1065))

                    return append(Metadata_ER_toRows(None, to_list(template.EndpointRepositories)), delay(_arrow1066))

                return append(singleton(SparseRowModule_fromValues(to_enumerable(["ERS"]))), delay(_arrow1067))

            return append(Metadata_Template_TemplateInfo_toRows_48C39BE1(template), delay(_arrow1068))

        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TEMPLATE"]))), delay(_arrow1069))

    return delay(_arrow1070)
...
def Template_toMetadataSheet(template: Template) -> FsWorksheet:
    sheet: FsWorksheet = FsWorksheet("isa_template")
...
def Template_fromFsWorkbook(doc: FsWorkbook) -> Template:
    ...
    match_value: FsWorksheet | None = doc.TryGetWorksheetByName("isa_template")
    if match_value is None:
        match_value_1: FsWorksheet | None = doc.TryGetWorksheetByName("SwateTemplateMetadata")
    ...
    def _arrow1072(__unit: None=None, doc: Any=doc) -> ArcTable:
        ...
        match_value_2: FsWorksheet | None = try_pick(try_table_name_matches, sheets)
        if match_value_2 is None:
            ...
            match_value_4: FsWorksheet | None = try_pick(try_wsname_matches, sheets)
            if match_value_4 is None:
                raise TemplateReadError(("No worksheet or table with name `" + template_info.Table) + "` found")

            else: 
                ws_3: FsWorksheet = match_value_4
                match_value_5: ArcTable | None = try_from_fs_worksheet(ws_3)
                if match_value_5 is None:
                    raise TemplateReadError(("Ws with name `" + ws_3.Name) + "` could not be converted to a table")

                else: 
                    return match_value_5



        else: 
            ws_2: FsWorksheet = match_value_2
            match_value_3: ArcTable | None = try_from_fs_worksheet(ws_2)
    ...
    return Template_fromParts(template_info, pattern_input[1], pattern_input[2], pattern_input[3], _arrow1072(), now())
