...
def Comment_encoder(comment: Comment) -> Json:
    ...
    def _arrow1098(value: str, comment: Any=comment) -> Json:
        return Json(0, value)

    def _arrow1099(value_2: str, comment: Any=comment) -> Json:
        return Json(0, value_2)

    return Json(5, choose(chooser, of_array([try_include("name", _arrow1098, comment.Name), try_include("value", _arrow1099, comment.Value)])))


def _arrow1102(get: IGetters) -> Comment:
    ...
    return Comment(_arrow1100(), _arrow1101())


Comment_decoder: Decoder_1[Comment] = object(_arrow1102)
...
def Comment_ROCrate_encoder(comment: Comment) -> Json:
    ...
    def _arrow1103(value_2: str, comment: Any=comment) -> Json:
        return Json(0, value_2)

    def _arrow1104(value_4: str, comment: Any=comment) -> Json:
        return Json(0, value_4)

    return Json(5, choose(chooser, of_array([("@id", Json(0, Comment_ROCrate_genID(comment))), ("@type", Json(0, "Comment")), try_include("name", _arrow1103, comment.Name), try_include("value", _arrow1104, comment.Value), ("@context", context_jsonvalue)])))


def _arrow1107(get: IGetters) -> Comment:
    def _arrow1105(__unit: None=None) -> str | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("name", string)

    def _arrow1106(__unit: None=None) -> str | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("value", string)

    return Comment(_arrow1105(), _arrow1106())


Comment_ROCrate_decoder: Decoder_1[Comment] = object(_arrow1107)
...
Comment_ISAJson_encoder: Callable[[Comment], Json] = Comment_encoder
...
def ARCtrl_Comment__Comment_toJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1108(c: Comment, spaces: Any=spaces) -> str:
...
def ARCtrl_Comment__Comment_toROCrateJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1109(c: Comment, spaces: Any=spaces) -> str:
...
def ARCtrl_Comment__Comment_toISAJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[Comment], str]:
    def _arrow1110(c: Comment, spaces: Any=spaces) -> str:
        value: Json = Comment_ISAJson_encoder(c)
        return to_string(default_spaces(spaces), value)
