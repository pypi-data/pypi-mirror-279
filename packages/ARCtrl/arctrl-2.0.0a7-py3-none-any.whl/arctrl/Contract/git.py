...
gitignore_contract: Contract = Contract.create_create(".gitignore", DTOType(7), DTO(1, dgi))

gitattributes_contract: Contract = Contract.create_create(".gitattributes", DTOType(7), DTO(1, dga))

def _expr1967() -> TypeInfo:
...
Init_reflection = _expr1967

def _expr1968() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow1974(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow1973(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow1972(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow1971(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow1970(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow1969(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow1969))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow1970))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow1971))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow1972))

        return append(singleton(Clone_get_clone()), delay(_arrow1973))

    return create_git_contract(to_array(delay(_arrow1974)))
