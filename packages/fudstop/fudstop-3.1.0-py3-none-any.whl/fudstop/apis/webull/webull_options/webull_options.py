...
trader = PaperTrader()
class WebullOptions:
    def __init__(self, database:str='market_data', user:str='chuck'):
        ...
        self.headers = {
            ...
            "Reqid": "503b310ac3014775819c6b113f32477e"}
    ...
    async def all_options(self, ticker, direction='all', expireDate =  None):
        try:
            self.headers.update({"Access-Token":await trader.get_token()})
            ...
            params = {"tickerId":ticker_id,"count":-1,"direction":direction,"type":0,"quoteMultiplier":100,"unSymbol":ticker}
            expireDate == nearest_friday if expireDate == None else params['expire_date']
    ...
    async def all_options_new(self, ticker, days_until_expiry):
        ticker_id = await self.get_ticker_id(ticker)
    ...
    async def fetch_volume_analysis(self, id, symbol, conn):
    ...
    async def option_flow(self, option_id):   
    ...
    async def vol_anal_new(self, ticker):
        await self.db_manager.get_connection()
    ...
    async def stream_options(self, option_id):
    ...
    async def order_flow(self, option_id):
        async with httpx.AsyncClient(headers=self.headers) as client:
    ...
    async def process_ids(self, ids, symbols):
