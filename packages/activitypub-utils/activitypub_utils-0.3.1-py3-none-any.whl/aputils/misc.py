...
class MessageDate(HttpDate):
	...
	def parse(cls: type[Self], date: datetime | str | int | float) -> Self:
		if isinstance(date, str):
			try:
				return cls.strptime("%Y-%m-%dT%H:%M:%S.%fZ", date)
...
class Signature:
	...
	def __init__(self,
		...
		self.created: HttpDate | None = None
		"Signature creation date"

		self.expires: HttpDate | None = None
	...
	def _set_date(self, key: str, value:  HttpDate | datetime | str | int | float | None) -> None:
		if value is None:
		...
		setattr(self, key, HttpDate.parse(value))


	def set_created(self, value: HttpDate | datetime | str | int | float | None) -> None:
		"""
	...
	def set_expires(self, value: HttpDate | datetime | str | int | float | None) -> None:
		"""
