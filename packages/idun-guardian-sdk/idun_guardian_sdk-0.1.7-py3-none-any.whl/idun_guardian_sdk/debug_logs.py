...
def log_connection_flag(flag_value):
    logger.debug(f"[BLE]: Connection: {flag_value}")


def log_connection_initialize_flag(flag_value):
    logger.debug(f"[BLE]: Initialize: {flag_value}")
...
def log_no_connection_established():
    logger.debug(
...
def log_device_not_connected_cannot_stop():
    logger.debug("[API]: Device not connected, cannot stop")
...
def logging_break():
    logger.debug("[API]: Breaking API client while loop")
...
def logging_connection_closed():
    logger.debug("[API]: Websocket client connection closed")


def logging_reconnection():
    logger.debug("[API]: Ping successfull, connection alive and continue..")
    logger.debug("Try to ping websocket successfull")
...
def logging_sending_stop():
    logger.debug("[BLE]: Stop command loaded into queue")


def logging_giving_time_api():
    logger.debug("[BLE]: Giving time to API client to send data")


def logging_sending_stop_device():
...
def logging_turn_ble_on():
    logger.debug("[BLE]: Turning BLE on")
...
def logging_recording_successfully_stopped():
    logger.debug("[BLE]: Recording successfully stopped")


def logging_keyboard_interrupt():
    logger.debug("[BLE]: KeyboardInterrupt applied, terminating...")
    logger.debug("[BLE]: Sending stop signal to device and cloud, please wait a moment ...")


def logging_stop_send():
    logger.debug("[API]: Sending stop signal to device and cloud, please wait a moment ...")
...
def logging_trying_to_connect_again(try_to_connect_timeout):
    logger.warning(
...
def logging_device_connected_general():
    logger.debug("[BLE]: Device connected")


def logging_recording_started():
...
def logging_subscribing_impedance_notification():
...
def logging_starting_impedance_measurement_commands():
