...
class SamplingParameters1(BaseModel):
    rowSampleCount: int = Field(..., description="The number of rows sampled per file")
...
class DataAssetFieldProfileStats(BaseModel):
    sampledRecordsCount: Optional[int] = Field(
        default=None, description="The number of rows sampled to generate the profile"
    )
    sampledDate: datetime = Field(..., description="The date the sample was taken")
    sampledFiles: Optional[List[str]] = Field(
        default=None,
        description="The files that were sampled to generate the profile",
        min_items=1,
    )
    samplingParameters: Optional[SamplingParameters1] = Field(
        default=None, description="The parameters used to sample the data"
    )
...
class DataAssetField(BaseModel):
    ...
    profile: Optional[DataAssetFieldProfileStats] = None
...
class GetChangelogResponse(BaseModel):
    changelogEvents: List[
        Union[AssetCreatedEvent, AssetModifiedEvent, ContractViolation]
    ] = Field(
        ...,
