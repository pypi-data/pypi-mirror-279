...
def _discover_file_paths_from_s3_bucket(
    ...
    if (
        ...
        and not any([incl in prefix or prefix in incl for incl in include_prefix])
    ):
        log_trace(f"({prefix})\tExcluded by include pattern")
        return
    if (
        exclude_prefix
        and len(exclude_prefix) > 0
        and any([excl in prefix for excl in exclude_prefix])
    ):
        # If include prefixes are passed in, then exclude prefixes should be a subset of the include prefixes to be considered
        log_trace(f"({prefix})\tExcluded by exclude pattern")
        return
    try:
        files = list(
            filter(
