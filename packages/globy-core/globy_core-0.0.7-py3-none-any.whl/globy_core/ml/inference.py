...
class Inferencer:
    ...
    def generate(self, sample, check_token_limit=False) -> str:
        """
        ...
        if self.noop:
            return 'NOOP'
        if self.model_type == 'OPENAI':
            return self._openai_inference(sample)
        raise ValueError(f"Model type {self.model_type} not supported.")

    def _openai_inference(self, sample) -> str:
        result = None
        try:
            completion = self.client.chat.completions.create(
            model=self.model,
            messages=self._create_openai_sample(sample),
            temperature=0,
            )
    ...
    def _create_openai_sample(self, sample) -> list:
        return [
...
def NewChatBot(context, inferencer_kwargs=None, model_type='OPENAI') -> ChatCompletion:
    """
    Required args:
    "context" - The context/instruction.
    "output_queue" - The queue to store the results.
    "model_type" - The model type to use for inference. Default is 'OPENAI'.
    Create a new instance of the BootstrapInferencer.
    "inferencer_args" is an optional dictionary passed to the inferencer:
