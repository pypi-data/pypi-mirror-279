...
from azureml.rag.utils.connections import (
    ...
    get_obo_or_default_azure_credential,
    get_target_from_connection,
    workspace_connection_to_credential,
)
from azureml.rag.utils.exceptions import (
    map_exceptions, 
    ValidationUserErrorIncorrectDeploymentState,
    ValidationUserErrorCannotPullCompletionModel,
    ValidationUserErrorCannotPullEmbeddingModel,
    ValidationUserErrorInvalidACSName,
)
...
def get_cognitive_services_client(subscription_id: str):
    ...
    return CognitiveServicesManagementClient(
        credential=get_obo_or_default_azure_credential(), subscription_id=subscription_id
    )
...
def validate_and_create_default_aoai_resource(
    ...
    if response_status in ["failed", "deleting"]:
        ...
        activity_logger.error(f"ValidationFailed: Deployment {model_params['deployment_id']} for model "
            + f"{model_params['model_name']} is in failed or deleting state. Please resubmit job with a "
            + "successful deployment.")
        raise ValidationUserErrorIncorrectDeploymentState()
...
def check_deployment_status(model_params: dict, model_type, activity_logger: Logger):
    ...
    openai.api_key = model_params.get("openai_api_key")
    ...
    if model_params["openai_api_type"].lower() != "azure" or not model_params["deployment_id"]:
        ...
        activity_logger.info(
            "[Validate Deployments]: Not an Azure Open AI resource, or the deployment name is missing - pass through deployment validation.",
            extra={
                "properties": {
        ...
        if model_type == "llm":
            ...
            if openai_v1plus:
                embedder = OpenAIEmbedder(
                    ...
                    api_base=model_params["openai_api_base"],
                    api_type=model_params["openai_api_type"],
                    api_version=model_params["openai_api_version"],
                    api_key=model_params.get("openai_api_key"),
                    azure_ad_token_provider=model_params.get("azure_ad_token_provider"),
                    deployment=model_params["deployment_id"],
                    batch_size=model_params.get("batch_size"),
                    max_retries=model_params.get("embedding_ctx_length"),
                )
            else:
            ...
            try:
                with tiktoken_cache_dir():
                    embedder.embed_documents("Embed this query to test if deployment exists")
            except Exception as ex:
                _throw_aoai_deployment_not_found_error(ex, model_params["deployment_id"], activity_logger)
                raise
        return True
...
def is_default_connection(connection) -> bool:
    ...
    if isinstance(connection, dict):
        return connection.get("name") == "Default_AzureOpenAI"
...
def validate_aoai_deployments(
    ...
    if llm_config and llm_connection_id:
        ...
        credential = connection_to_credential(llm_connection, data_plane=True)
        if hasattr(credential, "key") or callable(credential):
            ...
            if callable(credential):
                completion_params["azure_ad_token_provider"] = credential
            else:
        ...
        if not completion_params:
            activity_logger.error("ValidationFailed: Completion model LLM connection was unable to pull information")
            raise ValidationUserErrorCannotPullCompletionModel()
        activity_logger.info(
    ...
    if embeddings_model and embeddings_connection_id:
        ...
        credential = connection_to_credential(embeddings_connection, data_plane=True)
        ...
        if hasattr(credential, "key") or callable(credential):
            ...
            if callable(credential):
                embedding_params["azure_ad_token_provider"] = credential
            else:
        ...
        if not embedding_params:
            activity_logger.error("ValidationFailed: Embedding model connection was unable to pull information")
            raise ValidationUserErrorCannotPullEmbeddingModel()
        activity_logger.info(
...
def validate_openai_deployments(
    ...
    if llm_config and llm_connection_id:
        ...
        key = credential() if callable(credential) else credential.key
        model = llm_config["model_name"]
        print(f"Completion model name: {model}")
        get_openai_model(model, key, activity_logger)
...
def validate_acs(
    ...
    if (
        ...
        raise ValidationUserErrorInvalidACSName(ValueError(error_msg))
    ...
    for retry_counter in range(MAX_RETRIES):
        try:
            ...
            if retry_counter < MAX_RETRIES:
                activity_logger.info(
                    f"[Validate Deployments]: Failed to validate ACS. Retry after {SLEEP_DURATION} seconds."
                )
...
def main_wrapper(args, logger):
    ...
    with track_activity(
        ...
        try:
            map_exceptions(main, activity_logger, args, activity_logger)
        except Exception:
