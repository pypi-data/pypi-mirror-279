...
class MLIndex:
        ...
        with track_activity(logger, "MLIndex.as_langchain_vectorstore") as activity_logger:
            ...
            if index_kind == "acs":
                ...
                if langchain_pkg_version >= pkg_version.parse("0.1.00"):
                    from azureml.rag.utils.acs import _get_azuresearch_module_instance
                ...
                try:
                    ...
                    connection_credential = get_connection_credential(
                        self.index_config, credential=credential)
                    if not isinstance(connection_credential, AzureKeyCredential):
                        raise ValueError(
                    ...
                    client = QdrantClient(
                        url=qdrant_host_url, api_key=connection_credential.key)
                    ...
                    return Qdrant(
                        ...
                        content_payload_key=self.index_config.get(
                            "field_mapping", {}).get("content", "text"),
                        # None is treated as the default vector on qdrant side
                        vector_name=self.index_config.get(
                            "field_mapping", {}).get("embedding", None)
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create Qdrant vectorstore due to: {e}")
                    raise
    ...
    def as_native_index_client(self, credential: Optional[TokenCredential] = None):
        ...
        An azure.search.documents.SearchClient for acs indexes or an azureml.rag.indexes.indexFaissAndDocStore for faiss indexes.
        """
        index_kind = self.index_config.get("kind", None)
        if index_kind == "acs":
            connection_credential = get_connection_credential(self.index_config, credential=credential)
