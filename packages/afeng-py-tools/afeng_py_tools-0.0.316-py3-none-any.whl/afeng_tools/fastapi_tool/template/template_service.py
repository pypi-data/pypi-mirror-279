...
from afeng_tools.fastapi_tool.template.item.page_template_items import PageTemplateItem
...
def render_template(request: Request, template_file: str, template_item: PageTemplateItem,
                    is_cache: bool = False) -> _TemplateResponse:
    """响应模板"""
    if isinstance(template_item, Response):
        return template_item
    return create_template_response(request,
...
def create_html_head_data(_app_info: AppInfo, is_mobile: bool, title: str, description: str = None,
                          ...
                          baidu_tm_id: str = None) -> TemplateHtmlHeadData:
    """创建页面head"""
    if _app_info:
        ...
        if baidu_tm_id is None:
            baidu_tm_id = _app_info.baidu_tm_id
    else:
        if title is not None:
            title = f'{title}'
        if description is None:
    ...
    return TemplateHtmlHeadData(is_mobile=is_mobile, title=title, description=description,
                                ...
                                author=author, favicon=favicon, origin=origin, baidu_tm_id=baidu_tm_id)
