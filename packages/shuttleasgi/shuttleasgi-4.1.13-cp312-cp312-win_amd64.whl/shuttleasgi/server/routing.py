...
# from shuttleasgi.common import extend
# from shuttleasgi.common.types import (
#     HeadersType,
#     ParamsType,
#     normalize_headers,
#     normalize_params,
...
# from shuttleasgi.utils import ensure_bytes, ensure_str
...
#         self._path_to_methods: Dict[bytes, Set[str]] = defaultdict(set)
...
#         # Update the set of allowed methods for a given path
#         self._path_to_methods[new_route.pattern].add(method)
...
#     def get_allowed_methods(self, path: bytes) -> Set[str]:
#         """
#         Returns a set of allowed methods for a given path.
#         """
#         return self._path_to_methods.get(path, set())

#     def get_match(self, request: Request) -> Optional[RouteMatch]:
#         """
#         Gets a match for the given request, by method and request path.
#         """
#         match = self.get_match_by_method_and_path(request.method, request._path)
#         if not match:
#             # Check if the path exists but with different methods
#             allowed_methods = self.get_allowed_methods(request._path)
#             if allowed_methods:
...
class Router(RouterBase):
    ...
    def get_match(self, request: Request) -> Optional[RouteMatch]:
        ...
        Gets a match for the given request, by method and request path.
        """
        return self.get_match_by_path_regardless_of_method(request._path)
        # return self.get_match_by_method_and_path(request.method, request._path)
    ...
    def get_match_by_method_and_path(
        ...
        bytes_value: bytes = ensure_bytes(path)
    ...
    @lru_cache(maxsize=1200)
    def get_match_by_path_regardless_of_method(
        self, path: AnyStr
    ) -> Optional[RouteMatch]:
        bytes_value: bytes = ensure_bytes(path)
