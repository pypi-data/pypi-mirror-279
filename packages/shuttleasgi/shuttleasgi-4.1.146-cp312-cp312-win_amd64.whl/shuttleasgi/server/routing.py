...
class Router(RouterBase):
    ...
    def add_or_append_reverse_route(self, route: Route, method: AnyStr):
        if route in self.rr:
            self.rr[route].append(ensure_bytes(method))
        else:
            self.rr[route] = [ensure_bytes(method)]

    def add_route(self, method: AnyStr, route: Route):
        self.routes[ensure_bytes(method)].append(route)
        self.add_or_append_reverse_route(route, method)
    ...
    def get_match_by_method_and_path(
        ...
        bytes_value = ensure_bytes(path)
        print("NON REVERSED", self.routes)
