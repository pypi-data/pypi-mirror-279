...
class extractor:
    ...
    #--------------------------------
    def _plot_fit(self):
        log.warning('Fit plotting dissabled')
        return
    ...
    #--------------------------------
    def _print_model(self, model, name=None):
        txt_path = f'{self._plt_dir}/{name}.txt' 
        zut.print_pdf(model, txt_path=txt_path)
    #--------------------------------
    def _save_model(self, kind=None):
        for dsname in self._l_dset:
            [mod_bm, mod_qm], mod_ee = self._d_model[dsname]

            self._print_model(mod_bm, name=f'{dsname}_{kind}_bm_mm')
            self._print_model(mod_qm, name=f'{dsname}_{kind}_qm_mm')
            self._print_model(mod_ee, name=f'{dsname}_{kind}_bm_ee')
    #--------------------------------
    @utnr.timeit
    def get_fit_result(self):
        ...
        self._save_model('pre_fit')
        for dsname in self._l_dset:
            log.info(f'Adding {dsname} dataset')

            [mod_bm, mod_qm], mod_ee = self._d_model[dsname]
            [dat_bm, dat_qm], dat_ee = self._d_data [dsname]

            nll_bm = zfit.loss.ExtendedUnbinnedNLL(model=mod_bm, data=dat_bm)
            nll_qm = zfit.loss.ExtendedUnbinnedNLL(model=mod_qm, data=dat_qm)
            nll_ee = zfit.loss.ExtendedUnbinnedNLL(model=mod_ee, data=dat_ee)

            if nll is None:
                nll = nll_bm + nll_qm + nll_ee
            else:
                nll+= nll_bm + nll_qm + nll_ee
