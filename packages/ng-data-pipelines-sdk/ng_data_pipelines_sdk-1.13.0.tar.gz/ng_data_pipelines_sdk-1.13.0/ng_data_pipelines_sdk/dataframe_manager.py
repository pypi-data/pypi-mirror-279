...
class DataFrameManager:
    ...
    def apply_transform(
        ...
        if transform_params.apply_only_on:
            dfs_to_transform: DataFrameDict = {}
            for key in transform_params.apply_only_on:
                if key not in input_dfs_dict:
                    logger.warning(
                        f"'{key}' was specified in 'apply_only_on' but not found in the input DataFrames. Skipping transformation."
                    )
                    return input_dfs_dict
                else:
                    dfs_to_transform[key] = input_dfs_dict[key]

            dfs_to_keep: DataFrameDict = {}
            for key in input_dfs_dict:
                if key not in dfs_to_transform:
                    dfs_to_keep[key] = input_dfs_dict[key]
        else:
            dfs_to_keep: DataFrameDict = {}
            dfs_to_transform: DataFrameDict = input_dfs_dict

        if transform_params.fn_kind == FnKind.SINGLE:
            for df_name, df in dfs_to_transform.items():
                input_dfs_dict[df_name] = transform_fn(df, **fn_kwargs)
        elif transform_params.fn_kind == FnKind.BATCH:
            transformed_dfs = transform_fn(dfs_to_transform, **fn_kwargs)
            input_dfs_dict = {**dfs_to_keep, **transformed_dfs}
    ...
    def process_step(
        ...
        try:
            ...
            if step_params.transform_params:
                ...
                print("")
                logger.info("Finished applying transformations to DataFrames.")
            else:
