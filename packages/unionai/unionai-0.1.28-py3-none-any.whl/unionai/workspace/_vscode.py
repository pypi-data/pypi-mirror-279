...
UNIONAI_GITHUB_TOKEN_ENV = "WORKSPACE_GITHUB_TOKEN"
UNIONAI_SERVERLESS_API_KEY_ENV = "WORKSPACE_SERVERLESS_API_KEY"
GIT_CONFIG_PATH = ROOT_PATH / ".gitconfig"
...
project: default
domain: development
container_image: ghcr.io/unionai/workspace:0.0.3
...
class WorkspaceConfig(DataClassYAMLMixin, DataClassJSONMixin):
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.container_image, str) and os.getenv("UNIONAI_DEV"):
            ...
            image_spec = imagespec_with_local_unionai()
            self.container_image = ImageSpecConfig(
                name="workspace",
                builder="unionai",
                packages=image_spec.packages,
...
def configure_git(repo: str, git_name: str, git_email: str) -> str:
    ...
    github_token = secrets.get(key=UNIONAI_GITHUB_TOKEN_ENV)
    bashrc_content.append(f"export GITHUB_TOKEN={github_token}")

    api_key = secrets.get(key=UNIONAI_SERVERLESS_API_KEY_ENV)
    bashrc_content.append(f"export UNIONAI_SERVERLESS_API_KEY={api_key}")
...
@vscode
def workspace(config_content: str):
...
class WorkspaceResolver(TaskResolverMixin):
    ...
    def location(self) -> str:
        return "unionai.workspace._vscode.resolver"

    @property
    def name(self) -> str:
        return "unionai.workspace.vscode"
