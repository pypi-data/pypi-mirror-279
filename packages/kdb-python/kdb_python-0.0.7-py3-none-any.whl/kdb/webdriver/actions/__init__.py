...
from selenium.common.exceptions import InvalidSelectorException, NoSuchElementException, InvalidArgumentException, \
...
def _interact_element(driver, locator, on_iframe: bool, action_func, *params):
    ...
    if str(locator).startswith(LOCATOR_ACCESSIBILITY_ID_PREFIX):  # ios
        ...
        try:
            element_presence = EC.presence_of_element_located((By.XPATH, locator[len(LOCATOR_XPATH_PREFIX):]))
            wait.until(element_presence)
            element = driver.find_element(By.XPATH, locator[len(LOCATOR_XPATH_PREFIX):])
        except (NoSuchElementException, TimeoutException):
            return NOT_FOUND_ELEMENT
    else:
        try:
...
def perform(action_func, driver, locator, timeout, in_frame_on_ios, *params):
    ...
    if res != NOT_FOUND_ELEMENT:
        ...
        if res is True:
            ...
            # this using for get_* function
            return res
    # scroll to top of page before search & scroll
