...
from kdb.webdriver.common import report_passed_test_step, report_failed_test_step, take_screen_shot, log_start, \
...
class KDBWebDriver:
    ...
    def close_browser(self, log=True):
        ...
        try:
            ...
            report_failed_test_step(self._driver, self.close_browser.__name__, args_passed, start_time, str(ex))
    ...
    def update_text(self, locator, value, slow=False, decrypt=False, in_frame_on_ios=False,
        ...
        """
        args_passed = locals()
        start_time = log_start(self.update_text.__name__, args_passed, log)
        try:
            actions.perform(actions.update_text, self._driver, locator, timeout, in_frame_on_ios, value, slow, decrypt,
                            extra_time)
            report_passed_test_step(self.update_text.__name__, args_passed, start_time,
                                    InfoMessage.ACTION_SUCCESS % ("Update " + locator + " with value is" + value))
        except Exception as ex:
            report_failed_test_step(self._driver, self.update_text.__name__, args_passed, start_time, str(ex))
        return self
    ...
    def verify_text_on_page(self, text, reverse=False, in_frame_on_ios=False,
        ...
        """
        args_passed = locals()
        start_time = log_start(self.verify_text_on_page.__name__, args_passed, log)
        try:
            assert actions.verify(actions.verify_text_on_page, True, in_frame_on_ios, self._driver, reverse, timeout,
                                  text)
            report_passed_test_step(self.verify_text_on_page.__name__, args_passed, start_time,
                                    InfoMessage.ACTION_SUCCESS % ("Verify the '" + text + "' is on page"))
        except AssertionError:
            report_failed_test_step(self._driver, self.verify_text_on_page.__name__, args_passed, start_time,
                                    "This " + text + " text is not displayed in page")
        except Exception as ex:
            report_failed_test_step(self._driver, self.verify_text_on_page.__name__, args_passed, start_time, str(ex))
        return self
    ...
    def verify_title(self, title, reverse=False, timeout=DriverSettings.DRIVER_IMPLICITLY_WAIT, log=True):
        ...
        """
        args_passed = locals()
        start_time = log_start(self.verify_title.__name__, args_passed, log)
        try:
            page_title = self._driver.title
            assert actions.verify(actions.verify_title, False, False, self._driver, reverse, timeout, title)
            report_passed_test_step(self.verify_title.__name__, args_passed, start_time,
                                    InfoMessage.ACTION_SUCCESS % ("Verify the title is " + title))
        except AssertionError:
            report_failed_test_step(self._driver, self.verify_title.__name__, args_passed, start_time,
                                    "The title is not match. Current title is " + page_title)
        except Exception as ex:
            report_failed_test_step(self._driver, self.verify_title.__name__, args_passed, start_time, str(ex))
        return self

    def screen_shot(self, file_name=None, element_locator="xpath=//body", extra_time=None, log=True):
        """
        Screen shot and save image into folder
        """
        args_passed = locals()
        start_time = log_start(self.screen_shot.__name__, args_passed, log)
        try:
            path = take_screen_shot(self._driver, file_name, element_locator, extra_time)
            report_passed_test_step(self.screen_shot.__name__, args_passed, start_time, path)
            return path
        except Exception as ex:
            report_failed_test_step(self._driver, self.screen_shot.__name__, args_passed, start_time, str(ex))
    ...
    def select(self, locator, value=None, index=None, text=None, in_frame_on_ios=False,
        ...
        """
        args_passed = locals()
        start_time = log_start(self.select.__name__, args_passed, log)
        try:
            if WebDriverUtil.is_safari(self._driver):
                ...
                actions.perform(actions.select, self._driver, locator, timeout, in_frame_on_ios, value, index, text,
                                extra_time)
            report_passed_test_step(self.select.__name__, args_passed, start_time, InfoMessage.ACTION_SUCCESS % (
                    "Select locator %s with value is %s " % (locator, value)))
        except Exception as ex:
            report_failed_test_step(self._driver, self.select.__name__, args_passed, start_time, str(ex))
        return self
    ...
    def set_element_attribute(self, locator, name, value, in_frame_on_ios=False,
        ...
        """
        args_passed = locals()
        start_time = log_start(self.set_element_attribute.__name__, args_passed, log)
        try:
            actions.perform(actions.set_element_attribute, self._driver, locator, timeout, in_frame_on_ios, name, value,
                            extra_time)
            report_passed_test_step(self.set_element_attribute.__name__, args_passed, start_time,
                                    InfoMessage.ACTION_SUCCESS % (
                                            "Set element attribute with name is %s and value %s " % (name, value)))
        except Exception as ex:
            report_failed_test_step(self._driver, self.set_element_attribute.__name__, args_passed, start_time, str(ex))
        return self
