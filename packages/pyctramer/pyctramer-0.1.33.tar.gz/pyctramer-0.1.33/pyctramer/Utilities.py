...
def check_job_unfinished(job_control):
    final_step = 5
    ...
    for i in range(len(job_control)):
        ...
        if curr_step == 0 and job_control[i]['status'][curr_step+1] == 'unstarted':
            if dict_of_simulation['skip_QC']=='True':
                ...
                print('curr_step',curr_step,check_slurm_status(job_control[i]['QC_jobids']), job_control[i]['status'][curr_step])
...
def write_to_output(i, job_control, dict_of_simulation, str) :
