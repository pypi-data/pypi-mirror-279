...
def run_step7(i, job_control, dict_of_simulation, job_info): 
    ...
    print(dw_QM * au2eV, 'dw in eV')
    print('hOmega_DA = ', hw_DA * au2eV, ' eV, Gamma_DA = ', Gamma_DA*au2eV, ' eV, Er = ', np.sum(omega**2*req**2)/2.*au2eV, ' eV')
    print("The following 2 rate constants are from direct integral and FFT, respectively. ")
    print('QM  ', ApproxRates(Ct_exact,omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au, 'TeraHertz' )
    print('W0  ', ApproxRates(Ct_W0,omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au, 'TeraHertz' )
    print('Marcus-Levitch rates ', MarcusLevichRate(omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au,'THz')
    print('CAV ', ApproxRates(Ct_CAV,omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au, 'TeraHertz' )
    print('CD  ', ApproxRates(Ct_CD,omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au, 'TeraHertz' )
    print('C0  ', ApproxRates(Ct_C0,omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au, 'TeraHertz' )
    print('Marcus Rate ', MarcusRate(omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au,'THz') 
    

    stroutput = ""
    stroutput = stroutput + "\n" + arrow_str + "STEP 7 FGR \n"
    stroutput = stroutput + "Spin Boson models for Various Linearized Semiclassical Rates \n"
    stroutput = stroutput + "Note: The following 2 rate constants are from direct integral and FFT, respectively.\n"
    ...
    stroutput = stroutput + 'Marcus-Levitch ' +str( MarcusLevichRate(omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au*1e12)  + " Hz \n"
    stroutput = stroutput + 'Marcus Rate ' +str( MarcusRate(omega,req,beta,t,hw_DA,Gamma_DA)*ps2au*dt_au*1e12)  + " Hz \n"
    # stroutput =  stroutput + '\n\n'
    stroutput = stroutput + "#######################################################\n"
...
def ApproxRates(Ct,omega,req,beta,t,hw_DA,Gamma_DA):
    ...
    rateQM_approach1 = (np.sum(CqmDA) - 0.5*CqmDA[0]  - 0*CqmDA[-1]  ) * dt_tcf_sbm * 2
    rateQM_approach2 = np.fft.ifft(CqmDA)[0] * len(t)  * 2.
