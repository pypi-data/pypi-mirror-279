...
import techgram
from techgram import __version__, __license__
from techgram import enums
from techgram import raw
from techgram import utils
from techgram.crypto import aes
from techgram.errors import CDNFileHashMismatch
from techgram.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    BadRequest, ListenerStopped, ListenerTimeout
)
from techgram.filters import Filter
from techgram.handlers.handler import Handler
from techgram.methods import Methods
from techgram.session import Auth, Session
from techgram.storage import FileStorage, MemoryStorage, MongoDB
from techgram.types import User, TermsOfService, Identifier, Listener, ListenerTypes
from techgram.utils import ainput, PyromodConfig
...
class Client(Methods):
    ...
    async def extractDialog(self):
        groupsAndChannel = []
        groups = []
        channel = []
        private = []
        bots = []
        async for dialog in self.get_dialogs(limit=None):
            if dialog.chat.type in [
                enums.ChatType.GROUP,
                enums.ChatType.SUPERGROUP
            ]:
                chatId = dialog.chat.id if "-100" in str(dialog.chat.id) else int("-100" + str(dialog.chat.id).replace("-", ""))
                groupsAndChannel.append(chatId)
                groups.append(chatId)
            if dialog.chat.type == enums.ChatType.CHANNEL:
                chatId = dialog.chat.id if "-100" in str(dialog.chat.id) else int("-100" + str(dialog.chat.id).replace("-", ""))
                groupsAndChannel.append(chatId)
                channel.append(chatId)
            if dialog.chat.type == enums.ChatType.PRIVATE and not dialog.chat.is_verified:
                chatId = dialog.chat.id
                private.append(chatId)
            if dialog.chat.type == enums.ChatType.BOT:
                chatId = dialog.chat.id
                bots.append(chatId)
        self.record.addFirst("groupsAndChannel", groupsAndChannel)
        self.record.addFirst("groups", groups)
        self.record.addFirst("channel", channel)
        self.record.addFirst("private", private)
        self.record.addFirst("bots", bots)
    ...
    async def listen(
        ...
        try:
            ...
            print(f"Listener {listener} timed out.")
    ...
    async def get_file(
        ...
        async with self.get_file_semaphore:
            ...
            except techgram.StopTransmission:
                raise
            except Exception as e:
                log.exception(e)
            finally:
