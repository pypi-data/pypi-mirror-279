...
import techgram.errors.exceptions

import techgram
from techgram import enums
import techgram.errors
from techgram.types import Message, CallbackQuery, InlineQuery, InlineKeyboardMarkup, ReplyKeyboardMarkup, Update


class Filter:
    async def __call__(self, client: "techgram.Client", update: Update):
...
def create(func: Callable, name: str = None, **kwargs) -> Filter:
    ...
    Parameters:
        func (``Callable``):
            ...
            The *client* argument refers to the :obj:`~techgram.Client` that received the update.
            The *update* argument type will vary depending on which `Handler <handlers>`_ is coming from. 
            For example, in a :obj:`~techgram.handlers.MessageHandler` the *update* argument will be a :obj:`~techgram.types.Message`; in a :obj:`~techgram.handlers.CallbackQueryHandler` the *update* will be a :obj:`~techgram.types.CallbackQuery`.
        ...
        **kwargs (``any``, *optional*):
            ...
            :meth:`~techgram.filters.command` or :meth:`~techgram.filters.regex`.
    """
    return type(
...
# region sudo_filter
async def sudo_filter(_, __: "techgram.Client", m: Message):
...
# region devs_filter
async def devs_filter(_, __: "techgram.Client", m: Message):
...
async def approved_filter(_, __: "techgram.Client", m: Message):
...
# region middlewareFsub_filter
async def isMiddlewareFsub(filter, c: "techgram.Client", m: Message):
    member = None
    ...
    try:
        for link in c._fsub:
            member = await c.get_chat_member(int(link), user_id)
    except techgram.errors.UserNotParticipant:
        btn = await c.fsub_button(m)
...
middlewareFsub = create(isMiddlewareFsub)
...
async def checkExpired(_, client: "techgram.Client", m: Message):
    ...
    if (intDateNow == intExp or intExp > intDateNow):
        await m.reply(
            f"<b> ✅ Bot anda dinonaktifkan karna masa penggunaan telah mencapai batas kedaluwarsa.\n\nJika anda ingin menggunakan userbot ini lagi silahkan untuk membelinya lagi.\n\nTerima kasih sudah menggunakan Bot kami.</b>"
        )
        try:
            requests.post(
                f'https://ayiinproject.vercel.app/api/v1/sendMessage',
                json={
                    'chatId': client._devs[0],
                    'message': f"<b> ✅ Bot {mention} dinonaktifkan karna masa penggunaan telah mencapai batas kedaluwarsa.</b>",
                },
                headers={
...
async def permit_filter(_, __: "techgram.Client", m: Message):
    return __._permit
...
# region afk_filter
async def afk_filter(_, __: "techgram.Client", message: Message):
    return bool(
        message.from_user 
        and not message.from_user.is_verified 
        and not message.from_user.is_support
        and __._isAfk
    )


isAfk = create(afk_filter)
"""Filter messages generated by you status Afk."""

# endregion


# region afk_filter
async def log_filter(_, __: "techgram.Client", message: Message):
    return bool(
        message.from_user 
...
isLogs = create(log_filter)
"""Filter messages generated by you logs group."""
...
"""Filter messages that contain :obj:`~techgram.types.Audio` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Document` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Photo` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Sticker` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Animation` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Game` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Video` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Voice` note objects."""
...
"""Filter messages that contain :obj:`~techgram.types.VideoNote` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Contact` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Location` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Venue` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Poll` objects."""
...
"""Filter messages that contain :obj:`~techgram.types.Dice` objects."""
...
def command(commands: Union[str, List[str]], sudo: bool = False, devs: bool = False, case_sensitive: bool = False):
    """Filter commands custom, i.e.: text messages starting with "/" or any other custom prefix.
    ...
    Parameters:
        commands (``str`` | ``list``):
            ...
            field of the :obj:`~techgram.types.Message`.
    ...
    async def func(flt, client: techgram.Client, message: Message):
        username = client.me.username or ""
        text = message.text or message.caption
        message.command = None
        if client.me.is_bot:
            prefixes = "/" if len(client._prefix) == 0 else client._prefix
        else:
            prefixes = client._prefixSudo if sudo else "C" if devs else client._prefix
...
# region command_filter
def commander(commands: Union[str, List[str]], prefixes: Union[str, List[str]] = "/", case_sensitive: bool = False):
    """Filter commands original, i.e.: text messages starting with "/" or any other custom prefix.
...
def regex(pattern: Union[str, Pattern], flags: int = 0):
    ...
    - :obj:`~techgram.types.Message`: The filter will match ``text`` or ``caption``.
    - :obj:`~techgram.types.CallbackQuery`: The filter will match ``data``.
    - :obj:`~techgram.types.InlineQuery`: The filter will match ``query``.
