...
from .addons import Utils, Signal, Debugger, lunarlist, Database, Cooldown
from ..basetypes import Message, Channel, User, Interaction, Context, Reaction, Guild, Member, Snowflake, Role
...
class Gateway:
    ...
    async def _handleVoiceServerUpdate(self, data: dict):
        ...
        self.voiceGateway = VoiceGateway(guild, endpoint, token, self)
        await self.voiceGateway.start()
...
class VoiceGateway:
    ...
    def __init__(self, guild: Guild, endpoint: str, token: str, parent: Gateway):
        ...
        self.session: GatewaySession = self.parent.session
