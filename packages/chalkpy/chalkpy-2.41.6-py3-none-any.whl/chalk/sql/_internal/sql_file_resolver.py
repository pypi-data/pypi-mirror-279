        ...
        for variable, output in parsed.fields.items():
            if output.endswith("*"):
                ...
                for feature in features:
                    ...
                    ) or (feature.is_feature_time and not feature.is_autogenerated):
    ...
    if source_name not in _SOURCES:  # actual name of source
        ...
        for possible_source in sources:
            ...
            if source_type is not None and isinstance(possible_source, source_type):
                # verifies whether the name of the datasource is the same as the
                # data source, if so, we should return the datasource even if
