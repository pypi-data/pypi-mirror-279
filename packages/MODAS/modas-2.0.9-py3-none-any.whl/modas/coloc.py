...
def get_kin_info(qtl, gwas_dir, geno, pvalue):
    ...
    for phe_name in qtl.phe_name.unique():
        ...
        geno_sub = geno.sel(variant=var.reindex(gwas.loc[gwas.p_wald <= pvalue, 'rs']).values, drop=True)
...
def trait_coloc(kin_info, qtl, metric, eps, p):
    ...
    coloc_pairwise_res['coloc'] = 0
    coloc_pairwise_res.loc[coloc_pairwise_res['image_match_score'] <= 0.2, 'coloc'] = 1
