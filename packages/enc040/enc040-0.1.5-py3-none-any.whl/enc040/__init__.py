...
class Encoder:
    ...
    press_start_time = None  # To store when the button press started
    long_press_triggered = False  # Flag to prevent multiple callbacks for long press

    longThreshold = 1000  # Example: threshold in milliseconds for a long press
    ...
    def _switch_release(self):
        ...
        if self.press_start_time is not None:
            press_duration = (time() * 1000) - self.press_start_time
            if press_duration >= self.longThreshold and not self.long_press_triggered:
                self.long_press_triggered = True
                self.long_press_callback(press_duration)
