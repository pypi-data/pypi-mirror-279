...
class RDAEVQ(CompressionModel):
    def __init__(self, N=128, latent_dim=16, channels=3):
        super().__init__()
        self.entropy_bottleneck = EntropyBottleneck(latent_dim)
        self.device = torch.device('cpu')
        self.encode = nn.Sequential(
            nn.AvgPool2d(stride=2, kernel_size=2),
            nn.Conv2d(channels, 5, stride=2, kernel_size=5, padding=2),
            GDN1(5),
            nn.Conv2d(5, 9, stride=2, kernel_size=5, padding=2),
            GDN1(9),
            nn.Conv2d(9, latent_dim, stride=2, kernel_size=5, padding=2),
        )
        self.decode = nn.Sequential(
