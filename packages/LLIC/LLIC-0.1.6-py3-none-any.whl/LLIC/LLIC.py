...
class RDAEVQ(CompressionModel):
    def __init__(self, N=128, latent_dim=8, channels=3):
        super().__init__()
        ...
        self.encode = nn.Sequential(
            nn.AvgPool2d(stride=2, kernel_size=2),
            nn.Conv2d(channels, 4, stride=2, kernel_size=5, padding=2),
            GDN1(4),
            nn.Conv2d(4, 6, stride=2, kernel_size=5, padding=2),
            GDN1(6),
            nn.Conv2d(6, latent_dim, stride=2, kernel_size=5, padding=2),
        )
        self.decode = nn.Sequential(
