...
class CreatePaymentScheduleItemsRequestItems(BaseModel):
    """
    ...
    run_hour: Optional[StrictInt] = Field(None, alias="runHour", description="At which hour of the day in the tenant’s timezone this payment will be collected. Available values:`[0,1,2,~,22,23]`. If the payment `runHour` and `scheduledDate` are backdated, the system will collect the payment when the next runHour occurs. The default value is `0`. ")
    scheduled_date: date = Field(..., alias="scheduledDate", description="The date to collect the payment. ")
    additional_properties: Dict[str, Any] = {}
    __properties = ["amount", "currency", "description", "paymentGatewayId", "paymentMethodId", "paymentOption", "runHour", "scheduledDate"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreatePaymentScheduleItemsRequestItems:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreatePaymentScheduleItemsRequestItems:
        """Create an instance of CreatePaymentScheduleItemsRequestItems from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreatePaymentScheduleItemsRequestItems.parse_obj(obj)

        _obj = CreatePaymentScheduleItemsRequestItems.parse_obj({
