...
class PostBusinessRegionsRequest(BaseModel):
    """
    PostBusinessRegionsRequest
    """
    country: StrictStr = Field(..., description="The short name of a country or region where you must comply with e-invoicing requirements. For example, `IN` for India. For the full list of country names and codes, see <a href=\"https://knowledgecenter.zuora.com/Quick_References/Country%2C_State%2C_and_Province_Codes/A_Country_Names_and_Their_ISO_Codes\" target=\"_blank\">ISO Standard Country Codes</a>. ")
    business_name: constr(strict=True, max_length=255) = Field(..., alias="businessName", description="The full official name that the Seller is registered with the relevant legal authority. ")
    ...
    credit_note_enabled: Optional[StrictBool] = Field(None, alias="creditNoteEnabled", description="It will control that whether the credit note should be supported by the process type or not. ")
    debit_note_enabled: Optional[StrictBool] = Field(None, alias="debitNoteEnabled", description="It will control that whether the debit note should be supported by the process type or not. ")
    __properties = ["country", "businessName", "businessNumber", "businessNumberSchemaId", "tradeName", "taxRegisterNumber", "endpointId", "endpointSchemeId", "addressLine1", "addressLine2", "postalCode", "city", "state", "contactName", "email", "phoneNumber", "serviceProviderId", "digitalSignatureEnable", "digitalSignatureBoxEnable", "digitalSignatureBoxPosX", "digitalSignatureBoxPosY", "responseMapping", "processType", "invoiceEnabled", "creditNoteEnabled", "debitNoteEnabled"]
    ...
    def process_type_validate_enum(cls, value):
        ...
        if value not in ('Clearance', 'Cacellation', 'PostAudit', 'PEPPOL'):
            raise ValueError("must be one of enum values ('Clearance', 'Cacellation', 'PostAudit', 'PEPPOL')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PostBusinessRegionsRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PostBusinessRegionsRequest:
        """Create an instance of PostBusinessRegionsRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PostBusinessRegionsRequest.parse_obj(obj)

        _obj = PostBusinessRegionsRequest.parse_obj({
            ...
            "credit_note_enabled": obj.get("creditNoteEnabled"),
            "debit_note_enabled": obj.get("debitNoteEnabled")
        })
        return _obj
