...
class PreviewOrderResponse(BaseModel):
    ...
    preview_result: Optional[PreviewOrderResult] = Field(None, alias="previewResult")
    __properties = ["processId", "requestId", "reasons", "success", "previewResult"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PreviewOrderResponse:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of preview_result
        if self.preview_result:
            _dict['previewResult'] = self.preview_result.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> PreviewOrderResponse:
        """Create an instance of PreviewOrderResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PreviewOrderResponse.parse_obj(obj)

        _obj = PreviewOrderResponse.parse_obj({
            ...
            "preview_result": PreviewOrderResult.from_dict(obj.get("previewResult")) if obj.get("previewResult") is not None else None
        })
        return _obj
