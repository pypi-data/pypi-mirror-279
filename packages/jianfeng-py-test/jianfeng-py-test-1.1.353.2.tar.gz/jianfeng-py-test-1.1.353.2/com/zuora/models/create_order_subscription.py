...
class CreateOrderSubscription(BaseModel):
    ...
    order_actions: Optional[conlist(CreateOrderAction)] = Field(None, alias="orderActions", description="The actions to be applied to the subscription. Order actions will be stored with the sequence when it was provided in the request.")
    quote: Optional[QuoteObjectFields] = None
    ramp: Optional[CreateRamp] = None
    subscription_number: Optional[StrictStr] = Field(None, alias="subscriptionNumber", description="Leave this empty to represent new subscription creation. Specify a subscription number to update an existing subscription. ")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateOrderSubscription:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateOrderSubscription:
        """Create an instance of CreateOrderSubscription from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateOrderSubscription.parse_obj(obj)

        _obj = CreateOrderSubscription.parse_obj({
            "custom_fields": obj.get("customFields"),
            "order_actions": [CreateOrderAction.from_dict(_item) for _item in obj.get("orderActions")] if obj.get("orderActions") is not None else None,
