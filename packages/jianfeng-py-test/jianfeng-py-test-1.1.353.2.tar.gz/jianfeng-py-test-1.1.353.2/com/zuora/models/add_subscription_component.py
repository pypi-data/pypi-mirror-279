...
class AddSubscriptionComponent(BaseModel):
    ...
    """
    amended_by_order_on: Optional[StrictStr] = Field(None, alias="amendedByOrderOn", description="The date when the rate plan charge is amended through an order or amendment. This field is not updatable.  This field is to standardize the booking date information to increase audit ability and traceability of data between Zuora Billing and Zuora Revenue. It is mapped as the booking date for a sale order line in Zuora Revenue. ")
    apply_discount_to: Optional[StrictStr] = Field(None, alias="applyDiscountTo", description="Specifies the type of charges that you want a specific discount to apply to.  Values:  * `ONETIME` * `RECURRING` * `USAGE` * `ONETIMERECURRING` * `ONETIMEUSAGE` * `RECURRINGUSAGE` * `ONETIMERECURRINGUSAGE`  Available for the following charge type for the Discount-Fixed Amount and Discount-Percentage charge models:  * Recurring ")
    bill_cycle_day: Optional[StrictStr] = Field(None, alias="billCycleDay", description="Sets the bill cycle day (BCD) for the charge. The BCD determines which day of the month customer is billed.  Values: `1`-`31`  Available for the following charge types:  * Recurring * Usage-based ")
    ...
    discount_amount: Optional[Union[StrictFloat, StrictInt]] = Field(None, alias="discountAmount", description="Specifies the amount of fixed-amount discount.  Available for the following charge type for the Discount-Fixed Amount charge model:  * Recurring ")
    discount_level: Optional[StrictStr] = Field(None, alias="discountLevel", description="Specifies if the discount applies to the product rate plan only , the entire subscription, or to any activity in the account.  Values:  * `rateplan` * `subscription` * `account`  Available for the following charge type for the Discount-Fixed Amount and Discount-Percentage charge models:  * Recurring ")
    discount_percentage: Optional[Union[StrictFloat, StrictInt]] = Field(None, alias="discountPercentage", description="Specifies the percentage of a percentage discount.   Available for the following charge type for the Discount-Percentage charge model:  * Recurring ")
    ...
    list_price_base: Optional[StrictStr] = Field(None, alias="listPriceBase", description="The list price base for the product rate plan charge.  Values:  * `Per_Billing_Period` * `Per_Month` * `Per_Week` * `Per_Year` * `Per_Specific_Months`  Available for the following charge type for the Flat Fee, Per Unit, Volume Pricing, and Tiered Pricing charge models:  * Recurring ")
    number: Optional[StrictStr] = Field(None, description="Unique number that identifies the charge. System-generated if not provided. ")
    number_of_periods: Optional[StrictInt] = Field(None, alias="numberOfPeriods", description="Specifies the number of periods to use when calculating charges in an overage smoothing charge model.  Available for the following charge type for the Overage and Tiered with Overage charge models:  * Usage-based ")
    ...
    overage_unused_units_credit_option: Optional[StrictStr] = Field(None, alias="overageUnusedUnitsCreditOption", description="Determines whether to credit the customer with unused units of usage.  Values:  * `NoCredit` * `CreditBySpecificRate`  Available for the following charge type for the Overage and Tiered with Overage charge models:  * Usage-based ")
    ...
    price_change_option: Optional[StrictStr] = Field(None, alias="priceChangeOption", description="Applies an automatic price change when a termed subscription is renewed. The Zuora Billing Admin setting Enable Automatic Price Change When Subscriptions are Renewed?  must be set to Yes to use this field.  See Define Default Subscription Settings for more information on setting this option.  Values:  * `NoChange` (default) * `SpecificPercentageValue` * `UseLatestProductCatalogPricing`  Available for the following charge types:  * Recurring * Usage-based * Not available for the Fixed-Amount Discount charge model. ")
    ...
    quantity: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Number of units. Must be >=`0`.  Available for the following charge types for the Per Unit, Volume Pricing, and Tiered Pricing charge models:  * One-time * Recurring ")
    ...
    specific_billing_period: Optional[StrictInt] = Field(None, alias="specificBillingPeriod", description="Specifies the number of month or week for the charges billing period. Required if you set the value of the `billingPeriod` field to `Specific_Months` or `Specific_Weeks`.  Available for the following charge types:  * Recurring * Usage-based ")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AddSubscriptionComponent:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AddSubscriptionComponent:
        """Create an instance of AddSubscriptionComponent from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AddSubscriptionComponent.parse_obj(obj)

        _obj = AddSubscriptionComponent.parse_obj({
