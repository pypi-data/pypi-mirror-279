...
class BulkPdfGenerationJobResponse(BaseModel):
    """
    ...
    job_id: Optional[StrictStr] = Field(None, alias="jobId", description="Unique Id for the Job Triggered. ")
    invalid_ids: Optional[conlist(StrictStr)] = Field(None, alias="invalidIds", description="Collection of Ids that are not valid.    Id is considered to be invalid if,      * Billing Document Id doesn't exist in the database for the corresponding Billing Document Type   * generateMissingPDF property is false in the Job Request and Valid PDF doesn't exist for the Billing Document Id ")
    __properties = ["processId", "requestId", "reasons", "success", "jobId", "invalidIds"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BulkPdfGenerationJobResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> BulkPdfGenerationJobResponse:
        """Create an instance of BulkPdfGenerationJobResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BulkPdfGenerationJobResponse.parse_obj(obj)

        _obj = BulkPdfGenerationJobResponse.parse_obj({
            ...
            "invalid_ids": obj.get("invalidIds")
        })
        return _obj
