...
class CancelSubscriptionRequest(BaseModel):
    ...
    booking_date: Optional[date] = Field(None, alias="bookingDate", description="The booking date that you want to set for the amendment contract when you cancel the subscription. The default value is the current date when you make the API call. ")
    cancellation_effective_date: Optional[date] = Field(None, alias="cancellationEffectiveDate", description="Date the cancellation takes effect, in the format yyyy-mm-dd.  Use only if `cancellationPolicy` is `SpecificDate`. Should not be earlier than the subscription contract-effective date, later than the subscription term-end date, or within a period for which the customer has been invoiced. ")
    cancellation_policy: StrictStr = Field(..., alias="cancellationPolicy", description="Cancellation method. Possible values are: `EndOfCurrentTerm`, `EndOfLastInvoicePeriod`, `SpecificDate`. If using `SpecificDate`, the `cancellationEffectiveDate` field is required. ")
    ...
    contract_effective_date: Optional[date] = Field(None, alias="contractEffectiveDate", description="The date when the customer notifies you that they want to cancel their subscription. ")
    ...
    __properties = ["applicationOrder", "applyCredit", "applyCreditBalance", "bookingDate", "cancellationEffectiveDate", "cancellationPolicy", "collect", "contractEffectiveDate", "creditMemoReasonCode", "documentDate", "invoice", "invoiceCollect", "invoiceTargetDate", "orderDate", "runBilling", "targetDate"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CancelSubscriptionRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CancelSubscriptionRequest:
        """Create an instance of CancelSubscriptionRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CancelSubscriptionRequest.parse_obj(obj)

        _obj = CancelSubscriptionRequest.parse_obj({
            ...
            "cancellation_effective_date": obj.get("cancellationEffectiveDate"),
            "cancellation_policy": obj.get("cancellationPolicy"),
