...
class GetMetricsBySubscriptionNumbersResponse(BaseModel):
    """
    ...
    subscription_metrics: Optional[conlist(SubscriptionMetrics)] = Field(None, alias="subscriptionMetrics")
    __properties = ["processId", "requestId", "reasons", "success", "subscriptionMetrics"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetMetricsBySubscriptionNumbersResponse:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in subscription_metrics (list)
        _items = []
        if self.subscription_metrics:
            for _item in self.subscription_metrics:
                if _item:
                    _items.append(_item.to_dict())
            _dict['subscriptionMetrics'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> GetMetricsBySubscriptionNumbersResponse:
        """Create an instance of GetMetricsBySubscriptionNumbersResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetMetricsBySubscriptionNumbersResponse.parse_obj(obj)

        _obj = GetMetricsBySubscriptionNumbersResponse.parse_obj({
            ...
            "subscription_metrics": [SubscriptionMetrics.from_dict(_item) for _item in obj.get("subscriptionMetrics")] if obj.get("subscriptionMetrics") is not None else None
        })
        return _obj
