...
class CreateCreditMemoFromInvoiceRequest(BaseModel):
    ...
    exclude_from_auto_apply_rules: Optional[StrictBool] = Field(None, alias="excludeFromAutoApplyRules", description="Whether the credit memo is excluded from the rule of automatically applying credit memos to invoices.")
    invoice_id: Optional[StrictStr] = Field(None, alias="invoiceId", description="The ID of the invoice that the credit memo is created from. * If this field is specified, its value must be the same as the value of the `invoiceId` path parameter. Otherwise, its value overrides the value of the `invoiceId` path parameter.  * If this field is not specified, the value of the `invoiceId` path parameter is used. ")
    items: Optional[conlist(CreditMemoItemFromInvoiceItem, max_items=1000)] = Field(None, description="Container for items. The maximum number of items is 1,000.")
    ...
    tax_auto_calculation: Optional[StrictBool] = Field(True, alias="taxAutoCalculation", description="Whether to automatically calculate taxes in the credit memo.")
    additional_properties: Dict[str, Any] = {}
    __properties = ["IntegrationId__NS", "IntegrationStatus__NS", "Origin__NS", "SyncDate__NS", "Transaction__NS", "autoApplyToInvoiceUponPosting", "autoPost", "comment", "effectiveDate", "excludeFromAutoApplyRules", "invoiceId", "items", "reasonCode", "taxAutoCalculation"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateCreditMemoFromInvoiceRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateCreditMemoFromInvoiceRequest:
        """Create an instance of CreateCreditMemoFromInvoiceRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateCreditMemoFromInvoiceRequest.parse_obj(obj)

        _obj = CreateCreditMemoFromInvoiceRequest.parse_obj({
            ...
            "auto_apply_to_invoice_upon_posting": obj.get("autoApplyToInvoiceUponPosting"),
            ...
            "items": [CreditMemoItemFromInvoiceItem.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
