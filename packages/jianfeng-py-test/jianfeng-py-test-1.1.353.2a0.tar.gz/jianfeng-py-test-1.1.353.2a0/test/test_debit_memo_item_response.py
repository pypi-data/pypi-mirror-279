# coding: utf-8

"""
    Zuora API Reference

    REST API reference for the Zuora Billing, Payments, and Central Platform! Check out the [REST API Overview](https://www.zuora.com/developer/api-references/api/overview/).

    The version of the OpenAPI document: 2023-10-24
    Contact: docs@zuora.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from zuora_sdk.models.debit_memo_item_response import DebitMemoItemResponse

class TestDebitMemoItemResponse(unittest.TestCase):
    """DebitMemoItemResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> DebitMemoItemResponse:
        """Test DebitMemoItemResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `DebitMemoItemResponse`
        """
        model = DebitMemoItemResponse()
        if include_optional:
            return DebitMemoItemResponse(
                process_id = '',
                request_id = '',
                reasons = [
                    {"code":"code","message":"message"}
                    ],
                success = True,
                amount = 1.337,
                amount_without_tax = 1.337,
                applied_to_item_id = '',
                balance = 1.337,
                be_applied_amount = 1.337,
                comment = '',
                created_by_id = '',
                created_date = '',
                description = '',
                exclude_item_billing_from_revenue_accounting = True,
                finance_information = zuora_sdk.models.get_debit_memo_item_finance_information.GetDebitMemoItemFinanceInformation(
                    deferred_revenue_accounting_code = '', 
                    deferred_revenue_accounting_code_type = '', 
                    recognized_revenue_accounting_code = '', 
                    recognized_revenue_accounting_code_type = '', 
                    revenue_recognition_rule_name = '', 
                    revenue_schedule_number = '', ),
                id = '',
                processing_type = 'Charge',
                quantity = 1.337,
                reflect_discount_in_net_amount = True,
                service_end_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(),
                service_start_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(),
                sku = '',
                sku_name = '',
                sold_to_contact_id = '',
                sold_to_contact_snapshot_id = '',
                source_item_id = '',
                source_item_type = 'SubscriptionComponent',
                subscription_id = '',
                tax_items = [
                    { }
                    ],
                tax_mode = 'TaxInclusive',
                taxation_items = zuora_sdk.models.debit_memo_item_taxation_items.DebitMemoItemTaxationItems(
                    data = [
                        { }
                        ], 
                    next_page = '', ),
                unit_of_measure = '',
                unit_price = 1.337,
                updated_by_id = '',
                updated_date = ''
            )
        else:
            return DebitMemoItemResponse(
        )
        """

    def testDebitMemoItemResponse(self):
        """Test DebitMemoItemResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
