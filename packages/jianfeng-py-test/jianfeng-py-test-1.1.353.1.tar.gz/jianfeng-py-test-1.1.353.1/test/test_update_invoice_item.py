# coding: utf-8

"""
    Zuora API Reference

    REST API reference for the Zuora Billing, Payments, and Central Platform! Check out the [REST API Overview](https://www.zuora.com/developer/api-references/api/overview/).

    The version of the OpenAPI document: 2023-10-24
    Contact: docs@zuora.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from com.zuora.models.update_invoice_item import UpdateInvoiceItem  # noqa: E501

class TestUpdateInvoiceItem(unittest.TestCase):
    """UpdateInvoiceItem unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> UpdateInvoiceItem:
        """Test UpdateInvoiceItem
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `UpdateInvoiceItem`
        """
        model = UpdateInvoiceItem()  # noqa: E501
        if include_optional:
            return UpdateInvoiceItem(
                integration_id__ns = '',
                integration_status__ns = '',
                sync_date__ns = '',
                accounting_code = '',
                adjustment_liability_accounting_code = '',
                adjustment_revenue_accounting_code = '',
                amount = 1.337,
                charge_date = '',
                charge_name = '',
                contract_asset_accounting_code = '',
                contract_liability_accounting_code = '',
                contract_recognized_revenue_accounting_code = '',
                deferred_revenue_accounting_code = '',
                description = '',
                discount_items = [
                    { }
                    ],
                exclude_item_billing_from_revenue_accounting = True,
                id = '',
                item_type = '',
                purchase_order_number = '',
                quantity = 1.337,
                recognized_revenue_accounting_code = '',
                rev_rec_code = '',
                rev_rec_trigger_condition = 'ContractEffectiveDate',
                revenue_recognition_rule_name = '',
                service_end_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(),
                service_start_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(),
                sku = '',
                tax_code = '',
                tax_mode = 'TaxInclusive',
                unbilled_receivables_accounting_code = '',
                unit_price = 1.337,
                uom = ''
            )
        else:
            return UpdateInvoiceItem(
        )
        """

    def testUpdateInvoiceItem(self):
        """Test UpdateInvoiceItem"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
