...
class UpdateSubscriptionRatePlan(BaseModel):
    ...
    charge_update_details: Optional[conlist(UpdateRatePlanCharge)] = Field(None, alias="chargeUpdateDetails", description="Container for one or more product rate plan charges.  ")
    ...
    service_activation_date: Optional[date] = Field(None, alias="serviceActivationDate", description="The date when the update amendment is activated in yyyy-mm-dd format.  You must specify a Service Activation date if the Customer Acceptance date is set. If the Customer Acceptance date is not set, the value of the `serviceActivationDate` field defaults to be the Contract Effective Date.  The billing trigger dates must follow this rule:  contractEffectiveDate <= serviceActivationDate <= contractAcceptanceDate ")
    ...
    __properties = ["bookingDate", "chargeUpdateDetails", "contractEffectiveDate", "customerAcceptanceDate", "externalCatalogPlanId", "externalIdSourceSystem", "externallyManagedPlanId", "productRatePlanNumber", "ratePlanId", "serviceActivationDate", "specificUpdateDate", "subscriptionRatePlanNumber"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateSubscriptionRatePlan:
    ...
    def to_dict(self):
        ...
        if self.charge_update_details:
            for _item in self.charge_update_details:
                if _item:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpdateSubscriptionRatePlan:
        """Create an instance of UpdateSubscriptionRatePlan from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpdateSubscriptionRatePlan.parse_obj(obj)

        _obj = UpdateSubscriptionRatePlan.parse_obj({
            "booking_date": obj.get("bookingDate"),
            "charge_update_details": [UpdateRatePlanCharge.from_dict(_item) for _item in obj.get("chargeUpdateDetails")] if obj.get("chargeUpdateDetails") is not None else None,
