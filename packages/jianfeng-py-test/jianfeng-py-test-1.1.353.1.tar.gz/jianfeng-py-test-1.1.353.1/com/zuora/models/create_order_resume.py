...
class CreateOrderResume(BaseModel):
    ...
    """
    extends_term: Optional[StrictBool] = Field(None, alias="extendsTerm", description="Specifies whether to extend the subscription term by the length of time the suspension is in effect. ")
    ...
    resume_specific_date: Optional[date] = Field(None, alias="resumeSpecificDate", description="This field is applicable only when the `resumePolicy` field is set to `SpecificDate`.  A specific date when the subscription resumption takes effect, in YYYY-MM-DD format. The value should not be earlier than the subscription suspension date. ")
    __properties = ["extendsTerm", "resumePeriods", "resumePeriodsType", "resumePolicy", "resumeSpecificDate"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateOrderResume:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateOrderResume:
        """Create an instance of CreateOrderResume from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateOrderResume.parse_obj(obj)

        _obj = CreateOrderResume.parse_obj({
