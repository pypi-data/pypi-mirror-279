...
class CreateOrderOrderLineItem(BaseModel):
    ...
    item_number: Optional[StrictStr] = Field(default=None, description="The number of the Order Line Item. Use this field to specify a custom item number for your Order Line Item. If you are to use this field,  you must set all the item numbers in an order when there are several order line items in the order. ", alias="itemNumber")
    ...
    owner_account_number: Optional[StrictStr] = Field(default=None, description="Use this field to assign an existing account as the owner of an order line item. ", alias="ownerAccountNumber")
