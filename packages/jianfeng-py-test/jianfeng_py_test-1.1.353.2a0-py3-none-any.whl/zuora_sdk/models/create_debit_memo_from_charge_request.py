...
class CreateDebitMemoFromChargeRequest(BaseModel):
    """
    CreateDebitMemoFromChargeRequest
    """ # noqa: E501
    account_id: Optional[StrictStr] = Field(default=None, description="The ID of the account associated with the debit memo.  **Note**: When creating debit memos from product rate plan charges, you must specify `accountNumber`, `accountId`, or both in the request body. If both fields are specified, they must correspond to the same account.", alias="accountId")
    account_number: Optional[StrictStr] = Field(default=None, description="The number of the account associated with the debit memo.  **Note**: When creating debit memos from product rate plan charges, you must specify `accountNumber`, `accountId`, or both in the request body. If both fields are specified, they must correspond to the same account.", alias="accountNumber")
    ...
    auto_post: Optional[StrictBool] = Field(default=False, description="Whether to automatically post the debit memo after it is created.   Setting this field to `true`, you do not need to separately call the [Post a debit memo](https://www.zuora.com/developer/api-references/api/operation/Put_PostDebitMemo) operation to post the debit memo. ", alias="autoPost")
    charges: Optional[Annotated[List[DebitMemoItemFromChargeDetail], Field(max_length=1000)]] = Field(default=None, description="Container for product rate plan charges. The maximum number of items is 1,000.")
    ...
    __properties: ClassVar[List[str]] = ["accountId", "accountNumber", "autoPay", "autoPost", "charges", "comment", "customRates", "dueDate", "effectiveDate", "reasonCode", "currency", "number", "IntegrationId__NS", "IntegrationStatus__NS", "SyncDate__NS"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "charges": [DebitMemoItemFromChargeDetail.from_dict(_item) for _item in obj["charges"]] if obj.get("charges") is not None else None,
