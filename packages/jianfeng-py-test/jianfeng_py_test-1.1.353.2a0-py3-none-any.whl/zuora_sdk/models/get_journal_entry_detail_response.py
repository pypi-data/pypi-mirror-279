...
class GetJournalEntryDetailResponse(BaseModel):
    ...
    aggregate_currency: Optional[StrictBool] = Field(default=None, description="Returns true if the journal entry is aggregating currencies. That is, if the journal entry was created when the `Aggregate transactions with different currencies during a Journal Run` setting was configured to `Yes`. Otherwise, returns `false`. ", alias="aggregateCurrency")
    ...
    __properties: ClassVar[List[str]] = ["accountingPeriodName", "aggregateCurrency", "currency", "homeCurrency", "journalEntryDate", "journalEntryItems", "notes", "number", "organizationLabel", "segments", "status", "success", "timePeriodEnd", "timePeriodStart", "transactionType", "transferDateTime", "transferredBy", "transferredToAccounting"]
