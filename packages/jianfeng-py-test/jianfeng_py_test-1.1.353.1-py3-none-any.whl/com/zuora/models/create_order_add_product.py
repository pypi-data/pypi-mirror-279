...
class CreateOrderAddProduct(BaseModel):
    """
    A rate plan can be added to a subscription through one order action.   - For a rate plan, the following fields are available:       - `chargeOverrides`       - `clearingExistingFeatures`       - `customFields`       - `externalCatalogPlanId`       - `externallyManagedPlanId`         - `productRatePlanId`       - `productRatePlanNumber`       - `subscriptionProductFeatures`       - `uniqueToken`   # noqa: E501
    """
    ...
    rate_plan_overrides: Optional[conlist(CreateOrderProductRatePlanOverride)] = Field(None, alias="ratePlanOverrides")
    __properties = ["chargeOverrides", "clearingExistingFeatures", "customFields", "externalCatalogPlanId", "externallyManagedPlanId", "productRatePlanId", "productRatePlanNumber", "subscriptionProductFeatures", "uniqueToken", "subscriptionRatePlanNumber", "isFromExternalCatalog", "ratePlanName", "isAddingSubsetCharges", "ratePlanOverrides"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateOrderAddProduct:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in rate_plan_overrides (list)
        _items = []
        if self.rate_plan_overrides:
            for _item in self.rate_plan_overrides:
                if _item:
                    _items.append(_item.to_dict())
            _dict['ratePlanOverrides'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> CreateOrderAddProduct:
        """Create an instance of CreateOrderAddProduct from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateOrderAddProduct.parse_obj(obj)

        _obj = CreateOrderAddProduct.parse_obj({
            ...
            "rate_plan_overrides": [CreateOrderProductRatePlanOverride.from_dict(_item) for _item in obj.get("ratePlanOverrides")] if obj.get("ratePlanOverrides") is not None else None
        })
        return _obj
