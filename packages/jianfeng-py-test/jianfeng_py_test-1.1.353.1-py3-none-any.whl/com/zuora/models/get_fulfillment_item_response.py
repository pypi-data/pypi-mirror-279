...
class GetFulfillmentItemResponse(BaseModel):
    """
    ...
    fulfillment_item: Optional[GetFulfillmentItem] = Field(None, alias="fulfillmentItem")
    __properties = ["processId", "requestId", "reasons", "success", "fulfillmentItem"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetFulfillmentItemResponse:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of fulfillment_item
        if self.fulfillment_item:
            _dict['fulfillmentItem'] = self.fulfillment_item.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> GetFulfillmentItemResponse:
        """Create an instance of GetFulfillmentItemResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetFulfillmentItemResponse.parse_obj(obj)

        _obj = GetFulfillmentItemResponse.parse_obj({
            ...
            "fulfillment_item": GetFulfillmentItem.from_dict(obj.get("fulfillmentItem")) if obj.get("fulfillmentItem") is not None else None
        })
        return _obj
