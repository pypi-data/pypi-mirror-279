# coding: utf-8

"""
    Zuora API Reference

    REST API reference for the Zuora Billing, Payments, and Central Platform! Check out the [REST API Overview](https://www.zuora.com/developer/api-references/api/overview/).

    The version of the OpenAPI document: 2023-10-24
    Contact: docs@zuora.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import json
import pprint
import re  # noqa: F401
from aenum import Enum, no_arg





class GetPublicEmailTemplateResponseToEmailType(str, Enum):
    """
    Email receive type.
    """

    """
    allowed enum values
    """
    BILLTOCONTACT = 'BillToContact'
    SOLDTOCONTACT = 'SoldToContact'
    SPECIFICEMAILS = 'SpecificEmails'
    TENANTADMIN = 'TenantAdmin'
    BILLTOANDSOLDTOCONTACTS = 'BillToAndSoldToContacts'
    RUNOWNER = 'RunOwner'
    ALLCONTACTS = 'AllContacts'
    INVOICEOWNERBILLTOCONTACT = 'InvoiceOwnerBillToContact'
    INVOICEOWNERSOLDTOCONTACT = 'InvoiceOwnerSoldToContact'
    INVOICEOWNERBILLTOANDSOLDTOCONTACTS = 'InvoiceOwnerBillToAndSoldToContacts'
    INVOICEOWNERALLCONTACTS = 'InvoiceOwnerAllContacts'

    @classmethod
    def from_json(cls, json_str: str) -> GetPublicEmailTemplateResponseToEmailType:
        """Create an instance of GetPublicEmailTemplateResponseToEmailType from a JSON string"""
        return GetPublicEmailTemplateResponseToEmailType(json.loads(json_str))


