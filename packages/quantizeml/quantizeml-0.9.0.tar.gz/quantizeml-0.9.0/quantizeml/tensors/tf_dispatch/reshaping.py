...
def fp_zeros_like(input: FixedPoint, dtype=None, name=None):
    ...
    values = tf.zeros_like(input.values, dtype, name)
...
@tf.experimental.dispatch_for_api(tf.zeros_like)
def qf_zeros_like(input: QFloat, dtype=None, name=None):
    ...
    """
    # Create a tf.Tensor with all element set to zeros
    values = tf.zeros_like(input.fp, dtype, name)
