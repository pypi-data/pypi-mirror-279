...
def get_input_quantizer(graph):
    """Get a representation of the input quantizer.

    Args:
        graph (GraphProto): the graph containing the input to be quantized.

    Returns:
        Quantizer: tuple with the necessary information to quantize the input.
    """
    ...
    return Quantizer(quantizer, [], input_ts.name)
...
def quantize_calibrated(target_model, tensors_range, quantize_until=None):
    """
    ...
    Args:
        target_model: model to quantize
    ...
    """
    assert isinstance(target_model, ONNXModel)

    # Reject multi-input-output models (yet)
    if len(target_model.input) != 1 or len(target_model.output) != 1:
        raise RuntimeError("Only single input/output models are supported.")
    if quantize_until and not any(node.name == quantize_until for node in target_model.nodes()):
        raise ValueError(f"'{quantize_until}' is not a recognized node in "
                         f"{target_model.graph().name}")
    ...
    model_name = target_model.name or "quantized_model"
    qmodel = ONNXModel(onnx.helper.make_model(graph=onnx.GraphProto(name=model_name)))
    qmodel.input.extend(target_model.input)
    qmodel.set_opset_import(onnx_qlayers.DOMAIN, onnx_qlayers.VERSION)

    # Include an input quantizer at the beginning of the model
    qmodel.quantizers = [get_input_quantizer(graph)]

    # Split nodes in blocks.
    # Note: list(nodes) clone each NodeProto in graph.node field
    remaining_nodes = list(target_model.nodes())
    output_names = [qmodel.quantizers[0].out_name]
    # Start with all nodes linked to the input
    node_queue = []
    for node in model.nodes():
        if model.get_node_inputs(node) == [output_names[0]]:
    ...
    while len(node_queue) > 0:
        ...
        if block_qpattern := search_block_from_node(target_node, model, quantize_until):
            ...
            # TODO: this could be simplified if prepare_to_quantize is removed
            node_outputs = [node.output[0] for node in block_nodes]
            nodes_to_remove = []
            for node in remaining_nodes:
                if node.output[0] in node_outputs:
    ...
    partial_float_in, _ = infer_partial_io(remaining_nodes,
                                           exclude=list(target_model.get_initializer_name_set()))
    ...
    if len(remaining_nodes) > 0:
        ...
        extractor = onnx.utils.Extractor(target_model.model)
        remaining_model = extractor.extract_model(partial_float_in, [target_model.output[0].name])
