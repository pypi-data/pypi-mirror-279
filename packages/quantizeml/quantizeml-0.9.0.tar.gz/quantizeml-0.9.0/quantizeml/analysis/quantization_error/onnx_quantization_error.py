...
from ...onnx_support.quantization.weights import align_to
from ..tools import generate_onnx_random_samples, extract_submodel_from_qnode
from .common import compare, remove_outliers, merge_metrics, eval_metrics, compute_saturation
...
def cumulative_quantization_error(fmodel, qmodel, target_node=None, batch_size=16, seed=None):
    ...
    samples = [{k: v[idx:idx + 1] for k, v in samples.items()} for idx in range(batch_size)]
    ...
    for feed in samples:
        ...
        outputs = fsess.run(None, feed), qsess.run(None, feed)
        for qnode, foutputs, qoutputs in zip(qtarget_nodes, *outputs):
            ...
            qerror = compare_outputs(foutputs, qoutputs, qscales, per_channel=per_channel)
            if key in summary:
