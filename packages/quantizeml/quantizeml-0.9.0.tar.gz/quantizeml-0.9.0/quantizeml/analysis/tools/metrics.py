...
# ******************************************************************************
__all__ = ["WeightedMAPE", "Saturation", "print_metric_table"]
...
class WeightedMAPE(keras.metrics.Metric):
    """Compute the Weighted Mean Absolute Percentage Error (wMAPE) as
    `https://en.wikipedia.org/wiki/Mean_absolute_percentage_error#WMAPE`_:

    >>> sum(abs(x - y)) / sum(abs(x))

    Args:
        epsilon (float, optional): an epsilon value. Defaults to 1e-7.
    """

    def __init__(self, name="wmape", epsilon=1e-7, **kwargs):
        super().__init__(name=name, **kwargs)
        self.epsilon = epsilon
        self.sum_error = self.add_weight(name='sum_error', initializer='zeros', dtype="float64")
        self.gt_error = self.add_weight(name='gt_error', initializer='zeros', dtype="float64")

    def update_state(self, y_true, y_pred):
        self.sum_error.assign_add(np.sum(np.abs(y_true - y_pred)))
        self.gt_error.assign_add(np.sum(np.abs(y_true)))

    def result(self):
        gt_error = self.gt_error
        sum_error = self.sum_error
        if gt_error == 0:
            gt_error = self.epsilon
        return sum_error / gt_error

    def reset_states(self):
        self.sum_error.assign(0.0)
        self.gt_error.assign(0.0)
