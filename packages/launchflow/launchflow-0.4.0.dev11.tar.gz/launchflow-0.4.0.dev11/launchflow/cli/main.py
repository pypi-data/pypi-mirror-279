...
from launchflow.cli.service_utils import import_services
...
_LAUNCHFLOW_TEXT = r"""[purple]
 _                                 _      _____  _
| |                               | |    |  ___|| |
| |      __ _  _   _  _ __    ___ | |__  | |_   | |  ___ __      __
| |     / _` || | | || '_ \  / __|| '_ \ |  _|  | | / _ \\ \ /\ / /
| |____| (_| || |_| || | | || (__ | | | || |    | || (_) |\ V  V /
\_____/ \__,_| \__,_||_| |_| \___||_| |_|\_|    |_| \___/  \_/\_/ [/purple]
"""
...
async def init(
    ...
    rich.print(_LAUNCHFLOW_TEXT)
    try:
        if template is not None:
            await project_gen.generate_template_project(
                template=template, default_backend=default_backend
            )
        elif bootstrap:
...
async def deploy(
    ...
    service_refs = find_launchflow_services(scan_directory)
...
async def _connect_resources_not_found_okay(*resources: Resource) -> List[Outputs]:
    async def connect_not_found_okay(resource: Resource) -> Outputs:
        try:
...
async def run(
    ...
    try:
        ...
        local_container_ids = [
            resource.running_container_id for resource in local_resources
        ]
