...
def validate_project_name(project_name: str, raise_on_error: bool = True):
    ...
    if not re.match(PROJECT_PATTERN, project_name):
        if raise_on_error:
            raise ValueError(f"Project name must match pattern {PROJECT_PATTERN}")
        return False, f"Project name must match pattern {PROJECT_PATTERN}"
...
def validate_service_name(service_name: str):
    ...
    if " " in service_name:
        raise exceptions.InvalidResourceName("Service name cannot contain spaces")
