...
resource "aws_db_subnet_group" "default" {
  ...
  description = "Subnet group for ${var.launchflow_project}-${var.launchflow_environment}-${var.resource_name}"
}

# Define a Security Group for public access (only for public instances)
resource "aws_security_group" "rds_sg" {
  count       = var.publicly_accessible ? 1 : 0
  name        = "${var.launchflow_project}-${var.launchflow_environment}-${var.resource_name}-rds-sg"
...
resource "aws_db_instance" "default" {
  identifier_prefix      = substr("${var.resource_name}-${var.launchflow_environment}-", 0, 55)
