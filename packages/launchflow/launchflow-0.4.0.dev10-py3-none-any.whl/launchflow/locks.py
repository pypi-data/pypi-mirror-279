...
class DockerLock(Lock):
    ...
    async def acquire(self) -> LockInfo:
        return LockInfo(lock_id="docker", lock_operation=self.operation)
...
class LocalLock(Lock):
    ...
    async def force_unlock(file_path: str) -> None:
        ...
        if os.path.exists(lock_file):
            ...
            raise exceptions.EntityNotLocked(file_path)
...
class GCSLock(Lock):
    ...
    async def force_unlock(
        ...
        try:
            ...
            raise exceptions.EntityNotLocked(entity_file_path)
...
class LaunchFlowLock(Lock):
    ...
    async def force_unlock(
        ...
        access_token = launchflow_api_key or config.get_access_token()
        async with httpx.AsyncClient(timeout=60) as client:
            response = await client.post(
