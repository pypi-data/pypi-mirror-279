...
from launchflow.flows.resource_flows import create
...
async def create_environment(
    ...
    if env.status == EnvironmentStatus.READY and isinstance(
        manager.backend, LaunchFlowBackend
    ):
        # TODO: ideally this would all be combined into one workflow logger
        # but create(...) spins up it's own progress logger so we would
        # need to refactor it a bit to optionally take one in
        # Maybe when we move more things to the flow logger
        # we can do this
        ...
        if cloud_provider == CloudProvider.GCP:
            ...
            await create(manager.environment_name, resource, prompt=False)
