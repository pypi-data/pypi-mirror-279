...
from launchflow.gcp_clients import get_storage_client, read_from_gcs
    ...
    try:
        ...
        async with await target_rm.lock_resource(
            ...
            resource_client = ResourcesAsyncClient(
                ...
                base_url=target_rm.backend.lf_cloud_url,
                launchflow_account_id=config.get_account_id(),
                api_key=target_rm.backend.launchflow_api_key,
            )
    ...
    try:
        ...
        async with await target_sm.lock_service(
            ...
            services_client = ServicesAsyncClient(
                http_client=httpx_client,
                base_url=target_sm.backend.lf_cloud_url,
                launchflow_account_id=config.get_account_id(),
                api_key=target_sm.backend.launchflow_api_key,
            )
            for module, tofu_state in tofu_states.items():
                tofu_state_coros.append(
    ...
    if isinstance(em.backend, LocalBackend):
        ...
        if os.path.exists(tofu_path):
            with open(tofu_path, "r") as f:
                return json.load(f)
    elif isinstance(em.backend, GCSBackend):
        tofu_path = os.path.join(
            em.backend.prefix, em.project_name, em.environment_name, "default.tfstate"
        )
        try:
            return await read_from_gcs(em.backend.bucket, tofu_path)
...
async def _get_service_tofu_states(sm: ServiceManager) -> Dict[str, Dict[str, Any]]:
    if isinstance(sm.backend, LocalBackend):
        ...
        for root, dirs, files in os.walk(base_path):
            for f in files:
                if f == "default.tfstate":
                    ...
                    with open(tofu_path, "r") as f:
                        tofu_states[module] = json.load(f)
        return tofu_states
    elif isinstance(sm.backend, GCSBackend):
        base_path = os.path.join(
            sm.backend.prefix,
            sm.project_name,
            sm.environment_name,
        ...
        blobs = gcs_client.list_blobs(sm.backend.bucket, prefix=base_path)
        for blob in blobs:
            if blob.name.endswith("default.tfstate"):
                tofu_state = json.loads(
                    await read_from_gcs(sm.backend.bucket, blob.name)
                )
                module = os.path.basename(blob.name)
                tofu_states[module] = tofu_state
...
async def _get_resource_tofu_states(rm: ResourceManager) -> Dict[str, Dict[str, Any]]:
    if isinstance(rm.backend, LocalBackend):
            ...
            for f in files:
                if f == "default.tfstate":
                    ...
                    with open(tofu_path, "r") as f:
                        tofu_states[module] = json.load(f)
        return tofu_states
    elif isinstance(rm.backend, GCSBackend):
        base_path = os.path.join(
            rm.backend.prefix,
            rm.project_name,
            rm.environment_name,
        ...
        blobs = gcs_client.list_blobs(rm.backend.bucket, prefix=base_path)
        for blob in blobs:
            if blob.name.endswith("default.tfstate"):
                tofu_state = json.loads(
                    await read_from_gcs(rm.backend.bucket, blob.name)
                )
                module = os.path.basename(blob.name)
                tofu_states[module] = tofu_state
...
async def _migrate_environment(
    ...
    try:
        ...
        environment_client = EnvironmentsAsyncClient(
            http_client=httpx_client,
            launch_service_url=target_em.backend.lf_cloud_url,
            api_key=target_em.backend.launchflow_api_key,
            launchflow_account_id=config.get_account_id(),
        )
        async with target_em.lock_environment(
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        try:
            ...
            proj_client = ProjectsAsyncClient(
                ...
                api_key=target.launchflow_api_key,
                launchflow_account_id=config.get_account_id(),
            )
