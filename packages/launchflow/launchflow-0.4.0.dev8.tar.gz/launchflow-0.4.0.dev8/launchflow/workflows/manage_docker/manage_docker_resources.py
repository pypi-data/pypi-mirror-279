    ...
    while remaining_retries > 0:
        try:
            container = docker_client.start_container(
                name=_container_unique_name(inputs.resource.name, inputs.environment_name),
                image=inputs.image,
...
async def replace_docker_resource(
    ...
    with logging_output(inputs.logs_dir) as f:
        with redirect_stdout_stderr(f):
            container_name = _container_unique_name(inputs.resource.name, inputs.environment_name)
