...
class Datasets:
    ...
    def create_dataset(self, request: components.CreateDatasetRequest) -> operations.CreateDatasetResponse:
        r"""Create a dataset"""
    ...
    def update_dataset(self, request: components.DatasetUpdate) -> operations.UpdateDatasetResponse:
        r"""Update a dataset"""
    ...
    def delete_dataset(self, dataset_id: str) -> operations.DeleteDatasetResponse:
        r"""Delete a dataset"""
    ...
    def add_datapoints(self, dataset_id: str, request_body: operations.AddDatapointsRequestBody) -> operations.AddDatapointsResponse:
        r"""Add datapoints to a dataset"""
        hook_ctx = HookContext(operation_id='addDatapoints', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        url = utils.generate_url(operations.AddDatapointsRequest, base_url, '/datasets/{dataset_id}/datapoints', request)
        ...
        req_content_type, data, form = utils.serialize_request_body(request, operations.AddDatapointsRequest, "request_body", False, False, 'json')
        if req_content_type is not None and req_content_type not in ('multipart/form-data', 'multipart/mixed'):
            headers['content-type'] = req_content_type
        if data is None and form is None:
            raise Exception('request body is required')
        ...
        res = operations.AddDatapointsResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type'), raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(http_res.headers.get('Content-Type'), 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[operations.AddDatapointsResponseBody])
                res.object = out
            else:
                content_type = http_res.headers.get('Content-Type')
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
