...
class B2CHelper:
    ...
    def get_user_by_email(self, email: str) -> dict:
        ...
        user = requests.get(
            f"https://graph.microsoft.com/beta/users?$select=displayName,extension_f6f43009abde489484003a8fa6a732e0_OrganizationID&$filter=identities/any(c:c/issuerAssignedId eq '{email}' and c/issuer eq 'eightam72045b2ctenant.onmicrosoft.com')", headers=self.get_auth_header(),
            # "https://graph.microsoft.com/v1.0/users?$select=displayName,id&$filter=identities/any(c:c/issuerAssignedId eq 'j.smith@yahoo.com' and c/issuer eq 'My B2C tenant')"
            timeout=30
        ).json()
