...
def download_control_totals(
    ...
    if CENSUS_API_KEY is None:
        raise ValueError(
            "Empty Census API key, please create on at https://api.census.gov/data/key_signup.html and set with"
            + " os.environ['CENSUS_API_KEY'] = 'MY_KEY' before creating control totals"
        )
...
def aggregate_raw_data(data: pd.DataFrame, group_spec: List) -> pd.DataFrame:
    ...
    for control_group in group_spec:
        ...
        all_columns_this_group = pd.DataFrame(all_columns_this_group)  # type: ignore
        if not np.allclose(all_columns_this_group.sum(axis=1), data[control_group["check"]].sum(axis=1)):  # type: ignore
        ...
        control_totals.append(all_columns_this_group)
