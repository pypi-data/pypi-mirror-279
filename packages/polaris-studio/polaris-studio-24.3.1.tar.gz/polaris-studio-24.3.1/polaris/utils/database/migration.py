...
class Migration:
    ...
    def run(self, conn):
        ...
        if self.migration_id in applied_migration_ids:
            raise RuntimeError(f"Can't apply migration [{self.migration_id}] twice")
        ...
        with conn:
            conn.execute(f"INSERT INTO Migrations VALUES ('{self.migration_id}', {description}, DATETIME('now'))")
        conn.commit()
