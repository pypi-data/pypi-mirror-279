...
def dump_database_to_csv(
    conn, folder_name, signal=None, exclude_patterns=None, include_patterns=None, ext="csv"
) -> None:
    ...
    for table_name in all_tables:
        signal.emit(["start", "secondary", 2, table_name, txt])
        # get the schema
        schema = pd.read_sql_query(f"pragma table_info({table_name})", conn)
        schema.to_csv(folder_name / f"{table_name}.schema", index=False)

        # Get the table
        data = get_table_data(conn, table_name).round(decimals=6)
        signal.emit(["update", "secondary", 1, table_name, txt])
        df_to_file(data, folder_name / f"{table_name}.{ext}", index=False)
        signal.emit(["update", "secondary", 2, table_name, txt])
...
def get_table_data(conn, table_name):
    ...
    if geo_field is not None:
        fields.append(f'AsWKT("{geo_field}", 8) {geo_field}_wkt')
    keys = ",".join(fields)
