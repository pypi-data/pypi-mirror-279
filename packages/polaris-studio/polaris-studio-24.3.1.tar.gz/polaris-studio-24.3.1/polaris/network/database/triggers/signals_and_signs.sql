...
--##
create trigger if not exists signal_populates_times_new_record after insert on Signal
begin
    update Signal
...
--##
create trigger if not exists signal_populates_times_change_in_table after update on Signal
begin
    update Signal
    set "times" = (SELECT count(*) from Signal_Nested_Records where object_id=new.signal)
    where Signal.rowid = new.rowid;
...
--##
create trigger if not exists signal_nested_populates_times_new_record after insert on Signal_Nested_Records
begin
    update Signal
...
--##
create trigger if not exists signal_nested_populates_change_in_table after update on Signal_Nested_Records
begin
    update Signal
    set "times" = (SELECT count(*) from Signal_Nested_Records where object_id=new.object_id)
...
--##
create trigger if not exists signal_nested_populates_delete_in_table after delete on Signal_Nested_Records
begin
    update Signal
...
--##
create trigger if not exists phasing_nested_populates_movements_new_record after insert on Phasing_Nested_Records
begin
    update Phasing
...
--##
create trigger if not exists phasing_nested_populates_movements_change_in_table after update on Phasing_Nested_Records
begin
    update Phasing
    set "movements" = (SELECT count(*) from Phasing_Nested_Records where object_id=new.object_id)
...
--##
create trigger if not exists phasing_nested_populates_movements_delete_in_table after delete on Phasing_Nested_Records
begin
    update Phasing
...
create trigger if not exists phasing_progagates_delete after delete on Phasing
begin
    Delete from Phasing_Nested_Records where object_id=old.phasing_id;
end;

--##
create trigger if not exists phasing_populates_movements_new_record after insert on Phasing
begin
    update Phasing
...
--##
create trigger if not exists phasing_populates_movements_change_in_table after update on Phasing
begin
    update Phasing
    set "movements" = (SELECT count(*) from Phasing_Nested_Records where object_id=new.phasing_id)
...
--##
create trigger if not exists signal_delete_in_table after delete on Signal
begin
...
--##
create trigger if not exists timing_nested_updates_fields_on_insert_in_table after insert on Timing_Nested_Records
begin
    update Timing
...
--##
create trigger if not exists timing_nested_updates_fields_on_update_in_table after update on Timing_Nested_Records
begin
    update Timing
    set "cycle" = coalesce((select sum(value_maximum) + sum(value_red)+ sum(value_yellow) from Timing_Nested_Records where object_id=new.object_id),0),
    "phases" = coalesce((SELECT count(*) from Timing_Nested_Records where object_id=new.object_id),0)
...
--##
create trigger if not exists timing_nested_updates_fields_on_delete_in_table after delete on Timing_Nested_Records
begin
    update Timing
...
--##
create trigger if not exists timing_progagates_delete after delete on Timing
begin
    Delete from Timing_Nested_Records where object_id=old.timing_id;
end;

--##
create trigger if not exists timing_updates_fields_on_update_in_table after update on Timing
begin
    update Timing
...
--##
create trigger if not exists timing_updates_fields_on_insert_in_table after insert on Timing
begin
    update Timing
    set "cycle" = coalesce((select sum(value_maximum) + sum(value_red)+ sum(value_yellow) from Timing_Nested_Records where object_id=new.timing_id),0),
    "phases" = coalesce((SELECT count(*) from Timing_Nested_Records where object_id=new.timing_id),0)
...
--##
create trigger if not exists sign_populates_new_record after insert on Sign
begin
    update Sign
...
--##
create trigger if not exists sign_stop_on_freeway before insert on Sign
when
