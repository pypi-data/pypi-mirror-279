...
--##
create trigger if not exists network_new_link_node_a before insert on link
  when
    (SELECT count(*)
    FROM node
...
create trigger if not exists network_new_link_node_b before insert on link
  when
    (SELECT count(*)
    FROM node
...
create trigger if not exists network_update_link_node_a before update of geo on link
-- we use a before ordering here, as it is the only way to guarantee this will run before the node id update trigger.
-- when inserting a link end point to empty space, create a new node
  when
    (SELECT count(*)
...
create trigger if not exists network_update_link_node_b before update of geo on link
  when
    (SELECT count(*)
    FROM node
    WHERE node.geo = EndPoint(new.geo) AND
...
create trigger if not exists network_new_link after insert on link
  begin
...
create trigger if not exists network_enforces_link_bearing_a_update after update of "bearing_a" on link
begin
    update link
    set "bearing_a" =  round(Degrees(ST_Azimuth(StartPoint(geo), ST_PointN(SanitizeGeometry(geo), 2))),0)
    where link.rowid = new.rowid;
end;
...
create trigger if not exists network_enforces_link_bearing_b_update after update of "bearing_b" on link
begin
    update link
...
create trigger if not exists network_enforces_link_length_update after update of "length" on link
begin
...
create trigger if not exists network_deleted_link after delete on link
  begin
...
create trigger if not exists network_update_node_geo after update of geo on node
  begin
    ...
    INSERT INTO editing_table VALUES(NULL, 'Node', 'node',new.node, 'geo', NULL, 'EDIT', 0, '');
  end;
--##
...
create trigger if not exists network_cannibalise_node before update of geo on node
  when
...
create trigger if not exists network_no_duplicate_node before insert on node
  when
    (SELECT count(*)
    FROM node
...
create trigger if not exists network_dont_delete_node before delete on node
...
--##
create trigger if not exists network_populate_node_fields_on_insert after insert on node
begin
    update node
    set "x" = round(ST_X(new.geo), 8),
        "y" = round(ST_Y(new.geo), 8)
    ...
    INSERT INTO editing_table VALUES(NULL, 'Node', 'node', new.node, 'geo', NULL, 'ADD', 0, '');
end;

--##
create trigger if not exists network_enforces_x_field_node after update of "x" on node
begin
    update node
    set "x" = round(ST_X(new.geo), 8)
    where node.rowid = new.rowid;
...
--##
create trigger if not exists network_enforces_y_field_node after update of "y" on node
begin
    update node
...
create trigger if not exists network_updated_node before update of node on node
  begin
...
--##
create trigger if not exists network_updated_node_fix_index after update of node on node
  begin
...
--@
--
---- The following triggers don't actually work, but we will leave them here to have a record of the fact
---- prevents the user from manually changing node_a to an arbitrary number
-- create trigger if not exists links_table_protects_node_a AFTER UPDATE OF node_a ON link
-- BEGIN
--    update link
--    set node_a = (
--      select node
...
--
--@
-- -- prevents the user from manually changing node_b to an arbitrary number
-- create trigger if not exists links_table_protects_node_b AFTER UPDATE OF node_b ON link
-- BEGIN
--     -- detect that the node in the link's location does not have that node I am setting
--    update link
--    set node_b = (
--      select node
--      from node
