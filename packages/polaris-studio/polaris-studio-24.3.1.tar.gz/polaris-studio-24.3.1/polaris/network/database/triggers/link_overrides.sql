...
--##
create trigger if not exists link_overrides_new_override before insert on Link_Overrides
WHEN
...
create trigger if not exists link_overrides_change_field before update of field on Link_Overrides
WHEN
    ((SELECT count(*) from (SELECT name FROM pragma_table_info('link')) where name=new.field)=0)
BEGIN
