...
class ActiveTransportLink:
    ...
    def save(self, conn: Connection, commit=True) -> None:
        self.distance = self.geo.length
        sql = f"""INSERT into {self.__table_name__}({self.__field_name__},from_node, to_node, "length", ref_link, geo)
                                                    values(?,?,?,?,?,GeomFromWKB(?, ?))"""

        vals = [self.id, self.node_a, self.node_b, self.distance, self.ref_link, self.geo.wkb, self.srid]
        curr = conn.cursor()
