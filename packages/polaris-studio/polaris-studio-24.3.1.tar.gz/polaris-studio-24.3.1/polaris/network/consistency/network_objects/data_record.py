...
class DataRecord:
    ...
    def __init__(
        ...
        self.__srid__ = get_srid(conn=conn, database_path=data_storage._network_file)
        self.type = ""

        if not record.shape[0]:
            data = self._data.get_table(table_name, conn)
            record = data[data.index == element_id]

        self._id_field = record.index.name
        self.__dict__[self._id_field] = element_id
        if record.shape[0]:
            self.__original__[self._id_field] = element_id
            for col in record.columns:
                val = record[col].values[0] if col != "geo" else shapely.wkb.loads(record[col].values[0])
