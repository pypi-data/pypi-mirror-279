...
class TransitBikeFixer(Fixer):
    ...
    def execute(self, conn: sqlite3.Connection):
        if self.issue.id_field != "bike_link":
        ...
        for table, field in [["Location", "location"], ["Parking", "parking"]]:
            if self.issue.operator == "DELETE":
                sql = f"Select {field}, asbinary(geo) from {table} where bike_link=?"
            elif self.issue.operator in ["EDIT", "ADD"]:
                sql = "Select count(*) from Transit_Bike where bike_link=?"
                if sum(list(conn.execute(sql, [self.issue.id_value]).fetchone())) < 1:
                    return
                sql = f"""select {field}, asbinary(geo) from {table}
                          where MbrContains((Select Buffer(geo, 1000) from Transit_Bike where bike_link=?), geo)"""
            else:
                raise Exception("What do do with this operation?")
            items = list(conn.execute(sql, [self.issue.id_value]).fetchall())
            data = [(self.geotool.get_bike_link(shapely.wkb.loads(wkb)), p) for p, wkb in items]
            if data:
                conn.executemany(f"Update {table} set bike_link=? where {field}=?", data)
