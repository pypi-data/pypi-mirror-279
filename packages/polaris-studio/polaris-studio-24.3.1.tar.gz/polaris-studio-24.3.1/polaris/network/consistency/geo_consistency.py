...
class GeoConsistency(WorkerThread):
    ...
    def __init__(self, geotool, data_tables: DataTableCache):
    ...
    def update_all(self):
        ...
        sig = ["start", "master", self.__do_zone + self.__do_active + self.__do_network * 3 + self.__do_loc, self.__mt]
        ...
        with commit_and_close(self._network_path, spatial=True) as conn:
            if self.__do_zone:
                ...
                self.geoconsistency.emit(["update", "master", 1, self.__mt, self.__mt])

            if self.__do_active + self.__do_network:
                self.logger.info("Updating link and walk_link for Location Table")
                if self.__do_network:
                    self.__update_link_micromobility_table(conn=conn)
                self.geoconsistency.emit(["update", "master", 1, self.__mt, self.__mt])

                for tbl_nm in ["Location", "Parking"]:
                    self.logger.info(f"Updating link and walk_link for {tbl_nm} Table")
                    self.__update_active_and_net_links_for_park_loc(
                        tbl_nm,
                        conn=conn,
                        do_link=self.__do_network,
            ...
            if all([self.__do_active, self.__do_loc, self.__do_network, self.__do_zone]):
                sql = """UPDATE Editing_Table SET recorded_change="Global Geo consistency computation", checked=1
                                               WHERE checked=0"""
                conn.execute(sql)
                conn.commit()
    ...
    def update_location_association(self):
        ...
        self.logger.info("Updating Location geo-association throughout the database")
        with commit_and_close(self._network_path, spatial=True) as conn:
            self.__update_location_association(conn=conn)
    ...
    def update_zone_association(self, do_tables=None):
        ...
        self.logger.info("Updating Zone geo-association throughout the database")
        with commit_and_close(self._network_path, spatial=True) as conn:
    ...
    def update_link_association(self):
        ...
        with commit_and_close(self._network_path, commit=True, spatial=True) as conn:
            self.logger.info("Updating Link geo-association throughout the database")
            ...
            conn.execute(
                """Update Editing_Table set recorded_change="Global Geo consistency computation",
                            checked=1 where table_name="Location" and field="link" and issue > ?""",
                [self.m_iid],
            )
            conn.execute(
                """Update Editing_Table set recorded_change="Global Geo consistency computation",
                            checked=1 where table_name="Parking" and field="link" and issue > ?""",
                [self.m_iid],
            )
            conn.commit()
        self.finish()
    ...
    def update_active_network_association(self):
        ...
        self.geoconsistency.emit(["start", "master", 1, self.__mt])
        with commit_and_close(self._network_path, commit=True, spatial=True) as conn:
            self.logger.info("Updating walk_Link geo-association for parking facilities")
            self.__update_active_and_net_links_for_park_loc("Parking", conn, False, True, True)
            self.logger.info("Updating Walk_Link geo-association for locations")
    ...
    def __update_zone_association(self, do_tables: Optional[list], conn: sqlite3.Connection):
        ...
        self.messages.clear()
        zones = self.zone_layer.reset_index().rename(columns={"zone": "new_data"})
        ...
        conn.execute(
            """Update Editing_Table set recorded_change="Global Geo consistency computation",
                           checked=1 where field='zone' and issue > ?""",
            [self.m_iid],
        )
        conn.commit()
    ...
    def __update_simple_element(self, conn, list_jobs, ref_layer, ref_field):
        for table, field in list_jobs:
            ...
            data_orig = self.__data_storage.get_geo_layer(table_name=table, conn=conn).reset_index()
            if data_orig.shape[0] == 0:
                continue
    ...
    def __update_location_association(self, conn: sqlite3.Connection, do_tables: Optional[list] = None):
        ...
        conn.execute(
            """Update Editing_Table set recorded_change="Global Geo consistency computation",
                           checked=1 where field='location' and issue > ?""",
            [self.m_iid],
        )
        conn.commit()
    ...
    def __update_active_and_net_links_for_park_loc(
        ...
        gdf = self.__data_storage.get_geo_layer(table_name, conn).reset_index()
        if not gdf.shape[0]:
            return
        altered_link, altered_walk, altered_bike = pd.DataFrame([]), pd.DataFrame([]), pd.DataFrame([])
        # First we get the new links
        if do_link:
            gdf = gdf[[id_field, "link", "offset", "setback", "dir", gdf.geometry.name]]
            altered_link = self.__get_link_for_table("link", gdf, id_field, "AUTO", True, conn)

        if do_wlink:
            # Then we get the new walk links
            gdf = gdf[[id_field, "link", "offset", gdf.geometry.name]].assign(setback=0, dir=0)
            altered_walk = self.__get_link_for_table("walk", gdf, id_field, "WALK", True)
            altered_walk.rename(columns={"link": "walk_link", "offset": "walk_offset"}, inplace=True)

        if do_blink:
            # Then we get the new walk links
            gdf = gdf[[id_field, "link", "offset", gdf.geometry.name]].assign(setback=0, dir=0)
            altered_bike = self.__get_link_for_table("bike", gdf, id_field, "BIKE", True)
    ...
    def __update_link_micromobility_table(self, conn: sqlite3.Connection):
        gdf = self.__data_storage.get_geo_layer("Micromobility_Docks", conn).reset_index()
        if gdf.shape[0]:
        ...
        conn.execute(
            '''Update Editing_Table set recorded_change="Global Geo consistency computation",
                                      checked=1 where table_name="Micromobility_Docks"''',
        )
    ...
    def __get_link_for_table(
        ...
        if link_type == "link":
            ...
            if mode is not None and conn is not None:
                ltypes = self.__data_storage.get_table("Link_Type", conn).reset_index()
        ...
        gdf["nffst"] = gdf.apply(lambda row: row.link_geo.project(row[geo_name]), axis=1)
        if do_direction:
    ...
    def __clear_link_issues(self, conn, table_name: str, do_link=True, do_wlink=True, do_blink=True):
        if do_link:
            sql = f"""Update Editing_Table set recorded_change="Global Geo consistency computation",
                              checked=1 where table_name="{table_name}" and field="link" and issue > ?"""
            conn.execute(sql, [self.m_iid])
        if do_wlink:
            sql = f"""Update Editing_Table set recorded_change="Global Geo consistency computation",
                              checked=1 where table_name="{table_name}" and field="walk_link" and issue > ?"""
            conn.execute(sql, [self.m_iid])
        if do_blink:
            sql = f"""Update Editing_Table set recorded_change="Global Geo consistency computation",
                              checked=1 where table_name="{table_name}" and field="bike_link" and issue > ?"""
            conn.execute(sql, [self.m_iid])
        conn.commit()

    @property
    def location_layer(self):
        return self.__get_layer("location", "Location")

    @property
    def link_layer(self):
        return self.__get_layer("link", "Link")

    @property
    def walk_layer(self):
        return self.__get_layer("walk", "Transit_Walk")

    @property
    def bike_layer(self):
        return self.__get_layer("bike", "Transit_Bike")

    @property
    def zone_layer(self):
        return self.__get_layer("zone", "Zone")

    def __get_layer(self, lyr, lyr_name):
        if lyr not in self.__layers:
            with read_and_close(self._network_path, spatial=True) as conn:
                self.__layers[lyr] = self.__data_storage.get_geo_layer(lyr_name, conn)
        return self.__layers[lyr]
