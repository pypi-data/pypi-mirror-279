...
class SignalCyclePhasing:
    ...
    def __init__(self, signal: int, phasing: int, signal_record, conn: sqlite3.Connection):
        ...
        self.right_turn_on_red = signal_record.right_turn_on_red
        self.__id = uuid4().hex
    ...
    def recompute(self, conn: sqlite3.Connection):  # noqa: C901
        ...
        if arrivals == 1:
            ...
            if min(self.timing_proportion) <= 40:
                if self.timing_proportion[0] > 40:
                    self.timing_proportion = [0.3, 0.7]
                elif self.timing_proportion[1] > 40:
                    self.timing_proportion = [0.7, 0.3]
                else:
            ...
            # UTurns and Lefts would cause conflict, so we need to know if they will exist
            crit1 = full_df.type.isin(["UTURN", "LEFT"]).any()
        ...
        # Adds right-turn-on-red
        if self.right_turn_on_red:
            if logger.level == DEBUG:
                logger.debug("Adding right-turn-on-red")

            right_df = self.__connections[self.__connections["type"] == "RIGHT"]
            has_right: Dict[int, List[str]] = {int(i): [] for i in range(self.__phase_index)}
            for phase in phases:
                if "RIGHT" in phase[2]:
                    has_right[phase[0]].append(f"{phase[3]}-{phase[5]}")

            ph = pd.DataFrame(phases, columns=cols)

            for _, row in right_df.iterrows():
                for p in range(1, self.__phase_index):
                    if f"{row.link}-{row.to_link}" in has_right[p]:
                        continue
                    ...
                    right_phase = [p, -1, f"{d}_RIGHT", row.link, row.dir, row.to_link, STOP_PERMIT]
                    if right_phase not in phases:
                        phases.append(right_phase)
                        ph = pd.DataFrame(phases, columns=cols)
    ...
    def __phases_for_best_protection(self, df):
        ...
        for tp in ["THRU", "RIGHT", "LEFT", "UTURN"]:
