...
class FreewayIntersection(IntersecSuperClass):
    ...
    def __init__(self, intersection):
        super(FreewayIntersection, self).__init__(intersection)
        self.type = "diverge"
        self.intersection_type = "freeway"
        self.__pockets = ""
        incoming_ramp = sum([x.lanes for x in intersection.incoming if x.is_ramp()])
        outgoing_ramp = sum([x.lanes for x in intersection.outgoing if x.is_ramp()])
