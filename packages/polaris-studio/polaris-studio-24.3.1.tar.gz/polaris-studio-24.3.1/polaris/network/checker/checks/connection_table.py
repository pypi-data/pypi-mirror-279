...
class CheckConnectionTable:
    def __init__(self, supply_db: PathLike):
         ...
         * If all necessary connections exist
         ...
         Args:
                 *data_tables* (:obj:`DataTableCache`): Network's data table storage
         ...
         ::
             ...
             net.close()
        """
        self.data_tables = DataTableCache(supply_db)
    ...
    def lane_connection(self):
        ...
        with read_and_close(self.supply_db) as conn:
            pairs = [
                [pd.read_sql_query(sql1, conn), "link_end_connection"],
                [pd.read_sql_query(sql2, conn), "link_start_connection"],
            ]
        ...
        with read_and_close(self.supply_db, spatial=True) as conn:
            for node in all_nodes.node.values:
                intersec = Intersection(self.data_tables, self.supply_db)
