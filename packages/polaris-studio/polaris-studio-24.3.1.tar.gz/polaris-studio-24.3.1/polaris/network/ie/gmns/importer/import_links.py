...
def import_gmns_links(gmns_folder: str, transformer, conn: sqlite3.Connection, path_to_file: PathLike):
    # Thorough documentation of the assumptions used in this code is provided
    # with the package documentation
    link_file = join(gmns_folder, "link.csv")
    ...
    if "geometry_id" in links:
        if links.geometry_id.values[0] > 0:  # type: ignore
            ...
            geo_file = join(gmns_folder, "link.csv")
            links = links.join(pd.read_csv(geo_file))
            links.drop(columns=["geometry_id"], errors="ignore")
    ...
    ideal_links_table = DataTableCache(path_to_file).get_table("link", conn).reset_index()
    ...
    if transformer is not None:
        links.loc[:, "geo"] = apply_transform(links, transformer)
