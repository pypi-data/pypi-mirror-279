...
def export_links_to_gmns(gmns_folder: str, transformer, conn, path_to_file):
    links = DataTableCache(path_to_file).get_table("Link", conn=conn).reset_index()
    links.drop(columns=["setback_a", "setback_b"], inplace=True)
    links.drop(columns=["cap_ba", "cap_ab", "bearing_a", "bearing_b"], inplace=True)
    tolls = DataTableCache(path_to_file).get_table("Toll_Pricing", conn=conn).reset_index()
    ...
    geometries = [transform(transformer.transform, geo) for geo in links.geo.apply(shapely.wkb.loads)]
    geo_array = np.array([shapely.wkt.dumps(geo, rounding_precision=6) for geo in geometries])
