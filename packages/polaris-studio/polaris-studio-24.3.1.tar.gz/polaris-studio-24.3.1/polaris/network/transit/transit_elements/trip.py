...
from polaris.network.constants import constants, TRIP_ID_MULTIPLIER
...
class Trip(BasicPTElement):
    ...
    * shape (:obj:`LineString`): Shape for this trip. Directly from shapes.txt or rebuilt from sequence of stops
    ...
    def __init__(self):
        ...
        self.seated_capacity: Optional[int] = None
        self.design_capacity: Optional[int] = None
        self.total_capacity: Optional[int] = None
        self.is_artic = 0
        self.source_time = []
    ...
    def save_to_database(self, conn: Connection, commit=True) -> None:
        ...
        sql = """insert into Transit_Trips (trip_id, trip,"dir", pattern_id, seated_capacity, design_capacity,
        ...
        sql = """insert into Transit_Trips_Schedule(trip_id, "index", arrival, departure, time_source)
                                            values (?, ?, ?, ?, ?)"""
        data = []
