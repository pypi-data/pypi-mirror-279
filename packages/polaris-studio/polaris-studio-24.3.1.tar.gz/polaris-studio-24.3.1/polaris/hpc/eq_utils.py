...
from polaris.hpc.eqsql.eq_db import FAILED, QUEUED, RUNNING, tasks_table
...
def git_pull(engine, target):
    ...
    with engine.connect() as conn:
        insert_task(conn=conn, worker_id=f"^{target}$", definition=git_update_task, input={})


@clear_idle_pg_connection()
def restart_worker(engine, target):
    git_update_task = {"task-type": "control-task", "control-type": "EQ_RESTART"}
    with engine.connect() as conn:
        insert_task(conn=conn, worker_id=f"^{target}$", definition=git_update_task, input={})


@clear_idle_pg_connection()
def pull_and_restart_all(engine, worker_id_startswith=None):
...
@clear_idle_pg_connection()
def task_queue(engine, clear_dead_threshold="7 days", sort_on="exp_id", cols=None, style_df=True):
    with engine.connect() as conn:
        stmt = tasks_table.select().filter(tasks_table.c.status.in_([QUEUED, RUNNING]))
...
@clear_idle_pg_connection()
def query_workers(engine, clear_dead_threhold="3 days", clear_idle_threshold="10 minutes", style_df=True):
    with engine.connect() as conn:
        ...
        if clear_dead_threhold is not None:
            idx1 = df.time_since_update > clear_dead_threhold
...
@clear_idle_pg_connection()
def terminate_worker(engine, worker_id):
    eq_abort_task = {"task-type": "control-task", "control-type": "EQ_ABORT"}
    with engine.connect() as conn:
        insert_task(conn=conn, worker_id=f"^{worker_id}$", definition=eq_abort_task, input={})
...
def query_experiments(engine, experiment_ids, cols=None, style_df=True, cross_validate_to_workers=True):
    ...
    df = pd.concat([df, pd.DataFrame({"exp_id": list(missing_exp_id), "status": "no_record"})])
...
def add_time_since_update(df):
    ...
    df["time_since_update"] = datetime.datetime.utcnow() - df["updated_at"]
