...
DROP TABLE IF EXISTS network_gaps_by_link_type;
CREATE TABLE network_gaps_by_link_type AS
...
DROP TABLE IF EXISTS network_gaps_by_hour;
CREATE TABLE network_gaps_by_hour AS
...
SELECT person_toll, tnc_toll, person_toll + tnc_toll as total_toll
FROM
    (
        SELECT scaling_factor * sum(toll) as person_toll
        FROM Trip
        WHERE type = 22 or ((mode == 0 or mode == 9) and type == 11)
    ) as t1,
    (
        SELECT scaling_factor * sum(toll) as tnc_toll
        FROM TNC_Trip
    ) as t2;
