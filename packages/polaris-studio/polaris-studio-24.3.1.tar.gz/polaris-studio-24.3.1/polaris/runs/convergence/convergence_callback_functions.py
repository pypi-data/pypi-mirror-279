...
from polaris.runs.statistics_db.statistics_db import control_db_engine, record_statistics
...
def clean_db_for_dta(input_files: PolarisInputs):
...
def copy_back_files(config: ConvergenceConfig, current_iteration: ConvergenceIteration):
    type = current_iteration.type()
    if type != "pop_synth" and not (type == "skim" and (config.data_dir / config.highway_skim_file_name).exists()):
...
def default_end_of_loop_fn(
    ...
    aggregate_summaries(config.data_dir, config.db_name, save=True)
    ...
    if current_iteration.is_last:
        export_trajectories(output_dir, current_iteration.files)


@can_fail
def record_stats_in_central_db(config: ConvergenceConfig, current_iteration: ConvergenceIteration):
    engine = None
    try:
        engine = control_db_engine()
...
@can_fail
def record_stats_locally(config: ConvergenceConfig, current_iteration: ConvergenceIteration):
...
@function_logging("    Exporting Trajectories")
def export_trajectories(output_dir: Path, output_files: PolarisInputs):
    with commit_and_close((output_files.demand_db)) as conn:
        run_sql_file(sql_dir / "export_trajectories.sql", conn, attach={"a": output_files.supply_db})
        table_to_csv(conn, "vehicle_trajectory", output_dir / "vehicle_trajectory.csv")


def create_mep_outputs(output_dir: Path, output_files: PolarisInputs):
    with commit_and_close((output_files.demand_db)) as conn:
        run_sql_file(
...
def precache_kpis(current_iteration: ConvergenceIteration):
    from polaris.analyze.result_kpis import ResultKPIs

    ResultKPIs.from_iteration(current_iteration).cache_all_available_metrics()


@can_fail
@function_logging("        ~   Running Charging analysis")
def run_charging_events_analysis(config: ConvergenceConfig, current_iteration: ConvergenceIteration):
...
def default_async_fn(config: ConvergenceConfig, current_iteration: ConvergenceIteration, output_dir):
...
@can_fail
def copy_tables_to_stats_db(config: ConvergenceConfig, current_iteration: ConvergenceIteration):
    tables_to_copy = [
        "vmt_vht_by_mode",
...
def delete_big_files(model_dir: Path):
    """
