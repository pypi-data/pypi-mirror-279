...
def calibrate_non_transit_modes(config: ConvergenceConfig, output_files: PolarisInputs, output_dict: dict):
    ...
    target = load_targets(config.calibration.target_csv_dir / "mode_choice_targets.csv")

    boardings_targets_file = config.calibration.target_csv_dir / "mode_choice_boarding_targets.csv"

    # We don't want to calibrate boardings and transit mode shares as the same time, if boarding
    # file exists, set them to zero and renormalize everything else.
    if boardings_targets_file.exists():
        modes_to_zero = ("TRANSIT", "RAIL", "PNR", "PNRAIL")

        for trip_type in ["HBW", "HBO", "NHB", "TOTAL"]:
            transit_share = sum(v for mode, v in target[trip_type].items() if mode in modes_to_zero)
            target[trip_type] = {
                mode: v / (1 - transit_share) if mode not in modes_to_zero else 0.0
                for mode, v in target[trip_type].items()
            }
...
def load_targets(file):
    ...
    return {k.upper(): v for k, v in rv.items()}
