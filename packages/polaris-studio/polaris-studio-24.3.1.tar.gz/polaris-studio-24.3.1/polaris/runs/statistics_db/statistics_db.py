...
control_db_json_file = Path(__file__).absolute().parent / "control_db.json"


def control_db_engine():
    return DbConfig.from_json_file(control_db_json_file).create_engine()


# TODO: Return bool
def ensures_connection(engine):
    try:
        return engine.connect()
    except:
        logging.getLogger("polaris").warning("Running statistics database is not available. Saving to the model folder")
