...
def migrate(conn):
    ...
    num_tnc_operators = get_scenario_value(tnc_file, "NO_OF_OPERATORS")
    op_1_name = get_scenario_value(tnc_file, "OP_1")
    ...
    df = pd.read_csv(fleet_veh_dist_file, sep=sep)

    logging.debug(f"Current columns in veh distribution file: {df.columns.names}")
