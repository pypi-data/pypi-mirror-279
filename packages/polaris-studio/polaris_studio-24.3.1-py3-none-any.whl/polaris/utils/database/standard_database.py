...
from polaris.utils.database.db_utils import has_column, run_sql_file, has_table
...
class DatabaseType(IntEnum):
    ...
    def __str__(self):
        if self == DatabaseType.Supply:
            return "Supply"
        if self == DatabaseType.Demand:
            return "Demand"
        if self == DatabaseType.Results:
            return "Results"
...
class StandardDatabase:
    ...
    def tables(self, geotables=True, datatables=True):
        ...
        files = []
        if geotables:
            files.append(join(self.sql_directory, "geotables.txt"))
        if datatables:
            files.append(join(self.sql_directory, "datatables.txt"))

        def readlines(file):
            with open(file, "r") as f:
                return [x.strip() for x in f.readlines()]

        return [x for file in files for x in readlines(file)] + ["migrations"]
    ...
    def add_table(self, conn: Connection, table_name: str, srid: int, add_defaults=True) -> None:
        # Create the table
    ...
    def create_tables(self, conn: Connection, srid: int, add_defaults=True) -> None:
    ...
    def drop_columns_legacy(self, conn: Connection, table_name: str, column_names: List[str]):
        ...
        script_name = "setup_windows_spatialite.py" if is_windows() else "setup_linux_sqlite.sh"
