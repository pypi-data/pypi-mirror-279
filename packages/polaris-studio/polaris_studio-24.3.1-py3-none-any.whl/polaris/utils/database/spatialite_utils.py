...
from polaris.utils.env_utils import is_not_windows, is_windows
...
def ensure_spatialite_binaries_ubuntu() -> None:
    ...
    if not Path("/usr/local/lib/mod_spatialite.so").exists():
    ...
    os.environ["MOD_SPATIALITE_PATH"] = "/usr/local/lib/mod_spatialite"
...
def spatialize_db(conn, logger=None):
    logger = logger or logging.getLogger("polaris")
