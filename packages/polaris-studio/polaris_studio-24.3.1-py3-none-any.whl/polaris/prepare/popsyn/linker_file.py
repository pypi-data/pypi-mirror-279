...
class LinkerFile:
    ...
    def from_dir(cls, dir):
        linker_file = dir / "linker_file.txt"
        if not linker_file.exists():
            linker_file = dir.parent / "linker_file.txt"
        if not linker_file.exists():
            raise FileNotFoundError(f"Can't find linker_file.txt in {dir} or {dir.parent}")
        return cls(linker_file)

    def __init__(self, linker_file):
        self.linker_file_contents = self.load_linker_file(linker_file)
        self.file = self.extract_file_names(self.linker_file_contents, linker_file.parent)
    ...
    @staticmethod
    def extract_file_names(contents: list, base_dir: Path):
        return {
            "HH": base_dir / LinkerFile.parse_line_for_keyword(contents, "HHFILE")[0],
            "PERSON": base_dir / LinkerFile.parse_line_for_keyword(contents, "PERSONFILE")[0],
            "CONTROL": base_dir / LinkerFile.parse_line_for_keyword(contents, "ZONEFILE")[0],
        }
