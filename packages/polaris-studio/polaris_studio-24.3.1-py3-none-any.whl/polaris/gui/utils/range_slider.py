# From https://github.com/sleepbysleep/range_slider_for_Qt5_and_PyQt5
# As seen on https://stackoverflow.com/questions/67028200/pyqt5-qslider-two-positions
...
class RangeSlider(QtWidgets.QSlider):
    ...
    def __init__(self, *args, minval, maxval, label):
        ...
        max_val = maxval if maxval < 2147483647 else 2147483646
    ...
    def update_label(self):
        def format_val(val):
            if val >= 2147483646:
                return "inf"
            return f"{int(val):,}" if val > 10 else f"{val:,.2f}"

        self.label.setText(f"{format_val(self._low)} - {format_val(self._high)}")
