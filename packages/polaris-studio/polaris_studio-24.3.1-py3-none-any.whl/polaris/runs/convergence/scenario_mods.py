...
def base_scenario_mods(config: ConvergenceConfig, current_iteration: ConvergenceIteration):
    ...
    if config.num_outer_loops > 1 and (config.num_dta_runs) > 0:
        if current_iteration.iteration_number % (config.num_abm_runs + config.num_dta_runs) == 1:
...
def setup_scenario_for_pop_synth(config: ConvergenceConfig):
    mods = {
        ...
        "Population synthesizer controls.replan_workplaces": True,
        "ABM Controls.read_trip_factors": {"External": 0.0},
        "time_dependent_routing_weight_factor": 1.0,
...
def setup_scenario_for_normal_iteration(config, current_iteration):
    mods = {
        "Routing and skimming controls.time_dependent_routing_weight_factor": 1.0,
        "percent_to_synthesize": 1.0,
        "ABM Controls.read_trip_factors": {"External": 1.0, "ABM": 0.0},
        "read_population_from_database": True,
        "replan_workplaces": current_iteration.iteration_number <= config.num_workplace_runs,
        "vehicle_trajectory_sample_rate": config.trajectory_sampling / config.population_scale_factor,
    }
    ...
    if config.do_routing_MSA and current_iteration.iteration_number > max(config.num_workplace_runs, 1):
        time_dependent_weight = 1 / (current_iteration.iteration_number - config.num_workplace_runs)
    mods["time_dependent_routing_weight_factor"] = time_dependent_weight

    # mods["replan_workplaces_rate"] = 1 / current_iteration.iteration_number

    if current_iteration.is_last:
        mods["Output controls.write_lc_traffic_trajectory"] = True

    if config.fixed_demand:
        mods["percent_to_synthesize"] = 0.0
        mods["read_population_from_database"] = False
        mods["ABM Controls.read_trip_factors"] = {"External": 1.0, "ABM": 1.0, "TNC": 1.0}
...
def setup_scenario_for_dta_iteration(config, current_iteration):
    mods = {
        ...
        "percent_to_synthesize": 1.0,
        "ABM Controls.read_trip_factors": {"External": 1.0, "ABM": 1.0},
        "read_population_from_database": True,
        "replan_workplaces": False,
