...
@function_logging("Loading timing choice simulation values")
def load_simulated(output_files, population_sample_rate, warm_calibrate):
    ...
    with commit_and_close(output_files.demand_db) as conn:
        if not has_table(conn, "activity_start_distribition"):
...
@function_logging("Loading timing choice target values from {target_file}")
