...
def export_connection_to_gmns(gmns_folder: str, transformer, conn, path_to_file: PathLike):
    connec = DataTableCache(path_to_file).get_table("Connection", conn=conn).reset_index()
    ...
    pocket = DataTableCache(path_to_file).get_table("Pocket", conn=conn)
    ...
    geometries = [transform(transformer.transform, geo) for geo in connec.geo.apply(shapely.wkb.loads)]
    connec.loc[:, "geo"] = np.array([shapely.wkt.dumps(geo, rounding_precision=6) for geo in geometries])
