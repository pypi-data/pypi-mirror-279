...
def read_stop_times(conn: sqlite3.Connection, path_to_file: PathLike):
    tpm = DataTableCache(path_to_file).get_table("Transit_Pattern_Links", conn)
    tts = DataTableCache(path_to_file).get_table("Transit_Trips_Schedule", conn).reset_index()
    tl = DataTableCache(path_to_file).get_table("Transit_Links", conn).reset_index()
