...
class RouteSystem:
    def __init__(self, database_path: os.PathLike):
        ...
        self.dts = DataTableCache(self.__database_path)
        ...
        self.transformer = Transformer.from_crs(
            f"epsg:{get_srid(database_path=database_path)}", "epsg:4326", always_xy=True
        )
    ...
    def _read_stops(self, conn: sqlite3.Connection):
        self.stops = read_stops(conn, self.transformer, self.__database_path)
    ...
    def _read_patterns(self, conn: sqlite3.Connection):
        self.patterns = self.patterns or read_patterns(conn, self.transformer, self.__database_path)
    ...
    def _timezone(self, conn: sqlite3.Connection, allow_error=True):
        geotool = Geo(self.__database_path)
        try:
            return geotool.get_timezone(conn)
