...
class MMGraph:
    ...
    def __init__(self, geotool, stops, agency, mtmm):
        ...
        self.node_corresp = []
        self.__all_links = {}  # type: Dict[pd.Series]
        self.distance_to_project = -1
        self.df = pd.DataFrame([])
        self.logger = logging.getLogger("polaris")
        self._path_to_file = geotool._network_file
    ...
    def build_graph_with_broken_stops(self, mode_id: int, distance_to_project=50):
        ...
        self.__mode = mode_correspondence[self.mode_id]  # type: ignore
        ...
        with read_and_close(self._path_to_file, spatial=True) as conn:
            # When it is rail we don't care about the number of lanes to define link directionality
            extra_clause = " " if self.__mode == "RAIL" else "AND lanes_ab+lanes_ba>0"
            get_qry = f"""Select link link_id, 0 direction, node_a a_node, node_b b_node, max(fspd_ab, fspd_ba) speed,
                                 "length" distance, lanes_ab, lanes_ba, ST_AsText(geo) wkt from Link
                          INNER JOIN Link_Type ON Link.type = Link_Type.link_type
                          WHERE INSTR(Link_Type.use_codes, "{self.__mode}")>0 {extra_clause}"""

            self.geotool.logger.debug("  Reading links from physical network")
            self.df = pd.read_sql(get_qry, conn)
            if self.__mode != "RAIL":
                self.df.loc[self.df.lanes_ab == 0, "direction"] = -1
                self.df.loc[self.df.lanes_ba == 0, "direction"] = 1
            self.df.drop(columns=["lanes_ab", "lanes_ba"], inplace=True)
        ...
        if not self.df.shape[0]:
            ...
            return Graph()
        if modename not in self.geotool.mode_link_idx:
            self.geotool.build_mode_link_index(self.__mode)
    ...
    def __build_graph_from_scratch(self):
        ...
        self.df = self.df.assign(free_flow_time=np.inf, wrong_side=0, closest=1, to_remove=0)
    ...
    def connect_node(self, stop) -> None:
        ...
        is_closest = 1
        conn_found = 0
        distances = [stop.geo.distance(self.__all_links[lid].geo) for lid in list_nearest]
        ...
        criterium = criterium if nearest.dist.min() < 250 else ceil(nearest.dist.min() * 100) / 100
        nearest = nearest[nearest.dist <= max(criterium, nearest.dist.min())].link_id.tolist()

        for link_id in nearest:
