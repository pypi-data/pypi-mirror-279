...
from polaris.utils.signals import qgis
...
class Transit(WorkerThread):
            ...
            if hard_reset:
                ...
                for table in to_clear:
                    ...
                    if table.lower() in table_list:
                        ...
                        conn.execute(f'DELETE FROM geometry_columns where f_table_name="{table}";')
                    logger.debug(f"    Creating table: {table}")
                    qry_file = Path(join(dirname(dirname(realpath(__file__))), "database/sql_schema", f"{table}.sql"))
                    if isfile(qry_file):
                        run_sql_file(qry_file=qry_file, conn=conn, replacements={"SRID_PARAMETER": self.srid})
                    else:
    ...
    def fix_connections_table(self):
        ...
        with commit_and_close(self.network.path_to_file, spatial=True) as conn:
            data_tables = DataTableCache(self.network.path_to_file)
    ...
    def __purge_added_connections(self, conn: sqlite3.Connection):
        ...
        for n in nodes:
            ...
            intersec = Intersection(data_tables=DataTableCache(ptf), path_to_file=ptf, conn=conn)
