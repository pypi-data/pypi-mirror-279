...
from polaris.network.constants import constants, ROUTE_ID_MULTIPLIER, AGENCY_MULTIPLIER
...
class Route(BasicPTElement):
    ...
    * total_capacity (:obj:`float`): Total vehicle capacity for this route"""
    ...
    def from_database(self, conn: Connection, route_id: int):
        sql = """select route, agency_id, shortname, longname, description, "type", seated_capacity,
                 design_capacity, total_capacity, number_of_cars, ST_AsBinary(geo) from Transit_Routes
                 where route_id=?"""
        data = conn.execute(sql, [route_id]).fetchone()
        (
            self.route,
            self.agency_id,
            self.route_short_name,
        ...
        self.shape = shapely.wkb.loads(self.shape)
