...
class Diagnostics(WorkerThread):
    ...
    def __init__(self, geotools, data_tables):
        ...
        self.data_tables = data_tables  # type: polaris.network.data_table_cache.DataTableCache
    ...
    def short_detours(self, threshold: float, returns="links") -> dict:
        ...
        self.__connec = self.data_tables.get_table("connection")[["link", "to_link"]]
        ...
        all_links = self.data_tables.get_table("link")
        all_links = all_links[(all_links.node_a.isin(data.node)) | (all_links.node_b.isin(data.node))]

        # Builds the graph
        self.__nx__graph_builder(all_links)

        pairs_from = all_links[all_links.lanes_ab > 0][["node_a", "node_b"]]
        pairs_from.columns = ["f", "t"]

        pairs_to = all_links[all_links.lanes_ba > 0][["node_b", "node_a"]]
        pairs_to.columns = ["f", "t"]

        loops: Dict[int, Any] = {}
        for link_id, rec in pd.concat([pairs_from, pairs_to]).iterrows():  # type: (int, pd.Series)
            loop_data = self.__loop_finder(rec.f, rec.t, returns, threshold)
            if loop_data:
                dct = loops.get(link_id, {})
                loops[link_id] = {**dct, **loop_data}
