...
CREATE TABLE IF NOT EXISTS "Link" (
 ...
 "node_a"            INTEGER         NOT NULL DEFAULT 0,   --@ One of the nodes that the links connects to. Upstream (downstream) node of the link of the unidirectional link AB (BA)
 "node_b"            INTEGER         NOT NULL DEFAULT 0,   --@ Another node of the nodes that the links connects to. Downstream (upstream) node of the link of the unidirectional link BA (AB)
 ...
 "fspd_ab"           REAL                     DEFAULT 0,     --@ free flow speed of the unidirectional link from node a to node b
 "cap_ab"            INTEGER         NOT NULL DEFAULT 0,     --@ capacity in vehicles/hour of the link from node a to node b (used only if higher than default for the scenario),
 "lanes_ba"          INTEGER         NOT NULL DEFAULT 0,     --@ number of lanes of the unidirectional link from node b to node a
 "fspd_ba"           REAL                     DEFAULT 0,     --@ free flow speed of the unidirectional link from node b to node a
 "cap_ba"            INTEGER         NOT NULL DEFAULT 0,     --@ capacity in vehicles/hour of the link from node a to node b (used only if higher than default for the scenario)
...
create INDEX IF NOT EXISTS link_node_a_idx ON link (node_a);
create INDEX IF NOT EXISTS link_link ON link (link);
create INDEX IF NOT EXISTS link_node_b_idx ON link (node_b);
