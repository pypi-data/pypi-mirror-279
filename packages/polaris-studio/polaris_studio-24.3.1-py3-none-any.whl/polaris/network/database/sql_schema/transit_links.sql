...
CREATE TABLE IF NOT EXISTS "Transit_Links" (
    transit_link  INTEGER           PRIMARY KEY, --@ Link ID of the unidirectional transit service link
    pattern_id    INTEGER NOT NULL, --@ ID of the pattern in the format AARRRRPPPP0000 (Agency, Route, Pattern). For every from_node, to_node, pattern_id tuple, we generate a unique transit link
