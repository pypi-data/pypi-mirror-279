create trigger if not exists turn_overrides_rejects_if_not_connecting_insert before insert on Turn_Overrides
...
--##
create trigger if not exists turn_overrides_populates_node_todir_new_record after insert on Turn_Overrides
begin
...
--##
create trigger if not exists turn_overrides_enforce_node_todir_on_table_change after update on Turn_Overrides
begin
    update Turn_Overrides
    set "node" = (SELECT CASE
                         WHEN new.dir =1 THEN (SELECT NODE_A FROM LINK WHERE LINK.link=new.link)
...
--##
create trigger if not exists turn_overrides_rejects_if_not_connecting_update before update on Turn_Overrides
  when (select count(link) from
            (Select link from link where link.node_a in (SELECT NODE_A node FROM LINK WHERE link=new.link and new.dir=1)
            union ALL
            Select link from link where link.node_b in (SELECT NODE_A node FROM LINK WHERE link=new.link and new.dir=1)
