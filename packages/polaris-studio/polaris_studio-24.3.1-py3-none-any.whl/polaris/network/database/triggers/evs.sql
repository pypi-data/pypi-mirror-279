...
begin
    ...
    set
        Longitude = round(ST_Y(new.geo), 8),
        Latitude = round(ST_X(new.geo), 8)
    where
...
begin
    update EV_Charging_Stations set Longitude = round(ST_Y(new.geo), 8);
end;

--##
create trigger if not exists evs_enforces_latitude_field after update of "Latitude" on EV_Charging_Stations
begin
    update EV_Charging_Stations set Latitude = round(ST_X(new.geo), 8);
end;
