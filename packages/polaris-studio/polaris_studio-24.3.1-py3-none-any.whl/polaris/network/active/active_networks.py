...
from polaris.network.constants import WALK_AGENCY_NAME, AGENCY_MULTIPLIER
...
class ActiveNetworks(WorkerThread):
    ...
    def __init__(self, geotool, data_tables) -> None:
        ...
        logger.info("Creating active links for your network")
        ...
        self.__delete_links: List[WalkLink] = []

        self.connectors: Dict[int, int] = {}

        self.__walk_range = WALK_LINK_RANGE
        self.__bike_range = BIKE_LINK_RANGE
        self.broken = 0
        self.__time = 0
        self.master_msg_txt = "Rebuilding Walk network"
        self.__mlid__ = -1  # Tracker for Link ID so we don't repeat them
    ...
    def build(self):
        ...
        with commit_and_close(self._network_file, spatial=True) as conn:
            ...
            self.activenet.emit(["start", "master", 9, self.master_msg_txt])

            self.activenet.emit(["start", "secondary", 1, "Initializing table", self.master_msg_txt])
            self._initialize_table(conn)

            self.activenet.emit(["start", "secondary", 1, "Checking data", self.master_msg_txt])
            self._check_agency(conn)

            self.activenet.emit(["start", "secondary", 1, "Copying links", self.master_msg_txt])
            self._copy_links_from_road_network(conn)

            if not self.__do_update_associations__:
                create_network_triggers(conn, self.srid)
            ...
            if self.__do_update_associations__:
                ...
                conn.execute(
                    '''Update Editing_Table set recorded_change="Creation of new transit active table",
                                      checked=1 where table_name="Transit_Walk"'''
                )
                conn.execute(
                    '''Update Editing_Table set recorded_change="Creation of new transit active table",
                                      checked=1 where table_name="Transit_Stops"'''
                )
                conn.execute(
                    '''Update Editing_Table set recorded_change="Creation of new transit active table",
                                      checked=1 where table_name="Transit_Bike"'''
                )
                create_network_triggers(conn, self.srid)
            self.finish()
    ...
    def _check_agency(self, conn: sqlite3.Connection):
        ...
        if dt is None:
            ...
            conn.execute(sql, dt)
            conn.commit()
        self.activenet.emit(["update", "secondary", 1, "Checking data", self.master_msg_txt])
    ...
    def _initialize_table(self, conn: sqlite3.Connection):
        ...
        min_stop = WALK_AGENCY_ID * AGENCY_MULTIPLIER
        max_stop = (WALK_AGENCY_ID + 1) * AGENCY_MULTIPLIER
        conn.execute("DELETE from Transit_Stops where stop_id>=? and stop_id<?;", [min_stop, max_stop])
        conn.commit()
        self.activenet.emit(["update", "secondary", 1, "Initializing table", self.master_msg_txt])

    def _copy_links_from_road_network(self, conn: sqlite3.Connection) -> None:
        logger.info("   Copying links from road network")
        transfer_qry = """INSERT into Transit_Walk(from_node, to_node, "length",ref_link, geo)
                          SELECT Node_a, Node_b, "length", link, geo from Link
                          INNER JOIN Link_Type ON Link.type = Link_Type.link_type
                          WHERE INSTR(Link_Type.use_codes, 'WALK')>0"""

        if self.__do_osm:
            transfer_qry = """INSERT into Transit_Walk(from_node, to_node, "length",ref_link, geo)
                              SELECT Node_a, Node_b, distance, link_id, geo from OSM_Walk"""

        range_query = f"Update Transit_Walk set walk_link=ROWID + {self.__walk_range}"

        conn.execute(transfer_qry)
        conn.execute(range_query)
        conn.commit()
        self.__rebuild(conn)
        self.activenet.emit(["update", "secondary", 1, "Copying links", self.master_msg_txt])
    ...
    def _read_links(self, conn: sqlite3.Connection) -> None:
        ...
        dt = list(conn.execute(get_walk_links_qry))
        logger.info("   Reading walk links")
        self.link_list.clear()

        self.activenet.emit(["start", "secondary", len(dt), "Reading links", self.master_msg_txt])
        self.line_idx = GeoIndex()
        for i, record in enumerate(dt):
            self.activenet.emit(["update", "secondary", i + 1, "Reading links"])
            lnk = WalkLink(record)
            self.link_list[lnk.walk_link] = lnk
            self.line_idx.insert(feature_id=lnk.walk_link, geometry=lnk.geo)
        self.__mlid__ = conn.execute("Select max(walk_link) + 1 from Transit_Walk").fetchone()[0]
    ...
    def _read_stops(self, conn: sqlite3.Connection) -> None:
        ...
        self.activenet.emit(["start", "secondary", tot, "Reading stops", self.master_msg_txt])
        ...
        for mode, table_name, get_qry in cases:
            ...
            for counter, record in enumerate(dt):
                master_counter += 1
                self.activenet.emit(["update", "secondary", master_counter, "Reading stops", self.master_msg_txt])
                node = WalkNode()
    ...
    def _break_long_links(self):
        msg_txt = "Breaking long links"
        self.activenet.emit(["start", "secondary", len(self.link_list.items()), msg_txt, self.master_msg_txt])

        for j, (_, link) in enumerate(self.link_list.items()):
            self.activenet.emit(["update", "secondary", j + 1, msg_txt, self.master_msg_txt])
            ...
            for nodes in range(parts - 1):
                ...
                new_stop.zone = self.geo.get_zone(new_stop.geo)
    ...
    def _break_links_at_stops(self) -> list:
        ...
        excess_added = []
        for j, (link_id, link) in enumerate(self.link_list.items()):
            self.activenet.emit(["update", "secondary", j + 1, msg_txt, self.master_msg_txt])
    ...
    def _save_changes(self, conn: sqlite3.Connection):
        msg_txt = "Saving new walk links and nodes"
        tot = len(self.__delete_links) + len(self.__new_links) + len(self.__new_stops)
        self.activenet.emit(["start", "secondary", tot, msg_txt, self.master_msg_txt])

        j = 0
        for k, old_link in enumerate(self.__delete_links):
            self.activenet.emit(["update", "secondary", j + k, msg_txt, self.master_msg_txt])
            old_link.delete(conn, False)
        j += len(self.__delete_links)

        self.__delete_links.clear()
        conn.commit()

        for k, new_link in enumerate(self.__new_links):
            self.activenet.emit(["update", "secondary", j + k, msg_txt, self.master_msg_txt])
            new_link.save(conn, False)
        self.__new_links.clear()
        j += len(self.__new_links)

        for k, new_stop in enumerate(self.__new_stops):
            self.activenet.emit(["update", "secondary", j + k, msg_txt, self.master_msg_txt])
            new_stop.save(conn, False)
        self.__new_stops.clear()
        conn.commit()
        self.activenet.emit(["update", "secondary", tot, msg_txt, self.master_msg_txt])
    ...
    def _connect_overlapping_stops(self, conn: sqlite3.Connection):
        ...
        self.activenet.emit(
            ["start", "secondary", len(self.nodes_list.items()), "Analyzing overlapping stops", self.master_msg_txt]
        )

        for counter, (stop_id, stop) in enumerate(self.nodes_list.items()):
            self.activenet.emit(["update", "secondary", counter + 1, "Analyzing overlapping stops"])
        ...
        self.activenet.emit(
            ["start", "secondary", len(candidate_pairs.items()), "Connecting overlapping stops", self.master_msg_txt]
        )

        for i, (node1, node2) in enumerate(candidate_pairs.keys()):
            self.activenet.emit(["update", "secondary", i + 1, "Connecting overlapping stops"])
    ...
    def __add_nodes_split_link(self, distances: List[float], link: WalkLink, new_nodes: List[WalkNode]):
        ...
        self.__delete_links.append(link)
        p = 0.0
        for new_node, avg in zip(sorted_nodes, index):
            avg = round(avg, 6)
            if avg - p <= 0:
                logger.warning(f"Trying to break a link before its start. Check link: {link.ref_link}")
                raise Exception
    ...
    def copy_to_bike(self, conn: sqlite3.Connection):
        ...
        df.rename(columns={"walk_link": "bike_link"}, inplace=True)
        df["bike_link"] += self.__bike_range - self.__walk_range
