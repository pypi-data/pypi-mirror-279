...
class Location(LocParkBase):
    ...
    def update_location_links(self, conn: Connection, distance_threshold=2000, multiple=True, clear=True) -> None:
        ...
        links = self._create_multiple_location_links(distance_threshold)
        sql = "Insert into Location_Links(location, link, distance) VALUES (?,?,0)"
        conn.executemany(sql, links)
        conn.commit()

    def _create_multiple_location_links(self, distance_threshold: float) -> list:
        links_layer = self.geotool.get_layer("link")[["link", "use_codes", "geo"]]
        locs = gpd.GeoDataFrame(pd.DataFrame({"location": [self.id]}), geometry=[self.geo], crs=links_layer.crs)
    ...
    def update_location_parking(self, conn: Connection, parking_distance_from_location: float, commit=True) -> list:
        ...
        for park_id in self.geotool.get_parkings(self.geo, parking_distance_from_location):
            ...
            if park.type == "location":
                ...
                loc = self.geotool.get_locations(park.geo, 1)[0]
