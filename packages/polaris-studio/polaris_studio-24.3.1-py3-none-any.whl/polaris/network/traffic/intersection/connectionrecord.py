...
from ...database_connection import get_srid
...
class ConnectionRecord:
    ...
    def __init__(self, link: Approximation, to_link: Approximation, lanes="", to_lanes=""):
        ...
        self.from_geo = LineString(link.geo if link.direction == 0 else link.geo.coords[::-1])
        self.to_geo = LineString(to_link.geo if to_link.direction == 0 else to_link.geo.coords[::-1])
    ...
    def save_to_database(self, conn: Connection) -> None:
        ...
        conn.execute(self.insert_sql, self.data + [get_srid(conn=conn)])
        conn.commit()
