...
from polaris.network.traffic.intersection.connectionrecord import conn_table_fields
...
class Intersection:
    ...
    def __init__(
        self, data_tables, path_to_file: PathLike, conn: Optional[sqlite3.Connection] = None, osm: Optional[OSM] = None
    ):
        self.u_turn = False
        self.node = -1
        ...
        self.__osm = osm or OSM(path_to_file)
    ...
    def rebuild_connections(self, clear_it_first=True, conn: Optional[sqlite3.Connection] = None) -> None:
        ...
        logger.info(f"Rebuilt connections for node {self.node}")
    ...
    def __connection_rebuilding(self, clear_it_first: bool, conn: sqlite3.Connection):
        ...
        if not data.shape[0]:
            raise Exception("Node is not connected to any links")
    ...
    def osm_signal(self, buffer=30, re_check=False, return_when_fail=False) -> bool:
        ...
        if self.__osm_has_it is None or re_check:
            closest_traffic_light = self.__osm.get_traffic_signal(self.geo)
        ...
        return return_when_fail if self.__osm.failed else self.__osm_has_it
    ...
    def delete_stop_sign(self, conn: sqlite3.Connection):
        ...
        logger.info(f"Deleted stop sign for node {self.node}")
    ...
    def __connection_inventory(self, conn: sqlite3.Connection):
        ...
        sql = """SELECT ? node, link, lanes_ab, ST_AsBinary(geo), lt.rank, lt.turn_pockets, 0 direc, 'incoming', bearing_b FROM Link l
                 ...
                 UNION ALL
                 SELECT ? node, link, lanes_ba, ST_AsBinary(geo), lt.rank, lt.turn_pockets, 1 direc, 'incoming', bearing_a + 180  FROM Link l
                                                                    INNER JOIN link_type lt on l.type=lt.link_type
                 where l.lanes_ba>0 and l.node_a=? AND (lt.use_codes like '%AUTO%' or lt.use_codes like '%TRUCK%' or
                 lt.use_codes like '%BUS%' or lt.use_codes like '%HOV%' or lt.use_codes like '%TAXI%')"""

        self.incoming = [Approximation(item) for item in conn.execute(sql, data).fetchall()]

        sql = """SELECT ? node, link, lanes_ab, ST_AsBinary(geo), lt.rank, lt.turn_pockets, 0 direc, 'outgoing', bearing_a FROM Link l
                                                              INNER JOIN link_type lt on l.type=lt.link_type
                 where l.lanes_ab>0 and l.node_a=? AND (lt.use_codes like '%AUTO%' or lt.use_codes like '%TRUCK%' or
                 lt.use_codes like '%BUS%' or lt.use_codes like '%HOV%' or lt.use_codes like '%TAXI%')
                 UNION ALL
                 SELECT ? node, link, lanes_ba, ST_AsBinary(geo), lt.rank, lt.turn_pockets, 1 direc, 'outgoing', bearing_b + 180 FROM Link l
                                                                    INNER JOIN link_type lt on l.type=lt.link_type
                 where l.lanes_ba>0 and l.node_b=? AND (lt.use_codes like '%AUTO%' or lt.use_codes like '%TRUCK%' or
                 lt.use_codes like '%BUS%' or lt.use_codes like '%HOV%' or lt.use_codes like '%TAXI%')
                 """
        self.outgoing = [Approximation(item) for item in conn.execute(sql, data).fetchall()]
    ...
    def _creates_connections(self, conn: sqlite3.Connection) -> pd.DataFrame:
        ...
        # This is a simple link merge, where there is no much option to fix this
            ...
            connections = GenericIntersection(self).build(conn)
    ...
    def connection_data_records(self):
        if "_connection_data" not in self.__dict__:
    ...
    def connections(self, conn: sqlite3.Connection):
        if "_connection_data" not in self.__dict__:
            ...
            fields.append("round(Degrees(ST_Azimuth(StartPoint(geo), ST_PointN(SanitizeGeometry(geo), 2))),0) bearing")
