...
class Signal:
    ...
    def __init__(self, intersection, conn: sqlite3.Connection):
        ...
        snr = self._data_tables.get_table("Signal_Nested_Records", conn=conn)  # type: pd.DataFrame
        sig_nested = snr[snr.object_id == self.signal]

        self.records = [SignalRecord(record[1].to_dict(), self, conn) for record in sig_nested.iterrows()]
    ...
    def add_period(self, value_start: int, value_end: int, conn: sqlite3.Connection):
        ...
        sig_nested: pd.DataFrame = self._data_tables.get_table("Signal_Nested_Records")
