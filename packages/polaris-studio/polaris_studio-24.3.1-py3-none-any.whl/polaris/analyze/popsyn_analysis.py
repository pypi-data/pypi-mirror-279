...
class PopsynComparator:
    ...
    PERSON_SURVEY_ID = "id"
    # the name census_zone comes from supply_db locations and refers to tract ids; puma_id is created below
    CENSUS_TRACT_ID = "census_zone"
    ...
    def __init__(
        ...
        linker: Optional[LinkerFile] = None,
    ):
        self.linker = linker or LinkerFile.from_dir(result_dir)
    ...
    def attach_geo_fields(self):
        ...
        if self.locations_use_tract_ids():
            ...
            # locations are POIs, census_zone are census tract numbers.
        ...
        # I found one nan in DFW with census_zone = 0 - drop these and warn
        ...
        if no_location_filter.any():
            logging.warning(
                ...
                + ", dropping these now. Census_zone ids for these should be zero, unique values found:"
    ...
    def plot_scatter(
        ...
        with plt.style.context(plt_style):  # type: ignore
            ...
            reg and ax.axline(xy1=(0, reg.intercept), slope=reg.slope)
