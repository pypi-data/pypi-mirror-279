...
# TODO As we don't have google_contact_table can we avoid inheriting GenericCRUD?
class GoogleContacts(GenericCRUD, metaclass=MetaLogger,
    ...
    # I think we should move the goups_str to the public sync method as we might want to use the same class with multiple acocunts / groups_str
    def __init__(self):
        ...
        self.organization_profile = OrganizationProfilesLocal()
        self.contact_persons = ContactPersonsLocal()
        self.contact_user_external = ContactUserExternalLocal()
        self.contact_profiles = ContactProfilesLocal()
        self.contact_group = ContactGroups()
        self.contact_email_addresses = ContactEmailAdressesLocal()
        self.contact_phones = ContactPhoneLocal()
        self.contact_location = ContactLocationLocal()
        self.user_externals_local = UserExternalsLocal()
        self.organizations_local = OrganizationsLocal()
        self.domain = DomainLocal()
        ...
        self.google_client_id = our_get_env("GOOGLE_CLIENT_ID")
        self.google_client_secret = our_get_env("GOOGLE_CLIENT_SECRET")
        self.google_port_for_authentication = int(our_get_env("GOOGLE_PORT_FOR_AUTHENTICATION"))
        self.google_redirect_uris = our_get_env("GOOGLE_REDIRECT_URIS")
        self.google_auth_uri = our_get_env("GOOGLE_AUTH_URI")
        self.google_token_uri = our_get_env("GOOGLE_TOKEN_URI")
        self.google_contact_seq_start = int(our_get_env("GOOGLE_CONTACT_SEQ_START"))
        self.google_contact_seq_end = int(our_get_env("GOOGLE_CONTACT_SEQ_END"))
        ...
        super().__init__(default_schema_name="user_external", default_table_name="user_external_table",
                         default_view_table_name="user_external_view", default_column_name="user_external_id")
        self.google_system_id = self.select_one_value_by_column_and_value(
            schema_name="system", view_table_name="system_ml_view",
            select_clause_value="system_id", column_name="title", column_value="Google")
        self.people_api_id = self.select_one_value_by_column_and_value(
            schema_name="api_type", view_table_name="api_type_view",
            select_clause_value="api_type_id", column_name="name", column_value="Google People (Contact) Sync")
        self.data_source_type_id = self.select_one_value_by_column_and_value(
            schema_name="data_source", view_table_name="data_source_type_ml_en_view",
            select_clause_value="data_source_type_id", column_name="name", column_value="Google Contact- People API")
        self.contact_entity_type_id = self.select_one_value_by_column_and_value(
            ...
            select_clause_value="entity_type_id", column_name="entity_type_name", column_value="Contact")
    ...
    def authenticate(self):
        ...
        if not self.creds or not self.creds.valid:
            ...
            # TODO Please make sure we keep in the database the refresh token
            ...
            if access_token:
                ...
                user_externals_local.insert_or_update_user_external_access_token(
                    system_id=self.GOOGLE_CONTACT_SYSTEM_ID,
                    username=self.email,
        ...
        if not self.creds or not self.creds.valid:
            if self.creds and self.creds.expired and self.creds.refresh_token:
                ...
                select_result_dict = self.select_one_dict_by_where(
                    ...
                    params=(email, self.google_system_id), order_by="user_external_id DESC")
                ...
                if not self.creds and refresh_token:
                    ...
                    try:
                        ...
                        self.logger.error("Refresh token failed.", object={"exception": exception})
            ...
            if access_token:
                self.user_externals_local.insert_or_update_user_external_access_token(
                    system_id=self.google_system_id,
                    username=self.email,
                    profile_id=self.user_context.get_effective_profile_id(),    # We can't get profile_id by email for play1@circ.zone because it's not in profile_view, this method will always select from view
                    access_token=access_token,
                    expiry=expires_in,
                    refresh_token=refresh_token)
                # TODO Error handling of the above call
    ...
    def create_dict_representation(contact):
        ...
        last_modified_timestamp = sources[0].get(
            'updateTime') if sources and len(sources) > 0 else None
        last_modified_timestamp = GoogleContacts.__get_formatted_timestamp(last_modified_timestamp)
    ...
    def pull_people_api(self, email: str, groups_dicts: list[dict] = None) -> str:
        self.logger.start(log_message=f"Start of pull_people_api",
                          object={"email": email, "groups_dicts": groups_dicts})
        ...
        user_external_id_dict = self.select_one_dict_by_column_and_value(select_clause_value="user_external_id",
                                                                         column_name="username",
                                                                         column_value=email,
                                                                         order_by="user_external_id DESC")
        user_external_id = user_external_id_dict["user_external_id"]
        data_source_instance_id = self.__insert_data_source_instance_id(email=email, user_external_id=user_external_id)
        ...
        while next_google_contact_index <= self.google_contact_seq_end:
            try:
                ...
                # TODO Can we get the contacts from Google based on a field (sorted by something)? LAST MODIFIED DESC (we want to import the latest modify first)
                # TODO Please implement 
                google_connections = results.get('connections', [])
                page_token = results.get('nextPageToken')
                self.logger.info(log_message=f"Next page token: {page_token}",
                ...
                for index, google_contact_connection in enumerate(google_connections):
                    ...
                    if next_google_contact_index < self.google_contact_seq_start or next_google_contact_index > self.google_contact_seq_end:
                        next_google_contact_index += 1
                        continue
                    next_google_contact_index += 1
                    self.logger.info(log_message=f"Importing contact with index {index}", object={"index": index})
                    ...
                    contact_dict["data_source_type_id"] = self.data_source_type_id
                    contact_dict["owner_profile_id"] = self.profile_id
                    contact_dict["system_id"] = self.google_system_id
        ...
        self.logger.info(log_message=f"End of pull_people_api, page_token: {page_token}",
                         object={"page_token": page_token})
        return page_token
    ...
    def __insert_contact_details_to_db(self, contact_dict: dict, user_external_id: int,
        ...
        try:
            ...
            GoogleContacts.__insert_link_contact_notes_and_text_blocks(contact_dict=contact_dict)
    ...
    def __insert_data_source_instance_id(self, email: str, user_external_id: int) -> int:
        # name has to be "YYMMDD HHMMSS email  Google Contacts - People API"
        name = datetime.now(ZoneInfo("UTC")).strftime("%y%m%d %H%M%S") + " " + email + " Google Contacts - People API"
        data_source_instance_id = self.insert(
            schema_name="data_source_instance",
            table_name="data_source_instance_table",
            data_dict={
                "name": name,
                "data_source_type_id": self.data_source_type_id,
                "file_or_api": "api",
                "computer_name": None,
                "user_external_id": user_external_id,
    ...
    def __get_google_connections(self, user_external_id, authentication_auth_login_endpoint_url, headers,
                                 outgoing_body, incoming_message, page_token):
        ...
        self.logger.info('Listing all connection names along with their emails and phone numbers')
        indirect = InDirect()
        api_check, api_call_id, http_status_code, response_body = indirect.before_call_api(
            user_external_id=user_external_id,
        ...
        if http_status_code is None:
            ...
            if api_check != APILimitStatus.GREATER_THAN_HARD_LIMIT:
                ...
                seconds_to_sleep_after_passing_the_hard_limit = (
                    APIManagementsManager().get_seconds_to_sleep_after_passing_the_hard_limit(
                        api_type_id=self.people_api_id)
                )
        ...
        self.logger.info(log_message="Results from Google Contacts API", object={"results": results})

        indirect.after_call_api(user_external_id=user_external_id,
                                api_type_id=self.people_api_id,
                                endpoint_url=authentication_auth_login_endpoint_url, outgoing_header=headers,
                                outgoing_body=outgoing_body,
                                incoming_message=incoming_message,
    ...
    def pull_contacts_with_stored_token(self, email: str, groups_dicts: list[dict] = None,
        ...
        self.profile_id = ProfilesLocal().select_one_value_by_column_and_value(
            ...
            column_value=email)
        self.logger.info("Getting token data from DB", object={
            "email": email, "profile_id": self.user_context.get_effective_profile_id()})

        token_data = self.user_externals_local.get_auth_details(  # TODO: fix typing
            username=email, system_id=self.google_system_id, profile_id=self.user_context.get_effective_profile_id())
        if not token_data:
        ...
        self.pull_people_api(email=email, groups_dicts=groups_dicts)
        
        # TODO logger info with everything done "data_source_instance" "number_of_contacts_sync_from_google_to_circles" "number_contacts_sync_from_circlrs_to_google_contacts"
    ...
    def __insert_organization_profile(self, organization_id: int, profiles_ids_list: list[int]) -> Optional[list[int]]:

        if not organization_id or not profiles_ids_list:
            return

        organization_profiles_ids = self.organization_profile.insert_multiple_mappings_if_not_exists(
            organizations_ids=[organization_id], profiles_ids=profiles_ids_list)

        return organization_profiles_ids

    # TODO Shall we move it to contact_group repo? So both csv repo and this repo will use those methods - same to the other methods here.
    ...
    def __insert_link_contact_persons(self, contact_dict: dict) -> dict:
        ...
        contact_person_result_dict = self.contact_persons.insert_contact_and_link_to_existing_or_new_person(
            contact_dict=contact_dict,
    ...
    def __insert_link_contact_email_addresses(self, contact_dict: dict) -> None:
        ...
        if email2:
            self.contact_email_addresses.insert_contact_and_link_to_email_address(
                contact_dict=contact_dict,
                contact_email_address_str=email2
            )
        if email3:
            self.contact_email_addresses.insert_contact_and_link_to_email_address(
                contact_dict=contact_dict,
                contact_email_address_str=email3
            )

    @staticmethod
    # TODO Same
    def __insert_link_contact_notes_and_text_blocks(contact_dict: dict) -> Optional[int]:
    ...
    # TODO Same
    def __insert_link_contact_phones(self, contact_dict: dict) -> None:
        phone_dict1 = contact_dict.get("phone_dict1")
        phone_number_original1 = phone_dict1.get("canonicalForm") if phone_dict1 else None
        phone_dict2 = contact_dict.get("phone_dict2")
        phone_number_original2 = phone_dict2.get("canonicalForm") if phone_dict2 else None
        phone_dict3 = contact_dict.get("phone_dict3")
        phone_number_original3 = phone_dict3.get("canonicalForm") if phone_dict3 else None
    ...
    def __insert_contact_profiles(self, contact_dict: dict) -> dict:
        ...
        insert_information = self.contact_profiles.insert_and_link_contact_profile(contact_dict=contact_dict) or {}
    ...
    def __insert_link_contact_domains(self, contact_dict: dict) -> list[dict]:
        ...
        while website_url:
            domain_insert_information = self.domain.link_contact_to_url(contact_id=contact_id,
    ...
    # TODO Same
    def __insert_link_contact_location(self, contact_dict: dict) -> Optional[list[dict]]:
        contact_id = contact_dict.get("contact_id")
    ...
    def __insert_importer(self, contact_id: int, location_id: int, user_external_id: int,
                          data_source_instance_id: int, google_people_api_resource_name: str) -> int:
    ...
    def __conflict_resolution(self, last_modified_timestamp: str, contact_id: int) -> UpdateStatus:
        ...
        update_status: UpdateStatus = sync_conflict_resolution.get_update_status(
            last_modified_timestamp=last_modified_timestamp,
            data_source_instance_id=self.data_source_type_id,
            entity_type_id=self.contact_entity_type_id,
