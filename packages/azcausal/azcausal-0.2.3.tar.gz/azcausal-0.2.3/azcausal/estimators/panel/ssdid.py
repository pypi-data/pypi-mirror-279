...
from azcausal.estimators.panel.fsdid import demean, solve
...
class SSDID(DID):
    ...
    def fit(self,
        ...
        df, post, treat = panel['outcome'], panel.post, panel.treat
        ...
        if omega is None:
            dz = demean(df.loc[~post])
            omega = solve(dz.loc[~treat], dz.loc[~treat].mean(axis=0), alpha=noise * (n_treat * n_post) ** (1 / 4))
        if lambd is None:
            dz = demean(df.T.loc[~treat])
            lambd = solve(dz.loc[~post], dz.loc[post].mean(axis=0), alpha=noise * 1e-06)

        # get the results and also store omega as well as lambd
        result = super().fit(panel, lambd=lambd, omega=omega, by_time=by_time)
        result.effect.data.update(dict(lambd=lambd, omega=omega))
        return result
