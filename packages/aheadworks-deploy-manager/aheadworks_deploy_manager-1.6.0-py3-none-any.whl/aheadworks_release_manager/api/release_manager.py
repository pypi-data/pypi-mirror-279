...
class ReleaseManager:
    ...
    def __init__(self, jira_api_config):
        self.jira_api_manager = JiraApiManager(config=jira_api_config)
        self.teamwork_manager = TeamworkManager(jira_api_config.user, jira_api_config.token)
    ...
    def teamwork_release(self, project_id, composer_file, discord_bot_url, path_to_files, assign_to):
        ...
        for l in links:
            for t in l['tags']:
                tag_name = t['name']
                if tag_name in [self.PD_TASK_LABEL, self.TEST_TASK_LABEL, self.RELEASE_PACK_TASK_LABEL]:
                    release_tasks[tag_name] = l['id']
                    release_links.append(self.teamwork_manager.get_issue_url(l['id']))
                if tag_name == self.RELEASE_PACK_TASK_LABEL:
                    project_name = l['project']['name']

        if len(release_tasks) < 3:
            print(release_tasks)
            raise Exception('Some release tasks are missing.')

        release_issue = release_tasks[self.RELEASE_PACK_TASK_LABEL]

        # add attachments to the task RELEASE-PACK
        file_names = os.listdir(path_to_files)
        self.teamwork_manager.add_attachments_to_task(release_issue, file_names)

        # assign release pack to user
        self.teamwork_manager.reassign(release_issue, assign_to)
        ...
        self.teamwork_manager.add_comment(release_issue, f'Composer Package Name:\n{composer_package_name}')

        # Set PD and TEST to Done
        self.teamwork_manager.close_issue(release_tasks[self.PD_TASK_LABEL])
        self.teamwork_manager.close_issue(release_tasks[self.TEST_TASK_LABEL])
    ...
    def jira_release(self, jira_project_key, composer_file, discord_bot_url, path_to_files, assign_to):
        ...
        # Get 3 release tasks: RELEASE-PACK, PD and TEST
        ...
        if (not assign_to == "JIRA_ACCOUNT_ID"):
    ...
    def build_metapackage_pack(self, bitbucket_workspace, bitbucket_repo_slug, artifacts_dir):
        ...
        for full_module_name in composer["require"]:
            ...
            if old_name != conflicted_module and bitbucket_repo_slug != "b2b-suite":
            ...
            elif old_name != conflicted_module and bitbucket_repo_slug == "b2b-suite":
            ...
            if bitbucket_repo_slug != "b2b-suite":
                composer = self._load_composer_file(bitbucket_repo_slug, '.')
                self._replace_dependencies(f"{module_path}/composer.json", composer["require"], "subscription")
