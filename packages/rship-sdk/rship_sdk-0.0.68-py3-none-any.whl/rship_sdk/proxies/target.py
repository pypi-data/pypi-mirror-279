...
class TargetProxy():
    ...
    def add_emitter(self, args: EmitterArgs):
        emitter_proxy = EmitterProxy(self, args, self.client)
        emitter_proxy.save()
        return emitter_proxy

    # def add_action(self, args: EmitterArgs):
    #    return

    def save(self, status: TargetStatusEnum):
        target = Target(
          id=self.id(),
          name=self.args.name,
