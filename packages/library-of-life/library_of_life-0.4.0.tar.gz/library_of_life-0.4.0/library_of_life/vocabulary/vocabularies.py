...
class Vocabularies:
    ...
    def list_all_vocabularies(self, name: Optional[str]=None,
    ...
    def create_new_vocabulary(self, username, password, payload):
        ...
        if self.auth_type == "basic":
            ...
            response = hc.post_with_auth_and_json(base_url+self.endpoint, auth=auth, json=payload)
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def restore_deprecated_vocabulary(self, name, username, password, restore_deprecated_concepts: Optional[bool]=None):
        """
    ...
    def list_vocabulary_releases(self, name, version: Optional[str]=None,
    ...
    def export_single_vocabulary_release(self, name, version):
        ...
        response = hc.get_for_content(base_url+self.endpoint+resource).decode("utf-8")
        if response == "":
