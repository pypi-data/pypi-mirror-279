...
class Concepts:
    ...
    def create_new_concept(self, vocabulary_name, username, password, payload):
        ...
        if self.auth_type == "basic":
            ...
            response = hc.post_with_auth_and_json(base_url+self.endpoint+resource, auth=auth, json=payload)
            ...
            response = hc.post_with_auth_and_json(base_url+self.endpoint+resource, headers=headers, json=payload)
            if "400" in response["error"]:
                return "Bad request: the JSON is invalid or the request is not well-formed."
            elif "422" in response["error"]:
                return "The request is syntactically correct but the fields are invalid (required fields not set, duplicated keys, inconsistent keys, etc.)"
    ...
    def get_details_of_concept(self, vocabulary_name, name,
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def update_existing_concept(self, vocabulary_name, name, username, password, payload):
        """
        ...
        if self.auth_type == "basic":
            auth = (username, password)
            response = hc.put_with_auth_and_json(base_url+self.endpoint+resource, auth=auth, json=payload)
            if "400" in response["error"]:
                return "Bad request: the JSON is invalid or the request is not well-formed."
            elif "422" in response["error"]:
                return "The request is syntactically correct but the fields are invalid (required fields not set, duplicated keys, inconsistent keys, etc.)"
            ...
            headers = self.auth_headers
            response = hc.post_with_auth_and_json(base_url+self.endpoint, headers=headers, json=payload)
            if "400" in response["error"]:
                return "Bad request: the JSON is invalid or the request is not well-formed."
            elif "422" in response["error"]:
                return "The request is syntactically correct but the fields are invalid (required fields not set, duplicated keys, inconsistent keys, etc.)"
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def deprecate_existing_concept(self, vocabulary_name, name, username, password, payload=None):
        """
        ...
        if self.auth_type == "basic":
            ...
            headers = self.auth_headers
            response = hc.put_with_auth_and_json(base_url+self.endpoint, headers=headers, json=payload)
            if "400" in response["error"]:
                return "Bad request: the JSON is invalid or the request is not well-formed."
            elif "422" in response["error"]:
                return "The request is syntactically correct but the fields are invalid (required fields not set, duplicated keys, inconsistent keys, etc.)"
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def restore_deprecated_concept(self, vocabulary_name, name, username, password, restore_deprecated_children: Optional[bool]=None):
        """
        ...
        if self.auth_type == "basic":
            auth = (username, password)
            response = hc.delete_with_auth(base_url+self.endpoint+resource, auth=auth, params=params)
            if "400" in response["error"]:
                return "Bad request: the JSON is invalid or the request is not well-formed."
            elif "422" in response["error"]:
                return "The request is syntactically correct but the fields are invalid (required fields not set, duplicated keys, inconsistent keys, etc.)"
            ...
            headers = self.auth_headers
            response = hc.delete_with_auth(base_url+self.endpoint, headers=headers, params=params)
            if "400" in response["error"]:
                return "Bad request: the JSON is invalid or the request is not well-formed."
            elif "422" in response["error"]:
                return "The request is syntactically correct but the fields are invalid (required fields not set, duplicated keys, inconsistent keys, etc.)"
    ...
    def add_definition_to_concept(self, vocabulary_name, name, username, password, payload):
        """
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def update_concept_definition(self, vocabulary_name, name, key, username, password, payload):
        """
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def unlink_tag_from_concept(self, vocabulary_name, name, tag_name, username, password):
        """
    ...
    #Requires authentication. User must have an account with GBIF.
    def add_alternative_label_to_concept(self, vocabulary_name, name, username, password, payload):
        """
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def delete_alternative_label_from_concept(self, vocabulary_name, name, key, username, password):
        """
    ...
    #Requires authentication. User must have an account with GBIF.
    def add_hidden_label_to_concept(self, vocabulary_name, name, username, password, payload):
        """
    ...
    # Requires authentication. User must have an account with GBIF with the proper credentials.
    def delete_hidden_label_from_concept(self, vocabulary_name, name, key, username, password):
        """
    ...
    def suggest_concepts_from_latest_vocabulary_release(self, vocabulary_name, locale,
    ...
    def get_details_of_concept_from_vocabulary_latest_release(self, vocabulary_name, name,
                                                              include_parents: Optional[bool]=None,
                                                              include_children: Optional[bool]=None):
        """
    ...
    def list_definition_of_concept_from_latest_release_of_vocabulary(self, vocabulary_name, name, language):
        """
    ...
    def list_all_labels_of_concept_from_latest_release_of_vocabulary(self, vocabulary_name, name, language):
        """
    ...
    def list_all_alternative_labels_of_concept_from_latest_release_of_vocabulary(self, vocabulary_name, name, language,
    ...
    def list_all_hidden_labels_of_concept_from_latest_release_of_vocabulary(self, vocabulary_name, name,
                                                                                  limit: Optional[int]=None,
                                                                                  offset: Optional[int]=None):
        """
