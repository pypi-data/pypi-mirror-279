...
VENV := ~/.virtualenvs/snapbox/bin/activate
...
rebuild_docs:
	source $(VENV) && sphinx-apidoc . --full -o docs -H 'SnapBox' -A 'Artur Barseghyan <artur.barseghyan@gmail.com>' -f -d 20
...
update-version:
	...
	sed -i 's/__version__ = "[0-9.]\+"/__version__ = "$(VERSION)"/' snapbox.py
