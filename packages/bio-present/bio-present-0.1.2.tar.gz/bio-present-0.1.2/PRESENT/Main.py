...
def PRESENT_function(
    ...
    """
    PRESENT: Cross-modality representation of spatially resolved omics data
    ...
    ------
    adata
        AnnData containing the latent representation matrix in adata.obsm['embeddings']
    """

    print("Loading data and parameters...")
    device = torch.device("cuda") if device.lower() == "cuda" else torch.device("cpu")
    ...
    if adata_rna is not None:
        ...
        adata = sc.AnnData(pd.DataFrame(embeddings, index=index), obs=adata_atac.obs.copy(), obsm=adata_rna.obsm)
    else:
        adata = sc.AnnData(pd.DataFrame(embeddings, index=index), obs=adata_adt.obs.copy(), obsm=adata_rna.obsm)
    adata.obsm["embeddings"] = embeddings
    adata = run_leiden(adata, n_cluster=nclusters, use_rep="embeddings", key_added="LeidenClusters")
    return adata


def PRESENT_BC_function(
        spatial_key: str="spatial",
        batch_key: str="batch",
        adata_rna: Union[str, ad.AnnData, NoneType] = None,
        adata_atac: Union[str, ad.AnnData, NoneType] = None, 
        adata_adt: Union[str, ad.AnnData, NoneType] = None,
        gene_min_cells: Union[int, NoneType] = 1,
    ...
    """
    PRESENT-BC: multi-sample integration of spatially resolved omics data
    ...
    if adata_rna is not None: 
        adata_rna = adata_rna[index, :]
        rna_dim = adata_rna.shape[1]
        if batch_key in adata_rna.obs.keys(): batch_indices = adata_rna.obs[batch_key].values.astype(str)
        if spatial_key in adata_rna.obsm.keys(): spatial_mtx = adata_rna.obsm[spatial_key]
    if adata_atac is not None: 
        adata_atac = adata_atac[index, :]
        cas_dim = adata_atac.shape[1]
        if batch_key in adata_atac.obs.keys(): batch_indices = adata_atac.obs[batch_key].values.astype(str)
        if spatial_key in adata_atac.obsm.keys(): spatial_mtx = adata_atac.obsm[spatial_key]
    if adata_adt is not None: 
        adata_adt = adata_adt[index, :]
        adt_dim = adata_adt.shape[1]
        if batch_key in adata_adt.obs.keys(): batch_indices = adata_adt.obs[batch_key].values.astype(str)
        if spatial_key in adata_adt.obsm.keys(): spatial_mtx = adata_adt.obsm[spatial_key]
    assert spatial_mtx is not None, "Please provide spatial mtx in the adata_rna/adata_atac/adata_adt.obsm under spatial_key"
    assert batch_indices is not None, "Please provide batch indices in the adata_rna/adata_atac/adata_adt.obs under batch_key"
    n_batches = np.unique(batch_indices).shape[0]
    print("Input data has been loaded")
    ...
    embeddings, omics_lat, omics_impute = model.model_train(spa_mat=spatial_mtx, 
                                                            rna_counts=adata_rna.X,
                                                            cas_counts=adata_atac.X,
                                                            adt_counts=adata_adt.X,
