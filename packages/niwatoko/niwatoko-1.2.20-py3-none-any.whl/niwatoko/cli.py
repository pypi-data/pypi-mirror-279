            ...
            if len(parts) == 2:
                ...
                if '[' in import_path and not import_path.startswith('['):
                    ...
                    if extension == 'md':
                        ...
                        my_logger("その他のファイルタイプとして処理", level='DEBUG')
                    ...
                    if not os.path.exists(import_path):
                        ...
                        my_logger(f"パッケージの最上位ディレクトリを取得します: {package_first_dir}", level='DEBUG')
                        grimo_dir = os.path.join(package_first_dir, 'grimo/grimoires')
                        my_logger(f"grimoディレクトリのパス: {grimo_dir}", level='DEBUG')

                        package_dir = os.path.join(grimo_dir, import_path)
                        my_logger(f"パッケージディレクトリのパス: {package_dir}", level='DEBUG')
                        
                        version_dirs = [d for d in os.listdir(package_dir) if os.path.isdir(os.path.join(package_dir, d)) and d.count('.') == 2]
                        my_logger(f"バージョンディレクトリのリスト: {version_dirs}", level='DEBUG')
                        
                        latest_version_dir = sorted(version_dirs, key=lambda s: list(map(int, s.split('.'))), reverse=True)[0]
                        my_logger(f"最新バージョンディレクトリ: {latest_version_dir}", level='INFO')
                        
                        import_path = os.path.join(grimo_dir, import_path, latest_version_dir, "main.md")
                    ...
                    # 拡張子が指定されていない場合、新しい関数を使用して処理
                        ...
                        my_logger("拡張子が指定されていないため、process_no_extension_importを使用", level='DEBUG')
