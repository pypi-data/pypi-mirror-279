...
def find_latest_version_path(import_path):
    ...
    # もしimport_pathが './' で始まる場合、先頭の './' を削除します。
    # 例: import_pathが './example' の場合、'example' に変換されます。
    if import_path.startswith('./'):
        import_path = import_path[2:]
    
    # grimo_dirとimport_pathを結合してpackage_dirを作成します。
    # 例: grimo_dirが '/path/to/grimo/grimoires' で、import_pathが 'example' の場合、
    # package_dirは '/path/to/grimo/grimoires/example' になります。
...
def process_imports(file_path, model_input_image):
    ...
    """
    my_logger(
        f"process_imports関数が呼び出されました。ファイルパス: {file_path}, モデル入力画像: {model_input_image}",
        level="DEBUG",
    )
        ...
        if line.startswith("- "):
            ...
            if len(parts) == 2:
                ...
                if "[" in import_path and "]" in import_path:
                    ...
                    # インポート元のファイルパスから絶対パスを解決
                    if not os.path.isabs(import_path):
                        import_path = os.path.join(os.path.dirname(file_path), import_path)
                        my_logger(f"相対パスを絶対パスに変換しました: {import_path}", level="DEBUG")

                    # ファイルが存在しない場合は、find_latest_version_path を使って grimo/grimoires 内を探す
                    if not os.path.exists(import_path):
                        ...
                        if not import_path:
                            ...
                            raise FileNotFoundError(
                                f"\033[91m{import_path} が見つかりません。\n現在のディレクトリに配置するか、grimo install package_name -v version でインストールしてください。\033[0m"
                            )

                # 再帰的にインポート処理を実行
                output.extend(
                    process_import_by_extension(
                        import_path, line, model_input_image, recursive=True
                    )
                )
            else:
                output.append(line)
...
def process_import_by_extension(import_path, line, model_input_image, recursive=False):
    ...
    # インポートパスから拡張子を抽出します。
    # 例: import_pathが "example.md" の場合、extensionは "md" になります。
    # 例: import_pathが "example" の場合、extensionは空文字列 "" になります。
...
def find_latest_version_path(import_path):
    ...
    Returns:
        str: 最新バージョンディレクトリへのパス。openai 
             見つからない場合はNoneを返します。
    """
    my_logger(
        f"find_latest_version_path関数が呼び出されました。import_path: {import_path}",
        level="DEBUG",
    )
    package_first_dir = os.path.dirname(
    ...
    # import_pathから先頭の './' を削除します。
    my_logger("import_pathから先頭の './' を削除します。", level='DEBUG')
    # 例: import_pathが './example' の場合、import_pathは 'example' になります。
    my_logger(f"変更前のimport_path: {import_path}", level='DEBUG')
    import_path = import_path.replace('./', '')
    my_logger(f"変更後のimport_path: {import_path}", level='DEBUG')
    ...
    latest_version_dir = sorted(
        version_dirs, key=lambda s: list(map(int, s.split("."))), reverse=True
    )[0]
