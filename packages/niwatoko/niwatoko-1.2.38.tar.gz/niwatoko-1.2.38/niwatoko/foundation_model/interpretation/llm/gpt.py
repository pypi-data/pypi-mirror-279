...
niwatoko_dir = os.path.dirname(niwatoko.__file__)  # niwatokoパッケージのディレクトリパスを取得
...
if not api_key:
    raise ValueError(
        "api_keyクライアントオプションは、クライアントにapi_keyを渡すか、"
        "OPENAI_API_KEY環境変数を設定することで設定する必要があります。"
        "詳細は以下のURLでOpenAI API KEYをセットしてください: "
        "https://platform.openai.com/api-keys, ANTHROPIC_API_KEYを設定してください: "
