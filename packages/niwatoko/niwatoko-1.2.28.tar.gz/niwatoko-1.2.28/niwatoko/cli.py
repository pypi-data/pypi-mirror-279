    ...
    for line in lines:
        ...
        if line.startswith('- '):
            ...
            if len(parts) == 2:
                ...
                # 拡張子による処理を関数に集約
                output.extend(process_import_by_extension(import_path, line, model_input_image))
            else:
                output.append(line)
        else:
            output.append(line)
...
def process_import_by_extension(import_path, line, model_input_image):
    """拡張子に応じてインポート処理を行います。
    """
    my_logger(f"process_import_by_extension関数が呼び出されました。import_path: {import_path}, line: {line.strip()}, model_input_image: {model_input_image}", level='DEBUG')
    ...
    if extension == 'md':
        ...
        return process_md_import(import_path, line)
    elif extension == 'py':
        my_logger("Pythonファイルとして処理", level='DEBUG')
        return process_py_import(import_path, line, model_input_image)
    elif extension == 'rst':
        my_logger("reStructuredTextファイルとして処理", level='DEBUG')
        return process_rst_import(import_path, line)
        ...
        return process_no_extension_import(import_path, line)
...
def process_variable_imports(line):
    ...
    if '{{' in line and '}}' in line:
        ...
        my_logger(f"インポートコンテンツ: {import_content}", level='DEBUG')
        output.extend([line, '```\n', import_content, '```\n'])
    return output
...
def process_no_extension_import(import_path, line):
    ...
    Returns:
        list: 処理後の出力行のリスト
    """
    my_logger(f"process_no_extension_import関数が呼び出されました。import_path: {import_path}, line: {line.strip()}", level='DEBUG')
    import_content = get_file_content(import_path)
    my_logger(f"インポートコンテンツ: {import_content}", level='DEBUG')
    return [line, '```\n', import_content, '```\n']
...
def process_md_import(import_path, line):
    ...
    Returns:
        list: 処理後の出力行のリスト
    """
    my_logger(f"process_md_import関数が呼び出されました。import_path: {import_path}, line: {line.strip()}", level='DEBUG')
    import_content = get_file_content(import_path)
    my_logger(f"インポートコンテンツ: {import_content}", level='DEBUG')
    return [line, '```\n', import_content, '```\n']
...
def process_py_import(import_path, line):
    ...
    Returns:
        list: 処理後の出力行のリスト
    """
    my_logger(f"process_py_import関数が呼び出されました。import_path: {import_path}, line: {line.strip()}", level='DEBUG')
    import_content = get_file_content(import_path)
    my_logger(f"インポートコンテンツ: {import_content}", level='DEBUG')
    return [line, '```python\n', import_content, '```\n']
...
def process_rst_import(import_path, line):
    ...
    Returns:
        list: 処理後の出力行のリスト
    """
    my_logger(f"process_rst_import関数が呼び出されました。import_path: {import_path}, line: {line.strip()}", level='DEBUG')
    import_content = get_file_content(import_path)
    my_logger(f"インポートコンテンツ: {import_content}", level='DEBUG')
    return [line, '```rst\n', import_content, '```\n']
...
def process_other_import(import_path, line):
    ...
    Returns:
        list: 処理後の出力行のリスト
    """
    my_logger(f"process_other_import関数が呼び出されました。import_path: {import_path}, line: {line.strip()}", level='DEBUG')
...
def get_file_content(file_path, model_input_image):
    if os.path.isfile(file_path):
        with open(file_path, 'r', encoding='utf-8') as file:
            # 再帰的にインポート処理を行う
            content = file.read()
            # 再帰的にインポート処理を行う
            content = process_imports(file_path, model_input_image)
            return content
    else:
