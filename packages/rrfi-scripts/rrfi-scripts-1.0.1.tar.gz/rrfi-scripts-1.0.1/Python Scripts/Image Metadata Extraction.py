...
class AzureMetadataExtractor:
    ...
    def image_coordinates(self, image_data):
        img = ExifImage(image_data)
        if hasattr(img, 'has_exif') and img.has_exif:
            try:
                coords = (
                    self.decimal_coords(img.gps_latitude, img.gps_latitude_ref),
                    self.decimal_coords(img.gps_longitude, img.gps_longitude_ref)
                )
                return img.datetime_original, coords[0], coords[1]
            except AttributeError:
                logger.warning('No Coordinates found for image')
                return None, None, None
        else:
            logger.warning('The Image has no EXIF information')
            return None, None, None
 
    def extract_metadata_and_send_to_search(self, blob_client):
        #if 'BENTHAM_004.jpg' in blob_client.blob_name:
            ...
            for code in area_codes:
                if code in blob_name.upper() or code in full_file_path.upper():
                    area_name = code
                    break
            ...
            Incident_Datetime_completion = openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": folder_path},
                    {"role": "system", "content": "You are an AI assistant that helps process data in a pipeline. Just return the values without any additional response text. Always return the identified date in dd/mm/yyyy format. Only return one value based on prioritization. Higher priority value gets preference. Ignore numerical values preceding '.jpg' or '.jpeg' or '.png'."},    
                    {"role": "user", "content": "You are a helpful agent. Your task is to identify and extract date, month, year as a substring from the given text. It is possible to have multiple date, month within the URL text. Highest priority for a full date. If there is no full date, look for a month and year value to return, and just return the month and year. "}
                ],
                temperature=0.7,
                max_tokens=800,
                top_p=0.95,
            ...
            if 'EA-IMToolbox' in blob_name:
                if 'Current Incidents' in blob_name:
                    ...
                    elif event_folder=='' or event_folder=='None':
                    ...
                    elif incident_datetime=='' or incident_datetime=='None':
            ...
            if file_extension.lower() == '.dwg':
                logger.info(f"Extracting metadata from DWG file: {blob_name}")
                try:
                    doc = ezdxf.readfile(full_file_path)
                    metadata = {
                        'id': str(uuid.uuid4()),
                        'Area_Name': area_name,
                    ...
                    logger.error(f"Error processing DWG file {blob_name}: {str(e)}")
                    print(f"Error processing DWG file {blob_name}: {str(e)}")
                return
            ...
            if file_extension.lower() in ['.jpg', '.jpeg', '.png', '.gif']:
                ...
                try:
                    ...
                    metadata['Incident_Date_and_Time'], metadata['Latitude'], metadata['Longitude'] = self.image_coordinates(image_data)
                    ...
                    if not metadata['Incident_Date_and_Time']:
                        metadata['Incident_Date_and_Time'] = last_modified
