...
class AzureMetadataExtractor:
    ...
    def extract_metadata_and_send_to_search(self, blob_client):
        ...
        sas_token = "sp=r&st=2024-04-30T16:20:17Z&se=2024-05-01T10:30:17Z&spr=https&sv=2022-11-02&sr=c&sig=Ubelu1Jcs7omdDoGtGXtDquB%2FBmHvxZmDp24s4sEjhE%3D"
        # Format file path with SAS token appended
        sas_token_param = f"?{sas_token}" if sas_token else ""  # Check if SAS token exists
        #file_path_with_sas = f'https://{blob_service_client.account_name}.blob.core.windows.net/{container_name}/{blob.name}{sas_token_param}'
 
 
        #full_file_path = f"https://{blob_client.account_name}.blob.core.windows.net/{blob_client.container_name}/{blob_name}"
        ...
        if file_extension.lower() in ['.shp']:
        #if 'Flooded_Properties.shp' in blob_name:
            global metadata
            global filename                  
            ...
            Incident_Datetime_completion = openai_client.chat.completions.create(
                ...
                messages = [
                    {"role": "system", "content": full_file_path},
...
if __name__ == "__main__":
    ...
    #connection_string='BlobEndpoint=https://rrfimainstorage.blob.core.windows.net/;QueueEndpoint=https://rrfimainstorage.queue.core.windows.net/;FileEndpoint=https://rrfimainstorage.file.core.windows.net/;TableEndpoint=https://rrfimainstorage.table.core.windows.net/;SharedAccessSignature=sv=2022-11-02&ss=bfqt&srt=sco&sp=rtfx&se=2024-05-01T10:30:32Z&st=2024-04-30T14:38:32Z&spr=https&sig=r5ibpvqKOB9KKppjf32CEy5ZN4b0l%2F%2Bp2IQLrl%2FA1pc%3D'
    container_name = 'rrfimaincon'
    #container_name = 'testprabir'
