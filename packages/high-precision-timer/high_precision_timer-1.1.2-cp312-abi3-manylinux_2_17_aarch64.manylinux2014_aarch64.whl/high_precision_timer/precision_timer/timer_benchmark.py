...
allows calling the benchmark() method from the CLI without using the full method path
(high_precision_timer.precision_timer.timer_benchmark.benchmark).
...
without excessive time expenditures, but may not be optimal for all users. The commands are also listed in the online
API documentation.
"""
...
from high_precision_timer.precision_timer.utilities import convert_time  # type: ignore
