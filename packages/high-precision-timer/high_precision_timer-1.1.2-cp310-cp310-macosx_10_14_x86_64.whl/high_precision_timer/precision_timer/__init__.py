"""This module contains the PrecisionTimer class that wraps the nanobind-instantiated C-extension that contains the
core logic of the timer.

The module is automatically imported when the parent 'precision_timer' package is imported into any scope and its' main
purpose is to expose the PrecisionTimer class. All interactions with the low-level C-API should be carried out through
the wrapper class where possible.
"""
