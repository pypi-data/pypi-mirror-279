...
class Client:
    ...
    def calibrate(self):
        ...
        with open(video_path, 'rb') as f:
            response = requests.post(
                f'http://vytalgazeapilb-1065850898.us-east-1.elb.amazonaws.com/{function_endpoint}',
                # f'http://127.0.0.1:8000/{function_endpoint}',
                params={"api_key": self.api_key, "width": w, "height": h, "points": str(calibration_points), "time_slices": str(time_slices)},
                # files = {"file": f},
                data=f.read(),
                timeout=1000, 
            )
    ...
    def predict_from_video(self, video_path: str, calib_mat: np.array = None):    
    ...
    async def predict_from_websocket(self, cam_id: int = 0, calib_mat: np.array = None, verbose: bool = False, show_frame: bool = True):    
        # function_endpoint = f"ws://localhost:8000/ws/predict?api_key={self.key}"
        calib_mat_bytes = tensor_to_bytes(torch.from_numpy(calib_mat)) if calib_mat is not None else None
        function_endpoint = f"ws://vytalgazeapilb-1065850898.us-east-1.elb.amazonaws.com/ws/predict?api_key={self.api_key}"
    ...
    def real_time_pred(self, cam_id: int = 0, calib_mat: np.array = None, verbose: bool = False, show_frame: bool = True):
        result = asyncio.get_event_loop().run_until_complete(self.predict_from_websocket(cam_id, calib_mat, verbose, show_frame))
        return result