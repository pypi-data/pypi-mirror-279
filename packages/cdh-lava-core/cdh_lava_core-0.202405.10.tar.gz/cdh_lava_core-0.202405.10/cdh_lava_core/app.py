...
def reports():
    ...
    calling_page_url = request.args.get("calling_page")
    return render_template("reports.html", calling_page_url=calling_page_url, data=data)
...
def dashboards():
    ...
    calling_page_url = request.args.get("calling_page")
    return render_template("dashboards.html", calling_page_url=calling_page_url, data=data)

@cdc_modules_bp.route("/list_modules")
def list_modules():
    # Get the directory of the current file
    current_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    calling_page_url = request.args.get("calling_page")
    return render_template("modules/list_modules.html", data=data, calling_page_url=calling_page_url)


@cdc_files_bp.route("/data_product_job")
def data_product_job():
    ...
    calling_page_url = request.args.get("calling_page")
    return render_template("data_product_job.html", calling_page_url=calling_page_url, data=data)
...
def data_products():
    ...
    calling_page_url = request.args.get("calling_page")
    return render_template("data_products.html", calling_page_url=calling_page_url, data=data)

@cdc_files_bp.route("/upload_codes")
def upload_codes():
    data_product_id = request.args.get("data_product_id")
    calling_page_url = request.args.get("calling_page")
    return render_template("upload_codes.html", data_product_id=data_product_id, calling_page_url=calling_page_url )

@cdc_files_bp.route("/upload_edc")
def upload_edc():
    calling_page_url = request.args.get("calling_page")
    return render_template("upload_edc.html", calling_page_url=calling_page_url)
...
<p><a href="../files/upload_edc">EDC Upload Page</a></p>
    ...
    def get(self, altmetric_id=None):
        ...
        with tracer.start_as_current_span("__init__"):
            try:
                ...
                if results is None:
                    return jsonify({'error': 'Failed to download_edc Altmetric data'}), 500
...
class MetadataExcelFileDownloadEdc(Resource):
    ...
    def get(self, schema_id):
        ...
        with tracer.start_as_current_span(f"metadata_excel_file_download_edc/{schema_id}"):
            try:
                start_time = time.time()  # Record the start time
...
class MetadataExcelFileDownloadCodes(Resource):
    ...
    def get(self, schema_id):
        ...
        with tracer.start_as_current_span(f"metadata_excel_file_download_codes/{schema_id}"):
            try:
                start_time = time.time()  # Record the start time
...
upload_parser_edc.add_argument("file", location="files", type=FileStorage, required=True)


upload_parser_codes = api.parser()
upload_parser_codes.add_argument("file", location="files", type=FileStorage, required=True)
upload_parser_codes.add_argument("data_product_id",  location="form", type=str, required=True)
...
ns_cdc_admin.add_resource(
    MetadataExcelFileDownloadCodes, "/metadata_excel_file_download_codes/<int:schema_id>"
)
...
class MetadataExcelFileUploadCodes(Resource):
    ...
    @api.expect(upload_parser_codes, validate=True)
    ...
    def post(self):
        ...
        with tracer.start_as_current_span("metadata_excel_file_upload_codes"):
            try:
                start_time = time.time()  # Record the start time

                # Get the uploaded file
                args = upload_parser_codes.parse_args()
                file = args["file"]
                ...
                manifest_excel_file_path_temp = (
                    obj_excel_config_uploader.get_excel_config_file_path(repository_path, data_product_id, environment, authenticated_user_id)  
                )
                
                # Get the directory path without the file name
                directory_path = os.path.dirname(manifest_excel_file_path_temp)
                directory_path = directory_path.replace("/home/nfs/cdc/","/home/nfs/CDC/")

                # Log the action of creating directories
                logger.info(f"Ensure directory exists: {directory_path}")
                
                manifest_excel_file_path_temp = manifest_excel_file_path_temp.replace("/home/nfs/cdc/","/home/nfs/CDC/")
                ...
                with open(manifest_excel_file_path_temp, "ab") as f:
                    ...
                    logger.info(f"File opened successfully: {manifest_excel_file_path_temp}")
                ...
                #) = schema.upload_codes_manifest_excel(
                #    manifest_excel_file_path_temp,
                #    config,
        ...
        with tracer.start_as_current_span("metadata_json_file_upload_edc"):
            try:
                ...
                (
                    ...
                    authorized_tables_count,
                    unauthorized_table_count,
                ) = schema.upload_edc_manifest_json(
...
ns_cdh_lava.add_resource(MetadataExcelFileUploadCodes, "/metadata_excel_file_upload_codes")
