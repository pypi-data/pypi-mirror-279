...
def create_api(app, api_description):
    ...
    CDH_LAVA_DESCRIPTION = (
        "The CDC Data Hub Lifecycle, Analysis and Visualization Accelerator (CDC Data Hub LAVA)"
        "makes building and deploying data products faster and more reliable "
        "through standardizing public health data processes and technology."
    )

    ns_cdh_lava = api.namespace(
        "cdh_lava",
        description=CDH_LAVA_DESCRIPTION,
    )
    ...
    try:
        with tracer.start_as_current_span("create_app"):
            ...
            parameters = {
                ...
                "az_sub_web_client_secret_key": "az_sub_web_client_secret_key",
            ...
            client_secret = obj_key_vault_interactive.get_secret(az_sub_web_client_secret_key)
            
            logger.info(
                f"az_sub_web_client_secret_key:{az_sub_web_client_secret_key}"
            )

            if not client_secret:
                raise ValueError(f"client_secret:{az_sub_web_client_secret_key} is empty or has a length of 0")
            
            logger.info(f"az_sub_web_client_secret_key:{az_sub_web_client_secret_key}")
