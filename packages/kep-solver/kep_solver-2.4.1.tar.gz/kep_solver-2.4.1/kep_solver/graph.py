...
class CompatibilityGraph:
    ...
    def __init__(self, instance=None) -> None:
        ...
        if instance is not None:
            ...
            for transplant in instance.transplants():
                if transplant.donor.inPool() and transplant.recipient.inPool():
                    self.addEdges(
                        transplant.donor,
    ...
    def dot(self) -> str:
        ...
        for edge in self.edges():
            res += f'"{mk_name(edge.start)}" -> "{mk_name(edge.end)}";\n'
        res += "}"
        return res
...
def build_alternates_and_embeds(
    ...
    def okay_embedded(exchange: Exchange, orig: Exchange):
        ...
        if uk_variant == 2:
            ...
            if len(set(exchange.all_donors()) - set(orig.all_donors())) != 0:
                return False
        return True
