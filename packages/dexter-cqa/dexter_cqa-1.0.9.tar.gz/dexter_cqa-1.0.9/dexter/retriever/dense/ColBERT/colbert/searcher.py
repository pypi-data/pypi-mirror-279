...
from dexter.retriever.dense.ColBERT.colbert.data import Collection, Queries, Ranking

from dexter.retriever.dense.ColBERT.colbert.modeling.checkpoint import Checkpoint
from dexter.retriever.dense.ColBERT.colbert.search.index_storage import IndexScorer
...
class Searcher:
    def __init__(self, index, checkpoint=None, collection=None, config=None, index_root=None, verbose:int = 3):
        ...
        use_gpu = False #self.config.total_visible_gpus > 0
        if use_gpu:
    ...
    def search(self, text: str, k=10, filter_fn=None, full_length_search=False, pids=None):
        ...
        return self.dense_search(Q.cpu(), k, filter_fn=filter_fn, pids=pids)
    ...
    def dense_search(self, Q: torch.Tensor, k=10, filter_fn=None, pids=None):
        ...
        pids, scores = self.ranker.rank(self.config, Q.cpu(), filter_fn=filter_fn, pids=pids)
