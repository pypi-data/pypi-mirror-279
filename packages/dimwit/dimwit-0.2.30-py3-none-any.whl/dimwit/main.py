...
def week_kde_over_all_categories(ts, events, bw_vals, combine=[], exclude=[]):
    present_categories = set(events) - set(exclude)
    ...
    if len(combine) == 0:
        mapped_categories = {cat: cat for cat in present_categories}


    _, ax = plt.subplots(1, figsize=(8, 6))
    for (search_cat, target_cat), bw in zip(mapped_categories, bw_vals):
        # Fetch all timestamps where the event matches the category.
        cat_ts = list(
            map(
                lambda tup: tup[1],
