...
class ModelFitTransform(ModelInterface):
    ...
    def update_model(
        ...
        try:
            ...
            db_records = self.model_db.load_model_from_underlying_db_with_max_retries(
                max_tries = 1,
            )
            ...
            if len(text_encoded) > 0:
                ...
                model_save_json_string = self.model_grid_helper.model_to_json(
                    numpy_to_base64_str = True,
                    dump_to_json_str = True,
                )
                self.logger.debug(
                    'For user id "' + str(self.user_id) + '" model save json string: ' + str(model_save_json_string)
                )
    ...
    def predict(
        ...
        if self.is_need_to_sync_model_with_db():
            self.logger.info(
