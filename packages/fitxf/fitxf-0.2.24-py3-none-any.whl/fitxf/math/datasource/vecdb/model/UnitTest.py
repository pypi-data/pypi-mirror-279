...
from nwae.math.lang.encode.LangModelPtSingleton import LangModelPtSingleton
...
from nwae.math.datasource.vecdb.model.ModelFitXForm import ModelFitTransform, ModelInterface
...
class VecDbModelUnitTest:
    ...
    def test(
        ...
        if cleanup:
            ...
            for vdb in vec_dbs_list:
                ...
                vdb.model_db.get_underlying_db().delete_index(tablename_or_index=self.user_id)
        return
    ...
    def __get_vecdb_model(self) -> ModelInterface:
        ...
        intent = Class(
            ...
            llm_model  = LangModelPtSingleton.get_singleton(
                LmClass = LangModelPt,
    ...
    def __test(
        ...
        assert self.vec_model_2.get_data_length() >= self.len_unique,\
            'After training data cache type "' + str(self.db_params.db_type)\
        ...
        acc_thr = 0.9
        assert score_accuracy > acc_thr, 'Accuracy ' + str(score_accuracy) + ' < ' + str(acc_thr)
        ...
        if (self.db_params.db_type != 'memory'):
            ...
            records_test_sync = [
                {self.COL_LABEL_HUMAN: 'sync test', self.COL_LABEL_NO: 999, self.COL_CONTENT: 'I am in sync'},
                {self.COL_LABEL_HUMAN: 'sync test', self.COL_LABEL_NO: 999, self.COL_CONTENT: 'Are you in sync'},
            ]
