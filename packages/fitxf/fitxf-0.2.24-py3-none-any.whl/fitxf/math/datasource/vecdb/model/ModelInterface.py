...
class ModelInterface:
    ...
    LOAD_DATA_MAX_TRIES_DEFAULT = 1
    OLD_DATETIME = datetime(year=2000, month=1, day=1)
    ...
    def init_data_model(
            self,
            max_tries = LOAD_DATA_MAX_TRIES_DEFAULT,
            background = False,
    ):
        raise Exception('Must be implemented by derived class')
    ...
    def is_need_to_sync_model_with_db(
            self,
    ):
    ...
    def get_text_encoding_from_db_records(
        ...
        if (not self.numpy_to_b64_for_db) and (self.db_params.db_type == 'csv'):
            text_encoded_float = []
            for row in text_encoded:
                float_array = self.model_db.get_underlying_db().convert_csv_string_array_to_float_array(
                    string_array = row,
                    custom_chars_remove = [','],
                )
    ...
    def add_records_to_underlying_db__(
        ...
        self.model_db.underlying_db.add(
            records = recs_for_storage_base64,
            tablename_or_index = self.user_id,
        )
        self.logger.info(
    ...
    def __delete_single_record_from_underlying_db(
        ...
        res_del = self.model_db.underlying_db.delete(
