...
class LayoutConfig:
    ...
    barycenter_heuristic_n_sweeps: int = 2
...
def add_dummy_nodes(dig, verbose=False):
    ...
    for edge in edges_to_break:
        ...
        for i_dum in range(lay_diff - 1):
            ...
            dig.add_edge(prev_node, dummy_name, **edge_dict)  # value=edge_val
            if i_dum == lay_diff - 2:
...
def sweep_barycenter_crossing_reduction(
    dig: nx.DiGraph, n_sweeps: int = 2, verbose: bool = False
) -> None:
    """Apply barycenter crossing reduction a number of times in each direction

    Args:
        dig
        n_sweeps: the number of times the crossing reduction heuristic is applied
            in each direction, typically 2
...
def process_directed_graph(
    ...
    if vertical_pos_method in ["barycenter_heuristic", "lp"]:
        n_sweeps = layout_config.barycenter_heuristic_n_sweeps
        sweep_barycenter_crossing_reduction(dig, n_sweeps=n_sweeps)
