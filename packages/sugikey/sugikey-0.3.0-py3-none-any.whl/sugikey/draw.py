...
def backward_link_curve(x_from_to, y_from_to, edge_val, draw_config, below: bool):
    """
    Get geometry of backward link (as arises in the case of cycles)

    One half-circle to turn backwards, a spline and another half-circle to get forward into the node again
    """
    if below:
        x_radius = 0.4
    ...
    angle = np.linspace(-np.pi / 2, np.pi / 2, 20)
...
def get_link_geometry(
    ...
    x_mid, y_old, dy = get_node_position(dig, node)
    ...
    if x_from_to[1] < x_from_to[0]:
        ...
        x_below, y_below = backward_link_curve(
            x_from_to, y_from_to, edge_val, draw_config, below=True
        )
        x_above, y_above = backward_link_curve(
            x_from_to, y_from_to, edge_val, draw_config, below=False
        )
