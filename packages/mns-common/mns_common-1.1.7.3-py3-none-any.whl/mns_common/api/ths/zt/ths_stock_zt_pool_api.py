        ...
        try:
            ...
            if len(zt_reason_list) > 0:
                for zt_reason_one in zt_reason_list:
                    ...
                    zt_stock_one_df_str = zt_stock_one_df.to_string(index=False)
                    zt_reason_dict = {'zt_reason_name': zt_reason_one,
                                      'number': 1,
                                      'stock_list': zt_stock_one_df_str
                                      }
                    ...
                    if result_group_df is None:
                        ...
                        if data_frame_util.is_not_empty(exist_zt_reason):
                            ...
                            exist_zt_stock_one_df_str = list(exist_zt_reason['stock_list'])[0]

                            exist_zt_stock_one_df = pd.read_csv(StringIO(exist_zt_stock_one_df_str),
                                                                delim_whitespace=True, dtype={'symbol': str})

                            exist_zt_stock_one_df_new = pd.concat([exist_zt_stock_one_df, zt_stock_one_df])
                            exist_zt_stock_one_df_str_new = exist_zt_stock_one_df_new.to_string(index=False)

                            result_group_df.loc[result_group_df['zt_reason_name'] == zt_reason_one, "stock_list"] = \
                                exist_zt_stock_one_df_str_new
                        else:
...
if __name__ == '__main__':
    while True:
        res = get_zt_reason()
        result_df = zt_reason_group(res)
        result_df_one = result_df.iloc[0:1]
        exist_zt_stock_one_df_str_test = list(result_df_one['stock_list'])[0]
        exist_zt_stock_one_df_test = pd.read_csv(StringIO(exist_zt_stock_one_df_str_test),
                                                 delim_whitespace=True, dtype={'symbol': str})
        print(exist_zt_stock_one_df_test)
