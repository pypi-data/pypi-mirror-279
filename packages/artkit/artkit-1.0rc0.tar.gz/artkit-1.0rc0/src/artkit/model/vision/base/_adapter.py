"""
Implementation of LegacyCachedVisionModel.
"""
...
@inheritdoc(match="""[see superclass]""")
class LegacyCachedVisionModel(
    LegacyCachedGenAIModel[T_VisionModel_ret, list[str]],
    VisionModel,
    Generic[T_VisionModel_ret],
):  # pragma: no cover
    """
    ...
    @staticmethod
    def _make_cache_key(prompt: str | None, image: Image) -> tuple[str, str]:
        """
        Convert the image and prompt into a key for the cache
    ...
    async def image_to_text(
        ...
        **model_params: Any,
        ...
        """[see superclass]"""
        key = self._make_cache_key(prompt=prompt, image=image)
        try:
            return self._get(*key)
        except KeyError:  # pragma: no cover
            response = await self.model.image_to_text(
                image=image, prompt=prompt, **model_params
            )
            self._put(*key, value=response)
            return response
