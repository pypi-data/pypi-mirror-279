...
def paper_2_analysis( directory, output_directory, file_data, data ):
    ...
    for i in sample_mask:
        ...
        features_2 = features[specimen_mask, feature_names.index( "DSC_HFM_160" )]

        box_plot_list.append( features_2.tolist() )
    ...
    ax.set_xticks( [y + 1 for y in range( len( box_plot_list ) )], labels = [resin_data.loc[i]["Label"] for i in sample_mask] )

    ax.set_xlabel( "Samples" )
    ax.set_ylabel( "Secondary Peak at 160°C" )
...
def sandbox( directory, output_directory, file_data, data, first_derivative_data, second_derivative_data ):
    ...
    if perform_paper_2_analysis:

        paper_2_analysis( directory, output_directory, file_data, data )
