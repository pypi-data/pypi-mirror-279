...
from velar.exceptions import PytgcallsBaseException, PytgcallsError
from velar.group_call_type import GroupCallType
from velar.implementation.group_call import GroupCall
from velar.mtproto_client_type import MTProtoClientType
from velar.implementation.group_call_file import GroupCallFile
from velar.implementation.group_call_device import GroupCallDevice
from velar.implementation.group_call_raw import GroupCallRaw
...
def hot_load_mtproto_lib_or_exception(module):
    if find_spec(module):
        ...
        raise PytgcallsBaseException(
            ...
            f'Run this command: pip3 install -U velar[{module}]'
        )
...
class GroupCallFactory:
    ...
    def __init__(
        self,
        client,
        mtproto_backend: MTProtoClientType = None,
        ...
        if mtproto_backend is MTProtoClientType.FIPPER:
            ...
            from velar.mtproto.fipper_bridge import FipperBridge

            self.__mtproto_bride_class = FipperBridge
        elif mtproto_backend is MTProtoClientType.TECHGRAM:
            hot_load_mtproto_lib_or_exception(MTProtoClientType.TECHGRAM.value)
            from velar.mtproto.techgram_bridge import TechgramBridge

            self.__mtproto_bride_class = TechgramBridge
        elif mtproto_backend is MTProtoClientType.TELETHON:
            hot_load_mtproto_lib_or_exception(MTProtoClientType.TELETHON.value)
            from velar.mtproto.telethon_bridge import TelethonBridge
