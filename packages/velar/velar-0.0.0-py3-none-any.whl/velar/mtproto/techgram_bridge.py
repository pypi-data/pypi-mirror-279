...
from techgram.errors import (
    BadRequest as TechgramBadRequest,
    GroupcallSsrcDuplicateMuch as TechgramGroupcallSsrcDuplicateMuch,
)
from techgram.handlers import RawUpdateHandler
from techgram.raw import functions, types
from techgram.raw.types import (
    GroupCallDiscarded as TechgramGroupCallDiscarded,
    InputPeerChannel,
    InputPeerChat,
    UpdateGroupCallConnection,
)
from techgram.utils import get_peer_id

from velar import PytgcallsError
from velar.mtproto.data import GroupCallDiscardedWrapper, GroupCallWrapper, GroupCallParticipantWrapper
from velar.mtproto.data.update import UpdateGroupCallWrapper, UpdateGroupCallParticipantsWrapper
from velar.mtproto.exceptions import BadRequest, GroupcallSsrcDuplicateMuch
from velar.utils import int_ssrc

from techgram import Client, ContinuePropagation

from velar.mtproto import MTProtoBridgeBase


class TechgramBridge(MTProtoBridgeBase):
    ...
    async def _process_group_call_update(self, update):
        if not isinstance(update.call, TechgramGroupCallDiscarded):
            return
    ...
    async def check_group_call(self) -> bool:
        ...
        except TechgramBadRequest as e:
            # compatibility with pyro > 2.0
    ...
    async def join_group_call(
        ...
        try:
            ...
            await self.handle_updates(response)
        except TechgramGroupcallSsrcDuplicateMuch as e:
            # compatibility with pyro > 2.0
            if getattr(e, 'x', None) is None:
                setattr(e, 'x', getattr(e, 'value'))
