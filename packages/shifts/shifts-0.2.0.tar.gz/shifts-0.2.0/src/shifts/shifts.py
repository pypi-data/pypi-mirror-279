...
def event(
    ...
    """
    Use the following symbols to choose the shift:\n
    M: Morning\n
    T: Afternoon\n
    N: Night\n
    D: Rest Day\n
    ...
    \n
    Example:shifts "F T N D F M M T N D F M M F MN D F M M T F T N D F M N D F F LF"
    """
    c = Calendar()
        ...
        if shift == "F":
            ...
            c.events.add(
                Event(
                    ...
                    end=date(year, s.end_month, s.end_day),  # type: ignore
                )
            )
            continue
