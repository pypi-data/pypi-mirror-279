...
valid_sources = {'atm', 'atm_low', 'atm_mean', 'atm_high', 'j2', 'srp', 'sun', 'mercury', 'venus', 'earth', 'moon',
                 'mars', 'jupiter', 'saturn', 'uranus', 'neptune', 'cmplxmoon'}
...
class Trajectory:
    ...
    def propagate(self, tof: Optional, npoints: int = 2, tof_units: Optional = 'model', atol: float = 1e-10, rtol: float = 1e-10) -> None:
        """
        ...
        if tof_units != 'model' and tof_units != 'trajectory':
        ...
        if self.model['type'] == 'nbp':
            T, X = kiam.propagate_nbp(central_body=self.model['primary'], tspan=tspan, x0=self.states[0:, -1],
                                      ...
                                      atol=atol, rtol=rtol, control_function=self.model['control'])
        elif self.model['type'] == 'r2bp':
            T, X = kiam.propagate_r2bp(tspan=tspan, x0=self.states[0:, -1], atol=atol, rtol=rtol)
        elif self.model['type'] == 'cr3bp_fb':
            T, X = kiam.propagate_cr3bp(central_body='First', tspan=tspan, x0=self.states[0:, -1],
                                        mu=self.model['data']['mass_parameter'], stm=stm, atol=atol, rtol=rtol)
            ...
            T, X = kiam.propagate_br4bp(central_body='First', tspan=tspan, x0=self.states[0:, -1],
                                        ...
                                        theta0=self.model['data']['theta0'], stm=stm, atol=atol, rtol=rtol)
            ...
            T, X = kiam.propagate_hill(tspan=tspan, x0=self.states[0:, -1], atol=atol, rtol=rtol)
        else:
    ...
    def repropagate(self, tof: Optional, npoints: int = 2, tof_units: Optional = 'model', start_index: int = 0) -> None:
        """
        ...
        if start_index != 0:
            raise 'TBD.'
        self.clear()
        self.propagate(tof, npoints, tof_units)
    ...
    def _set_model_sources(self) -> None:
        ...
        for source in self.model['sources_list']:
            if source in ['atm_low', 'atm_mean', 'atm_high']:
