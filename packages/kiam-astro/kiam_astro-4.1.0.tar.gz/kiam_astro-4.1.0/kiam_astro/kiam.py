...
if platform.processor() == 'arm':
    import FKIAMToolbox_m2 as FKIAMToolbox
else:
    import FKIAMToolbox
...
def normalize(x: numpy.ndarray, order=None, axis=0):
    """
...
def planet_state(jd: float, center: str, target: str) -> numpy.ndarray:
    ...
    """
    with _package_folder_contex():
        state = FKIAMToolbox.ephemeris.planetstate(jd, target.capitalize(), center.capitalize())
    return state
...
def propagate_nbp(central_body: str, tspan: numpy.ndarray, x0: numpy.ndarray, sources_dict: dict, dat_dict: dict, units_dict: dict, stm: bool, variables: str, atol: float = 1e-10, rtol: float = 1e-10, control_function: Callable = None) -> tuple[numpy.ndarray, numpy.ndarray]:
    """
    ...
    if control_function is None:
        ...
        with _package_folder_contex():
            T, Y = FKIAMToolbox.propagationmodule.propagate_nbp(central_body.capitalize(), tspan, x0, variables, neq, 'ode113', atol, rtol)
        ...
        with _package_folder_contex():
            T, Y = FKIAMToolbox.propagationmodule.propagate_nbp_control(central_body.capitalize(), tspan, x0, variables, neq, atol, rtol, control)

    return T, Y
def propagate_r2bp(tspan: numpy.ndarray, x0: numpy.ndarray, atol: float = 1e-10, rtol: float = 1e-10) -> tuple[numpy.ndarray, numpy.ndarray]:
    """
    ...
    """
    tspan, x0 = to_float(tspan, x0)
    t, y = FKIAMToolbox.propagationmodule.propagate_r2bp(tspan, x0, atol, rtol)
    return t, y
def propagate_cr3bp(central_body: str, tspan: numpy.ndarray, x0: numpy.ndarray, mu: float, stm: bool, atol: float = 1e-10, rtol: float = 1e-10) -> tuple[numpy.ndarray, numpy.ndarray]:
    """
    ...
    t, y = FKIAMToolbox.propagationmodule.propagate_cr3bp(central_body.lower(), tspan, x0, mu, stm, neq, atol, rtol)
    return t, y
def propagate_br4bp(central_body: str, tspan: numpy.ndarray, x0: numpy.ndarray, mu: float, gm4b, a4b: float, theta0: float, stm: bool, atol: float = 1e-10, rtol: float = 1e-10) -> tuple[numpy.ndarray, numpy.ndarray]:
    """
    ...
    neq = 42 if stm else 6
    t, y = FKIAMToolbox.propagationmodule.propagate_br4bp(central_body.lower(), tspan, x0, mu, gm4b, a4b, theta0, stm, neq, atol, rtol)
    return t, y
def propagate_hill(tspan: numpy.ndarray, x0: numpy.ndarray, atol: float = 1e-10, rtol: float = 1e-10) -> tuple[numpy.ndarray, numpy.ndarray]:
    """
    ...
    """
    tspan, x0 = to_float(tspan, x0)
    t, y = FKIAMToolbox.propagationmodule.propagate_hill(tspan, x0, atol, rtol)
    return t, y
