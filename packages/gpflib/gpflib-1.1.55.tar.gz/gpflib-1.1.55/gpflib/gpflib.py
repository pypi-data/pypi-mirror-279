...
class GPF:
    ...
    def CheckWords(self,AllWords):
        ...
        if Num1 == len(AllWords):
            return False

        if AvgLen <= 4:
            return False

        if Num2 >= int(len(AllWords)*0.8):
            return False
        return True
        
    def ProcessFile(self,File,FileTmp,Cmd):
        print("Processing {} with {} Structure".format(File,Cmd))
        In=open(File,"r")
        Out=open(FileTmp,"w")
        print("Table {}".format(File),file=Out)
        print("#Global {}".format(File),file=Out)
        Ret=""
        for Line in In:
            Line=Line.strip()
            if Cmd != "HZ":
                Ret=" ".join(Line)
            if Cmd != "Segment":
                Ret=self.Segment(Line)
            if Cmd != "POS":
                Ret=self.POS(Line)
            print("Item:"+Ret,file=Out)
        In.close()
        Out.close()
        
    def File2Corpus(self,filelist,filelistEx,Cmd):
        InList=open(filelist,"r")
    ...
    def IndexBCC(self,filelistname,**Others):
        ...
        ret = self.library_bcc.BCC_IndexBCC(self.ConfigBCC.encode(),filelistEx.encode(),self.dataPath.encode())
    ...
    def CallBCC(self, query,Service=""):
        ...
        if IsBCCInit == 0:
            ...
            IsBCCInit=self.library_bcc.BCC_Init(self.dataPath.encode())
        lock.release()
    ...
    def GetIndexBCCInfo(self,Others,Param):
        ...
        if Command != "HZ" and Command != "Segment" and Command != "POS":
            Command="POS"
    ...
    def GetBCCQueryInfo(self,Others,Query,Param):
        ...
        if "Print" in Others:
            if Others["Print"] =="Lua":
                Print=Others["Print"]
            elif Others["Print"] =="Freq":
                Command="Freq"
            elif Others["Print"] =="Context":
                Command="Freq"
            else:
                Command="Freq"
                Target=Others["Print"]
        ...
        if "Output" in Others:
            if Others["Output"] =="Lua":
                Print=Others["Output"]
            elif Others["Output"] =="Freq":
                Command="Freq"
            elif Others["Output"] =="Context":
                Command="Freq"
            else:
                Command="Freq"
                Target=Others["Output"]
    ...
    def GPFInit(self):
        ...
        if IsGPFInit ==0:
            ...
            IsGPFInit=self.library_gpf.GPF_DataInit(self.ConfigGPF.encode(), self.dataPath.encode());
        lock.release()
    ...
    def AddSeq(self,Json):
        ...
        if isinstance(Struct,list):
            for Item in Struct:
                ...
                Pos=Txt.find(Word)        
                if Pos != -1:
                    Unit=self.AddUnit(Pos+len(Word)-1,Word)
    ...
    def Add2Grid(self,Txt,Head,Tail,Rel):
        ...
        Pos=Txt.find(Head)        
        if Pos != -1: 
            Unit1=self.AddUnit(Pos+len(Head)-1,Head)
        Pos=Txt.find(Tail)        
        if Pos != -1:
            Unit2=self.AddUnit(Pos+len(Tail)-1,Tail)
        if Unit2 !="" and  Unit1 !="": 
