...
class Float:
    ...
    def __init__(self, value: Union["Float", float] = None) -> None:
        self._value = None
        self.inputs = _RefList()
        self.outputs = _RefList()
        self._float_ref = None
        self._input_history_cache = None
    ...
    def value(self) -> float:
        """
        Get object from the end of the reference chain.
        """
    ...
    @property
    def input_history(self) -> _RefList:
        """
        Get all inputs of all inputs recursively
        """
        if self._input_history_cache is None:
            self._input_history_cache = _RefList()
            for i in self.inputs:
                if i not in self._input_history_cache:
                    self._input_history_cache.append(i)
                    for i2 in i.input_history:
                        if i2 not in self._input_history_cache:
                            self._input_history_cache.append(i2)
        return self._input_history_cache
    ...
    def __gt__(self, other):
        if isinstance(other, Float) or hasattr(other, "__float__"):
            other = float(other)
