...
from src.jobs import product_jobs, order_jobs, deliver_jobs, photo_jobs, client_payment_plan_jobs, representative_jobs, \
    colect_job
...
def get_job_from_name(job_name: str):
    ...
    #     return instantiate_deliver_job
...
def get_thread_job_from_name(job_name: str):
    # Estoque
    # if job_name == 'insere_estoque_produto_semaforo_job':
    #     return run_thread_instantiate_insert_stock_semaphore_job
    if job_name == 'envia_estoque_job':
    ...
    # Preco
    # elif job_name == 'insere_preco_produto_semaforo_job':
    #     return run_thread_instantiate_insert_price_semaphore_job
...
def verify_jobs_change(job_config_dict: dict):
    ...
    logger.info(f"Quantidade de Schedules ativos: {len(schedule.get_jobs())}")
    if src.jobs_qtd != len(schedule.get_jobs()):
        schedule.clear()
    ...
    if src.exibir_interface_grafica:
        ...
        while True:
            try:
                ...
                logger.error(f'Erro não tratado capturado: {str(e)}')
                send_log('__main__', src.client_data.get('enviar_logs'), True, f'{str(e)}', 'error', 'main', 'MAIN')
