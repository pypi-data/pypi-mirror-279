...
class OnlineLogger:
    ...
    def send_log(job_name: str, send_slack: bool, send_api: bool, message: str, log_type: str, job_method: str = 'LOG_ONLINE', api_log_identifier: str = ''):
        ...
        if send_api:
            api_okinghub.post_log(Log(f'{message}', api_log_identifier, job_name, src.client_data.get("integracao_id")
                                      , tipo_log, src.client_data.get("seller_id")))
...
def send_execution_notification(job_config: dict) -> None:
    with lock:
        ...
        mensagem = f'Oking em execucao desde {job_config.get("execution_start_time")} com {job_config.get("job_qty")}' \
                   f' jobs para o cliente {src.client_data.get("integracao_nome")}'
        api_okinghub.post_log(Log(mensagem, 'EXECUCAO', 'EXECUCAO', src.client_data.get("integracao_id")
                                  , 'X', src.client_data.get("seller_id")))
