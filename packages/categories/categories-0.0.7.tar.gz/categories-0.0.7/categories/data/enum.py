...
def boundedEnumFrom(enum : Enum[a], bounded : Bounded[a], x : a, /) -> Stream[a]:
    return map(enum.toEnum, range(enum.fromEnum(x), enum.fromEnum(bounded.maxBound()) + 1))


def boundedEnumFromThen(enum : Enum[a], bounded : Bounded[a], x : a, y : a, /) -> Stream[a]:
    match (enum.fromEnum(x), enum.fromEnum(y)):
        case x_, y_ if (y_ >= x_):
            return map(enum.toEnum, range(x_, enum.fromEnum(bounded.maxBound()) + 1, y_ - x_))
        case x_, y_:
            return map(enum.toEnum, range(x_, enum.fromEnum(bounded.minBound()) - 1, y_ - x_))
