...
class Pear(BuildSystem):
    ...
    def dist(
        self, session, resolver, target_directory: str, quiet: bool = False
    ):
    ...
    def test(self, session, resolver):
        run_detecting_problems(
            session, [guaranteed_which(session, resolver, "pear"), "run-tests"]
        )
    ...
    def install(self, session, resolver, install_target):
        run_detecting_problems(
            session,
            [
                guaranteed_which(session, resolver, "pear"),
                "install",
                self.path,
            ],
        )
    ...
    def get_declared_dependencies(self, session, fixers=None):
        ...
        try:
            root = xmlparse_simplify_namespaces(path, self.PEAR_NAMESPACES)
                ...
                for package_tag in root.findall("package"):
                    ...
                    yield (
                        ...
                        PhpPackageRequirement(
                            name,
                            channel=(
                                channel_tag.text if channel_tag else None
                            ),
...
class SetupPy(BuildSystem):
    ...
    def _determine_interpreter(self):
        interpreter = None
        if self.config:
            python_requires = self.config.get("options", {}).get(
                "python_requires"
            )
...
class Gradle(BuildSystem):
    ...
    def exists(cls, path):
        return os.path.exists(
            os.path.join(path, "build.gradle")
    ...
    def _run(self, session, resolver, task, args):
        ...
        try:
            ...
            if any(
                re.match(
                    r"Task '" + task + r"' not found in root project '.*'\.",
                    line,
                )
                for line in e.lines
            ):
...
class Meson(BuildSystem):
    ...
    def dist(self, session, resolver, target_directory, quiet=False):
        ...
        with DistCatcher([session.external_path("build/meson-dist")]) as dc:
            try:
                ...
                if (
                    "ninja: error: unknown target 'dist', did you mean 'dino'?"
                    in e.lines
                ):
                    raise NotImplementedError from e
                raise
    ...
    def _introspect(self, session, fixers, args):
        ret = run_with_build_fixers(
            fixers, session, ["meson", "introspect"] + args + ["./meson.build"]
        )
        import json
...
class Npm(BuildSystem):
    ...
    def get_declared_dependencies(self, session, fixers=None):
        for name, _version in self.package.get("dependencies", {}).items():
            # TODO(jelmer): Look at version
            yield "core", NodePackageRequirement(name)
        for name, _version in self.package.get("devDependencies", {}).items():
            # TODO(jelmer): Look at version
...
class Gem(BuildSystem):
    ...
    def dist(self, session, resolver, target_directory, quiet=False):
        ...
        with DistCatcher.default(session.external_path(".")) as dc:
            run_detecting_problems(
                session,
                [guaranteed_which(session, resolver, "gem2tgz"), gemfiles[0]],
            )
        return dc.copy_single(target_directory)
...
class DistZilla(BuildSystem):
        ...
        with open(self.path, "rb") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                if key.strip() == b"class" and value.strip().startswith(
                    b"'Dist::Inkt"
                ):
                    logging.debug(
    ...
    def test(self, session, resolver):
        ...
        self.setup(resolver)
        run_detecting_problems(
            session, [guaranteed_which(session, resolver, "dzil"), "test"]
        )

    def build(self, session, resolver):
        self.setup(resolver)
        run_detecting_problems(
            session, [guaranteed_which(session, resolver, "dzil"), "build"]
        )

    @classmethod
    def probe(cls, path):
        if os.path.exists(
            os.path.join(path, "dist.ini")
...
class CMake(BuildSystem):
    ...
    def build(self, session, resolver):
        self.setup(session, resolver)
        run_detecting_problems(session, ["cmake", "--build", self.builddir])

    def install(self, session, resolver, install_target):
        self.setup(session, resolver)
        run_detecting_problems(session, ["cmake", "--install", self.builddir])
    ...
    def get_declared_dependencies(self, session, fixers=None):
        ...
        with open(os.path.join(self.path, "CMakeLists.txt")) as f:
            for line in f:
                m = re.match(
                    r"cmake_minimum_required\(\s*VERSION\s+(.*)\s*\)", line
                )
                if m:
                    yield (
                        "build",
...
class Make(BuildSystem):
    ...
    def setup(self, session, resolver, prefix=None):
        ...
        if not makefile_exists() and not session.exists("configure"):
            if session.exists("autogen.sh"):
                if (
                    shebang_binary(os.path.join(self.path, "autogen.sh"))
                    is None
                ):
                    run_detecting_problems(
            ...
            elif session.exists("configure.ac") or session.exists(
                "configure.in"
            ):
    ...
    def build(self, session, resolver):
        ...
        if self.name == "qmake":
            ...
            default_target = "all"
        self._run_make(session, [default_target] if default_target else [])
        ...
        for target in ["check", "test"]:
            try:
                ...
                if (
                    ...
                    pass
                if (
                    f"make[1]: *** No rule to make target '{target}'.  Stop."
                    in e.lines
                ):
                    pass
                else:
    ...
    def dist(self, session, resolver, target_directory, quiet=False):
        ...
        with DistCatcher.default(session.external_path(".")) as dc:
            try:
                ...
                if (
                    "make: *** No rule to make target 'dist'.  "  # noqa:SIM114
                    "Stop." in e.lines
                ):
                    raise NotImplementedError from e
                elif (
                    ...
                    "ninja: error: unknown target 'dist', "  # noqa: SIM114
...
class Cabal(BuildSystem):
    ...
    def _run(self, session, args):
        try:
            run_detecting_problems(session, ["runhaskell", "Setup.hs"] + args)
        except UnidentifiedError as e:
    ...
    def get_declared_dependencies(self, session, fixers=None):
        ...
        if self.minilla:
            ...
            try:
                ...
                if (
                    "Do not run distmeta. "
                    "Install Minilla and `minil install` instead." in e.lines
                ):
                    self.minilla = True
                else:
                    raise
        with suppress(FileNotFoundError), open(
            os.path.join(self.path, "META.yml")
        ) as f:
            yield from _declared_deps_from_meta_yml(f)
