...
class AbstractProviderCollection(ABC):
    ...
    def start_up(self) -> None:
        ...
        if self._is_active:
            return
        logging.info("启动 %s...", self.__class__.__name__)

        if not self._providers:
            logging.debug("加载默认接口配置...")
            for name, provider_config in self.__defaults__.items():
                self.register_provider(name, provider_config)

        for provider in self._init_providers:
