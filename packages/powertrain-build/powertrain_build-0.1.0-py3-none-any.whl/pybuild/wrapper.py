...
import git
from pybuild import build, pt_matlab
...
class PyBuildWrapper(pt_matlab.Matlab):
    ...
    def _evaluate_run_and_models(self, args):
        ...
        """
        run_pybuild = args.build is not None or getattr(args, "project_config", None) is not None
        should_run = args.update or args.codegen or run_pybuild
    ...
    def build_automation(self, mode):
        ...
        script_name = f"pybuild_matlab_{mode}.m"
        self.log = f"pybuild_matlab_{mode}.log"

        # Set up matlab path
        matlab_scripts_path = os.path.join("pybuild_matlab_scripts")

        # Copy matlab-scripts to the project root
        matlab_script_folder = files("pybuild.matlab_scripts")
    ...
    def run_pybuild(self):
        """Execute PyBuild.

        Returns:
            exit_code: Exit code from PyBuild build step.
        """
        try:
    ...
    def add_args(parser):
        ...
        pybuild_parsers = parser.add_subparsers(help="PyBuild specific build.")
        build_specific_parser = pybuild_parsers.add_parser(
