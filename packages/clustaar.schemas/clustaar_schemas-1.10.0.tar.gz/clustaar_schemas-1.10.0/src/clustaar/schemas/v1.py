...
PREDICATE_SESSION_VALUE_GETTER = Schema(
    {
        ...
        "key": f.String(
            validators=v.Length(min=1, max=STORE_SESSION_VALUE_ACTION_KEY_MAX_LENGTH)
...
PREDICATE_USER_ATTRIBUTE_GETTER = Schema(
    {
        ...
        "key": f.String(
            validators=v.Length(min=1, max=SET_USER_ATTRIBUTE_ACTION_KEY_MAX_LENGTH)
            & v.Match(re.compile(r"^[\w\d_]+$"))
        ),
    },
...
SEND_EMAIL_ACTION = Schema(
    {
        ...
        "fromEmail": f.String(
            ...
            optional=True,
            allow_none=True,
            validators=v.Length(max=SEND_EMAIL_ACTION_FROM_EMAIL_MAX_LENGTH)
...
CARD = Schema(
    {
        ...
        "imageURL": f.String(
            ...
            binding="image_url",
            pre_load=[strip],
            validators=v.Length(max=EXTERNAL_URL_MAX_LENGTH) & v.URL(schemes={"http", "https"})
            | v.Equal(""),
        ),
        "url": f.String(
            optional=True,
...
ASKED_DOCUMENT = Schema(
    {
        ...
        "authorizedFileTypes": f.List(
            f.String(validators=[v.In(S3_CONTENT_TYPES)]), binding="authorized_file_types"
        ),
    },
    name="asked_document",
)
