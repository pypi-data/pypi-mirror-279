...
class _FileSystemHuggingFaceDataset(_HuggingFaceDataset, _FileSystemBitfountDataset):
    ...
    def _getitem(self, idx: Union[int, Sequence[int]]) -> _DataEntryAllowingTextWithKey:  # type: ignore[override] # Reason: [BIT-3851] temp fix for supporting data key passthrough # noqa: B950
...
class _FileIterableHuggingFaceDataset(
    ...
    def __iter__(self) -> Iterator[_DataEntryAllowingTextWithKey]:  # type: ignore[override] # Reason: [BIT-3851] temp fix for supporting data key passthrough # noqa: B950
        # Super method relies on calls to _getitem(), etc, which we have changed in
        # this class; this means we can constrain the return type even though we are
        # calling the super method
        return cast(Iterator[_DataEntryAllowingTextWithKey], super().__iter__())
