...
import platform
import ssl
import subprocess  # nosec B404 # import-subprocess # subprocess usage is acceptable/necessary here  # noqa: B950
import tempfile
...
def _get_root_CAs_macos() -> str:
    ...
    system_ca_certs = subprocess.run(  # nosec start_process_with_partial_path, subprocess_without_shell_equals_true # no untrusted input, partial path for `security` is fine # noqa: B950
        [
            "security",
            "export",
            "-t",
    ...
    system_roots_ca_certs = subprocess.run(  # nosec start_process_with_partial_path, subprocess_without_shell_equals_true # no untrusted input, partial path for `security` is fine # noqa: B950
        [
            "security",
            "export",
            "-t",
...
def _create_pem_bundle(pems: str) -> Path:
    ...
    cert_file = (Path(tmp_dir) / "caRootCerts.pem").absolute()
