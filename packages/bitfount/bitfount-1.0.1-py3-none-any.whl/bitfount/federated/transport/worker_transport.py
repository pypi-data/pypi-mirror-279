...
from bitfount.federated.types import TextGenerationDefaultReturnType, _PodResponseType
...
class _WorkerMailbox(_BaseMailbox):
    ...
    async def issue_saml_challenge(
        self,
        saml_request: str,
    ) -> None:
    ...
    async def send_evaluation_results(
        self, eval_results: Optional[Mapping[Hashable, Any]] = None
    ) -> None:
    ...
    async def send_model_response(
        self, response: TextGenerationDefaultReturnType
    ) -> None:
        """Sends evaluation results to the modeller."""
        logger.info("Sending response to modeller...")
        await self._send_aes_encrypted_message(
            response, _BitfountMessageType.MODEL_RESPONSE
        )
    ...
    async def send_task_abort_message(self, message: Optional[str] = None) -> None:
        """Send task abort message to Modeller."""
        await self._send_aes_encrypted_message(message, _BitfountMessageType.TASK_ABORT)
