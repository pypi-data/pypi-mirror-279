...
import PIL
import cv2  # type: ignore[import-untyped] # Reason: No Stubs for module
...
from bitfount.data.types import DataSplit, _SemanticTypeValue, _SingleOrMulti
...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(
        self, return_data_keys: bool = False
    ) -> Union[
        ...
        for _batch_idx, batch in enumerate(self.test_dataloader):
            batch = typing.cast(
                Union[
                    ...
                    List[Union[_SingleOrMulti[torch.Tensor], typing.Sequence[str]]],
                ],
                batch,
            )

            images: _SingleOrMulti[torch.Tensor] = typing.cast(
                _SingleOrMulti[torch.Tensor], batch[0]
            )
            images_storage_location: _SingleOrMulti[torch.Tensor] = typing.cast(
                _SingleOrMulti[torch.Tensor], batch[1]
            )
            ...
            if self.test_dataloader.expect_key_in_iter():
                ...
                # data keys
                keys_: _SingleOrMulti[str] = typing.cast(_SingleOrMulti[str], batch[-1])
            ...
            if isinstance(images_storage_location[0], tuple):  # type: ignore[unreachable] # Reason: see above # noqa: B950
                # Extract the image filename to a list
                images_storage_location = [item for item in images_storage_location[0]]  # type: ignore[unreachable] # Reason: see above # noqa: B950
            ...
            for index in range(self.batch_size):
                ...
                # and save it
                if len(masks) > 0:
                    seg_image, final_predictions = self._draw_masks_on_original_image(
