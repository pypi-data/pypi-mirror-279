...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(self) -> pd.DataFrame:
        ...
        if self.datasource.multi_table and isinstance(self.datasource, DatabaseSource):
            ...
            with self.datasource.con.connect() as conn:
            ...
            # Using `get_data` rather than the `data` attribute ensures compatibility
            # with datasource views
            df = self.datasource.get_data(table_name=self.table)
