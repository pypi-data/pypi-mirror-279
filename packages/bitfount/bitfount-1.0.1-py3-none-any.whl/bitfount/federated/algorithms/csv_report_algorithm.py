...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(
        ...
        if isinstance(results_df, pd.DataFrame) and len(results_df) == len(
            ...
            # need to retrieve
            if (
                ORIGINAL_FILENAME_METADATA_COLUMN in results_df
                and isinstance(
                    self.datasource,
                ...
                file_df = cast(FileSystemIterableSource, self.datasource)._get_data(
                    file_names
                )
                merged = file_df.merge(results_df, on=ORIGINAL_FILENAME_METADATA_COLUMN)
                dfs = [merged]

            elif not self.datasource.iterable:
            ...
            elif self.datasource.iterable and isinstance(
                self.datasource,
                (FileSystemIterableSource, _DataViewFromFileIterableSource),
            ):
                dfs = dataframe_iterable_join(
                    cast(IterableSource, self.datasource).yield_data(
                        file_names=self.datasource.selected_file_names
                    ),
                    results_df,
        ...
        # how to join it to the results_df.
            ...
            # need to retrieve
            if (
                ORIGINAL_FILENAME_METADATA_COLUMN in aux_results_df
                and isinstance(
                    self.datasource,
                    (FileSystemIterableSource, _DataViewFromFileIterableSource),
                )
                ...
                file_names: List[str] = aux_results_df[  # type: ignore[no-redef] # Reason: this is in a separate flow branch # noqa: B950
                    ORIGINAL_FILENAME_METADATA_COLUMN
                ].tolist()
                logger.debug(f"Getting data for: {file_names}")
                file_df = cast(FileSystemIterableSource, self.datasource)._get_data(
            ...
            # Otherwise, we raise an error.
                ...
                if self.datasource._test_idxs is None:
                    if not self.datasource.iterable:
                    ...
                    else:  # self.datasource.iterable == True
                        # If the datasource is iterable, we need to use the
                        # `get_filenames` method of the data splitter to get the
                        # filenames corresponding to the test set, which are then
                        ...
                        fsi_datasource = cast(FileSystemIterableSource, self.datasource)
                        test_filenames = data_splitter.get_filenames(
                            fsi_datasource, DataSplit.TEST
                        )

                        # Get generator to go through the datasource and compile
                        # dataframes limited to the test filenames
                        dfs = conditional_dataframe_yielder(
                            dfs=fsi_datasource.yield_data(),
                            condition=lambda df_: df_.loc[
                                df_[ORIGINAL_FILENAME_METADATA_COLUMN].isin(
                                    test_filenames
                                )
                            ],
                            reset_index=True,
                        )
                else:  # datasource has test rows
                    ...
                    logger.debug(
                        "Extracting test set dataframe from datasource dataframe"
                    )
                    dfs = [
                        self.datasource.data.loc[
                            self.datasource._test_idxs
                        ].reset_index(drop=True)
                    ]

                # Now we need to combine those datasource dataframes together with the
                # results dataframe.
                #
        ...
        for df in dfs:
            ...
            append_dataframe_fn: Callable[
                [pd.DataFrame, Path], None
            ] = lambda dataframe, path: dataframe.round(decimals=2).to_csv(
                path,
                mode="a",
                header=not csv_path.exists(),
                index=False,
            )
            safe_append_to_file(partial(append_dataframe_fn, df), csv_path)
...
class CSVReportAlgorithm(BaseAlgorithmFactory):
    ...
    def modeller(self, **kwargs: Any) -> _ModellerSide:
        """Modeller-side of the algorithm."""
        return _ModellerSide(save_path=self.save_path, **kwargs)
