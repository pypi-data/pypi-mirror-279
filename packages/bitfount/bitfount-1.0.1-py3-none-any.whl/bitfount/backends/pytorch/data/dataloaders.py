...
import random
import secrets
from typing import Any, DefaultDict, Iterator, List, Sequence, Tuple, Union, cast
...
class _BasePyTorchBitfountDataLoader(BitfountDataLoader):
    ...
    def _unshuffled_image_iter(self) -> Iterator[List[_SingleOrMulti[torch.Tensor]]]:
        ...
        # but NOT _PytorchFileIterableDataset or _PytorchFileSystemDataset, and
        # that we are working with images.
    ...
    def _shuffled_image_iter(self) -> Iterator[List[_SingleOrMulti[torch.Tensor]]]:
    ...
    def _image_iter_with_key(
        self,
    ) -> Iterator[List[Union[_SingleOrMulti[torch.Tensor], Sequence[str]]]]:
    ...
    def _unshuffled_image_iter_with_key(
        ...
        # _PytorchFileIterableDataset or _PytorchFileSystemDataset, and that we are
        # working with images.
    ...
    def _convert_input_target(
        ...
        #   contained type.
        return _convert_batch_to_tensor(cast(_DataBatchNoKey, batch))

    @staticmethod
    def _convert_input_target_key(
        batch: Union[_DataBatchWithKey, _PTImageIterDataBatchWithKey],
    ) -> List[Union[_SingleOrMulti[torch.Tensor], Sequence[str]]]:
    ...
    def expect_key_in_iter(self) -> bool:
        ...
        return isinstance(
            self.dataset,
            (_PytorchFileIterableDataset, _PytorchFileSystemDataset),
        )
...
class PyTorchBitfountDataLoader(_BasePyTorchBitfountDataLoader):
    ...
    def __len__(self) -> int:
        ...
        if self._data_entry_contains_multi_imgs():
            ...
            # will be given by len(dataset)/batch_size.
            return len(self.dataloader)

    def __iter__(self) -> Union[
        Iterator[List[_SingleOrMulti[torch.Tensor]]],
        Iterator[List[Union[_SingleOrMulti[torch.Tensor], Sequence[str]]]],
    ]:
    ...
    def _no_key_iter(self) -> Iterator[List[_SingleOrMulti[torch.Tensor]]]:
...
class PyTorchIterableBitfountDataLoader(_BasePyTorchBitfountDataLoader):
    ...
    def _with_key_iter(
        ...
        self.dataset = cast(
            Union[_PytorchFileIterableDataset, _PytorchFileSystemDataset], self.dataset
        )
        ...
        # Otherwise, handle it here
            ...
            # If we are in this method, we know that self.dataset is either
            # _PytorchFileIterableDataset or _PytorchFileSystemDataset.
            # In either case, this will return a type of _DataEntryWithKey in
            # its iteration
