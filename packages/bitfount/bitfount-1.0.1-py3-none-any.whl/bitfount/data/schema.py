...
from bitfount.pod_db_constants import ROW_INDEX_COLUMN
...
class TableSchema(_TableSchemaMarshmallowMixIn):
    ...
    def _dtype_based_stype_split(
        ...
        for col, typ in converted_data.items():
            semantic_types[TableSchema._determine_semantic_type(col, typ)].append(col)
    ...
    def _determine_semantic_type(col: str, typ: _DtypesValues) -> SemanticType:
        ...
        if isinstance(typ, pd.StringDtype) or typ == str or typ == bytes:
            return SemanticType.TEXT

        # Booleans are explicitly categorical
        if isinstance(typ, pd.BooleanDtype) or typ == bool:
            return SemanticType.CATEGORICAL
    ...
    def _process_image_features(
        ...
        """Process image features to get their shapes, formats, etc."""
        for img_feature in img_features:
            record = self.features["image"][img_feature]
            if (
                not isinstance(
                    datasource,
                    (FileSystemIterableSource, _DataViewFromFileIterableSource),
                )
                or datasource.cache_images is True
            ):
                for img in datasource.get_column(img_feature, table_name=self.name):
                    if img is not None and img is not np.nan and img is not pd.NA:
                        im = Image.open(img)
    ...
    def _add_datasource_features(
        ...
        col_names = datasource.get_column_names(table_name=self.name)
        ...
        if SemanticType.CATEGORICAL in semantic_types:
            categorical_values = datasource.get_values(
        ...
        for stype, features in semantic_types.items():
            ...
            if stype == SemanticType.IMAGE:
                ...
                if (
                    ...
                    self._process_image_features(datasource, img_features=features)

    def _add_features_from_dataframe(
        self,
        ...
        if dataframe.empty:
            logger.warning("Dataframe is empty. No features to add to the schema.")
            return

        ignore_cols = list(ignore_cols) if ignore_cols is not None else []
        force_stype = force_stype if force_stype is not None else {}
        data = dataframe.convert_dtypes()
        table_dtypes: _Dtypes = data.dtypes.to_dict()
        for name in list(table_dtypes):
            if is_datetime(data[name]):
                table_dtypes[name] = pd.StringDtype()
        if "image_prefix" in force_stype:
            forced_stype = self._extract_image_cols_from_force_stype(
                dataframe.columns, force_stype
            )
        else:
            # If `image_prefix` not present, then it's safe to cast
            forced_stype = cast(
        ...
        if SemanticType.CATEGORICAL in semantic_types:
            categorical_values = {
                ...
                for col in semantic_types[SemanticType.CATEGORICAL]
            }
        else:
            categorical_values = None
        for stype, features in semantic_types.items():
            ...
            for feature_name in self.features[cast(_SemanticTypeValue, stype)]:
                ...
                if feature_name not in columns:
                    ...
                    if stype == SemanticType.IMAGE.value:
                        dataframe[feature_name] = missing_image_value
    ...
    def add_datasource_features(
        ...
        Args:
            ...
            force_stype: Columns for which to change the semantic type.
                ...
                Example:
                `{'categorical': ['target_column'], 'continuous': ['age', 'salary']}`
...
class _BitfountSchemaMarshmallowMixIn:
    ...
    class _Schema(_CamelCaseSchema):
        ...
        def sort_tables(self, data: _JSONDict, **kwargs: Any) -> _JSONDict:
            ...
            new_data = {}
            new_data["tables"] = sorted(data["tables"], key=lambda d: d["name"])
...
class BitfountSchema(_BitfountSchemaMarshmallowMixIn):
    ...
    def add_tables_from_pod_database(
        self,
        pod_db_connector: PodDbConnector,
        pod_name: str,
        datasource_tables: List[str],
        ignore_cols: Optional[Mapping[str, Sequence[str]]] = None,
        force_stypes: Optional[
            Mapping[
                str,
        ...
        """Adds schema tables from the pod database.

        Args:
            pod_db_connector: The pod database connector object.
            pod_name: The name of the overarching pod.
            datasource_tables: A list of table names for the datasource.
            ignore_cols: A list of columns to be ignored in the datasource.
                Defaults to None.
            force_stypes: A mapping of table names to a mapping of semantic
                types to feature name list. Defaults to None.
        """
        with contextlib.closing(
            pod_db_connector.get_db_connection_from_name(pod_name=pod_name)
        ) as conn:
            cur = conn.cursor()
            cur.execute("SELECT name FROM sqlite_master WHERE type='table';")
            tables = [
                table[0] for table in cur.fetchall() if table[0] in datasource_tables
            ]
        ...
        for table_name in tables:
            ...
            if table_df is None:
                raise BitfountSchemaError(
                    f"Table {table_name} not found in the pod database."
                )

            if table_df.empty:
                raise BitfountSchemaError(
                    f"Table {table_name} found in the pod database but it is empty."
                )

            ignore_cols_ = None if not ignore_cols else ignore_cols.get(table_name)
            if ROW_INDEX_COLUMN in table_df.columns:
                if ignore_cols_ is not None:
                    ignore_cols_ = list(ignore_cols_)
                    ignore_cols_.append(ROW_INDEX_COLUMN)
                else:
                    ignore_cols_ = [ROW_INDEX_COLUMN]

            force_stype = None if not force_stypes else force_stypes.get(table_name)
            # Create table if it doesn't exist
            if table_name not in self.table_names:
                self.tables.append(TableSchema(table_name))
            table_schema = self.get_table_schema(table_name)
            table_schema._add_features_from_dataframe(
                dataframe=table_df,
                ignore_cols=ignore_cols_,
                force_stype=force_stype,
            )
