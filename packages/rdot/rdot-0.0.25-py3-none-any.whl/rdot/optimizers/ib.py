...
from ..information import information_cond, MI
from ..distortions import ib_kl
...
class IBOptimizer(BaseRDOptimizer):
    ...
    def compute_distortion(self, *args, **kwargs) -> float:
        # TODO: we may still need to debug this; watch out for negative values
        # return np.exp(logsumexp(self.ln_px + self.ln_qxhat_x + np.log(self.dist_mat)))
        I_xy = MI(np.exp(self.ln_pxy))
