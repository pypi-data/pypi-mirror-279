class LightcurveStruct(object):
    ...
    def plot(self, kind="lightcurve", colours=None, bands=None, freq=None):
        from ..Plotting.plotmap import map_to_plot

        return map_to_plot(self, kind=kind, colours=colours, bands=bands, freq=freq)
    ...
    def sigmaclip(self, sigma=3):
        ...
        if self.data:
            self.data = [
                sigma_clip(band, sigma) if band is not None else None
...
class GeneralQuery(object):
    def __init__(self, survey, radius, pos, username, password, pmra, pmdec):
        self.survey = survey
        self.radius = radius
        self.pos = pos
        self.url = ""
...
class AtlasQuery(GeneralQuery):
    ...
    def format_data(self, data):
        ...
        filtered_list = []
        for _, val in enumerate([obs_arr, mag, mag_err, mjd, mjd_ori, ra_arr, dec_arr]):
...
def query(survey, source, pos, radius, username=None, password=None):
