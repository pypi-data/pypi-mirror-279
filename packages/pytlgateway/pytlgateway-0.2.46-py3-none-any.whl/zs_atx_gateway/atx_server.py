...
class atxServer(MongoClientTradeGateway):
    ...
    def init_dbf_tables(self):
        self.insert_filename = self.recv_msg_dir + '\\' + FILES.TASKS + self.date + '.dbf'
        self.insert_order_table = dbf.Table(filename=self.insert_filename,codepage='utf8', on_disk=True)
    ...
    def monitor_algo_order_insert(self):
        while not self.is_stopped:
            ...
            with self.insert_order_table.open(mode=dbf.READ_WRITE):
                self.logger.info(f"[init_dbf_tables open] (filename){self.insert_filename}")
