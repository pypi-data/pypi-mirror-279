...
class kfServer(MongoClientTradeGateway):
    ...
    def req_buy_order_insert(self, obj):
        ...
        LimAction = 1  #涨跌停后交易
        try:
            LimAction = obj['executionPlan']["LimAction"]
        except:
            pass
        price = 0.0
        ...
        if algo_type == 105:
            trade_param += f':maxpercentageF={participate}'
    ...
    def update_order(self,record):
        ...
        if not oid in self.oid_to_acc: #shut_down or other problem
            ...
            if not order_info_target is None:
                ...
                self.logger.warning(f"[update_order] can't_find_oid (oid){oid}")
                return
        else:
    ...
    def on_trade_msg(self, record):
        ...
        if not oid in self.oid_to_acc: #shut_down or other problem
            ...
            if not order_info_target is None:
                ...
                self.logger.warning(f"[on_trade_msg] can't_find_oid (oid){oid}")
                return
        else:
            acc = self.oid_to_acc[oid]
            order_dict = self.oid_to_req[oid]
