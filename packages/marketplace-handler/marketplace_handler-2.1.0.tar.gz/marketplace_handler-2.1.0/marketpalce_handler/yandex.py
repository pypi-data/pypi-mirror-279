...
class Yandex(Marketplace):
            ...
            for product in products.get("result").get("offerMappings"):
                ...
                try:
                    products_from_market.update(
                        {
                            barcode: product.get("offer").get("basicPrice").get('value')
                        }
                    )
    ...
    def refresh_prices(self, products_data: List[YandexItem]):
        ...
        for i in range(0, len(valid_products), self.stock_limit):
            if count_after_await == 6:
                self._logger.info("Wait a minute before update again for avoid error")
                count_after_await = 0
                sleep(65)
            ...
            try:
                ...
                err_msg = error_response.get('errors')

                if err_msg and 'Duplicated offers found' in err_msg[0].get('message'):
                    continue

                self._logger.error(f"Cannot update many items. Error: {error_response}")
                raise exc
