...
class Wildberries(Marketplace):
    ...
    def get_prices_from_market(self) -> dict:
        ...
        while True:
            ...
            products.update(
                {
                    str(product["nmID"]): product["sizes"][0]["price"]
    ...
    def refresh_price(self, product_data: WbItem):
        try:
            ...
            if is_too_small_price(price_from_ms=product_data.price, price_from_market=product_data.market_price):
                self._logger.warning(f"Price decreased by 50% or more for ms_id: {product_data.ms_id}.")
                return {
                    product_data.ms_id: {
            ...
            self._update_prices([product_data], "price",)
            return True
        except HTTPError as e:
            self._logger.error(
    ...
    def refresh_prices(self, products_data: List[WbItem]):
        ...
        for i in range(0, len(valid_products), settings.WB_ITEMS_REFRESH_LIMIT):
            ...
            self._update_prices(valid_products[i: i + settings.WB_ITEMS_REFRESH_LIMIT], "price")

        return suspicious_products

    def _update_prices(self, items: List[WbItem], update_value):
