...
class AIOKafkaConsumer:
    ...
    async def getone(self, *partitions) -> ConsumerRecord:
        ...
        """
        log.info("Getting one message from Kafka")
        assert all(map(lambda k: isinstance(k, TopicPartition), partitions))
        if self._closed:
            log.error("Consumer is closed. Can't get messages.")
        ...
        with self._subscription.fetch_context():
            log.info("Fetching message from partitions %s", partitions)
            msg = await self._fetcher.next_record(partitions)

        log.info("Got message from Kafka: %s Exiting", msg)
        return msg
