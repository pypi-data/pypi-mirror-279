...
def get_ck_list(origin_key_list: List[str]) -> Tuple[List[Tuple[str, str]], dict]:
    """
    sn_list_key = f"res:cookie:*:jd:{plat_type}:*"
...
def __is_banded(sn: str):
    """某个cookie是否被禁用"""
    value = rds.client.get(f"res:banded:jd:{sn}")
    return value == "1"


def __banded_cookie(sn: str, freeze_time: int = 7200):
    """禁用某个cookie"""
    key = f"res:banded:jd:{sn}"
...
def get_cookie_info_by_sn(cookie_key: str) -> dict:
    cookie_str = rds.client.get(cookie_key)
    return json.loads(cookie_str)


def remove_cookie(biz_id: str, sn: str):
    """
    移除cookie
    """
...
def release_cookie(sn: str):
    ...
    把移除的cookie释放，解冻
    """
    rds.client.delete(f"res:jd:banded:{sn}")


def get_cookie(biz_id: str, keys: List[str] = []) -> Optional[ResCookie]:  # noqa
    """
    根据业务编码获取cookie
    :param biz_id: 业务编号，如：jd_price
    ...
    cookie_queue_key = f'res:cookie:{channel}:jd:{plat_type}:queue'
    ...
    while True:
        ...
        if __is_over_times(biz_id, sn, use_times):
            continue
        cookie_key = f'res:cookie:{channel}:jd:{plat_type}:{sn}'
        cookie_info = get_cookie_info_by_sn(cookie_key)
