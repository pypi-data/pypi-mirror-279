...
class FGA:
    ...
    def track(self, event: BaseEvent):
        ...
        if type(event) == Event:
            if any(x is None for x in [event.event_name, event.user_id, event.event_type]):
                raise InvalidEventError("Invalid event properties.")
        if type(event) == Asset:
            if any(x is None for x in [event.user_id, event.source_id, event.source_type, event.type]):
                raise InvalidEventError("Invalid asset properties.")
        if type(event) == Session:
            if any(x is None for x in [event.user_id, event.session_id]):
                raise InvalidEventError("Invalid session properties.")
        if type(event) == User:
            if any(x is None for x in [event.user_name, event.user_id]):
                raise InvalidEventError("Invalid user properties.")
        if type(event) == Account:
            if any(x is None for x in [event.wallet_address, event.user_id]):
                raise InvalidEventError("Invalid account properties.")

        try:
            self.configuration.server_url = constants.SERVER_URL[server_urls[type(event)]]
        except KeyError:
