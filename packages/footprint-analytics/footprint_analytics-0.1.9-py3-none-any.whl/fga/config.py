...
Classes:
    Config: Class that store configurations of FGA client
"""
...
from fga.event import BaseEvent, Plan, IngestionMetadata
from fga.storage import InMemoryStorageProvider, StorageProvider, Storage


class Config:
    """FGA client configuration class used to config client behavior

    Args:
        token (str): The API key of FGA project that events sent to. Required
        ...
        logger (optional): The logger used by FGA client. Default to logging.getLogger(constants.LOGGER_NAME).
        ...
        storage_provider (FGA.storage.StorageProvider, optional): Default to InMemoryStorageProvider.
            Provide storage instance for events buffer.
        plan (FGA.event.Plan, optional): Tracking plan information. Default to None.
        ingestion_metadata (FGA.event.IngestionMetadata, optional): Ingestion metadata. Default to None.
    ...
    def server_url(self):
        if self._url:
            return self._url
        return constants.SERVER_URL[constants.EVENT]
