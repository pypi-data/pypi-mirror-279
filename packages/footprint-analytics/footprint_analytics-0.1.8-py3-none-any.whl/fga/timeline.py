...
from fga.constants import LOGGER_NAME
        ...
        with self.locks[plugin_type]:
            for plugin in self.plugins[plugin_type]:
                ...
                try:
                    ...
                    self.logger.exception(f"Invalid event body {event}, {e}")
                except Exception as e:
                    self.logger.exception(f"Error for apply {plugin_type.name} plugin for event {event}, {e}")
        return result
