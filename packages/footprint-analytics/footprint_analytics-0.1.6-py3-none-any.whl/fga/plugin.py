...
from fga.event import BaseEvent, GroupIdentifyEvent, IdentifyEvent, RevenueEvent, User
...
from fga.worker import Workers
from fga.constants import DEFAULT_HEADER, UserPropertiesOperation
...
class UserEnrichmentPlugin(EventPlugin):

    def __init__(self):
        super().__init__(constants.PluginType.ENRICHMENT)
        self.configuration = None
        self.url = constants.SERVER_URL[constants.USER]
    ...
    def execute(self, event: BaseEvent) -> None:
        if not isinstance(event, User):
            return event
        default_header = copy.deepcopy(DEFAULT_HEADER)
        default_header["token"] = self.configuration.api_key
        default_header["project-id"] = str(self.configuration.project_id)
        get_use_url = f'{self.url}/{event.user_id}'
        r = requests.get(get_use_url, headers=default_header)
        result = r.json()
        if len(pydash.get(result, 'data', [])) == 0:
            return event
        current_user = pydash.get(result, 'data.0')
        if pydash.get(event, 'event_properties.user_operation') == constants.IDENTITY_OP_SET_ONCE \
                and pydash.get(current_user, pydash.get(event, 'event_properties.set_once_key')) is not None:
            return None
        user_properties = {}
        for key, value in current_user.items():
            user_properties[key] = value if pydash.get(event, f'user_properties.{key}') is None else event.user_properties[key]
        event['user_properties'] = user_properties
        return event


class EnrichmentPlugin(EventPlugin):
    ...
    def execute(self, event: BaseEvent):
        event["project_id"] = self.configuration.project_id
        return event
...
class FGADestinationPlugin(DestinationPlugin):
...
class ContextPlugin(Plugin):
    ...
    def execute(self, event: BaseEvent) -> BaseEvent:
        ...
        if not event.event_id:
            event["event_id"] = event["insert_id"]
