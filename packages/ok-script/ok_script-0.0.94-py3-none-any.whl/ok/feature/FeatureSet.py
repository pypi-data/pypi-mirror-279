...
def read_from_json(coco_json, width=-1, height=-1):
    ...
    for image_id, file_name in image_map.items():
        ...
        if width != -1 and height != -1 and (width != whole_image.shape[1] or height != whole_image.shape[0]):
            ...
            whole_image = cv2.resize(whole_image, (width, height))
        else:
            scale_x, scale_y = 1, 1
...
def filter_and_sort_matches(result, threshold, width, height):
    ...
    # Sort matches based on their location
    matches.sort(key=lambda pt: (pt[0], pt[1]))

    # Filter out overlapping matches
    unique_matches = []
    for pt in matches:
        # Only compare with the last match in unique_matches
        if not unique_matches or not (unique_matches[-1][0] <= pt[0] <= unique_matches[-1][0] + width and
                                      unique_matches[-1][1] <= pt[1] <= unique_matches[-1][1] + height):
