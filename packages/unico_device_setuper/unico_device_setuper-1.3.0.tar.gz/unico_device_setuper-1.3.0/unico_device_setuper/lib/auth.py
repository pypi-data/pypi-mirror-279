...
async def get_credentials[T: pydantic.BaseModel](
    ...
    cnsl.print_blue(f'Connexion à votre compte {login_platform_name.capitalize()}:')
...
async def choose_client(
    clients: list[unitech.LoginChooseResponseClientsItem], strategy: ClientStrategy
):
    match strategy:
        ...
        case PickInteractive():
            ...
            client = await cnsl.print_choose(
                clients, prompt='Client: ', formater=lambda c: c.name.strip()
            )
            cnsl.print()
...
async def get_unitech_auth_token(env: env.Env, client_strategy: ClientStrategy):
    ...
    client = await choose_client(login_first_stage_response.parsed.clients, client_strategy)
    if client is None:
        return None
