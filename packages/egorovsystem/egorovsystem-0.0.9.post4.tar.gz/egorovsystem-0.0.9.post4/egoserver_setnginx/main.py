...
def run():
    ...
    with open(nginx / "sites-available" / server_name, "w") as f:
        f.write(content)
    
    os.system(f"rm {nginx / 'sites-enabled' / server_name}")
    os.system(f"ln -s {nginx / 'sites-available' / server_name} {nginx / 'sites-enabled' / server_name}")
