...
from relari.core.types import UID, ToolCall
from relari.core.utils import type_hint_to_str
...
class DatasetManifest:
    ...
    def to_yaml(self):
        return {
            ...
            "fields": {
                field_name: field.to_dict() for field_name, field in self.fields.items()
            },
        }
...
class Dataset:
    def __init__(
        ...
        self._manifest = self._load_or_infer_manifest(dataset_path, manifest_path)
    ...
    def from_data(
        ...
        if manifest is not None:
            dataset._manifest = DatasetManifest.from_json(manifest)
        else:
    ...
    def _load_or_infer_manifest(
        self, dataset_path: Path, manifest_path: typing.Optional[Path]
    ) -> DatasetManifest:
        if manifest_path is None or not manifest_path.exists():
            return self._infer_manifest(dataset_path)
        else:
            with open(manifest_path, "r") as manifest_file:
                ...
                fields = {
                    field_name: DatasetField(
                        name=field_name,
                        type=(
                            eval(field_info["type"], _SAFE_DICT)
                            if field_name != "uid"
                            else UID
                        ),
                        description=field_info.get("description", ""),
    ...
    def _infer_manifest(
        self, dataset_path: typing.Optional[Path] = None
    ) -> DatasetManifest:
        fields = dict()
        for sample in self._data:
        ...
        return DatasetManifest(
            name=dataset_path.stem if dataset_path is not None else "",
            description="",
            format="jsonl",
            license="",
            fields=fields,
