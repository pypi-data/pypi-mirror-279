...
class SyncSession(SessionMixIn, object):
    ...
    def _aggregate(self, cname: str, pipline: List[Dict], **kwargs) -> List[Dict]:
        """
        根据pipline进行聚合查询
        Args:
            cname: collection name
        ...
        try:
            for doc in self.db.get_collection(cname).aggregate(pipline, **kwargs):
                if doc.get("_id", None) is not None:
                    doc["id"] = doc.pop("_id")
                result.append(doc)
        except InvalidName as e:
    ...
    def insert_one(self, query: Query) -> str:
        ...
        if not isinstance(document, MutableMapping):
            raise MongoError("insert one document failed, document is not a mapping type.")
        return self._insert_one(query._cname, self._update_doc_id(document))  # type: ignore
    ...
    # noinspection DuplicatedCode
    def aggregate(self, query: Query, **kwargs) -> List[Dict]:
        """
        根据pipline进行聚合查询
        Args:
            query: Query class
        ...
        return self._aggregate(query._cname, pipline, **kwargs)
