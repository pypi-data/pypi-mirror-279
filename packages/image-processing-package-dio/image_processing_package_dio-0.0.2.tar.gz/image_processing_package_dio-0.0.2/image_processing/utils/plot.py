...
def plot_result(*args):
    ...
    fig, axis  = plt.subplots(1, ncols=number_images, figsize=(12,4))
...
def plot_histogram(image):
    fig, axis =plt.subplots(nrows=1, ncols=3, figsize=(12,4), sharexx=True, sharey=True)
    color_lst = ['red', 'green', 'blue']
    for index, (ax, color) in enumerate(zip(axis, color_lst)):
        ax.set_tile('{} histogram'.format(color.title()))
