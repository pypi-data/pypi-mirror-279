...
from pih.tools import while_not_do, j, nns, lw, nnd, e
...
class BackupApi:
        ...
        def internal_start_robocopy_job(
            ...
            def run_robocopy_job(job_item: RobocopyJobItem) -> None:
                ...
                if not job_item.live:
                    ...
                    job_status.last_started = A.D.now_to_string(
                ...
                process: CompletedProcess = A.EXC.execute(
                    A.EXC.create_command_for_psexec(
                        (RBK.NAME, j((RBK.JOB_PARAMETER_NAME, job_path))),
                        job_item.host,
                ...
                if job_item.live:
                    ...
                    A.E.backup_robocopy_job_was_started(job_name(), job_status)
                    while_not_do(
                        lambda: not A.EXC.check_process_is_running(pid, job_item.host),
                        sleep_time=5,
                    )
                else:
    ...
    def get_job_path(name: str, source: str, destination: str) -> str:
        return A.PTH.for_windows(A.PTH.join(
