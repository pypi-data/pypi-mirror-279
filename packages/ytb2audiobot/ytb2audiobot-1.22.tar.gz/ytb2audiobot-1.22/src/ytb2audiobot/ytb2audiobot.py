...
COMMANDS_SPLIT = [['split', 'split']]
COMMANDS_BITRATE = [['bit', 'bitrate'], ['bitrate', 'bitrate']]
COMMANDS_SUBTITLES = [['sub', 'subtitles'], ['subs', 'subtitles'], ['subt', 'subtitles'], ['subtitles', 'subtitles']]
...
def subtitles(movie_id: str, discovered_word: str = ''):
    context = {
        'error': '',
        'text': ''
    }
    try:
        ...
        context['error'] = '⛔️ YouTubeTranscriptApi: TranscriptsDisabled'
        return context
    except (ValueError, Exception):
        context['error'] = '⛔️ Undefined problem in YouTubeTranscriptApi'
        return context
    print('🌽 Subtitles: ', subtitles)

    if not discovered_word:
        context['text'] = get_answer_text(subtitles)
        return context
...
async def processing_ytb2audio_container(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if False:
...
def run_bot(token: str, opt_keepfiles: str):
    ...
    application.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, processing_ytb2audio_container))
