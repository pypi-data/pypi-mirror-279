...
:LastEditTime: 2024-06-18 17:29:40
...
class CouponGrantInfoHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        coupon_list =  CouponHelper.get_user_grant_coupon(user_code, open_id, shop_id)
        if coupon_list: 
            # 验证可领取的列表
            ...
            if coupon_list:
                ret_data["coupon_model"] = {
                    ...
                    "coupon_price": coupon_list[0]["coupon_price"],
                    "grant_picture": coupon_list[0]["grant_picture"],
                    "coupon_discount": coupon_list[0]["coupon_discount"],
                    "use_price": coupon_list[0]["use_price"],
    ...
    def get_async(self, *args, **kwargs):
        ...
        if coupon_list:
            for item in coupon_list:
                ...
                item["coupon_price"] = item["coupon_info"]["coupon_price"]
                item["coupon_discount"] = item["coupon_info"]["coupon_discount"]
            ...
            if coupon_list:
                for item in coupon_list:
                    ...
                    if item["goods_limit"] == 0:
                        # 验证总金额是否满足
                        if check_result["data"]["goods_price"] >= item["use_price"]:
                            item["use_status"] = 1
                            continue
                        else:
                            item["use_status"] = 0
                        ...
                        if intersection_list:
                            # 获取交集商品总金额
                            if sum([goods_total_price_dict[x] for x in intersection_list]) >= item["use_price"]:
                                item["use_status"] = 1
                                continue
                            else:
                                item["use_status"] = 0
