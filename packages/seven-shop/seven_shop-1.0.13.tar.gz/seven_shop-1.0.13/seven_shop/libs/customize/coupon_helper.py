...
"""
:Author: KangWenBin
:Date: 2024-05-31 15:02:38
:LastEditTime: 2024-06-19 11:12:10
...
class CouponHelper:
    ...
    @classmethod
    def get_user_info(self,user_code,open_id):
        """
        :description: 获取用户信息
        ...
        result = requests.post("http://192.168.28.109:7789/client/user/check_user_info",json=param)       
    ...
    @classmethod
    def get_user_grant_coupon(self, user_code, open_id, shop_id):
        """
        ...
        # 获取当前用户已获取的优惠券
        user_coupon_list = CouponRecordModel().get_dict_list(where="coupon_id in %s and user_code=%s", params=[coupon_ids, user_code])
        ...
        if [x for x in coupon_list if 0 in x["grant_type"] or 3 in x["grant_type"]]:
            user_data = self.get_user_info(user_code,open_id)
        
        is_pay = None
        if [x for x in coupon_list if 1 in x["grant_type"] or 2 in x["grant_type"]]:
            order_model = OrderModel().get_dict(where="user_code=%s and pay_time > 0 and pay_order_id!=''", params=[user_code])
            if order_model:
                is_pay = True
            else:
                is_pay = False  
    ...
    def refund_coupon_check(self, coupon_id, order_id):
        ...
        if coupon_id > 0:
            ...
            if order_goods_count == order_refund_goods_count:
                # 退还优惠券
                CouponRecordModel().update_table("order_id = %s,use_time = 0,status = 0","id = %s",[order_id,coupon_id])
