...
:LastEditTime: 2024-06-12 14:50:01
...
class CouponModelEx(CouponModel):
    ...
    def get_grant_coupon_list(self,shop_id):
        ...
        """        
        sql = "SELECT a.*,b.id AS grant_id,b.grant_type FROM coupon_tb a JOIN coupon_grant_tb b ON a.id = b.coupon_id WHERE a.status = 1 AND b.status = 1 AND a.shop_id = %s and a.begin_time<= %s AND a.end_time>=%s AND b.begin_time<= %s AND b.end_time>=%s "
    ...
    def get_receive_coupon_list(self,user_code,shop_id):
        """
        :description: 获取用户领取的优惠券列表
        :last_editors: KangWenBin
        """ 
    ...
    def check_coupon_inventory(self,coupon_id):
        ...
        record_number = CouponRecordModel().get_total(where="coupon_id = %s",params=[coupon_id])
