...
class Table:
    ...
    playbooks: OptionalType[List]
    entity: ParseResults
    model_allowlist: OptionalType[List[str]]
    ...
    def is_in_allowlist(self) -> bool:
        ...
        return is_model_in_allowlist(self.name, self.model_allowlist)
    ...
    def construct_dapi_location(self) -> str:
        ...
        return os.path.join(self.app_full_path, "dapis", f"{self.name}.dapi.yaml")

    def construct_urn(self) -> str:
        """Construct URN for table"""
        return f"{self.org_name_snakecase}.typeorm.{self.app_name}.{self.name}"
    ...
    def construct_team_urn(self) -> OptionalType[str]:
        ...
        return (
            construct_owner_team_urn_from_playbooks(self.playbooks, self.name)
            if self.playbooks
            else None
        )
...
class TypeOrmDapiValidator(PackageScopedDapiValidatorBase):
    ...
    def find_tables(  # pylint: disable=too-many-locals
        self, filename: str, content: str, pkg: str, table_config: DictType
    ) -> List[Table]:
        ...
        for entity in entities:
            tables.append(
                Table(
                    ...
                    table_config.get("playbooks"),
                    entity,
                    table_config.get("model_allowlist", []),
                )
            )
        return tables
    ...
    def get_typeorm_entities(self) -> List[Table]:
        ...
        for filename, content, pkg, config in all_files:
            if any(keyword in content for keyword in self.CHECK_KEYWORD_IN_FILE) or any(
                keyword in filename for keyword in self.CHECK_KEYWORD_IN_FILENAME
            ):
                tables += self.find_tables(filename, content, pkg, config)
        return tables
    ...
    def process_tables_globally(self, tables: List[Table]) -> List[ProcessedTable]:
        ...
        for table in tables:
            ...
            processed_tables.append(
                ProcessedTable(
                    module=table.module,
                    app_full_path=table.app_full_path,
                    org_name_snakecase=table.org_name_snakecase,
                    playbooks=table.playbooks,
                    entity=table.entity,
                    model_allowlist=table.model_allowlist,
