...
class SequelizeDapiValidator(PackageScopedDapiValidatorBase):
    ...
    def get_sequelize_tables(self) -> List[Table]:
        ...
        in_scope_files = self.get_all_files()
        file_configs = {}
        file_pkgs = {}
        for filename, _, pkg, config in in_scope_files:
            file_configs[filename] = config
            file_pkgs[filename] = pkg

        def delegate(filename, node, _):
            table_config = file_configs.get(filename, {})
            if self.is_sequelize_define_call(node):
                ...
                if is_model_in_allowlist(
                    table.name, table_config.get("model_allowlist")
                ):
                    tables.append(table)
        ...
        for filename, content, pkg, config in in_scope_files:
            lines = content.split("\n")
        ...
        for class_name in model_classes:
            ...
            if is_model_in_allowlist(table.name, table_config.get("model_allowlist")):
                tables.append(table)
        return tables
