...
def refresh_models() -> None:
    ...
    for schema in ts.static_order():
        if schema_matches_any_mask(schema, mask):
            LOG.debug(f"Refreshing schema {schema}")
            # Adapter.clone_schema(f'{target_db}.{schema_only}',f'{target_db}.{schema_only}_{suffix}')
            refresh_schema(target_db, schema)
            # Adapter.swap_schema(f'{target_db}.{schema_only}_{suffix}',f'{target_db}.{schema_only}')


def refresh_schema(target_db: str, child_schema: str) -> None:
    LOG.debug(f"in refresh_schema target_db={target_db}, child_schema={child_schema}")
        ...
        if fqn.startswith(child_schema):
            if domain.models.sql_models.get(fqn):
                ...
                if thismodel and thismodel._materialization == "table":
                    LOG.debug(f"Refreshing model {fqn} in schema {target_schema}")
