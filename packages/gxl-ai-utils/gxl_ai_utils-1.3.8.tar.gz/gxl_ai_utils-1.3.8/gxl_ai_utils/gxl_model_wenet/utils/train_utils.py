...
from ..utils.scheduler import WarmupLR, NoamHoldAnnealing
...
def add_dataset_args(parser):
    parser.add_argument('--data_type',
                        default='raw',
                        choices=['raw', 'shard'],
                        help='train and cv gxl_data type')
    parser.add_argument('--train_data', required=True, help='train gxl_data file')
    parser.add_argument('--cv_data', required=True, help='cv gxl_data file')
...
def add_deepspeed_args(parser):
    parser.add_argument('--timeout', default=30, type=int,
                        help='timeout (in seconds) of ._join. ' +
                             '30s for aishell & 300s for .speech')
...
def check_modify_and_save_config(args, configs):
    if args.train_engine == "torch_ddp":
        ...
        # NOTE(xcsong): DeepSpeed does not support uneven gxl_data. When using custom
        #   dataset, we need to manually ensure that the gxl_data is evenly distributed
        #   across all processe. we impl `train_utils.py::._join` for this func
        ...
        #   dataloader uses PyTorch's torch.utils.gxl_data.DistributedSampler which does
        #   not support IterableDataset, IterableDataset is extremly useful in large
        #   scale training because it lets you stream the gxl_data without having to
