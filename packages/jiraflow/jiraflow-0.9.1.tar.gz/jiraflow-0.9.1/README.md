...
Once you have made sure python is installed, you can download `jiraflow` via pip:
`python -m pip install --upgrade jiraflow`

## Run jiraflow
If your installation was successfull, you can now run `jiraflow` via the commandline. When not supplied with any parameter for a configuration file, it will automatically copy the `ExampleConfig.json` to your current directory and run using. This will fail with the following error: 
```
Error while executing jiraflow:
...
```
As you must configure your connection to Jira first.
**Note**: It's recommended to rename your config file from *ExampleConfig.json* to something more meaningful (like *TeamNameConfig.json*) and to specify this configuration file when running it again: `jiraflow --ConfigFileNames "TeamNameConfig.json"`.

### Running jiraflow with multiple Configurations
...
`jiraflow --ConfigFileNames "TeamA_Config.json" "TeamB_Config.json" "TeamC_Config.json"`
...
Read on to see details about how to configure `jiraflow`.

## Configuration
In the [ExampleConfig.json](https://github.com/LetPeopleWork/jiraflow/tree/main/jiraflow/ExampleConfig.json) file you can see the default configuration.
There are general settings, Jira specific settings, configurations per chart you want to create and specific forecast settings. Below you can find a summary of the various options.
...
| remainingBacklogQuery           | The query to get the remaining items for your forecast. Note that this should contain all items that are relevant for your forecast, so most likely you want to filter out closed and removed items. Check out the [JQL reference](https://www.atlassian.com/blog/jira/jql-the-most-flexible-way-to-search-jira-14) if you need support.             | issuetype in (\"Story\", \"Bug\") AND project = \"YourProject\" AND status not in (\"Done\") AND fixVersion = \"Release 24.10\" |
...
After you configured `jiraflow` with your queries and with your token, you are ready to go. simply run `jiraflow --ConfigFileNames "YourConfig.json"`. It will generate all the charts according to your configurations and will show you your forecasts in the commandline.
...
from jiraflow.WorkItemService import WorkItemService
```

You can create the various services and provide the necessary configuration as it best fits you:
```
work_item_service = WorkItemService(jira_url, username, api_token, estimation_field, history_in_days)
...
```
work_items = work_item_service.get_items_via_jql(item_query)
...
See [Github](https://github.com/LetPeopleWork/jiraflow/blob/main/Examples/use_individual_services.py) for a full example of how you can use this.