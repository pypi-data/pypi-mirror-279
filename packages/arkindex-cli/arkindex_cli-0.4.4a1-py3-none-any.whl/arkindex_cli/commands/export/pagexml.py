...
def add_pagexml_parser(subcommands):
    ...
    pagexml_parser.add_argument(
        ...
        type=UUID,
        help="Only retrieve the transcriptions created by a specific worker run.",
    )
    pagexml_parser.add_argument(
        "-o",
        "--output",
...
def find_line_transcription(
    parent: Element, element: Element, transcription_source: UUID | None, **kwargs
) -> Transcription | None:
    ...
    for child in children:
        ...
        for (line, parent_node) in flattened_lines:
            ...
            text_equiv_node = ET.SubElement(
                parent_node, "TextEquiv", attrib={"conf": "1.0"}
            )
