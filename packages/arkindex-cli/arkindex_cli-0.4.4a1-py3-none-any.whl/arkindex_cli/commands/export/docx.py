...
from arkindex_export import Element, Transcription, open_database
...
def add_docx_parser(subcommands):
    ...
    sources.add_argument(
        ...
        help="""
            ...
            'manual' or UUIDs of the worker run(s) that produced the transcriptions to be exported. The order in which
    ...
    sources.add_argument(
        ...
        type=uuid_or_manual,
        help="""
            'manual' or UUIDs of the worker version(s) that produced the transcriptions to be exported. The order in which
...
def get_transcription_query(element, worker_version_ids, worker_run_ids):
    ...
    for version_id in worker_version_ids:
        if version_id == "manual":
            transcription = query.where(
                Transcription.worker_version_id.is_null()
            ).limit(2)
            yield transcription
        else:
            transcription = query.where(
                Transcription.worker_version_id == version_id
            ).limit(2)
            yield transcription
...
def run(
    ...
    element_type: Optional[str] = "page",
    line_type: Optional[str] = None,
    worker_version_id: Optional[List[str]] = [],
    worker_run_id: Optional[List[str]] = [],
        ...
        for element in list_children(folder_id).where(Element.type == element_type):
            ...
            if merge:
                ...
                saved_transcriptions_count += generate_docx(
                    [element_transcription],
                    output_path / folder_id / element.name,
                )
