        ...
        for count, page in enumerate(page_list, start=1):
            ...
            if layout_tag:
                ...
                for item in subelements:
                    ...
                    if item.worker_version_id is not None:
                        worker_set.add(item.worker_version_id)
                        alto_subelement.set(
                            "PROCESSINGREFS", f"worker_{item.worker_version_id}"
                        )
            ...
            if page_transcriptions.count():
                ...
                for line in list_children(page.id).where(Element.type == line_type):
                    ...
                    # overwriting attribute would create a new string element
                        ...
                        worker_version_id = transcriptions_dict[
                            line.id
                        ].worker_version_id
