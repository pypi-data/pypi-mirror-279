...
def get_jira_results_looped(
    ...
    start_at = 0
    with tqdm_to_logger(total=issue_count, desc=f"Downloading issue id/key pairs") as pbar:
...
def get_buffered_date(
    issues: set[IssueMetadata], pull_from: datetime,
) -> datetime:
    """
    Given a set of issues, determine the last updated date of the last issue in the set.
    Do some goofy logic to account for 48 hours of buffer time for issues that may have been updated,
    and try to avoid some of the jql parsing problems you get with strings
    :param issues:
    :param pull_from:
    :return: string in format 'YYYY-MM-DD'
    """
    if len(issues) == 0:
        last_run_date_with_buffer = pull_from
    else:
        ...
        if last_run_date == pytz.utc.localize(datetime.min) or last_run_date == datetime.min:
            # problems if we try to subtract from datetime.min
            last_run_date_with_buffer: datetime = last_run_date
        else:
            last_run_date_with_buffer: datetime = last_run_date - timedelta(days=2)

    return last_run_date_with_buffer
...
def fetch_id_to_key_for_updated(
    ...
    for proj_id, issues in project_id_to_issue.items():
        last_run_date_with_buffer = get_buffered_date(issues, pull_from)
        logger.info(f"Pulling updated issues since {last_run_date_with_buffer} for {proj_id}")
        # jql will need a string in the format 'YYYY-MM-DD'
        last_run_buffered_string = format_date_to_jql(last_run_date_with_buffer)

        issue_count = _get_all_project_issue_counts(
            jira_connection,
            [str(proj_id)],
            last_run_date_with_buffer,
            10,
        )
        jql_expression = (
            f"project = {proj_id} AND updatedDate > "
            f"'{last_run_buffered_string}' "  # has to be a string in the jql query
            f"ORDER BY id ASC"
        )
...
def get_issue_list_to_download(
    ...
    * deletes since last pull:
    ...
    project_to_issue_map = defaultdict(set)
    for issue in jellyfish_issue_metadata:
        project_id_from_issue = int(issue.project_id)
        if issue.project_id not in valid_project_ids:
            continue
        project_to_issue_map[project_id_from_issue].add(issue)
    ...
    logger.info(
        f"{len(id_to_key_updated)} remote issues updated across all projects since last pull"
    )
        ...
        for project_key, count in project_issue_count_map.items():
            ...
            for start_at in range(0, count, batch_size):
                pool.submit(
                    ...
                    include_fields=include_fields,
                    exclude_fields=exclude_fields,
                ).add_done_callback(partial(custom_call_back, project_key=project_key))
