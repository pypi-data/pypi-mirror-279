        ...
        try:
            ...
            if retry < max_retries_for_retry_for_status:
                logging_helper.log_standard_error(
                    logging.ERROR,
                    error_code=3072,
                    extra={"retry": retry + 1, "max_retries": max_retries_for_retry_for_status},
                )
                continue
            else:
...
def tqdm_to_logger(*args, **kwargs) -> tqdm:
    ...
    ingestion_type = logging_helper.get_ingestion_type()
...
class ThreadPoolWithTqdm(ThreadPoolExecutor):
    ...
    desc: str
    total: int
    futures_lookup: dict[str, Future]
    ...
    def __init__(
        ...
        self.desc = desc
        self.total = total
        self.futures_lookup: dict[str, Future] = {}
    ...
    def __enter__(self):
        ...
        """
        self.prog_bar = tqdm_to_logger(desc=self.desc, total=self.total)
        return super().__enter__()

    def submit(self, fn: Callable, *args, **kwargs) -> Future:
        future = super().submit(fn, *args, **kwargs)
        self.futures_lookup[hash(future)] = future
        return future

    def get_results(self) -> Generator[Any, None, None]:
        for future in as_completed(self.futures_lookup.values()):
            # Update the progress bar each time we get a future back
            try:
                logging_helper.send_to_agent_log_file('Updating progress bar')
                self.update_progress_bar(future)
                logging_helper.send_to_agent_log_file('Getting future result in get_results')
                yield future.result()
                logging_helper.send_to_agent_log_file(
                    'Finished getting future result in get_results'
                )
            except Exception as e:
                logging_helper.send_to_agent_log_file(
                    f'Exception encountered in ThreadPoolExecutor: {e}', level=logging.WARNING
                )
                ...
                del self.futures_lookup[hash(future)]
    ...
    def __exit__(
        ...
        logging_helper.send_to_agent_log_file(f'__exit__ - {len(self.futures_lookup.keys())}')
        if self.futures_lookup.keys():
            logging_helper.send_to_agent_log_file(f'__exit__ - Getting results')
            for _ in self.get_results():
                # Iterate through the get_results generate to force progress bar load
                pass
        logging_helper.send_to_agent_log_file(f'__exit__ - Got results')
        ...
        self.prog_bar.close()
        logging_helper.send_to_agent_log_file(f'__exit__ - Exited successfully')
    ...
    def update_progress_bar(self, future: Future):
        ...
        """
        logging_helper.send_to_agent_log_file(f'Checking future.exception() in update_progress_bar')
        if future.exception():
            return

        logging_helper.send_to_agent_log_file(f'Getting future.exception() in update_progress_bar')
        result = future.result()

        logging_helper.send_to_agent_log_file(f'Getting tqdm_lock() in update_progress_bar')
        with tqdm.get_lock():
