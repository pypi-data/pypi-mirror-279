    ...
    def lookup(self):
        while True:
            ...
            if barcode in [None,]:
                return
            with Session(ENGINE) as session:
                query=session.query(DisplayItem).filter(or_(DisplayItem.Code.icontains(barcode),DisplayItem.Barcode.icontains(barcode),DisplayItem.Name.icontains(barcode),DisplayItem.Note.icontains(barcode)))
                query=query.group_by(DisplayItem.Aisle,DisplayItem.Date,DisplayItem.DisplayLocation)
                displays=query.all()
                ct=len(displays)
                headers=f"\n{Fore.light_green}DisplayItem No./{Fore.light_yellow} Total Display Items {Fore.orange_red_1} -> Code|Barcode|Name|DisplayLocation|Note{Style.reset}\n"
                print(headers)
                for num,d in enumerate(displays):
                    if d.Note in [None,]:
                        setattr(d,'Note','')
                        session.commit()
                        session.refresh(d)
                    short_display=f'''{d.Code}|{d.Barcode}|{d.Name}|{d.DisplayLocation}|{d.Note}'''
