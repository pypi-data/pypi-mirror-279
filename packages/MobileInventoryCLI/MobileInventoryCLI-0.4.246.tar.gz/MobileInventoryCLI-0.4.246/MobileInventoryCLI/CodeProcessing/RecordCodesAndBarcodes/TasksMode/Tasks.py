    ...
    def NewEntrySchematic(self):
        while True:
            ...
            with Session(self.engine) as session:
                ...
                if not check:
                    newEntry=self.mkNew(code=code,data={'Name':code,'Code':code,'CaseCount':1,'Facings':1})
                    if newEntry == None:
                    ...
                    'Name':check.Name,
                    'Code':check.Code,
                    'Facings':check.Facings,
                    'CaseCount':check.CaseCount,
                    }
                    print(f"{Fore.light_red}Item Exists please use '{Fore.light_yellow}ni{Fore.light_red}' to {Fore.light_sea_green}bypass... {Fore.light_magenta}prompting now for {Style.bold}updates...\n\n{Style.reset}")
                    updates=self.mkNew(code=check.Barcode,data=data)
                    if updates != None:
                        updates['InList']=True
                        updates['ListQty']=1
                        session.query(Entry).filter(Entry.Barcode==check.Barcode).update(updates)
                        session.commit()
                        session.flush()
...
{Fore.light_red}nfa|new entry from all|new_entry_from_all - {expo_color}new from all{Style.reset}
...
schematic{Style.reset}'''
        while True:
            try:
                doWhat=Prompt.__init2__(None,func=mkTl,ptext=f"{h}Do What?",helpText=htext,data=self)
                if doWhat in [None,]:
                    return
                elif doWhat.lower() in ['nfa',f"nfa","new entry from all","new_entry_from_all"]:
                    self.NewEntryAll()
    ...
    def NewEntryShelf(self):
        while True:
            ...
            with Session(self.engine) as session:
                check=session.query(Entry).filter(Entry.Barcode==code).first()
                if not check:
                    newEntry=self.mkNew(code=code,data={'Name':code,'Code':code,'CaseCount':1,'Price':1})
                    if newEntry == None:
                        print(f"{Fore.orange_red_1}User canceled!{Style.reset}")
                        return
                    newEntry['Barcode']=code
