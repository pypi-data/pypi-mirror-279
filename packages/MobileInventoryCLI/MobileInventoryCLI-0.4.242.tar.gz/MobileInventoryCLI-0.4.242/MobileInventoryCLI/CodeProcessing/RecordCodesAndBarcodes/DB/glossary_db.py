...
{Fore.light_green}{'|'.join(['delete_id','remove_id','rmid','dlid'])}{Style.reset} - {Fore.light_steel_blue}delete/remove a glossary term{Style.reset}
...
{Fore.light_green}{'|'.join(['ihsv','import_hsv',])}{Style.reset} - {Fore.light_steel_blue}Import HSV data (hashtag separated columns{Style.reset}
    '''
            ...
            if filename.exists() and filename.is_file():
                ...
                with Session(ENGINE) as session:
                    for num,row in enumerate(df.itertuples()):
                        check=session.query(Glossary).filter(Glossary.Term==row.Term).first()
                        if check:
    ...
    def auto_search(self,selection=False):
        try:
