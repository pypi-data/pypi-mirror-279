...
class YeeduOperator():
    ...
    def __init__(self, job_url, connection_name,login_password, *args, **kwargs):
        """
        ...
        **kwargs: Additional keyword arguments.
        ...
        self.job_url = job_url
        self.base_url, self.tenant_id, self.workspace_id, self.job_type, self.conf_id = self.extract_ids(self.job_url)
        self.logger = get_run_logger()

        self.hook: YeeduHook = YeeduHook(conf_id=self.conf_id, tenant_id=self.tenant_id, base_url=self.base_url, 
                                         workspace_id=self.workspace_id, connection_name=self.connection_name, password=self.password)


    def get_password(self):
        secret_block = Secret.load(self.login_password)
        password = secret_block.get()
        if not password:
            raise ValueError(f"password is not set'{self.login_password}'")
        return password
...
class YeeduJobRunOperator():
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.hook: YeeduHook = YeeduHook(conf_id = self.job_conf_id, tenant_id=self.tenant_id, base_url=self.base_url, workspace_id=self.workspace_id, connection_name=self.connection_name,password=self.password)
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            self.logger.info("Logs for Job ID %s (%s)", job_id, job_log)
...
class YeeduNotebookRunOperator():
    ...
    def __init__(self, base_url, workspace_id, notebook_conf_id, tenant_id, connection_name, password, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        self.password=password
        self.logger = get_run_logger()
        self.hook: YeeduHook = YeeduHook(conf_id = self.notebook_conf_id, tenant_id=self.tenant_id, base_url=self.base_url, workspace_id=self.workspace_id, connection_name=self.connection_name,password=self.password)
