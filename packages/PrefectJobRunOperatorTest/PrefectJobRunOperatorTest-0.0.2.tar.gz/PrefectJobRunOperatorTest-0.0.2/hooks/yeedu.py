...
class YeeduHook():
    ...
    def __init__(self, conf_id: int, tenant_id: str, base_url: str, workspace_id: int, connection_name: str, password: str, *args, **kwargs) -> None:
        """
    ...
    # def get_cert_file():
    #     yeedu_ssl_cert_file = Variable.get()
    #     return yeedu_ssl_cert_file.value
    
    def get_connection_details(self):
        json_data = JSON.load(self.connection_name).value
        username = json_data.get("username")
        YEEDU_SSL_CERT_FILE = json_data.get("YEEDU_SSL_CERT_FILE")
        YEEDU_VERIFY_SSL = json_data.get("YEEDU_VERIFY_SSL")
        if not username:
            raise ValueError(f"Username is not set in the connection '{self.connection_name}'")
        ...
        try:
            ...
            if self.YEEDU_VERIFY_SSL == 'true':
                ...
                if not self.YEEDU_SSL_CERT_FILE:
                    ...
                    raise ValueError(f"Please provide YEEDU_SSL_CERT_FILE if YEEDU_VERIFY_SSL is set to: {self.YEEDU_VERIFY_SSL} (default: true)")
                else:
                    ...
                    if os.path.isfile(self.YEEDU_SSL_CERT_FILE):
                        ...
                        raise ValueError(f"Provided self.YEEDU_SSL_CERT_FILE: {self.YEEDU_SSL_CERT_FILE} doesnot exists")
    ...
    #         raise ValueError(f"Username or password is not set in the connection '{self.connection_id}'")
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    attempts_failure += 1
                    delay = 20
                    self.logger.info(f"GET Job Status API request failed (attempt {attempts_failure}/{max_attempts}) due to {e}")
