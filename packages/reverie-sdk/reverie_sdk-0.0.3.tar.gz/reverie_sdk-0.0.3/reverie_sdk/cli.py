...
def get_parser() -> ArgumentParser:
    ...
    def _add_subparsers(_parser: ArgumentParser, _parser_data: dict):
        subparsers: dict = _parser_data.get("subparsers", {})
        if len(subparsers) > 0:
    ...
    try:
        ...
        if os.path.exists(config_fp) and os.stat(config_fp).st_size > 0:
            with open(config_fp, "rb") as f:
                creds: list[dict[str, str]] = json.loads(f.read().decode(ENCODING))
