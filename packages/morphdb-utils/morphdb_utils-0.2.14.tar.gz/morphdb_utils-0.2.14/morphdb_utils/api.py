...
def _get_presigned_url_path(file_path, team_slug):
    ...
    return "morph-storage://resources/fsLog/{}/{}/{}".format(team_slug, last_dir, file_name)
...
def ref(reference: str) -> RefResponse:
    ...
    canvas_cell_conf = next(
        (cell for cell in canvas_json_data["cells"] if cell["filename"] == os.path.basename(reference)),
        None,
    )
    ...
    return RefResponse(
        ...
        storage_path=canvas_cell_conf["storagePath"] if "storagePath" in canvas_cell_conf else None,
    )
    ...
    if args and isinstance(args[0], RefResponse):
        if args[0].cell_type == "sql":
            ...
            ref_file_dir = os.path.join(
                os.path.dirname(args[0].canvas_json_path), os.path.join("outputs", os.path.splitext(args[0].filename)[0])
            )
            file_list = os.listdir(ref_file_dir)
            if len(file_list) < 1:
                raise MorphApiError(f"Cell {args[0].cell_name} has no output file")
            storage_path = _get_presigned_url_path(os.path.join(ref_file_dir, file_list[0]), config_from_env["team_slug"])
            ref_dict = {
