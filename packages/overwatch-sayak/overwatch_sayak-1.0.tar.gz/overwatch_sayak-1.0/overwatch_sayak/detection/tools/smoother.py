...
class DetectionsSmoother:
    ...
    Example:
        ...
        video_info = ov.VideoInfo.from_video_path(video_path=<SOURCE_FILE_PATH>)
        frame_generator = ov.get_video_frames_generator(source_path=<SOURCE_FILE_PATH>)

        model = YOLO(<MODEL_PATH>)
        tracker = ov.ByteTrack(frame_rate=video_info.fps)
        smoother = ov.DetectionsSmoother()

        annotator = ov.BoundingBoxAnnotator()

        with ov.VideoSink(<TARGET_FILE_PATH>, video_info=video_info) as sink:
