...
use bs_gl_client::bitcoin::Network;
use bs_gl_client::credentials::{NodeIdProvider, RuneProvider};
use bs_gl_client::credentials::TlsConfigProvider;
use bs_gl_client::pb;
use bs_gl_client::scheduler;
use prost::Message;
    ...
    R: TlsConfigProvider + RuneProvider + NodeIdProvider + Clone,
{
    ...
    async fn register(
        &self,
        signer: &bs_gl_client::signer::Signer,
...
impl Scheduler {
    ...
    fn new(network: &str, creds: Credentials) -> PyResult<Scheduler> {
        ...
        let uri = bs_gl_client::utils::scheduler_uri();

        let inner = match creds.inner {
            crate::credentials::UnifiedCredentials::Nobody(_) => {
                let scheduler = exec(async move {
                    bs_gl_client::scheduler::Scheduler::with(network, creds.inner.clone(), uri)
                        .await
                })
