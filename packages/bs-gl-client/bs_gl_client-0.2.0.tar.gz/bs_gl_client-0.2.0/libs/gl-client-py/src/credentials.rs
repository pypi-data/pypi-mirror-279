...
use bs_gl_client::credentials::{self, NodeIdProvider, RuneProvider, TlsConfigProvider};
...
where
    ...
    R: TlsConfigProvider + RuneProvider + NodeIdProvider + Clone,
{
    fn tls_config(&self) -> bs_gl_client::tls::TlsConfig {
        match self {
...
impl Credentials {
    #[new]
    pub fn new() -> Self {
        let inner = UnifiedCredentials::Nobody(bs_gl_client::credentials::Nobody::default());
    ...
    pub fn nobody_with(cert: &[u8], key: &[u8]) -> Self {
        let inner = UnifiedCredentials::Nobody(bs_gl_client::credentials::Nobody::with(cert, key));
        log::debug!("Created NOBODY credentials");
        Self { inner }
    }
    ...
    pub fn from_path(path: &str) -> Self {
        let inner = UnifiedCredentials::Device(bs_gl_client::credentials::Device::from_path(path));
    ...
    pub fn from_bytes(data: &[u8]) -> Self {
        let inner = UnifiedCredentials::Device(bs_gl_client::credentials::Device::from_bytes(data));
        log::debug!("Created device credentials");
        Self { inner }
    }
    ...
    pub fn from_parts(cert: &[u8], key: &[u8], rune: &str) -> Self {
        let inner =
            UnifiedCredentials::Device(bs_gl_client::credentials::Device::with(cert, key, rune));
        Self { inner }
    }
