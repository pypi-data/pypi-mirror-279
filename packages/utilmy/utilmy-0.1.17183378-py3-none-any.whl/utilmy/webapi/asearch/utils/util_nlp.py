...
Doc::
    ...
    ### Single hash function
        ...
        Feb 9, 2024 — Usually in these hash function one sees xor being used to combine the results of multiple values. I myself usually also..
        ...
        In this paper, we study several critical issues which must be tackled before one can apply b-bit minwise hashing to the volumes of data often..

        5
        favicon-rcor.me
        rcor.me > papers > cgo22f3m.pdf
        F3M: Fast Focused Function Merging - Rodrigo Rocha
        by S Stirling · Cited by 3 — The MinHash fingerprint is a fixed-size integer vector containing multiple hashes computed across all instructions of the..
...
def simhash(features):
    ...
    # Create the simhash by comparing each element of v to 0
    simhash = 0
...
def test_simhashes():
    pairs = [
        ["How much Pepto Bismol should I give my dogs?", "How much Pepto Bismol should I give my dog?", " <=5 "],
        ["What is wildest thing you have ever done in your life?", "What is wildest thing you have ever done in life?", "<=5"],
        ["the cat sat on the mat", "the cat sat on a mat", "<=5"],
        ["How are you?", "This is a simhash test.", ">= 20"],
    ]

    for pair in pairs:
        h1 = scrapinghub_simhash(pair[0])
        h2 = scrapinghub_simhash(pair[1])

        distance = hamming_distance(h1, h2)
        print("hamming distance using scrapinghub is ", distance)
        assert eval(str(distance) + pair[2])

        h1 = int(floc_simhash(pair[0]), 16)
        h2 = int(floc_simhash(pair[1]), 16)

        distance = hamming_distance(h1, h2)
        print("hamming distance using floc is ", distance)
...
def hamming_distance(a: int, b: int, f = 64):
    """ Calculate the hamming distance between a and b
        ...
        Returns:
            int: count of the number of bits by which a & b differ
    """
    x = (a ^ b) & ((1 << f) - 1)
    ans = 0
    while x:
...
def floc_simhash(xstr: str):
    """
        ...
        Returns:
            Simhash as a string
    """
    return SimHash(n_bits=64).hash(xstr)

def scrapinghub_simhash(xstr: str):
    """
        Generates a fingerprint from a sequence of hashes of the input string
        ...
        Returns:
            Simhash as an int
    """
    return fingerprint(list(map(hash, xstr)))
