...
##########################################################################################
def report_create_textsample(dirin="ztmp/bench", dataset="ag_news", dirquery=None):
    ...
    query_df = query_df[["text_id", "question"]]
    ...
    os_makedirs(dirout)
    df_all = report_get_combined_metrics_df(dense_config, sparse_config, neo4j_config)
    assert len(query_df) == len(df_all)


    query_df = pd.concat([query_df, df_all], axis=1)

    # log(query_df.columns)

    with open(f"{dirout}/report.txt", "a") as fp:
        for i, row in enumerate(query_df.itertuples(), start=1):
            ...
            fp.write(f"Source:{source_str}\n")
            fp.write(f"=================\nDense --top5\n=================\n")
            dense_result_ids = row.topk_docids_dense.split(";")
            result_string = report_get_result_string_by_docids(dense_result_ids)
            fp.write(result_string)

            fp.write(f"\n=================\nSparse --top5\n=================\n")
            sparse_result_ids = row.topk_docids_sparse.split(";")
            result_string = report_get_result_string_by_docids(sparse_result_ids)
            fp.write(result_string)

            fp.write(f"\n=================\nNeo4j --top5\n=================\n")
            neo4j_result_ids = row.topk_docids_neo4j.split(";")
...
def report_get_combined_metrics_df(dense_config, sparse_config, neo4j_config):
    dense_metric_df  = pd_read_file(f"{dense_config.dirout2}/dfmetrics.csv")
    sparse_metric_df = pd_read_file(f"{sparse_config.dirout2}/dfmetrics.csv")
    neo4j_metric_df  = pd_read_file(f"{dense_config.dirout2}/dfmetrics.csv")

    dense_metric_df = dense_metric_df[["topk_docids"]]
    dense_metric_df.rename(columns={"topk_docids": "topk_docids_dense"}, inplace=True)
    
    sparse_metric_df = sparse_metric_df[["topk_docids"]]
    sparse_metric_df.rename(columns={"topk_docids": "topk_docids_sparse"}, inplace=True)
    
    neo4j_metric_df = neo4j_metric_df[["topk_docids"]]
    neo4j_metric_df.rename(columns={"topk_docids": "topk_docids_neo4j"}, inplace=True)

    assert len(dense_metric_df) == len(sparse_metric_df) == len(neo4j_metric_df)
    # join dfs based on id
    combined_df = pd.concat([dense_metric_df, sparse_metric_df, neo4j_metric_df],
                            axis=1)
    return combined_df
