# -*- coding: UTF-8 -*-
"""
:filename: whintpy.deposit.filesfilters.py
...
from ..deposit.document_utils import DocumentUtils
...
class DocumentsFilters(BaseFilters):
    ...
    @staticmethod
    def merge_data(filtered_sets: list, match_all=False) -> FilteredSet:
        ...
        # Apply '&' or '|' on the filtered sets
        files_set = filtered_sets[0]
        if match_all is True:
            # Apply "&" operator between the filtered sets ("and" does not work)
            for i in range(1, len(filtered_sets)):
                files_set = files_set & filtered_sets[i]
                if len(files_set) == 0:
                    return FilteredSet()
        else:
            # Apply "|" operator between the filtered sets ("or" does not work)
            for i in range(1, len(filtered_sets)):
                files_set = files_set | filtered_sets[i]
