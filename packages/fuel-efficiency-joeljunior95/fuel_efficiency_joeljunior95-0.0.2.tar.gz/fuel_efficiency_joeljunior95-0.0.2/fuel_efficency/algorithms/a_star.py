...
class AStarStrategy(PathfindingStrategy):
    ...
    def find_path(grid:List[List[Node]], start:Node, end:Node) -> List[List[Node]]:
        ...
        while len(open_set) > 0:
            ...
            if current == end:
                path = AStarStrategy.reconstruct_path(came_from, current)
                break

            for neighbor in AStarStrategy.get_neighbors(grid, current):
                g = g_score[current] + neighbor.weight

                if neighbor not in g_score or g < g_score[neighbor]:
                    came_from[neighbor] = current
                    g_score[neighbor] = g
                    f = g + AStarStrategy.calculate_distance(current, neighbor)
                    heapq.heappush(open_set, (f, neighbor))

        return path

    def get_neighbors(grid:List[List[Node]], node:Node) -> List[Node]:
        neighbors: List[Node] = PathfindingStrategy.get_neighbors(grid, node, AStarStrategy.allowed_directions)
        # x_len = len(grid)
        # y_len = len(grid[0])
