...
class PolibaseDBApi:
    ...
    def create_dump(file_name: str | None = None, test: bool | None = None) -> None:
        ...
        polibase_folder_path: str = A.PTH.for_windows(
            A.PTH.join(A.CT_H.POLIBASE1.NAME, local_polibase_database_dump_folder_name)
        )

        if test:
            file_name = TEST_NAME
        else:
            file_name = "16-06-2024-22-40-00"
            if n(file_name):
        ...
        nas_folder_path: str = A.PTH.for_windows(
            A.PTH.join(A.CT_H.NAS.IP, "backups", nas_polibase_database_dump_folder_relative_path)
        )
        ...
        polibase2_copy_command: tuple[str, ...] =  (
                A.CT_RBK.NAME,
                polibase_folder_path,
        ...
        A.L.debug_bot(js((polibase2_copy_command)))
        A.L.debug_bot(A.EXC.execute(A.EXC.create_command_for_psexec(polibase2_copy_command, A.CT_H.WS255.NAME, "Administrator", "Fortun@90", True, False, True), True).returncode)
