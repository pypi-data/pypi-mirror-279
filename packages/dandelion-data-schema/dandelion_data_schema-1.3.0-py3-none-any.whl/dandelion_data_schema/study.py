...
class ImagingStudy(Study):
    ...
    def validate_dicom(cls, v):
        ...
        for item in v:
            if not isinstance(item, FileDataset):
                raise ValueError('dicom must be a list of pydicom.dataset.FileDataset instances')
        return v
    ...
    @classmethod
    def convert_dicom_from_bytes_to_filedataset(cls, b64_encoded: List[str]) -> List[FileDataset]:
        """
