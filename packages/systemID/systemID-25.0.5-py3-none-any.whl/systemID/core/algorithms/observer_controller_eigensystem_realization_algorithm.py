...
def observer_controller_eigensystem_realization_algorithm(markov_controller_parameters: list,
                                                          state_dimension: int,
                                                          p: int = None,
                                                          q: int = None
):
    ...
    max_size = int(numpy.floor((len(markov_controller_parameters) - 1) / 2))
    p = min(p, max_size)
    q = min(q, max_size)

    # Dimensions
    (output_dimension, input_dimension) = markov_controller_parameters[0].shape

    # Hankel matrices H(0) and H(1)
    H0 = numpy.zeros([p * (output_dimension + input_dimension), q * (input_dimension + output_dimension)])
    H1 = numpy.zeros([p * (output_dimension + input_dimension), q * (input_dimension + output_dimension)])
    for i in range(p):
        for j in range(q):
            H0[i * (output_dimension + input_dimension):(i + 1) * (output_dimension + input_dimension), j * (input_dimension + output_dimension):(j + 1) * (input_dimension + output_dimension)] = markov_controller_parameters[i + j + 1]
            H1[i * (output_dimension + input_dimension):(i + 1) * (output_dimension + input_dimension), j * (input_dimension + output_dimension):(j + 1) * (input_dimension + output_dimension)] = markov_controller_parameters[i + j + 2]
    ...
    #     Op = np.matmul(Rn, scipy.linalg.sqrtm(Sigman))
    #     Rq = np.matmul(scipy.linalg.sqrtm(Sigman), Snt)
    #     A_id = np.matmul(scipy.linalg.pinv(Op), np.matmul(H1, scipy.linalg.pinv(Rq)))
    ...
    B_id = Rq[:, 0:input_dimension]
    G_observer_id = Rq[:, input_dimension:input_dimension + output_dimension]
    C_id = Op[0:output_dimension, :]
    G_feedback_id = Op[output_dimension:output_dimension + input_dimension, :]
    D_id = markov_controller_parameters[0]
    ...
    def G_feedback(tk):
        return G_feedback_id

    def G_observer(tk):
        return G_observer_id
