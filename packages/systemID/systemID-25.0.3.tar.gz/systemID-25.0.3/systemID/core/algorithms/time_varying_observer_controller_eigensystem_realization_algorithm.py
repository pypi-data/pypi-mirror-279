...
def time_varying_observer_controller_eigensystem_realization_algorithm(hki: numpy.ndarray,
                                                                       D: numpy.ndarray,
                                                                       state_dimension: int,
                                                                       dt: float,
    ...
    D_id = D
    G_feedback_id = numpy.zeros([input_dimension, state_dimension, number_steps])

    # Store observability matrices at each step (for eigenvalue check)
    Ok = numpy.zeros([p * (input_dimension + output_dimension), state_dimension, number_steps])
    Ok1 = numpy.zeros([p * (input_dimension + output_dimension), state_dimension, number_steps])
    ...
    # First Hpq1
    Hpq1 = numpy.zeros([(p + 1) * (input_dimension + output_dimension), q * (input_dimension)])
    for i in range(p + 1):
        for j in range(q):
            Hpq1[i * (input_dimension + output_dimension):(i + 1) * (input_dimension + output_dimension), j * (input_dimension):(j + 1) * (input_dimension)] = hki[(q + i - 1) * (input_dimension + output_dimension):(q + i) * (input_dimension + output_dimension), (q - 1 - j) * (input_dimension + output_dimension):(q - j) * (input_dimension + output_dimension)][:, 0:input_dimension]
    ...
    for k in range(q, number_steps - p - 1):
        ...
        Hpq2 = numpy.zeros([(p + 1) * (input_dimension + output_dimension), q * (input_dimension)])
        for i in range(p + 1):
            for j in range(q):
                Hpq2[i * (input_dimension + output_dimension):(i + 1) * (input_dimension + output_dimension), j * (input_dimension):(j + 1) * (input_dimension)] = hki[(k + 1 + i - 1) * (input_dimension + output_dimension):(k + 1 + i) * (input_dimension + output_dimension), (k + 1 - 1 - j) * (input_dimension + output_dimension):(k + 1 - j) * (input_dimension + output_dimension)][:, 0:input_dimension]
        ...
        # if k == q+1:
        #     print('O1', O1)
        #     print('R1', numpy.matmul(scipy.linalg.sqrtm(Sigman1), Snt1))
        #     print('Hpq1', Hpq1)
        #     print('O1R1', O1 @ numpy.matmul(scipy.linalg.sqrtm(Sigman1), Snt1))

        # Deleting last block-line of observability matrices to conform with sizes
        O11 = O1[0:p * (input_dimension + output_dimension), :]
        O22 = O2[0:p * (input_dimension + output_dimension), :]
        ...
        # Identified matrices
        A_id[:, :, k] = numpy.matmul(scipy.linalg.pinv(O22), O1[(input_dimension + output_dimension):, :])
        B_id[:, :, k] = R1[:, 0:input_dimension]
        C_id[:, :, k] = O1[0:output_dimension, :]
        G_feedback_id[:, :, k] = O1[output_dimension:(input_dimension + output_dimension), :]
    ...
    def G_feedback(tk):
        return G_feedback_id[:, :, int(round(tk / dt))]
