...
def controller_eigensystem_realization_algorithm(markov_controller_parameters: list,
                                                  state_dimension: int,
                                                  p: int = None,
                                                  q: int = None
):
    ...
    H0 = numpy.zeros([p * (output_dimension + input_dimension), q * (input_dimension)])
    H1 = numpy.zeros([p * (output_dimension + input_dimension), q * (input_dimension)])
    for i in range(p):
        for j in range(q):
            H0[i * (output_dimension + input_dimension):(i + 1) * (output_dimension + input_dimension), j * (input_dimension):(j + 1) * (input_dimension)] = markov_controller_parameters[i + j + 1][:, 0:input_dimension]
            H1[i * (output_dimension + input_dimension):(i + 1) * (output_dimension + input_dimension), j * (input_dimension):(j + 1) * (input_dimension)] = markov_controller_parameters[i + j + 2][:, 0:input_dimension]
