...
def vxffrdim(fd,vxst):
    ...
    while True:
      ...
      if key == "VX_BBX":
          ...
          #print (xs, ys)
      if key == "VX_PBYTE" or key == "VX_PSHORT" or \
         key == "VX_PIDX" or key == "VX_PFLOAT" or \
         key == "VX_PDOUBLE" or key == "VX_PCHAR" or \
         key == "VX_PINT" :
             numel = len(data)
             pchan = numel // ( xs * ys)
      ...
      if key == "VX_CMAP":
                  vxst.m = True
                  vxst.map = np.asarray(data, dtype=nptype)
                  vxst.map = vxst.map.reshape((vxst.map.shape[0]//3,3))
...
def vxfrdaim(fd, vxst):
    ...
    vxst.m = False
    while True:
        status, ixs, iys, ichan, inptype, imdata = vxffrdim(fd, vxst)
    ...
    if chan != 1:
           ...
           ishape += (ys, xs)
    if vxst.m: ### color mapped image
           img = np.asarray(imt, dtype=nptype).reshape(ishape)
           ishape += (3,)
           imgc = np.zeros(ishape, dtype='uint8')
           sh = len(ishape)
           if sh == 3:
              imgc[:,:,0] = vxst.map[img,0]
              imgc[:,:,1] = vxst.map[img,1]
              imgc[:,:,2] = vxst.map[img,2]
           else:
              imgc[:,:,:,0] = vxst.map[img,0]
              imgc[:,:,:,1] = vxst.map[img,1]
              imgc[:,:,:,2] = vxst.map[img,2]
...
def vxfreadim ( fname ):
    ...
    if magic != b'#vIsX\n' :
        fd.close()
        print ("Not VisionX File")
        return(vxst)
    while True: 
      (key, data, nptype) = vxfreadelem(fd)
      #print (key)
      if key == "EOF":
         print ("Empty VisionX File")
         fd.close()
         return (vxst)
      if key == "VX_FCMND":
           vxst.h = data
...
def vximp (name):
    ...
    atype = type(name)
    if atype == str:
      if name.endswith('.png') or name.endswith('.jpg') or name.endswith('.gif'):
        grey, img = read_image_ck(name)
        ...
        if key == "VX_FHIST":
           if len(head) >= 2:
               if head[-1] != '\n' and head[-2] != '\n' :
                  ...
                  head += data
        elif key in ['VX_FMACH', 'VX_FUID', 'VX_FTIME',]:
           continue
        else: 
