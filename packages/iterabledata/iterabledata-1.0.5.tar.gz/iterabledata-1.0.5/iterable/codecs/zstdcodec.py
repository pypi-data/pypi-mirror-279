...
class ZSTDCodec(BaseCodec):
    def __init__(self, filename:str, compression_level:int = 0, mode:str = 'rb', open_it:bool = False):
        self.compression_level = compression_level
        rmode = 'rb' if mode in ['r', 'rb'] else 'wb'
        super(ZSTDCodec, self).__init__(filename, mode=rmode , open_it=open_it)
