...
def make_df(data: list[dict]):
    ...
    pdf_data = pdz.testing.make_df(data)
    if "time" in pdf_data.columns:
        return spark.createDataFrame(pdz.testing.make_df(data)).withColumn(
            "time", date_format("time", "yyyy-MM-dd HH:mm:ss")
        )
    else:
