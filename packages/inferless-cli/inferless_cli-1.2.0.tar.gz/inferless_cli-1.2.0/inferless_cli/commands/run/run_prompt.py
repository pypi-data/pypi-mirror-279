...
def run_prompt(runtime: str, runtime_type: str, name: str, env_dict: dict):
    try:
        ...
        with Progress(
            ...
            is_docker_running()
            yaml_data = get_inferless_config(name, env_dict)
            ...
            configure_run_model(
                config, progress, task_id, model_name, runtime_file_path, volume_path
            )
...
def configure_run_model(
    ...
    print(inputs)
    build_docker_image(runtime_dockerfile)
    progress.update(
        task_id,
    ...
    if config.get_value("source_framework_type") == "PYTORCH":
        ...
        start_docker_container(volume_path=volume_path, autostart=True, env=env_vars)
