...
class RSBase(osvg.SCRectangle, RotatedGroupShape, SCRectangleBasedShape):
    ...
    def _plain_etree_element(self) -> ET.Element:
        ...
        inner_side = min(self.width.value, self.height.value) - self.border_width * 5
        ...
        if self.width.value > self.height.value:
            arrow_stroke_width = self.height.value / 12
            arrow_length = arrow_stroke_width * 3
            arrow_width = arrow_stroke_width * 2
            right_x_shift1 = inner_side / 2 + self.border_width * 2
            line_length = self.width.value / 2 - right_x_shift1 - self.border_width * 4
            left_x_shift1 = -right_x_shift1
            upper_y_shift = -self.height.value / 6
            lower_y_shift = self.height.value / 6
            # upper_left_arrow
            upper_left_start = osvg.ShiftedPosition(
                origin=self.position,
                x_shift=left_x_shift1,
                y_shift=upper_y_shift,
            )
            upper_left_arrow = osvg.Line(
                parent=g,
                start=upper_left_start,
                end=osvg.ShiftedPosition(
            ...
            # lower_left_arrow
            lower_left_start = osvg.ShiftedPosition(
                origin=self.position,
                x_shift=left_x_shift1,
                y_shift=lower_y_shift,
            )
            ...
            # upper_right_arrow
            upper_right_start = osvg.ShiftedPosition(
                origin=self.position,
                x_shift=right_x_shift1,
                y_shift=upper_y_shift,
            )
            ...
            # lower_right_arrow
            lower_right_start = osvg.ShiftedPosition(
                origin=self.position,
                x_shift=right_x_shift1,
                y_shift=lower_y_shift,
            )
        ...
        elif self.width.value < self.height.value:
            arrow_stroke_width = self.width.value / 12
            arrow_length = arrow_stroke_width * 3
            arrow_width = arrow_stroke_width * 2
            lower_y_shift1 = inner_side / 2 + self.border_width * 2
            line_length = self.height.value / 2 - lower_y_shift1 - self.border_width * 4
            upper_y_shift1 = -lower_y_shift1
            right_x_shift = self.width.value / 6
            left_x_shift = -right_x_shift
            # upper_left_arrow
            upper_left_start = osvg.ShiftedPosition(
                origin=self.position,
                x_shift=left_x_shift,
                y_shift=upper_y_shift1,
            )
            upper_left_arrow = osvg.Line(
                parent=g,
                start=upper_left_start,
            ...
            # upper_rigth_arrow
            upper_rigth_start = osvg.ShiftedPosition(
                origin=self.position,
                x_shift=right_x_shift,
                y_shift=upper_y_shift1,
            )
            upper_right_arrow = osvg.Line(
                parent=g,
