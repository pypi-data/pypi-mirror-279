...
class Routes(Enum):
    ...
    BAG_GRIPPER_CONFIGURATION = "/v1/python/configuration/pneumatic_bag_gripper"
    ...
    SET_SPEED = "/v1/python/set_speed"
    GET_ACCELERATION = "/v1/python/get_acceleration"
    SET_ACCELREATION = "/v1/python/set_acceleration"
    ...
    GET_BRAKE_STATE = "/v1/python/brakes"
    LOCK_BRAKES = "/v1/python/brakes/lock"
    UNLOCK_BRAKES = "/v1/python/brakes/unlock"
...
class Api:
    ...
    def _make_route(self, route: Routes) -> str:
        return self._conn_str + str(route.value)
    ...
    def get_machine_configuration(self) -> MachineConfiguration:
        ...
        if not response.ok:
            raise ApiException(
                f"Failed to fetch the machine configuration: {str(response)}"
            )
    ...
    def get_input_details(self, input_uuid: str) -> input_details.DigitalInputDetails:
        ...
        if not response.ok:
            raise ApiException(
                f"Unable to get details for input with uuid: {input_uuid}"
            )
    ...
    def get_output_details(self, output_uuid: str) -> output_details.OutputDetails:
        ...
        if not response.ok:
            raise ApiException(
                f"Unable to get details for output with uuid: {output_uuid}"
            )
        return cast(output_details.OutputDetails, response.json())

    def get_pneumatic_details(
        self, pneumatic_uuid: str
    ) -> pneumatic_details.PneumaticDetails:
        ...
        if not response.ok:
            raise ApiException(
                f"Unable to get details for pneumatic with uuid: {pneumatic_uuid}"
            )
        return cast(pneumatic_details.PneumaticDetails, response.json())

    def get_bag_gripper_details(
        self, pneumatic_uuid: str
    ) -> bag_gripper_details.BagGripperDetails:
        """Retrieve the details about a bag gripper.

        Args:
            pneumatic_uuid (str): ID of the bag gripper

        Returns:
            BagGripperDetails: Details of the bag gripper
        Raises:
            VentionException: When we fail to query the details
        """
        params = {"pneumaticUuid": pneumatic_uuid}
        response = requests.get(
            self._make_route(Routes.BAG_GRIPPER_CONFIGURATION),
            params=params,
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )
        if not response.ok:
        ...
        return cast(bag_gripper_details.BagGripperDetails, response.json())

    def get_ac_motor_details(
        self, pneumatic_uuid: str
    ) -> ac_motor_details.ACMotorDetails:
        ...
        if not response.ok:
            raise ApiException(
                f"Unable to get details for ac_motor with uuid: {pneumatic_uuid}"
            )
    ...
    def set_speed(self, axis_uuid: str, new_speed: float) -> bool:
        """Set the max speed of an axis

        Args:
            axis_uuid (str): ID of the axis
            new_speed (float): New max speed

        Returns:
            bool: True if it was successful
        """
        data = {"axisUuid": axis_uuid, "newSpeed": new_speed}
        response = requests.post(
            self._make_route(Routes.SET_SPEED),
            json=data,
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )
        if not response.ok:
    ...
    def set_acceleration(self, axis_uuid: str, new_acceleration: float) -> bool:
        """Set the acceleration of the axis

        Args:
            axis_uuid (str): ID of the axis
            new_acceleration (float): New acceleration

        Returns:
            bool: True if the acceleration was set, otherwise False
        """
        data = {"axisUuid": axis_uuid, "newAcceleration": new_acceleration}
        response = requests.post(
            self._make_route(Routes.SET_ACCELREATION),
            json=data,
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )
        if not response.ok:
    ...
    def home(
        ...
        response = requests.post(
            self._make_route(Routes.HOME), json=data, timeout=timeout
        )
        if not response.ok:
            warnings.warn(response.text)
        return response.ok
    ...
    def stop_continuous_move(
        self, axis_uuid: str, acceleration: Optional[float] = 100
    ) -> bool:
    ...
    def read_input(self, input_uuid: str) -> int:
        ...
        if "value" not in response_dict:
            raise ApiException(
                f"The 'value' key was not found in the response while reading input with uuid: {input_uuid}"
            )
    ...
    def get_brake_state(self, axis_uuid: str) -> BrakeState:
        """Retrieve the brake state of the axis.

        Args:
            axis_uuid (str): UUID of the axis

        Returns:
            BrakeState: State of the brake
        """
        params = {"axisUuid": axis_uuid}
        response = requests.get(
            self._make_route(Routes.GET_BRAKE_STATE),
            params=params,
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )

        response_dict = response.json()
        state = response_dict["status"]

        if state not in [BrakeState.LOCKED, BrakeState.UNLOCKED, BrakeState.UNKNOWN]:
            raise ApiException(f"Unknown brake state: {state}")

        return BrakeState(state)

    def lock_brakes(self, axis_uuid: str) -> bool:
        """Lock the brakes of the axis with the provided uuid.

        Args:
            axis_uuid (str): UUID of the axis

        Returns:
            bool: True if the brakes were locked, otherwise False.
        """
        data = {"axisUuid": axis_uuid}
        response = requests.post(
            self._make_route(Routes.LOCK_BRAKES),
            json=data,
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )
        if not response.ok:
    ...
    def unlock_brakes(self, axis_uuid: str) -> bool:
        """Unlock the brakes of the axis with the provided uuid.

        Args:
            axis_uuid (str): UUID of the axis

        Returns:
            bool: True if the brakes were unlocked, otherwise False.
        """
        data = {"axisUuid": axis_uuid}
        response = requests.post(
            self._make_route(Routes.UNLOCK_BRAKES),
            json=data,
            timeout=DEFAULT_TIMEOUT_SECONDS,
        )
        if not response.ok:
    ...
    def wait_for_motion_completion(
        ...
        """
        data = {
            "axisUuidList": [axis_uuid] if isinstance(axis_uuid, str) else axis_uuid
        }
        response = requests.get(
    ...
    def pneumatic_push(
        self, pneumatic_uuid: str, wait_for_motion_completion: bool
    ) -> bool:
        """
    ...
    def set_axes_positions(
        self, axis_uuid_and_position_list: list[Tuple[str, float]]
    ) -> bool:
        """
    ...
    def stop_all_motion(self) -> bool:
        ...
        """
        response = requests.post(
            self._make_route(Routes.STOP_ALL_MOTION), timeout=DEFAULT_TIMEOUT_SECONDS
        )
        if not response.ok:
            warnings.warn(response.text)
        return response.ok
    ...
    def move_ac_motor(
        self, pneumatic_uuid: str, direction: Literal["forward", "reverse"]
    ) -> bool:
        """
    ...
    def open_bag_gripper(self, pneumatic_uuid: str) -> bool:
        """
        Opens the bag gripper motor with the provided uuid.

        Args:
            pneumatic_uuid (str): ID of the pneumatic of the actuator

        Returns:
            bool: True if the open was successful, otherwise False.
        """
        data = {
            "pneumaticUuid": pneumatic_uuid,
            "waitOnMotionCompletion": False,
        }
        response = requests.post(
            self._make_route(Routes.OPEN_BAG_GRIPPER),
            json=data,
            timeout=MOVEMENT_TIMEOUT_SECONDS,
        )
        if not response.ok:
    ...
    def close_bag_gripper(self, pneumatic_uuid: str) -> bool:
        """
        Closes the bag gripper with the provided uuid.

        Args:
            pneumatic_uuid (str): ID of the pneumatic of the actuator
        Returns:
            bool: True if the close was successful, otherwise False.
        """
        data = {
            "pneumaticUuid": pneumatic_uuid,
            "waitOnMotionCompletion": False,
        ...
        response = requests.post(
            self._make_route(Routes.CLOSE_BAG_GRIPPER),
            json=data,
            timeout=MOVEMENT_TIMEOUT_SECONDS,
        )
        if not response.ok:
    ...
    def set_tcp_offset(self, robot_uuid: str, tcp_offset: CartesianPose) -> bool:
        """
    ...
    def _camera_action(
        ...
        try:
            response = requests.get(
                self._make_route(route), timeout=DEFAULT_TIMEOUT_SECONDS, params=params
            )
            ...
            raise ApiException(
                f"Error during camera action: No completion value received from the server. Got: {response.text}"
            ) from err
        except ApiException as err:
            raise err
        except Exception as err:
            raise ApiException(
                f"An error occurred during camera action: {err}"
            ) from err
    ...
    def camera_calibration_save(self) -> str:
        ...
        """
        data: Union[Dict[str, Any], str] = self._camera_action(
            Routes.VISION_CAMERA_CALIBRATION_SAVE
        )
    ...
    def camera_calibration_pose(
        self, actual_tcp_pose: CartesianPose
    ) -> Union[Dict[str, Any], str]:
        """
    ...
    def camera_calibration_reset(
        self, new_calibration_file: Optional[str] = None
    ) -> Union[Dict[str, Any], str]:
        """
    ...
    def camera_target_get_position(
        self, actual_tcp_pose: Optional[CartesianPose] = None
    ) -> Union[Dict[str, Any], str]:
        """
