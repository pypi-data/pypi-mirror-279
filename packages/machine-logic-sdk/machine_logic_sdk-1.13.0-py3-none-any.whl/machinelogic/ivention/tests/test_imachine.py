...
class DummyActuator(IActuator):
    ...
    def move_continuous_async(
        self, speed: float = 100.0, acceleration: float = 100.0
    ) -> None:
        raise NotImplementedError
    ...
    def move_relative(self, distance: float, timeout: float = 0) -> None:
        raise NotImplementedError
...
class DummyRobot(IRobot):
    ...
    def compute_forward_kinematics(
        self, joint_angles: JointAnglesDegrees
    ) -> CartesianPose:
        raise NotImplementedError
    ...
    def get_cartesian_position(self) -> CartesianPose:
        raise NotImplementedError

    def on_system_state_change(
        self, callback: Callable[[RobotOperationalState, RobotSafetyState], None]
    ) -> int:
        raise NotImplementedError
...
class TestIMachine(unittest.TestCase):
    def test_given_machine_motion_without_actuators_when_get_actuator_then_raises_machine_exception(
        ...
        # Arrange
        imachine = IMachine(
            machine_motions=[], mqtt_client=MagicMock(), estop=MagicMock()
        )
    ...
    def test_given_multiple_machine_motions_and_multiple_actuators_when_get_unknown_actuator_then_raises_machine_exception(
        ...
        estop = MagicMock()
        imachine = IMachine(machine_motions, mqtt_client, estop)
    ...
    def test_given_multiple_machine_motions_and_multiple_actuators_when_get_actuator_then_finds_actuator(
        ...
        for expected_actuator in actuators:
            with self.subTest(actuator_name=expected_actuator.configuration.name):
                ...
                # Act
                actual_actuator = imachine.get_actuator(
                    expected_actuator.configuration.name
                )
    ...
    def test_given_machine_motion_and_robot_when_get_robot_finds_robot(self):
        ...
        machine_motions: list[IMachineMotion] = [
            DummyMachineMotion(robots=[DummyRobot(robot_1_configuration)])
        ]

        imachine = IMachine(
            machine_motions=machine_motions, mqtt_client=MagicMock(), estop=MagicMock()
        )
