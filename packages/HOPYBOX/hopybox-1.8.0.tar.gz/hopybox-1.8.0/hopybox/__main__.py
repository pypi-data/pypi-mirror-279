      ...
      if len(_process(command)) == 1:
        ...
        if _process(command)[1][0] == '-':
          if len(_process(command)) == 3:
            command_data['Global'][_command]['run'] = _process(command)[2]
          exec(command_data['Global'][_command]['code'][_process(command)[1]])
        else:
          if len(_process(command)) < 3:
            command_data['Global'][_command]['run'] = _process(command)[1]
          else:
            command_data['Global'][_command]['run'] = _process(command,blank=1)[1]
          exec(command_data['Global'][_command]['code'])
...
# start
def start():
  global _command, _store, _windows, _command_list
  mouse_support = False
  times = 0
  # start text
