...
class DocumentConsumer(Consumer):
    ...
    async def consume(self, elements):
        return await Mesh.context_safe(self.do_consume(elements))

    async def do_consume(self, elements):
        Mesh.context().set_attribute(DISCARD, True)
        if len(elements) > BUFFER_SIZE - 1:
            try:
                await self.data_house.writes(elements)
        ...
        if not LIMITER.acquire(True):
            return
        try:
            await self.data_house.write(elements)
        except BaseException as e:
            self.exception(e, elements)
        finally:
            LIMITER.release(1)
