...
class Event:
    ...
    async def new_instance(payload: Any, topic: Topic) -> "Event":
        ...
        environ = await ServiceLoader.load(Network).get_default().get_environ()
        target = Principal()
        target.node_id = environ.node_id
        target.inst_id = environ.inst_id
        return await Event.new_instance_with_target(payload, topic, target)
    ...
    async def new_instance_with_target(payload: Any, topic: Topic, target: Principal) -> "Event":
        ...
        environ = await ServiceLoader.load(Network).get_default().get_environ()
        source = Principal()
        source.node_id = environ.node_id
        source.inst_id = environ.inst_id
