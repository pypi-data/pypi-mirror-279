...
class Cache(ABC):
    ...
    async def put_with_duration(self, key: str, value: Any, duration: int):
        """
    ...
    async def compute_if_absent(self, key: str, kind: Type[T], duration: int, fn):
        """
        ...
        value = fn(key)
        if value is not None:
            await self.put_with_duration(key, value, duration)
        return value
