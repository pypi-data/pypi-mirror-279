...
class TestServiceLoad(unittest.TestCase):
    ...
    async def test_transport_split_async(self):
        ...
        session = await transport.with_address("127.0.0.1:7304").local().open('session_id_001', {
        ...
        session2 = await transport.with_address("127.0.0.1:7304").local().open('session_id_001', {
            Metadata.MESH_VERSION.key(): '',
            Metadata.MESH_TECH_PROVIDER_CODE.key(): 'LX',
            Metadata.MESH_TRACE_ID.key(): Mesh.context().get_trace_id(),
            Metadata.MESH_TOKEN.key(): 'x',
