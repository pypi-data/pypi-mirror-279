...
from mesh.prsim import RuntimeHook, Subscriber, Registry, Scheduler, Builtin
...
class MeshEden(Eden, RuntimeHook, Subscriber):
    """
    Like: create.tenant.omega.json.http2.lx000001.mpi.ducesoft.net
    """
    ...
    def refer(self, metadata: mpi, reference: Type[T], method: Inspector) -> Execution[Reference]:
        return self.make_reference_exec(metadata, reference).get(method.get_signature())
    ...
    def infer(self, urn: str) -> Execution[Service]:
        return self.make_service_exec().get(URN.parse(urn).name)
    ...
    def make_reference_exec(self, md: mpi, ref: Type[T]) -> Dict[Any, Instance[Reference]]:
        ...
        if not self.indies.get(ref).get(md):
            ...
            for kind, methods in tool.get_declared_methods(ref).items():
                for method in methods:
                    refer = self.make_method_as_reference(md, kind, method)
    ...
    def make_service_exec(self) -> Dict[str, Instance[Service]]:
        for provider in self.providers.values():
            if provider.making:
                continue
            for kind, methods in tool.get_declared_methods(provider.kind).items():
                for method in methods:
                    ...
                    schemas: List[Service] = self.make_schema(provider.kind, kind, method)
    ...
    def make_schema(self, interface: T, kind: T, method: Any) -> List[Service]:
        schemas = []
        macro = MPS.get_mps_if_present(interface)
        if macro:
            schemas.append(self.make_mps_as_service(macro, kind, method))
        bindings = Binding.get_binding_if_present(interface)
        for metadata in bindings:
            if metadata.meshable:
                schemas = schemas.append(self.make_binding_as_service(metadata, kind, method))
        return schemas

    def make_method_as_reference(self, metadata: MPI, interface: T, method: Any) -> Reference:
        ...
        reference.timeout = self.choose_if_present(lambda x: x.timeout, 10000, metadata, om, ok)
        reference.retries = self.choose_if_present(lambda x: x.retries, 3, metadata, om, ok)
        ...
        reference.urn = self.get_urn(alias, self.get_reference_urn_flag(reference))
        return reference

    def make_mps_as_service(self, metadata: MPS, kind: T, method: Any) -> Service:
        om = MPI.get_mpi_if_present(method)
        ...
        service.timeout = self.choose_if_present(lambda x: x.timeout, metadata.timeout, metadata, om, ok)
        service.urn = self.get_urn(alias, self.get_service_urn_flag(service))
        return service

    def make_binding_as_service(self, metadata: Binding, kind: T, method: Any) -> Service:
        service = self.init_service()
        ...
        service.timeout = self.choose_if_present(lambda x: x.timeout, metadata.timeout, metadata)
    ...
    def init_service(self) -> Service:
        ...
        service.retries = 0
        service.node = self.env.node_id
        service.inst = self.env.inst_id
        service.zone = self.env.lattice.zone
        service.cluster = self.env.lattice.cluster
        service.cell = self.env.lattice.cell
        service.group = self.env.lattice.group
    ...
    def get_urn(self, alias: str, flag: URNFlag) -> str:
        urn = URN()
        urn.domain = MESH_DOMAIN
        urn.node_id = str.lower(self.env.node_id)
        urn.name = alias
        urn.flag = flag
        return urn.string()
    ...
    async def start(self):
        ...
        self.task_id = await self.scheduler().period(30 * 1000, self.topic)

    async def stop(self):
        await self.scheduler().cancel(self.task_id)
        await self.scheduler().shutdown(3 * 1000)
        metadata = Resource()
        metadata.services = []
        metadata.references = []
        try:
            await self.registry().unregister(self.new_registration(metadata, {}))
        except BaseException as e:
    ...
    async def subscribe(self, event: Event):
        ...
        await self.registry().register(self.new_registration(metadata, attachments))
