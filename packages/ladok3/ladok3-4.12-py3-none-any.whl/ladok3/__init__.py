...
class LadokSession:
    ...
    def course_instances_JSON(self, course_code, lang="sv"):
        ...
        r = self.session.get(
            url=self.base_gui_proxy_url
            + "/resultat/kurstillfalle/filtrera?kurskod="
            + course_code
    ...
    def i18n_translation_JSON(self, lang="sv"):
        r = self.session.get(
            url=self.base_gui_proxy_url
            + "/kataloginformation/i18n/oversattningar/sprakkod/"
            + lang,
            headers=self.headers,
        ).json()
        return r
    ...
    def studieordning_JSON(self):
        r = self.session.get(
            url=self.base_gui_proxy_url + "/kataloginformation/grunddata/studieordning",
            headers=self.headers,
        ).json()
        return r
