        ...
        for name in FileProfile.profile_names():
            ...
            if name == "dicom" or name in config.keys():
                ...
                if name not in NESTED_PROFILE_NAMES:
                    self.file_profiles.append(FileProfile.factory(name, config=profile_config, log=self.log))
                elif name in NESTED_PROFILE_NAMES:
                    nested_profile = FileProfile.factory(name, config=config, log=self.log)
