...
try:  # conditional import
    ...
    log.info("Pydicom deid module not found. Dicom 'alter_pixels' action is not available.")
...
DICOM_TAG_REPEATER_TUPLE_RE = re.compile(r"\(\s*([56]0[Xx]{2})\s*,\s*([0-9a-fA-F]{4})\s*\)")
...
class DicomTagStr(str):
    ...
    def is_wild_sequence(self):
        if self._is_wild_sequence is None:
            if self.dicom_tag and self.is_sequence and "*" in list(map(str, self.dicom_tag)):
    ...
    def parse_field_name(self, name):
        ...
        for attr in dir(self):
            if attr.startswith(self.parsers_method_prefix) and callable(getattr(self, attr)):
...
class DicomDeIdFieldMixin(DeIdFieldMixin):
    ...
    def deidentify(self, profile, state, record):
        ...
        if len(fieldnames) == 0 or (len(fieldnames) == 1 and self.fieldname == fieldnames[0]):
    ...
    def _deidentify_fieldnames(self, profile, state, record, fieldnames):
        ...
        for fieldname in fieldnames:
            ...
            tmp_field = DeIdField.factory({"name": fieldname, **value}, mixin=self.__class__)
    ...
    def _list_fieldname_wild_sequence(self, record):
        ...
        fieldnames = [DicomTagStr(".".join(map(convert_tag_to_hex, tag))) for tag in dcm_tags]
        return fieldnames
    ...
    def _list_fieldname_recurse_sequence(self, record):
        ...
        if self.is_regex:
            raise ValueError(f"regex field {self.fieldname} not compatible with recurse-sequence")
        if self.fieldname.is_sequence:
            raise ValueError(f"Field {self.fieldname} not compatible with recurse-sequence")

        if self.fieldname.dicom_tag is None:  # keyword
            attrs = get_dicom_data_elements_keyword_path(record)
            fieldnames += [DicomTagStr(a) for a in attrs if a.endswith(f".{self.fieldname}")]
        else:
            if self.fieldname.is_private:
                group, private_creator, el = self.fieldname.dicom_tag
                attributes_path = self._get_private_tag_in_sequences(record, group, el, private_creator)
    ...
    def _jitter(self, profile, state, record, jitter_range, jitter_type, jitter_min, jitter_max):
        ...
        new_value = super(DicomDeIdFieldMixin, self)._jitter(profile, state, record, jitter_range, jitter_type, jitter_min, jitter_max)
    ...
    def _decrypt(self, profile, state, record):
        ...
        decrypted_value = profile.read_field(state, profile.decrypted_attributes, self.fieldname)
        if decrypted_value:
...
class DicomFileProfile(FileProfile):
    ...
    def __init__(self, file_filter=None):
        ...
        super(DicomFileProfile, self).__init__(packfile_type="dicom", file_filter=file_filter)
        ...
        self.lc_kw_dict = {
            keyword.lower(): keyword for keyword in pydicom.datadict.keyword_dict if keyword  # non-blank
        }
        ...
        with tempfile.NamedTemporaryFile(suffix=".txt") as cms:
            with tempfile.NamedTemporaryFile() as enc:
                ...
                if pr.returncode == 0:
                    ...
                    record.EncryptedAttributesSequence = pydicom.Sequence([pydicom.Dataset()])
                    record.EncryptedAttributesSequence[0].EncryptedContentTransferSyntaxUID = pydicom.uid.ExplicitVRLittleEndian
        ...
        with tempfile.NamedTemporaryFile(suffix=".txt") as cms:
            with tempfile.NamedTemporaryFile() as enc:
                ...
                cmd = [
                    ...
                    "cms",
                    "-encrypt",
                    *list(itertools.chain(*[["-recip", key] for key in self.public_key])),
                    "-binary",
                    "-outform",
                    "PEM",
                    "-in",
                ...
                if pr.returncode == 0:
                    ...
                    record.EncryptedAttributesSequence = pydicom.Sequence([pydicom.Dataset()])
                    record.EncryptedAttributesSequence[0].EncryptedContentTransferSyntaxUID = pydicom.uid.ExplicitVRLittleEndian
                    record.EncryptedAttributesSequence[0].EncryptedContent = encrypted_mas
    ...
    def load_config(self, config):
        self.patient_age_from_birthdate = config.get("patient-age-from-birthdate", self.patient_age_from_birthdate)
        self.patient_age_units = config.get("patient-age-units", self.patient_age_units)
        self.remove_private_tags = config.get("remove-private-tags", self.remove_private_tags)
        ...
        self.asymmetric_encryption = config.get("asymmetric-encryption", self.asymmetric_encryption)
    ...
    def get_data_element(self, record, fieldname):
        ...
        if isinstance(fieldname, DicomTagStr) and fieldname.dicom_tag:
            if fieldname.is_sequence:
                data_element = self._get_data_element_if_sequence(record, fieldname.dicom_tag)
            elif fieldname.is_private:
                try:
                    data_element = self._get_data_element_if_private(record, fieldname.dicom_tag)
                except KeyError:
                    data_element = None
            else:
    ...
    def _get_data_element_if_private(self, record, private_dicom_tag):
        ...
        """
        return record.get_private_item(private_dicom_tag[0], private_dicom_tag[2], private_dicom_tag[1])
    ...
    def _get_or_create_data_element_if_sequence(self, record, tag):
        ...
        if not len(tag) == 1:
            try:
                return self._get_or_create_data_element_if_sequence(record[tag[0]], tag[1:])
        ...
        with tempfile.NamedTemporaryFile() as dec:
            ...
            if pr.returncode == 0:
                with open(dec.name, 'rb') as fp:
                    envelope = pydicom.filereader.read_dataset(fp, is_implicit_VR=False, is_little_endian=True)
                return envelope
            else:
                raise Exception(
    ...
    def replace_field(self, state, record, fieldname, value):
        ...
        if isinstance(fieldname, DicomTagStr) and fieldname.dicom_tag:
            if fieldname.is_sequence:  # this is a sequence
                de = self._get_or_create_data_element_if_sequence(record, fieldname.dicom_tag)
                de.value = value
            elif fieldname.is_private:
                try:
                    de = self._get_data_element_if_private(record, fieldname.dicom_tag)
                    ...
                    try:
                        ...
                        vr = pydicom.datadict.private_dictionary_VR(tag, fieldname.dicom_tag[1])
                    except KeyError:  # not found
                        log.error(f"Invalid replace-with action. Unknown VR for tag {fieldname.dicom_tag}.")
                        sys.exit(1)
                    block = record.private_block(fieldname.dicom_tag[0], fieldname.dicom_tag[1], create=True)
    ...
    def _validate_hash(self, field, errors):
        ...
        if (
            ...
            errors.append(f"{field.fieldname} cannot be hashed - VR not compatible ({vr})")
    ...
    def validate(self, enhanced=False):
        ...
        if self.pixel_actions and not DEID_PACKAGE_FOUND:
            errors.append("Pixel actions requires pydicom deid package which was not found.")
        ...
        for field in self.fields:
            ...
            if self.recurse_sequence:
                if field.fieldname.dicom_tag and field.fieldname.is_sequence:
                    errors.append(f"recurse-sequence=True is incompatible with field SQ {field.fieldname}.")
                if field.is_regex:
                    errors.append(f"recurse-sequence=True is incompatible with regex field {field.fieldname}.")
