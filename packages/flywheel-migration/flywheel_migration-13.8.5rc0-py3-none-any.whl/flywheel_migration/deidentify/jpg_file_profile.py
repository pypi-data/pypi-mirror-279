...
class JPGRecord:
    ...
    def metadata(self):
        ...
        if self._metadata is None:
            ...
            self._metadata = piexif.load(self.image.info["exif"]) if "exif" in self.image.info else {}
...
class JPGFileProfile(FileProfile):
    ...
    def __init__(self, file_filter=None):
        ...
        super(JPGFileProfile, self).__init__(packfile_type="jpg", file_filter=file_filter)
    ...
    def read_field(self, state, record, fieldname):
        ...
        # decode value
        if fieldname in self.field_map:  # require for filenames manipulation without any field associated to it
            if self.field_map[fieldname].key in ["increment-datetime", "hashuid", "hash"] and value:
                try:
                    value = value.decode("utf8")
