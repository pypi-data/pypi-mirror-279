...
class NiftiRecord:
    ...
    def validate(self):
        ...
        if type(self.image) not in self.types:
            raise TypeError(f"Expected Nifti1Image or Nifti2Image, found {type(self.image)}.")
...
class NiftiFileProfile(FileProfile):
    ...
    allowed_fields = set(list(nib.Nifti1Header().keys()) + list(nib.Nifti2Header().keys()))

    def __init__(self, file_filter=None):
        file_filter = file_filter if file_filter else self.default_file_filter
        super(NiftiFileProfile, self).__init__(packfile_type=self.name, file_filter=file_filter)
    ...
    def replace_field(self, state, record, fieldname, value):
        ...
        if self.read_field(state, record, fieldname):
            try:
                ...
                log.error("Incorrect type: Cannot replace field %s with value %s", fieldname, value)
