...
from flywheel_migration.util import sanitize_filename, matches_byte_sig
...
class FileProfile:
    ...
    jitter_type = DEFAULT_JITTER_TYPE  # default type for jitter action ("int" or "float")
    replace_with_insert = True  # if True, insert data element with replace-with if it cans
    ...
    def __init__(self, packfile_type=None, file_filter=None):
        ...
        self.uid_prefix_fields = self.uid_default_prefix_fields  # How many prefix nodes to keep
        self.uid_suffix_fields = self.uid_default_suffix_fields  # How many suffix nodes to keep
    ...
    def add_field(self, field):
        ...
        if getattr(field, "_is_regex", None) and not self.regex_compatible:
            raise ValueError(f"regex in field is not compatible for profile {self.name}")
    ...
    def load_config(self, config):
        ...
        self.replace_with_insert = config.get("replace-with-insert", self.replace_with_insert)
        self.hash_salt = config.get("salt", None)
        self.uid_prefix_fields = config.get("uid-prefix-fields", self.uid_default_prefix_fields)
        self.uid_suffix_fields = config.get("uid-suffix-fields", self.uid_default_suffix_fields)
            ...
            for i, filename in enumerate(self.filenames):
                if "groups" in filename:
                    for grp in filename.get("groups"):
                        ...
                        self.add_field(DeIdField.factory(grp_tmp, dry=True, mixin=self.deidfield_mixin))
    ...
    def _add_keep_fields(self, filename):
        ...
        for out_var in out_vars:
            if out_var not in self.field_map and out_var not in group_names:
                ...
                self.add_field(DeIdField.factory(field_config, mixin=self.deidfield_mixin))
    ...
    def get_log_entry(self, path, entry_type, state, record):
        ...
        for fieldname in fieldnames:
            if fieldname in self.field_map:
                ...
                # - Nate
                if (hasattr(fieldname, "is_flat") and not fieldname.is_flat) or field.is_regex:
    ...
    def validate(self, enhanced=False):
        ...
        if self.datetime_format:
            errors += deid_validation.validate_datetime_format_code(self.datetime_format)
        ...
        if self.uid_numeric_name:
            errors += deid_validation.validate_uid_numeric_name(self.uid_numeric_name, self.uid_prefix_fields)
        ...
        for fieldname, field in self.field_map.items():
            if isinstance(field, DeIdIncrementDateTimeField):
                if field.datetime_format is not None:
                    errors += deid_validation.validate_datetime_format_code(field.datetime_format)

            if isinstance(field, DeIdIncrementDateField):
                if field.date_format is not None:
                    errors += deid_validation.validate_datetime_format_code(field.date_format)
            ...
            if isinstance(field, DeIdJitterField):
                if field.jitter_range is not None:
                    errors += deid_validation.validate_jitter(field.jitter_range, field.jitter_type)
            ...
            for grp in groups:
                if grp["name"] == k:
                    if "increment-date" in grp:
                        ...
                        datetime_format = grp.get("datetime-format", self.datetime_format)
    ...
    def __deepcopy__(self, memodict=None):
        return FileProfile.factory(name=self.name, config=self.to_config(), log=self.log)
