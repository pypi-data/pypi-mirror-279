...
class PNGRecord:
    ...
    def metadata(self):
        ...
        if self._metadata is None:
            ...
            self._metadata = list(reader.chunks())  # NB: The row image is a part of chunk (IDAT)
            reader.file.close()
...
class PNGFileProfile(FileProfile):
    ...
    file_signatures = [(0, b"\x89\x50\x4E\x47\x0D\x0A\x1A\x0A")]

    def __init__(self, file_filter=None):
        file_filter = file_filter if file_filter else self.default_file_filter
        super(PNGFileProfile, self).__init__(packfile_type="png", file_filter=file_filter)
    ...
    def load_record(self, state, src_fs, path):
        ...
        if self.remove_private_chunks:
            ...
            record._metadata = [chunk for chunk in record.metadata if chunk[0].decode() in self.public_chunks]
            modified = True
    ...
    def add_field(self, field):
        ...
        if field.fieldname.lower() in self.lc_kw_dict:
            field.fieldname = ChunkStr(self.lc_kw_dict[field.fieldname.lower()], is_pub=True)
    ...
    def remove_field(self, state, record, fieldname):
        ...
        if chunk_tag:
            # pylint: disable=protected-access
            record._metadata = [chunk for chunk in record.metadata if chunk[0] != fieldname.encode()]
        else:
    ...
    def replace_field(self, state, record, fieldname, value):
        chunk_tag = getattr(fieldname, "_is_chunk", None)
        if chunk_tag:
            raise NotImplementedError("replace-field action for this profile is not yet supported")
        ...
        for field in self.fields:
            if getattr(field.fieldname, "_is_pub", None):
                ...
                if lc_field not in self.lc_kw_dict:
                    errors.append(f"Chunk type not recognized as a public chunk type: {field.fieldname}")
            else:
            ...
            if field.key in [
                ...
                errors.append(f"Action is not currently supported for this profile: {field.key}")
