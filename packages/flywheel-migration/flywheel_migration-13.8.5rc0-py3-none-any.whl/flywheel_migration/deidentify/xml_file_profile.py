...
class XMLFileProfile(FileProfile):
    ...
    def __init__(self, file_filter=None):
        ...
        super(XMLFileProfile, self).__init__(packfile_type=self.name, file_filter=file_filter)
    ...
    def add_field(self, field):
        ...
        def list_fieldname(field, record):
            ...
            for el in record.tree.xpath(field.fieldname):
                fieldnames.append(XPathStr(record.tree.getpath(el), is_xpath=True, original=field.fieldname))
            return fieldnames
    ...
    def read_field(self, state, record, fieldname):
        ...
        original = fieldname if not hasattr(fieldname, "_original") else getattr(fieldname, "_original")
        if original in self.field_map:  # require for filenames manipulation without any field associated to it
            if self.field_map[original].key in ["increment-datetime", "increment-date"] and value:
                if self.field_map[original].key == "increment-datetime":
                    fmt = self.field_map[original].datetime_format or self.datetime_format
