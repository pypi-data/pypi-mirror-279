...
class TIFFRecord:
    ...
    def validate(self):
        ...
        if self.image.n_frames > 1:
            raise ValueError(
                f"{self.image.n_frames} frames found in {self.path}. " f"Currently only supporting 1 frames"
            )
    ...
    def save_as(self, filepath, file_type=None, **kwargs):
        ...
        try:
            self.image.save(filepath, format=file_type, tiffinfo=self.metadata, **kwargs)
        except RuntimeError:
            self.image.save(filepath, format=file_type, tiffinfo=self.metadata, **kwargs)
...
class TIFFFileProfile(FileProfile):
    ...
    file_signatures = [(0, b"\x49\x49\x2A\x00"), (0, b"\x4D\x4D\x00\x2A")]

    def __init__(self, file_filter=None):
        file_filter = file_filter if file_filter else self.default_file_filter
        super(TIFFFileProfile, self).__init__(packfile_type=self.name, file_filter=file_filter)
    ...
    def save_record(self, state, record, dst_fs, path):
        record.save_as(os.path.join(dst_fs.root_path, path), file_type=self.output_format)
