...
V6_7_PATH = files("ipfabric.tools.managed_rbac.v6.7")
V6_8_PATH = files("ipfabric.tools.managed_rbac.v6.8")

DEFAULT_SETUP = {
    "v6.7": {
        "policies": json.loads(V6_7_PATH.joinpath("policies.json").read_text()),
        "roles": json.loads(V6_7_PATH.joinpath("roles.json").read_text()),
    },
    "v6.8": {
        "policies": json.loads(V6_8_PATH.joinpath("policies.json").read_text()),
        "roles": json.loads(V6_8_PATH.joinpath("roles.json").read_text()),
    },
}
...
class ManagedRBAC(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        self._roles = DEFAULT_SETUP[v]["roles"]
        self._current_roles = {
