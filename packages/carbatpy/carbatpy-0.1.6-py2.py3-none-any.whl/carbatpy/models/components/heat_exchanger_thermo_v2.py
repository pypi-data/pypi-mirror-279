...
class StaticHeatExchanger:
    ...
    def pinch_calc(self,  verbose=False, plot_info={}):
        ...
        if abs(self.dt_min) - self.d_temp_separation_min < -1e-3:
            self.warning = 900
        ...
        if len(plot_info)>0:
            if plot_info["what"][0] ==2:
                data = (w_array[:,plot_info["what"][0]] -
                        w_array[:,plot_info["what"][0]].min()) \
                    *self.m_dot_w + plot_info["x-shift"][0]
                plot_info["ax"].plot(data,
                                     w_array[:,plot_info["what"][1]],
                                     plot_info["col"][0],
                                     label=plot_info["label"][0])
                data = (s_array[:,plot_info["what"][0]] -
                        s_array[:,plot_info["what"][0]].min()) \
                    *self.m_dot_s + plot_info["x-shift"][1]
                plot_info["ax"].plot(data,
                                     s_array[:,plot_info["what"][1]],
                                     plot_info["col"][1],
                                     label=plot_info["label"][1])
            else:
                print(f"H-Ex: plotting only implemented fot T-H_dot [2,0]. You requested{plot_info['what']}")
        if verbose:
            print(
                f"Min T-distance {self.dt_min}, Mean T-distance {self.dt_mean}")
            if self.heating > 0:
    ...
    def find_pinch(self, secondary=True):
        ...
        try:
            result = minimize(self.pinch_root, x0, args =(secondary,),
        ...
        # except:
            ...
            print("find pinch:", type(inst))    # the exception type
...
if __name__ == "__main__":
    ...
    hex0.pinch_calc(plot_info=PLOT_INFO)
    ax_act.legend()
    #-----------------------------------------------
    #       Previous way
    ...
    if hex1.warning > 2:
        ...
        hex1.pinch_calc(plot_info=PLOT_INFO)
        ax_act.legend()
