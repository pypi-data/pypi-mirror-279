...
class TargetArduino(TargetBase):
    ...
    def add_library(self, lib):
        if lib == 'Servo':
            ...
            self.add_sources(f'{self._shared.library_root}/Servo/src/avr/Servo.cpp')
            self.add_include_directories(f'{self._shared.library_root}/Servo/src')
            self.add_compile_options('-DARDUINO_ARCH_AVR')
        else:
    ...
    def _gen_app(self):
        ...
        for file in self.sources:
            ...
            self._writeln(f'\t{self._shared.cpp} {self._shared.cpp_opts} {self._compile_opts} {self._inc_dirs} {file} -o {obj}')
    ...
    def _gen_link(self):
        ...
        self._gen_rule(rule, f'{self._shared.core_tgt} {self._hexfile}', f'{self.target}: link sketch and core')
        self._writeln('')
