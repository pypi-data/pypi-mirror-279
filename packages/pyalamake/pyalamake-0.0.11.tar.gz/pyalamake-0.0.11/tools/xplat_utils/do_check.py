...
class DoCheck:
    ...
    def _doxyfile_excludes(self, tag, line, state):
        if svc.cfg.mod_tech == 'python':
            ...
            m = re.search(r'EXCLUDE\s*\+=\s*(.+)/build_info\.txt', line)
        if not m:
            return

        if svc.cfg.is_module:
            if svc.cfg.mod_tech == 'python':
                exp_dir = f'./{svc.cfg.mod_dir_name}/lib'
            else:
                exp_dir = './lib'
        else:
            if svc.cfg.mod_tech == 'python':
    ...
    def _check_gitignore(self):
        ...
        with open(fname, 'r', encoding='UTF-8') as fp:
            for line in fp:
                ...
                if svc.cfg.mod_tech in ['cpp', 'arduino']:
                    ...
                    if re.search(r'^\s*/version.h\s*$', line):
                        state.found_version = True
                        continue
                else:  # python
                    ...
                    if re.search(r'^\s*/version.json\s*$', line):
                        state.found_version = True
                        continue
        ...
        if not state.found_version:
            svc.gbl.rc += 1
            svc.log.err(f'{tag}: version line not found')
            state.ok = False
    ...
    def _check_license(self):
        ...
        with open(path, 'r', encoding='UTF-8') as fp:
            ...
            if line4:
                svc.gbl.rc += 1
                ok = False
                svc.log.err(f'{fname}: line4 should be blank: "{line4}"')
    ...
    def _check_publish_files(self):
        ...
        #         svc.gbl.rc += 1
        #         svc.log.err(f'{tag} apps/modules should not have an "{fname}" file')
        #         ok = False
        ...
        for fname in ['do_publish']:
            ...
            if svc.cfg.is_module and not os.path.isfile(path):
                ...
                ok = False
        if svc.cfg.is_module:
            svc.log.check(ok, f'{tag} files exist')
        else:
            svc.log.check(ok, f'{tag} files do not exist')
    ...
    def _check_requirements_txt(self):
        ...
        for mod, found in mod_names.items():
            if mod in ['twine', 'setuptools']:
                if not svc.cfg.is_module:  # an app
                    if found:
                        svc.log.warn(f'{tag} apps do not require {mod}')
                    continue

            if not found:
                if mod == 'medver-pytest' and svc.cfg.mod_name == 'medver-pytest':
                    pass
                else:
                    svc.gbl.rc += 1
                    svc.log.err(f'{tag} missing {mod}')
                    ok = False
