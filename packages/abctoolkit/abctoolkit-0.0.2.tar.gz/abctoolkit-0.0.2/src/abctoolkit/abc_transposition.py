...
import re
import random
from abctoolkit.utils import extract_metadata_and_tunebody, extract_metadata_and_parts, extract_global_and_local_metadtata
...
def transpose_to_abc_lines(abc_lines, key):

    transposed_abc_text, _, _ = transpose_an_abc_text(abc_lines, key)
    transposed_abc_lines = transposed_abc_text.split('\n')
    transposed_abc_lines = [line for line in transposed_abc_lines if line.strip() != '']
    transposed_abc_lines = [line + '\n' for line in transposed_abc_lines]

    return transposed_abc_lines
