...
def batch_convert_abc(ori_folder, des_folder, unified_L=8, no_line_breaks=True):
    ...
    for i in range(num_cpu):
        ...
        arg_lists[i].append((file_list[start_idx:end_idx], cmd_local, des_folder))

    pool = Pool(processes=os.cpu_count())
    pool.map(convert_abc, arg_lists)


def batch_convert_xml(ori_folder, des_folder):
    '''
    ori_folder: str
    des_folder: str
    '''
    ...
    pool = Pool(processes=os.cpu_count())
    pool.map(convert_xml, arg_lists)
