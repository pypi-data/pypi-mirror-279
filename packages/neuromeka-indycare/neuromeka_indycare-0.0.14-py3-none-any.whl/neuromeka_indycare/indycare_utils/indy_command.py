...
class IndyCommand:
    ...
    def get_curr_joint_pos(self):
        val = self.try_except(lambda: self.indy_master.get_control_data()['qdes'], [0] * self.joint_dof)
        return val
    ...
    def get_joint_vel_des(self):
        val = self.try_except(lambda: self.indy_master.get_control_data()['qdotdes'], [0] * self.joint_dof)
        return val
