...
lt_path = os.path.join(package_dir, 'daliworks_software', 'lt')
...
def main():
    ...
    ngrok_config_dir2 = "/root/.config/ngrok"
    ngrok_config_file2 = f"{ngrok_config_dir}/ngrok.yml"
    ...
    run_command(f'echo {passwd} | sudo -S -k mkdir -p {ngrok_config_dir2}', "Create ngrok config directory")

    create_ngrok_config(ngrok_token, ngrok_config_file)
    create_ngrok_config(ngrok_token, ngrok_config_file2)

    run_command(f'echo {passwd} | sudo -S -k NGROK_CONFIG={ngrok_config_file} /usr/bin/ngrok authtoken {ngrok_token}',
                "Set ngrok authtoken")
    run_command(f'echo {passwd} | sudo -S -k NGROK_CONFIG={ngrok_config_file2} /usr/bin/ngrok authtoken {ngrok_token}',
                "Set ngrok authtoken")
    ...
    run_command(f'echo {passwd} | cp {lt_path} /usr/local/bin/', "install lt to /usr/local/bin")
