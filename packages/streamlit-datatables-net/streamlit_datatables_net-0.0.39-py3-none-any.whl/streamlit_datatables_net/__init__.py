...
try:
    ...
    from .utility_functions import allow_top_navigation
except:
    from date_formatter import python_to_luxon_format
    from utility_functions import allow_top_navigation
...
def st_datatable(data: pd.DataFrame | list[dict], column_order: List[str] = None, props: Dict = None, dt_config: str = "default", table_id: str = None, class_name: str = None, js_scripts: List[str] = None, stylesheets: List[str] = None, disabled: bool = False, enable_top_navigation: bool = False, date_format: str = None, is_datetime: bool = False, export_file_name: str = None, on_click: Optional[Callable] = None, args: Optional[Tuple[Any, ...]] = None,
    ...
    component_value = _component_func(
        props=props, table_id=table_id, class_name=class_name, js_scripts=js_scripts, stylesheets=stylesheets, key=key, disabled=disabled, date_format=luxon_date_format, export_file_name=export_file_name, default={})
        ...
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    props = {}
                    props["lengthChange"] = False
                    props["pageLength"] = -1
                    props["layout"] = {
                        "top2Start":  {"buttons": ['excel']},
                        "topStart": "",
                        'topEnd': 'search',
                        'bottomStart': 'info',
                        'bottomEnd': 'paging'
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            big_data, column_order=column_order, props=props, table_id="example_1", key="example_1", dt_config="default", date_format="%m/%d/%Y", is_datetime=True, disabled=False, enable_top_navigation=True, export_file_name="Example 1", on_click=my_on_click_function, args=["example_1"])
                        st.json(dt_response)
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            data, props=props, table_id="example_1", key="example_1b", enable_top_navigation=True, date_format="%b %d %Y", is_datetime=True)
                        st.json(dt_response)
        ...
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            data, key="example_2", date_format='%Y-%b-%d')
                        st.json(dt_response)
