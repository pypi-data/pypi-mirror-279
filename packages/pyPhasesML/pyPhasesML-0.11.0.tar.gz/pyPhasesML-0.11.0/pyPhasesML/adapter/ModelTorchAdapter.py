...
class ModelTorchAdapter(ModelAdapter):
    ...
    def validate(self, validationData):
        ...
        if len(self.validationMetrics) > 0:
            ...
            return metrics, justPrintMetrics
    ...
    def prepareTargetsForLoss(self, targets, oneHotDecoded=None):
        ...
        mask = None
        if oneHotDecoded:
            targets = targets.reshape(-1, self.config.numClasses)
            if self.ignoreClassIndex is not None:
                mask = targets.eq(0).sum(-1) < self.config.numClasses
    ...
    def mapOutputForLoss(self, output, mask=None):
        output = (
            output.reshape(-1, self.config.numClasses)
