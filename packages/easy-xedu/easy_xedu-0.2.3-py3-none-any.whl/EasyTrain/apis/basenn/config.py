...
def update_pretrained_path(pretrained):
    ...
    if os.name == "nt":
        pretrained_path = os.path.join(pwd,"checkpoints","basenn_model",global_varibles['dataset'].split("\\")[0],pretrained)
    else:
        pretrained_path = os.path.join(pwd,"checkpoints","basenn_model",global_varibles['dataset'].split("/")[0],pretrained)
    global_varibles['pretrained_path'] = pretrained_path
...
def generate_basenn_code():
    ...
    if global_varibles['pretrained_path'] is None:
        ...
        train_part = "\t"+f"model.train(epochs={global_varibles['epochs']},lr={global_varibles['lr']},loss='{global_varibles['loss']}',metrics=['{global_varibles['metrics']}'],checkpoint=r'{global_varibles['pretrained_path']}')" + "\n"
