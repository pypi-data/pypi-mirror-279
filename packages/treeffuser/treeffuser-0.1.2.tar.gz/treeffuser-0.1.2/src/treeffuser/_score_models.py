...
def _fit_one_lgbm_model(
    yXt: Float[np.ndarray, "batch x_dim"],
    target: Float[np.ndarray, "batch y_dim"],
    yXt_val: Float[np.ndarray, "batch x_dim"],
    target_val: Float[np.ndarray, "batch y_dim"],
    seed: int,
    verbose: int,
    ...
    **lgbm_args,
    ...
    model = lgb.LGBMRegressor(
        ...
        n_jobs=n_jobs,
        linear_tree=False,
        # interaction_constraints=[[0], [yXt.shape[1]-1], list(range(1, yXt.shape[1]-1))],
        **lgbm_args,
    )
    eval_set = None if yXt_val is None else (yXt_val, target_val)
    if cat_idx is None:
        cat_idx = "auto"
    model.fit(
        X=yXt,
...
class LightGBMScoreModel(ScoreModel):
    ...
    **lgbm_args
    ...
    def fit(
        ...
        lgb_yXt_train, lgb_yXt_val, lgb_target_train, lgb_target_val, cat_idx = _make_training_data(
            X=X,
            y=y,
            sde=self.sde,
        ...
        for i in range(y_dim):
            lgb_target_val_i = lgb_target_val[:, i] if lgb_target_val is not None else None
            score_model_i = _fit_one_lgbm_model(
                yXt=lgb_yXt_train,
                target=lgb_target_train[:, i],
                yXt_val=lgb_yXt_val,
                target_val=lgb_target_val_i,
                cat_idx=cat_idx,
                seed=self.seed,
                n_jobs=self.n_jobs,
                **self._lgbm_args,
