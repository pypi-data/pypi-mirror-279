...
def search_publication_by_doi(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Always show form so user can set the image URL if they want
            return render(request, "NEMO_publications/publication_metadata.html", {"form": publication_metadata_form})
    else:
...
def get_carousel_parameters(number_of_results, content):
    ...
    if content in ["publications", "mixed"]:
        items += math.ceil(min(number_of_latest_pub_to_display, number_of_results) / 2)
    refresh_time = items * interval_time_in_ms
    return {"interval_time": interval_time_in_ms, "refresh_time": refresh_time, "content": content}
