...
class DataSource:
    ...
    def dailyByClassification(self, df: pd.DataFrame, filters: str, dateFrom: str = None, dateTo: str = None,
                              compensate: bool = False, considerBackground: bool = False, totalRow: bool = True,
                              totalColumn: bool = True) -> pd.DataFrame:
        """
        @param df: 
        @param filters: 
        @param dateFrom: 
        @param dateTo: 
        @param considerBackground:
        @param totalColumn:
        @param totalRow:
        @return:
        """

        avgDailyDict = self._parent.tabStats.avgDailyDict
        ...
        for cl in classList:
            ...
            for dt in dtList:
                ...
                if countDict[dt] >= 0 and avgDailyDict is not None and len(avgDailyDict):

                    if 'OVER' and 'UNDER' in filters:
                        background = avgDailyDict['OVER'] + avgDailyDict['UNDER']
                    elif 'OVER' in filters:
                        background = avgDailyDict['OVER']
                    elif 'UNDER' in filters:
                        background = avgDailyDict['UNDER']
                    else:
                        background = 0

                    if compensate and countDict[dt] < background:
                        countDict[dt] = background
    ...
    def getCfgRevisionFromID(self, eventId: int):
        ...
        if self._adf is not None:
            ...
            revision = \
                self._adf.loc[(self._adf['id'] == eventId) & (self._adf['event_status'] == 'Fall'), ['revision']].iloc[0, 0]
        return revision
    ...
    def classifyEvents(self, fromID: int, toID: int, overwrite: bool = False):
        ...
        if df is not None:
            ...
            nextReferenceN = self.DBFS_RANGE_MAX
                ...
                if cla == '':
                    ...
                    if RFIfilter > 0:
                        ...
                        if stdDev[1] > RFIfilterThreshold or (
                            ...
                            print("eventID#{} is a fake due to radio interferences".format(myId))
                    ...
                    if CAR1filter > 0:
                        ...
                        # first, in order to let the filter work with any detection range setting.
    ...
    def makeDaily(self, df: pd.DataFrame, dtStart: str, dtEnd: str, dtRes: str, filters: str = '',
        ...
        """
        avgDailyDict = self._parent.tabStats.avgDailyDict
        avgHourDf = self._parent.tabStats.avgHourDf
        avg10minDf = self._parent.tabStats.avg10minDf
        ...
        if dtRes == 'D':
            ...
            for i in range(0, len(dtRange) - 1):
                ...
                if count >= 0 and avgDailyDict is not None and len(avgDailyDict):
                    if 'OVER' and 'UNDER' in filters:
                        background = avgDailyDict['OVER'] + avgDailyDict['UNDER']
                    elif 'OVER' in filters:
                        background = avgDailyDict['OVER']
            ...
            for i in range(0, len(dtRange) - 1):
                ...
                if count >= 0 and avgHourDf is not None and not avgHourDf.empty:
                    if 'OVER' and 'UNDER' in filters:
                        background = avgHourDf.loc['Total'][column]
                    elif 'OVER' in filters:
                        background = avgHourDf.loc['OVER'][column]
                    elif 'UNDER' in filters:
                        background = avgHourDf.loc['UNDER'][column]
                    else:
                        background = 0
            ...
            for i in range(0, len(dtRange) - 1):
                ...
                if count >= 0 and avg10minDf is not None and not avg10minDf.empty:
                    if 'OVER' and 'UNDER' in filters:
                        background = avg10minDf.loc['Total'][column]
                    elif 'OVER' in filters:
                        background = avg10minDf.loc['OVER'][column]
                    elif 'UNDER' in filters:
                        background = avg10minDf.loc['UNDER'][column]
                    else:
                        background = 0
