...
def connect():
    ...
    html_css = """
    <style>
    .element-container:has(.elim), .element-container:has(.elim.next) + .element-container, .element-container:has(iframe[title="streamlit_execute.streamlit_execute_init"]), .element-container:has(iframe[title="streamlit_execute.streamlit_execute_run"]), .element-container:has(iframe[title="__init__.streamlit_execute_init"]), .element-container:has(iframe[title="__init__.streamlit_execute_run"]) {
        display: none;
    }
    </style>
    ...
    <script>                
        function sendMessageToRecipient(event) {
            if (event.data.isStreamlitMessage && event.data.type === "streamlit_execute:run"){
                window.parent.document.querySelectorAll('iframe[title="streamlit_execute.streamlit_execute_init"]').forEach((iframe) => {
                ...
                window.parent.document.querySelectorAll('iframe[title="streamlit_execute.streamlit_execute_run"]').forEach((iframe) => {
                    iframe.contentWindow.postMessage(event.data, "*");
                });
...
def init(code="", key="st_exec:init"):
    ...
    component_value = _component_func_init(code=code, key=key, default={"id": "", "code":"", "status":"", "value":"", "stdout":""})
...
def run(code, target_key="st_exec:init", key=None):
    ...
    # "default" is a special argument that specifies the initial return
    # value of the component before the user has interacted with it.
    component_value = _component_func_run(code=code, target_key=target_key, key=key, default={"id": "", "code":"", "status":"", "value":"", "stdout":""})
