...
class Diff:
    ...
    def _find_new_filepath(self) -> str:
        ...
        """
        if self.attrs:
            return self.attrs.b_path_fallback[2:]
    ...
    def _parse_patch_attributes(self) -> DiffAttr:
        ...
        for _header in re_header.finditer(encoded_body):
            ...
            diff_attrs.append(
                DiffAttr(
                    a_path.decode("utf-8") if a_path else None,
                    b_path.decode("utf-8") if b_path else None,
                    b_path_fallback.decode("utf-8") if b_path_fallback else None,
                )
            )
...
class CommitDiff:
    ...
    def _segment_diffs(self, patch: str):
        """
        Segments the patch into diffs
        """
    ...
    # def get_test_context(self) -> List[Tuple[str, str]]:
    #     """
    #     Finds the function and class context of a test file
    #     """
    #     test_context = []
    #     for test_file, hunk in [h for h in self.hunks if h[0] in self.test_files]:
    #         test_code = self.
    #         # if hunk declares a new test function, use that as the context
    #         new_func_decl = hunk.new_func_decl()
    #         if new_func_decl:
    #             test_context.append((new_func_decl, None))
    #         else:
    #             fun_name, class_name = self.find_context(test_code, hunk.start, hunk.end)
    #             test_context.append((fun_name, class_name))
