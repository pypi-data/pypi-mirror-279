...
from modelbit.utils import toUrlBranch, dumpJson, inDeployment
import re, requests
...
def inChunks(lst: List[Any], n: int):
...
def callDeployment(region: str,
                   ...
                   batchChunkSize: Optional[int] = DefaultBatchSize) -> Dict[str, Any]:
  ...
  if inDeployment():
    raise UserFacingError(f"Using modelbit.get_inference is not supported within deployments.")
