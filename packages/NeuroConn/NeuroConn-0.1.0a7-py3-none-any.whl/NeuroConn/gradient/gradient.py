...
def align_gradients(gradients, n_components, ref = 'hcp10', custom_ref = None, *args):
    ...
    *args :
    ...
    if gradients.shape[-1] != ref_gradients.shape[-1]:
            ref_gradients = ref_gradients.T
    ...
    aligned_gradients = np.array(Alignment.fit(gradients, ref_gradients).aligned_)
    return aligned_gradients
    
def get_gradients(data, subject, n_components, task, output_space=None, parcellation = None, n_parcels = None, kernel = None, approach = 'pca', conn_threshold = 0.9, from_mat = True, mat_path = None,aligned = True, align_to = None, save = True, save_to = None):
    """
    ...
    if n_parcels is None:
        n_parcels = ''
    suffix = f'-{parcellation}{n_parcels}'
    ...
    if from_mat:
        if mat_path is None:
            ...
            input_path =[os.path.join(mat_path, i) for i in os.listdir(mat_path) if subject in i and output_space in i and parcellation in i and str(n_parcels) in i][0]
    input_data = np.load(input_path)
    if len(input_data.shape) == 3:
        gradients = []
        var_explained = []
        for i in input_data:
            gm.fit(i, sparsity = conn_threshold)
        ...
        gm.fit(input_data, sparsity = conn_threshold)
        gradients = gm.gradients_
        var_explained = gm.lambdas_
    if aligned:
        gradients = align_gradients(gradients, n_components, custom_ref=align_to)
        prefix = "aligned-"
    else:
        prefix = ""
    if save:
        if save_to is None:
            ...
            save_to_grads = os.path.join(save_dir, f'{prefix}{n_components}gradients-sub-{subject}-{task}{suffix}-{output_space}.npy')
            save_to_var_explained = os.path.join(save_dir, f'{n_components}comp_eigenvalues-sub-{subject}-{task}{suffix}-{output_space}.npy')
        else:
            save_to_grads = os.path.join(save_to, f'{prefix}{n_components}gradients-sub-{subject}-{task}{suffix}-{output_space}.npy')
            save_to_var_explained = os.path.join(save_to, f'{n_components}comp_eigenvalues-sub-{subject}-{task}{suffix}-{output_space}.npy')
        np.save(file = save_to_grads, arr = gradients)
        np.save(file = save_to_var_explained, arr = var_explained)
