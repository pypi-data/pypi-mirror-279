...
class APIExperiment(CommonExperiment):
    ...
    def log_parameters(
        ...
        """
        results = []
        if nested_support and exclusions.can_be_flattened(
            param_dict, source=messages.ParameterMessage.source_manual
        ):
            flatten_op_result = flattener.flatten_dict(d=param_dict)
            param_dict = flatten_op_result.flattened
            if flatten_op_result.max_depth_limit_reached:
                LOGGER.warning(LOG_PARAMS_MAX_DEPTH_REACHED, param_dict)
