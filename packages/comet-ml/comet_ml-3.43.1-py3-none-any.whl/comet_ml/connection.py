...
from .logging_messages import (
    ...
    CONNECTION_UPLOAD_THUMBNAIL_FAILED_ERROR,
...
def should_report(config: Config, server_address: str) -> bool:
...
class RestServerConnection(object):
    ...
    def get_old_run_id(self, previous_experiment: str) -> ExperimentHandshakeResponse:
        """
    ...
    def offline_experiment_start_end_time(
        self, run_id: str, start_time: int, stop_time: int
    ) -> None:
    ...
    def get(
        ...
        if response.status_code != 200:
            if response.status_code == 401:
                ...
                raise InvalidAPIKey(self.api_key, self.server_url)  # probably
...
class RestApiClientWithCache(RestApiClient):
    ...
    EXTRA_PAYLOAD = {
        "experiment/asset/list": {
            "type": ["all", "histogram_combined_3d", "image", "audio", ASSET_TYPE_VIDEO]
        }
    }
...
def upload_file(
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            elif check_upload_images_limit_reached_error_response(
...
def upload_asset_item(
    ...
    try:
        ...
        if (
            ...
            # to avoid spamming user with errors
            LOGGER.debug(CONNECTION_UPLOAD_ASSET_ITEM_FAILED_ERROR, e, exc_info=True)
        else:
            LOGGER.error(CONNECTION_UPLOAD_ASSET_ITEM_FAILED_ERROR, e, exc_info=True)
...
def upload_thumbnail(
    ...
    try:
        ...
        if (
            ...
            # to avoid spamming user with errors
            LOGGER.debug(CONNECTION_UPLOAD_THUMBNAIL_FAILED_ERROR, e, exc_info=True)
        else:
            LOGGER.error(CONNECTION_UPLOAD_THUMBNAIL_FAILED_ERROR, e, exc_info=True)
        Reporting.report(
            event_name=FILE_UPLOADED_FAILED,
            experiment_key=options.experiment_id,
            project_id=options.project_id,
