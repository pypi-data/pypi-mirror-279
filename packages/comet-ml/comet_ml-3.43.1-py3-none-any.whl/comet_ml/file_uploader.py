...
def is_user_text(input: Any) -> bool:
...
def save_matplotlib_figure(figure: Optional[Any] = None) -> str:
    ...
    if figure.get_axes():
        ...
        LOGGER.warning(FILE_UPLOADER_SAVE_MATPLOTLIB_FIGURE_EMPTY_EXCEPTION)
    ...
    def process_upload_to_be_converted(
        ...
        if PlotlyFigure is not None and isinstance(user_input, PlotlyFigure):
            ...
            try:
                ...
                LOGGER.warning(
                    FILE_UPLOADER_SAVE_MATPLOTLIB_FAILED_WARNING, exc_info=True
                )
                # An error occurred
                return None
...
def handle_in_memory_file_upload(
    tmp_dir: str, upload_filepath: ValidFilePath
) -> Union[None, ValidFilePath, TemporaryFilePath]:
    # If we cannot remove the uploaded file or need the file content will
    # be frozen to the time the upload call is made, pass copy_to_tmp with
    # True value
