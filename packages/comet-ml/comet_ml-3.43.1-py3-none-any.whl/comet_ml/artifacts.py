...
from .assets.data_writers import (
    AssetDataWriter,
    AssetDataWriterFromGCS,
    AssetDataWriterFromResponse,
    AssetDataWriterFromS3,
)
...
from .config import Config, get_check_tls_certificate, get_comet_timeout_http
from .connection import RestApiClient
from .connection_helpers import get_thread_session
from .exceptions import (
    ArtifactAssetNotFound,
    ArtifactConflictingAssetLogicalPath,
    ArtifactDownloadException,
    ArtifactNameTooLongException,
...
PreparedRequest = Tuple[str, Dict[str, Any], Dict[str, Any]]
...
def _call_post_prepared_request(
    prepared_request: Tuple[str, Dict[str, Any]], timeout: float, verify_tls: bool
) -> requests.Response:
...
class Artifact(object):
    def __init__(
        ...
        # Artifact fields
        self.name = name
        if len(self.name) > MAX_ARTIFACT_NAME_LENGTH:
            raise ArtifactNameTooLongException(
                "Artifact name must be no more than %d characters"
    ...
    def add_remote(
        ...
        """
        if asset_type:
            LOGGER.warning(ARTIFACT_ASSET_TYPE_DEPRECATED_WARNING)
        asset_type = None
...
class LoggedArtifact(object):
    ...
    def download(
        ...
        Args:
            ...
            sync_mode: Boolean. Enables download of remote assets from the cloud storage platforms
            (AWS S3, GCP GS).

        Overwrite strategy allowed values:

            * False or "FAIL": If a file already exists and its content is different, raise the
            `comet_ml.exceptions.ArtifactDownloadException`. * "PRESERVE": If a file already exists
            and its content is different, show a WARNING but preserve the existing content. * True
            or "OVERWRITE": If a file already exists and its content is different, replace it by the
            asset version asset.
...
def _log_artifact(artifact: Artifact, experiment: Any) -> LoggedArtifact:
