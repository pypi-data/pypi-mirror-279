...
class LogRemoteModelException(CometException):
    def __init__(self, backend_err_msg=None, sdk_error_code=None):
        # type: (Optional[str], Optional[int]) -> None
        self.backend_err_msg = backend_err_msg
        self.sdk_error_code = sdk_error_code
    ...
    def __repr__(self):
        if self.sdk_error_code is not None and self.backend_err_msg is not None:
            return "Log remote model failed with an error, %r [%s]" % (
                self.backend_err_msg,
                self.sdk_error_code,
            )
        else:
            return "Log remote model failed with an error, %r" % (self.backend_err_msg)
...
class ArtifactNameTooLongException(CometException):
    pass
...
class ImagesUploadLimitReachedException(CometException):
    def __str__(self):
        return UPLOAD_IMAGES_LIMIT_REACHED
