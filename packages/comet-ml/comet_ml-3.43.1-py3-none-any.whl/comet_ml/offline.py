...
from .utils import generate_guid, local_timestamp, random_ascii_string, wait_for_done
...
class OfflineExperiment(BaseExperiment):
    ...
    def display(self, *args: Any, **kwargs: Any) -> None:
        """Do nothing"""
        LOGGER.error(OFFLINE_DISPLAY_METHOD_ERROR)

    def display_project(self, *args: Any, **kwargs: Any) -> None:
        """Do nothing"""
        LOGGER.error(OFFLINE_DISPLAY_PROJECT_METHOD_ERROR)
    ...
    def log_embedding(
        ...
        """
        raise NotImplementedError(OFFLINE_LOG_EMBEDDING_METHOD_ERROR)
...
class OfflineSender(object):
    ...
    def _do_comet_start_handshake(self) -> ExperimentHandshakeResponse:
        ...
        if not self.user_provided_experiment_key:
            ...
            return self.connection.get_run_id(
                creation_project_name,
                creation_workspace,
                offline=True,
            )
        ...
        with io.open(
            ...
            for i, line in enumerate(messages_files):
                ...
                try:
                    ...
                    elif message_type == RemoteModelMessage.type:
                        self._process_remote_model_message(message)
                    elif message_type == Log3DCloudMessage.type:
                        self._process_3d_cloud_message(message)
                    else:
...
def main_upload(
    archives, force_upload, override_workspace=None, override_project_name=None
):
