...
class Compute:
    ...
    Note:
        ...
        `baseten instance type reference <https://docs.baseten.co/performance/instances>`_.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            predict_concurrency: number of concurrent requests a single replica of a
              deployed chainlet handles. Concurrency concepts are explained in
              `this guide <https://truss.baseten.co/guides/concurrency>`_. It is
              important to understand the difference between `predict_concurrency` and
              the concurrency target (used for autoscaling, i.e. adding or removing
              replicas).
              Furthermore, the `predict_concurrency` of a single instance is
              implemented in two ways:
                  1. Via python's `asyncio`, if `run_remote` is an async def. This
                     requires that `run_remote` yields to the event loop.
                  2. With a threadpool if it's a synchronous function. This requires
...
class AssetSpec(SafeModel):
    """Parsed and validated assets."""

    # TODO: this is not stable yet and might change or refer back to truss.
    secrets: dict[str, str] = pydantic.Field({}, description="TEST")
...
class Assets:
    ...
    def __init__(
        self,
        cached: Iterable[truss_config.ModelCache] = (),
        secret_keys: Iterable[str] = (),
    ) -> None:
        """
        Args:
            cached: One or more `truss_config.ModelCache` objects. See
              `truss caching guide <https://truss.baseten.co/guides/model-cache#enabling-caching-for-a-model>`_.
...
class DeploymentContext(SafeModelNonSerializable, Generic[UserConfigT]):
    ...
    Args:
        ...
        secrets: A mapping from secret names to secret values. It contains only the
          secrets that are listed in `remove_config.assets.secret_keys` of the
          current chainlet.
    """
