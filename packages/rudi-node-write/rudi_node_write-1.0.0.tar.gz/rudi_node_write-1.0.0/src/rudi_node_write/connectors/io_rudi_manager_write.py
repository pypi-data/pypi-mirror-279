...
class RudiNodeManagerConnector(Connector):
    ...
    # ----------[ Basic API calls ]-------------------------------------------------------------------------------------
    def _get_api(self, url: str, headers: dict, keep_alive: bool = False, should_log_request=True):
        """
    ...
    def _put_api(self, url: str, body: dict | str, headers: dict, keep_alive: bool = False, should_log_request=True):
        """
    ...
    def _post_api(self, url: str, body: dict | str, headers: dict, keep_alive: bool = False, should_log_request=True):
        """
        Performs an identified PUT request through /api path
        :param url: part of the URL that comes after /api
        :param payload: the PUT request payload
    ...
    def _del_api(self, url: str, headers: dict, keep_alive: bool = False, should_log_request=True):
        """
        Performs an identified PUT request through /api path
        :param url: part of the URL that comes after /api
        :param payload: the PUT request payload
    ...
    def test_connection(self):
        test = self._get_api(url="open/test", headers=self._def_headers, should_log_request=False)
        if test == "test":
    ...
    @property
    def tags(self):
        return self._get_api(url="open/tags", headers=self._def_headers)

    @property
    def hash(self):
        return self.request(relative_url="api/open/hash", req_method="GET", headers=self._def_headers)
    ...
    @property
    def _pm_headers(self):
        if self._cached_pm_headers is None:
            self._cached_pm_headers = self._def_headers | {"Authorization": f"Bearer {self._pm_jwt}"}
    ...
    @property
    def _media_jwt(self) -> str:
        if is_jwt_expired(self._cached_media_jwt):
            res = self._get_api(url="media/jwt", headers=self._pm_headers)
    ...
    def _media_connector(self):
        if self._cached_media_connector is None:
            self._cached_media_connector = RudiNodeMediaConnector(self.media_url, self._media_jwt)
        if is_jwt_expired(self._cached_media_connector.jwt):
            self._cached_media_connector.jwt = self._media_jwt
    ...
    def get_api_data(self, url: str, keep_alive: bool = False):
        ...
        return self.request(
            req_method="GET",
            relative_url=slash_join("api/data", url),
    ...
    def _clean_obsolete_cache(self) -> None:
        ...
        for cached_type in self._data_cache.keys():
            if time() - int(self._data_cache[cached_type][_REFRESH_KEY]) > _DELAY_REFRESH_S:
    ...
    def get_data(self, obj_type: RudiObjectTypeStr | str) -> list[dict] | dict:  # type: ignore
        ...
        obj_cache = self._data_cache.get(obj_type)

        if obj_cache is None or time() - obj_cache.get(_REFRESH_KEY) > _DELAY_REFRESH_S:
            if obj_type in RUDI_OBJECT_TYPES:
                obj_data: list[dict] = self._get_full_obj_list(obj_type)
            else:
                obj_data = self.get_api_data(obj_type)  # type: ignore

            self._data_cache[obj_type] = {"data": obj_data, _REFRESH_KEY: time()}
        obj_data = self._data_cache[obj_type]["data"]
    ...
    def _get_full_obj_list(self, url_bit: str, max_count: int = 0) -> list[dict]:
        ...
        obj_nb = self._count_obj(base_url)
    ...
    def used_contact_list(self) -> list[dict]:
        ...
        for meta in self.metadata_list:
            ...
            if not isinstance(meta_contacts, list):
                raise Exception("Contacts should be a list of dict")
    ...
    # ----------[ Communication with Media module ]---------------------------------------------------------------------
    def post_local_file(
        self,
        file_local_path: str,
        ...
        # Posting the file as a binary directly to RUDI node's "Media" storage module
        res = self._media_connector.post_local_file(
    ...
    def commit_media(self, media_id: str, commit_uuid: str, zone_name: str):
        ack = self._post_api(
            url="media/commit",
            body={
                "media_id": media_id,
...
class RudiNodeMediaConnector(Connector):
    def __init__(self, server_url: str, jwt: str):
        ...
        self._initial_headers = {
            "User-Agent": "RudiNodeMediaConnector",
    ...
    def media_list(self) -> list:
        media_list = self._get_api_media("list")
    ...
    def post_local_file(self, file_local_path: str, media_id: str = uuid4_str()):
        ...
        headers = self.get_media_headers_for_file(media_info)
...
if __name__ == "__main__":  # pragma: no cover
    ...
    log_d(tests, "nb orgs", pm_connector._count_obj("organizations"))
    ...
    rudi_media_url = pm_connector.media_url if pm_connector.media_url is not None else rudi_node_creds["url"]
    rudi_media = RudiNodeMediaConnector(server_url=rudi_media_url, jwt=pm_connector._media_jwt)
