...
from .const import *
from .model import Model, Table, metadata
...
class PermissionApi(Model):
    ...
    __table_args__ = (UniqueConstraint("api_id", "permission_id"),)
    id = Column(Integer, primary_key=True, index=True)

    api_id = Column(Integer, ForeignKey(f"{API_TABLE}.id"), nullable=False)
...
class Api(Model):
    ...
    permissions = relationship(PermissionApi, back_populates="api", lazy="selectin")
...
class Permission(Model):
    ...
    apis = relationship(PermissionApi, back_populates="permission", lazy="selectin")
...
class User(Model):
    ...
    fail_login_count = Column(Integer, default=0, nullable=False)
