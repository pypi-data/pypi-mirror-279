...
from .utils import both_are_filled_or_empty

PRIMARY_KEY = int | str | Dict[str, str | int]


class IgnoredData(BaseModel):
...
class FilterInfo(BaseModel):
    """
    Represents information about a filter.

    Attributes:
        filters (list[SearchFilter]): A list of search filters.
        label (str): The label for the filter.
        schema (ColumnInfo | ColumnRelationInfo): The schema for the filter.
    """

    filters: list[SearchFilter] = []
    label: str = ""
    schema: ColumnInfo | ColumnRelationInfo
...
class BaseResponse(BaseModel):
    ...
    Attributes:
        result (Union[dict, List[dict], None]): The result of the response. It can be a dictionary, a list of dictionaries, or None.
    """
...
class BaseResponseSingle(BaseResponse):
    ...
    Attributes:
        id (PRIMARY_KEY): The ID of the response object.
    """
...
class BaseResponseMany(BaseResponse):
    ...
    Attributes:
        ...
        ids (List[PRIMARY_KEY]): The list of item IDs.
    """
...
class QuerySchema(BaseModel):
    ...
    def validate_order_fields(cls, v: str | None, info: ValidationInfo):
        data = info.data
        try:
            both_are_filled_or_empty(v, data.get("order_column"))
