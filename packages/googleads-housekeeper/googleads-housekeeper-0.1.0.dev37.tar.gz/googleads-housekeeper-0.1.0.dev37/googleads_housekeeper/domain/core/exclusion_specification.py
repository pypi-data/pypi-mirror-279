...
class BaseExclusionSpecificationEntry:
    ...
    def is_satisfied_by(self, placement_info: GaarfRow) -> tuple[bool, dict]:
        ...
        """
        placement_as_dict = {}
        if ('conversion_name' in placement_info.column_names):
            if self.name in ['conversion_name', 'conversions_', 'all_conversions_']:
                if self.operator in ('regexp', 'contains', 'has_latin_letters'):
                    return self._check_regexp(placement_info), self._to_dict(placement_info)
                return self._eval_expression(placement_info), self._to_dict(placement_info)
    ...
    def _to_dict(self, gaarf_row: GaarfRow) -> dict[str, int | float | str]:
        """Converts a GaarfRow object into a dictionary."""
        return {name: value for name, value in zip(gaarf_row.column_names, gaarf_row.data)}
