...
class PlacementExcluder:
    ...
    excludable_from_account_only = ('PERFORMANCE_MAX', 'DISCOVERY')

    def __init__(self, client: gaarf.api_clients.GoogleAdsApiClient,
                 exclusion_level: enums.ExclusionLevelEnum) -> None:
        self._client = client
        self.exclusion_level = (
            enums.ExclusionLevelEnum[exclusion_level] if isinstance(
                exclusion_level, str) else exclusion_level)
    ...
    def exclude_placements(
        ...
        if (associated_with_list_placements :=
        ...
        if (excludable_from_account_only :=
    ...
    def _create_exclusion_operations(
        ...
        excludable_from_account_only_placements: list[
        ...
        for placement_info in placements:
            ...
            if not placement_operation:
                excludable_from_account_only_placements.append(
                    relevant_placement_info)
                continue
    ...
    def _create_placement_operation(
        ...
        if (self.exclusion_level != enums.ExclusionLevelEnum.ACCOUNT and
                placement_info.campaign_type
                in self.excludable_from_account_only):
        ...
        if (self.exclusion_level in (enums.ExclusionLevelEnum.CAMPAIGN,
            ...
            if (placement_info.campaign_type
                    in self.associatable_to_negative_lists):
        ...
        if placement_info.placement_type == (
            ...
            entity_criterion.placement.url = (
                self._format_website(placement_info.placement))
        if (placement_info.placement_type ==
                enums.PlacementTypeEnum.MOBILE_APPLICATION.name):
    ...
    def _add_placements_to_shared_set(self, customer_id: int,
        ...
        Args:
            customer_id:
                Google Ads account number.
            operations:
                Shared set attachment operations for a given customer_id.
        """
        ...
        try:
            ...
            logging.error(
                'Cannot add placements to exclusion list for account '
                "'%s' %d times", customer_id,
