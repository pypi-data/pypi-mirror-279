...
class PlacementFetcher:
    ...
    def get_placements_for_account(
        ...
        if runtime_options.is_conversion_query:
            ...
            self._get_placement_conversion_split_data(account, task_obj)):
                ...
                self._extend_placement_with_conversion_split_columns(placements=placements)
    ...
    def _join_conversion_split(
        ...
        for row in placements:
            ...
            conversion_row = placements_by_conversion_name.loc[
                is_conversion_row]
            data = list(row.data)
            if not (conversions := sum(conversion_row['conversions_'].values)):
                conversions = 0.0
            if not (all_conversions := sum(
                    conversion_row['all_conversions_'].values)):
                all_conversions = 0.0
    ...
    def _extend_placement_with_conversion_split_columns(
            self,
            placements: gaarf.report.GaarfReport) -> None:
        ...
        for row in placements:
            row['conversion_name'] = ''
            row['conversions_'] = 0.0
            row['all_conversions_'] = 0.0
