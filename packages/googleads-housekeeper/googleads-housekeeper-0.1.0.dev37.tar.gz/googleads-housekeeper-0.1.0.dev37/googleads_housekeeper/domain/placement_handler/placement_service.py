...
def exclude_placements(
        task_obj: task.Task, uow: unit_of_work.AbstractUnitOfWork, client,
        runtime_options: RuntimeOptions) -> placement_excluder.ExclusionResult:
    ...
    excluder = placement_excluder.PlacementExcluder(client,
    ...
    placement_exclusion_lists = fetcher.get_placement_exclusion_lists(
        customer_ids)
    return excluder.exclude_placements(to_be_excluded_placements,
...
def _is_already_excluded_placement(
        ...
        exclusion_level: enums.ExclusionLevelEnum) -> bool:
    ...
    return (placement_info.placement
            in already_excluded_placements.get(level_id, []))
...
def find_placements_for_exclusion(
    ...
    for account in task_obj.accounts:
        account_allowlisted_placements = views.allowlisted_placements_as_tuples(
            uow, account)
        if not (placements := fetcher.get_placements_for_account(
                account, task_obj)):
            continue
                    ...
                    specifications=[[
                        ...
                        .YouTubeChannelExclusionSpecificationEntry(
                            'subscriberCount > 0')
                                    ...
                                    [
                                        exclusion_specification.
                                        YouTubeVideoExclusionSpecificationEntry(
                                            'likesCount > 0')
                                    ]]))
        ...
        placements = inject_extra_data(placements, extractor,
                                       account_allowlisted_placements,
                                       already_excluded_placements,
                                       task_obj.exclusion_level)
        if (placements := specification.apply_specifications(placements)):
...
def inject_extra_data(
        ...
        already_excluded_placements: dict[int, list[int]],
        exclusion_level: enums.ExclusionLevelEnum) -> gaarf.report.GaarfReport:
