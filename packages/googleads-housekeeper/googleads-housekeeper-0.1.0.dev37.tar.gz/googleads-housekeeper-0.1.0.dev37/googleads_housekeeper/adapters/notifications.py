...
class GoogleCloudAppEngineEmailNotifications(BaseNotifications):
    ...
    def _prepare_message(self, payload: MessagePayload) -> mail.Message:
        ...
        if placements_excluded := payload.placements_excluded_sample:
            ...
            body = self._generate_notification_html(
                payload.task_id, payload.task_name, payload.task_formula,
                payload.task_output, placements_excluded)
        else:
    ...
    def _generate_notification_html(
            ...
            placements: gaarf.report.GaarfReport) -> str:
        html_body = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        html_body += f"""
                ...
                <a href="https://{self.project_id}.ew.r.appspot.com/newtask?task={task_id}"
        ...
        for placement in placements:
            html_body += f"""
                <ul class="placement-list">
                    ...
                    {placement["placement"]}
                    <br/>
                    Reason: {_to_human_readable(getattr(placement, 'reason', 'Task without filters'))}</li>
                </ul>
            """
        html_body += """
            </div>
            ...
            <div class="logo-placeholder">
                <img src="https://services.google.com/fh/files/emails/gtech_logo_transparent.png" alt="gPS Ads Logo" width="32" height="32">
            </div>
...
class SlackNotifications(BaseNotifications):
    ...
    def send(self, payload: MessagePayload) -> None:
        ...
        if not (placements_excluded_sample :=
                payload.placements_excluded_sample):
            self.client.chat_postMessage(
                channel=channel,
                text=f'{task_name}: No placements were excluded')
        else:
            file = placements_excluded_sample['name'].to_pandas().to_csv(
                index=False, sep='\t')
...
class NullNotifications(BaseNotifications):
    ...
    def send(self, payload: MessagePayload) -> None:
        raise ValueError(f'Cannot send notification via '
...
class NotificationFactory:
    ...
    def create_nofication_service(
            self, notification_type: str) -> type[BaseNotifications]:
