...
class MixedData(Categorical):
    ...
    Julia Equivalent:
    `IAI.make_mixed_data <https://docs.interpretable.ai/v3.2.1/IAIBase/reference/#IAI.make_mixed_data>`
    ...
    """
    def __init__(self, values, ordinal_levels=None, categories=None, dtype=None):
        if not ordinal_levels:
            ordinal_levels = None
        self._ordinal_levels = ordinal_levels

        Categorical.__init__(self, values, categories, None, dtype)
    ...
    def copy(self):
        ...
        out = super().copy()
        out._ordinal_levels = ordinal_levels
