...
from engine import StructuredPrefix, TemplatingEngine, build_structured_prefix
from exceptions import MessageStartIdxNotFound, MissingContextData
...
def mock_raise_missing_context_data(key):
    del key
    pass  # The function does nothing, it's just a placeholder for testing
...
class TestBuildStructuredPrefix(unittest.TestCase):
    ...
    def test_narrator_injection(self):
        ...
        result = build_structured_prefix(
            self.mock_logger, structured_prefix, truncation_step=1
        )
    ...
    def test_pretruncation_step(self):
        ...
        chat_context_messages = [
            ChatContextMessage(
                author="Balderdash", text="filler", type=1, is_pinned=False
            )
        ] * 200
    ...
    def test_truncation_steps(self):
        ...
        for truncation_step, expectation in zip(
            [1, 100] + TRUNCATION_STEPS_CHOICES, expected_token_counts
        ):
            result = build_structured_prefix(
                self.mock_logger, structured_prefix, truncation_step=truncation_step
            )
    ...
    def test_edge_cases(self):
        ...
        with self.assertRaises(MessageStartIdxNotFound):
            structured_prefix = StructuredPrefix(
                character_definitions=[],
                pinned_history=[],
                chat_history=[],
    ...
    def test_templates(self):
        """Validate the correctness of each template in the registry."""
        for template_name in os.listdir("templates"):
            ...
            engine = TemplatingEngine(logger=self.mock_logger)
            rendered_template = engine.render_template(
                {
                    "chat_context_messages": [],
                    "character_definition_messages": [
                        "Lorem ipsum dolor \r\n sit amet."
                    ],
                    "username": MOCK_USERNAME,
                    "character": MOCK_CHARACTER,
                    "raise_missing_context_data": mock_raise_missing_context_data,
                    "get_character_priming": lambda x, y: [
                        {"src": "Foo", "text": "Bar"}
                    ],
                    "canonicalize_user_name": lambda x: x,
                    "escape_sequences": lambda x: x.replace("\n", "\\n").replace(
                        "\r", "\\r"
                    ),
                    "maybe_inject_narrator": lambda x: x,
                    "pretruncate_messages": lambda x, _: x,
                },
                template_name=template_name,
            )
            prompt_parts = yaml.load(rendered_template, Loader=yaml.CSafeLoader)
            self.assertEqual(prompt_parts[0]["name"], "bod_ts")
            self.assertEqual(prompt_parts[-1]["name"], "reply_prompt")
    ...
    def test_names(self):
        ...
        _ = build_structured_prefix(
            self.mock_logger, base_structured_prefix, truncation_step=1
        )
    ...
    def test_production_raw_template(self):
        ...
        with self.assertRaisesRegex(MissingContextData, r".*character.*"):
            _ = build_structured_prefix(
                self.mock_logger, structured_prefix, truncation_step=1
            )
        ...
        prompt_string = self.tokenizer.detokenize(result["tokenized_context"].tokens)
        self.assertIn("narrator: [some messages omitted]", prompt_string)
        ...
        prompt_string = self.tokenizer.detokenize(result["tokenized_context"].tokens)
        self.assertNotIn("narrator: [some messages omitted]", prompt_string)
