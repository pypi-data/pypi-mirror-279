...
class data_loader():
    ...
    def __init__(self, 
        ...
        self.x_cpl_valid = torch.FloatTensor(self.scaling(X_cpl_valid,X_scaler_mean,X_scaler_std))
        self.T_cpl_valid = torch.FloatTensor(T_cpl_valid.reshape(-1,1))
        self.y_cpl_valid = torch.FloatTensor(y_cpl_valid.reshape(-1,1))

        self.x_cpl_test = torch.FloatTensor(self.scaling(X_cpl_test,X_scaler_mean,X_scaler_std))
    ...
    def print_data(self):
        ...
        print("Number of unique compositions in training (heat capacity): {}".format(size_train))
        # print("Dataset separations are {:.2f} in train, {:.2f} in valid, {:.2f} in test".format(size_train/size_total,
        #                                                                             size_valid/size_total,
        #                                                                             size_test/size_total))
...
class model(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size = 300, num_layers = 4, nb_channels_raman = 800, 
        ...
        self.log_vars = torch.nn.Parameter(
             torch.tensor(
             [np.log(0.2*0.2), # viscosity
              np.log(0.05*0.05), # raman
              np.log(0.01*0.01), # density 
              np.log(0.2*0.2), # entropy
              np.log(0.002*0.002), #refractive index
              np.log(3.*3.)]), # Cpl
            requires_grad=True
            )
    ...
    def partial_cpl(self,x):
        ...
        """
        return torch.exp(self.out_thermo(self.forward(x))[:,22:])
...
def training(neuralmodel, ds, 
    ...
    # scaling coefficients for loss function
    # viscosity is always one
    # if using the method from Kendall et al. 2017
    # comment here and uncomment below
    ls = loss_scales()
    precision_visco = 1.0
    precision_raman = ls.raman
    precision_density = ls.density
    precision_entro = ls.entro
    precision_ri = ls.ri
    precision_cpl = ls.cpl
    ...
    while val_ex <= train_patience:
        ...
        for i in range(nb_folds): # loop for K-Fold training to reduce memory footprint
            ...
            # precision_visco = 1/(2*torch.exp(neuralmodel.log_vars[0]))
            # precision_raman = 1/(2*torch.exp(neuralmodel.log_vars[1]))
            # precision_density = 1/(2*torch.exp(neuralmodel.log_vars[2]))
            # precision_entro = 1/(2*torch.exp(neuralmodel.log_vars[3]))
            # precision_ri = 1/(2*torch.exp(neuralmodel.log_vars[4]))
            # precision_cpl = 1/(2*torch.exp(neuralmodel.log_vars[5]))
            ...
            loss_ri = precision_ri * criterion(y_ri_pred_train,y_ri_train) #+ neuralmodel.log_vars[4]
            loss_cpl = precision_cpl * criterion(y_cpl_pred_train,y_cpl_train)  #+ neuralmodel.log_vars[5]
        ...
        with torch.set_grad_enabled(False):
            ...
            loss_ri_v = precision_ri * criterion(y_ri_pred_valid,ds.y_ri_valid.to(device)) #+ neuralmodel.log_vars[4]
            loss_cpl_v = precision_cpl * criterion(y_clp_pred_valid,ds.y_cpl_valid.to(device)) #+ neuralmodel.log_vars[5]

            loss_v = (loss_ag_v + loss_myega_v + loss_am_v + loss_cg_v + loss_tvf_v + 
                    loss_raman_v + loss_density_v + loss_entro_v + loss_ri_v + loss_cpl_v)
        ...
        if verbose == True:
            if (epoch % 100 == 0):
                print('\nTRAIN -- Raman: {:.2f}, d: {:.2f}, Sc: {:.2f}, ori: {:.2f}, visco: {:.2f}, cpl: {:.2f}'.format(
                loss_raman, loss_density, loss_entro,  loss_ri, loss_ag, loss_cpl
                ))
                print('VALID -- Raman: {:.2f}, d: {:.2f}, Sc: {:.2f}, ori: {:.2f}, visco: {:.2f}, cpl: {:.2f}\n'.format(
                loss_raman_v, loss_density_v, loss_entro_v,  loss_ri_v, loss_ag_v, loss_cpl_v
                ))
            if (epoch % 20 == 0):
    ...
    if verbose == True:
        ...
        print("--- Train ---")
        print('Raman: {:.2f}, d: {:.2f}, Sc: {:.2f}, ori: {:.2f}, visco: {:.2f}, cpl: {:.2f}'.format(
        loss_raman, loss_density, loss_entro,  loss_ri, loss_ag, loss_cpl
        ))
        print("--- Valid ---")
        print('Raman: {:.2f}, d: {:.2f}, Sc: {:.2f}, ori: {:.2f}, visco: {:.2f}, cpl: {:.2f}'.format(
        ...
        with torch.no_grad():
            if method == "raman_pred":
                ...
                if sampling == True:
                    ...
                    if method in frozenset(('ag', 'myega', 'am', 'cg', 'tvf','density_melt','cpl')):
                        for i in range(self.n_models):
                            for j in range(n_sample):
