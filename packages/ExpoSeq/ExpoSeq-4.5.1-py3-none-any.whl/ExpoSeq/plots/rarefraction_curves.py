...
class PrepareData:
    ...
    def cleaning_data(sequencing_report, samples, region_of_interest, fraction_column):
        ...
        sub_table = (
            sub_table.groupby("Experiment")[
                [region_of_interest, fraction_column, "readCount"]
            ]
            .agg(apply_fun)
            .reset_index()
        )
    ...
    def tidy(
        ...
        samples,
        region_of_interest,
        fraction_column="cloneFraction",
    ):
        region_of_interest = region_of_interest.replace("aaSeq", "nSeq")
        sub_table = self.cleaning_data(
            sequencing_report, samples, region_of_interest, fraction_column
        )
        x_axis = []
        y_axis = []
        sample_names = []
        for sample in range(len(sub_table)):
            ...
            while counter < 100:
                temp = random.choices(
                    sequences, weights=fraction, k=int(mean_count)
                )  # randomly sample 100th of the sequences with the recalculated clonefractions as probablities
                temp_list.extend(
                    temp
                )  ## extend the temp_list with the samples from each 'pull-out' of the 100 'pull-outs'
                unique = Counter(
                    temp_list
                ).keys()  ## count unique seequences from the increasing temp_list
                unique_list.append(
                    unique
                )  ## for each run append the number of unique sequences to a new cell in this list - will be accumulative
                total_list.append(
                    len(temp_list)
                )  ## append the total sequences amount for plotting
                counter += 1
        ...
        results_plot = pd.DataFrame(
            {"samples": sample_names, "x_axis": x_axis, "y_axis": y_axis}
        )
        return results_plot
