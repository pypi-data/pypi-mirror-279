...
P4_TRUST = "53:1B:D8:ED:15:D9:CB:1A:C9:20:CD:BC:BC:FA:EB:5A:CC:22:85:FD"
    ...
    try:
        ...
        if check_version:
            ...
            if (v != my_version):
                ...
                if not is_admin:
                    secho(f"Upgrade suggested, run any unk command again in an administrator prompt or run this manually: {upgrade_cmd}.", fg="yellow")
                    return
    ...
    try:
        ...
        for ssl_connection in ssl_connection_list:
            ...
            matching_trust_line = next(
                (t for t in trust_line_list if t.startswith(p4_port) and t.endswith(P4_TRUST)),
                None
            )
            if matching_trust_line:
                # Connection fingerprint is up to date; no action needed.
                continue
