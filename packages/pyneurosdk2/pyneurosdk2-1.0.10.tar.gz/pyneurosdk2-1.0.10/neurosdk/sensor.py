...
class Sensor:
    def __init__(self, ptr):
        ...
        if self.is_supported_parameter(SensorParameter.BattPower):
    ...
    def sampling_frequency(self) -> SensorSamplingFrequency:
        if self.is_supported_parameter(SensorParameter.SamplingFrequency):
            status = OpStatus()
    ...
    def data_offset(self) -> SensorDataOffset:
        if self.is_supported_parameter(SensorParameter.Offset):
            status = OpStatus()
            cp = POINTER(c_uint8)
    ...
    def channels_count(self) -> int:
        return _neuro_lib.getChannelsCountSensor(self.sensor_ptr)
