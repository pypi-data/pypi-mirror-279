                ...
                case "multimodal":
                    ...
                    for item in self._conf["items"]:
                        if item.get("name") in data:
                            ...
                            if err: 
                                return res, "", ValueError(f"Key {item_name}: {str(err)}")
                            match item["type"]:
                                case "file", "image", "binary", "audio", "video":
                                    rf = RequestField(name=item_name, data=res, filename=f"{item_name}.bin", headers={'Content-Type': ct})
                                    form_data[item_name] = rf
                                case _:
