...
class IAPListener():
    ...
    async def send_reply(self, out, uid, source, data, error=""):
        if error is None:
            error = ""
        breply = "".encode()
        contentType = ""
                ...
                match source:
                    case "object_store":
                        ...
                        if err:
                            ...
                            await self.data_store.put(uid, bdata)
                    case _:
    ...
    async def _process_data(self, name: str,
        ...
        for uid, src, raw, ctype, params in zip(uids, sources, raw_datas, content_types, reqs_parameters):
            data = DataHandler(raw, ctype, params, dh_conf, uid, src)
            if data.error:
                task = asyncio.create_task(self.send_reply(self.error_queue, uid, src,str(data.error), "Wrong input"))
                tasks.append(task)
            else:
                p_datas.append(data)
                ...
                if is_batch:
                    ...
                    if error:
                        for uid, source in zip(p_uids, p_sources):
                            task = asyncio.create_task(self.send_reply(queue_out, uid, source, None, error))
                            tasks.append(task)
                    else:
                        for uid, source, res in zip(p_uids, p_sources, result):
                            task = asyncio.create_task(self.send_reply(queue_out, uid, source, res))
                            tasks.append(task)
                    ...
                    task = asyncio.create_task(self.send_reply(
                        queue_out, p_uids[0], p_sources[0], result, error=error))
                    tasks.append(task)
