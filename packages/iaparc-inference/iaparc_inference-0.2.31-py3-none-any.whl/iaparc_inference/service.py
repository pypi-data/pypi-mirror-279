...
class IAPListener():
    ...
    async def _run_async(self):
        ...
        for q_name in self.inputs_name:
            ...
            sub_in = await self.js.subscribe(queue_in+".>",
                                             queue=self.queue+"-"+item["link"],
                                             stream=self.queue)
            self._subs_in[item["link"]] = sub_in
    ...
    async def handle_msg(self, subject, name, msgs, is_batch: bool):
        if is_batch:
            batch, uids, sources, params_lst, content_types = zip(*[await self.get_data(subject, msg) for msg in msgs])
            batch = list(batch)
            ...
            try:
                ...
                if self.batch == 1 or pending_msgs == 0:
                    ...
                    await asyncio.gather(
                        self.handle_msg(sub_in.subject, name, [msg], False),
                        self.term_msg([msg])
                    )
                else:
                    ...
                    await asyncio.gather(
                        self.handle_msg(sub_in.subject, name, msgs, True),
                        self.term_msg(msgs)
                    )
