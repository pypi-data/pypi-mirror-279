...
class DataHandler():
    ...
    def __init__(self, data: bytes, content_type: str, parameters: dict, conf: dict, uid: str, source: str):
        """
        Constructor
        Arguments:
        ...
        if self._is_multi:
            if not self.items:
                ...
                for item in self._conf["items"]:
                    ...
                    if item_data:
                        dh = DataHandler(item_data, "", {}, item, "", "")
                        self.items[item["name"]] = dh.get()
            return self.items
        else:
            if not self.items:
                match self._conf["type"]:
                    case "file" | "binary" | "audio" | "video":
                        self.items[self.name] = self.file
                    case "text" | "string":
                        self.items[self.name] = self.text
                    case "image":
                        self.items[self.name] = self.image
                    case "json" | "matrix":
                        self.items[self.name] = self.json
                    case _:
                        self.items[self.name] = self._raw
                self.items[self.name] = self
            return self.items
    
    def get(self, item: str|None = None):
        if not item:
            if len(self._conf["items"]) > 1:
                return self.items
            else:
                return self.items[self._conf["items"][0]["name"]]
