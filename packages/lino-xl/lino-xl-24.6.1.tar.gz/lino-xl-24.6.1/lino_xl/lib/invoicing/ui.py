...
class Tasks(dd.Table):
    ...
    required_roles = dd.login_required(LedgerStaff)
    column_names = 'seqno user target_journal invoice_generators log_level disabled weekdays_text status *'
...
class MyPlan(Plans):
    ...
    help_text = _("Open your invoicing plan in order to generate invoices or service reports.")
    required_roles = dd.login_required(LedgerUser)
    default_record_id = 'row'

    @classmethod
    def get_row_by_pk(cls, ar, pk):
        return rt.models.invoicing.Plan.objects.get(user=ar.get_user())
