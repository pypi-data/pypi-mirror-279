...
class PopulateMembers(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        ar.success(_("Added %d children.") % n, refresh_all=True)
...
class Household(contacts.Partner):
    ...
    def after_ui_save(self, ar, cw):
        ...
        self.populate_members.run_from_ui(ar)

    def add_member(self, person, role=None):
        mbr = rt.models.households.Member(household=self,
                                          person=person,
                                          role=role)
        mbr.full_clean()
        mbr.save()
...
person_fields = dd.fields_list(Member,
...
class CreateHousehold(dd.Action):
    ...
    parameters = dict(head=dd.ForeignKey(config.person_model,
                                         verbose_name=_("Head of household")),
                      partner=dd.ForeignKey(config.person_model,
...
dd.inject_action(config.person_model, create_household=CreateHousehold())
