...
from lino_xl.lib.accounting.utils import ZERO, ONE, myround
...
class VatVoucher(VatDocument, PaymentRelated, Payable):
...
QtyVatItemBase.set_widget_options('unit_price', hide_sum=True)
...
class VatDeclaration(accounting.Declaration):
        ...
        def annotator():
            ...
            def build_case(model):
                ...
                def get_filter():
                    ...
                    filter_kw.update(self.get_period_filter(""))
                    return filter_kw
            ...
            if dd.is_installed('trading'):
                kw['vpi_base'] = build_case(rt.models.trading.VatProductInvoice)
            else:
                kw['vpi_base'] = Cast(Value(0.0),
                                      output_field=models.DecimalField())
            return kw
    ...
    def unused_intracom_statement_iterator(self):
        # AccountingPeriod = rt.models.accounting.AccountingPeriod
        intracom_regimes = set([
            r for r in VatRegimes.get_list_items() if r.vat_area == VatAreas.eu
        ])
        # ap = AccountingPeriod.objects.get(ref="2014-02")
        qs = rt.models.contacts.Partner.objects.all()
        # qs = qs.filter(vat_id__isnull=False)
        # qs = qs.exclude(vat_id__startswith="BE")
        qs = qs.order_by('vat_id').distinct()
        if False:
            qs = qs.annotate(
                total_base=Sum('accounting_movement_set_by_partner__amount'))
            qs = qs.filter(
                accounting_movement_set_by_partner__journal__trade_type=TradeTypes.
                sales,
                # accounting_movement_set_by_partner__accounting_period=ap,
                accounting_movement_set_by_partner__vat_regime__in=intracom_regimes,
                total_base__gte=0,
                **self.get_period_filter('accounting_movement_set_by_partner__'))
            return qs
        else:
            if True:
                qs = qs.annotate(vataccountinvoice_base=Sum(
                    'vat_vataccountinvoice_set_by_partner__total_base',
                    filter=Q(
                        vat_vataccountinvoice_set_by_partner__journal__trade_type
                        =TradeTypes.sales,
                        # vat_vataccountinvoice_set_by_partner__accounting_period=ap,
                        vat_vataccountinvoice_set_by_partner__vat_regime__in=
                        intracom_regimes,
                        **self.get_period_filter(
                            'vat_vataccountinvoice_set_by_partner__')))
                                 )  #.filter(vataccountinvoice_base__gte=0)
            ...
            #     **self.get_period_filter('vat_vataccountinvoice_set_by_partner__'))

            if dd.is_installed("trading") and True:
                qs = qs.annotate(vatproductinvoice_base=Sum(
                    'sales_vatproductinvoice_set_by_partner__total_base',
                    filter=Q(
                        sales_vatproductinvoice_set_by_partner__journal__trade_type
                        =TradeTypes.sales,
                        # sales_vatproductinvoice_set_by_partner__accounting_period=ap,
                        sales_vatproductinvoice_set_by_partner__vat_regime__in=
                        intracom_regimes,
                        **self.get_period_filter(
                            'sales_vatproductinvoice_set_by_partner__')))
                                 )  #.filter(vatproductinvoice_base__gte=0)
                # qs = qs.annotate(total_base=Sum(F('vatproductinvoice_base'), F('vataccountinvoice_base')))
            # else:
            #     qs = qs.filter(flt1)
            # qs = qs.annotate(total_base=F('vataccountinvoice_base'))
            # print("20210903", str(qs.query).replace('"', ''))
            # from pprint import pprint
            # print(qs.query)
            # qs = qs.order_by('vat_id').distinct()
            qs.annotate(total_base=F('vataccountinvoice_base') +
                        F('vatproductinvoice_base')).filter(total_base__gte=0)
            return qs
            # for p in qs:
            #     # p.total_base = 0
            #     # if True:
            #     #     p.total_base += p.vataccountinvoice_base or ZERO
            #     # if dd.is_installed("trading") and True:
            #     #     p.total_base += p.vatproductinvoice_base
            #     yield p
