...
class BiographyOwner(dd.Model):
    ...
    def get_language_knowledge(self, *buttons):
        ...
        if len(self._mother_tongues):
            lst.append("{}: {}".format(_("Mother tongues"),
                                       self.mother_tongues))
        lst += buttons
...
class SectorFunction(dd.Model):
    ...
    sector = dd.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="%(app_label)s_%(class)s_set_by_sector")
    function = dd.ForeignKey(
        "cv.Function",
        blank=True,
        null=True,
        related_name="%(app_label)s_%(class)s_set_by_function")
...
class PersonHistoryEntry(DateRange):
    ...
    duration_text = models.CharField(_("Duration"), max_length=200, blank=True)
