...
from .mixins import JournalRef, PeriodRangeObservable, Payable
...
class JournalDetail(dd.DetailLayout):
    ...
    account partner build_method:20 template:20 uploads_volume #invoicing_area
    dc make_ledger_movements make_storage_movements
...
class JournalsOverview(Journals):
    ...
    def this_year(self, obj, ar):
        ...
        if issubclass(tbl.model, PeriodRangeObservable):
            pv.update(start_period=AccountingPeriod.get_default_for_date(
                dd.today().replace(month=1, day=1)))
            pv.update(end_period=AccountingPeriod.get_default_for_date(
    ...
    def this_month(self, obj, ar):
        ...
        if issubclass(tbl.model, PeriodRangeObservable):
            pv.update(
                start_period=AccountingPeriod.get_default_for_date(dd.today()))
        return tbl.request(master_instance=obj, param_values=pv)
...
class ByJournal(dd.Table):
    ...
    def create_journal(cls, trade_type=None, account=None, **kw):
        ...
        return rt.models.accounting.Journal(trade_type=trade_type,
                                        voucher_type=vt,
                                        **kw)
...
class AccountingPeriods(dd.Table):
    required_roles = dd.login_required(LedgerStaff)
    model = 'accounting.AccountingPeriod'
...
class Vouchers(dd.Table):
    ...
    parameters = dict(year=dd.ForeignKey('accounting.FiscalYear', blank=True),
...
class VouchersByPartnerBase(Vouchers):
    ...
    def get_table_summary(self, obj, ar):
        ...
        if not ar.get_user().user_type.readonly:
            ...
            elems += [E.br(), str(_("Create voucher in journal")), " "
                      ] + actions
        return E.div(*elems)
...
class ExpectedMovements(dd.VirtualTable):
    ...
    parameters = dd.ParameterPanel(
        ...
        project=dd.ForeignKey(dd.plugins.accounting.project_model, blank=True),
...
class PartnerVouchers(Vouchers):
    ...
    parameters = dict(project=dd.ForeignKey(dd.plugins.accounting.project_model,
                                            blank=True,
                                            null=True),
    ...
    def get_simple_parameters(cls):
        for s in super().get_simple_parameters():
            yield s
        yield 'partner'
...
class AccountBalances(dd.Table):
    ...
    def get_request_queryset(self, ar, **kwargs):
        ...
        # see https://docs.djangoproject.com/en/5.0/ref/models/expressions/#using-aggregates-within-a-subquery-expression
        AccountingPeriod = rt.models.accounting.AccountingPeriod
        if (pv := ar.param_values) is None: return qs
...
class Movements(dd.Table):
    ...
    parameters = mixins.ObservedDateRange(
        year=dd.ForeignKey('accounting.FiscalYear', blank=True),
        journal_group=JournalGroups.field(blank=True),
        partner=dd.ForeignKey('contacts.Partner', blank=True, null=True),
        project=dd.ForeignKey(dd.plugins.accounting.project_model,
                              blank=True,
                              null=True),
...
class MovementsByPartner(Movements):
    ...
    def get_table_summary(cls, mi, ar):
        elems = []
        sar = ar.spawn(rt.models.accounting.MovementsByPartner,
                       master_instance=mi,
                       param_values=dict(cleared=dd.YesNo.no))
        bal = ZERO
        for mvt in sar:
            bal += mvt.amount
        txt = _("{0} open movements ({1} {2})").format(
            sar.get_total_count(), bal, dd.plugins.accounting.currency_symbol)
