...
from .mixins import ProjectRelated, VoucherNumber, PeriodRange, PeriodRangeObservable, PaymentRelated, Payable, MovementBase, LedgerRegistrable
...
class Journal(BabelNamed, Sequenced, Referrable, PrintableType):
    ...
    def get_next_number(self, voucher):
        ...
        if self.yearly_numbering:
            qs = qs.filter(
                accounting_period__year=voucher.accounting_period.year)
        if voucher.pk:
...
class AccountingPeriod(DateRange, Referrable):
    ...
    year = dd.ForeignKey('accounting.FiscalYear', blank=True, null=True)
    ...
    def get_available_periods(cls, entry_date):
        ...
        return rt.models.accounting.AccountingPeriod.objects.filter(**fkw)
    ...
    def get_ref_for_date(cls, d):
        ...
        # if dd.plugins.accounting.fix_y2k:
        #     return rt.models.accounting.FiscalYear.from_int(d.year).ref \
        #         + "{:0>2}".format(d.month)
    ...
    def get_default_for_date(cls, d):
        ...
        obj = rt.models.accounting.AccountingPeriod.get_by_ref(ref, None)
        if obj is None:
...
class Voucher(UserAuthored, Duplicable, UploadController,
    ...
    accounting_period = dd.ForeignKey('accounting.AccountingPeriod', blank=True)
    ...
    def on_create(self, ar):
        ...
        if self.entry_date is None:
            if ar is None:
                ...
                if multi_ledger:
                    self.entry_date = info.entry_date if (info := ar.get_user().ledger) is not None else dd.today()
                else:
                    self.entry_date = LedgerInfo.get_for_user(ar.get_user()).entry_date or dd.today()
    ...
    def todo_get_detail_action(self, ar):
    ...
    def unused_accounting_period_choices(cls, entry_date):
        ...
        return rt.models.accounting.AccountingPeriod.get_available_periods(
            entry_date)
    ...
    def create_journal(cls, trade_type=None, account=None, **kw):
        if 'voucher_type' not in kw:
            kw.update(voucher_type=VoucherTypes.get_for_model(cls))
        ...
        return rt.models.accounting.Journal(trade_type=trade_type, **kw)
    ...
    def as_summary_item(self, ar, *args, **kwargs):
        ...
        if ar is None:
            return str(self)
        return ar.obj2html(self.get_mti_leaf(), *args, **kwargs)
...
class Declaration(Voucher, PaymentRelated, Payable, Certifiable, PeriodRange):
    ...
    def full_clean(self, *args, **kw):
        if self.entry_date:
            AP = rt.models.accounting.AccountingPeriod
            # declare the previous month by default
            if not self.start_period_id:
                self.start_period = AP.get_default_for_date(self.entry_date)
...
class Movement(ProjectRelated, MovementBase):
    ...
    def match_link(self, ar):
        if ar is None or not self.match:
            return ''
        sar = rt.models.accounting.MovementsByMatch.request(
