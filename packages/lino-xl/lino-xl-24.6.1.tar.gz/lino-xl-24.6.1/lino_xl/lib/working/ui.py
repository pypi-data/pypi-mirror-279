...
from lino.utils.html import E, join_elems, forcetext
...
from lino_xl.lib.calview.ui import Day, ParameterClone, DayNavigator, DaysTable
...
class Sessions(dd.Table):
    ...
    parameters = ObservedDateRange(
        ...
        site=dd.ForeignKey('tickets.Site', null=True, blank=True),
...
class MySessionsByDate(MySessions):
    ...
    column_names = (
        ...
        'workflow_buttons:10 site_ref:10 *')
...
class TicketsByReport(Tickets, DurationReport):
    ...
    def get_request_queryset(self, ar):
        ...
        qs = super().get_request_queryset(ar)
        for obj in qs:
            sar = SessionsByTicket.request(master_instance=obj,
                                           param_values=spv)
...
class SitesByReport(Sites, DurationReport):
    ...
    def get_request_queryset(self, ar):
        ...
        for obj in qs:
            sar = SessionsBySite.request(master_instance=obj, param_values=spv)
            load_sessions(obj, sar)
            # obj._invested_time = compute_invested_time(
            #     obj, start_date=mi.start_date, end_date=mi.end_date,
            #     user=mi.user)
