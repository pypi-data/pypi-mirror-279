...
from .choicelists import TicketEvents, TicketStates, Priorities, SiteStates
...
class RefTickets(Tickets):
    ...
    column_names = 'ref:20 summary:50 workflow_buttons:30 id user:10 site:10 *'
    order_by = ["ref"]
...
class TicketsByEndUser(Tickets):
    master_key = 'end_user'
    column_names = ("detail_link:50 workflow_buttons * ")
    # display_mode = ((None, constants.DISPLAY_MODE_SUMMARY),)
    required_roles = dd.login_required((TicketsReader, Triager))
    details_of_master_template = _("%(details)s of end user %(master)s")
    params_layout = """
    user assigned_to not_assigned_to interesting_for site state deployed_to
    ...
    def get_table_summary(self, obj, ar):
        ...
        chunks = []
        items = [o.as_summary_item(ar) for o in sar]
        if len(items) > 0:
...
class PublicTickets(Tickets):
    ...
    order_by = ["-id"]
    column_names = 'detail_link:50 ticket_type:10 site:10 *'
...
class ActiveTickets(Tickets):
    ...
    column_names = 'detail_link:50 priority site ' \
