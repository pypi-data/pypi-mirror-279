...
class Loader:
    ...
    def _index_and_cache(self, type_: Type[T]) -> int:
        """
        Same as index() but the result is added to the cache
        """
        r = self.index(type_)
        self._indexcache[type_] = self.handlers[r][1]
        return r
...
def _dictload(l: Loader, value: Any, type_) -> Dict:
    ...
    try:
        value_f = l._indexcache[value_type]
    except KeyError:
        try:
...
def _tupleload(l: Loader, value: Any, type_) -> Tuple:
    ...
    try:
        return tuple(v if t in l.basictypes and type(v) == t else (l._indexcache.get(t) or l.handlers[l._index_and_cache(t)][1])(l, v, t)
            for v, t in zip(
                compress(value, ctr),
                args
            )
...
def _dataclassload(l: Loader, value: Dict[str, Any], type_) -> Any:
    ...
    return _objloader(l, fields, necessary_fields, type_hints, value, type_, False)
...
def _objloader(l: Loader, fields: Set[str], necessary_fields: Set[str], type_hints, value: Any, type_, early_needed_fields_check: bool) -> Any:
    ...
    if l.dictequivalence and not isinstance(value, dict):
        newvalue = _dictequivalence(l, value)

        if newvalue is value:
            raise TypedloadAttributeError('Not a dictionary: %s' % value, value=value, type_=type_)
        else:
            value = newvalue

    if early_needed_fields_check or l.failonextra:
        vfields = set(value.keys())

    if early_needed_fields_check and len(necessary_fields.intersection(vfields)) != len(necessary_fields):
        raise TypedloadTypeError(
    ...
    for k, v in value.items():
        ...
        try:
            loader_f = l._indexcache[field_type]
        except KeyError:
            try:
    ...
    try:
        ...
        vfields = set(value.keys())
        # Check the necessary fields only if there was an error loading
...
def _typeddictload(l: Loader, value: Any, type_) -> Any:
    ...
    return _objloader(l, fields, necessary_fields, type_hints, value, type_, True)
...
def _unionload(l: Loader, value: Any, type_) -> Any:
    ...
    try:
        ...
        l._union_sorted_args_cache[type_id] = sorted_args
    ...
    if hasattr(value, 'get'):
        ...
        # First time generate the deep inspection for literal
            ...
            l._unionload_discriminatorcache[type_id] = discriminatorscache
...
def _attrload(l: Loader, value: Any, type_) -> Any:
    ...
    try:
        ...
        for attribute in type_.__attrs_attrs__:
            if type(attribute.default) is NOTHING and attribute.init:
...
def _iterload(l: Loader, value: Any, type_, function) -> Any:
    ...
    try:
        ...
        if t in l.basictypes:
            if function is list:
                # Copy paste but a list comprehension directly instead of a generic one
                return [i if isinstance(i, t) else f(l, i, t) for i in compress(value, ctr)]
            elif function is set:
                return {i if isinstance(i, t) else f(l, i, t) for i in compress(value, ctr)}
            ...
            if function is list:
                return [i if type(i) in types else f(l, i, t) for i in compress(value, ctr)]
            elif function is set:
                return {i if type(i) in types else f(l, i, t) for i in compress(value, ctr)}
