...
class Leaderboard(BaseModel, extra=Extra.allow):
    ...
    def to_display_df(self, includes=None, excludes=None, sort_params=None, header_mapping=None, filter_eligibility=True) -> pd.DataFrame:
    ...
    def to_html(self, includes=None, header_mapping=None, sort_params=None, filter_eligibility=True):
        includes = includes or DEFAULT_LEADERBOARD_INCLUDES
        sort_params = sort_params or {"by": "celo_rating", "ascending": False}
        df = self.to_display_df(
            includes=includes, header_mapping=header_mapping, sort_params=sort_params, filter_eligibility=filter_eligibility
        )
...
def _aggregate_max_columns(df):
    for column in ["celo_rating", "double_thumbs_up_ratio", "thumbs_down_ratio", "win_ratio", "safety_score", "model_score"]:
