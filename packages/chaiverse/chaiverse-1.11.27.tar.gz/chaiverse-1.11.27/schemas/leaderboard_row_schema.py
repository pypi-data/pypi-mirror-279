...
class BaseLeaderboardRow(BaseModel, extra=Extra.allow):
    ...
    def double_thumbs_up_ratio(self):
        return self.double_thumbs_up / self.feedback_count if self.feedback_count > 0 else None

    @property
    def thumbs_up_ratio(self):
        return (self.double_thumbs_up + self.thumbs_up) / self.feedback_count if self.feedback_count > 0 else None

    @property
    def single_thumbs_up_ratio(self):
        return self.thumbs_up / self.feedback_count if self.feedback_count > 0 else None

    @property
    def thumbs_down_ratio(self):
        return self.thumbs_down / self.feedback_count if self.feedback_count > 0 else None
