...
def downsample_posterior(posterior_list, num_samples=1000):
    ...
    num_samples : int
        Downsample the posterior to have a `num_sample` number of samples. Default is 1000.

    Returns
    -------

    m1 : array
        Downsampled mass 1 samples.
    m2 : array
        Downsampled mass 2 samples.
    chi1 : array
        Downsampled spin 1 samples.
    chi2 : array    
        Downsampled spin 2 samples.
    tilt1 : array
        Downsampled tilt 1 samples.
    tilt2 : array
        Downsampled tilt 2 samples.
    phi_12 : array
        Downsampled phi_12 samples.
    phi_jl : array
        Downsampled phi_jl samples.
    theta_jn : array
        Downsampled theta_jn samples.
    phase : array
        Downsampled phase samples.

    """

    num_total_samples = posterior_list['mass_1'].shape[0]
    random_indices    = np.random.choice(num_total_samples, size=num_samples, replace=False)
    random_indices    = np.sort(random_indices)

    m1          = posterior_list['mass_1'][random_indices]
    m2          = posterior_list['mass_2'][random_indices]
    chi1        = posterior_list[   'a_1'][random_indices]
    chi2        = posterior_list[   'a_2'][random_indices]
    tilt1       = posterior_list['tilt_1'][random_indices]
    tilt2       = posterior_list['tilt_2'][random_indices]
    phi_12      = posterior_list['phi_12'][random_indices]
    phi_jl      = posterior_list['phi_jl'][random_indices]
    theta_jn    = posterior_list['phi_jl'][random_indices]
    phase       = posterior_list[ 'phase'][random_indices]

    return m1, m2, chi1, chi2, tilt1, tilt2, phi_12, phi_jl, theta_jn, phase
    ...
    try:
        ...
        if('O4a' in file): 
            ...
            with h5py.File(file, 'r') as h5file: 
                ...
                except: downsample_N = -1
                
                print(f'\n* Downsampling the IMR posterior distribution to {downsample_N} samples.\n')

                if(downsample_N>0):
                    m1, m2, chi1, chi2, tilt1, tilt2, phi_12, phi_jl, theta_jn, phase = downsample_posterior(posterior_list, downsample_N)
                else:
                    ...
                    phi_12   = posterior_list['phi_12'][:]
                    phi_jl   = posterior_list['phi_jl'][:]
                    theta_jn = posterior_list['theta_jn'][:]
                    phase    = posterior_list['phase'][:]                

                try   : remnant_fits = str(Config.get("Plot",'imr-remnant-fits'))
                except: remnant_fits = 'UIB'
                ...
                if remnant_fits== 'UIB':
                ...
                elif remnant_fits == 'NRSUR':

                    if any(var is None for var in [m1, m2, chi1, chi2, tilt1, tilt2]):
                        raise ValueError("Some data arrays are missing or contain None values.")
                    ...
                    raise ValueError(f"* Remnant fits {remnant_fits} not recognized. Please choose a valid option among ['UIB', 'NRSUR'].")
            ...
            with h5py.File(file, 'r') as f:
                ...
                dataset     = f[f"{group_name}/posterior_samples"]
                params_list = ['mass_1', 'mass_2', 'a_1', 'a_2', 'tilt_1', 'tilt_2', 'azimuth', 'cosalpha', 'theta_jn', 'phase']
                
                for param in params_list: locals()[param] = dataset[param][:]
        ...
        try:
            ...
            print(f'* Skipping Mf-af reading due to the error: {e}')
