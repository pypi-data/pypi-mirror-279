...
from ..scheduler_core.status_center import FedMLStatusCenter, JobStatus
...
class FedMLServerRunner(FedMLMessageCenter, FedMLStatusCenter):
    ...
    def run_server_job(
        ...
        try:
            ...
            self.run_server_job_impl(process_event, completed_event,
                                     message_center_queue=message_center_queue)
        except RunnerError:
            logging.info("Runner stopped.")
            self.status_reporter.report_server_id_status(
                self.run_id, ServerConstants.MSG_MLOPS_SERVER_STATUS_KILLED, edge_id=self.edge_id,
    ...
    def callback_start_train(self, topic=None, payload=None):
        ...
        run_id = request_json["runId"]
        run_id_str = str(run_id)
        print(f"callback_start_train, run_id: {run_id_str}")
    ...
    def start_status_listener_center(self):
        self.start_status_center(message_center_queue=self.message_center.get_message_queue())
    ...
    def rebuild_status_center(self, status_center_queue):
        ...
        self.status_queue = status_center_queue
    ...
    def run_stop(self, process_event, edge_id_status_queue, edge_device_info_queue,
                 edge_device_info_global_queue, message_center_queue, status_center_queue):
        if platform.system() != "Windows":
            os.setsid()
    ...
    def get_message_runner(self):
        ...
        self.message_status_runner = FedMLServerRunner(
            self.args, run_id=self.run_id, request_json=self.request_json,
            agent_config=self.agent_config
        )
        self.message_status_runner.run_as_edge_server_and_agent = self.run_as_edge_server_and_agent
        self.message_status_runner.edge_id = self.edge_id
        self.message_status_runner.server_agent_id = self.server_agent_id
        self.message_status_runner.start_request_json = self.start_request_json
        self.message_status_runner.unique_device_id = self.unique_device_id
        self.message_status_runner.user_name = self.user_name
        self.message_status_runner.run_as_edge_server_and_agent = self.run_as_edge_server_and_agent
        self.message_status_runner.run_as_cloud_agent = self.run_as_cloud_agent
        self.message_status_runner.run_as_cloud_server = self.run_as_cloud_server
    ...
    def get_status_runner(self):
        if self.message_status_runner is None:
            ...
            if self.message_status_runner is not None:
                self.message_status_runner.message_queue = self.message_center.get_sender_message_queue()
