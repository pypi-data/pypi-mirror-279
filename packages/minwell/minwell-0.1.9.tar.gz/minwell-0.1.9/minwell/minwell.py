...
def sistema_ident_1():
    return('''
    df = df.set_index(['i','t']) #i территориальный, t отвечает за временной момент
    y = df['Y']
    X = df.drop(['Y'], axis=1)
    ...
    pool_resid = pool.resids
    bp_test = het_breuschpagan(pool_resid, X) #так не работает))))!!!!!!!!!!!!!!!!!!!!!!
    print('pooled:', bp_test)
    ''')
def sistema_ident_2():
    return('''
...
d = {
        ...
        'сур' : sur(),
        'sur' : sur(),
        'идентифицируемость, приведенная форма, правила ранга и порядка' : sistema_ident_1()
