...
def Foster_Stuart_test():
  ...
  def Foster_Stuart_test(X, p_level=0.95):
    ...
    # здесь определяются две последовательности для сравнения каждого уровня ряда с предыдущим: u и l
    # смысл: для u - 1, если yi больше всех предыдущих, иначе 0
    # смысл: для l - 1, если yi меньше, чем все предыдущие
    u = l = list()
    Xtemp = np.array(X[0])
    for i in range(1, n):
        Xmax = np.max(Xtemp)
    ...
    # рассчитываются велечины, характеризующие изменение временного ряда и дисперсии
                
    d = np.int64(np.sum(u - l))
    S = np.int64(np.sum(u + l))
        
    # статистика для расчета t статистики
    mean_d = 0
...
d = {
        ...
        'Foster_Stuart_test' : Foster_Stuart_test(),
        'Метод Фостера Стьюарта' : Foster_Stuart_test(),
        'Взвешенной (средневзвешенной) скользящей средней' : weighted_moving_average()
