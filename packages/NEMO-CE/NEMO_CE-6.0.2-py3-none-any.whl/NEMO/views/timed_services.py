...
def get_granted_badge_reader_access(
    qualification_since: datetime.date, today_date: datetime.date
) -> Dict[str, Set[User]]:
    new_qualifications: QuerySetType[Qualification] = Qualification.objects.filter(
        ...
        qualified_on__gte=qualification_since,
        qualified_on__lt=today_date,
...
def get_granted_physical_access(qualification_since: date, today_date: date) -> Dict[str, Set[User]]:
    ...
    qualification_since_datetime = localize(datetime.combine(qualification_since, datetime.min.time()))
    today_datetime = localize(datetime.combine(today_date, datetime.min.time()))
    physical_access_level_type = ContentType.objects.get_for_model(PhysicalAccessLevel)
    user_type = ContentType.objects.get_for_model(User)
    memberships: QuerySetType[MembershipHistory] = MembershipHistory.objects.filter(
        parent_content_type=physical_access_level_type,
        child_content_type=user_type,
        action=MembershipHistory.Action.ADDED,
        date__gte=qualification_since_datetime,
        date__lt=today_datetime,
    )
