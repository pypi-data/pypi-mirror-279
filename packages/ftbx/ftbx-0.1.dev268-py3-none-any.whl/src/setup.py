...
def setup_command_func(args):
    ...
    with open(os.path.join(os.path.expanduser('~'), '.ftbx', 'variables'), 'r') as variables_file:
        flex_version_to_sdk_map = yaml.safe_load(variables_file)

        sdk = flex_version_to_sdk_map.get('flex_version_to_sdk_version').get(version_x)
        if not sdk: raise KeyError(
            f"Cannot find {version} nor {version_x} in the variables. Please check the information provided. ")
