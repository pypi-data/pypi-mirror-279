...
from ftbx.utils import get_full_items, create_folder
...
def list_command_func(args):
    ...
    filename = list_items(config_item=args.config_item, filters=args.filters, post_filters=args.post_filters, environment=args.from_, filename=args.name)

    return filename


def list_items(config_item: str, filters: List[str] = [], post_filters: List[str] = [],
               environment: str = "default", filename: str = None) -> str:
    """
    List items ids.
