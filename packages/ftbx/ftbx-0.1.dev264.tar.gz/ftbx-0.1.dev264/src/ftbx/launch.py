...
from ftbx.utils import get_items, launch_config_item_instance, push_item, listen
...
def launch_command_func(args):
    ...
    print(f"Instance ID {instance.get('id')} [{args.config_item[:-1]}:{instance.get('name')}] has been "
          f"launched successfully.\n ")

    # listen
    if args.listen:
        listen(config_item=args.config_item, item_name=instance.get('id'), environment=args.in_)
