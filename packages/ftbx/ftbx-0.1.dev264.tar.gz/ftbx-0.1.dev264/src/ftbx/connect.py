...
from ftbx.env import read_environments_json, add_or_update_environments_json
...
def connect(url_or_alias: str, username: str = None, password: str = None, alias: str = None, log: bool = True):
    ...
    # if env already exists in environments.json and username and password are not provided
    if existing_env and not username and not password:
        password = decrypt_pwd(existing_env.get('password'))
    # if username is provided but not password, most likely user wants to set/change password
