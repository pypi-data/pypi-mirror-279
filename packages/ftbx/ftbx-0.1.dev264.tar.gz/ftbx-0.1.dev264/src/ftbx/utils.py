...
from ftbx.encryption import decrypt_pwd
from ftbx.env import get_env, get_default_env_alias
...
def enumerate_sub_items(config_item: str):
    ...
    if config_item == 'accounts':
        return get_variable('sub_items.accounts')
    elif config_item == 'actions':
        return get_variable('sub_items.actions')
    elif config_item == 'assets':
        return get_variable('sub_items.assets')
    elif config_item == 'collections':
        return get_variable('sub_items.collections')
    elif config_item == 'eventHandlers':
        return get_variable('sub_items.event_handlers')
    elif config_item == 'events':
        return get_variable('sub_items.events')
    elif config_item == 'groups':
        return get_variable('sub_items.groups')
    elif config_item == 'jobs':
        return get_variable('sub_items.jobs')
    elif config_item == 'messageTemplates':
        return get_variable('sub_items.message_templates')
    elif config_item == 'metadataDefinitions':
        return get_variable('sub_items.metadata_definitions')
    elif config_item == 'objectTypes':
        return get_variable('sub_items.object_types')
    elif config_item == 'profiles':
        return get_variable('sub_items.profiles')
    elif config_item == 'quotas':
        return get_variable('sub_items.quotas')
    elif config_item == 'resources':
        return get_variable('sub_items.resources')
    elif config_item == 'roles':
        return get_variable('sub_items.roles')
    elif config_item == 'tagCollections':
        return get_variable('sub_items.tag_collections')
    elif config_item == 'taskDefinitions':
        return get_variable('sub_items.task_definitions')
    elif config_item == 'tasks':
        return get_variable('sub_items.tasks')
    elif config_item == 'timedActions':
        return get_variable('sub_items.timed_actions')
    elif config_item == 'userDefinedObjectTypes':
        return get_variable('sub_items.user_defined_object_types')
    elif config_item == 'users':
        return get_variable('sub_items.users')
    elif config_item == 'variants':
        return get_variable('sub_items.variants')
    elif config_item == 'wizards':
        return get_variable('sub_items.wizards')
    elif config_item == 'workflowDefinitions':
        return get_variable('sub_items.workflow_definitions')
    elif config_item == 'workflows':
        return get_variable('sub_items.workflows')
    elif config_item == 'workspaces':
        return get_variable('sub_items.workspaces')
...
def get_full_items(config_item, filters: List = [], post_filters: List = [], save: bool = False,
    ...
    if config_item == 'accounts':
        ...
        metadata_definitions = get_items(config_item="metadataDefinitions",
                                         sub_items=get_variable('sub_items.metadata_definitions'),
                                         environment=environment)
...
def save_items(config_item: str, items: dict, backup: bool = False, log: bool = True, environment: str = "default"):
    ...
    for item in items:
        ...
        if 'structure' in items.get(item) and items.get(item).get('structure'):
            if get_variable('render_workflow_graph'):
...
def get_variable(variable: str):
    """
    Retrieve a variable from the variables.yml file.

    TEST STATUS: DOES NOT REQUIRE TESTING

    :param variable: variable to get from variables.yml file separated by "."
    """

    if os.environ.get('FTBX'):
        with open(os.path.join(os.environ.get('FTBX'), 'variables.yml'), 'r') as variables_file:
