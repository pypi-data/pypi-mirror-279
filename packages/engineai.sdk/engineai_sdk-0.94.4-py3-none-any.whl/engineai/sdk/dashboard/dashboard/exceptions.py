...
class DashboardNoDashboardFoundError(EngineAIDashboardError):
    ...
    def __init__(
        self, slug: str, app_id: str, version: Optional[str], *args: object
    ) -> None:
        """Construct for DashboardAPINoDashboardFoundError class."""
        super().__init__(slug, app_id, version, *args)
        version = f" and {version=}" if version else ""
        self.error_strings.append(
            f"No Dashboard found with {slug=}{version} in your app with {app_id=}. "
            f"Please create the Dashboard first, before using skip_data flag."
        )
