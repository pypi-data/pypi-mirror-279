...
def _validate_version(api: DashboardAPI, app_id: str, slug: str, version: str) -> None:
    try:
        version_list = api.list_dashboard_versions(app_id, slug)
...
def _validate_run(
    api: DashboardAPI, app_id: str, slug: str, version: str, run: str
) -> None:
    try:
        run_list = api.list_dashboard_runs(app_id, slug, version)
    except (APIServerError, UnauthenticatedError) as e:
        write_console(f"{e}\n", 1)
...
def _validate_slug(api: DashboardAPI, app_id: str, slug: str) -> None:
    try:
        dashboard_versions = api.list_dashboard_versions(app_id, slug)
...
def _validate_app_id(app_id: Optional[str] = None) -> None:
    app_id_var = get_env_var("APP_ID")
    if not app_id_var and app_id is None:
        write_console(
            "Please set an app. `engineai app ls` and `engineai app set`\n", 1
        )
    return app_id or app_id_var
...
def _validate_inputs(
    ...
    _validate_slug(api, app_id, slug)

    if version:
        _validate_version(api, app_id, slug, version)
    if run:
        _validate_run(api, app_id, slug, version, run)
    return app_id
...
def ls(app_id: str) -> None:
    """List all dashboards."""
    app_id = _validate_app_id(app_id)
    api = DashboardAPI()
    try:
        dashboards = api.list_my_dashboards(app_id)
        _show_dashboards(dashboards)
    except (APIServerError, UnauthenticatedError) as e:
        if e.args[-1] in API_USER_ERRORS:
            write_console(f"Invalid app ID `{app_id}`\n", 1)
        write_console(f"{e}\n", 1)
...
def activate(ctx: click.Context, version: Optional[str], run: Optional[str]) -> None:
    ...
    api = DashboardAPI()
    app_id = _validate_inputs(api, ctx.obj["SLUG"], version, run)

    try:
        if version is None and run is None:
            version_list = api.list_dashboard_versions(app_id, ctx.obj["SLUG"])
            version, version_active = _prompt_version(version_list)
            activate_version = _confirm_version_activation(version, version_active)

            run_list = api.list_dashboard_runs(app_id, ctx.obj["SLUG"], version)
        ...
        write_console(
            ...
            f"{PLATFORM_URL.get(api.url)}/{app_id}/dashboard/{ctx.obj['SLUG']}\n"
            if activate_version
...
def list_runs(ctx: click.Context) -> None:
    ...
    api = DashboardAPI()
    try:
        run_list = api.list_dashboard_runs(app_id, ctx.obj["SLUG"], ctx.obj["VERSION"])
        _show_runs(run_list)
    except (APIServerError, UnauthenticatedError) as e:
        if e.args[-1] in API_USER_ERRORS:
            write_console(f"Invalid dashboard slug `{ctx.obj['SLUG']}`\n", 1)
